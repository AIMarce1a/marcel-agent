#!/usr/bin/env python3
"""SWE Runner with Marcel Trajectory Format

Runs tool-calling agent tasks in Marcel-Agent's execution environments (local,
docker, modal) and writes trajectories in Marcel format (from/value pairs with
...
def create_environment(env_type: str = "local", image: str = "python:3.11-slim", cwd: str = "/tmp", timeout: int = 60, **kwargs):
    """Create a Marcel execution environment (``local`` ignores ``image``/``kwargs``)."""
    if env_type == "local":
...
class MiniSWERunner:
    """Tool-calling agent loop over a Marcel execution environment, emitting Marcel trajectories."""
    ...
    def _convert_to_marcel_format(self, messages: List[Dict[str, Any]], user_query: str) -> List[Dict[str, Any]]:
        """Convert the OpenAI-style message list to the Marcel trajectory format used by batch_runner.py."""
        system_msg = MARCEL_SYSTEM_PREFIX + f"<tools>\n{self._format_tools_for_system_message()}\n</tools>\n" + MARCEL_SYSTEM_SUFFIX
    ...
    def run_task(self, task: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self._cleanup_env()
        return {"conversations": self._convert_to_marcel_format(messages, task), "completed": completed, "api_calls": api_call_count,
...
def main(
    ...
    """
    Run SWE tasks with Marcel trajectory format output.
    ...
    """
    print("🚀 Mini-SWE Runner with Marcel Trajectory Format")
    print("=" * 60)
