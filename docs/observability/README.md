# Marcel Observer Hooks

Marcel observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Marcel' planner, model providers, memory, tool registry,
...
Marcel also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "marcel.observer.v1"
```

Hook callbacks are fail-open. Marcel catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Marcel only builds
...
The native NeMo Relay SDK integration maps Marcel session, turn, LLM, and tool
