...
Marcel includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Marcel and does not require a Marcel observability plugin. Marcel remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
Marcel execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `marcel-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the Marcel `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `MARCEL_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `MARCEL_NEMO_RELAY_ATOF_*` and `MARCEL_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, Marcel does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Marcel requires NeMo Relay 0.8.3 or later within the 0.8 release line. That
line provides the provider-codec and canonical tool-result contracts Marcel
uses for managed provider and tool calls.

## Runtime Dependency and Data Boundary

Marcel installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Marcel process so configured interceptors can
...
Collection remains off unless Marcel policy enables it:
...
opt-in. Set `MARCEL_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, Marcel does not invoke Relay's plugin initializer, so Relay
...
selected file cannot be loaded, Marcel reports the error and does not invoke
...
retain the same `session_id` and add `marcel.session.segment` plus
`marcel.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a Marcel
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Marcel process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Marcel logs:
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Marcel
process and share the same policy boundary.

Run profiles in separate Marcel processes when they require different trust
...
Marcel core owns one Relay host and one isolated Relay session scope per Marcel
...
marks do not require Marcel plugin registration. Shared-metrics marks must
...
```text
Marcel turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Marcel shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Marcel sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Marcel used for the logical call, such as
...
New calls use `marcel.model_route.count`. The previous
`marcel.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `marcel.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Marcel
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Marcel processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `marcel.task_run`, parented to
the owning Marcel session. The start counter contains only bounded execution
...
provider attempts for the same Marcel API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Marcel tool-call
...
cancellations. Active task ownership follows the task ID if Marcel rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`marcel.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Marcel
...
tool or plugin names. Marcel does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `marcel.tool_approval` marks and
...
Successful skill mutations emit `marcel.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `marcel.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Marcel derives reuse and patch-generation
...
```text
$MARCEL_HOME/telemetry/shared_metrics/metrics.sqlite3
$MARCEL_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Marcel version,
...
schema field name, its current scope is one `MARCEL_HOME`, so it is more
...
Deleting `$MARCEL_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `MARCEL_HOME`. To reset it, stop Marcel
processes and remove `$MARCEL_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Marcel CLI turn against the deterministic local model server:
...
`$MARCEL_HOME/telemetry/shared_metrics/outbox/` to the Marcel telemetry ingest
...
Removing `$MARCEL_HOME/telemetry/shared_metrics` still resets local identity,
