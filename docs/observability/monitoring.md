...
Marcel gateway daemon, exported over OTLP/HTTP to an operator-configured
...
capture is a separate plane served by Marcel's native NeMo Relay SDK
...
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `marcel.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `marcel.platform.up/degraded` with bounded `error_code` attributes |
...
`marcel.gateway.active_agents`, `marcel.gateway.background_work`, and
`marcel.gateway.background_delegations` are complementary. `active_agents`
...
```bash
marcel monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'marcel-agent[otlp]'`),
...
a plain `marcel gateway run`: the exporter lives in the gateway process, so
...
`marcel.gateway.up`, `marcel.platform.up`, and `marcel.platform.degraded`.
...
absent_over_time(marcel_gateway_up[5m])

# Locally owned bridge is explicitly down.
marcel_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
marcel_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
marcel_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
marcel_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(marcel_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `marcel.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
marcel.status = failed|unknown
marcel.delivery_outcome = failed|not_configured
marcel.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `marcel.job_key`;
...
Marcel Agent-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/marcel_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/marcel_otel_capture.jsonl --wait 8
...
`marcel.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
marcel monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `marcel monitoring` CLI intentionally exposes `status` only. This first
release covers only Marcel Agent-owned service-health and operational-diagnostic
signals, including Marcel Agent-owned Relay transport health. Team Gateway's
