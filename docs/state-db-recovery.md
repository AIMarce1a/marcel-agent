...
`marcel doctor` reports the explicit repair command.
...
repaired or restored file. Do not run `marcel doctor --fix` while the gateway
is still up. Next steps:

```bash
marcel gateway stop
MARCEL_HOME="$HOME/.marcel" marcel sessions recover --source "$HOME/.marcel/state.db" --inspect-only
# if recoverable:
MARCEL_HOME="$HOME/.marcel" marcel sessions recover --source "$HOME/.marcel/state.db" --output "$HOME/recovered-state.db"
```
...
```bash
marcel gateway stop
MARCEL_HOME="$HOME/.marcel" marcel sessions repair --check-only
MARCEL_HOME="$HOME/.marcel" marcel sessions repair
```
