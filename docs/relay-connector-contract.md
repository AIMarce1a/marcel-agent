...
This document is the formal interface between the **Marcel gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `marcel gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`marcel gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `marcel` CLI.
