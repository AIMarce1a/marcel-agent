...
`marcel config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.marcel/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `MARCEL_HOME`, secret scope, and
