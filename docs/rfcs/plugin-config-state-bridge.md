...
or change arbitrary Marcel configuration.
...
small host fact requested by the RFC. Settings writes use Marcel'
...
`MARCEL_HOME`, so one globally loaded plugin context follows multiplexed
...
```text
<MARCEL_HOME>/plugin-data/<plugin-data-namespace>/state.json
```
...
- **Profile isolation:** the data root resolves from the active context-local
  Marcel home on every operation.
...
The implementation is covered with real temporary-Marcel-home tests for:
