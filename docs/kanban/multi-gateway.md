...
Marcel supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.marcel/config.yaml`:
...
Or set the env var: `MARCEL_KANBAN_DISPATCH_IN_GATEWAY=false`
