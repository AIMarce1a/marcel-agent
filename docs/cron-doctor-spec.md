...
a provider run fails, or delivery starts failing. `marcel cron list` shows some of
...
Add `marcel cron doctor` as a read-only diagnostic command that summarizes cron
...
- `marcel cron doctor` returns `0` and prints a healthy message when active jobs
...
- It returns `1` and prints grouped job-level issues when any active job has:
  ...
  - script path missing/outside `MARCEL_HOME/scripts`, or
