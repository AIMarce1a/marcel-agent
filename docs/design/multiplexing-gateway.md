...
_profile_runtime_scope(profile_home)           (gateway/run.py)
   ├── set_marcel_home_override(home)          config / state.db / skills /
...
- `get_secret(name)` resolves: global allowlist → active scope → fallback.
  ...
  - multiplexing **on**, no scope installed: **raises `UnscopedSecretError`**
    ...
    value.
- A small allowlist (`MARCEL_HOME`, `MARCEL_PROFILE`, proxy settings,
...
reload helper: `marcel_cli.env_loader.load_marcel_dotenv` skips the
...
## The MARCEL_HOME override

`marcel_constants.py` holds a context-local override consulted by
`get_marcel_home()` before the `MARCEL_HOME` env var. Everything that resolves
...
automatically. `get_process_marcel_home()` exists for the few machine-level
assets that must not follow the override. `marcel_home_key()` gives
...
`profiles_to_serve(multiplex, profile_allowlist)` in `marcel_cli/profiles.py`
...
DB handles are resolved at call time through the active MARCEL_HOME override —
...
MARCEL_HOME override. Asset writes are atomic, type- and size-capped.
...
| Built-in tool registry | Built-ins are process-global; plugin-registered tools are overlaid per profile via `marcel_home_key()`. |
...
- `agent/secret_scope.py`, `marcel_constants.py`, `gateway/profile_routing.py`,
  `gateway/run.py` (`_profile_runtime_scope`), `marcel_cli/profiles.py`
