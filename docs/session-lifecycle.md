...
The canonical transcript store is SQLite via `SessionDB` (from `marcel_state`). The
...
Counts consecutive restarts via a JSON file (`{MARCEL_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `marcel update`, `marcel gateway restart`,
or `/restart`.
