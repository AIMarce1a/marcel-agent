...
When running Marcel inside Docker, the default `network_mode: host` gives the
...
The Marcel [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   MARCEL_UID=$(id -u) MARCEL_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=marcel,marcel-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://marcel-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Marcel trust model and vulnerability reporting
