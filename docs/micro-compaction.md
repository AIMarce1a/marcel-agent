...
has to be thrown away or summarized. Marcel has always done this in one batch:
...
Marcel folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Marcel's normal session flush is
...
With `micro_compact` unset or `false` Marcel behaves exactly as it always has:
...
Defaults to `$MARCEL_HOME/logs/agent.log`. It reports passes, outcome mix, net
