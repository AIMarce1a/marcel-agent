"""SQLite journal-mode and PRAGMA policy for state.db (split from marcel_state).

Patchable helpers are looked up as module globals at call time, so tests patch ``marcel_state_wal.<name>``.
"""
...
from marcel_cli.sqlite_runtime import is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable
...
# line would repeat per connection). Tests clear these via ``marcel_state_wal.<name>``.
...
def apply_wal_with_fallback(conn: sqlite3.Connection, *, db_label: str = "state.db", require_wal: bool = False) -> str:
    ...
    Returns the mode actually set. Shared by :class:`SessionDB` and ``marcel_cli.kanban_db_connect.connect``.
...
def _wal_reset_repair_hint() -> str:
    """Repair hint matching what ``marcel update`` can actually do for this install type.

    See #75153.
    """
    try:
        from marcel_cli.config import detect_install_method, get_project_root, recommended_update_command_for_method
        method = detect_install_method(get_project_root())
        cmd = recommended_update_command_for_method(method)
        if cmd == "marcel update":
            cmd = runtime_command("update")
        if method in {"git", "unknown"}:
            return f"{runtime_brand()}-managed installs can repair the embedded runtime with `{cmd}`"
        ...
        return (
            ...
            f"(or backports 3.50.7 / 3.44.6) and restart {runtime_brand()}"
        )
...
_ONCE_LOGS = {
    "wal_reset_bug": (_wal_reset_bug_warned_lock, "_wal_reset_bug_warned_paths", logging.WARNING,
        ...
        "%s. See `%s`. This warning fires once per process per database."),
...
def _log_wal_reset_bug_once(db_label: str, *, kept_wal: bool, indeterminate: bool = False) -> None:
    ...
    _log_once(
        "wal_reset_bug", db_label, sqlite3.sqlite_version, sys.executable, action,
        _wal_reset_repair_hint, runtime_command("doctor"),
    )
...
def apply_database_pragmas(conn: sqlite3.Connection, *, db_label: str = "state.db") -> None:
    ...
    try:
        from marcel_cli.config import cfg_get, load_config_readonly  # local: avoids a circular import
