...
本 skill 专为 Marcel agent 设计。它使用 Marcel 工具、委派、调度和记忆来支撑完整的研究生命周期。

### 相关 Skill

将本 skill 与其他 Marcel skill 组合用于特定阶段：
...
| **plan** | 阶段 0（设置）：执行前创建结构化计划。写入 `.marcel/plans/` | `skill_view("plan")` |
