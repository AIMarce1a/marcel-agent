...
description: "Run Marcel Agent directly on an Android phone with Termux"
---
...
That does not stop Marcel from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
APT-managed Marcel installs are marked with install method `apt`. Marcel therefore does not run its Git self-updater against package-owned files; use the package manager instead:

```bash
pkg update
pkg upgrade marcel-agent
```

Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. Marcel runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.
...
- links `marcel` into `$PREFIX/bin` so it stays on your Termux PATH
...
Marcel requires **Python >=3.11,&lt;3.14**. Current Termux ships `python`
...
### 5. Put `marcel` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/marcel" "$PREFIX/bin/marcel"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `marcel` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.marcel/.env`.
...
### `marcel doctor` says ripgrep or Node is missing
