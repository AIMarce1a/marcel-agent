...
This page covers all commands related to [Marcel profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `marcel profile` without a subcommand shows help.
...
```bash
marcel profile use <name>
```

Sets `<name>` as the active profile. All subsequent `marcel` commands (without `-p`) will use this profile.
...
marcel profile use default
```

## `marcel profile create`

```bash
marcel profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `marcel profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `marcel update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.marcel`), use `marcel skills opt-out` / `marcel skills opt-in`. |
...
marcel profile create work2 --clone-from work

# Clone everything from a specific profile
marcel profile create work2-backup --clone-from work --clone-all
```

## `marcel profile describe`

```bash
marcel profile describe [<name>] [options]
```
...
marcel profile describe researcher

# Set it explicitly
marcel profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
marcel profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
marcel profile describe --all --auto
```

## `marcel profile delete`

```bash
marcel profile delete <name> [options]
```
...
```bash
marcel profile delete mybot
marcel profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.marcel`) cannot be deleted — use `marcel uninstall` to remove everything.
:::

## `marcel profile show`

```bash
marcel profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Marcel home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ marcel profile show work
Profile: work
Path:    ~/.marcel/profiles/work
...
## `marcel profile alias`

```bash
marcel profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Marcel installation.
...
```bash
marcel profile alias work
# Creates/updates ~/.local/bin/work

marcel profile alias work --name mywork
# Creates ~/.local/bin/mywork

marcel profile alias work --remove
# Removes the wrapper script
```

## `marcel profile rename`

```bash
marcel profile rename <old-name> <new-name>
```
...
```bash
marcel profile rename mybot assistant
# ~/.marcel/profiles/mybot → ~/.marcel/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `marcel profile export`

```bash
marcel profile export <name> [options]
```
...
```bash
marcel profile export work
# Creates work.tar.gz in the current directory

marcel profile export work -o ./work-2026-03-29.tar.gz
```
...
## `marcel profile import`

```bash
marcel profile import <archive> [options]
```
...
```bash
marcel profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

marcel profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `marcel -p telemetry`). |
...
marcel profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
marcel profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
marcel profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
marcel profile install ./telemetry/
```

### `marcel profile update`

```bash
marcel profile update <name> [--force-config] [--yes]
```
...
```bash
marcel profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Marcel version, author, env var requirements, the source URL/path, and
...
`marcel profile list` also shows the distribution name and version in a
`Distribution` column, and `marcel profile show <name>` / `delete <name>`
...
marcel profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
marcel profile install https://github.com/your-org/internal-assistant.git
```
...
`marcel_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Marcel version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Marcel can clone from.
3. Tell recipients to run `marcel profile install <your-repo-url>`.
...
## `marcel -p` / `marcel --profile`

```bash
marcel -p <name> <command> [options]
marcel --profile <name> <command> [options]
```

Global flag to run any Marcel command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
marcel -p work chat -q "Check the server status"
marcel --profile dev gateway start
marcel -p personal skills list
marcel -p work config edit
```

## `marcel completion`

```bash
marcel completion <shell>
```
...
- `marcel profile <TAB>` — subcommands (list, use, create, etc.)
- `marcel profile use <TAB>` — profile names
- `marcel -p <TAB>` — profile names
