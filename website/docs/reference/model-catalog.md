...
Marcel fetches curated model lists for **OpenRouter** and **Nous Portal** from a JSON manifest hosted alongside the docs site. This lets maintainers update picker lists without shipping a new `marcel-agent` release.

When the manifest is unreachable (offline, network blocked, hosting failure), Marcel silently falls back to the in-repo snapshot that ships with the CLI. The manifest never breaks the picker — worst case you see whatever list was bundled with your installed version.
...
- **`version`** — integer schema version. Future schemas bump this; Marcel refuses manifests with versions it doesn't understand and falls back to the hardcoded snapshot.
- **`metadata`** — free-form dict at the manifest, provider, and model level. Any keys. Marcel ignores unknown fields, so you can annotate entries (`"tier": "paid"`, `"tags": [...]`, etc.) without coordinating a schema change.
...
| When | What happens |
|---|---|
| `/model` or `marcel model` | Fetches if disk cache is stale, else uses cache |
...
Cache location: `~/.marcel/cache/model_catalog.json`.
...
The exclusion is matched case-insensitively against every key a provider can surface under — the Marcel id and models.dev id (built-in mapped providers), the overlay pid and resolved Marcel slug (overlay providers), and the canonical slug (canonical providers) — so a single entry like `copilot` hides the provider regardless of which section emits it. It is honored by every `/model` picker surface: the gateway interactive/text pickers, the TUI picker, and the interactive `marcel model` CLI picker. An empty list (or omitting the key) has no effect.
...
# editing OPENROUTER_MODELS or _PROVIDER_MODELS["nous"] in marcel_cli/models.py).
