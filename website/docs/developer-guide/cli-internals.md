...
sidebar_position: 15
title: "CLI Internals"
description: "How marcel_cli is shaped: slash dispatch, config loaders, the skin engine, the transactional update pipeline, and process-identity rules"
---

# CLI Internals

Companion to `marcel_cli/AGENTS.md` (the rules) — this page holds the longer explanations.

## Update pipeline

The stage-by-stage contract (`plan → snapshot → apply → restart-per-kind → verify → report`) and the
field failure each stage guards are documented in `marcel_cli/AGENTS.md`; user-facing behaviour
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line` (gateway run),
  `marcel_cli.update_cmd._marcel_holder_subcommand` (top-level subcommand of any Marcel argv). Never
...
Built-in skins (`_BUILTIN_SKINS` in `marcel_cli/skin_engine.py`): `default` (classic gold/kawaii),
...
User skins are `~/.marcel/skins/<name>.yaml` with the same keys, activated with `/skin <name>` or
`display.skin: <name>`; the full YAML template is in the
[Skins & Themes](../user-guide/features/skins.md) user guide.

## Profiles: multi-instance support

Marcel supports profiles — fully isolated instances, each with its own `MARCEL_HOME` (config, API
keys, memory, sessions, skills, gateway). `_apply_profile_override()` in `marcel_cli/main.py` sets
`MARCEL_HOME` before any module imports, so every `get_marcel_home()` reference scopes to the active
profile. Profile operations are HOME-anchored (`_get_profiles_root()` returns
`Path.home() / ".marcel" / "profiles"`, not `get_marcel_home() / "profiles"`) so
`marcel -p coder profile list` sees all profiles regardless of which one is active — intentional.
