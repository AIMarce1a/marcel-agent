...
Marcel Agent uses a SQLite database (`~/.marcel/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source files: `marcel_state.py` (facade) plus the `marcel_state_*.py` siblings (schema, fts, search, compression, portability, gateway, ...)


## Architecture Overview

```
~/.marcel/state.db (SQLite, WAL mode)
...
`marcel sessions recover` copies the row-bearing tables above into the
...
Abridged — see `SCHEMA_SQL` in `marcel_state_common.py` (applied by `marcel_state_schema.py`) for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `marcel_state_common.py` for the exact SQL.
...
Multiple marcel processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.marcel/state.db
...
Default path: `~/.marcel/state.db`

This is derived from `marcel_constants.get_marcel_home()` which resolves to
`~/.marcel/` by default, or the value of `MARCEL_HOME` environment variable.
