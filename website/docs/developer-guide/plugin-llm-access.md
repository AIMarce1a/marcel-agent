...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Marcel already has applies. The plugin never sees a
...
marcel-agent itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Marcel provider id (e.g. "openrouter")
...
using Marcel built-in auxiliary tasks; it does not permit another plugin's
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Marcel provider
        # (must be one Marcel already knows about — same names as
        # `marcel model` and config.yaml model.provider).
        ...
        # plugin sends — Marcel does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.marcel/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Marcel' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Marcel subsystem:
