...
title: "Desktop Plugin SDK (@marcel/plugin-sdk)"
description: "Extend the native Marcel Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Marcel Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `MarcelPlugin`.
It imports one module — `@marcel/plugin-sdk` — and gets everything: the app's
...
`$MARCEL_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Marcel. This page is the **native
desktop app** (`marcel desktop`) SDK — the `@marcel/plugin-sdk` module and
`$MARCEL_HOME/desktop-plugins/`. The **web dashboard** (`marcel dashboard`) has
its own, unrelated plugin system on `window.__MARCEL_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Marcel Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$MARCEL_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$MARCEL_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `MarcelPlugin` contract, appear in **Settings → Plugins**,
...
Create `$MARCEL_HOME/desktop-plugins/hello/plugin.js` (that's `~/.marcel/...`
by default, or `~/.marcel/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.marcel/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@marcel/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Marcel' }),
      jsx('div', {
...
The only importable specifiers are `@marcel/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `MarcelPlugin`:

```ts
interface MarcelPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `marcel.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@marcel/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@marcel/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@marcel/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@marcel/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@marcel/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@marcel/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@marcel/plugin-sdk'
...
`window.marcel.send('get-price eth')` (or a declarative
`<button data-marcel-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@marcel/plugin-sdk'
...
for requests; `targetProfile` is the backend Marcel profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Marcel
...
ctx.os.notify({
  ...
  // Body click → focus Marcel + navigate. Same vocabulary as OS deep links:
  activate: 'marcel://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'marcel://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `marcel://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `marcel://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@marcel/plugin-sdk'
...
```javascript
import { queryClient } from '@marcel/plugin-sdk'
...
a package into `~/.marcel/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.marcel/plugins` is not reachable as a filesystem —
...
the `marcel://` scheme — a plain anchor on your website or README:
...
`marcel-dev://`. Full link reference:
...
`dashboard/` subfolder of a regular Marcel plugin and declare it in a
`manifest.json`:
...
marcel-agent codebase directly (`marcel_state`, `marcel_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.marcel/`)
...
(`marcel.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `MarcelPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@marcel/plugin-sdk` alias.
2. It's still lint-fenced to `@marcel/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@marcel/plugin-sdk`, `react`,
...
| Plugin contract | `MarcelPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `MarcelOpenTarget`, `Contribution` |
...
### Agents: the `marcel-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`marcel-desktop-plugins`** skill — it carries the same contract as this page in
...
`$MARCEL_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `marcel logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@marcel/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.marcel/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.marcel/logs/errors.log` for
