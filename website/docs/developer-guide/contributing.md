...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Marcel Agent — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Marcel Agent! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Marcel core? Start with [Build a Marcel Plugin](../developer-guide/plugins/index.md)
- Building a new built-in core tool for Marcel itself? Start with [Adding Tools](./adding-tools.md)
...
You can also run a fully isolated Marcel instance (throwaway MARCEL_HOME, separate Electron
userData, distinct Electron app name to avoid the single-instance lock):

```bash
scripts/dev-sandbox.sh python -m marcel_cli.main
scripts/dev-sandbox.sh --persistent python -m marcel_cli.main desktop  # state survives restarts, but lives in the worktree :)
```
...
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.marcel/.env
```
...
- **Profile-safe paths**: Never hardcode `~/.marcel` — use `get_marcel_home()` from `marcel_constants` for code paths and `display_marcel_home()` for user-facing messages. See [AGENTS.md](https://github.com/NousResearch/hermes-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
Projects built on (or reviewed by) Marcel can keep reviewer checklists inside the repository under `.agents/checks/`. Each file is a focused, plain-markdown checklist that an agent loads before reviewing a change touching the matching area:
...
When you ask Marcel to review a PR in a repository that has `.agents/checks/`, tell it (or teach it via a skill) to read the relevant checklists first and report against them. This gives review agents the project-specific bar that generic review prompts miss.
