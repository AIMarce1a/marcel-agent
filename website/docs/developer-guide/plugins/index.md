...
title: "Build a Marcel Plugin"
description: "Step-by-step guide to building a complete Marcel plugin with tools, hooks, data files, and skills"
---

# Build a Marcel Plugin

This guide walks through building a complete Marcel plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
Marcel has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `MARCEL_LOCAL_STT_COMMAND` to an argv-tokenized template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.marcel/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `marcel skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
```bash
marcel plugins install owner/repository --no-enable
marcel plugins list
marcel plugins enable <plugin-name>
```
...
Marcel validates `plugin.json`, Agent Skills frontmatter, fixed component
...
profile-scoped writable directory managed by Marcel.
...
Portable `streamable-http` entries are routed through Marcel' existing native
...
same full-trust posture as other installed Marcel plugins.
...
records it as Published. Marcel keys behavior on the canonical v1.0.0 schema
...
not by one global plugin API number. Marcel does not expose a
...
continue to work after a normal Marcel upgrade.
...
- **Hook payloads are keyword payloads.** New hook data is added as keyword
  ...
  Marcel inspects callback signatures: a legacy callback receives the fields it
...
- **Manifests are open to additions.** Unknown `plugin.yaml` fields are ignored.
  Older Marcel releases can therefore load a plugin whose manifest contains
...
Marcel enforces this contract with frozen external-plugin fixtures discovered
from an isolated `MARCEL_HOME`. Those tests load and invoke the plugin through
...
Marcel's internals were split into `<stem>_<topic>` sibling modules in Sep 2026 (PR #102117). **Internal
...
- **Check your plugin:** `marcel plugins compat /path/to/your/plugin` lists every `file:line` with the
  old path and the new one, and exits 1 while any remain. `COMPAT_MANIFEST.md` in the repo is the full map.
- **What users see:** a notice under the CLI banner, in `marcel doctor` and after `marcel update`, and a
  one-time Desktop dialog naming the plugin. Each resolution through an old path also emits a
  `MarcelPluginCompatWarning` once per process.
- **From 2026-09-14:** plugins that still import old paths are **not loaded** (the reason shows in
  `marcel plugins list`). Users can force-load with `plugins.allow_deprecated_imports: true` until the
...
```bash
mkdir -p ~/.marcel/plugins/calculator
cd ~/.marcel/plugins/calculator
```

### Validate with Plugin Doctor

`marcel plugins doctor [path-or-id]` runs the same directory discovery,
manifest parser, namespaced import, `register(ctx)`, hook registry, and tool
registry used by Marcel itself. It reports invalid hook names, callbacks that do
...
```bash
marcel plugins doctor . --ci
```

Doctor uses a temporary `MARCEL_HOME`, restores plugin registration state after
...
This tells Marcel: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
`llm.task_override` (see `marcel_cli/plugin_capabilities.py` for the
...
`marcel_agent.plugin_capabilities` entry-point group. Each declaration is
named `<plugin-id>.<capability-id>` and points at the same object as your
`marcel_agent.plugins` entry point:

```toml
[project.entry-points."marcel_agent.plugins"]
calculator = "my_pkg:register"

[project.entry-points."marcel_agent.plugin_capabilities"]
"calculator.tools.override" = "my_pkg:register"
```

Marcel reads these from installed metadata without importing your code, so
`marcel plugins capabilities` and the consent flow stay accurate for pip
installs.
...
this Marcel understands still loads with a warning.
...
| `python_dependencies` | list of str | Declared pip requirements (e.g. `"requests>=2.0,<3"`). **Declaration seam only** — Marcel validates them, and `marcel plugins install` / `marcel plugins doctor` surface missing ones with a `pip install` hint, but Marcel **never auto-installs** them. Pin upper bounds. |
...
arbitrary packages into Marcel' shared venv is a conflict and supply-chain
...
4. **Accept `**kwargs`:** Marcel may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `marcel my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Marcel continues fine
...
Use plugin-relative config keys for user-visible behavior. Marcel resolves them
...
`<MARCEL_HOME>/plugin-data/`. Neither API exposes another plugin's namespace.
...
If your plugin doesn't show up — or shows up but isn't loading — set `MARCEL_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
MARCEL_PLUGINS_DEBUG=1 marcel plugins list
```
...
The same logs are always written to `~/.marcel/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
marcel logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `marcel plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout:** Native packages use `~/.marcel/plugins/<plugin-name>/plugin.yaml` (flat) or one category level. Portable packages use root `plugin.json` in the same locations. Anything deeper is ignored.
...
tree, and `marcel plugins update` / `remove` git-pull or delete it — your
...
# <marcel home>/plugin-data/<name>/ — created on first use
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.marcel/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.marcel/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `marcel plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing Marcel into installing arbitrary packages — only specs Marcel itself ships are eligible |
...
auditable in `~/.marcel/logs/agent.log`. Plugins load after built-in
...
does not ship with Marcel core (user, project, or pip source),
...
plugins:
  entries:
    my-plugin:                    # the plugin's registry key from `marcel plugins list`
...
loader, the plugin is disabled and Marcel continues. The gate exists
...
You normally never edit this key by hand. `marcel plugins enable <name>`
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), Marcel injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: marcel-agent"}
...
Per-hook context is capped at `10,000` characters by default. Anything above the cap is written to `$MARCEL_HOME/hook_outputs/<session_id>/<uuid>.txt` and replaced with a head/tail preview plus the saved path. The model can read the full content via `read_file` or `terminal` if it genuinely needs it. This keeps a runaway plugin from inflating every subsequent turn's prompt and blowing out the prompt cache prefix. Tune in `config.yaml`:
...
hooks:
  output_spill:
    ...
    # directory: null      # default: $MARCEL_HOME/hook_outputs
```
...
- **The system prompt is Marcel's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
The canonical list of kinds is `VALID_MIDDLEWARE` in `marcel_cli/middleware.py`:
...
| `llm_request` | `request`, `original_request`, context kwargs | Return `{"request": {...}}` to replace the effective provider kwargs before Marcel sends them. |
...
- Middleware payloads carry `middleware_schema_version` (`marcel.middleware.v1`) alongside the observer telemetry fields.
- Unknown kinds register with a warning instead of failing, so a plugin written against a newer Marcel still loads on an older one.

### Register CLI commands

Plugins can add their own `marcel <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for marcel my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: marcel my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for marcel my-plugin."""
...
After registration, users can run `marcel my-plugin status`, `marcel my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `marcel name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `marcel chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:

- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `MARCEL_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `marcel <subcommand>` (e.g. `marcel kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "marcel kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive Marcel from a hook.
...
Marcel has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.marcel/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `marcel gateway status` without SDK instantiation.
...
Marcel also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into Marcel without any Python plugin. Declare them in `~/.marcel/config.yaml`:
...
Marcel connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.marcel/hooks/<name>/`:

```yaml
# ~/.marcel/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.marcel/hooks/long-task-alert/handler.py
...
marcel skills search my-workflow --source myorg/skills-repo
marcel skills install myorg/skills-repo/my-workflow
```
...
```bash
pip install marcel-plugin-calculator
# Plugin auto-discovered on next marcel startup
```
...
```python
# Wrong — will break if Marcel passes extra context
def handler(args):
    ...
