...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Marcel Agent — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Marcel tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying Marcel core, use the plugin route instead:

- [Plugins](/user-guide/features/plugins)
- [Build a Marcel Plugin](/developer-guide/plugins)
...
2. **`toolsets.py`** — add tool name to `_MARCEL_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `marcel_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `marcel_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `marcel chat -q "Use the weather tool for London"`
