...
sidebar_position: 1
title: "Architecture"
description: "Marcel Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Marcel Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ marcel_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # MarcelCLI facade — mixins in marcel_cli/cli_*_mixin.py
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── marcel_state.py           # SQLite session/state database facade (+ marcel_state_*.py siblings)
├── marcel_constants.py       # MARCEL_HOME, profile-aware paths
...
│
├── marcel_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — `marcel` subcommands (parsers in subcommands/, main_*.py)
...
│   ├── skills_config.py      # marcel skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # marcel tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # marcel gateway start/stop
│
...
```text
User input → MarcelCLI.process_input()
...
Three discovery sources: `~/.marcel/plugins/` (user), `.marcel/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `marcel plugins` or `config.yaml`.
...
Exposes Marcel as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`marcel -p <name>`) gets its own MARCEL_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
