...
| Variable | Meaning |
|----------|---------|
| `MARCEL_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `MARCEL_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `MARCEL_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Marcel itself — they're private to these helpers. The variables Marcel *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `marcel --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_marcel_root)" || { echo "htui: not in a Marcel checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$MARCEL_MAIN_CHECKOUT/.venv/bin/python" -m marcel_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `MARCEL_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_marcel_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `MARCEL_TUI_DIR` are mutually exclusive
`MARCEL_TUI_DIR` points Marcel at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `marcel --tui --dev` exits with an error. Run `unset MARCEL_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_marcel_root)" || { echo "hgui: not in a Marcel checkout" >&2; return 1; }
  deps="${MARCEL_GUI_DEPS_CHECKOUT:-$MARCEL_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _marcel_link_deps "$desktop" "$deps/apps/desktop"
    _marcel_link_deps "$root" "$deps"
  else
  ...
  trap '_marcel_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    MARCEL_DESKTOP_MARCEL_ROOT="$root" \
    MARCEL_DESKTOP_PYTHON="$MARCEL_MAIN_CHECKOUT/.venv/bin/python" \
    MARCEL_DESKTOP_IGNORE_EXISTING=1 \
    MARCEL_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `MARCEL_DESKTOP_MARCEL_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `marcel`. |
| `MARCEL_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `MARCEL_DESKTOP_IGNORE_EXISTING` | Ignores any `marcel` on `PATH` so it can't shadow the worktree. |
| `MARCEL_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Marcel checkout.
_marcel_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/marcel_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_marcel_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'marcel_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `marcel --tui --dev` and the `MARCEL_TUI_DIR` prebuild path
...
- [Environment Variables](../reference/environment-variables.md) — every `MARCEL_*` variable Marcel reads
