...
Marcel runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.marcel/plugins/`, registering a backend the user selects
...
| `marcel setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `marcel status` / `marcel doctor` | `doctor_checks()` |
...
```python title="~/.marcel/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.marcel"    # where synced cache files land, or None
...
```yaml title="~/.marcel/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
marcel plugins enable acmebox
marcel config set terminal.backend acmebox
```
