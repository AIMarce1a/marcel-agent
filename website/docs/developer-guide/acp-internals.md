...
The ACP adapter wraps Marcel' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
marcel acp / marcel-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.marcel/.env
  -> configure stderr logging
  -> construct MarcelACPAgent
...
- `allow_once` -> Marcel `once`
- `allow_always` -> Marcel `always`
- reject options -> Marcel `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Marcel tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["marcel-acp"])
...
Instead it reuses Marcel' runtime resolver:

- `acp_adapter/auth.py`
- `marcel_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Marcel provider/credentials. It also always advertises a terminal setup auth method (`marcel-setup`, args `--setup`) so first-run ACP clients can open Marcel' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.marcel/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `marcel-acp` toolset definition
- `marcel_cli/main.py` — `marcel acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `marcel-acp` script
