...
Plugins can launch and supervise fresh Marcel child sessions without importing
...
active agent turn fails closed with `No active Marcel parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Marcel can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Marcel's existing unsafe-tool block remains enforced.
