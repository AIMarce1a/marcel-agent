...
title: "Git Worktrees"
description: "Run multiple Marcel agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Marcel Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Marcel so each session has a clean, isolated working directory.

## Why Use Worktrees with Marcel?

Marcel treats the **current working directory** as the project root:

- CLI: the directory where you run `marcel` or `marcel chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.marcel/config.yaml`
...
Marcel creates `.worktrees/my-experiment/` inside the repo (branch
`marcel/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `marcel-<id>`
...
git worktree add ../repo-feature feature/marcel-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/marcel-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Marcel there:

```bash
cd ../repo-feature

# Start Marcel in the worktree
marcel
```
...
git worktree add ../repo-experiment-a feature/marcel-a
git worktree add ../repo-experiment-b feature/marcel-b
```
...
- Works on its own branch (`feature/marcel-a` vs `feature/marcel-b`).
...
- Marcel checkpoint data under `~/.marcel/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Marcel experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/marcel-checkpoints-docs`, `feature/marcel-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Marcel from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `marcel -w` (Automatic Worktree Mode)

Marcel has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `marcel/marcel-<hash>`).
...
```bash
marcel -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `marcel -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Marcel session its own clean checkout.
