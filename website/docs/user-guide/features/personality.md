...
description: "Customize Marcel Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Marcel Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `MARCEL_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Marcel is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Marcel now seeds a default `SOUL.md` automatically in:

```text
~/.marcel/SOUL.md
```

More precisely, it uses the current instance's `MARCEL_HOME`, so if you run Marcel with a custom home directory, it will use:
...
- Marcel creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Marcel loads `SOUL.md` only from `MARCEL_HOME`
- Marcel does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Marcel falls back to a built-in default identity
...
If Marcel loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `MARCEL_HOME`, the personality belongs to the Marcel instance itself.

That also makes it easier to teach users:
- "Edit `~/.marcel/SOUL.md` to change Marcel' default personality."
...
- how Marcel should handle uncertainty, disagreement, or ambiguity
...
## What Marcel injects into the prompt
...
Marcel ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Marcel, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Marcel its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

Built-in personalities are always available on every surface (CLI, messaging platforms, TUI, and the desktop app). You can add your own — or override a built-in by reusing its name — in `~/.marcel/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.marcel/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-marcel)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Marcel speaks
- `display.skin` and `/skin` affect how Marcel looks in the terminal
