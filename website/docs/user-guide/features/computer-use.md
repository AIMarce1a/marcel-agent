...
Marcel Agent can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended Marcel integration. It
...
**Fresh installs already have the driver.** The Marcel installer
...
- **`marcel tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
marcel computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `marcel computer-use
status` to verify the install.

Already have cua-driver? Marcel reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `marcel update`, and the first
`computer_use` call of a session, Marcel checks the local version and
...
selected with `MARCEL_CUA_DRIVER_CMD` stays
under your control, so Marcel reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Marcel autodetection is a
planned cua-driver follow-up, so currently point Marcel at that directory or
...
interface. The built-in toolset provides Marcel actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `marcel computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the Marcel host identity. |
...
```
marcel -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.marcel/config.yaml`.

## Permission modes and logged-in browser profiles

Marcel maps its existing approval UX onto cua-driver's immutable runtime
modes. Permission mode and capability manifest approval are launch settings.
They cannot change after the runtime starts:

| Marcel session | cua-driver mode | Human intervention |
|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Marcel approvals; Cua stops at its protected boundary |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Marcel risk acceptance; no runtime Cua prompts |
...
computer_use:
  ...
  capability_manifest: ~/.marcel/cua-manifest.yaml
```
...
for the format). Marcel launches a private runtime with
...
identity instead of resetting with every Marcel build), and Marcel verifies
...
resetting or closing the Marcel session, cancellation cleanup, or process exit
closes that transport session. Marcel also stops private runtimes that it
launched for bounded or unrestricted access. One Marcel
conversation cannot change another runtime's mode or grants. Bounded and
unrestricted modes use a private
embedded service under the Marcel host identity.
...
## `marcel computer-use doctor` — your first triage stop

`marcel computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ marcel computer-use doctor
...
Marcel run declares a public cua-driver **session name** (something like
`marcel-3a7b9c14d2e8`). The name labels cursor identity and related state, so
...
without it. Marcel disables it automatically where it is a known failure
...
Marcel keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Marcel-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. Marcel
autodetection is a planned cua-driver follow-up, so currently point Marcel at
...
These are **platform deep dives, not duplicates of the Marcel skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Marcel
...
image capture also saves a bounded, shareable copy under Marcel' image cache
...
Marcel applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.marcel/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Marcel applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Marcel agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Marcel agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Marcel warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Marcel from inside the RDP / console
...
On Linux, native Wayland support remains an explicit opt-in. Marcel passes the
...
```
MARCEL_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
MARCEL_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Marcel disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Marcel sets
...
When it's on, `marcel computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Marcel at a binary you built from source instead
of the published release. Marcel resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`MARCEL_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Marcel testing (see notes).
...
### Option B — point Marcel straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`MARCEL_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
MARCEL_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
MARCEL_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Marcel is using your build

- `marcel computer-use status` prints the resolved binary path and
  version.
- `marcel computer-use doctor` confirms the binary is reachable and
...
- **Marcel spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Marcel YOLO, Marcel instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Marcel
  change needed either way.
- **Local builds skip the version check.** Marcel warns when the
...
**First action when anything's off: run `marcel computer-use doctor`.**
...
Run `marcel computer-use install` to fetch the cua-driver binary, or
run `marcel tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `marcel computer-use doctor` will
...
- **Marcel-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Marcel `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Marcel autodetection is a
  planned follow-up; currently point Marcel at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Marcel among them)
