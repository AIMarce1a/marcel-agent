...
Marcel can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<MARCEL_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Marcel renders the sprite at full fidelity when your
...
marcel pets list
marcel pets list cat

# Install a pet and make it active in one step
marcel pets install boba --select
...
## `marcel pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `marcel pets list [query] [--limit N]` |
| List installed pets | `marcel pets list --installed` |
| Install a pet | `marcel pets install <slug> [--select] [--force]` |
| Set the active pet | `marcel pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `marcel pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `marcel pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `marcel pets off` |
| Remove an installed pet | `marcel pets remove <slug>` |
| Diagnose setup | `marcel pets doctor` |

`marcel pets show` flags:
...
Beyond installing pre-made pets from the gallery, Marcel can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `marcel pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `marcel tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `MARCEL_PET_IMAGE_PROVIDER` env var (e.g. `MARCEL_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Marcel is
...
Run `marcel pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Marcel uses its own
  profile-scoped `<MARCEL_HOME>/pets/` instead — install through `marcel pets`.

## See also

- The [`marcel-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-marcel-agent.md)
