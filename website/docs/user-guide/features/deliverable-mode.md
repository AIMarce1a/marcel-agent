...
When Marcel Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.marcel/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.marcel/config.yaml` (switchable per session
via `/personality`).
...
If you use Marcel' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.marcel/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Marcel Agent's deliverable
