...
```bash
# Custom port
marcel dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
marcel dashboard --host 0.0.0.0

# Start without opening browser
marcel dashboard --no-open
```
...
child with the selected profile's `MARCEL_HOME`, so the conversation runs
...
processes (manage them via `marcel -p <name> gateway …`), each profile's
session database, and cron schedulers (the Cron page already aggregates
across profiles with its own filter).

## Prerequisites

The default `marcel-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
cd ~/.marcel/marcel-agent && uv pip install -e ".[web,pty]"
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `cd ~/.marcel/marcel-agent && uv pip install -e ".[all]"` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `marcel dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `marcel dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- **Disk warnings** — the volume holding `~/.marcel` is nearly full
...
- Node.js (same requirement as `marcel --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`cd ~/.marcel/marcel-agent && uv pip install -e ".[web,pty]"`, or `[all]` covers both)
...
```ini
[Service]
EnvironmentFile=%h/.marcel/.env
ExecStart=/path/to/venv/bin/python -m marcel_cli.main dashboard \
    --host 0.0.0.0 --port 9119 --no-open
```

with `~/.marcel/.env` containing:

```bash
MARCEL_DASHBOARD_BASIC_AUTH_USERNAME=admin
MARCEL_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
MARCEL_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `marcel config set` and the gateway read from.
:::
...
Create and manage [profiles](../profiles.md) — isolated Marcel instances with their own config, skills, and sessions.
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `marcel profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
- **Edit model / description / SOUL** — inline editors writing into that profile
- **Rename / Delete** — named profiles only

### Skills

Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.marcel/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `marcel skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `marcel mcp` reads from.
...
`marcel mcp catalog` / `marcel mcp install` use.
...
Connect Marcel to any messaging platform from the browser — full parity with
`marcel setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.marcel/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page.
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and Marcel versions, CPU core count + utilization, memory, disk usage of the Marcel home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The Marcel version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `marcel update` in the background. On Docker/Nix installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **Nous Portal** — login status, the active inference provider, and the Tool Gateway routing table (which tools run via the Portal vs. locally), with a link to manage your subscription. Read-only mirror of `marcel portal`.
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `marcel curator`.
...
This re-reads `~/.marcel/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
`MARCEL_HOME`. Omitted = the dashboard's own profile. Unknown profile names
...
- **`disk`** — a live `shutil.disk_usage()` sample of the volume holding
  `~/.marcel`. Fields: `pressure`, `free_mb`, `total_mb`, `used_percent`,
...
| `GET /api/marcel/update/check` | Report update availability (commits behind, install method) without applying. For git installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 6h cache |
...
|-------|-----------|----------|
| `marcel dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `marcel dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no Nous plugin, no custom plugin), `marcel dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.
...
- **CLI — `marcel dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Nous login (run `marcel setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `MARCEL_DASHBOARD_OAUTH_CLIENT_ID` into `~/.marcel/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).

  ```bash
  marcel dashboard register
  # ✓ Registered dashboard "swift_falcon"
  # …writes MARCEL_DASHBOARD_OAUTH_CLIENT_ID to ~/.marcel/.env
  ```

- **GUI — the Local Dashboards page.** Open [`/local-dashboards`](https://portal.nousresearch.com/local-dashboards) in the Nous Portal to register, name, manage, and revoke self-hosted dashboards from the browser. Copy the resulting `agent:{id}` client ID into `MARCEL_DASHBOARD_OAUTH_CLIENT_ID` (env) or `dashboard.oauth.client_id` (config.yaml). This is also where you revoke a dashboard registered via the CLI.
...
|---------|-----------|--------|----------------|
| `MARCEL_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `marcel dashboard register` |

Per the Marcel Agent convention (`~/.marcel/.env` is for API keys / secrets only), **`config.yaml` is the recommended place to set these values** for local dev, on-prem, and any deployment you control directly. The environment-variable path exists so a hosting platform's secret injection can push per-deploy `client_id`s without anyone having to edit `config.yaml` inside the image — that's its primary purpose.
...
Bundled providers reported these issues:
  • nous: MARCEL_DASHBOARD_OAUTH_CLIENT_ID is not set (and
...
Configure an auth provider before exposing the dashboard:
  ...
  • OAuth: run `marcel dashboard register` (Nous Portal) or install a
...
From a logged-in Marcel install to a Nous-gated dashboard in three steps.

**1. Log in and register the dashboard.** `marcel dashboard register` uses your existing Nous login to provision an OAuth client and writes `MARCEL_DASHBOARD_OAUTH_CLIENT_ID` into `~/.marcel/.env` for you:

```bash
marcel setup            # if you're not already logged into Nous Portal
marcel dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes MARCEL_DASHBOARD_OAUTH_CLIENT_ID to ~/.marcel/.env
```
...
```bash
marcel dashboard --host 0.0.0.0 --port 9119 --no-open
```
...
`GET /api/auth/me` then returns the verified session (`provider: nous`). For an internet-facing host, register with `--redirect-uri https://marcel.example.com/auth/callback` and set `MARCEL_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
|---------|-----------|-------|
| `MARCEL_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `MARCEL_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `MARCEL_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `MARCEL_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `MARCEL_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
**1. Set credentials in `~/.marcel/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
cat >> ~/.marcel/.env <<EOF
MARCEL_DASHBOARD_BASIC_AUTH_USERNAME=admin
MARCEL_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
...
dashboard:
  oauth:
    ...
    self_hosted:
      issuer: https://auth.example.com/application/o/marcel/   # required
      client_id: marcel-dashboard                              # required
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `MARCEL_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `MARCEL_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `MARCEL_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-marcel.json` — it defines a `marcel` realm, a **public PKCE client** (`marcel-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```json
{
  ...
  "clients": [
    {
      "clientId": "marcel-dashboard",
      "name": "Marcel Agent Dashboard",
      "enabled": true,
      "publicClient": true,
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-marcel.json:/opt/keycloak/data/import/realm-marcel.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/marcel/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/marcel`). The admin console is at
...
```bash
export MARCEL_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/marcel"
export MARCEL_DASHBOARD_OIDC_CLIENT_ID="marcel-dashboard"
export MARCEL_DASHBOARD_PUBLIC_URL="http://localhost:9119"
marcel dashboard --host 0.0.0.0 --port 9119 --no-open
```

`MARCEL_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
> admin console (Clients → marcel-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `MARCEL_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:

```yaml
dashboard:
  public_url: "https://dashboard.example.com/marcel"
  trusted_proxies:
    - "172.20.0.5"
```
...
first; without one, Marcel fails closed at startup. This prevents the local SPA
...
Marcel always preserves loopback trust and rejects `*`, `0.0.0.0/0`, and
...
marcel dashboard --host 127.0.0.1 --port 9119 --no-open
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `MARCEL_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `MARCEL_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
5. Server exchanges the code for an access token at `POST /api/oauth/token`, verifies the JWT signature against the Portal's JWKS (`/.well-known/jwks.json`), and sets the `marcel_session_at` cookie.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `marcel_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `marcel_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip. SameSite=None + Secure over HTTPS (must survive the cross-site IDP redirect chain — Chromium drops SameSite=Lax cookies set on a 302 in a cross-site chain); SameSite=Lax on loopback HTTP |
| `marcel_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
Every login start, success, failure, and session-verify failure is written as a JSON line to `$MARCEL_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.
...
```python
# ~/.marcel/plugins/dashboard-auth-myidp/__init__.py
from marcel_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
The bundled first consumer is the **drain** provider (`plugins/dashboard_auth/drain`): `nous-account-service` provisions a per-agent secret via `MARCEL_DASHBOARD_DRAIN_SECRET`, and the provider verifies inbound bearer tokens against it with a constant-time compare, registering `/api/gateway/drain` as token-authable. It **fails closed** — a weak/short secret (< 256 bits) is rejected at registration and the endpoint stays disabled; it's a no-op when the env var is unset. Behavioural knobs (`scope`, `min_secret_chars`) live under `dashboard.drain_auth` in `config.yaml`.
...
```bash
# Quick env-var path.
MARCEL_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  marcel dashboard --host 0.0.0.0
...
```bash
# 1. Set the dashboard login credentials in ~/.marcel/.env (secrets file, 0600).
cat >> ~/.marcel/.env <<'EOF'
MARCEL_DASHBOARD_BASIC_AUTH_USERNAME=admin
MARCEL_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
marcel dashboard --no-open --host 0.0.0.0 --port 9119
```

Prefer no plaintext at rest? Use `MARCEL_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/marcel` are supported if you front it with a reverse proxy)
...
The session refreshes automatically and survives restarts when `MARCEL_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `MARCEL_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `MARCEL_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |
...
- **Signed out on every restart** — set `MARCEL_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `marcel_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `marcel update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `marcel dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Marcel Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Marcel Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
