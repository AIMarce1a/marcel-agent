...
Marcel runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Marcel
...
1. Marcel captures a baseline of current diagnostics for the file.
...
…). Marcel auto-detects the binary on PATH or in
`<MARCEL_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Marcel at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<MARCEL_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`marcel lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Marcel installs both packages together when you
run `marcel lsp install typescript` or auto-install fires on first
use.

## CLI

```
marcel lsp status          # service state + per-server install status
marcel lsp list            # registry, optionally --installed-only
marcel lsp install <id>    # eagerly install one server
marcel lsp install-all     # try every server with a known recipe
marcel lsp restart         # tear down running clients
marcel lsp which <id>      # print resolved binary path
```

`marcel lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <MARCEL_HOME>/lsp/bin
...
When `install_strategy: auto`, Marcel installs binaries into
`<MARCEL_HOME>/lsp/bin/`. NPM packages land in
`<MARCEL_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Marcel-owned and is
...
**single process** per Marcel process: the first Python project spawns
...
**`marcel lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<MARCEL_HOME>/lsp/bin/`. Run
`marcel lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `marcel lsp status`**
...
When `marcel lsp status` shows a `Backend warnings` section, install
...
`~/.marcel/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.marcel/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `marcel lsp restart` to clear the set;
