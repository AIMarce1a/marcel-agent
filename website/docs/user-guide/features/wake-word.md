...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey Marcel' wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey Marcel")

The wake word turns Marcel into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, Marcel listens in the background for a spoken
trigger phrase. Say it, and Marcel starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** Marcel backend (for example a
...
Marcel supports **client capture** for that case:
...
|--------|------|---------|-------|
| **openWakeWord** (default) | Free | None | Local ONNX models. Ships a bundled **"hey marcel"** model (default); also supports `hey_jarvis`, `alexa`, `hey_mycroft`, … and custom models |
...
By default the phrase is **"hey marcel"** — a model for it ships with Marcel, so
...
```bash
cd ~/.marcel/marcel-agent && uv pip install -e ".[wake]"
```

## Quick start

```bash
# In an interactive `marcel` session:
...
desktop ear button — also writes `wake_word.enabled` to `~/.marcel/config.yaml`,
...
wake_word:
  ...
  phrase: "hey marcel"        # cosmetic label only — detection is keyed by the model/keyword below
  ...
  openwakeword:
    model: hey_marcel         # bundled default; OR a built-in name OR a path to a custom .onnx/.tflite
...
- **`confirmation_frames`** (default `3`, openWakeWord only) — how many
  *consecutive* over-threshold frames are required before the wake fires. A real
  "hey marcel" holds a high score across several frames; an ambient blip spikes
...
- **`sensitivity`** (default `0.6`) — the detection threshold, `0.0`–`1.0`.
  ...
  **all** engines — for openWakeWord it's the raw per-frame score threshold, for
  ...
  if real "hey marcel" utterances are missed.
...
default) to let Marcel choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which Marcel installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three Marcel surfaces, and `surface` picks which one
...
| `cli` | Only the classic `marcel` CLI. |
| `tui` | Only `marcel --tui`. |
| `gui` | Only the desktop app. |

The detector is on-device and single-mic, so only one surface listens at a time,
including when Marcel surfaces run in separate processes. Ownership is sticky:
the first eligible claimant keeps the listener until it stops, disconnects, or
its process exits. Marcel does not silently fail over to another open surface.
...
"Hey Marcel" works out of the box — the bundled openWakeWord model
(`model: hey_marcel`) is the default. To wake on something else, the easiest
...
command (`marcel -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.marcel/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
syllables with an uncommon word ("marcel" qualifies) beat common words like
"hello" or "stop".
:::

### Option C — Porcupine (custom keyword in seconds)

Create a "Hey Marcel" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
wake_word:
  ...
  phrase: "hey marcel"
  porcupine:
    keyword: ~/.marcel/wakewords/hey_marcel.ppn
```

Set your access key in `~/.marcel/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `marcel tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `marcel-agent[wake]`).
...
but the phrase never fires. Marcel detects this (`/wake status` shows
"mic delivers only silence"; the desktop ear tooltip carries the same hint).
Fix: System Settings → Privacy & Security → Microphone → enable the Marcel
backend (it may appear as your terminal, `python`, or Marcel), then toggle the
...
```bash
marcel config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
marcel config set wake_word.input_device null
```
