...
title: "Plugins"
description: "Extend Marcel with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Marcel has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/developer-guide/adding-tools) page is for built-in Marcel
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Marcel Plugin](/developer-guide/plugins)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.marcel/plugins/` with a `plugin.yaml` and Python code:

```
~/.marcel/plugins/my-plugin/
...
Start Marcel — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.marcel/plugins/hello-world/plugin.yaml`**
...
**`~/.marcel/plugins/hello-world/__init__.py`**

```python
"""Minimal Marcel plugin — registers a tool and a hook."""
...
Drop both files into `~/.marcel/plugins/hello-world/`, restart Marcel, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

The model-facing tool description belongs in `schema["description"]`. The optional `ctx.register_tool(description=...)` value is separate `ToolEntry` registry metadata: when omitted, it defaults to the schema description, but Marcel does not copy it back into a schema that lacks `description`. Prefer defining the text once in the schema. If you provide both values, keep them synchronized; the model sees the schema value.

Project-local plugins under `./.marcel/plugins/` are disabled by default. Enable them only for trusted repositories by setting `MARCEL_ENABLE_PROJECT_PLUGINS=true` before starting Marcel.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `marcel <plugin> <subcommand>` |
...
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `marcel plugins install` |
| Distribute via pip | `[project.entry-points."marcel_agent.plugins"]` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Marcel — see [Built-in Plugins](/user-guide/features/built-in-plugins) |
| User | `~/.marcel/plugins/` | Personal plugins |
| Project | `.marcel/plugins/` | Project-specific plugins (requires `MARCEL_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `marcel_agent.plugins` entry_points | Distributed packages |
| Nix | `services.marcel-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/getting-started/nix-setup#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, Marcel also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.marcel/plugins/model-providers/<name>/` and `~/.marcel/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `marcel plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.marcel/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
marcel plugins                    # interactive toggle (space to check/uncheck)
marcel plugins enable <name>      # add to allow-list
marcel plugins disable <name>     # remove from allow-list + add to disabled
```

After `marcel plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

For a reproducible install, pin a full immutable commit (tags, branches, and
abbreviated SHAs are not accepted):

```bash
marcel plugins install owner/repo --ref 0123456789abcdef0123456789abcdef01234567
```

Marcel checks out the commit detached, verifies that `HEAD` exactly matches the
requested SHA, and records the canonical source, installed revision, and pin
status in the current profile. `marcel plugins update` refuses to move a pinned
plugin; choose a new exact commit explicitly with
`marcel plugins install <source> --force --ref <new-commit>`. The
...
Several categories of plugin bypass `plugins.enabled` — they're part of Marcel' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.marcel/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.marcel/plugins/`.

### Approval transports

An approval transport changes **where a human sees and answers** an existing
Marcel tool-approval request. It does not decide whether a command needs
...
`present` may be synchronous or async. Marcel runs it on a bounded worker and
...
`transport_fallback: builtin`. Without that exact opt-in, Marcel never
materializes the prompt on another surface.

Marcel still owns hardline blocks, sudo-stdin protection, user deny rules,
...
When you upgrade to a version of Marcel that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.marcel/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)

## Available hooks

Plugins can register the 26 lifecycle events currently accepted by `marcel_cli.plugins.VALID_HOOKS`. The **[Event Hooks catalog](/user-guide/features/hooks#shipped-plugin-hook-catalog)** is canonical for exact timing, return handling, payload fields, and privacy notes.
...
Marcel has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.marcel/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Marcel also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Marcel Plugin](/developer-guide/plugins) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Marcel Plugin](/developer-guide/plugins) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Marcel Plugin](/developer-guide/plugins) · [Extending the CLI](/developer-guide/extending-the-cli) |
| A **subcommand** for `marcel <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| An **STT backend** (any CLI — whisper.cpp, custom whisper binary, local ASR CLI) | Config-driven (recommended) — declare under `stt.providers.<name>` with `type: command` in `config.yaml`, or set `MARCEL_LOCAL_STT_COMMAND` for the legacy single-command escape hatch. OR Python backend plugin — `ctx.register_transcription_provider()` for Python-SDK engines (OpenRouter, SenseAudio, Gemini-STT, etc.). | [STT Setup](/user-guide/features/tts#stt-custom-command-providers) · [Python plugin guide](/user-guide/features/tts#python-plugin-providers-stt) |
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Marcel auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `marcel skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.marcel/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `marcel plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
```bash
marcel plugins                               # unified interactive UI
marcel plugins list                          # table: enabled / disabled / not enabled
marcel plugins search <term>                 # search the community plugin index
marcel plugins install <name>                # install by index name (resolved to repo @ pinned ref)
marcel plugins install user/repo             # install from Git, then prompt Enable? [y/N]
marcel plugins install user/repo --enable    # install AND enable (no prompt)
marcel plugins install user/repo --no-enable # install but leave disabled (no prompt)
marcel plugins update my-plugin              # pull latest (local edits are autostashed and re-applied)
marcel plugins remove my-plugin              # uninstall
marcel plugins enable my-plugin              # add to allow-list
marcel plugins disable my-plugin             # remove from allow-list + add to disabled
marcel plugins capabilities [my-plugin]      # declared vs granted capabilities
```

### One-click install links (Desktop)

Marcel Desktop registers the `marcel://` URL scheme, so a website, README, or
chat message can link straight to a plugin install:

```
marcel://plugin/install?repo=owner/repo            # main install link
marcel://plugin/install?repo=owner/repo&enable=1   # enable the agent plugin after install
marcel://plugin/install?repo=owner/repo&force=1    # replace an existing install
```

Clicking one opens Marcel and shows a **confirmation dialog** — the repo id,
...
Install from Git**. Legacy `marcel://plugin-agent/…` and
`marcel://plugin-desktop/…` URLs route into the same dialog. In dev builds
(`npm run dev`) the scheme is `marcel-dev://`.

Websites need no SDK — a normal anchor works:

```html
<a href="marcel://plugin/install?repo=owner/repo&enable=1">Install in Marcel</a>
```

MCP servers have the equivalent link form — see
[Add to Marcel link](/reference/mcp-config-reference#add-to-marcel-link).
...
When a plugin declares capabilities, `marcel plugins install` (and
`marcel plugins enable`) shows the list with one-line risk descriptions and
...
granted, `marcel plugins update` surfaces the additions and asks again. New
...
`marcel plugins enable <id>` interactively to grant them later.

Inspect the state at any time:

```bash
marcel plugins capabilities             # all plugins with declared/granted capabilities
marcel plugins capabilities my-plugin   # one plugin, declared vs granted
```
...
not a code audit, and Marcel has not reviewed the plugin's code. Only install
...
`marcel plugins search <term>` searches the **community plugin index** — a
static, machine-readable JSON catalog of community plugins. Matching is fuzzy
across name, description, and tags:

```bash
marcel plugins search telegram               # fuzzy search
marcel plugins search                        # browse the whole index
marcel plugins search --capability platform  # filter by declared capability
marcel plugins search media --json           # machine-readable output
marcel plugins search --refresh              # bypass the 24h local cache
```
...
```bash
marcel plugins install hermes-media-studio
```
...
(`https://raw.githubusercontent.com/NousResearch/marcel-plugin-index/main/index.json`,
overridable via `marcel config set plugins.index_url <url>`). Fetches are
cached under `~/.marcel/cache/plugin_index.json` for 24 hours; when the
remote is unreachable the stale cache is used, and when there is no cache at
all a bundled seed copy ships with Marcel — so search works fully offline.
...
(`marcel_cli/data/plugin_index.json` in the repo) is the format reference.

**Submitting a plugin.** The index is maintained as a plain JSON file —
submit a pull request to the
[marcel-plugin-index](https://github.com/NousResearch/marcel-plugin-index)
...
A **plugin pack** is a declarative, shareable YAML file (`marcel-pack.yaml`)
...
```bash
marcel plugins pack show ./marcel-pack.yaml     # dry-run review
marcel plugins pack install ./marcel-pack.yaml  # review → confirm → install
marcel plugins pack export > marcel-pack.yaml   # snapshot the current install
marcel plugins pack export --enabled-only       # only plugins.enabled
```
...
same pinned install path as `marcel plugins install --ref <sha>` and record
...
capability-consent prompt — identical to a single `marcel plugins install`.
...
provenance (installed via `marcel plugins install`). Local-only plugins are
...
auto-installed — install those manually for now (`marcel skills`). Wiring
skill-hub ids into pack install is a documented follow-up seam.

### Install-time security scanning

Every `marcel plugins install` and `marcel plugins update` runs a static
...
On `marcel plugins update`, a dangerous verdict on the updated tree
...
Running `marcel plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `marcel plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
- Marcel reuses that session's stored platform, chat, thread, profile, and conversation history. Plugins cannot supply a new chat route through this API.
- Marcel rechecks the stored route against the gateway's current authorisation rules before dispatch.
- Routes that relied only on an adapter-time or upstream authorisation decision are rejected unless Marcel can revalidate them from current core allowlists, pairing, or explicit allow-all configuration.
...
- The route and conversation are pinned while dispatch is pending. Marcel drops the request if topic recovery changes the route or the session rotates before handling starts.
...
:::warning
Only grant gateway injection to plugins you trust. Marcel checks this host API permission and restricts it to existing session routes, but Python plugins run in-process and this setting is not a sandbox.
:::
...
`ctx.call_mcp()` lets a plugin call a tool on one of the user's configured MCP servers — synchronously, from any hook or tool handler — routing through Marcel' existing native MCP client (same connections, trust-tier gates, circuit breaker, and reconnect logic as model-invoked MCP tools; never a parallel client).
