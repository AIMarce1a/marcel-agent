...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Marcel's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Marcel instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
marcel memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `marcel` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Marcel syncs them back at session init.
...
The `marcel honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `marcel memory setup honcho` (or run `marcel memory setup` and pick it from the list); the `marcel honcho` subcommand then appears on the next invocation.

```bash
marcel memory setup honcho    # Configure Honcho directly (works before activation)
marcel honcho status          # Connection status, config, and key settings
marcel honcho setup           # Redirects to `marcel memory setup` (post-activation alias)
marcel honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
marcel honcho peer            # Show or update peer names + dialectic reasoning level
marcel honcho mode            # Show or set recall mode (hybrid/context/tools)
marcel honcho tokens          # Show or set token budget for context and dialectic
marcel honcho identity        # Seed or show the AI peer's Honcho identity
marcel honcho sync            # Sync Honcho config to all existing profiles
marcel honcho peers           # Show peer identities across all profiles
marcel honcho sessions        # List known Honcho session mappings
marcel honcho map             # Map current directory to a Honcho session name
marcel honcho enable          # Enable Honcho for the active profile
marcel honcho disable         # Disable Honcho for the active profile
marcel honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `marcel honcho`

If you previously used the standalone `marcel honcho setup`:
...
No re-login or re-setup needed. Run `marcel memory setup` and select "honcho" — the wizard detects your existing config.
