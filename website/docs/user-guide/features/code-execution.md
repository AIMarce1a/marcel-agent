...
The `execute_code` tool lets the agent write Python scripts that call Marcel tools programmatically, collapsing multi-step workflows into a single LLM turn. The script runs in a child process on the agent host, communicating with Marcel over a Unix domain socket RPC.

## How It Works

1. The agent writes a Python script using `from marcel_tools import ...`
2. Marcel generates a `marcel_tools.py` stub module with RPC functions
3. Marcel opens a Unix domain socket and starts an RPC listener thread
4. The script runs in a child process — tool calls travel over the socket back to Marcel
5. Only the script's `print()` output is returned to the LLM; intermediate tool results never enter the context window

```python
# The agent can write scripts like:
from marcel_tools import web_search, web_extract
...
```python
from marcel_tools import search_files, read_file
import json
...
```python
from marcel_tools import search_files, read_file, patch
...
```python
from marcel_tools import terminal, read_file
import json
...
`execute_code` has two execution modes controlled by `code_execution.mode` in `~/.marcel/config.yaml`:

| Mode | Working directory | Python interpreter |
|------|-------------------|--------------------|
| **`project`** (default) | The session's working directory (same as `terminal()`) | Active `VIRTUAL_ENV` / `CONDA_PREFIX` python, falling back to Marcel's own python |
| `strict` | A temp staging directory isolated from the user's project | `sys.executable` (Marcel's own python) |
...
### `MARCEL_*` variables in the child

The child process receives only a small, fixed set of operational `MARCEL_*`
...
(plus `MARCEL_RPC_DIR` / `MARCEL_RPC_SOCKET` / `TZ` / `HOME`, which Marcel
injects explicitly so the RPC channel works).

:::note Behavior change
Earlier versions passed **any** variable whose name began with `MARCEL_`
through to the child. That broad prefix was removed for security hardening: it
could leak `MARCEL_*`-named configuration that doesn't match a secret substring
(for example `MARCEL_BASE_URL`, `MARCEL_KANBAN_DB`, or a `MARCEL_*_WEBHOOK`
endpoint) into arbitrary sandboxed code.

If an `execute_code` script — or a repo/plugin module it imports at import time
— relied on a `MARCEL_*` variable outside the four operational names above, it
...
the secret-stripping guarantee (Marcel-managed provider credentials can never
...
**Diagnosing it.** When the child drops one or more non-allowlisted `MARCEL_*`
variables, Marcel emits a one-line `debug` log naming them and pointing at the
`env_passthrough` escape hatch. Run with debug logging (`marcel logs --level
DEBUG`, or check `~/.marcel/logs/agent.log`) and look for
`execute_code: dropped N non-allowlisted MARCEL_* var(s)` if a script behaves
as though a `MARCEL_*` variable is missing.

Marcel always writes the script and the auto-generated `marcel_tools.py` RPC stub into a temp staging directory that is cleaned up after execution. In `strict` mode the script also *runs* there; in `project` mode it runs in the session's working directory (the staging directory stays on `PYTHONPATH` so imports still resolve). The child process runs in its own process group so it can be cleanly killed on timeout or interruption.
...
**Rule of thumb:** Use `execute_code` when you need to call Marcel tools programmatically with logic between calls. Use `terminal` for running shell commands, builds, and processes.

## Platform Support

Code execution is available on **Linux, macOS, and Windows**. On Linux and macOS the RPC channel uses a Unix domain socket; on Windows, where `AF_UNIX` is unreliable, Marcel automatically falls back to a loopback TCP socket for the sandbox RPC transport. Remote terminal backends (Docker/SSH/Modal/etc.) use a file-based RPC transport instead and additionally require Python 3 inside the backend.
