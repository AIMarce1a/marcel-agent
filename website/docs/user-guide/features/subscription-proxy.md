...
chat completions — use your Marcel-managed provider subscription as their
...
| Use case | "Use Marcel as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Marcel stores
the refresh token in `~/.marcel/auth.json` — the same place all Marcel
...
Model:      Marcel-4-70B    # or Marcel-4.3-36B, Marcel-4-405B
```
...
interface in `marcel_cli/proxy/adapters/`.
...
If you see `not logged in`, run `marcel portal`. If you see
...
`marcel_cli/proxy/adapters/<provider>.py` and registering it in
