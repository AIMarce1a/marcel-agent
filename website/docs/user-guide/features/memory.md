...
description: "How Marcel Agent remembers across sessions — MEMORY.md, USER.md, and session search"
---

# Persistent Memory

Marcel Agent has bounded, curated memory that persists across sessions. This lets it remember your preferences, your projects, your environment, and things it has learned.
...
- All CLI and messaging sessions are stored in SQLite (`~/.marcel/state.db`) with FTS5 full-text search
...
```bash
marcel sessions list    # Browse past sessions
```
...
The learning journey is a timeline view of everything Marcel has learned — saved skills and memory entries plotted over time (oldest at top, newest at bottom), with a playable "constellation" scrubber that replays the build-up. The same graph data drives three surfaces:

- **Classic CLI / standalone** — `marcel journey` (aliases: `marcel learning`, `marcel memory-graph`) renders the timeline in the terminal. Flags: `--play` animates the build-up (`--fps` to tune it), `--width`/`--height` override the render size, `--no-color` disables color, and `--json` dumps the raw graph payload.
...
Beyond viewing, the journey is also where you **prune and correct** what Marcel has learned:

| Command | What it does |
|---------|--------------|
| `marcel journey list` | List node ids — skill names and `memory:<source>:<index>` ids for memory chunks. |
| `marcel journey delete <node> [-y]` | Delete a node. Skills are **archived** (restorable), memory chunks are removed. `-y` skips the confirmation. |
| `marcel journey edit <node>` | Open the node's content (a skill's `SKILL.md` or the memory chunk) in `$EDITOR`. |
...
or update a skill. This is Marcel' consent-aware learning loop: repeated
...
`~/.marcel/pending/skills/<id>.json` when you want to read the whole change.
Full details in [Gating agent skill writes](/user-guide/features/skills#gating-agent-skill-writes-skillswrite_approval).


## External Memory Providers

For deeper, persistent memory that goes beyond MEMORY.md and USER.md, Marcel ships with 8 external memory provider plugins — including Honcho, OpenViking, Mem0, Hindsight, Holographic, RetainDB, ByteRover, and Supermemory.

External providers run **alongside** built-in memory (never replacing it) and add capabilities like knowledge graphs, semantic search, automatic fact extraction, and cross-session user modeling.

```bash
marcel memory setup      # pick a provider and configure it
