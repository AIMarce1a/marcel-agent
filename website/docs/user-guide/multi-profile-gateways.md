...
If you only run one Marcel agent, you don't need this page — see
...
[Connecting Desktop to Many Marcel Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Marcel agents that should all
...
(`ai.marcel.gateway-<name>.plist`) or systemd user service
(`marcel-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
marcel profile create coder
marcel profile create personal-bot
marcel profile create research
...
```bash
marcel config set gateway.multiplex_profiles true
marcel gateway restart
```

Equivalently, in the default profile's `~/.marcel/config.yaml`:
...
You do **not** run `marcel gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `marcel gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.marcel/profiles/coder/.env`; the default listener key is rejected.
...
home). `marcel status` reports the multiplexer and the profiles it serves;
`marcel status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/marcel-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: marcel-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    marcel gateway "$action"
  else
    marcel -p "$profile" gateway "$action"
  fi
}
...
```bash
marcel-gateways start      # start every configured profile
marcel-gateways stop       # stop every configured profile
marcel-gateways restart    # restart all
marcel-gateways status     # status across all
marcel-gateways list       # delegates to `marcel gateway list`
```

:::tip
The `default` profile is targeted with `marcel gateway <action>` (no `-p`),
not `marcel -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `marcel -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.marcel.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/marcel-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.marcel.gateway.plist` /
`marcel-gateway.service`.
...
```bash
# Default profile
tail -f ~/.marcel/logs/gateway.log
tail -f ~/.marcel/logs/gateway.error.log

# Named profile
tail -f ~/.marcel/profiles/<name>/logs/gateway.log
tail -f ~/.marcel/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.marcel/logs/gateway.log ~/.marcel/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
marcel logs -f                  # follow default profile
marcel -p coder logs -f         # follow one profile
marcel logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
marcel profile list             # profiles + model + gateway state
marcel-gateways status          # full status across every profile
launchctl list | grep marcel    # macOS — PIDs and labels
systemctl --user list-units 'marcel-gateway-*'   # Linux — units
```
...
```
~/.marcel/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.marcel/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
marcel config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.marcel/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=marcel --why="gateways running" \
  sleep infinity &
...
`marcel-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.marcel/.env ~/.marcel/profiles/*/.env
```

## Updating the code

`marcel update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `marcel gateway start` after a previous `marcel gateway stop`. The
...
```bash
ps -ef | grep "marcel_cli.*-p <profile>"
cat ~/.marcel/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.marcel.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.marcel.gateway-<profile>.plist

# Linux
systemctl --user restart marcel-gateway-<profile>.service
```

### Health check

```bash
marcel doctor                  # default profile
marcel -p <profile> doctor     # one profile
```
