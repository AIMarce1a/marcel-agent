...
`~/.marcel/config.yaml`, `~/.marcel/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/marcel`:

```text
/etc/marcel/
├── config.yaml     # managed config layer (wins over ~/.marcel/config.yaml)
└── .env            # managed env layer (wins over ~/.marcel/.env + shell)
```
...
The location can be relocated with the `MARCEL_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `MARCEL_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Marcel.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export MARCEL_MANAGED_DIR=/opt/org/marcel-policy
```

:::warning
A user who can set `MARCEL_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `marcel doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/marcel/config.yaml` (managed) | `/etc/marcel/.env` (managed) |
| 2 | `~/.marcel/config.yaml` (user) | `~/.marcel/.env` (user) |
...
```bash
marcel config        # shows a header naming the managed source + the pinned keys
marcel doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Marcel refuses and names the source:

```bash
$ marcel config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/marcel/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `marcel config set` / setup will not write
...
sudo tee /etc/marcel/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/marcel/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/marcel
sudo chmod 0644 /etc/marcel/config.yaml /etc/marcel/.env
```

Changes take effect on the next Marcel start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`marcel doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Marcel as `root`), managed scope is advisory.
