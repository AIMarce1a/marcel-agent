...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`marcel-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-marcel-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Marcel terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Marcel itself, `model.provider: openai-codex` uses Marcel-managed Codex
OAuth from `~/.marcel/auth.json` after `marcel auth add openai-codex`. For the
...
## Marcel Gateway Caveat

When invoking the Codex CLI from a Marcel gateway/service context (for example,
