...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _marcel_env="${MARCEL_HOME:-$HOME/.marcel}/.env"; [ -f "$_marcel_env" ] && grep -q "^GITHUB_TOKEN=" "$_marcel_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_marcel_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
    elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
      GITHUB_TOKEN=$(uv run python3 "${MARCEL_HOME:-$HOME/.marcel}/skills/github/github-auth/scripts/git-credential-token.py")
    fi
  fi
fi
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Marcel Agent\",
    \"comments\": [
...
```bash
source "${MARCEL_HOME:-$HOME/.marcel}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Marcel Agent. Code looks clean — good test coverage, no security concerns."
