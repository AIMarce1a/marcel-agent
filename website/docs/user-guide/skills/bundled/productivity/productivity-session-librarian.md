...
| List/filter by metadata (age, source, cost, tokens, workspace) | `marcel sessions list` / `stats` via terminal |
| Rename | `marcel sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `marcel sessions archive <filters>` |
| Delete (destructive) | `marcel sessions delete` / `marcel sessions prune <filters>` |
| Export before deleting anything valuable | `marcel sessions export --session-id <id> --format md` |
...
`marcel sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `marcel sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `marcel sessions list`
