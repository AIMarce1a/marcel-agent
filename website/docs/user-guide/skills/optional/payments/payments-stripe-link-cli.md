...
| | |
|---|---|
| Source | Optional — install with `marcel skills install official/payments/stripe-link-cli` |
| Path | `optional-skills/payments\stripe-link-cli` |
| Version | `0.1.0` |
| Author | Teknium (teknium1), Marcel Agent |
| License | MIT |
...
Wraps [@stripe/link-cli](https://github.com/stripe/link-cli) so Marcel can complete purchases on the user's behalf using one-time-use virtual cards or Shared Payment Tokens (SPT). Every spend is gated by an in-app approval in the Link mobile/web app — Marcel cannot self-approve.
...
The Link account, payment method, and spend-approval app do NOT need to be set up before Marcel attempts to pay — the CLI walks the user through them on first run:
...
```
link-cli auth login --client-name "Marcel" --interval 5 --timeout 300
```
...
`@stripe/link-cli --mcp` exposes the same commands as MCP tools over stdio. To register it with Marcel' native MCP:

```
marcel mcp add stripe-link --command "npx" --args "@stripe/link-cli --mcp"
```

Then `marcel mcp list` should show `stripe-link`. The same approval rules apply — MCP doesn't bypass the Link app approval step.
