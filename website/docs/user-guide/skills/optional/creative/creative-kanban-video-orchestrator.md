...
| | |
|---|---|
| Source | Optional — install with `marcel skills install official/creative/kanban-video-orchestrator` |
...
short to a music video to an ASCII loop — in a Marcel Kanban pipeline that
...
3. **Generates** a setup script that creates Marcel profiles, project workspace, and the initial kanban task
...
For mapping role → which Marcel skills + toolsets it loads, see
...
3. Creates each Marcel profile via `marcel profile create --clone`
...
7. Fires the initial `marcel kanban create` task assigned to the director
...
```bash
marcel kanban watch --tenant <project-tenant>     # live events
marcel kanban list  --tenant <project-tenant>     # board snapshot
marcel dashboard                                   # visual board UI
```
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `marcel kanban create` call to a specialist profile. The kanban
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${MARCEL_HOME:-~/.marcel}/.env` or the user's secret store.
