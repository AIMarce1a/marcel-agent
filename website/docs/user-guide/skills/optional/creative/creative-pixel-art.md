...
| | |
|---|---|
| Source | Optional — install with `marcel skills install official/creative/pixel-art` |
...
import sys
import os
sys.path.insert(0, os.path.expanduser("~/.marcel/skills/creative/pixel-art/scripts"))
...
```bash
cd ~/.marcel/skills/creative/pixel-art/scripts
...
- ffmpeg on PATH (only needed for video — Marcel installs package this)
