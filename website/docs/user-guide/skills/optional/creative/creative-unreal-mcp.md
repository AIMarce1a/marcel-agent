...
| | |
|---|---|
| Source | Optional — install with `marcel skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Marcel MCP catalog. The
...
Two halves, in this order: the editor side must be up before Marcel connects.
...
### One-time, Marcel side

    marcel mcp install unreal-engine
...
`~/.marcel/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Marcel — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Marcel connects
...
2. Start the Marcel session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Marcel
they appear as:
...
then happens at `marcel mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Marcel runs batched calls
...
- **Start order matters.** Editor + server up first, then the Marcel
...
- **Timeouts on long operations.** Marcel' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.marcel/config.yaml` for
