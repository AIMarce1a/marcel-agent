...
| | |
|---|---|
| Source | Optional — install with `marcel skills install official/creative/touchdesigner-mcp` |
...
```
Marcel Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${MARCEL_HOME:-$HOME/.marcel}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Marcel config (if missing)
...
3. **Restart Marcel session** to pick up the new MCP server
