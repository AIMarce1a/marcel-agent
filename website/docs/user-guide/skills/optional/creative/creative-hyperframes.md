...
| | |
|---|---|
| Source | Optional — install with `marcel skills install official/creative/hyperframes` |
...
```bash
bash "$(dirname "$(find ~/.marcel/skills -path '*/hyperframes/SKILL.md' 2>/dev/null | head -1)")/scripts/setup.sh"
```
