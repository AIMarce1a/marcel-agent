...
| | |
|---|---|
| Source | Optional — install with `marcel skills install official/creative/baoyu-comic` |
...
Adapted from [baoyu-comic](https://github.com/JimLiu/baoyu-skills) for Marcel Agent's tool ecosystem.
...
Marcel' `image_generate` tool is **prompt-only** — it accepts a text prompt and an aspect ratio, and returns an image URL. It does **NOT** accept reference images. When the user supplies a reference image, use it to **extract traits in text** that get embedded in every page prompt:
...
Use Marcel' built-in `image_generate` tool for all image rendering. Its schema accepts only `prompt` and `aspect_ratio` (`landscape` | `portrait` | `square`); it **returns a URL**, not a local file. Every generated page or character sheet must therefore be downloaded to the output directory.
