...
| | |
|---|---|
| Source | Optional — install with `marcel skills install official/mcp/fastmcp` |
...
| Related skills | [`marcel-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-marcel-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Marcel or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Marcel. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.marcel/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.marcel/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Marcel integration, either:

- configure the server in `~/.marcel/config.yaml` using the `native-mcp` skill, or
...
### Marcel cannot see the deployed server

The server-building part may be correct while the Marcel config is not. Load the `native-mcp` skill and configure the server in `~/.marcel/config.yaml`, then restart Marcel.
