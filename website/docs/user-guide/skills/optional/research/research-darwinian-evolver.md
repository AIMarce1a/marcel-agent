...
| | |
|---|---|
| Source | Optional — install with `marcel skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research\darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Marcel Agent |
| License | MIT |
...
import upstream classes into Marcel itself.
...
```bash
mkdir -p ~/.marcel/cache/darwinian-evolver && cd ~/.marcel/cache/darwinian-evolver
...
```bash
cd ~/.marcel/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.marcel/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.marcel/skills/research/darwinian-evolver
DE_DIR=~/.marcel/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Marcel core.
   Custom driver scripts under `~/.marcel/skills/...` are user-side and fine.
