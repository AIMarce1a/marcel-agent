...
| | |
|---|---|
| Source | Optional — install with `marcel skills install official/productivity/shop` |
| Path | `optional-skills/productivity\shop` |
| Version | `1.0.1` |
| Author | Joe Rinaldi Johnson (joerj123), Marcel Agent |
| License | MIT |
...
| Telegram (Marcel Agent + all other agents) | Do **not** send an image. Send separate messages — never one combined message. |
