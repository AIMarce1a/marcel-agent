...
| | |
|---|---|
| Source | Optional — install with `marcel skills install official/productivity/telephony` |
...
This optional skill gives Marcel practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${MARCEL_HOME:-~/.marcel}/.env`
...
It does **not** turn Marcel into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Marcel memory
...
### 1) "I want Marcel to own a real phone number"
Use **Twilio**.
...
- pairs well with Marcel `text_to_speech` plus a public file host or tunnel

## Files and persistent state

The skill persists telephony state in two places:

### `${MARCEL_HOME:-~/.marcel}/.env`
...
```bash
SCRIPT="$(find ~/.marcel/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
marcel skills search telephony
marcel skills install official/productivity/telephony
```
...
Then save credentials into Marcel:
...
3. Buy it and save it into `${MARCEL_HOME:-~/.marcel}/.env` + state:
```bash
python "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Marcel calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Marcel's existing `text_to_speech` support.

Use this when:
- you want the call to use Marcel's configured TTS voice rather than Twilio `<Say>`
...
Recommended Marcel TTS -> Twilio Play workflow:

1. Generate the audio with Marcel `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Marcel to create the message audio with `text_to_speech`
...
Important note:
- Marcel TTS is great for prerecorded outbound messages
...
- Marcel STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Marcel memory.
...
- Do not store arbitrary third-party phone numbers in Marcel memory.
...
3. persist that number to `${MARCEL_HOME:-~/.marcel}/.env`
