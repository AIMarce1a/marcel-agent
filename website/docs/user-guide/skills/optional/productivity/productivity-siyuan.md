...
| | |
|---|---|
| Source | Optional — install with `marcel skills install official/productivity/siyuan` |
...
3. Store it in `${MARCEL_HOME:-~/.marcel}/.env`:
   ```
...
```yaml
# In ~/.marcel/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
