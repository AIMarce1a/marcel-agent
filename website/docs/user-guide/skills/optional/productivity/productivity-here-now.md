...
| | |
|---|---|
| Source | Optional — install with `marcel skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${MARCEL_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${MARCEL_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${MARCEL_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client marcel
```
...
```bash
PUBLISH="${MARCEL_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client marcel
```
...
```bash
DRIVE="${MARCEL_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `marcel`)    |
