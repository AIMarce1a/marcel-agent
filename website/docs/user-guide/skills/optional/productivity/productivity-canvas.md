...
| | |
|---|---|
| Source | Optional — install with `marcel skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Marcel Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${MARCEL_HOME:-~/.marcel}/.env`:
...
```bash
CANVAS="python $MARCEL_HOME/skills/productivity/canvas/scripts/canvas_api.py"
