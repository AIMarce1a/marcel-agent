...
| | |
|---|---|
| Source | Optional — install with `marcel skills install official/security/1password` |
| Path | `optional-skills/security\1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Marcel Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Marcel terminal calls (desktop app flow only)
...
### Service Account (recommended for Marcel)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${MARCEL_HOME:-~/.marcel}/.env` (the skill will prompt for this on first load).
...
## Marcel Execution Pattern (desktop app flow)

Marcel terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/marcel-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/marcel-op.sock"
