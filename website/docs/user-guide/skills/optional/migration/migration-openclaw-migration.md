---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Marcel"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into Marcel"
---
...
Import an OpenClaw setup (memories, skills) into Marcel.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `marcel skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration\openclaw-migration` |
| Version | `1.0.0` |
| Author | Marcel Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Marcel`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Marcel Migration

Use this skill when a user wants to move their OpenClaw setup into Marcel Agent with minimal manual cleanup.
...
```bash
marcel claw migrate              # Full interactive migration
marcel claw migrate --dry-run    # Preview what would be migrated
marcel claw migrate --preset user-data   # Migrate without secrets
marcel claw migrate --overwrite  # Overwrite existing conflicts
marcel claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `marcel setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_marcel.py` to:

- import `SOUL.md` into the Marcel home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Marcel memory entries
- merge OpenClaw command approval patterns into Marcel `command_allowlist`
- migrate Marcel-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Marcel working-directory configuration
- copy OpenClaw skills into `~/.marcel/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Marcel workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.marcel/tts/`
- archive non-secret docs that do not have a direct Marcel destination
...
- `scripts/openclaw_to_marcel.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.marcel/skills/migration/openclaw-migration/scripts/openclaw_to_marcel.py`

Do not guess a shorter path like `~/.marcel/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.marcel/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Marcel-compatible secrets, currently:
...
Marcel CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Marcel. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Marcel workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Marcel skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.marcel/skills/migration/openclaw-migration/scripts/openclaw_to_marcel.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.marcel/skills/migration/openclaw-migration/scripts/openclaw_to_marcel.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.marcel/skills/migration/openclaw-migration/scripts/openclaw_to_marcel.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.marcel/skills/migration/openclaw-migration/scripts/openclaw_to_marcel.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.marcel/skills/migration/openclaw-migration/scripts/openclaw_to_marcel.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.marcel/skills/migration/openclaw-migration/scripts/openclaw_to_marcel.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Marcel unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Marcel targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Marcel destination. Unsupported auth blobs must still be reported as skipped.
...
- Marcel persona state imported
- Marcel memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.marcel/skills/openclaw-imports/`
