...
| | |
|---|---|
| Source | Optional — install with `marcel skills install official/finance/3-statement-model` |
...
Many passages below say "use the S&P Kensho MCP / Daloopa MCP / FactSet MCP". Those are commercial financial-data MCPs from the original Cowork plugin context. In Marcel:

- **If you have any structured financial-data MCP configured** (Marcel supports MCP — see `native-mcp` skill), prefer it for point-in-time comps, precedent transactions, and filings.
