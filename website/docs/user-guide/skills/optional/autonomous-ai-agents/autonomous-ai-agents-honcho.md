---
title: "Honcho — Configure and troubleshoot Honcho memory for Marcel"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Marcel"
---
...
Configure and troubleshoot Honcho memory for Marcel.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `marcel skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Marcel

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Marcel profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/marcel#running-honcho-locally-with-marcel

### Verify

```bash
marcel honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Marcel profile's AI peer
...
Honcho models conversations as interactions between **peers**. Marcel creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Marcel instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Marcel run |
| `global` | Single session across all directories |

Manual override: `marcel honcho map my-project-name`
...
Each Marcel profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block marcel.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `marcel.coder` host block in `honcho.json`
...
```bash
marcel honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Marcel when Honcho memory is active.
...
Config file: `$MARCEL_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `marcel honcho setup`. Ensure `memory.provider: honcho` is in `~/.marcel/config.yaml`.

### Memory not persisting across sessions
Check `marcel honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `marcel profile create <name> --clone`. For existing profiles: `marcel honcho sync`.
...
| Command | Description |
|---------|-------------|
| `marcel honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `marcel honcho status` | Show resolved config, connection test, peer info for active profile |
| `marcel honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `marcel honcho disable` | Disable Honcho for the active profile |
| `marcel honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `marcel honcho peers` | Show peer identities across all profiles |
| `marcel honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `marcel honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `marcel honcho sessions` | List known directory-to-session-name mappings |
| `marcel honcho map <name>` | Map current working directory to a Honcho session name |
| `marcel honcho identity` | Seed AI peer identity or show both peer representations |
| `marcel honcho sync` | Create host blocks for all Marcel profiles that don't have one yet |
| `marcel honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Marcel + Honcho |
| `marcel memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `marcel memory status` | Show active memory provider and config |
| `marcel memory off` | Disable external memory provider |
