---
title: "Bot Mode"
description: "Turn your Marcel profiles into a roster of named Bots — each with its own chat, role, model, memory, skills, and avatar. Bots run routines, share group chats, and message each other."
---

# Bot Mode

**Bot Mode** turns your [Marcel profiles](./profiles.md) into a roster of named **Bots**. Each Bot has its own role, model, memory, skills, and avatar; Bots run recurring routines, deliberate together in group chats, and message each other directly. Build a specialist Bot once and it is there forever, one click away.
...
There is no new primitive to learn: a Bot **is** a Marcel profile — isolated config, memory, skills, credentials, and chat history under `~/.marcel/profiles/<name>/`. Bot Mode is a UI over that primitive, so everything you do in it is visible from the CLI too: `marcel -p <bot> chat` opens the same agent, and Bot routines appear in `marcel cron list`. No core patches, no background daemons, no extra storage.
:::
...
- **Model & provider pin** — give the Bot its own model. Any provider/model pair Marcel knows about works, and different Bots can run on different models side by side. Leave it unset to inherit from the launch profile.
...
- **A pixel pet** — a companion from the [petdex gallery](./features/pets.md) that bounces beside the avatar while the Bot is busy. Run `marcel pets` in a terminal to explore the gallery.
...
Every gateway you register in **Settings → Connections** — local, remote URL, SSH, Marcel Cloud, docker — is a persistent line the Desktop holds open, and Bot Mode uses those lines for messaging automatically. No extra setup:
...
```bash
marcel peer add spark --url http://spark.lan:8377 --key <API_SERVER_KEY>
marcel peer list
marcel peer dm spark < /tmp/dm.txt        # message body from a file (nothing shell-interpreted)
marcel peer dm spark/researcher < /tmp/dm.txt   # named profile on a multiplexed peer
marcel peer run spark --idempotency-key ticket-123 < /tmp/long-task.txt
marcel peer status spark run_abc123
marcel peer stop spark run_abc123
```

`marcel peer dm` delivers into the remote agent's canonical Bot Chat over the peer's existing API server, runs one agent turn there, and prints the reply on stdout — the exact cross-machine twin of the local `marcel -p <bot> chat` command.
...
Requirements: the peer machine runs the `api_server` gateway platform with a strong `API_SERVER_KEY`; reachability is your network's business (LAN, Tailscale, VPN). The key is a credential and lives in `~/.marcel/.env` as `MARCEL_PEER_<NAME>_KEY`; peer names/URLs live in `config.yaml` under `bot_peers`.
...
| --- | --- |
| Chat with a Bot | `marcel -p <bot> chat` |
| A Bot's files, skills, memory | `~/.marcel/profiles/<bot>/` |
| Routines | `marcel cron list` (jobs named `[bot:<name>] …`) |
| Create / inspect profiles | `marcel profile create`, `marcel profile list` |
