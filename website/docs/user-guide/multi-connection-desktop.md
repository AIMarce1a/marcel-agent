...
# Connecting Desktop to Many Marcel Instances

Register every Marcel backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and Marcel Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another Marcel gateway…"**) deep-links straight to
...
list of Marcel gateways. Its intro says it plainly: *"Manage this device and
every Marcel gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The Marcel runtime managed by this app." | automatic |
| **Remote gateway** | "A Marcel gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A Marcel install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **Marcel Cloud** | "A hosted instance discovered through your Marcel Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the Marcel Cloud sign-in/discovery flow at
  the top of the Gateways page — the **Marcel Cloud** kind in the add-connection
...
3. Pick the kind: **Local**, **Marcel Cloud**, **Remote gateway**, or **SSH**.
   (**Local** is disabled while the app-managed local entry exists — which is
   almost always; **Marcel Cloud** directs you to the cloud sign-in/discovery
   flow above.)
4. Fill the fields:
   - **Name** — required, unique; the "device name" shown everywhere this
     instance appears (placeholder: `Homelab`). Max 64 characters.
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `marcel serve` backend,
...
Gateways are machines or hosted backends; profiles are isolated Marcel agents
...
connection is registered) dispatches `marcel update` to every eligible
...
- **Marcel Cloud** instances are skipped with a *"Managed by Marcel Cloud"*
...
panel, ⌘K **Update Marcel**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `marcel serve` is running on the remote host, the
...
- **"Update Marcel Desktop to chat with agents on other connections"** — the
