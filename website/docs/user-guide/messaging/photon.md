...
Connect Marcel to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Marcel — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Marcel runs it in a
...
marcel photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Marcel Agent` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.marcel/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.marcel/.env`
...
(device token, dashboard project id) lives in `~/.marcel/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Marcel
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Marcel replies with a pairing code. Approve it with:

```bash
marcel pairing approve photon <CODE>
```

Use `marcel pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.marcel/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.marcel/.env`):
...
By default Marcel responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Marcel` and
`@Marcel agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Marcel will reply.

## Status & troubleshooting

```bash
marcel photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Marcel uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run marcel photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `marcel photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `marcel photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `marcel photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Marcel sends images, voice
  ...
  media.
- **Native polls are supported.** Marcel sends poll content through
...
- **Read receipts are supported.** The sidecar marks an inbound iMessage
  read after forwarding it to Marcel, so the sender sees `Read` without
  waiting for a model/tool turn. Inbound receipts for Marcel-sent messages
...
- **Message effects are supported.** Marcel sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `marcel send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<marcel-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Marcel is set
...
| `PHOTON_MENTION_PATTERNS` | Marcel wake words  | JSON list / comma / newline regex patterns for group mentions |
