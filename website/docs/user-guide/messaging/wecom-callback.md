...
Connect Marcel to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Marcel supports two WeCom integration modes:
...
> Run `marcel gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Marcel decrypts the message, queues it for the agent
...
(Use `marcel gateway start` only after `marcel gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Marcel and the token the
...
in `~/.marcel/.env` values around `=` will also break signature checks. After
fixing, restart `marcel gateway run`.
...
Check `marcel gateway run` logs for the bound host/port. If the adapter bound to
