...
description: "Run Marcel Agent on Windows via WSL2 — setup, filesystem access between Windows and Linux, networking, and common pitfalls"
...
Marcel Agent now supports **both** native Windows and WSL2.  This page covers the WSL2 path; for the native PowerShell install see the dedicated **[Windows (Native) Guide](./windows-native.md)**.
...
- You're doing POSIX-heavy development work and want your Marcel sessions to share the same filesystem / paths as your dev tools.
- You already have a WSL2 environment and don't want to maintain a second install.

**When native is fine (or better):**
- Interactive chat, gateway (Telegram/Discord/etc.), cron scheduler, browser tool, MCP servers, and most Marcel features all run natively on Windows.
...
This guide covers the parts of that split that specifically affect Marcel: installing WSL2, getting files back and forth between Windows and Linux, networking in both directions, and the pitfalls people actually hit.
...
Marcel does not work reliably on WSL1 — WSL1 translates Linux syscalls on the fly and some behaviors (procfs, signals, network) diverge from real Linux.
...
The marcel gateway (and anything else you want to keep running) is easier to manage with systemd. On modern WSL, enable it once inside your distro:
...
### Where to put Marcel and your projects

**Rule of thumb: keep everything Linux-ish inside the Linux filesystem.**

- Your Marcel install (`~/.marcel/`) — Linux side. The installer already does this.
...
Clone inside WSL. Always, unless you have a specific reason not to. A typical Marcel workflow (`marcel chat`, tool calls that `rg`/`ripgrep` the repo, file watchers, background gateway) will be dramatically faster and more reliable against `~/code/myrepo` than `/mnt/c/Users/you/myrepo`.

One exception: **MCP bridges that launch Windows binaries.** If you're using `chrome-devtools-mcp` through `cmd.exe` (see [MCP guide: WSL → Windows Chrome](/guides/use-mcp-with-marcel#wsl2-bridge-marcel-in-wsl-to-windows-chrome)), Windows may complain with a `UNC` warning if Marcel's current working directory is `~`. In that case, start Marcel from somewhere under `/mnt/c/` so the Windows process has a drive-letter cwd.
...
### Case 1 — Marcel in WSL talks to a service on Windows

Most common: you're running **Ollama, LM Studio, or a llama-server on Windows**, and Marcel (inside WSL) needs to hit it.
...
### Case 2 — Something on Windows (or your LAN) talks to Marcel in WSL

This is the reverse direction and is less documented elsewhere, but it's what you need for:

- Using the Marcel **web dashboard** from a Windows browser.
- Using the **OpenAI-compatible API server** (exposed by `marcel gateway` when `API_SERVER_ENABLED=true`) from a Windows-side tool. See the [API Server feature page](/user-guide/features/api-server).
...
On **NAT mode** (Windows 10 / older Windows 11), the default "localhost forwarding" in WSL2 will generally forward Linux-side `127.0.0.1` binds to Windows `localhost`, so a Marcel service started with `--host 127.0.0.1` is usually reachable as `http://localhost:PORT` from Windows. If it isn't:
...
2. **Port-forward Windows → WSL VM.** In mirrored mode this is automatic. In NAT mode you have to do it yourself, per port, in Admin PowerShell:
   ...
   New-NetFirewallRule -DisplayName "Marcel WSL 8080" `
...
## Running Marcel services long-term on Windows

The Marcel [Tool Gateway](/user-guide/features/tool-gateway) and the API server are long-lived processes. In WSL2 you have a few options for keeping them up.

### Desktop shortcut for opening Marcel quickly

If you just want a double-click launcher for an interactive Marcel shell, create
...
2. For the target, use your distro name (replace `Ubuntu` if needed):
   ...
   ```text
   wt.exe -w 0 -p "Ubuntu" wsl.exe -d Ubuntu --cd ~ -- bash -ic "marcel"
   ```

3. Name it something obvious like `Marcel`.

That opens Windows Terminal, starts your WSL distro, drops you in your Linux
home directory, and launches Marcel. If `marcel` is not on PATH yet, open WSL
once manually and run `source ~/.bashrc`, or replace the command with
`uv run marcel` inside your project checkout.

Optional polish:

- **Custom icon:** open **Properties -> Change Icon** and point it at an `.ico`
  file, such as the Marcel favicon from the repo.
...
If you enabled systemd per the setup section above, `marcel gateway` and the API server work the way they do on any Linux machine. Use the gateway setup wizard:
...
AMD ROCm and Intel Arc support inside WSL2 is still evolving and outside Marcel's test matrix — it may work with current drivers but we don't have a recipe to recommend.
...
**Massive slowness on `git status` / `marcel chat` in a repo.**
...
Marcel's cwd is inside the Linux filesystem, and Windows `cmd.exe` doesn't know what to do with it. Start Marcel from `/mnt/c/...` for that session, or use a wrapper that `cd`s to a Windows-reachable path before invoking the Windows executable.
...
**`marcel` not found after running the installer.**
...
- **[MCP guide → WSL → Windows Chrome](/guides/use-mcp-with-marcel#wsl2-bridge-marcel-in-wsl-to-windows-chrome)** — controlling your signed-in Windows Chrome from Marcel in WSL.
