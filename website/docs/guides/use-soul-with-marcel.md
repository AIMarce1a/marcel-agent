---
sidebar_position: 7
title: "Use SOUL.md with Marcel"
description: "How to use SOUL.md to shape Marcel Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Marcel

`SOUL.md` is the **primary identity** for your Marcel instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Marcel to feel like the same assistant every time you talk to it — or if you want to replace the Marcel persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Marcel should be
- what Marcel should avoid stylistically
- how Marcel should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Marcel is and how Marcel speaks
...
Marcel now uses only the global SOUL file for the current instance:

```text
~/.marcel/SOUL.md
```

If you run Marcel with a custom home directory, it becomes:
...
Marcel automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Marcel does not overwrite it
- if the file exists but is empty, Marcel adds nothing from it to the prompt

## How Marcel uses it

When Marcel starts a session, it reads `SOUL.md` from `MARCEL_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Marcel feels.
...
Marcel already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Marcel should sound.

# Avoid
What Marcel should not do.

# Defaults
How Marcel should behave when ambiguity appears.
```
...
Then restart Marcel or start a new session.
...
### I edited SOUL.md but Marcel still sounds the same

Check:
- you edited `~/.marcel/SOUL.md` or `$MARCEL_HOME/SOUL.md`
...
### Marcel is ignoring parts of my SOUL.md
