...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Marcel Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Marcel instantly when PRs are opened or updated.
:::
...
- **Marcel Agent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  marcel gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.marcel/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Marcel can access GitHub. Start a chat:
...
Still in the chat, ask Marcel to review a real PR:
...
A skill gives Marcel consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.marcel/skills/code-review
```

Create `~/.marcel/skills/code-review/SKILL.md`:
...
Verify it loaded — start `marcel` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Marcel your team's standards:
...
2. Teach Marcel your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
marcel gateway status    # Is the gateway running?
marcel cron list         # Is the job enabled?
```
