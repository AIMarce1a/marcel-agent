...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `marcel send`."
---

# Pipe Script Output to Messaging Platforms

`marcel send` is a small, scriptable CLI that pushes a message to any
messaging platform Marcel is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | marcel send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `marcel
...
marcel send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
marcel send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
marcel send --list

# Filter by platform
marcel send --list telegram
```
...
Any platform Marcel ships adapters for works as a target:
...
`marcel send` resolves the message body in this order:

1. **Positional argument** — `marcel send --to telegram "hi"`
2. **`--file PATH`** — `marcel send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | marcel send --to telegram`

When stdin is a TTY (no pipe), Marcel does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  marcel send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `marcel send` reuses your Marcel config, the same script works on
any host where Marcel is installed — no need to export bot tokens into
...
`marcel send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  marcel send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | marcel send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.marcel/bin/marcel send \
...
```bash
./train.py --epochs 200 && \
  marcel send --to telegram "training done" || \
  marcel send --to telegram "training failed (exit $?)"
```
...
marcel send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(marcel send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `marcel send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `marcel send` calls
the platform's REST endpoint directly using credentials from
`~/.marcel/.env` and `~/.marcel/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `marcel gateway start` and retry.
...
# Machine-readable
marcel send --list --json
```

The listing is built from `~/.marcel/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`marcel gateway
...
| Approach | Multi-platform | Reuses Marcel creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `marcel send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`marcel send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `marcel send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `marcel send` shares with cron delivery.
