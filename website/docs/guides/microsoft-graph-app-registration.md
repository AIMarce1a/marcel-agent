...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.marcel/.env` at the end.
...
4. Fill in:
   - **Name:** `Marcel Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `marcel-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
Put the three values you collected into `~/.marcel/.env`:
...
Marcel ships a Graph auth smoke-test. From your Marcel install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.marcel/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `marcel gateway restart`.
