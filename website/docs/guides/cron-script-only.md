...
Marcel calls this **no-agent mode**. It's the cron system minus the LLM.
...
The real win of no-agent mode is that the agent itself can set up the watchdog for you — no editor, no shell, no remembering CLI flags. You describe what you want, Marcel writes the script, schedules it, and tells you when it'll fire.

### Example transcript

> **You:** ping me on telegram if RAM is over 85% every 5 minutes
>
> **Marcel:** *(writes `~/.marcel/scripts/memory-watchdog.sh`, then calls `cronjob(...)` with `no_agent=true`)*
>
...
write_file(
    path="~/.marcel/scripts/memory-watchdog.sh",
...
>
> **Marcel:** *(calls `cronjob(action='pause', job_id='abc123')`)*
>
> Paused. Resume with "turn it back on" or via `marcel cron resume abc123`.

> **You:** change it to every 15 minutes
>
> **Marcel:** *(calls `cronjob(action='update', job_id='abc123', schedule='every 15m')`)*
...
```bash
# 1. Write your script
cat > ~/.marcel/scripts/memory-watchdog.sh <<'EOF'
#!/usr/bin/env bash
...
EOF
chmod +x ~/.marcel/scripts/memory-watchdog.sh
...
# 3. Verify
marcel cron list
marcel cron run <job_id>    # fire it once to test
```
...
Scripts must live in `~/.marcel/scripts/`. This is enforced at both job-creation time and run time — absolute paths, `~/` expansion, and path-traversal patterns (`../`) are rejected. The same directory is shared with the pre-check script gate used by LLM jobs.
...
```bash
marcel cron create "every 5m"        # interval
marcel cron create "every 2h"
marcel cron create "0 9 * * *"       # standard cron: 9am daily
marcel cron create "30m"             # one-shot: run once in 30 minutes
```
...
--deliver local                          # just save to ~/.marcel/cron/output/
```

No running gateway is required at script-run time for bot-token platforms (Telegram, Discord, Slack, Signal, SMS, WhatsApp) — the tool calls each platform's REST endpoint directly using the credentials already in `~/.marcel/.env` / `~/.marcel/config.yaml`.

## Editing and Lifecycle

```bash
marcel cron list                                    # see all jobs
marcel cron pause <job_id>                          # stop firing, keep definition
marcel cron resume <job_id>
marcel cron edit <job_id> --schedule "every 10m"    # adjust cadence
marcel cron edit <job_id> --agent                   # flip to LLM mode
marcel cron edit <job_id> --no-agent --script …     # flip back
marcel cron remove <job_id>                         # delete it
```
...
```bash
cat > ~/.marcel/scripts/disk-alert.sh <<'EOF'
#!/usr/bin/env bash
...
'
EOF
chmod +x ~/.marcel/scripts/disk-alert.sh

marcel cron create "*/15 * * * *" \
  --no-agent \
...
|----------|-----------|-------------|
| `cronjob --no-agent` (this page) | Your script on Marcel' schedule | Recurring watchdogs / alerts / metrics that don't need reasoning |
| `cronjob` (default, LLM) | Agent with optional pre-check script | When the message content requires reasoning over data |
| OS cron + `curl` to a [webhook subscription](/user-guide/messaging/webhooks) | Your script on the OS schedule | When Marcel might be unhealthy (the thing you're monitoring) |

For critical system-health watchdogs that must fire *even when the gateway is down*, use OS-level cron with a plain `curl` to a Marcel webhook subscription (or any external alerting endpoint) — those run as independent OS processes and don't depend on Marcel being up. The in-gateway scheduler is the right choice when the thing being monitored is external.
