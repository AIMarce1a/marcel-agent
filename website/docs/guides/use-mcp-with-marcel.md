---
sidebar_position: 6
title: "Use MCP with Marcel"
description: "A practical guide to connecting MCP servers to Marcel Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Marcel

This guide shows how to actually use MCP with Marcel Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Marcel tool
- you want Marcel to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Marcel to internal APIs, databases, or company systems without modifying Marcel core

Do not use MCP when:
- a built-in Marcel tool already solves the job well
...
- Marcel remains the agent
- MCP servers contribute tools
- Marcel discovers those tools at startup or reload time
...
If you installed Marcel with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Marcel banner/status should show MCP integration when configured
- ask Marcel what tools it has available
...
## WSL2: bridge Marcel in WSL to Windows Chrome

This is the practical setup when:

- Marcel runs inside WSL2
...
In this setup, Marcel does **not** connect to Chrome directly. Instead:

- Marcel runs in WSL
- Marcel starts a local stdio MCP server
...
```text
Marcel (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Marcel stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Marcel core browser transport
...
```bash
marcel mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
marcel mcp test chrome-devtools-win
```

Then start a fresh Marcel session or run:
...
Once loaded, Marcel can use the MCP-prefixed browser tools directly. For example:
...
If Marcel runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Marcel from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Marcel from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Marcel:
...
2. Marcel-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Marcel will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Marcel to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Marcel to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Marcel remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
marcel mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
marcel mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `marcel mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Marcel to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Marcel to merge, publish, deploy, or spawn runtimes.
...
Now Marcel can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Marcel core.
...
Because Marcel now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
