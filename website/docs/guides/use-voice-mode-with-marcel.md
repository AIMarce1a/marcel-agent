---
sidebar_position: 8
title: "Use Voice Mode with Marcel"
description: "A practical guide to setting up and using Marcel voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Marcel
...
- you want Marcel sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Marcel.
...
## Step 1: make sure normal Marcel works first
...
Marcel supports both local and cloud speech stacks.
...
### If you use `marcel setup`

If you choose NeuTTS in the setup wizard, Marcel checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Marcel transcribes and responds
...
- asking Marcel to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Marcel loop.

## Tuning CLI behavior

### Silence threshold

If Marcel starts/stops too aggressively, tune:
...
Marcel stays a normal chat bot, but can speak replies.
...
- you want Marcel to function like a portable research or ops assistant
...
Marcel joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Marcel detects speech boundaries
- transcripts are posted in the associated text channel
- Marcel responds in text and audio
...
1. get text Marcel working
2. run `marcel setup tts` to enable voice support
