...
```bash
marcel teams-pipeline validate
```
...
```bash
marcel teams-pipeline token-health
marcel teams-pipeline token-health --force-refresh
```
...
```bash
marcel teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
marcel teams-pipeline maintain-subscriptions
marcel teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Marcel cron (recommended if you already run the Marcel gateway)

Marcel ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.marcel/scripts/`. First create the script:

```bash
mkdir -p ~/.marcel/scripts
cat > ~/.marcel/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec marcel teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.marcel/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
marcel cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/marcel-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Marcel Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=marcel
EnvironmentFile=/etc/marcel/env
ExecStart=/usr/local/bin/marcel teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/marcel-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Marcel Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now marcel-teams-pipeline-maintain.timer
systemctl list-timers marcel-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/marcel teams-pipeline maintain-subscriptions >> /var/log/marcel/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.marcel/.env` at the top of a wrapper script that crontab calls.
...
```bash
marcel teams-pipeline subscriptions   # should show expirationDateTime advanced
marcel teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
marcel teams-pipeline list
marcel teams-pipeline list --status failed
marcel teams-pipeline show <job-id>
```

### Replay a stored job

```bash
marcel teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
marcel teams-pipeline fetch --meeting-id <meeting-id>
marcel teams-pipeline fetch --join-web-url "<join-url>"
marcel teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `marcel teams-pipeline maintain-subscriptions --dry-run`
- inspect `marcel teams-pipeline list --status failed`
...
- run `marcel teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `marcel teams-pipeline run`
...
- [ ] `marcel teams-pipeline validate` returns an OK snapshot
- [ ] `marcel teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Marcel cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
