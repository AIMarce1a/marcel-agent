...
Copy-paste blueprints for common automation patterns. Each blueprint uses Marcel's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`marcel webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `marcel webhook subscribe`) |
...
```bash
marcel webhook subscribe github-pr-review \
  --events "pull_request" \
...
3. For each merged PR, check if it modified:
   ...
   - CLI commands (marcel_cli/commands.py, marcel_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (marcel_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
marcel cron create "0 6 * * *" \
  "Run a dependency security audit on the marcel-agent project.

1. cd ~/.marcel/marcel-agent && source .venv/bin/activate
...
```bash
marcel webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
marcel webhook subscribe alert-triage \
...
```python title="~/.marcel/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Marcel-Monitor/1.0"})
...
marcel cron create "every 30m" \
  ...
  --script ~/.marcel/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
marcel cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Marcel Agent development." \
...
```bash
marcel webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
marcel webhook subscribe stripe-payments \
...
```bash
marcel cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the marcel-agent codebase.
