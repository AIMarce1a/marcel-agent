...
def render_skill_page(
    ...
    marcel_meta = (fm.get("metadata") or {}).get("marcel") or {}
    tags = marcel_meta.get("tags") or []
    related = marcel_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `marcel skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Marcel Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Marcel ships with a large built-in skill library copied into `~/.marcel/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Marcel also syncs bundled skills on `marcel update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.marcel/skills/` tree, it is still shipped with Marcel; restore it with `marcel skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with marcel-agent — install via marcel skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with marcel-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "marcel skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "marcel skills install official/blockchain/solana",
        "marcel skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "marcel skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
