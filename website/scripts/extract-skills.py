...
2. The unified Marcel Skills Index at ``website/static/api/skills-index.json``,
    ...
    for base_dir, source_label in LOCAL_SKILL_DIRS:
        ...
        for root, _dirs, files in os.walk(base_path):
            ...
            if isinstance(metadata, dict):
                marcel_meta = metadata.get("marcel", {})
                if isinstance(marcel_meta, dict):
                    tags = marcel_meta.get("tags", [])
            if not tags:
                tags = fm.get("tags", [])
