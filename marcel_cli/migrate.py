"""CLI handlers for ``marcel migrate ...``.

Currently exposes only ``marcel migrate xai`` — diagnoses and (with --apply) rewrites references to
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``marcel migrate <subtype>``."""
    if getattr(args, "migrate_type", None) == "xai":
        return cmd_migrate_xai(args)

    print("usage: marcel migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `marcel migrate xai --apply` to rewrite "
    ...
    print()
    print(color("Run `marcel doctor` to confirm no retired xAI models remain.", Colors.DIM))
    return 0
