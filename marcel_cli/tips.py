...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or marcel -c.",
    ...
    "Ctrl+Z suspends Marcel to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "marcel -c resumes your most recent CLI session. marcel -c \"project name\" resumes by title.",
    "marcel -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "marcel -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "marcel chat -t web,terminal enables only specific toolsets for a focused session.",
    "marcel chat -s github-pr-workflow preloads a skill at launch.",
    "marcel chat -q \"query\" runs a single non-interactive query and exits.",
    "marcel chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "marcel chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "marcel --yolo bypasses all dangerous command approval prompts for the entire session.",
    "marcel chat --source telegram tags the session for filtering in marcel sessions list.",
    "marcel -p work chat runs under a specific profile without changing your default.",
    # --- CLI Subcommands ---
    "marcel doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "marcel dump outputs a compact setup summary — great for bug reports.",
    "marcel config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "marcel config edit opens config.yaml in your default editor.",
    "marcel config check scans for missing or stale configuration options.",
    "marcel sessions browse opens an interactive session picker with search.",
    "marcel sessions stats shows session counts by platform and database size.",
    "marcel sessions prune --older-than 30 cleans up old sessions.",
    "marcel skills search react --source skills-sh searches the skills.sh public directory.",
    "marcel skills check scans installed hub skills for upstream updates.",
    "marcel skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "marcel skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "marcel mcp add github --command npx adds MCP servers from the command line.",
    "marcel mcp serve runs Marcel itself as an MCP server for other agents.",
    "marcel auth add lets you add multiple API keys for credential pool rotation.",
    "marcel completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "marcel logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "marcel backup creates a zip backup of your entire Marcel home directory.",
    "marcel profile create coder creates an isolated profile that becomes its own command.",
    "marcel profile create work --clone copies your current config and keys to a new profile.",
    "marcel update syncs new bundled skills to ALL profiles automatically.",
    "marcel gateway install sets up Marcel as a system service (systemd/launchd).",
    "marcel memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "marcel webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: marcel tools disables unused tools, marcel skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "marcel models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    # --- Configuration ---
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as marcel -w).",
    ...
    "MARCEL_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Marcel tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'marcel profile create coder' creates the 'coder' command.",
    "marcel profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Marcel prints a resume command with session ID and stats.",
    "marcel sessions export backup.jsonl exports all sessions for backup or analysis.",
    "marcel -r SESSION_ID resumes any specific past session by its ID.",
    # --- Memory ---
    ...
    "marcel skills install official/security/1password installs optional skills from the repo.",
    ...
    "/plan writes a markdown implementation plan to .marcel/plans/ without executing anything.",
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: marcel cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Marcel runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "marcel gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Marcel via a local macOS server.",
    ...
    "Context files (.marcel.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Marcel loads project context from .marcel.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "marcel mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "marcel mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "marcel mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — marcel tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.marcel/checkpoints/ — your project's .git is never touched.",
    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "marcel chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "marcel plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "marcel acp runs Marcel as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "MARCEL_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "marcel auth add nous or marcel auth add openai-codex sets up OAuth-based providers.",
    ...
    "marcel status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.marcel/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Marcel your own.",
    ...
    "Before a gateway session reset, Marcel auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.marcel/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Marcel state via MARCEL_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "MARCEL_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in MARCEL_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.marcel/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'marcel profile create ls' would be rejected.",
    "marcel profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "marcel claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "MARCEL_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via marcel tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.marcel/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.marcel/skills/.hub/quarantine/ holds skills pending security review.",
    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Marcel auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Marcel config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Marcel is working.',
    ...
    '/reload reloads ~/.marcel/.env into the running session — pick up new API keys without restarting.',
    ...
    "MARCEL_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.marcel/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.marcel/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    # --- Curator ---
    'marcel curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "marcel curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'marcel curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',
    # --- Credential Pools & Routing ---
    'marcel auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'MARCEL_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "MARCEL_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'marcel dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'marcel dashboard embeds the full Marcel TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.marcel/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.marcel/dashboard-plugins/ — no npm build required.',
    ...
    'MARCEL_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'MARCEL_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'MARCEL_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'MARCEL_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.marcel/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.marcel/skills/ so drop-in skills appear without restarting the session.',
    ...
    'marcel -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'marcel chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'marcel chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'marcel chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "marcel chat --source tool tags programmatic chats so they don't clutter marcel sessions list.",
    'marcel dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'marcel sessions rename <ID> "new title" renames any past session; marcel sessions delete <ID> removes one.',
    'marcel import restores a full Marcel backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'marcel fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'marcel pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'marcel setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'marcel status --deep runs the full health sweep across every component; plain marcel status is the quick view.',
    # --- Agent Behavior Env Vars ---
    'MARCEL_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'MARCEL_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.marcel/plugins/ — trust-gated by design.',
    "MARCEL_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'MARCEL_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'MARCEL_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'MARCEL_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'MARCEL_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'MARCEL_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'MARCEL_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',
    # --- Gateway Behavior Env Vars ---
    'MARCEL_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'MARCEL_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'MARCEL_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'MARCEL_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Marcel block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from marcel sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.marcel/dashboard-plugins/.',
]
...
# Task-oriented example prompts for the empty composer. Kept generic — Marcel is
