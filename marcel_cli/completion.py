"""Shell completion script generation for marcel CLI. Walks the live argparse parser tree, so
    ...
    for cmd, info in subcommands:
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_marcel_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;")
            ...
            cases.append(
                ...
                f"            ;;")
    cases_str = "\n".join(cases)
    return f"""# Marcel Agent bash completion
# Add to ~/.bashrc:
#   eval "$(marcel completion bash)"

_marcel_profiles() {{
    local profiles_dir="$HOME/.marcel/profiles"
...
_marcel_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_marcel_profiles)" -- "$cur"))
        return
    fi
...
complete -F _marcel_completion marcel
"""
    ...
    for cmd, info in subcommands:
        ...
        if cmd == "profile":
            ...
            sub_cases.append(
                ...
                f"                    ;;")
    sub_cases_str = "\n".join(sub_cases)
    return f"""#compdef marcel
# Marcel Agent zsh completion
# Add to ~/.zshrc:
...
_marcel_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.marcel/profiles" ]]; then
        profiles+=($HOME/.marcel/profiles/*(N/:t))
    fi
...
_marcel() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_marcel_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'marcel command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Marcel Agent fish completion",
        "# Add to your config:",
        "#   marcel completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __marcel_profiles",
        "    echo default",
        "    if test -d $HOME/.marcel/profiles",
        "        for d in $HOME/.marcel/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c marcel -f -s p -l profile"
        " -d 'Profile name' -xa '(__marcel_profiles)'",
        "",
        "# Top-level subcommands"]
    for cmd, info in subcommands:
        lines.append(
            f"complete -c marcel -f "
        ...
        if cmd == "profile":  # profile names for the actions that take one
            for action in sorted(_PROFILE_NAME_ACTIONS):
                lines.append(
                    ...
                    f"-a '(__marcel_profiles)' -d 'Profile name'")
    lines.append("")
    return "\n".join(lines)
