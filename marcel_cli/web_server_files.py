...
_MANAGED_FILES_ROOT_ENV = "MARCEL_DASHBOARD_FILES_ROOT"
...
def _default_marcel_root_is_opt_data() -> bool:
    raw = os.environ.get("MARCEL_HOME", "").strip()
    if not raw:
        return False
    try:
        from marcel_constants import get_default_marcel_root

        root = get_default_marcel_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """True when the dashboard should not offer ``marcel update``.

    Containerized dashboards are updated by the outer launcher/image — except a
    ``git`` install (bind-mounted checkout, e.g. the marcel-webui image), where
    ...
    if _default_marcel_root_is_opt_data():
        return True
    try:
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # install still browses its home). Lock to /opt/data only when the Marcel
    # root actually IS /opt/data or MARCEL_DASHBOARD_FILES_ROOT is set.
    if _default_marcel_root_is_opt_data():
