"""Marcel-managed uv and Python runtime repair.

The Python backing the install is shared by every Marcel profile because the checkout's ``venv``
is shared. Runtime repair therefore uses an install-scoped store under
``<checkout>/.marcel-runtime/python``. A vulnerable interpreter is never reinstalled in place.
"""
...
from marcel_cli.sqlite_runtime import (
    SQLiteRuntimeInfo, isolated_interpreter_env, probe_sqlite_runtime)

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".marcel-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
_REPAIR_LOCK_NAME = "runtime-repair.lock"
_MACOS_MANAGED_PYTHON_IDENTIFIER = "com.nousresearch.marcel.managed-python"

_Provisioned = tuple[Path, Path, SQLiteRuntimeInfo]


def managed_uv_path() -> Path:
    """Path of Marcel' own uv binary (``$MARCEL_HOME/bin/uv[.exe]``); may not exist yet."""
    return get_marcel_home() / "bin" / ("uv.exe" if platform.system() == "Windows" else "uv")
...
def managed_python_env(
    ...
    """Return a sanitized environment for Marcel-private uv Python commands."""
    target = (
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print("    Sessions stay protected meanwhile: Marcel keeps databases "
              "out of WAL mode on this SQLite build. The next `marcel update` "
              "will retry.")
        return
...
def _uv_self_update_stamp() -> Path:
    from marcel_constants import get_marcel_home
    return get_marcel_home() / "cache" / ".uv_self_update_stamp"


def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    """True when ``uv self update`` ran recently enough to skip.

    uv releases roughly weekly while many users run ``marcel update`` daily; a blocking network
...
def _reload_marcel_constants():
    """Re-execute ``marcel_constants`` from disk (the imported one may predate venv_python_path)."""
    import marcel_constants
    return importlib.reload(marcel_constants)
...
def _attempt_install_generation(
    ...
    try:
        ...
        return reject("uv resolved Python outside the Marcel generation: %s", python)
...
def _install_safe_python_generation(
    ...
    # supported minor (e.g. 3.11 → 3.12) so the user isn't stuck on every `marcel update`. The
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("marcel_cli.main")
    ...
    if holders:
        ...
        return True, f"other Marcel processes still hold the venv (PID {pids})"
    return False, ""
...
def _windows_runtime_self_lock(live: Path) -> tuple[bool, str]:
    ...
    (``marcel update`` itself runs from the venv python), which is correct for the dependency-sync
    ...
    updater's own ``python.exe`` (or a waiting ``marcel.exe`` launcher ancestor) can never be renamed from
    ...
    # Belt-and-braces: the venv\Scripts\marcel.exe launcher stays mapped while it waits for this
...
def _repair_windows_preflight(
    ...
    if self_locked:
        ...
        for line in (
            ...
            # See #93032.
            "    Retrying `marcel update` from inside this venv cannot help: "
            ...
            f"      cd {root}",
            "      <system Python> -m marcel_cli.main update",
            "    Sessions stay protected meanwhile: Marcel keeps databases "
...
def _install_uv(target: Path) -> None:
    ...
    into ``$MARCEL_HOME/bin/`` instead of ``~/.local/bin/``.
    """
