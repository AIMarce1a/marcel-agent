"""Gateway fleet restart + post-update verification for ``marcel update``.

Split out of ``marcel_cli/update_cmd.py``; every name is re-imported there so
``marcel_cli.update_cmd.<name>`` keeps resolving/monkeypatching. Origin helpers are
...
from marcel_cli.update_cmd_common import _best_effort

# Log-record parity with the origin module.
logger = logging.getLogger("marcel_cli.update_cmd")

# Under MARCEL_HOME (not next to the venv): records the fleet-restart obligation
...
_LIST_GATEWAY_UNITS = ["list-units", "marcel-gateway*", "marcel-serve*", "--plain", "--no-legend", "--no-pager"]


def _write_gateway_update_exit_code(ok: bool) -> None:
    from marcel_cli.update_cmd import get_marcel_home
    path = get_marcel_home() / ".update_exit_code"
    with suppress(OSError):
        path.write_text("0" if ok else "1", encoding="utf-8")


def _fleet_restart_pending_marker_path() -> Path:
    """MARCEL_HOME breadcrumb for a pull that has not yet restarted the fleet."""
    from marcel_cli.update_cmd import get_marcel_home
    return get_marcel_home() / _FLEET_RESTART_PENDING_NAME


def _write_fleet_restart_pending_marker(*, expected_sha: str = "") -> None:
    """Drop the pull→restart obligation breadcrumb. Never raises."""
    from marcel_cli.update_cmd import _m
...
def _current_checkout_sha() -> str | None:
    ...
    from marcel_cli.update_cmd import _capture_head_sha, _m
    try:
        from marcel_cli.build_info import get_code_identity
...
def _receipt_reports_stale_runtime(expected_sha: str | None = None) -> bool:
    ...
    *before* the pull, so a finished update's plan always looks stale and must not
    ...
    See #95294.
    """
    from marcel_cli.update_cmd import _current_checkout_sha
    try:
        from marcel_cli.update_receipt import read_latest_receipt
...
def _warn_pending_fleet_restart(*, startup: bool = False) -> None:
    ...
    print("⚠ A previous `marcel update` pulled new code but did not restart running gateways.", file=stream)
    print("  Gateways may still be serving pre-update modules (mixed sys.modules).", file=stream)
    if startup:
        print("  Run `marcel update` or `marcel gateway restart`.", file=stream)
...
def _restart_systemd_gateway_units_best_effort(failed: list) -> None:
    """Best-effort ``systemctl restart`` of every marcel-gateway/serve unit."""
...
def _run_pending_fleet_restart() -> bool:
    ...
    with suppress(Exception):
        _m()._purge_stale_marcel_modules()
    # Warn if legacy Marcel gateway unit files are still installed. When both marcel.service (from a
    # pre-rename install) and the current marcel-gateway.service are enabled, they SIGTERM-fight for the
    # same bot token (see PR #11909). Flagging here means every `marcel update` surfaces the issue until the
    # user migrates.
    try:
        from marcel_cli.gateway import (
    ...
    failed: list = []
    try:
        # --- Systemd services (Linux) --- Discover all marcel-gateway* units (default + profiles) plus
        # marcel-serve* units (the Desktop app's backend, #83438).
        if supports_systemd_services():
            _restart_systemd_gateway_units_best_effort(failed)
        # --- Launchd services (macOS) --- Restart EVERY ai.marcel.gateway* LaunchAgent, not only the
...
def _apply_pending_fleet_restart_catchup() -> None:
    """On an already-up-to-date ``marcel update``, finish a skipped restart.

    No-op when nothing is pending; exits 1 on incomplete catch-up so automation
    does not treat the fleet as healthy.
    """
    from marcel_cli.update_cmd import _run_pending_fleet_restart
    ...
    if _run_pending_fleet_restart():
        ...
        return
    print("  ⚠ Fleet restart incomplete. Recover with: marcel gateway restart")
    sys.exit(1)
...
def _is_marcel_gateway_unit(unit: str) -> bool:
    """Exact base unit or hyphenated profile family only: ``startswith("marcel-serve")``
    would accept ``marcel-server.service``."""
    return (
        ...
        unit == "marcel-gateway.service"
        or unit.startswith("marcel-gateway-")
        or unit == "marcel-serve.service"
        or unit.startswith("marcel-serve-")
    )


def _for_each_systemd_gateway_unit(list_units_stdout: str, *, process_unit, on_unit_timeout) -> None:
    """Process each marcel-gateway*/marcel-serve* unit from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not parts:
            continue
        unit = parts[0]
        if not unit.endswith(".service") or not _is_marcel_gateway_unit(unit):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``marcel-gateway*`` runs ``gateway/run.py`` (the handler); SIGUSR1 would just
    kill ``marcel-serve*`` and burn the drain budget, so those go straight to the blunt
    restart. Same exact/hyphenated shape as ``_for_each_systemd_gateway_unit`` so a
    near-prefix unit like ``marcel-gatewayd`` is never signalled.

    See #83438.
    """
    return svc_name == "marcel-gateway" or svc_name.startswith("marcel-gateway-")
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    from marcel_cli.gateway import is_macos
    if not failed_units:
        return
    ...
    print("    marcel gateway status")
    if any(not name.startswith("ai.marcel.") for name in ordered):
        print("    systemctl --user restart <unit>   # user-scope")
        print("    sudo systemctl restart <unit>     # system-scope")
    if any(name.startswith("ai.marcel.") for name in ordered):
...
def _restart_launchd_gateway_after_update(*, supervision_verify: bool = True) -> tuple[list, list]:
    ...
    try:
        ...
        try:
            ...
            print(
                ...
                "Recover manually: marcel gateway restart"
            )
    ...
    print(
        ...
        "    Check logs, then: marcel gateway restart"
    )
    return [], [current_label]
...
def _restart_macos_launchd_gateways(restarted_services: list, failed_or_stale_units: list, drain_budget: float) -> None:
    ...
    The pull is shared across profiles, so every ``ai.marcel.gateway*`` LaunchAgent
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    ``None`` when undeterminable (notably ``marcel_cli.gateway`` no longer importing
    ...
    """
    try:
        from marcel_cli.gateway import find_gateway_pids
...
def _gateway_service_matches_profile(profile: str, service: object) -> bool:
    ...
    Never substring-match: ``foo`` must not claim ``marcel-gateway-foobar.service``.
    """
    name = str(service).removesuffix(".service")
    if profile == "default":
        return name in {"marcel-gateway", "ai.marcel.gateway", "gateway", "gateway-default"}
    return name in {f"marcel-gateway-{profile}", f"ai.marcel.gateway-{profile}", f"gateway-{profile}"}
...
# NOT a claim that no supervisor exists: a systemd-launched serve sets neither MARCEL_SPAWN
# nor MARCEL_PARENT_PID ("manual-serve"). Unit-backed serves are recovered by the fresh
...
_SERVE_SKIP_REASON = (
    ...
    " systemd unit pass when it owns a marcel-serve* unit, else left running for explicit"
    " operator restart"
)
...
def _gateway_recovery_partition(plan, *, skip_profiles: set[str] | None = None) -> tuple[dict[str, str], list[dict]]:
    ...
    Uses only the pre-checkout inventory: re-importing ``marcel_cli.gateway`` in the
    ...
    child's ``marcel-serve*`` systemd pass enumerates units from systemd; leftovers are
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    logged at debug level, so an early failure (e.g. importing ``marcel_cli.gateway`` from the freshly
...
def _resolve_manage_cmd(cache: dict, scope_: str, scope_cmd_: list, svc_name_: str):
    ...
    so a least-privilege sudoers entry scoped to marcel-gateway* qualifies (idempotent
...
def _restart_one_systemd_gateway_unit(
    ...
    # → drain → exit, Restart=always respawns. marcel-serve has no handler → blunt restart below.
    _main_pid = 0
    ...
    if _manage_cmd is None:
        ...
        print(
            ...
            f"    To let `marcel update` restart it automatically, allow\n"
    ...
    # budget, restart-policy mismatch). Mirrors `marcel gateway restart` (`systemd_restart()`).
...
def _restart_systemd_gateway_units(restarted_services, failed_or_stale_units, restarted_scoped_units, drain_budget):
    """Restart every active marcel-gateway*/marcel-serve* systemd unit (user + system).
    ...
    """
    from marcel_cli.gateway import supports_systemd_services, _ensure_user_systemd_env
    ...
    def _on_list_timeout(scope: str, exc: subprocess.TimeoutExpired) -> None:
        ...
        print(
            ...
            f"Check the gateway with: marcel gateway status"
        )
...
class _GatewayRestartOutcome:
    ...
    def record_receipt(self, **extra) -> None:
        ...
        with suppress(Exception):
            from marcel_cli.update_receipt import record_gateway_restart
...
def _restart_manual_gateways(out: _GatewayRestartOutcome, _drain_budget) -> None:
    ...
    if out.restarted_services or out.killed_pids:
        ...
        if unmapped_count:
            ...
            print("    Restart manually: marcel gateway run")
            if unmapped_count > 1:
                print("    (or: marcel -p <profile> gateway run  for each profile)")
...
def _force_kill_stuck_gateways(killed_pids) -> None:
    ...
    with _best_effort('Post-restart survivor sweep failed: %s'):
        from marcel_cli.gateway import find_gateway_pids, _get_service_pids
...
def _recover_after_restart_phase_abort(
    ...
    from marcel_cli.update_cmd import (
...
def _restart_gateway_fleet_after_update(_pre_update_plan, gateway_mode: bool):
    ...
    """
    from marcel_cli.update_cmd import _m, _write_gateway_update_exit_code
    ...
    # Scope-qualified twin (``user/marcel-serve`` vs ``system/marcel-serve`` are different
    ...
    # Purge stale cached Marcel modules FIRST: the import below loads new gateway
    ...
    try:
        ...
        from marcel_cli.gateway import (  # noqa: F401
            is_macos,
            find_gateway_pids,
        ...
        try:
            from marcel_cli.gateway import _get_restart_exit_wait_budget
        ...
        # macOS: EVERY ai.marcel.gateway* LaunchAgent (systemd parity).
        if is_macos():
...
def _print_legacy_units_warning() -> None:
    """Legacy marcel.service fights marcel-gateway.service over the bot token; warn on
    every update until migrated."""
    from marcel_cli.gateway import (has_legacy_marcel_units, _find_legacy_marcel_units, supports_systemd_services)
    if not (supports_systemd_services() and has_legacy_marcel_units()):
        return
    print()
    print("⚠ Legacy Marcel gateway unit(s) detected:")
    for name, path, is_sys in _find_legacy_marcel_units():
        scope = "system" if is_sys else "user"
        print(f"    {path}  ({scope} scope)")
    print()
    print("  These pre-rename units (marcel.service) fight the current")
    print("  marcel-gateway.service for the bot token and cause SIGTERM")
    print("  flap loops. Remove them with:")
    print()
    print("    marcel gateway migrate-legacy")
    print()
...
def _collect_fleet_snapshot(restart, rows_expected: bool) -> list:
    ...
    """
    from marcel_cli.update_receipt import collect_fleet_versions
...
def _verify_fleet_after_update(restart, *, _pre_update_plan, _windows_gateway_resume, node_failures, update_complete):
    ...
    # units, so a unit-less `marcel serve` keeps stale sys.modules. Runs AFTER
    ...
    # units, so an sshd-spawned `serve --isolated` or a manual `marcel serve` (no unit) is left running its
    ...
    print("  marcel model              # Select provider and model")
    ...
    with _best_effort('Fleet version verification failed: %s'):
        from marcel_cli.update_receipt import print_fleet_version_matrix
    ...
    with _best_effort('Runtime-outcome reconciliation failed: %s'):
        ...
        if _pre_update_plan is not None and _pre_update_plan.runtimes:
            from marcel_cli.update_inventory import (match_runtime_outcomes, report_unaccounted_runtimes)
            ...
            if report_unaccounted_runtimes(_runtime_outcomes):
                restart.incomplete = True
            with suppress(Exception):
                import marcel_cli.update_receipt as _ur
    ...
    with _best_effort('Update receipt finalize failed: %s'):
        from marcel_cli.update_receipt import finalize_update_receipt
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state (typically the freshly-pulled
    ``marcel_cli.gateway`` no longer imports, one of the ways the phase aborts). That is proof-of-safety
