...
_DEFAULT_PLATFORM_TOOLSETS = {"cli": "marcel-cli"}
...
def _has_agent_browser() -> bool:
    ...
    from marcel_constants import agent_browser_runnable
    ...
    try:
        ...
        # Marcel-managed Node dirs ($MARCEL_HOME/node, prepended to PATH at runtime but usually absent
        ...
        search_paths = [None, with_marcel_node_path().get("PATH", ""), str(local_bin_dir) if local_bin_dir.is_dir() else ""]
        return any(
...
def _local_stt_backend_available() -> bool:
    ...
    if get_env_value("MARCEL_LOCAL_STT_COMMAND"):
        return True
    try:
...
def prompt_enable_tool_gateway(config: Dict[str, object], *, force_fresh: bool = True) -> set[str]:
    ...
    if not unconfigured and not has_direct:
        return set()
    try:
        from marcel_cli.setup import prompt_checklist
    except Exception:
        return set()
...
# Inline Nous Portal login for the Tool Gateway picker (`marcel tools`)
