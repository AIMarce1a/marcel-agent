"""Configuration management for Marcel Agent: config.yaml / .env loading, saving,
validation, migration, and the ``marcel config`` command."""
...
# Re-export from marcel_constants — canonical definition lives there.
from marcel_constants import get_marcel_home, get_process_marcel_home  # noqa: F401
...
_PARSE_FAILURE_FALLBACK_MSG = {
    ...
    "refuse-write": (
        ...
        "Fix the YAML (marcel config edit) and retry.")}
...
def _warn_config_parse_failure(
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  marcel config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
# interpreter init (PYTHON*, NODE_*: Marcel restarts through them), PATH (fix tool lookup
...
# and Marcel runtime-location / security-policy flags (config.yaml is the supported surface).
#
# ``MARCEL_*`` overall is NOT blocked — many integration credentials use that prefix
# (MARCEL_LANGFUSE_PUBLIC_KEY, MARCEL_SPOTIFY_CLIENT_ID, ...). The denylist is name-by-name so
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Marcel runtime location
    "MARCEL_HOME", "MARCEL_PROFILE", "MARCEL_CONFIG", "MARCEL_ENV",
    "MARCEL_CONFIG_PATH", "MARCEL_ENV_PATH",
    # MCP catalog trust root; package-manager wrappers may still set it in the process env.
    "MARCEL_OPTIONAL_MCPS",
    # Local ACP subprocess selection (executable/argv authority).
    "MARCEL_COPILOT_ACP_COMMAND", "MARCEL_COPILOT_ACP_ARGS",
    # Security policy / approval-routing context — set via their dedicated controls only.
    "MARCEL_YOLO_MODE", "MARCEL_ACCEPT_HOOKS", "MARCEL_REDACT_SECRETS",
    "MARCEL_INTERACTIVE", "MARCEL_EXEC_ASK", "MARCEL_GATEWAY_SESSION",
    "MARCEL_CRON_SESSION", "MARCEL_SINGLE_QUERY_SESSION",
    "MARCEL_SESSION_KEY", "MARCEL_SESSION_PLATFORM"})
...
def validate_env_var_name_for_write(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "or Marcel runtime location and security policy (MARCEL_HOME, MARCEL_YOLO_MODE, ...) "
            "cannot be persisted via the env writer. If you really need this, edit ~/.marcel/.env "
            "directly.")
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # back-compat fallback. The boolean MARCEL_TOOL_PROGRESS variant is unsupported (its only
    # consumer, the v3->4 migration, is below the v12 support floor); doctor flags it as ignored.
    "MARCEL_TOOL_PROGRESS_MODE",
    ...
    "MARCEL_LANGFUSE_ENV", "MARCEL_LANGFUSE_RELEASE", "MARCEL_LANGFUSE_SAMPLE_RATE",
    "MARCEL_LANGFUSE_MAX_CHARS", "MARCEL_LANGFUSE_CAPTURE", "MARCEL_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY", "LANGFUSE_SECRET_KEY", "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so profiles can use different backends.
    "MARCEL_ACP_AUTH_METHOD", "MARCEL_ACP_AUTO_APPROVE", "MARCEL_COPILOT_ACP_COMMAND",
    "MARCEL_COPILOT_ACP_ARGS", "COPILOT_CLI_PATH", "COPILOT_ACP_BASE_URL"})
...
# MARCEL_MANAGED). Module-level so tests can patch it without touching /nix/store.
...
def get_managed_system() -> Optional[str]:
    ...
    Signals: MARCEL_MANAGED env var (systemd service) or a ``.managed`` marker file in
    MARCEL_HOME (NixOS activation script — interactive shells don't see the service env)."""
    marker = os.getenv("MARCEL_MANAGED", "").strip().lower() or None
    managed_marker = get_marcel_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Marcel is running in package-manager-managed mode."""
...
_NIX_UPDATE_MSG = (
    "Update Marcel through the Nix source that installed it "
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Marcel was installed: apt/docker/nix/nixos/home-manager/git/unknown.
    Order: code-scoped ``<install tree>/.install_method`` stamp (authoritative) -> legacy
    ``$MARCEL_HOME/.install_method`` -> managed marker -> /nix/store path -> .git dir -> unknown.
    The stamp lives next to the code because MARCEL_HOME is shared data: a container and a host
    install can bind-mount the same home, so a home-scoped ``docker`` stamp would make the host
    ``marcel update`` refuse to run. A legacy ``docker`` value is therefore ignored unless we are
    ...
    ``/opt/marcel`` at build time. An unsupported manual install dropped into a container (no stamp) falls
    through to the ``.git`` checks and behaves like any off-path install. See issue #34397.
    """
    # The stamp is a property of the running code tree (parent of marcel_cli/), NOT of $MARCEL_HOME,
    ...
    method = _install_method_stamp(get_marcel_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Import-safe wrapper around ``marcel_constants.is_container``."""
    try:
...
_UPDATE_COMMAND_BY_METHOD = {
    ...
    "apt": "pkg upgrade marcel-agent",  # "apt" == Termux APT by contract; uses Termux's `pkg`.
}
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    return _UPDATE_COMMAND_BY_METHOD.get(method, "marcel update")
...
# Shared by ``cmd_update`` and ``_cmd_update_check`` (marcel_cli/main.py) so the wording never
# forks. The published image excludes ``.git``, so the git update path can never succeed there
# and the generic "reinstall via install.sh" fallback would install a NEW host-side Marcel.
_DOCKER_UPDATE_MESSAGE = """\
✗ ``marcel update`` doesn't apply inside the Docker container.

Marcel Agent runs as a published image (NousResearch/hermes-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull NousResearch/hermes-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate marcel-agent
...
Notes:
  ...
  • Your config and session history live under ``$MARCEL_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``marcel update`` inside Docker."""
    return _DOCKER_UPDATE_MESSAGE


def format_managed_message(action: str = "modify this Marcel installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this Marcel installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Marcel.")
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from MARCEL_HOME/.container-mode.
    ...
    already inside the container, or when MARCEL_DEV=1 is set. Only FileNotFoundError is
    swallowed; other errors (permissions, malformed data) propagate."""
    if os.environ.get("MARCEL_DEV") == "1":
        return None
    ...
    try:
        info = {}
        with open(get_marcel_home() / ".container-mode", "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
    ...
    return {
        ...
        "container_name": info.get("container_name", "marcel-agent"),
        "exec_user": info.get("exec_user", "marcel"),
        "marcel_bin": info.get("marcel_bin", "/data/current-package/bin/marcel")}


# ---- Config paths / MARCEL_HOME skeleton ----

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_marcel_home() / "config.yaml"
...
def require_parseable_user_config(*, ignore_user_config: bool = False) -> None:
    ...
    states; ``--ignore-user-config`` / MARCEL_IGNORE_USER_CONFIG=1 remain authoritative."""
    if ignore_user_config or os.environ.get("MARCEL_IGNORE_USER_CONFIG") == "1":
        return
...
def _resolve_marcel_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read MARCEL_UID / MARCEL_GID (set by Docker deployments); (None, None) if unset/invalid/Windows.
    The entrypoint chowns MARCEL_HOME once, but subdirs created at runtime (``profiles/<name>/``)
    need the same chown or they land root:root and block later uid-mapped workers.

    Docker containers running Marcel commonly set these to map the in-container user to a host user so
    ...
    return _env_int("MARCEL_UID"), _env_int("MARCEL_GID")


def _chown_to_marcel_uid(path) -> None:
    """Chown ``path`` to ``MARCEL_UID:MARCEL_GID`` when set; EPERM/ENOENT are non-fatal (the
    ...
    :func:`ensure_marcel_home` on Docker deployments. See #34107.
    """
    uid, gid = _resolve_marcel_uid_gid()
...
def _secure_dir(path):
    """chmod a directory owner-only (0700) and apply MARCEL_UID/GID ownership. No-op when managed.
    MARCEL_HOME_MODE (e.g. 0701) overrides the mode so a web server can traverse MARCEL_HOME to
    a served subdirectory without directory listings.

    Also applies ``MARCEL_UID``/``MARCEL_GID``-based ownership when those env vars are set (#34107 — Docker
    ...
    if is_managed():
        return
    try:
        mode = int(os.environ.get("MARCEL_HOME_MODE", "").strip() or "700", 8)
    except ValueError:
        mode = 0o700
    try:
        os.chmod(path, mode)
        ...
        pass
    _chown_to_marcel_uid(path)


def _is_container() -> bool:
    """Detect Docker/Podman/LXC (or MARCEL_CONTAINER / MARCEL_SKIP_CHMOD opt-out).
    ...
    if (os.environ.get("MARCEL_CONTAINER") or os.environ.get("MARCEL_SKIP_CHMOD")
...
_MARCEL_HOME_ENSURED: set = set()
_MARCEL_HOME_SUBDIRS = (
    "cron", "sessions", "logs", "logs/curator", "memories",
    "pairing", "hooks", "image_cache", "audio_cache", "skills")


def ensure_marcel_home():
    """Ensure the ~/.marcel directory skeleton exists with secure permissions.
    ...
    from marcel_constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _MARCEL_HOME_ENSURED and home.is_dir():
        return
    if is_managed():
        ...
        try:
            if not home.is_dir():
                raise RuntimeError(f"MARCEL_HOME {home} does not exist.")
        ...
        _secure_dir(home)
        for subdir in _MARCEL_HOME_SUBDIRS:
            d = home / subdir
            d.mkdir(parents=True, exist_ok=True)
            _secure_dir(d)
        _ensure_default_soul_md(home)

    _MARCEL_HOME_ENSURED.add(key)


# ---- Config loading/saving ----

from marcel_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: E402,F401
from marcel_cli.config_providers import (  # noqa: E402,F401  (re-exported; callers/tests use marcel_cli.config.<name>)
...
# Back-compat re-exports — :mod:`marcel_cli.personality` owns personality/overlay semantics.
from marcel_cli.personality import (  # noqa: E402,F401
...
def _split_key_path(key: str) -> list[str]:
    ...
    ``marcel config set`` uses ``.`` as the nesting separator, so a key that itself contains a literal dot
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `marcel config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    return (
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # A malformed SKILL.md must never break `marcel update`; this prompting is a nicety.
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (marcel_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard
    "platform_toolsets",     # written by the setup wizard
    "known_plugin_toolsets", # marcel_cli/tools_config.py toolset-save flow
    ...
    for _key in ("backend", "search_backend", "extract_backend"):
        ...
        if note:
            _issue(issues, "warning",
                   ...
                   "Run 'marcel tools' and pick a different Web Search & Extract provider")
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'marcel setup' to create a valid config")]
    ...
    if cp and not config.get("model"):
        _issue(issues, "warning",
               "custom_providers defined but no 'model' section — Marcel won't know which provider to use",
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'marcel doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from marcel_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ marcel config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
...
def _disable_suspicious_mcp_servers(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if not isinstance(raw_mcp_servers, dict):
        return
    try:
        from marcel_cli.mcp_security import validate_mcp_server_entry
    except Exception:
        return
    mcp_touched = False
...
def _warn_invalid_platform_toolsets(results: Dict[str, Any], quiet: bool) -> None:
    ...
    try:
        ...
        from marcel_cli.toolset_validation import validate_platform_toolsets
        from marcel_cli.toolset_scope import toolset_allowed_for_platform
...
def _offer_list(heading: str, items: List[str], question: str) -> bool:
    ...
    if not _ask_yes_no(question):
        print("  Set later with: marcel config set <key> <value>")
        return False
    print()
    return True
...
def _env_expand_match(m: re.Match) -> str:
    ...
    if inner.startswith("env:"):
        logger.warning(
            "Config ref %r: %s is not set (check ~/.marcel/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    made against a different environment (load before ``load_marcel_dotenv()``, in-process
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    LiteLLM users reach for, and ``marcel config set`` blindly accepts any dotted key — so
    ...
    from OpenAI-compatible backends) — while display paths (``marcel status``/``dump``) read ``name`` and
...
# the MARCEL_MAX_ITERATIONS env bridge, works in every ``<``/``>=``/``max - used`` comparison in
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    """Whether the remote SSH shell must expand *cwd* itself: ``~`` expanded on the Marcel host
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # load_marcel_dotenv() pins unexpanded literals for the process lifetime.
            # Without this, a load_config() that ran before load_marcel_dotenv() pins unexpanded literals
...
#   openai-codex (OAuth — marcel auth) — OpenAI Codex
#   nous         (OAuth — marcel auth) — Nous Portal
...
def save_config(
    ...
    """Save configuration to ~/.marcel/config.yaml.
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Marcel writes itself (bare, 'single', or "double" with
    ``\\"`` / ``\\\\`` escapes)."""
...
def load_env() -> Dict[str, str]:
    """Load ~/.marcel/.env as a dict (memoised; ``get_env_value()`` runs hundreds of times per
...
def sanitize_env_file() -> int:
    """Rewrite ~/.marcel/.env with normalized line formatting; returns the number of changed lines."""
    env_path = get_env_path()
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.marcel/.env (also matching ``export KEY=`` lines, so a save
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    so two endpoints on one host get separate slots. The fixed ``MARCEL_CUSTOM_`` prefix keeps the
    ...
    return f"MARCEL_CUSTOM_{slug}_API_KEY" if slug else "MARCEL_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.marcel/.env and os.environ; True if it was found and removed."""
...
def save_env_value_secure(key: str, value: str) -> Dict[str, Any]:
    ...
    from marcel_cli.credential_lifecycle import save_provider_env_credential
...
def reload_env() -> int:
    """Re-read ~/.marcel/.env into os.environ; returns count of vars changed.
    Removes deleted vars only when known to Marcel (OPTIONAL_ENV_VARS and _EXTRA_ENV_KEYS) so
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` (scope-aware) or ``~/.marcel/.env``.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a Marcel-managed credential preferring ``~/.marcel/.env`` over ``os.environ``, so a
...
def _show_model_section(config: Dict[str, Any]) -> None:
    ...
    # Read the .env FILE directly so a stale MARCEL_MAX_ITERATIONS ghost is caught even when the
    # gateway bridge already overrode os.environ.
    try:
        env_ghost = load_env().get("MARCEL_MAX_ITERATIONS")
    except Exception:
        env_ghost = None
    if env_ghost is not None and str(env_ghost).strip() != str(cfg_max_turns).strip():
        print(color(f"                ⚠ .env has stale MARCEL_MAX_ITERATIONS={env_ghost} "
                    f"(run 'marcel doctor --fix' to remove)", Colors.YELLOW))
...
def _show_display_section(config: Dict[str, Any]) -> None:
    ...
    try:
        from marcel_cli.personality import active_personality_name
...
def show_config():
    ...
    print(color("│              ⚕ Marcel Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from marcel_cli.auth import get_anthropic_key
    ...
    print(color("  marcel config edit     # Edit config file", Colors.DIM))
    print(color("  marcel config set <key> <value>", Colors.DIM))
    print(color("  marcel setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    Mirrors the scheduler's precedence: a truthy configured model wins over ``MARCEL_MODEL``; the
    ...
    return provider, configured_model or _model_assignment_text(env.get("MARCEL_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "model/provider. Inspect with `marcel cron list`, then pin the intended values with "
        "`marcel cron edit <job_id> --provider <provider> --model <model>`.")
...
def _coerce_config_set_value(key: str, value: str) -> Any:
    """Auto-coerce a ``marcel config set`` string to bool/None/int/float/list/dict.
...
def _redirect_platform_display_key(key: str) -> tuple[str, Optional[str]]:
    ...
    Before #71047 a write such as ``marcel config set platforms.telegram.streaming false`` landed on a key
...
def _guard_section_overwrite(key: str, value: Any, user_config: Dict[str, Any], force: bool) -> str:
    ...
    err += [
        ...
        f"    marcel config set {key}.<sub-key> <value>",
        "  Or use --force to replace the entire section:",
        f"    marcel config set --force {key} {value!r}"]
...
def _touch_skin_file(key: str, value: Any) -> None:
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_marcel_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
            pass
...
def _print_unknown_key_notice(key: str, suggestion: Optional[str]) -> None:
    print(color(
        ...
        "but Marcel may not read it.", Colors.YELLOW))
    if suggestion:
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    if key.strip().lower() in ("model.api_base", "api_base"):
        # Normalize the api_base → base_url alias at set-time too (issue #8919), so a fresh `marcel config
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # See #51071.
        from marcel_cli.credential_lifecycle import remove_provider_env_credential
...
_USAGE_GET = ("Usage: marcel config get <key> [--json]", [
    "marcel config get model", "marcel config get terminal.backend",
    "marcel config get skills.config --json"], None)
_USAGE_SET = ("Usage: marcel config set [--force] <key> <value>", [
    "marcel config set model anthropic/claude-sonnet-4", "marcel config set terminal.backend docker",
...
_USAGE_UNSET = ("Usage: marcel config unset <key>", [
    "marcel config unset model", "marcel config unset terminal.backend",
    "marcel config unset OPENROUTER_API_KEY"], None)
...
def _cmd_config_check(args):
    ...
    if missing_config:
        ...
        print("    Run 'marcel config migrate' to add them")
...
_CONFIG_USAGE = """Available commands:
  marcel config           Show current configuration
  marcel config edit      Open config in editor
  marcel config get <key>          Print a resolved config value
  marcel config set <key> <value>   Set a config value
  marcel config unset <key>        Remove a config value
  marcel config check     Check for missing/outdated config
  marcel config migrate   Update config with new options
  marcel config path      Show config file path
  marcel config env-path  Show .env file path"""
...
def _inject_platform_plugin_env_vars() -> None:
    ...
    Chat etc. are configurable in ``marcel config`` UI without the core knowing they exist.
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``marcel_cli/`` — i.e. the git checkout for source
    installs, ``/opt/marcel`` inside the published image. It is a property of
    the running interpreter, NOT of ``$MARCEL_HOME``, which is why a
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$MARCEL_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/marcel`` in the published image, which instead bakes the stamp at
...
_PLUGIN_COMPAT_LAZY = {
    'normalize_route_base_url': ('marcel_cli.route_identity', 'normalize_route_base_url'),
}
