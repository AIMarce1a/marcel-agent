...
_WINDOWS_GATEWAY_BREAKAWAY_ENV = "_MARCEL_GATEWAY_BREAKAWAY"
...
def noninteractive_git_env(base: "Mapping[str, str] | None" = None) -> dict[str, str]:
    ...
    Marcel's plumbing calls. ``GIT_ASKPASS``/``SSH_ASKPASS`` are deliberately left alone: a
    *working* askpass helper or ssh-agent should still succeed non-interactively. Pair with
    ...
    Marcel shells out to git from many non-interactive contexts — MCP catalog installs, plugin
...
def _text_names_marcel(text: str) -> bool:
    r"""True when *text* names Marcel at a path-segment / token boundary.

    A bare ``"marcel" in text`` substring test would also match unrelated processes whose paths
    merely contain the letters (``...\smarcela\...``) — the false-positive class this prevents.
    """
    return any(token.startswith(("marcel", ".marcel"))
               for token in re.split(r"[\\/\s=,;\"']+", text.lower()))


def _process_command_is_marcel(pid: int) -> bool:
    """Best-effort check that *pid* currently runs Marcel code."""
    try:
        import psutil
        ...
        return _text_names_marcel(f"{command} {executable}")
    except Exception:
        return False


def pid_is_marcel(pid: int, *, expected_start_time: int | None = None) -> bool:
    """Whether it is safe to use ``taskkill`` for *pid*.

    The PID must be valid, currently exist, and identify a Marcel process. When the caller captured
    ...
    if expected_start_time is not None and current_start_time != expected_start_time:
        return False
    try:
        return _process_command_is_marcel(pid)
    except Exception:
        return False
...
def _legacy_kill_process_tree(proc: "subprocess.Popen") -> None:
    ...
    if IS_WINDOWS:
        ...
        # be recycled while we hold it. The fail-closed ``pid_is_marcel`` guard is for BARE pids.
        try:
...
def bounded_probe_run(
    ...
    ``Get-CimInstance Win32_Process`` gateway scan hit exactly this during ``marcel update`` on slow-WMI
