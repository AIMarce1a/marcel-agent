...
Split out of ``marcel_cli/config.py``; every name is re-imported there, so
``marcel_cli.config.<name>`` keeps resolving (and monkeypatching) as before. Functions that call
``get_compatible_custom_providers`` / ``load_config*`` import them lazily from ``marcel_cli.config``
...
logger = logging.getLogger("marcel_cli.config")
...
_KNOWN_PROVIDER_KEYS = {
    # ``provider`` duplicates the ``providers.<name>`` mapping key and is unused here, but Marcel'
...
def _normalize_custom_provider_entry(
    ...
    # ``models_discovered`` marks a mapping auto-discovered by Marcel, not hand-curated.
...
def get_custom_provider_model_capability(
    ...
    from marcel_cli.config import get_compatible_custom_providers, load_config_readonly
