...
The provider list shown by ``marcel model`` (CLI/TUI) and the desktop Settings → Providers tabs
...
come from :data:`marcel_cli.auth.PROVIDER_REGISTRY` (credential truth); ``display_name`` /
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `marcel model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """One descriptor per provider in the ``marcel model`` universe (:data:`CANONICAL_PROVIDERS`,
    ...
    PROVIDER_REGISTRY = _safe_import("marcel_cli.auth", "PROVIDER_REGISTRY", {})
    OPTIONAL_ENV_VARS = _safe_import("marcel_cli.config", "OPTIONAL_ENV_VARS", {})
    ...
    MARCEL_OVERLAYS = _safe_import("marcel_cli.providers", "MARCEL_OVERLAYS", {})
    try:
