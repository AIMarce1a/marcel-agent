"""ZIP-download fallback for ``marcel update`` (Windows with broken git): two-phase stage/commit swap, dirty-tree guard.

Split out of ``update_cmd.py``; every name is re-imported there so ``marcel_cli.update_cmd.<name>`` keeps
...
from marcel_cli.release_coordinates import MARCEL_RELEASE_ARCHIVE_URL

# Log-record parity with the origin module.
logger = logging.getLogger("marcel_cli.update_cmd")

_ZIP_STAGING_ARTIFACT_SUFFIXES = ".marcel-update-staging", ".marcel-update-old"
...
_STASH_HINT = "  Stash or commit your changes, then rerun `marcel update`."
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    Thin alias over the two-phase helpers; retained for the ``marcel_cli.main`` re-export surface.
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    staging = f"{dst}.marcel-update-staging"
    backup = f"{dst}.marcel-update-old"
...
def _zip_overlay_block_reason(root: Path, *, ignore_staging_artifacts: bool = False) -> Optional[str]:
    ...
    is for the pre-swap re-check: phase 1 leaves our own ``*.marcel-update-staging`` siblings that git
...
def _extracted_root(tmp_dir: str, branch: str) -> str:
    """GitHub ZIPs extract to ``marcel-agent-<branch>/``; fall back to the first non-``__MACOSX`` dir."""
    extracted = os.path.join(tmp_dir, f"marcel-agent-{branch}")
...
def _require_staging_space(extracted: str, entries: list[str], project_root: str) -> None:
    ...
    need = sum(
        ...
        # root holds 20 first-party modules (run_agent.py, cli.py, marcel_constants.py, ...). Check up front
...
def _stage_entries(extracted: str, entries: list[str], project_root: str) -> list[tuple[str, str]]:
    ...
    try:
        for item in entries:
            ...
            # desktop app, win-unpacked/ Marcel.exe) exists only in the LIVE tree. Graft the live release
...
def _download_and_swap_zip(branch: str, zip_url: str) -> None:
    ...
    tmp_dir = tempfile.mkdtemp(prefix="marcel-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"marcel-agent-{branch}.zip")
        ...
        print("  Re-run `marcel update` to retry; if the agent won't start, reinstall from https://hermes-agent.nousresearch.com")
        _m().sys.exit(1)
    finally:
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `marcel update "
            f"--branch {branch}`, or update against main with `marcel update`."
        )
        _m().sys.exit(1)
    _abort_zip_update_if_dirty_tree()
    _download_and_swap_zip(branch, MARCEL_RELEASE_ARCHIVE_URL.format(ref=f"refs/heads/{branch}"))
    ...
    if not import_ok:
        ...
        print("  Re-run `marcel update` to complete it.")
        _m().sys.exit(1)
