"""Top-level argparse construction for the marcel CLI.

Only the top-level parser and the ``chat`` subparser live here. Every other subparser (model,
gateway, sessions, …) is built by ``marcel_cli/subcommands/<group>.py`` and wired in
...
# (it sets ``MARCEL_HOME`` and strips itself from ``sys.argv``), so it isn't on the parser.
...
# live parser fails. The derived path is authoritative; tests/marcel_cli/
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    argparse surface — the drift that made ``marcel --reasoning high chat …`` misread ``high`` as
...
def _inherited_flag(parser, *args, **kwargs):
    """``parser.add_argument`` + tag the Action ``inherit_on_relaunch`` for ``marcel_cli.relaunch``."""
...
_EPILOGUE = """
Examples:
    marcel                        Start interactive chat
    marcel chat -q "Hello"        Single query mode
    marcel --tui                  Launch the modern TUI (or set display.interface: tui)
    marcel --cli                  Force the classic REPL (overrides display.interface: tui)
    marcel -c                     Resume the most recent session
    marcel -c "my project"        Resume a session by name (latest in lineage)
    marcel --resume <session_id>  Resume a specific session by ID
    marcel --resume latest        Resume the most recent session (same as -c)
    marcel --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    marcel setup                  Run setup wizard
    marcel logout                 Clear stored authentication
    marcel auth add <provider>    Add a pooled credential
    marcel auth list              List pooled credentials
    marcel auth remove <p> <t>    Remove pooled credential by index, id, or label
    marcel auth reset <provider>  Clear exhaustion status for a provider
    marcel model                  Select default model
    marcel fallback [list]        Show fallback provider chain
    marcel fallback add           Add a fallback provider (same picker as `marcel model`)
    marcel fallback remove        Remove a fallback provider from the chain
    marcel config                 View configuration
    marcel config edit            Edit config in $EDITOR
    marcel config set model gpt-4 Set a config value
    marcel gateway                Run messaging gateway
    marcel -s marcel-agent-dev,github-auth
    marcel -w                     Start in isolated git worktree
    marcel gateway install        Install gateway background service
    marcel sessions list          List past sessions
    marcel sessions browse        Interactive session picker
    marcel sessions rename ID T   Rename/title a session
    marcel logs                   View agent.log (last 50 lines)
    marcel logs -f                Follow agent.log in real time
    marcel logs errors            View errors.log
    marcel logs --since 1h        Lines from the last hour
    marcel debug share             Upload debug report for support
    marcel console                Open the safe Marcel command console
    marcel update                 Update to latest version
    marcel dashboard              Start web UI dashboard (port 9119)
    marcel dashboard --stop       Stop running dashboard processes
    marcel dashboard --status     List running dashboard processes
...
def _display_epilogue(entrypoint: str) -> str:
    """Brand command examples without renaming compatibility identifiers."""
    text = _EPILOGUE.replace("    marcel ", f"    {entrypoint} ")
    text = text.replace("    marcel\n", f"    {entrypoint}\n")
    return text.replace("`marcel ", f"`{entrypoint} ")


def _active_entrypoint() -> str:
    return "marcel" if os.path.basename(sys.argv[0]).lower().startswith("marcel") else "marcel"
...
def _add_top_level_flags(parser: argparse.ArgumentParser) -> None:
    ...
    inherited(parser, "-m", "--model", default=None, help=(
        ...
        "Applies to -z/--oneshot and --tui. Also settable via MARCEL_INFERENCE_MODEL env var."))
    inherited(parser, "--provider", default=None, help=(
        "Provider override for this invocation (e.g. openrouter, anthropic). "
        "Applies to -z/--oneshot and --tui. The persistent provider lives in config.yaml "
        f"under model.provider — use `{entrypoint} setup` or edit the file to change it."))
    ...
    inherited(parser, "--accept-hooks", action="store_true", default=False, help=(
        ...
        "without a TTY prompt.  Equivalent to MARCEL_ACCEPT_HOOKS=1 or "
    ...
    inherited(parser, "--ignore-user-config", action="store_true", default=False,
              help="Ignore ~/.marcel/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)")
...
def _build_chat_parser(subparsers) -> argparse.ArgumentParser:
    ...
    ``marcel -m foo chat`` argparse first sets ``args.model`` from the top-level parser, then
    ...
    unless the flag is actually passed after the subcommand (tests/marcel_cli/
    test_argparse_flag_propagation.py).
    """
    chat_parser = subparsers.add_parser(
        "chat", help="Interactive chat with the agent",
        description="Start an interactive chat session with Marcel Agent")
    ...
    inherited(chat_parser, "--accept-hooks", action="store_true", default=SUPPRESS, help=(
        "Auto-approve any unseen shell hooks declared in config.yaml "
        "without a TTY prompt (see also MARCEL_ACCEPT_HOOKS env var and "
    ...
    inherited(chat_parser, "--ignore-user-config", action="store_true", default=SUPPRESS,
              help="Ignore ~/.marcel/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.")
    ...
    inherited(chat_parser, "--safe-mode", action="store_true", default=SUPPRESS,
              help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Marcel itself.")
...
def build_top_level_parser():
    ...
    entrypoint = _active_entrypoint()
    description = (
        "Marcel - autonomous multi-agent framework"
        if entrypoint == "marcel"
        else "Marcel Agent - AI assistant with tool-calling capabilities"
    )
    parser = argparse.ArgumentParser(
        prog=entrypoint, description=description,
        formatter_class=argparse.RawDescriptionHelpFormatter, epilog=_display_epilogue(entrypoint))
