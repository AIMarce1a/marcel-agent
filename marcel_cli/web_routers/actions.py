"""Gateway restart/drain, Marcel update and background-action status dashboard routes.

Extracted from ``marcel_cli.web_server``; helpers/state that tests monkeypatch on
...
from marcel_cli.config import format_docker_update_message, recommended_update_command_for_method
from marcel_cli.web_deps import LateState, late
from marcel_cli.web_server_gateway import _ACTION_LOG_FILES
from marcel_cli.web_routers._common import http_failure
...
detect_install_method = late("detect_install_method", "marcel_cli.config")
get_marcel_home = late("get_marcel_home", "marcel_cli.config")
_ACTION_COMMANDS = LateState("_ACTION_COMMANDS", "marcel_cli.web_server_gateway")
_ACTION_IDS = LateState("_ACTION_IDS", "marcel_cli.web_server_gateway")
_ACTION_PROCS = LateState("_ACTION_PROCS", "marcel_cli.web_server_gateway")
_ACTION_RESULTS = LateState("_ACTION_RESULTS", "marcel_cli.web_server_gateway")
...
def _server_path(name: str) -> Path:
    ...
    import marcel_cli.web_server_gateway as gw
...
_UPDATE_ACTION_COMPLETED_RE = re.compile(r"^=== marcel-update completed ([0-9a-f]{32}) ===$")

_MANAGED_EXTERNALLY_MESSAGE = "Marcel updates are managed outside this dashboard in containerized environments."
...
def _durable_completed_update_action_id(lines: List[str]) -> Optional[str]:
    ...
    for index, line in enumerate(lines):
        if line.startswith("=== marcel update started "):
            last_start = index
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``marcel gateway restart`` in the background."""
...
def _update_refused(error: str, message: str, update_command: str) -> Dict[str, Any]:
    _record_completed_action("marcel-update", message, exit_code=1)
    return {
        "ok": False, "pid": None, "name": "marcel-update", "error": error, "message": message,
        "update_command": update_command,
    }


@router.post("/api/marcel/update")
async def update_marcel():
    """Kick off ``marcel update`` in the background."""
    ...
    existing = _ACTION_PROCS.get("marcel-update")
    if existing is not None and existing.poll() is None:
        response = {"ok": True, "pid": existing.pid, "name": "marcel-update", "already_running": True}
        action_id = _ACTION_IDS.get("marcel-update")
        if action_id:
            response["action_id"] = action_id
        return response

    action_id = secrets.token_hex(16)
    with http_failure("Failed to spawn marcel update", 500, "Failed to start update"):
        proc = _spawn_marcel_action(["update"], "marcel-update", env_overrides={"MARCEL_ACTION_ID": action_id})
    return {"ok": True, "pid": proc.pid, "name": "marcel-update", "action_id": action_id}
...
_NON_APPLYABLE_MESSAGES = {
    ...
    "apt": lambda: "Marcel is managed by Termux APT; run `pkg upgrade marcel-agent`.",
}


@router.get("/api/marcel/update/check")
async def check_marcel_update(force: bool = False):
    """Report whether a Marcel update is available, without applying it.
    ...
    try:
        from marcel_cli.banner import check_for_updates

        if force:
            with contextlib.suppress(OSError):
                (get_marcel_home() / ".update_check").unlink()
...
async def get_action_status(name: str, lines: int = 200):
    ...
    if name == "marcel-update":
        ...
        if durable_update_action_id:
            marker = f"=== marcel-update completed {durable_update_action_id} ==="
...
def _latest_update_receipt_summary() -> Optional[Dict[str, Any]]:
    """Compact summary of the latest receipt (written by EVERY ``marcel update`` run,
    incl. refused/failed), or None; never raises. Steps/skips stay in the full endpoint.

    Phase-1 bullet 3 (#91277): the receipt (written by EVERY ``marcel update`` run since #91283, including
...
@status_router.get("/api/marcel/update/receipt")
async def get_update_receipt():
    ...
    receipt = _read_latest_receipt()
    if not receipt:
        raise HTTPException(status_code=404, detail="No update receipt found (no `marcel update` run recorded).")
