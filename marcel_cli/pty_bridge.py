"""PTY bridge for `marcel dashboard` chat tab.
...
``marcel_cli.web_server``. POSIX-only: depends on ``fcntl``, ``termios`` and ``ptyprocess`` (native
...
# ui-tui/packages/marcel-ink/src/ink/termio/host.ts — keep the two in sync.
PTY_HOST_ENV = "MARCEL_PTY_HOST"
        ...
        if not _PTY_AVAILABLE:
            if sys.platform.startswith("win"):
                raise PtyUnavailableError("Pseudo-terminals are unavailable on this platform. "
                                          "Marcel Agent supports Windows only via WSL.")
