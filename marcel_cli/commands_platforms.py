...
from marcel_cli.commands import (
    COMMAND_REGISTRY, _is_gateway_available, _iter_plugin_command_entries, _resolve_config_gates)

# Logger name parity with the origin module (tests capture "marcel_cli.commands").
logger = logging.getLogger("marcel_cli.commands")
...
def discord_skill_commands_by_category(
    ...
    parity, external-dir skills are visible via ``marcel skills list`` and the agent's ``/skill-name``
...
# Canonical commands deliberately routed through ``/marcel <command>`` on Slack only: the registry
...
_SLACK_VIA_MARCEL_ONLY = frozenset({
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    _SLACK_VIA_MARCEL_ONLY are skipped. ``/marcel`` is always first for anything dropped."""
...
def slack_app_manifest(
    request_url: str = "https://marcel-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """name/alias -> "/command" for the Slack ``/marcel`` handler, plugin commands included."""
