...
dependency/config-schema warnings. Mixed into :class:`marcel_cli.plugins.PluginManager`.

Origin-internal names (``PluginContext``, ``LoadedPlugin``, ``_PLUGINS_DEBUG`` …) are imported lazily
through ``marcel_cli.plugins`` so tests that patch them on the origin keep working.
"""
...
from marcel_constants import get_marcel_home, reset_marcel_home_override, set_marcel_home_override
from registration_lifecycle import replacement_coordinator
from marcel_cli.plugins_discovery import ENTRY_POINTS_GROUP, _select_entry_point_group
from marcel_cli.plugins_manifest import PluginManifest, manifest_key, validate_config_schema
from marcel_cli.plugins_state import _plugin_settings_entry

if TYPE_CHECKING:  # pragma: no cover
    from marcel_cli.plugins import LoadedPlugin

logger = logging.getLogger("marcel_cli.plugins")

_NS_PARENT = "marcel_plugins"
...
def _plugin_home_scope(home: Path):
    """Bind discovery and loading to the manager's immutable Marcel home."""
...
class PluginLoaderMixin:
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``marcel plugins list`` until then."""
        from marcel_cli.plugins import LoadedPlugin
    ...
    def _register_deferred_platform_tools(self, manifest: PluginManifest, loaded: LoadedPlugin) -> None:
        ...
        would miss them in ``marcel tools`` / ``platform_toolsets``. Opt-in is explicit via ``provides_tools``;
        ...
        returns ``[]``, the toolset is missing from the ``marcel tools`` checklist, and even an explicit
        ...
        """
        from marcel_cli.plugins import PluginContext, _PLUGINS_DEBUG
        ...
        try:
            ...
            # Tools registered before the raise are live: credit them or `marcel plugins list` under-reports
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        See #64165.
        python_dependencies is a declaration seam ONLY: Marcel validates and prints the requirements with an
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. Marcel does not install plugin dependencies "
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        from marcel_cli.plugins import LoadedPlugin, PluginContext, _PLUGINS_DEBUG
        ...
        # skipped with a clear reason instead of dying on ImportError mid-register (marcel_cli.plugin_compat).
        from marcel_cli.plugin_compat import disable_reason
    ...
    def _track_tool_override_policy(self, manifest: PluginManifest, module_name: str) -> None:
        ...
        from marcel_cli.plugins import PluginContext
    ...
    def _attribute_registrations(
        ...
        # Discovery-time tools predate registration_start; credit them back or `marcel plugins list`
    ...
    def _load_portable_plugin(self, manifest: PluginManifest, loaded: LoadedPlugin) -> None:
        ...
        lookup_key = manifest_key(manifest)
        try:
            from marcel_cli.agent_plugins import load_agent_plugin
            package = load_agent_plugin(
                Path(manifest.path), get_marcel_home() / "plugin-data" / manifest.skill_namespace)
    ...
    def _directory_module_name(self, manifest: PluginManifest) -> str:
        """Profile-safe import namespace for a directory plugin: the bare ``marcel_plugins.<slug>`` for the
    ...
    def _load_directory_module(
        ...
        """Import a directory plugin as ``marcel_plugins.<slug>`` (slug from ``manifest.key`` so
        ...
        # Evict stale entries for this slug (same slug cached from another Marcel home, or an earlier force
