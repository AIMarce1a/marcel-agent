...
from marcel_cli.config import DEFAULT_CONFIG, get_process_marcel_home
from marcel_cli.web_models import MCPServerCreate
from marcel_cli.web_server_gateway import _ACTION_LOG_FILES
from marcel_cli.web_server_mcp import _normalize_mcp_server_create
...
@contextmanager
def _marcel_home_scope(path) -> Any:
    """Scope ``load_config``/``save_config`` (anything resolving ``get_marcel_home()`` at call
    time) to ``path`` for the block via the context-local MARCEL_HOME override."""
...
def _is_other_profile(profile: Optional[str]) -> bool:
    ...
    try:
        ...
        return True
    return target.resolve() != get_process_marcel_home().resolve()
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_marcel_home()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    """Write MCP server entries into ``profile_dir``'s config.yaml (MARCEL_HOME-scoped).
    ...
    from marcel_cli.mcp_config import _save_bearer_auth_token
    written = 0
...
def _profile_scope(profile: Optional[str]):
    ...
    Two seams: (1) ``load_config``/``save_config`` resolve ``get_marcel_home()`` at call
    ...
    ``get_marcel_home()`` so writes land in the live home even when the import-time binding
    is stale (test isolation, late MARCEL_HOME override). Yields the profile dir for a named
    ...
    with _config_profile_scope(profile) as scoped:
        profile_dir = get_marcel_home() if scoped is None else scoped
        modules = (_skills_tool, _skill_mgr)
        with _SKILLS_PROFILE_LOCK:
            saved = [(m.MARCEL_HOME, m.SKILLS_DIR) for m in modules]
            for m in modules:
                m.MARCEL_HOME, m.SKILLS_DIR = profile_dir, profile_dir / "skills"
            try:
                yield scoped
            finally:
                for m, (home, skills_dir) in zip(modules, saved):
                    m.MARCEL_HOME, m.SKILLS_DIR = home, skills_dir


@contextmanager
def _config_profile_scope(profile: Optional[str]):
    """Await-safe, config-only profile scope: touches ONLY the task-local MARCEL_HOME
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    a fresh ``marcel`` subprocess whose ``_apply_profile_override()`` reads ``-p`` from argv —
...
def _hub_action_name(verb: str, key: str) -> str:
    """Unique per-skill hub action name (+ registered log file): ``_spawn_marcel_action``
