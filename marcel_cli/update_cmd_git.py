"""Git plumbing for ``marcel update``: fork/upstream sync, trampoline-git detection, lockfile/EOL churn cleanup, orphan rescue refs, parked-branch assessment, fetch-failure classification.
...
from marcel_cli.release_coordinates import MARCEL_RELEASE_HTTPS_URL, UPSTREAM_COMPARISON_REPOSITORY
...
_BAR = "=" * 68
UPSTREAM_COMPARISON_ADD_CMD = f"git remote add upstream {UPSTREAM_COMPARISON_REPOSITORY}"
...
def _git_stdout(git_cmd, args, cwd, **kw) -> Optional[str]:
    ...
    from marcel_cli.update_cmd import _git_run
...
def _prune_orphan_rescue_refs(
    ...
    """Expire old orphan rescue refs (``refs/marcel-update-backups/orphan-<branch>-<ts>-<sha>``).
    ...
    with suppress(OSError):
        prefix = f"refs/marcel-update-backups/orphan-{branch}-"
...
def _print_parked_branch_skip_warning(git_cmd: list[str], cwd: Path, current_branch: str, target_branch: str, reason: str) -> None:
    ...
    print(
        ...
        f"    git -C {cwd} status\n"
        f"    git -C {cwd} checkout {target_branch} && marcel update\n"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_marcel_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    with suppress(Exception):
        from marcel_constants import get_marcel_home
        (get_marcel_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
...
def _offer_upstream_remote(git_cmd: list[str], cwd: Path, *, assume_yes: bool, input_fn) -> bool:
    ...
    from marcel_cli.update_cmd import _add_upstream_remote, _mark_skip_upstream_prompt
    print(
        "\nℹ Your fork is not tracking the official Marcel repository.\n"
        "  This means you may miss updates from the official Marcel repository.\n"
    )
    if assume_yes or (input_fn is None and not (sys.stdin.isatty() and sys.stdout.isatty())):
        print(f"  Skipping upstream setup (non-interactive run).\n  Add it later with: {UPSTREAM_COMPARISON_ADD_CMD}")
        return False
    ...
    if response not in {"", "y", "yes"}:
        print(f"  Skipped. Run '{UPSTREAM_COMPARISON_ADD_CMD}' to add later.")
    ...
    if not _add_upstream_remote(git_cmd, cwd):
        ...
        return False
    print(f"  ✓ Added upstream comparison remote: {UPSTREAM_COMPARISON_REPOSITORY}")
    return True
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path, *, assume_yes: bool = False, input_fn=None) -> bool:
    ...
    See #97052.
    """
    from marcel_cli.update_cmd import _count_commits_between, _has_upstream_remote, _no_prompt_git_kwargs, _should_skip_upstream_prompt
...
def _portable_git_candidates() -> list:
    ...
    profile-scoped MARCEL_HOME), then profile home as a fallback for custom layouts.

    The Marcel-managed PortableGit tree lives under the SHARED root (``<root>/git/...``), not the
    profile-scoped MARCEL_HOME (``<root>/profiles/<name>``), so a profile-scoped ``marcel update`` must look
    there (monerostar review, #87876).
    """
    from marcel_cli.update_cmd import get_default_marcel_root, get_marcel_home
    candidates = []
    with suppress(Exception):
        candidates += [root / "git" / "mingw64" / "libexec" / "git-core" / "git.exe" for root in (get_default_marcel_root(), Path(get_marcel_home()))]
    return candidates
...
def _ensure_non_trampoline_git(git_cmd: list) -> list:
    ...
    from marcel_cli.update_cmd import _locate_real_git
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    on update; install.ps1 pins ``false`` but older checkouts never got it and only ``marcel update`` can
    ...
    reuses its build-pinned ``install.ps1`` forever — so ``marcel update``, which ships with the checkout
