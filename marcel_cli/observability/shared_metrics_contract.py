"""Bounded product contract for the first Marcel shared-metrics slice."""
...
SCHEMA_KEY = "marcel.metrics.schema_version"
SCHEMA_VERSION = "marcel.metrics.event.v2"
MODEL_CALL_SCOPE = "marcel.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "marcel.task_run"
TOOL_CALL_SCOPE = "marcel.tool_call"
CLIENT_ACTIVE_MARK = "marcel.client.active"
TOOL_APPROVAL_MARK = "marcel.tool_approval"
SKILL_LIFECYCLE_MARK = "marcel.skill.lifecycle"
SKILL_LOAD_MARK = "marcel.skill.load"
SUBSCRIBER_NAME = "marcel.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "marcel.client.active"
LEGACY_MODEL_CALL_METRIC = "marcel.model_call.count"
MODEL_ROUTE_METRIC = "marcel.model_route.count"
TASK_STARTED_METRIC = "marcel.task_run.started"
TASK_FINISHED_METRIC = "marcel.task_run.finished"
TOOL_CALL_METRIC = "marcel.tool_call.count"
TOOL_APPROVAL_METRIC = "marcel.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "marcel.skill.lifecycle.count"
SKILL_LOAD_METRIC = "marcel.skill.load.count"
...
CLIENT_RESOURCE_KEYS = frozenset({"architecture", "marcel_version", "install_method", "os_family"})
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Marcel installation method."""
    normalized = _norm(value)
    return _allowlisted("nixos" if normalized == "nix" else normalized, CLIENT_INSTALL_METHODS)


def client_resource(
    marcel_version: Any, *, os_name: Any, architecture: Any, install_method: Any
) -> dict[str, str]:
    """Build the bounded client resource attached to aggregate packages."""
    version = str(marcel_version or "").strip()
    return {
        "architecture": client_architecture(architecture),
        "marcel_version": version if 0 < len(version) <= 64 else "unknown",
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("marcel_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Marcel logical scope."""
    metadata = _relay_metadata(
        event, RUNTIME_SCHEMA_KEY, RUNTIME_SCHEMA_VERSION, "marcel.call_role"
    )
    call_role = (metadata or {}).get("marcel.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from marcel_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Marcel terminal state to bounded (outcome, end_reason, termination)."""
...
_TOOL_CATEGORY_PREFIXES = (
    ...
    (("discord", "email", "feishu", "marcel-yuanbao", "slack", "sms"), "communication"),
)


def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Marcel registry toolset metadata to a low-cardinality category."""
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Marcel tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Marcel."""
