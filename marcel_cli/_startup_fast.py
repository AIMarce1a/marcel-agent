...
Imported by ``marcel_cli/main.py`` BEFORE its heavy import wall (config, argparse tree, logging,
providers). Everything here must stay **stdlib-only and cheap** (os/sys file probes; no yaml, no
marcel_cli.config, no argparse). Exists so version-printing stops being reimplemented as
...
def active_profile_may_override_home(marcel_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect MARCEL_HOME?"""
    active = (_read_text(os.path.join(marcel_root, "active_profile")) or "").strip()
    return bool(active and active != "default")


def _default_home() -> str:
    return os.path.join(os.path.expanduser("~"), ".marcel")


def _resolved_home() -> str:
    return os.environ.get("MARCEL_HOME", "").strip() or _default_home()
...
def container_mode_may_be_active() -> bool:
    ...
    be active".
    """
    if os.environ.get("MARCEL_DEV") == "1" or is_container_startup_environment():
        return False
    marcel_home = os.environ.get("MARCEL_HOME", "").strip()
    if marcel_home:
        if os.path.exists(os.path.join(marcel_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(marcel_home)))
        return parent_name != "profiles" and active_profile_may_override_home(marcel_home)
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    """THE canonical ``marcel --version`` output (also used by /version).
    ...
    try:
        ...
        from marcel_cli import __release_date__, __version__

        print(f"Marcel Agent v{__version__} ({__release_date__})")
    ...
    try:
        from marcel_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``marcel --version`` before the heavy import wall.
    ...
    MARCEL_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
    is_termux = is_termux_env()
    if is_termux and os.environ.get("MARCEL_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
