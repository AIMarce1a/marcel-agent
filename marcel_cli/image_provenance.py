"""Image-authored deployment provenance for immutable Marcel runtimes.

The image bakes ``/etc/marcel/image-provenance.json`` outside ``$MARCEL_HOME`` and the checkout, so a
...
IMAGE_PROVENANCE_PATH = Path("/etc/marcel/image-provenance.json")
