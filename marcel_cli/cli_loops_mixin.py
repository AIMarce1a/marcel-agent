...
Mixin on ``MarcelCLI``; cli.py symbols are imported lazily inside methods (import cycle)."""
...
class CLILoopsMixin:
    ...
    def _cmd_version(self, cmd_original: str):
        from marcel_cli.main import _print_version_info
        _print_version_info(check_updates=True)

    def _cmd_reload(self, cmd_original: str):
        from marcel_cli.config import reload_env
        count = reload_env()
    ...
    def _cmd_plugins(self, cmd_original: str):
        ...
        try:
            # Discover from disk (bundled + user) like `marcel plugins list`, so
            ...
            # on one line (full catalog behind `marcel plugins list`).
            user_entries = [e for e in entries if e[3] != "bundled"]
            bundled_count = len(entries) - len(user_entries)
            if not user_entries:
                print("No user plugins installed.")
                print("  Install one: marcel plugins install owner/repo")
                print(f"  Or drop a plugin directory into {display_marcel_home()}/plugins/")
                if bundled_count:
                    print(f"  ({bundled_count} bundled plugins available — see: marcel plugins list)")
                return
            ...
            if bundled_count:
                print(f"  (+{bundled_count} bundled — see: marcel plugins list)")
            print("  Enable/disable: marcel plugins enable/disable <name>")
    ...
    def _get_heartbeat_manager(self):
        ...
        def load():
            from marcel_cli.heartbeat import HeartbeatManager
    ...
    def _start_heartbeat_watchdog(self):
        ...
        from marcel_cli.heartbeat import POLL_SECONDS
    ...
    def _maybe_continue_goal_after_turn(self) -> None:
        ...
        if not last_response.strip():
            return
        try:
            from marcel_cli.goals import gather_background_processes as _gather_bg
