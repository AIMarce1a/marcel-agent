"""Dashboard process-hygiene helpers — extracted from ``marcel_cli/main.py``.

Helpers defined in ``marcel_cli.main_dashboard`` / ``marcel_cli.main_install_repair`` are imported at
...
_DASHBOARD_PATTERNS = tuple(
    ...
    for launcher in ("marcel", "marcel_cli.main", "marcel_cli/main.py"))
...
def _iter_process_table() -> list[tuple[int, str]]:
    ...
    if sys.platform == "win32":
        ...
        return rows
    # ps, not `pgrep -f "marcel.*dashboard"` (greedy regex; consistent with gateway pid scan).
...
def _scan_dashboard_processes(*, exclude_pids: set[int] | None = None) -> list[tuple[int, str]]:
    ...
    ``marcel update`` (every API call 401s). *exclude_pids* (Desktop's MARCEL_DESKTOP_CHILD_PID
    backends) are never returned.

    *exclude_pids* is an optional set of PIDs that must never be returned. This is used by the Marcel
    Desktop Electron app to protect its own backend child process: when the desktop spawns ``marcel serve``
    as a backend and triggers an auto-update, the update must not kill the backend that the desktop itself
    manages. The desktop sets the environment variable ``MARCEL_DESKTOP_CHILD_PID`` on the spawned backend
    ...
    try:
        ...
        return []
    # Spawn-ledger augmentation: substring patterns miss profiled launches (`marcel --profile p
    ...
    with contextlib.suppress(Exception):
        ...
        from marcel_cli.process_identity import ledger_entries
...
def _marcel_home_for_pid(pid: int) -> str | None:
    """Best-effort ``MARCEL_HOME`` from *pid*'s environment (psutil, then /proc)."""
    with contextlib.suppress(Exception):
        import psutil
        if home := psutil.Process(pid).environ().get("MARCEL_HOME"):
            return home
    try:
    ...
    for part in raw.split(b"\x00"):
        if part.startswith(b"MARCEL_HOME="):
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    ``marcel update`` multiplies listening backends because ``--port 0`` binds a fresh port.
...
def _profile_key_for_respawn(argv: list[str], marcel_home: str | None = None) -> str:
    """Stable owner key: ``MARCEL_HOME`` when known, else ``--profile`` / ``-p``.
    ...
    """
    if marcel_home:
        parts = _resolved_home(marcel_home).parts
        if len(parts) >= 2 and parts[-2] == "profiles" and parts[-1]:
            return f"profile:{parts[-1]}"
        return f"home:{_normalized_home_for_compare(marcel_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    """Select which killed manual backends ``(pid, argv, marcel_home)`` to respawn after update.

    Rules: never resurrect Desktop ``--port 0`` backends; never replay a backend from a
    **foreign** ``MARCEL_HOME`` (the argv-only respawn would come back on this install's home
    ...
    (``MARCEL_DESKTOP_CHILD_PID``) owns their lifecycle. These are also the PPID-1 orphans that previously
    ...
    Intentionally does **not** blanket-skip every PPID-1 process: a prior ``marcel update`` respawn detaches
    ...
    for _pid, argv, marcel_home in candidates:
        if not argv or _is_ephemeral_port_zero_backend(argv):
            continue
        if own_key and marcel_home and _normalized_home_for_compare(marcel_home) != own_key:
            continue
        norm = _normalize_dashboard_cmdline(argv)
        profile_key = _profile_key_for_respawn(argv, marcel_home)
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (``MARCEL_DESKTOP_CHILD_PID``, comma-separated)."""
    out: set[int] = set()
    for part in os.environ.get("MARCEL_DESKTOP_CHILD_PID", "").split(","):
...
def _kill_pids_windows(pids: list[int], killed: list[int], failed: list[tuple[int, str]]) -> None:
    ...
    for pid in pids:
        try:
            ...
            elif not pid_is_marcel(pid, expected_start_time=expected_start_time):
                failed.append((pid, "not marcel-owned or process identity changed"))
            else:
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``marcel dashboard`` / ``marcel serve`` processes (update end, ``--stop``).
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the caller already restarted directly —
    e.g. ``marcel update``'s systemd fleet-restart loop, which restarts ``marcel-serve*`` units before this
    ...
    """
    from marcel_cli import main_dashboard as _dash

    if restart_managed and _dash._restart_managed_dashboard_service(reason):
        # The dashboard unit is handled but other backends (e.g. marcel-serve.service) are not:
        # mark the unit handled so the filter below drops its PIDs, and keep going.
        _dash_unit = getattr(_dash, "_DASHBOARD_SYSTEMD_UNIT", "marcel-dashboard.service")
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid] and (cmdline := _dash._dashboard_cmdline_for_pid(pid)):
                # Manual process: exact argv + MARCEL_HOME for the respawn and its profile cap.
                # Manually-started process: preserve its exact argv so we can respawn it after the update
                # (#40449, #68934). Snapshot MARCEL_HOME before the kill so per-profile caps still work
                # after the process is gone (#78821).
                pid_cmdline[pid] = cmdline
                pid_home[pid] = _marcel_home_for_pid(pid)
...
def _restart_killed_backends(
    ...
    # Two categories: Without this, a remote backend (marcel serve) under Restart=on-failure never comes
...
def _detect_concurrent_marcel_instances(
    ...
    Windows blocks DELETE/REPLACE on a running .exe, so a Desktop-spawned ``marcel.EXE`` makes
    ...
    *shim* ancestor (the setuptools launcher is a separate native process from its
    ...
    """
    from marcel_cli.main_install_repair import _marcel_exe_shims, _is_windows
    ...
    try:
        ...
        return []
    shim_paths = {_norm_exe(shim) for shim in _marcel_exe_shims(scripts_dir)}
    if not shim_paths:
        return []
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    """True for the Desktop-local shape ``marcel serve [--isolated] --host 127.0.0.1 --port 0``.
    ...
    """
    cmd = command.lower()
    if "serve" not in cmd or ("marcel" not in cmd and "marcel_cli" not in cmd):
        return False
...
# (apps/desktop/electron/remote-lifecycle.ts) for every ``marcel serve`` it spawns. Such a backend
...
def _marcel_home_dir() -> Path:
    """Resolved Marcel home (MARCEL_HOME override or ~/.marcel)."""
    override = os.environ.get("MARCEL_HOME", "").strip()
    return Path(override).expanduser() if override else Path.home() / ".marcel"
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    if any(not isinstance(parsed.get(f), str) or len(parsed[f]) > 1024
           for f in ("profile", "marcelPath", "marcelHome", "logPath", "startedAt")):
        return False
    # Suffix-only check of logPath so a relocated MARCEL_HOME can't reject a legitimate backend.
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    """PIDs claimed by valid ``{marcel_home}/desktop-ssh/<ownershipId>/backend.lock.json`` records
    (best-effort: a bad record contributes no PID; never raises)."""
    import json
    root = base_dir if base_dir is not None else _marcel_home_dir() / _REMOTE_LOCK_SUBDIR
    owned: set[int] = set()
    try:
...
def _reap_orphaned_desktop_local_serves(
    *, reason: str = "orphaned desktop-local marcel serve", signal_term=None, signal_kill=None,
    sleep_fn=None, lock_owned_pids_fn=None, process_age_seconds_fn=None) -> dict[str, list]:
    """Kill leftover Desktop-local ``marcel serve`` backends with no parent. Never raises.
    ...
    0/1; not self / parent / MARCEL_DESKTOP_CHILD_PID; not claimed by a valid
    ...
    after MARCEL_BACKEND_READY, so a live sibling mid-startup is briefly unowned).
    """
    import signal as _signal
    import time as _time
