...
def auto_setup_telegram_bot_result(
    api_url: str | None = None, manager_bot: str = "MarcelSetupBot",
    ...
    print(f"\n  Contacting Marcel Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Marcel Telegram onboarding service.\n"
...
DEFAULT_MANAGER_BOT = "MarcelSetupBot"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``marcel_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``marcel_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"marcel_{generate_username_slug()}_bot"
