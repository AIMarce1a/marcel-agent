...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    API-key prompt for users who logged in via ``marcel model`` -> xAI Grok OAuth."""
    try:
...
_TTS_PROVIDER_CHOICES = [
    ...
    ("xai", "xAI TTS (recommended premium — multilingual Grok voices, OAuth or API key)"),
...
def _xai_api_key_path():
    ...
    return ("No xAI API key provided for TTS. Configure XAI_API_KEY via marcel setup model "
            f"or {_dhh()}/.env to use xAI TTS. Falling back to Edge TTS.")


def _tts_xai_step(config: dict, *, prompt_for_voice: bool = True) -> str:
...
def _setup_tts_provider(config: dict, *, prompt_for_voice: bool = True):
    ...
    if selected == "nous-openai":
        ...
        selected = _tts_xai_step(config, prompt_for_voice=prompt_for_voice)
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'marcel setup tts')."""
