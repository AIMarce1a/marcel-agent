...
from marcel_cli._subprocess_compat import windows_detach_flags
...
def _probe_gateway_health() -> tuple[bool, dict | None]:
    ...
    """
    from marcel_cli.web_server import _GATEWAY_HEALTH_TIMEOUT, _GATEWAY_HEALTH_URL
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    """
    try:
        from marcel_cli.profiles import _check_gateway_running, profiles_to_serve
...
# ~/.marcel/logs/<action>.log which the dashboard tails.

_ACTION_LOG_DIR: Path = get_marcel_home() / "logs"
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "marcel-update": "marcel-update.log",
...
def _dashboard_spawn_executable() -> str:
    ...
    """
    from marcel_cli.web_server import PROJECT_ROOT
...
def _spawn_marcel_action(
    subcommand: List[str], name: str, *, env_overrides: Optional[Dict[str, str]] = None
) -> subprocess.Popen:
    """Spawn ``marcel <subcommand>`` detached (via ``marcel_cli.main``) and record the handle."""
    from marcel_cli.web_server import PROJECT_ROOT
    _ACTION_LOG_DIR.mkdir(parents=True, exist_ok=True)
    log_file = open(_ACTION_LOG_DIR / _ACTION_LOG_FILES[name], "ab", buffering=0)
    log_file.write(f"\n=== {name} started {time.strftime('%Y-%m-%d %H:%M:%S')} ===\n".encode())

    cmd = [_dashboard_spawn_executable(), "-m", "marcel_cli.main", *subcommand]
    # The dashboard runs inside the gateway process, so os.environ carries _MARCEL_GATEWAY=1;
    ...
    action_env = {**os.environ, "MARCEL_NONINTERACTIVE": "1"}
    action_env.pop("_MARCEL_GATEWAY", None)
    ...
    action_id = (env_overrides or {}).get("MARCEL_ACTION_ID")
    if action_id:
...
def _gateway_subcommand(profile: Optional[str], verb: str) -> List[str]:
    from marcel_cli.web_server_profiles import _profile_cli_args
...
def _restart_gateway_after(profile: Optional[str], *, what: str, label: str) -> dict[str, Any]:
    ...
    from marcel_cli.web_server import _spawn_gateway_restart
    try:
