"""Marcel Desktop (Chat GUI) uninstaller: removes only GUI state — built Electron artifacts, the packaged
...
def desktop_userdata_dir() -> Path:
    """Electron ``app.getPath('userData')`` for an app named "Marcel" on each platform (GUI-only state)."""
    home = Path.home()
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Marcel"
    if sys.platform == "win32":
        return _env_dir("APPDATA", home / "AppData" / "Roaming") / "Marcel"
    return _env_dir("XDG_CONFIG_HOME", home / ".config") / "Marcel"


def source_built_gui_artifacts(marcel_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``marcel desktop`` inside the checkout (same ``marcel-agent/`` layout
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Marcel" product."""
    home = Path.home()
    if sys.platform == "darwin":
        return [Path("/Applications/Marcel.app"), home / "Applications" / "Marcel.app"]
    if sys.platform == "win32":
        ...
        return [local_base / "Programs" / "Marcel", local_base / "marcel-desktop"] + (
            [Path(program_files) / "Marcel"] if program_files else [])
    ...
    from marcel_cli.linux_desktop_entry import desktop_entry_path
    ...
    return [desktop_entry_path(), data_base / "applications" / "Marcel.desktop"] + [
        icons / size / "apps" / "marcel.png"
        for size in ("scalable", "24x24", "32x32", "48x48", "256x256", "512x512", "1024x1024")]


def agent_is_installed(marcel_home: Path) -> bool:
    """True when a usable Python agent install exists under MARCEL_HOME (gates the desktop UI's options).
    Package source or a venv alone is enough — a source checkout without a venv is still "the agent is here"."""
    return any((marcel_home / "marcel-agent" / sub).is_dir() for sub in ("marcel_cli", "venv", ".venv"))


def gui_is_installed(marcel_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    return any(p.exists() for p in (*source_built_gui_artifacts(marcel_home), *packaged_gui_app_paths(), desktop_userdata_dir()))


def gui_install_summary(marcel_home: "Path | None" = None) -> dict:
    ...
    return {"marcel_home": str(home), "agent_installed": agent_is_installed(home),
...
def uninstall_gui(marcel_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    if sys.platform.startswith("linux"):
        # The desktop entry was removed above but the menu caches still list it; reindex so Marcel
        # disappears from the launcher.
        try:
            from marcel_cli.linux_desktop_entry import desktop_entry_path, refresh_desktop_databases
        ...
        log_info("If you installed the desktop via a .deb / .rpm package, remove it with your package manager "
                 "(e.g. 'sudo apt remove marcel' or 'sudo dnf remove marcel'). AppImage builds are a single "
