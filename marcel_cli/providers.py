"""Single source of truth for provider identity in Marcel Agent."""
...
# -- Marcel overlay: metadata models.dev doesn't provide ----------------------

@dataclass(frozen=True)
class MarcelOverlay:
    """Marcel-specific provider metadata layered on top of models.dev."""
...
MARCEL_OVERLAYS: Dict[str, MarcelOverlay] = {
    "moa": MarcelOverlay(auth_type="virtual", base_url_override="moa://local"),
    "openrouter": MarcelOverlay(is_aggregator=True, base_url_env_var="OPENROUTER_BASE_URL"),
    "nous": MarcelOverlay(auth_type="oauth_device_code", base_url_override="https://inference-api.nousresearch.com/v1"),
    "openai-codex": MarcelOverlay(transport="codex_responses", auth_type="oauth_external",
                                  base_url_override="https://chatgpt.com/backend-api/codex"),
    "openai-api": MarcelOverlay(transport="codex_responses", base_url_override="https://api.openai.com/v1",
                                base_url_env_var="OPENAI_BASE_URL"),
    "xai-oauth": MarcelOverlay(transport="codex_responses", auth_type="oauth_external",
                               base_url_override="https://api.x.ai/v1", base_url_env_var="XAI_BASE_URL"),
    "qwen-oauth": MarcelOverlay(auth_type="oauth_external", base_url_override="https://portal.qwen.ai/v1",
                                base_url_env_var="MARCEL_QWEN_BASE_URL"),
    "lmstudio": MarcelOverlay(extra_env_vars=("LM_API_KEY",), base_url_override="http://127.0.0.1:1234/v1",
                              base_url_env_var="LM_BASE_URL"),
    "copilot-acp": MarcelOverlay(transport="codex_responses", auth_type="external_process",
                                 base_url_override="acp://copilot", base_url_env_var="COPILOT_ACP_BASE_URL"),
    "github-copilot": MarcelOverlay(extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN")),
    "anthropic": MarcelOverlay(transport="anthropic_messages", extra_env_vars=("ANTHROPIC_TOKEN", "CLAUDE_CODE_OAUTH_TOKEN")),
    "zai": MarcelOverlay(extra_env_vars=("GLM_API_KEY", "ZAI_API_KEY", "Z_AI_API_KEY"), base_url_env_var="GLM_BASE_URL"),
    "kimi-for-coding": MarcelOverlay(base_url_env_var="KIMI_BASE_URL"),
    "stepfun": MarcelOverlay(extra_env_vars=("STEPFUN_API_KEY",),
                             base_url_override="https://api.stepfun.ai/step_plan/v1",
                             base_url_env_var="STEPFUN_BASE_URL"),
    "minimax": MarcelOverlay(transport="anthropic_messages", base_url_env_var="MINIMAX_BASE_URL"),
    "minimax-oauth": MarcelOverlay(transport="anthropic_messages", auth_type="oauth_external",
                                   base_url_override="https://api.minimax.io/anthropic"),
    "minimax-cn": MarcelOverlay(transport="anthropic_messages", base_url_env_var="MINIMAX_CN_BASE_URL"),
    "deepseek": MarcelOverlay(base_url_env_var="DEEPSEEK_BASE_URL"),
    "alibaba": MarcelOverlay(base_url_env_var="DASHSCOPE_BASE_URL"),
    "alibaba-coding-plan": MarcelOverlay(base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL"),
    "vercel": MarcelOverlay(is_aggregator=True),
    "opencode": MarcelOverlay(is_aggregator=True, base_url_env_var="OPENCODE_ZEN_BASE_URL"),
    "opencode-go": MarcelOverlay(is_aggregator=True, base_url_env_var="OPENCODE_GO_BASE_URL"),
    "opencode-free": MarcelOverlay(is_aggregator=True, base_url_override="https://opencode.ai/zen/v1", keyless=True),
    "kilo": MarcelOverlay(is_aggregator=True, base_url_env_var="KILOCODE_BASE_URL"),
    "huggingface": MarcelOverlay(is_aggregator=True, base_url_env_var="HF_BASE_URL"),
    "novita": MarcelOverlay(is_aggregator=True, base_url_env_var="NOVITA_BASE_URL"),
    "xai": MarcelOverlay(transport="codex_responses", base_url_override="https://api.x.ai/v1", base_url_env_var="XAI_BASE_URL"),
    "nvidia": MarcelOverlay(base_url_override="https://integrate.api.nvidia.com/v1", base_url_env_var="NVIDIA_BASE_URL"),
    "xiaomi": MarcelOverlay(base_url_env_var="XIAOMI_BASE_URL"),
    "tencent-tokenhub": MarcelOverlay(base_url_env_var="TOKENHUB_BASE_URL"),
    "tencent-tokenplan": MarcelOverlay(transport="anthropic_messages",
                                       base_url_override="https://api.lkeap.cloud.tencent.com/plan/anthropic",
                                       base_url_env_var="TOKENPLAN_BASE_URL"),
    "arcee": MarcelOverlay(base_url_override="https://api.arcee.ai/api/v1", base_url_env_var="ARCEE_BASE_URL"),
    "gmi": MarcelOverlay(extra_env_vars=("GMI_API_KEY",), base_url_override="https://api.gmi-serving.com/v1",
                         base_url_env_var="GMI_BASE_URL"),
    "fireworks": MarcelOverlay(extra_env_vars=("FIREWORKS_API_KEY",),
                               base_url_override="https://api.fireworks.ai/inference/v1"),
    "actual": MarcelOverlay(transport="codex_responses", extra_env_vars=("ACTUAL_API_KEY", "ACTUAL_BASE_URL"),
                            base_url_override="https://api.actual.inc/v1", base_url_env_var="ACTUAL_BASE_URL"),
    "upstage": MarcelOverlay(extra_env_vars=("UPSTAGE_API_KEY",), base_url_override="https://api.upstage.ai/v1",
                             base_url_env_var="UPSTAGE_BASE_URL"),
    "nebius-token-factory": MarcelOverlay(extra_env_vars=("NEBIUS_API_KEY", "NEBIUS_TOKEN_FACTORY_API_KEY"),
                                          base_url_override="https://api.tokenfactory.nebius.com/v1",
                                          base_url_env_var="NEBIUS_BASE_URL"),
    "ollama-cloud": MarcelOverlay(base_url_override="https://ollama.com/v1", base_url_env_var="OLLAMA_BASE_URL"),
    # Azure Foundry serves OpenAI- and Anthropic-style endpoints; transport comes from model.api_mode.
    "azure-foundry": MarcelOverlay(base_url_env_var="AZURE_FOUNDRY_BASE_URL"),
    "bedrock": MarcelOverlay(transport="bedrock_converse", auth_type="aws_sdk"),
    ...
    "vertex": MarcelOverlay(auth_type="vertex"),
}
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "marcel", "user-config"
...
def _overlay_pdef(canonical, ov: MarcelOverlay, name, env_vars, base_url, doc, source) -> ProviderDef:
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    """Look up a built-in provider by id or alias: models.dev catalog merged with the Marcel overlay;
    Marcel-only overlay (nous, openai-codex, …); plugin provider profiles with a concrete endpoint."""
    canonical = normalize_provider(name)
    mdev_info = _models_dev_info(canonical, allow_network)
    overlay = MARCEL_OVERLAYS.get(canonical)
    if mdev_info is not None:
        ov = overlay or MarcelOverlay()
    ...
    if overlay is not None:
        return _overlay_pdef(canonical, overlay, _LABEL_OVERRIDES.get(canonical, canonical), overlay.extra_env_vars,
                             overlay.base_url_override, "", "marcel")
    # Plugin-registered profiles (plugins/model-providers/<name>/) absent from models.dev and
    # MARCEL_OVERLAYS would otherwise be "Unknown provider" in /model, --provider and model-switch
...
def _lossy_alias_registry_pdef(raw: str, canonical: str) -> Optional[ProviderDef]:
    """Exact Marcel registry ids win over LOSSY alias collapsing (kimi-coding-cn must stay distinct
    ...
    try:
        from marcel_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
...
def _llamacpp_pdef() -> Optional[ProviderDef]:
    ...
    try:
        from marcel_cli.local_runtime.endpoint import resolve_llamacpp_endpoint
