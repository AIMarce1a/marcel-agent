...
def build_logs_parser(subparsers, *, cmd_logs: Callable) -> None:
    ...
    logs_parser = subparsers.add_parser(
        "logs", help="View and filter Marcel log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log / gui.log / desktop.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    marcel logs                    Show last 50 lines of agent.log
    marcel logs -f                 Follow agent.log in real time
    marcel logs errors             Show last 50 lines of errors.log
    marcel logs gateway -n 100     Show last 100 lines of gateway.log
    marcel logs gui -f             Follow gui.log in real time
    marcel logs desktop -f         Follow desktop.log (Electron app boot/backend)
    marcel logs --level WARNING    Only show WARNING and above
    marcel logs --session abc123   Filter by session ID
    marcel logs --component tools  Only show tool-related lines
    marcel logs --since 1h         Lines from the last hour
    marcel logs --since 30m -f     Follow, starting from 30 min ago
    marcel logs list               List available log files with sizes
""")
