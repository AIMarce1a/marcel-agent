"""``marcel debug`` subcommand parser."""
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Marcel Agent. Use 'marcel debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    marcel debug share              Upload debug report (asks for confirmation)
    marcel debug share --yes        Skip confirmation (for scripts/CI)
    marcel debug share --lines 500  Include more log lines
    marcel debug share --expire 30  Keep paste for 30 days
    marcel debug share --local      Print report locally (no upload)
    marcel debug share --no-redact  Disable upload-time secret redaction
    marcel debug share --nous       Upload to Nous-internal storage (private)
    marcel debug delete <url>       Delete a previously uploaded paste
""")
    ...
    delete_parser = debug_sub.add_parser(
        "delete", help="Delete a paste uploaded by 'marcel debug share'")
