...
def _strip_marcel_path_lines(content: str) -> str:
    """Drop the ``# Marcel Agent`` marker (+ its PATH line) and any marcel PATH line; squash blank runs."""
    new_lines = []
    skip_next = False
    for line in content.split('\n'):
        if '# Marcel Agent' in line or '# marcel-agent' in line:
            skip_next = True
            continue
        if skip_next and ('marcel' in line.lower() and 'PATH' in line):
            skip_next = False
            continue
        skip_next = False
        if 'marcel' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
            continue
...
def remove_path_from_shell_configs():
    """Remove Marcel PATH entries from shell configuration files."""
    removed_from = []
    for config_path in (c for c in (Path.home() / n for n in _SHELL_RC_NAMES) if c.exists()):
        try:
            content = config_path.read_text(encoding="utf-8")
            new_content = _strip_marcel_path_lines(content)
...
def remove_wrapper_script():
    """Remove the marcel wrapper script if it exists."""
    def _unlink_ours(wrapper: Path) -> bool:
        # Only our wrapper (contains a marcel_cli / marcel-agent reference).
        content = wrapper.read_text(encoding="utf-8")
        return _unlink_if('marcel_cli' in content or 'marcel-agent' in content, wrapper)

    candidates = (
        bin_dir / name
        for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin"))
        for name in ("marcel", "marcel-acp", "marcel-agent"))
...
def remove_node_symlinks(marcel_home: Path) -> list:
    ...
    node_dir = (marcel_home / "node").resolve()
...
def uninstall_gateway_service():
    ...
    try:
        from marcel_cli.gateway import kill_gateway_processes, find_gateway_pids
...
def _remove_systemd_gateway() -> bool:
    ...
    from marcel_cli.gateway import _systemctl_cmd, get_service_name, get_systemd_unit_path
...
def _remove_launchd_gateway() -> bool:
    ...
    from marcel_cli.gateway import get_launchd_plist_path
...
def _remove_windows_gateway() -> bool:
    ...
    from marcel_cli import gateway_windows as gw
...
# MARCEL_HOME / MARCEL_GIT_BASH_PATH (HKCU\Environment), User-scope PATH entries
# (%LOCALAPPDATA%\marcel\git\{cmd,bin,usr\bin}, ...\marcel\node), PortableGit + Node copies
...
def _marcel_path_markers(marcel_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Prefixes identifying Marcel-owned User-PATH entries (prefix match sweeps git\cmd, git\bin,
    ...
    root = str(marcel_home).rstrip("\\/")
    subs = ("marcel-agent", "git", "node", "venv") + (("bin",) if include_managed_bin else ())
    return [f"{root}\\{sub}" for sub in subs]


def remove_path_from_windows_registry(marcel_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Marcel-owned entries from User-scope PATH in the registry (see ``_marcel_path_markers``)."""
    markers = tuple(m.lower() for m in _marcel_path_markers(marcel_home, include_managed_bin=include_managed_bin))
...
def remove_marcel_env_vars_windows() -> list[str]:
    """Delete MARCEL_HOME and MARCEL_GIT_BASH_PATH from User-scope env vars."""
    def edit(winreg, key, removed):
        for name in ("MARCEL_HOME", "MARCEL_GIT_BASH_PATH"):
            try:
...
def remove_portable_tooling_windows(marcel_home: Path) -> list[Path]:
    """Delete the PortableGit / Node / gateway-service dirs the Windows installer created under
    ``marcel_home`` (isolated from any system Git / Node, so nothing else breaks)."""
    targets = (marcel_home / sub for sub in ("git", "node", "gateway-service"))
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``marcel`` launchers install.ps1 staged in the managed ``bin`` dir: every mode
    ...
    try:
        ...
        from marcel_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from marcel_constants import get_default_marcel_root
        bin_dir = get_default_marcel_root() / "bin"
    except Exception as e:
...
def _is_default_marcel_home(marcel_home: Path) -> bool:
    """Return True when ``marcel_home`` points at the default (non-profile) root."""
    try:
        from marcel_constants import get_default_marcel_root
        return marcel_home.resolve() == get_default_marcel_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    out to ``marcel -p <name> gateway stop|uninstall`` because service names / unit paths derive
    from the current MARCEL_HOME and can't be switched in-process."""
    name = profile.name
    log_info(f"Uninstalling profile '{name}'...")

    # 1. Gateway service, via `python -m marcel_cli.main` (the `marcel` wrapper may be half-gone).
    marcel_invocation = [sys.executable, "-m", "marcel_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                marcel_invocation + ["gateway", subcmd], capture_output=True, text=True,
    ...
    # 3. Wipe the profile's MARCEL_HOME directory.
    _rmtree_step(profile.path, indent="  ", fully=False)


def run_gui_uninstall(args):
    """``marcel uninstall --gui``: remove the desktop app's built artifacts, packaged bundle
    (best-effort) and Electron userData — never config/sessions/.env, the agent or its venv."""
    from marcel_cli.gui_uninstall import agent_is_installed, gui_install_summary, uninstall_gui
    marcel_home = get_marcel_home()
    summary = gui_install_summary(marcel_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    _print_box("│         ⚕ Marcel Chat GUI Uninstaller                  │", Colors.MAGENTA)
    print()

    if not summary["gui_installed"]:
        print("No Marcel Chat GUI installation was found.")
        print(f"  Checked: {marcel_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Marcel agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(marcel_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Marcel agent at {marcel_home / 'marcel-agent'}")
        print(f"  • Your config, sessions, and secrets under {marcel_home}")
        print()
    ...
    print()
    uninstall_gui(marcel_home)

    print()
    _print_box("│            ✓ Chat GUI Uninstalled!                      │", Colors.GREEN)
    print()
    print("The Marcel agent is still installed. Run 'marcel' to use the CLI,")
    print("or 'marcel uninstall' to remove the agent too.")
    print()


def run_uninstall(args):
    """Interactive/``--yes`` uninstall: full removes code and ``~/.marcel/``; keep-data removes only
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            project_root=project_root, marcel_home=marcel_home, full_uninstall=full_flag)
        return

    # Named profiles (only when uninstalling from the default root) are offered for cleanup too,
    # instead of leaving zombie MARCEL_HOMEs and systemd units behind.
    is_default_profile = _is_default_marcel_home(marcel_home)
    ...
    print()
    _print_box("│            ⚕ Marcel Agent Uninstaller                  │", Colors.MAGENTA)
    print()

    # Show what will be affected
    print(color("Current Installation:", Colors.CYAN, Colors.BOLD))
    print(f"  Code:    {project_root}")
    print(f"  Config:  {marcel_home / 'config.yaml'}")
    print(f"  Secrets: {marcel_home / '.env'}")
    print(f"  Data:    {marcel_home / 'cron/'}, {marcel_home / 'sessions/'}, {marcel_home / 'logs/'}")
    print()
    ...
    # services, alias wrappers, MARCEL_HOME dirs) — otherwise they leave zombie services behind.
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Marcel data!", Colors.RED, Colors.BOLD))
        print(color("   Including: configs, API keys, sessions, scheduled jobs, logs", Colors.RED))
        if remove_profiles:
            print(color(f"   Plus {n_profiles} profile(s): {profile_names}", Colors.RED))
    else:
        print("This will remove the Marcel code but keep your configuration and data.")
    ...
    _perform_uninstall(
        project_root=project_root, marcel_home=marcel_home, full_uninstall=full_uninstall,
        remove_profiles=remove_profiles, named_profiles=named_profiles)


def _print_uninstall_dry_run(*, project_root: Path, marcel_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Marcel PATH entries from shell configs / Windows User PATH")
    print("  • Marcel wrapper scripts and Marcel-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if not full_uninstall:
        print(f"  • Keep Marcel config/data: {marcel_home}")
    else:
        print(f"  • Marcel config/data: {marcel_home}")
        profiles = _discover_named_profiles() if _is_default_marcel_home(marcel_home) else []
        if profiles:
...
def _perform_uninstall(
    *,
    ...
    the checkout -> (Windows) PortableGit/Node -> optionally ``$MARCEL_HOME`` and named profiles."""
    print()
    ...
    # 2-3b. PATH entries, wrapper, Windows launchers, node symlinks. Windows: marcel_home is
    #    %VAR%-expanded because install.ps1 writes literal C:\Users\<u>\...; marcel\bin (launchers +
    ...
    windows = _is_windows()
    sweep_managed_bin = windows and full_uninstall and _is_default_marcel_home(marcel_home)
    for on_this_platform, label, remove, success_fmt, none_msg in (
        ...
        (windows, "Removing PATH entries from Windows User environment...",
         lambda: remove_path_from_windows_registry(
             Path(os.path.expandvars(str(marcel_home))), include_managed_bin=sweep_managed_bin),
         "Removed from User PATH: {}", "No Marcel-owned PATH entries in User environment"),
        (windows, "Removing MARCEL_HOME / MARCEL_GIT_BASH_PATH User env vars...",
         remove_marcel_env_vars_windows, "Removed User env var: {}", "No Marcel-set User env vars to remove"),
        (True, "Removing marcel command...", remove_wrapper_script, "Removed {}", "No wrapper script found"),
        (windows, "Removing Windows marcel launchers...",
         remove_windows_bin_launchers, "Removed {}", "No Windows marcel launchers found"),
        (True, "Removing Marcel-managed node/npm/npx symlinks...",
         lambda: remove_node_symlinks(marcel_home), "Removed {}", "No Marcel-managed node/npm/npx symlinks found"),
    ):
        if on_this_platform:
    ...
    #     .env (safe in keep-data mode); the packaged app + Electron userData live OUTSIDE MARCEL_HOME.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from marcel_cli.gui_uninstall import uninstall_gui
        if not uninstall_gui(marcel_home):
    ...
    if windows:
        _remove_step(
            ...
            lambda: remove_portable_tooling_windows(marcel_home), "Removed {}",
            "No Windows installer artifacts to remove")

    # 5. Optionally remove ~/.marcel/ data directory (and named profiles)
    if full_uninstall:
        ...
        _rmtree_step(marcel_home)
    else:
        log_info(f"Keeping configuration and data in {marcel_home}")
    ...
    print()
    print("Thank you for using Marcel Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """``python -m marcel_cli.uninstall --mode <gui|lite|full>``. Imports only stdlib +
    ``marcel_constants`` + ``marcel_cli.colors``, so it runs under a bare system Python (no venv)."""
    import argparse
    parser = argparse.ArgumentParser(prog="python -m marcel_cli.uninstall")
    parser.add_argument(
