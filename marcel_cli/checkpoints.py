"""`marcel checkpoints` CLI subcommand.
...
from marcel_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: marcel checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``marcel checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `marcel checkpoints` → status
...
_PLUGIN_COMPAT_LAZY = {
    'cmd_list': ('marcel_cli.plugins_cmd', 'cmd_list'),
}
