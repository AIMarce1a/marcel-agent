"""Default configuration data for Marcel Agent: DEFAULT_CONFIG and OPTIONAL_ENV_VARS.

Pure-data leaf module — must not import from marcel_cli.config. Comments are the user-facing
docs of config.yaml.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["marcel-cli"],
    # journal_mode: SQLite journal mode for every Marcel DB. "wal" default; use "delete" on
    ...
    "session": {
        # Per-terminal `marcel -c`: each CLI session writes a breadcrumb under
        # $MARCEL_HOME/terminal-sessions/<terminal-id>, so bare -c/--continue resumes THIS
    ...
    "agent": {
        ...
        # Default 30 min is a safety valve for wedged agents, not a target latency — an interactive `marcel
        ...
        # Marcel-level retry attempts for API errors (connection drops, timeouts, 5xx) wrapping the
        ...
        # wrapping Marcel (sandbox runner, managed platform) can describe proxy/credential/ mount
        # layout without editing SOUL.md. Env MARCEL_ENVIRONMENT_HINT overrides it.
        "environment_hint": "",
        ...
        # Marcel is final-answer-first by default. Users can opt into heartbeat messages explicitly.
        ...
        # share or `marcel doctor` (status channel only; prompt cache untouched). 0 = no escalation
        ...
        # gateway status (`marcel status` / fleet monitoring). Retries never stop — a signal, not a
        ...
        # MARCEL_LOCAL_STREAM_STALE_TIMEOUT overrides.
    ...
    "terminal": {
        ...
        # sandboxes). Empty = TMPDIR/TMP/TEMP if set, else MARCEL_HOME/cache/terminal (auto-pruned
        ...
        # MARCEL_HOME/home; "real" = force real HOME; "profile" = force MARCEL_HOME/home when it
        ...
        # ["/home/user/.marcel/cache/documents:/output"]. For gateway MEDIA delivery, write to
        ...
        # default for images whose entrypoints must start as root (e.g. the bundled Marcel image,
        # which drops to `marcel` via s6-setuidgid). When on, SETUID/SETGID caps are omitted.
    ...
    "web": {
        ...
        # parallel, firecrawl, keenable; tavily is opt-in keyless via `marcel tools`, not a ring
        # member). Set by the `marcel tools` picker. "free" = always anonymous endpoint even with a
    ...
    "browser": {
        ...
        # Consent to browse with the user's REAL logins locally: runs on a Marcel-managed SNAPSHOT
        # of the ACTIVE default-Chromium profile (Local State -> profile.last_used; cookies, logins,
        # prefs copied and re-synced per fresh session) driven by Marcel' packaged Chromium. The
        # snapshot dir sidesteps Chrome 136+'s default-profile debugging block and never contends
        # with the running browser. Turning off deletes ~/.marcel/browser-profile/ so credentials
        ...
        # runs `marcel browser close-profile` (kills that profile's browser tree, unsaved tabs lost)
    ...
    # write_file/patch call); restore with /rollback. Opt-in via `marcel chat --checkpoints` or
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ~/.marcel/checkpoints/ size (MB); the oldest checkpoint per project
        ...
        # must not guess. Orphans: `marcel checkpoints prune` (`--keep-orphans` to skip).
        "auto_prune": True,
        "retention_days": 7,
        "min_interval_hours": 24,
    },
    # Hard cap (chars) for one auto-loaded context file (SOUL.md, AGENTS.md, CLAUDE.md, .marcel.md,
    ...
    # Same bound for single-query mode (``marcel -q/-z``). With only ONE turn there is no
    ...
    "compression": {
        ...
        # Marcel' summarizer cannot shrink it. native = codex decides; marcel = Marcel' threshold
    ...
    # Marcel distribution settings. The router speaks an OpenAI-compatible wire protocol; secrets
    # are resolved from key_env and are never persisted in config.yaml.
    "marcel": {
        "brand": {
            "name": "Marcel",
            "mascot": "monkey",
    ...
    # Account registry metadata only. OAuth tokens, client secrets, and mailbox passwords must be
    # stored in the secret backend and referenced by environment-variable name.
    "accounts": {
        "profiles": [],
    },
    ...
    "auxiliary": {
        ...
        # of skills); route cheaper via `marcel model` → auxiliary → Curator.
        "curator": _aux(600),
    ...
    "display": {
        "compact": False,
        "personality": "",
        # Marcel defaults to a clean customer-facing chat. Per-platform overrides can opt back in.
        "tool_progress": "off",
        "resume_display": "full",
        ...
        # New messages inform/adjust the active run without cancelling it.
        ...
        # Interface bare `marcel`/`marcel chat` launches: "cli" (prompt_toolkit REPL) | "tui" (Ink).
        # Flags win: `--cli` forces the REPL, `--tui` / MARCEL_TUI=1 forces the TUI.
        "interface": "cli",
        # `marcel --tui` auto-resumes the most recent human-facing session (like `marcel -c`).
        # MARCEL_TUI_RESUME=<id> always wins.
        ...
        # `path`/`paths` = MARCEL_HOME-relative YAML files/dirs (or conventional status_phrases.yaml
        ...
        # Gateway runtime footer on the FINAL message, e.g. `model · 68% · ~/projects/marcel`.
        ...
        # CLI/TUI/desktop, managed with `marcel pets`. No effect on prompt caching.
        "pet": {
            "enabled": False,
            "slug": "",   # active pet slug in get_marcel_home()/pets/; empty → first installed
    ...
    "dashboard": {
        ...
        # client disconnects. 0 = park forever. Env: MARCEL_TUI_WS_ORPHAN_REAP_GRACE_S.
        ...
        # newest message older than MARCEL_TUI_SESSION_TTL_S, default 6h) with
        ...
        # plugin. Env MARCEL_DASHBOARD_OAUTH_CLIENT_ID / MARCEL_DASHBOARD_PORTAL_URL win when
        ...
        # no-op. Env MARCEL_DASHBOARD_BASIC_AUTH_USERNAME / _PASSWORD_HASH / _PASSWORD / _SECRET /
        ...
        # MARCEL_DASHBOARD_DRAIN_SECRET; no-op unless >=256-bit, weak secrets rejected
        # (fail-closed). scope = capability label; min_secret_chars in url-safe-b64 chars.
        "drain_auth": {"scope": "drain", "min_secret_chars": 43},
        # Public URL (env MARCEL_DASHBOARD_PUBLIC_URL): full authority (scheme + host + optional
        # prefix, e.g. https://example.com/marcel) for the OAuth redirect_uri; its hostname is
    ...
    "tts": {
        ...
        "elevenlabs": {
            "voice_id": "pNInz6obpgDQGcFmaJgB",  # ElevenLabs default voice
        ...
        "piper": {
            ...
            # (~/.marcel/cache/piper-voices/), use_cuda, length_scale (2.0 = twice as slow),
    ...
    # "Hey Marcel" hands-free wake word: always-on, on-device hotword detection that starts a fresh
    ...
    "wake_word": {
        ...
        "openwakeword": {
            # "hey_marcel" | built-in openWakeWord name ("hey_jarvis", "alexa", ...) | path to a
    ...
    # plugins/context_engine/<name>/ or ~/.marcel/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "memory": {  # Persistent memory — bounded curated memory injected into the system prompt
        ...
        "provider": "",
        # Marcel schedules a durable memory-curation job. The wizard can change the interval or
        # disable it; this is intentionally distinct from nudge_interval (which counts turns).
        "maintenance": {
            "enabled": True,
            "interval_hours": 6,
            "timezone": "UTC",
    ...
    "delegation": {
        ...
        # ~/.marcel/cache/delegation/ with a head+tail window + read_file offset footer, nothing
        ...
        # Marcel's declarative worker registry. Registry size is intentionally unbounded; runtime
        # concurrency remains controlled globally and per worker.
        "workers": {},
        "worker_routing": {
            "strategy": "cheapest_capable",
            "default_worker": "",
    ...
    "moa": {
        ...
        # <marcel_home>/moa-traces/<session_id>.jsonl (or trace_dir) for auditing.
        "save_traces": False,
        "trace_dir": "",
    ...
    # and resolved; read-only — creation goes to ~/.marcel/skills/ unless create_dir redirects it.
    "skills": {
        ...
        # MARCEL_HOME, scanned alongside the local dir.
        "create_dir": "",
        # In a git checkout, <root>/.marcel/skills/ and <root>/.agents/skills/ load as the
        # highest-precedence tier — ONLY if the root is in trusted_project_dirs. false = no scan, no
        # untrusted-skills notice.
        "project_discovery": True,
        # Trusted project roots; managed by `marcel skills trust` / `untrust`.
        "trusted_project_dirs": [],
        # Substitute ${MARCEL_SKILL_DIR} / ${MARCEL_SESSION_ID} in SKILL.md content.
        "template_vars": True,
        ...
        # Advisory NVIDIA SkillEvaluator Tier 1 scan on `marcel skills install` (alongside the
        ...
        "write_approval": False,
        # Audit ledger: every skill mutation appends to ~/.marcel/skills/.curator_ledger.jsonl with
        # before/after hashes (blobs under ~/.marcel/.curator_backups/blobs/); powers `marcel
    ...
    # daemon. `marcel curator status` shows the last run.
    "curator": {
        "enabled": True,
        ...
        # aux-model cost. `marcel curator run --consolidate` overrides once.
        "consolidate": False,
        # Also prune bundled built-ins (a suppression list stops `marcel update` restoring them);
        ...
        "prune_builtins": True,
        # TTL purge of skills/.archive/: 0 = never; > 0 lets the explicit `marcel curator purge`
        # delete older archived skills (never automatic; logged in the ledger).
        "archive_ttl_days": 0,
        # Before every real (non-dry-run) pass, snapshot ~/.marcel/skills/ to
        # ~/.marcel/skills/.curator_backups/<utc-iso>/skills.tar.gz (`marcel curator rollback`).
        "backup": {
            "enabled": True,
    ...
    # peerName, sessions, enabled); marcel-specific overrides only here.
    "honcho": {},
    ...
    "whatsapp": {
        # No product header by default; an operator can configure reply_prefix explicitly.
        "reply_prefix": "",
    },
    ...
    #   cron job, a -q session (MARCEL_INTERACTIVE=1 but nobody to answer), or an unattended
    ...
    "approvals": {
        ...
        # export MARCEL_INTERACTIVE=1 (for interactive sudo prompts) but have NO user waiting to answer
        ...
        # Marcel runs with full tool access by default. Users can opt back into manual/smart gates.
        "mode": "off",
        "timeout": 300,
        "cron_mode": "approve",
        ...
        # Approve" → false. MARCEL_TUI_NO_CONFIRM=1 skips the TUI modal.
    ...
    # are given.
    "platform_hints": {},
    # Plugin system. `enabled`/`disabled` lists are written by `marcel plugins enable|disable` and
    ...
    "plugins": {
        ...
        # 2026-09-14 removal date (see COMPAT_MANIFEST.md, `marcel plugins compat`). Stopgap only: the
    ...
    # approvals persist in ~/.marcel/shell-hooks-allowlist.json. Schema + examples:
    # website/docs/user-guide/features/hooks.md.
    "hooks": {},
    # Auto-accept shell-hook registrations without a TTY prompt (also --accept-hooks or
    # MARCEL_ACCEPT_HOOKS=1). Gateway/cron/non-interactive runs need one of these to pick up
    # newly-added hooks.
    ...
    "security": {  # Security: pre-exec scanning via tirith plus related guards.
        ...
        # .marcel config) always need human approval, even under yolo. Extra patterns are fnmatch
        ...
        # banner. Add via `marcel doctor --ack <id>`; remove by editing the list. Catalog:
        # marcel_cli/security_advisories.py.
        "acked_advisories": [],
    ...
    "cron": {
        ...
        # an installed provider (plugins/cron_providers/<name>/ or $MARCEL_HOME/plugins/ <name>/),
        ...
        # Env override: MARCEL_CRON_MAX_PARALLEL.
        ...
        "output_retention": 50,
        # Timeout (seconds) for a no-agent cron script. Env: MARCEL_CRON_SCRIPT_TIMEOUT. Keep in
        ...
        # job's dispatch guard forever. Env: MARCEL_CRON_SESSION_DB_TIMEOUT. 0 = unlimited.
        "session_db_timeout_seconds": 10,
        # Timeout (seconds) per media attachment send during gateway delivery; large attachments
        # (long TTS audio, big exports) need more than 30s. Env: MARCEL_CRON_MEDIA_SEND_TIMEOUT.
    ...
    # promotes dependency-satisfied todos to ready, and fires `marcel -p <assignee> chat -q ...` per
    ...
    "kanban": {
        ...
        # Auto-run the decomposer on Triage tasks every tick. False = manual via `marcel kanban
    ...
    "code_execution": {  # execute_code settings (programmatic tool calls).
        ...
        # relative paths resolve. strict = isolated temp dir with marcel-agent's own python
    ...
    # by tool_search / tool_describe / tool_call bridges and surfaced on demand. Core Marcel tools
    ...
    "logging": {  # File logging to ~/.marcel/logs/: agent.log captures INFO+, errors.log WARNING+.
    ...
    "model_catalog": {
        ...
        # CLI refetches on the next /model or `marcel model` once the cache is older. Network
    ...
    # defaults (200K context, tools on, vision/reasoning off) and get patched. Provider keys: Marcel
    ...
    "monitoring": {
        ...
        "gateway_health_export": {  # Gateway health & diagnostics export.
            ...
            "resource_attributes": {
                "service.name": "marcel-gateway", "deployment.environment.name": "production"
            },
        },
    ...
    "gateway": {  # Gateway settings (messaging platforms: Telegram, Discord, Slack, ...).
        ...
        # to MARCEL_GATEWAY_PLATFORM_CONNECT_TIMEOUT, which wins if set explicitly.
        ...
        # run_gateway() bridges these to MARCEL_STARTUP_WATCHDOG / MARCEL_STARTUP_WATCHDOG_TIMEOUT_S
        # and re-arms the live handle; explicit env wins.
        "startup_watchdog": True,
        "startup_watchdog_timeout_seconds": 300,
        # Keep writing the legacy ~/.marcel/sessions/sessions.json mirror of the routing index
        ...
        # (MARCEL_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent AND a wakeUrl is
        ...
        # MARCEL_GATEWAY_MAX_STARTS / MARCEL_GATEWAY_START_WINDOW_S.
        ...
        # credential/system denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.marcel/.env, auth.json). True:
        # files must be under the Marcel cache, media_delivery_allow_dirs, or fresher than
        # trust_recent_files_seconds — recommended for public-facing gateways so prompt injection
        # can't exfiltrate host secrets. Bridged to MARCEL_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra roots (project/scratch dirs, mounted shares) from which bare file paths may be
        # uploaded; the Marcel cache is always trusted. List of absolute paths or one
        # os.pathsep-separated string; tildes expanded. Bridged to MARCEL_MEDIA_ALLOW_DIRS. Honored
        # in both modes.
        ...
        # pure-allowlist mode. Bridged to MARCEL_MEDIA_TRUST_RECENT_FILES. Only consulted when
        # strict is true.
        "trust_recent_files": True,
        # Recency window in seconds; 600 covers a multi-tool turn. Bridged to
        # MARCEL_MEDIA_TRUST_RECENT_SECONDS. Only consulted when strict is true.
    ...
    # Automatic cleanup of ~/.marcel/state.db, which otherwise grows without bound and slows FTS5
    ...
    "sessions": {
        ...
        "auto_prune": True,
        # Inactive days of ended-session history to keep (= `marcel sessions prune`).
        ...
        # processes).
        "min_interval_hours": 24,
        # Legacy ~/.marcel/sessions/session_{sid}.json snapshots rewritten every turn. state.db is
        ...
        # stay until `marcel sessions optimize-storage` runs, since the rebuild is disk-heavy on
        # large DBs. advise = `marcel update` prints a one-line notice with reclaimable size when a
        ...
        # (native/fts5_cjk/build.sh → ~/.marcel/lib/libfts5_cjk.so), 1-2 char CJK terms get exact
        # index matches instead of LIKE scans. True = use when present (inert otherwise); False =
        # never load/serve it. Bridged to MARCEL_CJK_FTS.
        "cjk_fts": True,
        ...
        # Max active messages per session for in-memory export (`marcel sessions export`); checked
    ...
    "doctor": {
        # Per-probe timeout (seconds) for `marcel doctor --live` real-call probes.
        "live_probe_timeout": 10,
    },

    "updates": {
        # Pre-update backup. quick = snapshot small critical state (pairing JSONs, cron jobs,
        # config.yaml, .env, auth.json, profile DBs) into <MARCEL_HOME>/state-snapshots/, skipping
        # files >1 GiB; restore via ``/snapshot``. full = quick PLUS a ``marcel backup`` zip in
        # <MARCEL_HOME>/backups/ (``marcel import`` restores; slow on large homes; ``--backup``
        ...
        # update cleanly. `marcel update --switch-branch` overrides to switch for one run.
        "parked_branch_strategy": "switch",
        # Refresh an installed cua-driver during `marcel update` (best-effort, macOS only). Turn off
    ...
    "lsp": {
        ...
        "wait_mode": "document",
        "wait_timeout": 5.0,
        # Missing server binaries: auto = install via npm/go/pip into <MARCEL_HOME>/lsp/bin/ on
    ...
    # (SuperGrok OAuth or XAI_API_KEY) AND the toolset is enabled in `marcel tools`.
    "x_search": {
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # ~/.marcel/.env (or the shell), never in config.yaml.
            ...
            # Last-good fallback for NETWORK/TIMEOUT outages: AES-GCM cache under ~/.marcel/cache/,
            ...
            # Auto-download bws into ~/.marcel/bin/ on first use; False = bws must be on PATH.
            "auto_install": True,
    ...
    "computer_use": {
        # cua-driver's upstream PostHog telemetry defaults ON; Marcel sets
        ...
        # #47032; Linux/WSL2 idle spin #28152). cua-driver ≥ 0.6.x supports --no-overlay; Marcel also calls
    ...
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned binary into ~/.marcel/bin/; False = iron-proxy on PATH.
        "auto_install": True,
    ...
    "desktop": {  # Marcel Desktop (Electron) launch options; only affect `marcel desktop`.
        ...
        # Bridged to MARCEL_DESKTOP_DISABLE_GPU: auto = disable GPU only on remote displays
        ...
        # to MARCEL_DESKTOP_PASSWORD_STORE; ignored off-Linux.
        "password_store": "auto",
    ...
    "local_runtime": {
        # Off = detection-only (Marcel still finds an external llama-server you run).
        "enabled": False,
        # Pinned llama.cpp release tag; bumped by Marcel releases after validation.
        "tag": "b10679",
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": _prov(
        ...
        "OAuth2, not a static API key — this points at the credentials Marcel mints short-lived "
    ...
    "AZURE_FOUNDRY_BASE_URL": _prov(
        "Azure Foundry base URL (set via 'marcel model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": _tool(
        ...
        "read from the Marcel auth store)", "Tool-gateway user token", None, advanced=True),
    ...
    "PORCUPINE_ACCESS_KEY": _tool(
        "Picovoice access key for the Porcupine 'Hey Marcel' wake word engine (optional; "
    ...
    "MARCEL_LANGFUSE_PUBLIC_KEY": _tool("Langfuse project public key (pk-lf-...)",
        "Langfuse public key", "https://cloud.langfuse.com", password=False),
    "MARCEL_LANGFUSE_SECRET_KEY": _tool("Langfuse project secret key (sk-lf-...)",
        "Langfuse secret key", "https://cloud.langfuse.com"),
    "MARCEL_LANGFUSE_BASE_URL": _tool("Langfuse server URL (default: https://cloud.langfuse.com)",
    ...
    "SLACK_ALLOWED_USERS": _msg(
        "Comma-separated Slack member IDs allowed to use Marcel, e.g. U01ABC2DEF3. Without "
    ...
    "MATRIX_USER_ID": _msg("Matrix user ID (e.g. @marcel:example.org)",
    ...
    "MATRIX_DEVICE_ID": _msg(
        "Stable Matrix device ID for E2EE persistence across restarts (e.g. MARCEL_BOT)",
    ...
    "IRC_CHANNEL": _msg("IRC channel to join (e.g. #marcel)", "IRC channel", None),
    "IRC_NICKNAME": _msg("Bot nickname on IRC (default: marcel-bot)", "IRC nickname", None),
    ...
    "API_SERVER_MODEL_NAME": _msg(
        "Model name advertised on /v1/models. Defaults to the profile name (or 'marcel-agent' "
    ...
    "GATEWAY_PROXY_URL": _msg(
        "URL of a remote Marcel API server to forward messages to (proxy mode). When set, the "
        ...
        "Remote Marcel API server URL (e.g. http://192.168.1.100:8642)", None, advanced=True),
    "GATEWAY_PROXY_KEY": _msg(
        "Bearer token for authenticating with the remote Marcel API server (proxy mode). Must "
    ...
    # MARCEL_TOOL_PROGRESS_MODE (deprecated; use display.tool_progress) is intentionally NOT listed:
    # this dict feeds user-facing surfaces (dashboard keys page, setup checklists), so deprecated
    # knobs stay in config._EXTRA_ENV_KEYS only. MARCEL_TOOL_PROGRESS is unsupported.
    "MARCEL_PREFILL_MESSAGES_FILE": _setting(
        "Path to JSON file with ephemeral prefill messages for few-shot priming",
        "Prefill messages file path", None),
    "MARCEL_EPHEMERAL_SYSTEM_PROMPT": _setting(
