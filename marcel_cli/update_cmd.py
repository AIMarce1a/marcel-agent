"""Marcel update pipeline: dispatchers (``_cmd_update_impl``/``_cmd_update_check``) + git plumbing.

Each concern lives in ``update_cmd_<concern>.py`` and is re-imported here so
``marcel_cli.update_cmd.<name>`` keeps resolving (and stays monkeypatchable). Imports are one-way:
main -> update_cmd -> update_cmd_*; ``_m()`` resolves ``marcel_cli.main`` at call time.
"""
...
from marcel_cli.config import get_marcel_home  # noqa: F401  (re-exported; patched via update_cmd)
from marcel_cli.update_cmd_common import _best_effort
from marcel_constants import get_default_marcel_root, venv_python_path

# Re-exports: every split-module name stays reachable (and monkeypatchable) as update_cmd.<name>.
from marcel_cli.update_abort_recovery import (  # noqa: F401
...
from marcel_cli.update_cmd_windows import (  # noqa: F401
    ...
    _handoff_reapable_backend_pids, _marcel_holder_subcommand, _holder_value_flags,
...
from marcel_cli.update_cmd_fleet import (  # noqa: F401
...
from marcel_cli.update_cmd_zip import (  # noqa: F401
...
from marcel_cli.update_cmd_stash import (  # noqa: F401
...
from marcel_cli.update_cmd_config import (  # noqa: F401
...
from marcel_cli.update_cmd_deps import (  # noqa: F401
...
from marcel_cli.update_cmd_git import (  # noqa: F401
...
from marcel_cli.update_cmd_maint import (  # noqa: F401
    ...
    _print_verified_update_completion, _purge_stale_marcel_modules, _read_project_version,
...
def _m():
    """Lazy ``marcel_cli.main`` handle: keeps main-side test patches effective, import one-way."""
    from marcel_cli import main
    return main
...
_UPDATE_CRITICAL_FILES = (
    "marcel_cli/main.py", "marcel_cli/config.py", "marcel_cli/__init__.py",
    "marcel_cli/web_server.py", "cli.py", "run_agent.py", "model_tools.py", "toolsets.py",
    "marcel_constants.py")
...
def _record_update_step(step: str, ok: bool, detail: str = "") -> None:
    ...
    with suppress(Exception):
        from marcel_cli.update_receipt import record_step
...
def _validate_python_files_syntax(root, relpaths) -> tuple[bool, str | None, str | None]:
    ...
    import tempfile
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="marcel-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    from marcel_constants import get_marcel_home  # noqa: F811  (deliberate: constants variant)
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    See #87331.
    """
    print("✗ Cannot continue the update: live Marcel launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["marcel.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically Marcel")
    print("  Desktop, a gateway, or another marcel REPL — and mutating the venv")
    print("  now would strand it half-updated.")
    print("  The dependency install has been deferred: close the process(es)")
    print("  above, then run any `marcel` command to finish it automatically.")
...
def _invalidate_update_cache():
    ...
    default_home = get_default_marcel_root()
...
def _format_concurrent_instances_message(matches: list[tuple[int, str]], scripts_dir: Path) -> str:
    """Explanation + remediation hint for the Windows concurrent-marcel.exe gate."""
    shim = scripts_dir / "marcel.exe"
    lines = [
...
def _classify_concurrent_instance(pid: int) -> str:
    ...
    from marcel_cli._scan_venv_blockers import _is_pausable_gateway  # noqa: PLC0415
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """``marcel update --check``: fetch and report without installing. ``branch_explicit`` is
    ...
    from marcel_cli.update_contract import evaluate_update_admission, record_refusal_receipt
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            return
        from marcel_cli.banner import _github_compare_behind
...
def _repair_venv_on_current_checkout(
    *, assume_yes, gateway_mode, pre_update_snapshot_id, desktop_dir,
    ...
    _write_update_incomplete_marker()
    from marcel_cli.managed_uv import ensure_uv
    repair_uv = ensure_uv()
    ...
    if not healthy_after:
        ...
        print("  Close all Marcel windows/gateways and re-run: marcel update")
        return False
...
def _repair_current_checkout(
    *, assume_yes, gateway_mode, pre_update_snapshot_id, desktop_dir,
    ...
    from marcel_cli.managed_uv import ensure_uv, update_managed_uv
    runtime_repairs = []
    ...
    if handed_off_sync:
        print("→ Finishing the dependency install handed off by marcel.exe...")
    elif not healthy:
    ...
    if runtime_repaired is not None and not _m()._is_windows():
        ...
        print(
            "  Any running Marcel gateways, Desktop backends, or other "
...
def _reconcile_diverged_checkout(git_cmd, branch: str, pre_pull_sha) -> None:
    ...
    if not has_common_ancestor and pre_pull_sha:
        ...
        rescue_ref = (
            f"refs/marcel-update-backups/orphan-{branch}-"
...
def _rollback_if_pulled_syntax_error(git_cmd, pre_pull_sha) -> None:
    ...
    if pre_pull_sha:
        ...
        if rollback_result.returncode == 0:
            ...
            print("  Try ``marcel update`` again later once a fix lands.")
        else:
...
def _pull_updates(
    ...
    # critical-path file (PR #28452 incident: orphan merge-conflict markers in marcel_cli/config.py bricked
    # every user who ran ``marcel update`` for the 7 minutes between the bad commit and the fix landing).
...
class _UpdateOptions:
    """Resolved ``marcel update`` inputs (flags, config, pre-update snapshots)."""
...
def _begin_update_receipt_and_plan(args):
    ...
    pre-update plan (None if the probe failed); ``sys.exit(2)`` when a non-gateway marcel.exe
    holds the venv shim."""
    ...
    with _best_effort('Update receipt unavailable: %s'):
        ...
        from marcel_cli.update_receipt import begin_update_receipt
    ...
    # Plan phase (#91277 Phase 2): snapshot the pre-update fleet — every running Marcel runtime, its
    ...
    with _best_effort('Update plan phase failed: %s'):
        from marcel_cli.update_inventory import collect_runtime_inventory, record_plan_in_receipt
    ...
    # Windows: another marcel.exe holding the venv shim means WinError 32 spam and a
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        concurrent = _m()._detect_concurrent_marcel_instances(scripts_dir) if scripts_dir is not None else []
...
def _prepare_git_command() -> tuple[bool, list, bool]:
    ...
    if use_zip_update and sys.platform != "win32":
        ...
        print("  curl -fsSL https://raw.githubusercontent.com/AdMind-ai/marcel-agent/main/scripts/install.sh | bash")
        sys.exit(1)
...
def _verify_head_after_pull(
    ...
    # step re-detaches to the SHA). Before this guard, ``marcel update`` printed "✓ Code updated!" and
    # reinstalled deps + rebuilt the desktop app against the stale tree — no error, no warning, ``marcel
    ...
    if pre_pull_sha and post_pull_sha == pre_pull_sha:
        ...
        print(
            ...
            f"git -C {_m().PROJECT_ROOT} checkout {branch} && marcel update")
        _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
        sys.exit(1)
        ...
        if _called_process_error_is_python_dep_install(e):
            ...
            if _m()._is_windows():
                ...
                print(
                    ...
                    '"from marcel_cli.main import main; main()" update --yes')
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # Any venv python still running (typically the Desktop `marcel serve` backend) keeps .pyd
    ...
    # Self-lock deferral moved: the venv-holder sweep above excludes this process by design (a CLI `marcel
