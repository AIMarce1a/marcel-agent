"""Messaging-platform setup wizards (Telegram, BlueBubbles, webhooks) and the ``marcel setup
gateway`` flow. setup.py re-exports the public names, and tests monkeypatch prompt/print/env
helpers on marcel_cli.setup, so those are imported lazily per function."""
...
def _profile_name_from_marcel_home(marcel_home) -> str | None:
    """Return the active profile name when MARCEL_HOME is a profile dir."""
    return marcel_home.name if marcel_home.parent.name == "profiles" else None


def _setup_telegram_auto_result():
    """Attempt automatic Telegram bot creation via managed QR onboarding."""
    from marcel_cli.setup import get_marcel_home
    try:
        from marcel_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
    profile_name: str | None = None
    with contextlib.suppress(Exception):
        profile_name = _profile_name_from_marcel_home(Path(get_marcel_home()))
...
def _declines_reconfigure(env_var: str, label: str, question: str) -> bool:
    ...
    from marcel_cli.setup import get_env_value, print_info, prompt_yes_no
...
def _save_prompted(env_var: str, question: str, *, password: bool = False, success_msg: str | None = None,
    ...
    """
    from marcel_cli.setup import print_success, print_warning, prompt, save_env_value
...
def _save_allowlist(env_var: str, users: str, success_msg: str) -> None:
    ...
    from marcel_cli.setup import print_success, save_env_value
...
def _prompt_allowlist(env_var: str, question: str, success_msg: str, open_msg: str, preset: str | None = None) -> str:
    ...
    from marcel_cli.setup import print_info, prompt
...
def _save_port(env_var: str, value: str, default: str) -> None:
    ...
    from marcel_cli.setup import print_success, print_warning, save_env_value
    if not value:
        return
    try:
...
def _prompt_telegram_bot_token() -> str | None:
    from marcel_cli.setup import print_error, print_info, prompt
...
def _telegram_allowlist_nudge() -> None:
    ...
    from marcel_cli.setup import get_env_value, print_info, prompt, prompt_yes_no
...
def _configure_telegram_access(detected_owner_id: str = "") -> str:
    """Ask explicitly who may use Telegram and persist the selected policy."""
    from marcel_cli.setup import (
        print_info, print_success, print_warning, prompt, prompt_choice,
        remove_env_value, save_env_value,
    )
    choice = prompt_choice(
        "Who should be allowed to use this Telegram bot?",
        [
            "Only me — block every other Telegram account",
            "Approved users — new people request access with a pairing code",
            "Everyone — open access (may generate API costs)",
        ],
        0,
    )
    if choice == 2:
        remove_env_value("TELEGRAM_ALLOWED_USERS")
        save_env_value("TELEGRAM_ALLOW_ALL_USERS", "true")
        save_env_value("GATEWAY_ALLOW_ALL_USERS", "true")
        print_warning("Telegram open access enabled — anyone who finds the bot can use it.")
        return ""
    remove_env_value("TELEGRAM_ALLOW_ALL_USERS")
    remove_env_value("GATEWAY_ALLOW_ALL_USERS")
    if choice == 1:
        remove_env_value("TELEGRAM_ALLOWED_USERS")
        from marcel_cli.gateway import _set_platform_unauthorized_dm_behavior
        _set_platform_unauthorized_dm_behavior("telegram", "pair")
        print_success("Telegram pairing enabled for new users.")
        return ""
    owner_id = detected_owner_id or prompt("Your Telegram user ID")
    if not owner_id:
        print_info("No owner ID entered; Telegram will remain in pairing mode.")
        return ""
    _save_allowlist(
        "TELEGRAM_ALLOWED_USERS", owner_id,
        "Telegram restricted to the selected owner account",
    )
    from marcel_cli.gateway import _set_platform_unauthorized_dm_behavior
    _set_platform_unauthorized_dm_behavior("telegram", "ignore")
    return owner_id.replace(" ", "")


def _obtain_telegram_token():
    """Return (token, setup_result); auto flow first when chosen, else manual paste."""
    from marcel_cli.setup import _info, print_error, prompt_choice
    method = prompt_choice(
        "How would you like to connect Telegram?",
        [
            "Automatic (recommended) — scan a QR code and confirm in Telegram",
            "Manual — create a bot with @BotFather and paste its token",
        ],
        0,
    )
...
def _setup_telegram():
    ...
    from marcel_cli.setup import _info, print_info, print_header, print_success, prompt, prompt_yes_no, save_env_value
    ...
    allowed_users = _configure_telegram_access(detected_id)
    _info(None, "📬 Home Channel: where your agent delivers scheduled results,",
...
def _setup_bluebubbles():
    ...
    from marcel_cli.setup import _info, print_header, print_success, prompt, prompt_yes_no
    print_header("BlueBubbles (iMessage)")
    if _declines_reconfigure("BLUEBUBBLES_SERVER_URL", "BlueBubbles", "Reconfigure BlueBubbles?"):
        return
    _info("Connects Marcel to iMessage via BlueBubbles — a free, open-source",
...
def _setup_webhooks():
    ...
    from marcel_cli.setup import _info, print_header, print_success, print_warning, prompt, save_env_value
    ...
    _info(f"   1. Define webhook routes in {_dhh()}/config.yaml",
          ...
          "   Open config in your editor:  marcel config edit",
          "   Open config in your editor:  marcel config edit")
...
def _warn_missing_home_channels() -> None:
    ...
    from marcel_cli.setup import get_env_value, _info, print_warning
    missing_home = [
    ...
    _info("   Without a home channel, cron jobs and cross-platform",
          ...
          *(f"     marcel config set {plat.upper()}_HOME_CHANNEL <channel_id>" for plat in missing_home))
...
def _restart_running_gateway(any_messaging: bool, supports_systemd: bool) -> None:
    ...
    from marcel_cli.setup import print_error, prompt_yes_no
...
def setup_gateway(config: dict):
    ...
    from marcel_cli.setup import _info, print_header, print_info, print_success, prompt_checklist
    from marcel_cli.gateway import _all_platforms, _platform_status, _configure_platform
    print_header("Messaging Platforms")
    _info("Connect to messaging platforms to chat with Marcel from anywhere.",
    ...
    if not selected:
        print_info("No platforms selected. Run 'marcel setup gateway' later to configure.")
    ...
    # mode (cron keeps running; adapters come up once tokens are added via `marcel import` /
    # `marcel setup gateway`). Gating it on messaging config left install-then-import machines
    # with cron jobs and bot tokens but no process to serve them.
    from marcel_cli.gateway import _is_service_running, supports_systemd_services, ensure_gateway_service
