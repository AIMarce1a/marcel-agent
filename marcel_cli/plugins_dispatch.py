...
Mixed into :class:`marcel_cli.plugins.PluginManager`. ``_resolve_hook_callback_timeout`` stays on
...
from marcel_cli.middleware import OBSERVER_SCHEMA_VERSION
...
PLUGIN_SECTIONS_START = "<!-- marcel-plugin-sections:start -->"
PLUGIN_SECTIONS_END = "<!-- marcel-plugin-sections:end -->"
...
def format_system_prompt_section(section_id: str, content: str) -> str:
    ...
    return (
        ...
        f"<!-- marcel-plugin-section-chars:{len(content)} -->\n\n{content}")
...
# Reserved event namespace prefix — only core may publish ``marcel:<event>``.
MARCEL_EVENT_NAMESPACE = "marcel"
...
class PluginDispatchMixin:
    ...
    def invoke_hook(self, hook_name: str, **kwargs: Any) -> List[Any]:
        ...
        """
        from marcel_cli.plugins import _resolve_hook_callback_timeout
    ...
    def _run_hook_callback_bounded(
        ...
        thread = threading.Thread(target=_runner, name=f"marcel-hook-{callback_name}"[:40], daemon=True)
        thread.start()
    ...
    def _ensure_event_worker_locked(self) -> None:
        ...
        worker = threading.Thread(
            target=self._event_worker_loop, args=(self._event_queue,), name="marcel-plugin-events",
            daemon=True,
        )
    ...
    def _deliver_event(self, item: _QueuedPluginEvent) -> None:
        ...
        from marcel_cli.plugins import resolve_plugin_command_result
        with self._event_lock:
