...
from marcel_cli.auth import AuthError, resolve_provider
from marcel_cli.colors import Colors, color
from marcel_cli.config import get_env_path, get_env_value, get_marcel_home, load_config
from marcel_cli.models import provider_label
from marcel_cli.runtime_provider import resolve_requested_provider
from marcel_cli.vercel_auth import describe_vercel_auth
from marcel_cli.status_auth import (  # renderers wired into _SECTIONS below
    _render_api_keys, _render_apikey_providers, _render_auth_providers, _render_nous_gateway)
from marcel_constants import OPENROUTER_MODELS_URL
...
def _estop_status_line():
    """One-line pause banner for `marcel status`, or None when not paused."""
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{f' — reason: {reason}' if reason else ''}; `marcel resume` to lift)"
...
def _render_terminal(ctx):
    ...
    if terminal_env in _TERMINAL_ENV_ROWS:
        ...
        _kv_flag("SDK:", importlib.util.find_spec("vercel") is not None, "installed",
                 "missing (install: pip install 'marcel-agent[vercel]')")
        ...
        # (fail-soft — never break `marcel status`).
        try:
...
def _render_gateway(ctx):
    ...
    try:
        from marcel_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
...
def _render_cron(ctx):
    ...
    jobs_file = get_marcel_home() / "cron" / "jobs.json"
...
def _render_sessions(ctx):
    ...
    if gateway_rows:
        ...
        if freshest > 0:
            from marcel_cli.timefmt import relative_time
            print(f"  Last activity:{relative_time(freshest):>13}")
    elif not (sessions_file := get_marcel_home() / "sessions" / "sessions.json").exists():
        _kv("Active:", 0)
    else:
        try:
    ...
    try:
        from marcel_cli.active_sessions import (
...
def _render_footer(ctx):
    _banner(("─" * 60, "  Run 'marcel doctor' for detailed diagnostics", "  Run 'marcel setup' to configure"),
            Colors.DIM)
    print()


# Print order of `marcel status`; each renderer takes the shared _StatusContext.
_SECTIONS = (
...
def show_status(args):
    """Show status of all Marcel Agent components."""
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_nous_portal_account_info': ('marcel_cli.nous_account', 'get_nous_portal_account_info'),
