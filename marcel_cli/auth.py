"""Multi-provider authentication system for Marcel Agent.

- ``ProviderConfig`` / ``PROVIDER_REGISTRY`` describe every known inference provider.
- The auth store (``~/.marcel/auth.json``) holds per-provider state, the credential pool and
...
- ``OAUTH_PROVIDER_FLOWS`` maps each OAuth provider to its resolver/status builder; the flows live in
  ...
  re-imported here so ``marcel_cli.auth.<name>`` stays the public/patchable surface."""
...
from marcel_cli.config import (
    get_marcel_home, get_config_path, read_raw_config, require_readable_config_before_write)
from marcel_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
from marcel_cli.auth_zai_kimi import (  # noqa: F401  re-exported
...
from marcel_cli.auth_model_picker import (  # noqa: F401  re-exported
    _prompt_model_selection, _save_model_choice)
from marcel_cli.auth_device_flow import (  # noqa: F401  re-exported
...
from marcel_cli.auth_oauth_grants import (  # noqa: F401  re-exported
...
from marcel_cli.auth_nous import (  # noqa: F401  re-exported
...
from marcel_cli.auth_minimax import (  # noqa: F401  re-exported
...
from marcel_cli.auth_xai import (  # noqa: F401  re-exported
...
from marcel_cli.auth_codex import (  # noqa: F401  re-exported
...
from marcel_cli.auth_spotify import (  # noqa: F401  re-exported
    _refresh_spotify_oauth_state, get_spotify_auth_status, login_spotify_command,
    resolve_spotify_runtime_credentials)
from marcel_cli.auth_qwen import (  # noqa: F401  re-exported
...
from marcel_cli.auth_constants import (  # noqa: F401  re-exported
...
GOOGLE_WORKSPACE_SCOPE_MAP: Dict[str, Tuple[str, ...]] = {
...
GOOGLE_WORKSPACE_CLIENT_ID_ENV = "GOOGLE_WORKSPACE_CLIENT_ID"
GOOGLE_WORKSPACE_CLIENT_SECRET_ENV = "GOOGLE_WORKSPACE_CLIENT_SECRET"
GOOGLE_WORKSPACE_REFRESH_TOKEN_ENV = "GOOGLE_WORKSPACE_REFRESH_TOKEN"
GOOGLE_WORKSPACE_ACCESS_TOKEN_ENV = "GOOGLE_WORKSPACE_ACCESS_TOKEN"
GOOGLE_WORKSPACE_TOKEN_EXPIRY_ENV = "GOOGLE_WORKSPACE_TOKEN_EXPIRY"
GOOGLE_WORKSPACE_SCOPES_ENV = "GOOGLE_WORKSPACE_SCOPES"


def google_workspace_scopes(services: Iterable[str]) -> List[str]:
    """Return the minimum stable OAuth scope set for the selected Workspace services."""
    scopes: List[str] = []
    for service in services:
        for scope in GOOGLE_WORKSPACE_SCOPE_MAP.get(str(service).strip().lower(), ()):
            if scope not in scopes:
                scopes.append(scope)
    return scopes


def _google_token_expiry(value: str) -> Optional[datetime]:
    try:
...
def get_google_workspace_credentials(
    *,
    refresh: bool = True,
    _credentials_factory: Optional[Callable[..., Any]] = None,
    _request_factory: Optional[Callable[[], Any]] = None,
) -> Any:
    """Resolve Google credentials from profile-scoped secret environment values."""
    from marcel_cli.config import get_env_value_prefer_dotenv, save_env_value_secure

    if _credentials_factory is None:
        from google.oauth2.credentials import Credentials
        _credentials_factory = Credentials
    if _request_factory is None:
        from google.auth.transport.requests import Request
        _request_factory = Request

    client_id = get_env_value_prefer_dotenv(GOOGLE_WORKSPACE_CLIENT_ID_ENV)
    client_secret = get_env_value_prefer_dotenv(GOOGLE_WORKSPACE_CLIENT_SECRET_ENV)
    refresh_token = get_env_value_prefer_dotenv(GOOGLE_WORKSPACE_REFRESH_TOKEN_ENV)
    access_token = get_env_value_prefer_dotenv(GOOGLE_WORKSPACE_ACCESS_TOKEN_ENV)
    expiry = _google_token_expiry(
        get_env_value_prefer_dotenv(GOOGLE_WORKSPACE_TOKEN_EXPIRY_ENV) or ""
    )
    scopes = (get_env_value_prefer_dotenv(GOOGLE_WORKSPACE_SCOPES_ENV) or "").split()
    if not client_id or not client_secret or not refresh_token:
        raise AuthError("Google Workspace is not authorized for this Marcel profile.")

    credentials = _credentials_factory(
        token=access_token,
    ...
    if refresh and (expiry is None or not credentials.valid):
        credentials.refresh(_request_factory())
        save_env_value_secure(GOOGLE_WORKSPACE_ACCESS_TOKEN_ENV, credentials.token)
        if credentials.expiry:
            save_env_value_secure(
                GOOGLE_WORKSPACE_TOKEN_EXPIRY_ENV, credentials.expiry.isoformat()
            )
    return credentials


def _verify_google_workspace_access(credentials: Any, services: Iterable[str]) -> None:
    """Make one harmless read against a selected API, falling back to token introspection."""
    selected = list(dict.fromkeys(str(service).strip().lower() for service in services))
    ...
    if "calendar" in selected:
        build("calendar", "v3", credentials=credentials).calendarList().list(maxResults=1).execute()
        return
    if "drive" in selected:
        build("drive", "v3", credentials=credentials).about().get(fields="user").execute()
        return
    if "contacts" in selected:
        build("people", "v1", credentials=credentials).people().connections().list(
            resourceName="people/me", pageSize=1, personFields="names"
        ).execute()
        return

    # Docs and Sheets have no account-level read that does not require a user resource ID.
    # Token introspection is read-free and confirms the grant and its effective scopes.
    response = httpx.post(
    ...
    granted = set(str(response.json().get("scope") or "").split())
    required = set(google_workspace_scopes(selected))
    if not required.issubset(granted):
        raise AuthError("Google authorized fewer Workspace permissions than Marcel requested.")


def login_google_workspace(
    services: Iterable[str],
    *,
    open_browser: bool = True,
    _flow_factory: Optional[Callable[..., Any]] = None,
    _request_factory: Optional[Callable[[], Any]] = None,
    _verify: Optional[Callable[[Any, Iterable[str]], None]] = None,
) -> Dict[str, Any]:
    """Authorize selected Google Workspace services and persist tokens outside YAML.

    The OAuth application's client credentials must already be available in the profile's
    secret environment. The interactive flow never asks the user to paste those credentials.
    """
    from marcel_cli.config import (
    ...
    selected_services = list(dict.fromkeys(
        str(service).strip().lower() for service in services if str(service).strip()
    ))
    client_id = get_env_value_prefer_dotenv(GOOGLE_WORKSPACE_CLIENT_ID_ENV)
    client_secret = get_env_value_prefer_dotenv(GOOGLE_WORKSPACE_CLIENT_SECRET_ENV)
    if not client_id or not client_secret:
        raise AuthError(
            "Google Workspace OAuth is not provisioned for this Marcel installation. "
            f"Configure {GOOGLE_WORKSPACE_CLIENT_ID_ENV} and "
            f"{GOOGLE_WORKSPACE_CLIENT_SECRET_ENV} in secret storage."
        )

    scopes = google_workspace_scopes(selected_services)
    if not scopes:
        raise AuthError("Select at least one Google Workspace service before authorizing.")

    if _flow_factory is None:
        try:
            from google_auth_oauthlib.flow import InstalledAppFlow
        except ImportError as exc:
            raise AuthError(
                "Google OAuth support is not installed. Install Marcel with the 'google' extra."
            ) from exc
        _flow_factory = InstalledAppFlow.from_client_config
    ...
    flow = _flow_factory(client_config, scopes=scopes)
    credentials = flow.run_local_server(
        ...
        authorization_prompt_message=(
            "Opening Google consent for the selected Workspace services: {url}"
        ),
        success_message="Google Workspace authorization complete. You can close this window.",
    )

    refresh_token = credentials.refresh_token or get_env_value_prefer_dotenv(
        GOOGLE_WORKSPACE_REFRESH_TOKEN_ENV
    )
    if not refresh_token:
        raise AuthError(
            "Google did not return a refresh token. Revoke Marcel's Google access and authorize again."
        )

    # A refresh is a minimal, read-free Google call that proves the grant can be used safely.
    if _request_factory is None:
        from google.auth.transport.requests import Request
        _request_factory = Request
    credentials.refresh_token = refresh_token
    credentials.refresh(_request_factory())
    (_verify or _verify_google_workspace_access)(credentials, selected_services)

    save_env_value_secure(GOOGLE_WORKSPACE_REFRESH_TOKEN_ENV, refresh_token)
    save_env_value_secure(GOOGLE_WORKSPACE_ACCESS_TOKEN_ENV, credentials.token)
    if credentials.expiry:
        save_env_value_secure(
            GOOGLE_WORKSPACE_TOKEN_EXPIRY_ENV, credentials.expiry.isoformat()
        )
    save_env_value_secure(GOOGLE_WORKSPACE_SCOPES_ENV, " ".join(scopes))
    return {"verified": True, "services": selected_services, "scopes": scopes}
...
_REGISTRY_ROWS: Tuple[Any, ...] = (
    ...
    # is a 401), so there is no secret to configure. Select via `marcel model` / `/model free`.
...
def get_anthropic_key() -> str:
    ...
    Checks both the ``.env`` file and the process environment, preferring ``~/.marcel/.env`` so a deliberate
    key rotation isn't shadowed by a stale shell export (matches the api-key resolution path — see #20591).
    """
    from marcel_cli.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(provider_id: str, pconfig: ProviderConfig) -> tuple[str, str]:
    ...
    if provider_id == "copilot":
        ...
        try:
            from marcel_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    # Prefer ~/.marcel/.env over os.environ so a deliberate key rotation in .env isn't shadowed by
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    re-authenticating cannot fix it, so callers should say "retry later", not ``marcel auth``."""
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `marcel model` to re-authenticate."
...
# ── Auth Store — persistence layer for ~/.marcel/auth.json ──────────────────────────────────────────

def _auth_file_path() -> Path:
    path = get_marcel_home() / "auth.json"
    # Seat belt: under pytest, refuse to touch the real user's auth store (tests that forgot to
    # monkeypatch MARCEL_HOME or escaped the hermetic conftest). In production: one dict lookup.
    if (os.environ.get("PYTEST_CURRENT_TEST")
            and _same_path(path, Path.home() / ".marcel" / "auth.json")):
        raise RuntimeError(
            f"Refusing to touch real user auth store during test run: {path}. "
            "Set MARCEL_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    try:
        ...
        global_root = get_default_marcel_root()
    except Exception:
        return None
    return None if _same_path(get_marcel_home(), global_root) else global_root / "auth.json"
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST") and os.environ.get("HOME"):
        real_root = Path(os.environ["HOME"]) / ".marcel" / "auth.json"
        try:
...
def _save_auth_store(auth_store: Dict[str, Any], target_path: Optional[Path] = None) -> Path:
    ...
    # enforced); ignore failures. secure_parent_dir refuses to chmod /, top-level dirs, or the marcel-agent
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    """Set ``active_provider`` only when none is set yet: the first ``marcel auth add`` credential must
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    when the profile has zero entries for it (``marcel auth add`` in the profile shadows global)."""
...
def write_credential_pool(
    ...
    *status_cleared_ids* were cleared deliberately (``marcel auth reset``) and skip the
...
def _explicit_pool_entry_present(normalized: str) -> bool:
    """Pool rows from EXPLICIT Marcel flows (manual add / device-code / PKCE) or live env keys;
...
# Set by Claude Code itself, not by the user explicitly configuring anthropic in Marcel.
_IMPLICIT_ENV_VARS = frozenset({"CLAUDE_CODE_OAUTH_TOKEN"})
_EXPLICIT_POOL_SOURCES = frozenset({"device_code", "loopback_pkce", "marcel_pkce", "manual"})
...
def _explicit_env_credentials_present(normalized: str) -> bool:
    ...
    if pconfig is None:
        from marcel_cli.providers import get_provider
...
def _pool_entry_is_explicit(entry: Any) -> bool:
    """True for pool rows the user created via an explicit Marcel flow (or a still-live env key)."""
...
def _keyless_provider_has_explicit_config(normalized: str) -> bool:
    """Vertex / Bedrock count as explicit when Marcel-scoped routing config is present.
    ...
    Marcel-scoped signals (VERTEX_PROJECT_ID / vertex.project_id / VERTEX_CREDENTIALS_PATH) count
    here."""
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    Marcel-initiated flow, or Marcel-scoped routing config for keyless cloud-SDK providers. Ambient
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    ...
    ``marcel logout``. Returns True if something was cleared."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        from marcel_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""
        lines = ["Config issue detected — run 'marcel doctor' for full diagnostics:"]
        for ci in issues:
...
def _refuse_env_adoption_if_config_corrupt() -> None:
    ...
    try:
        from marcel_cli.config import get_active_config_parse_failure
    ...
    raise AuthError(
        ...
        f"was saved next to it) or run marcel setup.",
        code="corrupt_config")
...
def _openrouter_auto_detected(scoped_key_env: Callable[[str], str]) -> bool:
    ...
    `marcel auth add openrouter` has no env var; without the pool check it is invisible to
    ...
    if any(has_usable_secret(scoped_key_env(v)) for v in ("OPENAI_API_KEY", "OPENROUTER_API_KEY")):
        return True
    try:
        # Auto-detect an OpenRouter credential added via `marcel auth add openrouter` (manual pool entry, no
        # env var). Without this, a key that only lives in the credential pool is invisible to
        # auto-detection — the user sees `marcel auth list` showing the credential while requests go out
...
def _env_key_auto_detected(
    ...
    preempts a logged-in OAuth provider so a stale key in ~/.marcel/.env never switches silently."""
            ...
            if has_usable_secret(scoped_key_env(env_var)):
                if oauth_active and oauth_active != pid:
                    logger.warning(
                        ...
                        # ~/.marcel/.env isn't silently switched without knowing why.
...
def resolve_provider(
    ...
    *,
    ...
    if normalized != "auto":
        ...
        tail = (f"\n\n{hint}" if hint else " Check 'marcel model' for available providers, "
                "or run 'marcel doctor' to diagnose config issues.")
    ...
    raise AuthError(
        "No inference provider configured. Run 'marcel model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.marcel/.env.",
...
def _nous_portal_base_url(state: Dict[str, Any]) -> str:
    """MARCEL_PORTAL_BASE_URL / NOUS_PORTAL_BASE_URL is the trusted operator override and wins
...
def resolve_nous_access_token(
    *,
    ...
    with _provider_state_transaction("nous") as (auth_store, state, state_source_path):
        if not state:
            raise _nous_err("Marcel is not logged into Nous Portal.", relogin=True)
...
class OAuthProviderFlow:
    ...
    ``monkeypatch.setattr("marcel_cli.auth.resolve_codex_runtime_credentials", ...)`` applies."""
    provider_id: str
    resolve_fn: str
    status_fn: str
    terminal_refresh_codes: FrozenSet[str] = frozenset()  # retrying the same refresh token cannot succeed
    # ``marcel logout`` with no active provider falls back to config.yaml ``model.provider`` only
...
def _provider_is_keyless(provider_id: str) -> bool:
    """MarcelOverlay keyless flag — the same source the provider catalog and GUI contract tests use."""
    try:
        from marcel_cli.providers import MARCEL_OVERLAYS
        return bool(getattr(MARCEL_OVERLAYS.get(provider_id), "keyless", False))
    except Exception:
        return False
...
def _external_process_auth_evidence(provider_id: str) -> tuple[bool, Optional[str]]:
    ...
    Marcel can't read). Deliberately subprocess-free: spawning ``gh auth token`` from status
    ...
    try:
        from marcel_cli.copilot_auth import COPILOT_ENV_VARS, validate_copilot_token
...
def _external_process_spec(
    ...
    Launch details come from the provider's own profile (copilot-acp: MARCEL_COPILOT_ACP_COMMAND /
    COPILOT_CLI_PATH / MARCEL_COPILOT_ACP_ARGS), so out-of-tree providers describe their binary."""
...
def get_auth_status(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    tests that patch ``marcel_cli.auth.get_*_auth_status`` still apply."""
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    CLI startup flat; ``marcel doctor`` runs the live probe). ``api_key`` (default): usable
    ``AZURE_FOUNDRY_API_KEY``."""
    info: Dict[str, Any] = {"provider": "azure-foundry"}
    try:
        from marcel_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            info.update(
                ...
                hint=(
                    ...
                    "is skipped here. Run `marcel doctor` to verify token acquisition."
                ) if installed else (
                    "azure-identity not installed. Install with: "
                    "pip install azure-identity  (or rely on Marcel' "
...
def _update_config_for_provider(
    ...
    *default_model*, when given, is written as ``model.default`` in the same step so the gateway
    ...
    from marcel_cli.config import clear_model_endpoint_credentials
...
def login_command(args) -> None:
    """Deprecated: use 'marcel model' or 'marcel setup' instead."""
    print("The 'marcel login' command has been removed.\nUse 'marcel auth' to manage credentials,\n"
          "'marcel model' to select a provider, or 'marcel setup' for full setup.")
    raise SystemExit(0)
...
def logout_command(args) -> None:
    ...
    if not should_reset_config:
        ...
        print("Marcel will use OpenRouter for inference.")
    else:
        print("Run `marcel model` or configure an API key to use Marcel.")
...
_PLUGIN_COMPAT_LAZY = {
    'CODEX_OAUTH_USER_AGENT': ('marcel_cli.auth_constants', 'CODEX_OAUTH_USER_AGENT'),
    'CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS': ('marcel_cli.auth_codex', 'CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS'),
    'DEFAULT_SPOTIFY_REDIRECT_URI': ('marcel_cli.auth_constants', 'DEFAULT_SPOTIFY_REDIRECT_URI'),
    'DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS': ('marcel_cli.auth_constants', 'DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS'),
    'MINIMAX_OAUTH_GRANT_TYPE': ('marcel_cli.auth_constants', 'MINIMAX_OAUTH_GRANT_TYPE'),
    'NOUS_INFERENCE_INVOKE_SCOPE': ('marcel_cli.auth_constants', 'NOUS_INFERENCE_INVOKE_SCOPE'),
    'NOUS_SHARED_STORE_FILENAME': ('marcel_cli.auth_nous', 'NOUS_SHARED_STORE_FILENAME'),
    'OAUTH_OVER_SSH_DOCS_URL': ('marcel_cli.auth_constants', 'OAUTH_OVER_SSH_DOCS_URL'),
    'QWEN_OAUTH_CLIENT_ID': ('marcel_cli.auth_constants', 'QWEN_OAUTH_CLIENT_ID'),
    'QWEN_OAUTH_TOKEN_URL': ('marcel_cli.auth_constants', 'QWEN_OAUTH_TOKEN_URL'),
    'SINGLE_USE_OAUTH_SINGLETON_FILES': ('marcel_cli.auth_oauth_grants', 'SINGLE_USE_OAUTH_SINGLETON_FILES'),
    'SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS': ('marcel_cli.auth_constants', 'SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS'),
    'SPOTIFY_DASHBOARD_URL': ('marcel_cli.auth_constants', 'SPOTIFY_DASHBOARD_URL'),
    'XAI_OAUTH_DEVICE_CODE_URL': ('marcel_cli.auth_constants', 'XAI_OAUTH_DEVICE_CODE_URL'),
    'XAI_OAUTH_DISCOVERY_URL': ('marcel_cli.auth_constants', 'XAI_OAUTH_DISCOVERY_URL'),
    'XAI_OAUTH_ISSUER': ('marcel_cli.auth_constants', 'XAI_OAUTH_ISSUER'),
    'refresh_nous_oauth_pure': ('marcel_cli.auth_nous', 'refresh_nous_oauth_pure'),
}
