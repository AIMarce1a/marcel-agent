"""On-demand supply-chain audit for Marcel Agent installs.
...
def _discover_plugins(marcel_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.marcel/plugins``. Plugins typically don't install
...
def _discover_components(
    *, skip_venv: bool = False, skip_plugins: bool = False, skip_mcp: bool = False, marcel_home: Optional[Path] = None
) -> list[Component]:
    """Discover all scannable components across the enabled sources."""
    home = marcel_home or Path(get_marcel_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `marcel security audit`."""
    home = Path(get_marcel_home())
    ...
    components = _discover_components(marcel_home=home, **skips)
    ...
    try:
        findings = run_audit(marcel_home=home, components=components)
