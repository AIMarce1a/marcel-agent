...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    values: Dict[str, Any] = {}
    for path in (marcel_home / f"{name}.json", marcel_home / name / "config.json"):
    ...
    # Holographic stores under plugins.marcel-memory-store.
    plugins_cfg = cfg.get("plugins")
    if name == "holographic" and isinstance(plugins_cfg, dict):
        holographic_cfg = plugins_cfg.get("marcel-memory-store")
