...
from marcel_cli.pty_session import PtySessionRegistry

# Same logger the code used before extraction (record parity).
_log = logging.getLogger("marcel_cli.web_server")


# /api/pty spawns ``marcel --tui`` behind a pseudo-terminal and forwards bytes +
...
try:
    if sys.platform.startswith("win"):
        from marcel_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    else:
        from marcel_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _ws_host_origin_reason(ws: "WebSocket") -> Optional[str]:
    ...
    """
    from marcel_cli.web_server import _is_accepted_host, app
...
_GATEWAY_WS_PROTOCOL = "marcel-gateway-v1"
_GATEWAY_WS_TICKET_PROTOCOL_PREFIX = "marcel-gateway-ticket."
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    """
    from marcel_cli.web_server import _SESSION_TOKEN, app
    auth_required = bool(getattr(app.state, "auth_required", False))
    if auth_required:
        # Lazy import — keeps this function importable in test harnesses
        # that don't bring in the dashboard_auth layer.
        from marcel_cli.dashboard_auth.audit import AuditEvent, audit_log
        from marcel_cli.dashboard_auth.ws_tickets import (
        ...
        try:
            ...
            if protocol_ticket:
                ...
                ws._marcel_ws_subprotocol = _GATEWAY_WS_PROTOCOL
...
def _resolve_chat_argv(
    ...
    """Resolve the argv + cwd + env for the chat PTY (what ``marcel --tui`` runs).

    Tests monkeypatch this with a tiny fake command.  Env contract: resume goes
    through ``MARCEL_TUI_RESUME`` (``ui-tui`` does not parse argv), resolved to
    the newest descendant; ``MARCEL_TUI_GATEWAY_URL`` attaches to this process's
    ...
    ``profile`` scopes the ENTIRE chat by pointing ``MARCEL_HOME`` at the profile
    dir, the same propagation ``marcel -p <name>`` performs.
    """
    from marcel_cli.web_server_profiles import _config_profile_scope, _resolve_profile_dir
    from marcel_cli.web_server_sessions import _open_session_db_for_profile, _session_latest_descendant
    from marcel_cli.main import PROJECT_ROOT
    from marcel_cli.main_tui_launch import _apply_tui_python_env, _make_tui_argv
    ...
    # scope overrides MARCEL_HOME before config is bridged into the env.
    from tools.environments.local import build_subprocess_env
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=True)
    if profile_dir is not None:
        env["MARCEL_HOME"] = str(profile_dir)
    try:
    ...
    env.setdefault("MARCEL_TUI_DISABLE_MOUSE", "1")
    env.setdefault("MARCEL_TUI_INLINE", "1")
    ...
    if resume:
        ...
        env["MARCEL_TUI_RESUME"] = resume

    if sidecar_url:
        env["MARCEL_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # which inherits the profile MARCEL_HOME set above.
    if profile_dir is None and (gateway_ws_url := _build_gateway_ws_url()):
        env["MARCEL_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> Optional[str]:
    """Host the in-container WS client should dial: ``MARCEL_DASHBOARD_WS_HOST``
    wins always; a wildcard bind becomes ``127.0.0.1``; others verbatim."""
    from marcel_cli.web_server import app
    explicit = os.environ.get("MARCEL_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _server_internal_ws_url(path: str, **extra_qs) -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from marcel_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def _resolve_chat_argv_async(
    ...
    from marcel_cli.web_server import _get_chat_argv_lock, app
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    from marcel_cli.web_server import _get_pty_active_session_files
    files = _get_pty_active_session_files(app)
    if files.get(channel) is None:
        fd, raw_path = tempfile.mkstemp(prefix="marcel-pty-active-", suffix=".json")
        os.close(fd)
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    max_workers=_CONSOLE_EXECUTOR_MAX_WORKERS, thread_name_prefix="marcel-console")
