"""Cross-process mutual exclusion for in-flight Marcel updates.
...
MARKER_NAME = ".marcel-update-in-progress"

# Set by an orchestrating updater (Tauri `marcel-setup --update`) to its own pid before
# spawning `marcel update` as a child stage; the parent holds the marker for its whole run,
...
HANDOFF_PID_ENV = "MARCEL_UPDATE_HANDOFF_PID"
...
# Tauri updater (UPDATE_EXIT_CONCURRENT in update.rs) to show "Marcel is still running".
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Marcel home (never the context-local profile override): the Rust
    updater resolves ``$MARCEL_HOME`` or the platform default and the desktop pins that same
    ...
    return get_process_marcel_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``marcel update`` as a (grand)child, so a live marker
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Marcel update is already running (PID {holder.pid}, "
