#!/usr/bin/env python3
"""Marcel CLI - Main entry point.

Usage:
    marcel                     # Interactive chat (default)
    marcel chat / gateway / setup / status / cron / doctor / update / ...
    marcel --version           # Show version and update status
    marcel <cmd> --help        # Per-command help
"""

# marcel_bootstrap must be the very first import — it sets up UTF-8 stdio on
# Windows (no-op on POSIX). Guarded: after a ``git pull`` / interrupted
# ``marcel update`` the editable install's ``.pth`` may not list it yet; crashing
# here would block ``marcel update``.
try:
    import marcel_bootstrap  # noqa: F401
...
from marcel_cli._subprocess_compat import suppress_platform_ver_console
...
# Preserve the public entrypoint across gateway supervisor/replacement
# subprocesses, whose reconstructed argv may use the compatibility module name.
if os.path.basename(sys.argv[0]).lower().startswith("marcel"):
    os.environ.setdefault("MARCEL_BRANDING", "1")

# Inline path math so ``python marcel_cli/main.py`` (script mode: sys.path[0]
# is marcel_cli/, not the repo root) can import marcel_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
# Managed cutover must happen before _startup_fast/config/env imports can
# observe an empty new home. The importer reads no secret values and raises a
# clear error if the old gateway is still live.
from marcel_migration import migrate_if_needed as _migrate_legacy_if_needed

_migrate_legacy_if_needed()
from marcel_cli import _startup_fast  # noqa: E402

# Early venv self-heal — MUST run before any third-party import below. A prior
# ``marcel update`` may have left a recovery marker with a core package wiped;
# the marcel_cli.config/env_loader imports further down would then crash before
...
# PID" class it exists for. ``marcel_startup_watchdog`` is stdlib-only so it
...
if _argv_is_gateway_run(sys.argv[1:]):
    try:
        from marcel_startup_watchdog import arm_startup_watchdog as _arm_sw
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from marcel_cli.oneshot import run_oneshot
...
def _set_process_title() -> None:
    """Cosmetic: show 'marcel' instead of 'python3.xx' in ps/top/htop.

    Order: opt-in ``setproctitle`` dep; ctypes ``prctl(PR_SET_NAME)`` (Linux,
    15-char limit); ``pthread_setname_np`` (macOS — lldb/top only, not ``ps
    aux``); no-op on Windows (the .exe is already ``marcel.exe``). Never fatal.
    """
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("marcel")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"marcel", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"marcel")
    except Exception:
        pass
...
# marcel_cli.config is importable. Cached so early callers don't re-parse YAML.
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("MARCEL_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".marcel", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    Precedence: ``--cli`` wins, then ``--tui``/``MARCEL_TUI=1``, then a
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("MARCEL_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `resetTerminalModes()` is the later cousin. ``MARCEL_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("MARCEL_TUI_NO_EARLY_DISABLE") == "1":
        return
...
# ``marcel --version`` is answered before config/logging imports.
...
from marcel_cli.subcommands.cron import build_cron_parser
from marcel_cli.subcommands.sync import build_sync_parser
from marcel_cli.subcommands.gateway import build_gateway_parser
from marcel_cli.subcommands.profile import build_profile_parser
from marcel_cli.subcommands.model import build_model_parser
from marcel_cli.subcommands.setup import build_setup_parser

from marcel_cli.subcommands.whatsapp import build_whatsapp_parser, build_whatsapp_cloud_parser
from marcel_cli.subcommands.slack import build_slack_parser
from marcel_cli.subcommands.login import build_login_parser
from marcel_cli.subcommands.logout import build_logout_parser
from marcel_cli.subcommands.auth import build_auth_parser
from marcel_cli.subcommands.status import build_status_parser
from marcel_cli.subcommands.pause import build_pause_parser
from marcel_cli.subcommands.webhook import build_webhook_parser
from marcel_cli.subcommands.hooks import build_hooks_parser
from marcel_cli.subcommands.doctor import build_doctor_parser
from marcel_cli.subcommands.verify import build_verify_parser
from marcel_cli.subcommands.security import build_security_parser
from marcel_cli.subcommands.approvals import build_approvals_parser
from marcel_cli.subcommands.dump import build_dump_parser
from marcel_cli.subcommands.debug import build_debug_parser
from marcel_cli.subcommands.backup import build_backup_parser
from marcel_cli.subcommands.import_cmd import build_import_cmd_parser
from marcel_cli.subcommands.import_agent import build_import_agent_parser
from marcel_cli.subcommands.config import build_config_parser
from marcel_cli.subcommands.skin import build_skin_parser
from marcel_cli.subcommands.console import build_console_parser
from marcel_cli.subcommands.update import build_update_parser
from marcel_cli.subcommands.uninstall import build_uninstall_parser
from marcel_cli.subcommands.dashboard import build_dashboard_parser, build_serve_parser
from marcel_cli.subcommands.gui import build_gui_parser
from marcel_cli.subcommands.logs import build_logs_parser
from marcel_cli.subcommands.prompt_size import build_prompt_size_parser
from marcel_cli.subcommands.memory import build_memory_parser
from marcel_cli.subcommands.acp import build_acp_parser
from marcel_cli.subcommands.tools import build_tools_parser
from marcel_cli.subcommands.insights import build_insights_parser
from marcel_cli.subcommands.monitoring import build_monitoring_parser
from marcel_cli.subcommands.skills import build_skills_parser
from marcel_cli.subcommands.pairing import build_pairing_parser
from marcel_cli.subcommands.plugins import build_plugins_parser
from marcel_cli.subcommands.mcp import build_mcp_parser
from marcel_cli.subcommands.claw import build_claw_parser
from marcel_cli.subcommands.moa import build_moa_parser
from marcel_cli.subcommands.fallback import build_fallback_parser
from marcel_cli.subcommands.worktree import build_worktree_parser
from marcel_cli.subcommands.browser import build_browser_parser
from marcel_cli.subcommands.secrets import build_secrets_parser
from marcel_cli.subcommands.egress import build_egress_parser
from marcel_cli.subcommands.migrate import build_migrate_parser
from marcel_cli.subcommands.migrate_legacy import build_migrate_legacy_parser
from marcel_cli.subcommands.checkpoints import build_checkpoints_parser
from marcel_cli.subcommands.bundles import build_bundles_parser
from marcel_cli.subcommands.curator import build_curator_parser
from marcel_cli.subcommands.pets import build_pets_parser
from marcel_cli.subcommands.journey import build_journey_parser
from marcel_cli.subcommands.computer_use import build_computer_use_parser
from marcel_cli.subcommands.sessions import build_sessions_parser
from marcel_cli.subcommands.completion import build_completion_parser
...
def _require_tty(command_name: str) -> None:
    ...
    if not sys.stdin.isatty():
        print(
            f"Error: 'marcel {command_name}' requires an interactive terminal.\n"
...
# Profile override — MUST happen before any marcel module import: many modules
# cache MARCEL_HOME at import time. --profile/-p is pre-parsed from sys.argv,
# MARCEL_HOME set, and the flag stripped so argparse never sees it. Falls back
# to ~/.marcel/active_profile for the sticky default.
_PROFILE_NAME_RE = r"^[a-z0-9][a-z0-9_-]{0,63}$"  # mirrors marcel_cli.profiles._PROFILE_ID_RE


def _inside_mcp_add_args(argv: list, index: int) -> bool:
    """True once argv reaches `marcel mcp add ... --args <command argv>`.
    ...
    ``--profile``), not to Marcel' own profile selector.
    """
    try:
...
def _scan_profile_flag(argv: list) -> tuple:
    ...
    Historically the flag worked even after the subcommand (`marcel chat -p
    ...
    """
    from marcel_cli._parser import top_level_value_flag_sets
...
def _resolve_sudo_user_profile_env(name: str) -> str | None:
    """Resolve `sudo marcel -p <name>` against the invoking user's home.
    ...
    try:
        ...
        candidate = Path(pwd.getpwnam(sudo_user).pw_dir) / ".marcel" / "profiles" / name
...
def _under_gateway_supervisor(argv: list) -> bool:
    ...
    ``-p <name>`` or pin MARCEL_HOME to the profile dir; a bare invocation
    means "the root MARCEL_HOME profile". If a supervised default-profile
    child read active_profile, switching the active profile (dashboard,
    ``marcel profile use``) would silently redirect the default gateway into
    ...
    MARCEL_SUPERVISED_CHILD (systemd unit / launchd plist / Windows task),
    MARCEL_S6_SUPERVISED_CHILD (legacy s6 container), INVOCATION_ID (systemd
    ...
    self-hosted CI runners), MARCEL_GATEWAY_EXTERNAL_SUPERVISOR (explicit
    opt-in). XPC_SERVICE_NAME is deliberately NOT consulted: interactive macOS
    terminals set it too.
    """
    if os.environ.get("MARCEL_SUPERVISED_CHILD") or os.environ.get("MARCEL_S6_SUPERVISED_CHILD"):
        return True
...
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set MARCEL_HOME before imports."""
    argv = sys.argv[1:]
    profile_name, consume, profile_index = _scan_profile_flag(argv)

    # MARCEL_HOME already set with no explicit flag: trust it only when it
    # points at a specific profile dir ("profiles" as immediate parent). If it
    # points at the marcel root (systemd hardcodes MARCEL_HOME=/root/.marcel)
    # we must still read active_profile — the user may have run
    # `marcel profile use` and the gateway should honour it (#22502).
    marcel_home_env = os.environ.get("MARCEL_HOME", "")
    if profile_name is None and marcel_home_env and Path(marcel_home_env).parent.name == "profiles":
        return

    if profile_name is None and not _under_gateway_supervisor(argv):
        try:
            from marcel_constants import get_default_marcel_root

            active_path = get_default_marcel_root() / "active_profile"
    ...
    if profile_name is None:
        return
    try:
        from marcel_cli.profiles import resolve_profile_env

        marcel_home = resolve_profile_env(profile_name)
    except FileNotFoundError as exc:
        marcel_home = _resolve_sudo_user_profile_env(profile_name)
        if not marcel_home:
        ...
        # A bug in profiles.py must NEVER prevent marcel from starting
        print(f"Warning: profile override failed ({exc}), using default", file=sys.stderr)
        return
    os.environ["MARCEL_HOME"] = marcel_home
...
# Windows launcher self-heal — the ``marcel`` command is a COPY of the venv
# console script staged into the managed bin dir (outside the checkout, since
# ``marcel update``'s autostash once swept ``<checkout>\bin`` copies off disk;
...
# app's ``python -m marcel_cli.main`` spawn. Gates fail toward inaction. Sits
# AFTER the profile override on purpose — no marcel module may import before
...
# ``<checkout>\bin``, where ``marcel update``'s autostash (``git stash push --include-untracked``) swept
# them off disk; with the desktop updater's ``--keep-stash`` nothing restored them and ``marcel`` stopped
...
if sys.platform == "win32":
    try:
        from marcel_cli import _install_repair as _install_repair_mod

        _install_repair_mod.ensure_windows_bin_launchers(_bootstrap_root)
    except Exception:
        pass

# Load .env from ~/.marcel/.env first, then project root as dev fallback.
...
# Bridge security.redact_secrets → MARCEL_REDACT_SECRETS BEFORE marcel_logging
...
try:
    ...
    # marcel_logging and later raw reads: 3-4 config.yaml parses become one.
    from marcel_cli.config import read_raw_config as _read_raw_early

    _cfg_path = get_marcel_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            from marcel_cli import managed_scope
            _early_cfg_raw = managed_scope.apply_managed_overlay(_early_cfg_raw)
        except Exception:
            pass
        if "MARCEL_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["MARCEL_REDACT_SECRETS"] = str(_early_redact).lower()
...
try:
    from marcel_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from marcel_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if marcel_constants not importable yet

import logging
import threading
from datetime import datetime

from marcel_cli import __version__, __release_date__

from marcel_cli.model_setup_flows import (
...
from marcel_cli.main_agent_cmds import (
    cmd_acp,
    cmd_insights,
    cmd_memory,
    cmd_monitoring,
...
from marcel_cli.main_platform_setup import (
    cmd_slack,
    cmd_sync,
    cmd_whatsapp,
    cmd_whatsapp_cloud,
)
from marcel_cli.main_dashboard import (
...
from marcel_cli.main_dashboard import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _respawn_dashboard_processes,
)
from marcel_cli.main_provider_setup import (
...
from marcel_cli.main_install_repair import (
    _cleanup_quarantined_exes,
    _recover_from_interrupted_install,
)
from marcel_cli.main_install_repair import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    ShimQuarantineError,
    _UPDATE_REEXEC_ENV,
...
from marcel_cli.main_desktop import (
    cmd_gui,
)
from marcel_cli.main_desktop import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _desktop_build_needed,
    _desktop_dist_exists,
    _desktop_macos_relaunchable_fixup,
    _desktop_packaged_executable,
)
from marcel_cli.main_web_build import (
    _sweep_stale_bytecode_if_checkout_changed,
)
from marcel_cli.main_web_build import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _build_web_ui,
    _nixos_build_env,
    _record_bytecode_fingerprint,
    _run_npm_install_deterministic,
)
from marcel_cli.main_tui_launch import (
    _launch_tui,
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_marcel_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("MARCEL_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("MARCEL_TERMUX_PREFETCH_UPDATES") == "1"


def _dotenv_has_provider_key(env_file: Path, provider_env_vars: set) -> bool:
    """True if ~/.marcel/.env assigns a non-empty value to any provider key."""
...
def _auth_store_logged_in(auth_file: Path, registry, strict_profile_scope: bool) -> bool:
    ...
    from marcel_cli.auth import get_auth_status
...
def _has_any_provider_configured(*, strict_profile_scope: bool = False) -> bool:
    ...
    """
    from marcel_cli.config import DEFAULT_CONFIG, get_env_path, get_marcel_home, load_config
    from marcel_cli.auth import PROVIDER_REGISTRY, get_auth_status
    ...
    if isinstance(model_cfg, dict):
        ...
        if isinstance(_model_name, dict):
            from marcel_cli.config import split_model_config_default
    ...
    _has_marcel_config = _model_name and _model_name != DEFAULT_CONFIG.get("model", "")
    ...
    if _auth_store_logged_in(get_marcel_home() / "auth.json", PROVIDER_REGISTRY, strict_profile_scope):
        return True
    ...
    # Claude Code OAuth credentials count only once Marcel is explicitly
    # configured — having Claude Code installed isn't consent to use its tokens.
    if _has_marcel_config and not strict_profile_scope:
        try:
...
def _confirm_startup_expensive_model_override(args) -> None:
    ...
    try:
        ...
        from marcel_cli.model_selection_guards import (
            combined_message,
            selection_warnings,
        )
    except Exception as exc:
...
def _session_db():
    """Yield a ``SessionDB`` (lazy import, so test patches on ``marcel_state``
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``marcel -c`` from repo A continues repo A's last session rather than the
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, marcel_bin
        cli_args: the original CLI arguments (everything after 'marcel')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    marcel_bin = container_info["marcel_bin"]
    ...
    if _probe_container(inspect_cmd, backend).returncode != 0:
        ...
        if not sudo_path:
            print(
                ...
                f"The container may be running under root. Try: sudo marcel {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        if _probe_container(cmd_prefix[:2] + inspect_cmd, backend, via_sudo=True).returncode != 0:
            print(
                ...
                f"  }}];\n"
                f"\n"
                f"Or run: sudo marcel {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print(
                    "Use 'marcel sessions list' to see available sessions, or "
            ...
            try:
                from marcel_cli.terminal_breadcrumbs import resolve_breadcrumb_session
...
def _import_foreign_resume(args) -> None:
    ...
    if not (isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in ("@claude", "@codex")):
        return
    from marcel_cli.foreign_sessions import import_foreign_session, pick_foreign_session
    ...
    print(f"  (later: marcel --resume {_imported_id})")
...
def _resolve_chat_session_args(args, use_tui: bool) -> None:
    ...
    if isinstance(_resume_raw, str) and _resume_raw.strip().lower() == "latest":
        ...
        if _last_id:
            ...
            print("Use 'marcel sessions list' to see available sessions.")
            sys.exit(1)
...
def _warn_retired_xai_models() -> None:
    ...
    try:
        from marcel_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from marcel_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'marcel doctor' for details.\033[0m\n\n")
    except Exception:
        pass
...
def _start_chat_background_prefetch() -> None:
    ...
    The ONE exception is an unseeded ~/.marcel/skills: there the banner
    ...
    if _termux_should_prefetch_update_check():
        try:
            from marcel_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    def _skills_dir_is_unseeded() -> bool:
        try:
            from marcel_cli.config import get_marcel_home
            skills_dir = Path(get_marcel_home()) / "skills"
    ...
    if _skills_dir_is_unseeded():
        ...
        try:
            import marcel_cli.banner as _banner_mod
...
def _first_run_setup_guard(args) -> None:
    """No provider configured: offer `marcel setup` (TTY) or exit 1 with guidance."""
    print()
    print(
        "It looks like Marcel isn't configured yet -- no API keys or providers found."
    )
    print()
    print("  Run:  marcel setup")
    print()
    ...
    if reply in {"", "y", "yes"}:
        ...
        return
    print()
    print("You can run 'marcel setup' at any time to configure.")
    sys.exit(1)
...
def cmd_chat(args):
    ...
    if getattr(args, "ignore_rules", False):
        os.environ["MARCEL_IGNORE_RULES"] = "1"
    # --source: tag session source for filtering (e.g. 'tool' for integrations)
    if getattr(args, "source", None):
        os.environ["MARCEL_SESSION_SOURCE"] = args.source
    ...
    try:
        ...
        # Mixed-version installs (new cli.py, older marcel_cli.config) crash
        # here — e.g. missing resolve_turn_limit / split_model_config_default
        # (#96900). The agent-setup mixin prints this hint too late: MarcelCLI
        # construction already failed. Fast-chat launch also goes through
        # cmd_chat, so this one catch covers `marcel` / `marcel chat`.
        from marcel_constants import emit_partial_update_hint
...
def cmd_gateway(args):
    ...
    from marcel_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from marcel_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def _forward_command(name: str, module: str, attr: str, *, forward_return: bool = False, doc: str = ""):
    """A ``marcel <cmd>`` handler that hands ``args`` to ``<module>.<attr>``.
...
cmd_setup = _forward_command("cmd_setup", "marcel_cli.setup", "run_setup_wizard", doc='Interactive setup wizard.')
cmd_login = _forward_command("cmd_login", "marcel_cli.auth", "login_command", doc='Authenticate Marcel CLI with a provider.')
cmd_logout = _forward_command("cmd_logout", "marcel_cli.auth", "logout_command", doc='Clear provider authentication.')
cmd_auth = _forward_command("cmd_auth", "marcel_cli.auth_commands", "auth_command", doc='Manage pooled credentials.')
cmd_status = _forward_command("cmd_status", "marcel_cli.status", "show_status", doc='Show status of all components.')
cmd_cron = _forward_command("cmd_cron", "marcel_cli.cron", "cron_command", doc='Cron job management.')
cmd_webhook = _forward_command("cmd_webhook", "marcel_cli.webhook", "webhook_command", doc='Webhook subscription management.')
cmd_kanban = _forward_command("cmd_kanban", "marcel_cli.kanban", "kanban_command", forward_return=True, doc='Multi-profile collaboration board.')
cmd_project = _forward_command("cmd_project", "marcel_cli.projects_cmd", "projects_command", forward_return=True, doc='Manage projects (named, multi-folder workspaces).')
cmd_hooks = _forward_command("cmd_hooks", "marcel_cli.hooks", "hooks_command", doc='Shell-hook inspection and management.')
cmd_doctor = _forward_command("cmd_doctor", "marcel_cli.doctor", "run_doctor", doc='Check configuration and dependencies.')
cmd_dump = _forward_command("cmd_dump", "marcel_cli.dump", "run_dump", doc='Dump setup summary for support/debugging.')
cmd_debug = _forward_command("cmd_debug", "marcel_cli.debug", "run_debug", doc='Debug tools (share report, etc.).')
cmd_skin = _forward_command("cmd_skin", "marcel_cli.skin_cmd", "skin_command", doc='Skin management (list / use / set).')
cmd_import = _forward_command("cmd_import", "marcel_cli.backup", "run_import", doc='Restore a Marcel backup from a zip file.')
cmd_dashboard_register = _forward_command("cmd_dashboard_register", "marcel_cli.dashboard_register", "cmd_dashboard_register", doc='Register a self-hosted dashboard OAuth client with Nous Portal.')
cmd_gateway_enroll = _forward_command("cmd_gateway_enroll", "marcel_cli.gateway_enroll", "cmd_gateway_enroll", doc='Enroll a self-hosted gateway with a relay connector.')
cmd_prompt_size = _forward_command("cmd_prompt_size", "marcel_cli.prompt_size", "cmd_prompt_size", doc='Show a byte/char breakdown of the system prompt + tool schemas.')
cmd_pairing = _forward_command("cmd_pairing", "marcel_cli.pairing", "pairing_command")
cmd_plugins = _forward_command("cmd_plugins", "marcel_cli.plugins_cmd", "plugins_command")
cmd_mcp = _forward_command("cmd_mcp", "marcel_cli.mcp_config", "mcp_command")
cmd_claw = _forward_command("cmd_claw", "marcel_cli.claw", "claw_command")
cmd_import_agent = _forward_command("cmd_import_agent", "marcel_cli.agent_import", "import_agent_command")
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from marcel_cli.models import clear_provider_models_cache
            clear_provider_models_cache()
            print("  Cleared model picker cache.")
        except Exception:
            pass
    from marcel_cli.setup import run_setup_action_with_navigation
...
def _resolve_active_provider(config, model_cfg, effective_provider, custom_provider_map):
    ...
    """
    from marcel_cli.auth import AuthError, format_auth_error, resolve_provider
    from marcel_cli.config import get_compatible_custom_providers, get_env_value
    from marcel_cli.providers import custom_provider_aliases, resolve_provider_full
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            print(
                f"Warning: Unknown provider '{effective_provider}'. Check 'marcel model' for "
                "available providers, or run 'marcel doctor' to diagnose config "
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``marcel model``) and the setup wizard
    ...
    effective_provider = config_provider or os.getenv("MARCEL_INFERENCE_PROVIDER") or "auto"
    ...
    from marcel_cli.models import _PROVIDER_LABELS
    ...
    # marcel_cli.main keep intercepting.
    ...
    # "custom") leaves a stale OPENAI_BASE_URL in ~/.marcel/.env that poisons
...
# ``marcel_cli/update_cmd*.py`` files resolve these names via ``_m().<name>``
# on marcel_cli.main; importing update_cmd eagerly would cost every ``marcel``
...
_FROZEN_UPDATER_SURFACE: dict[str, tuple[str, ...]] = {
    "marcel_cli.update_cmd": (
        ...
        "_print_parked_branch_skip_warning", "_purge_stale_marcel_modules",
    ...
    "marcel_cli.dashboard_procs": (
        "_detect_concurrent_marcel_instances", "_kill_stale_dashboard_processes",
    ),
}
...
def cmd_verify(args):
    ...
    from marcel_cli.verify_cmd import run_verify_command

    sys.exit(run_verify_command(args))


def cmd_security(args):
    """Dispatch `marcel security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from marcel_cli.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    """Dispatch `marcel approvals <subcmd>`."""
    from marcel_cli.approvals_suggest import approvals_command
...
def cmd_config(args):
    ...
    from marcel_cli.config import config_command
...
def cmd_backup(args):
    """Back up Marcel home directory to a zip file."""
    from marcel_cli import backup

    (backup.run_quick_backup if getattr(args, "quick", False) else backup.run_backup)(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    # Shared with the `marcel --version` pre-import fast path.
...
def cmd_uninstall(args):
    """Uninstall Marcel Agent (or just the Chat GUI with --gui).
    ...
    if getattr(args, "gui_summary", False):
        from marcel_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from marcel_cli.uninstall import run_gui_uninstall
    ...
    from marcel_cli.uninstall import run_uninstall
...
def _finalize_update_receipt(code: int, reason: str) -> None:
    ...
    try:
        ...
        from marcel_cli.update_receipt import finalize_pending_update_receipt
...
def _update_preflight_handled(args) -> bool:
    ...
    if is_managed():
        managed_error("update Marcel Agent")
        return True
    ...
    if getattr(args, "plan", False):
        # Read-only plan phase (#91277 Phase 2): inventory every running Marcel runtime across profiles, its
        # supervisor, and its running code version — without mutating anything. Safe on a live fleet.
        from marcel_cli.update_inventory import (
    ...
    from marcel_cli.update_contract import (
    ...
    if getattr(args, "check", False):
        ...
        from marcel_cli.update_cmd import _cmd_update_check
...
def cmd_update(args):
    """Update Marcel Agent: hangup protection + update lock around ``_cmd_update_impl``."""
    ...
    from marcel_cli.update_lock import (
    ...
    from marcel_cli.update_cmd import _cmd_update_impl
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    ``marcel -c Pokemon Agent Dev`` → ``['-c', 'Pokemon Agent Dev']``; tokens
...
from marcel_cli.profile_cmd import cmd_profile
...
def _dashboard_lifecycle_flags(args, token_file) -> None:
    ...
    if getattr(args, "stop", False):
        if not _find_stale_dashboard_pids():
            print("No marcel dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `marcel update`;
        # it prints outcomes itself. Exit 1 only if every pid was unkillable.
        from marcel_cli.dashboard_procs import _kill_stale_dashboard_processes
...
def _dashboard_validate_serve_args(args, headless_backend, token_file):
    ...
    # `marcel serve` is headless/non-interactive: fail closed on a corrupt
    ...
    if token_file and not headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with marcel serve")
...
def _dashboard_sanitize_desktop_env(headless_backend) -> None:
    ...
    Desktop Electron spawns its backend with MARCEL_DESKTOP=1 plus
    MARCEL_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    MARCEL_SERVE_HEADLESS=1). A shell inheriting those then running
    `marcel dashboard` would serve the desktop renderer ("Desktop IPC bridge
    ...
    (MARCEL_DESKTOP=1) keeps its dist. Headless `serve` re-sets
    MARCEL_SERVE_HEADLESS itself.
    """
    if os.environ.get("MARCEL_DESKTOP") != "1":
        if _is_electron_packaged_web_dist(os.environ.get("MARCEL_WEB_DIST", "")):
            os.environ.pop("MARCEL_WEB_DIST", None)
    if not headless_backend:
        os.environ.pop("MARCEL_SERVE_HEADLESS", None)
...
def _dashboard_prepare_runtime(args, headless_backend) -> bool:
    ...
    # the same logs directory as every other Marcel surface.
    try:
        from marcel_logging import setup_logging as _setup_logging_gui
    ...
    try:
        ...
        # #61115, #65696.
        from marcel_cli.config import apply_terminal_config_to_env
    ...
    # dashboard_auth, …).
    try:
        from marcel_cli.plugins import discover_plugins
        discover_plugins()
    except Exception as exc:
    ...
    mcp_discovery_after_bind = headless_backend and os.environ.get("MARCEL_DESKTOP") == "1"
    if not mcp_discovery_after_bind:
        try:
            from marcel_cli.mcp_startup import start_background_mcp_discovery
...
def cmd_dashboard(args):
    ...
    from marcel_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from marcel_cli import completion
...
def cmd_logs(args):
    """View and filter Marcel log files."""
    from marcel_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Marcel command console."""
    from marcel_cli.console_engine import run_console_repl
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    """
    first = _first_positional_argv()  # None = bare ``marcel`` → chat
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    on import; ``discover_plugins()`` alone leaves ``marcel photon`` failing
    ...
    not import it, so the CLI registration never happens and ``marcel photon`` fails with argparse ``invalid
    choice`` (issue #54678).
    """
    if not command_name:
        return
...
def _is_tui_chat_launch(args) -> bool:
    if getattr(args, "tui", False) or os.environ.get("MARCEL_TUI") == "1":
        return True
    # The chat path decides TUI-vs-classic via _resolve_use_tui (--cli/--tui
    # flags, TTY gate, MARCEL_TUI env, display.interface config). Bare
    # `marcel`/`marcel chat` with a TUI display config was previously missed
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo chokepoint: MARCEL_YOLO_MODE must be set before any discovery
    ...
    if not _is_tui_chat_launch(args):
        ...
        try:
            from marcel_cli.plugins import start_background_plugin_discovery
    ...
    try:
        from marcel_cli.mcp_startup import set_mcp_server_filter
    ...
    if _run_inline_mcp_discovery:
        try:  # synchronous for entrypoints without a later bounded startup path
            from marcel_cli.mcp_startup import get_mcp_server_filter
...
def _apply_safe_mode(args) -> None:
    if not getattr(args, "safe_mode", False):
        return
    os.environ["MARCEL_SAFE_MODE"] = "1"
    os.environ["MARCEL_IGNORE_USER_CONFIG"] = "1"
    os.environ["MARCEL_IGNORE_RULES"] = "1"
...
def _light_chat_parser():
    ...
    from marcel_cli._parser import build_top_level_parser
...
def _try_fast_serve_launch() -> bool:
    ...
    """
    if os.environ.get("MARCEL_DISABLE_FAST_SERVE_LAUNCH") == "1":
        return False
    ...
    try:
        from marcel_cli.config import get_container_exec_info
...
def _try_fast_chat_launch() -> bool:
    ...
    """
    if os.environ.get("MARCEL_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if not _is_termux_startup_environment():
        return False
    if os.environ.get("MARCEL_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["MARCEL_DEFER_AGENT_STARTUP"] = "1"
            os.environ["MARCEL_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["MARCEL_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `marcel --tui` is the hot path on phones and the TUI immediately execs
...
def _advertise_agent_env() -> None:
    ...
    (``marcel-agent``) — matching is exact. ``MARCEL_AGENT`` is the
    Marcel-specific marker. setdefault: never clobber an outer harness.
    ...
    harness that spawned them. Marcel running inside another agent's terminal).
    """
    os.environ.setdefault("AI_AGENT", "marcel-agent")
    os.environ.setdefault("MARCEL_AGENT", "true")
...
def _register_plugin_cli_commands(subparsers) -> None:
    ...
    try:
        ...
        from marcel_cli.plugins import discover_plugins, get_plugin_manager
...
def _cmd_sessions_lazy(args, **kwargs):
    """``marcel sessions`` handler; sessions_cmd imports only when the subcommand runs."""
    from marcel_cli.sessions_cmd import cmd_sessions

    return cmd_sessions(args, **kwargs)


def _brand_parser_help_for_marcel(parser) -> None:
    """Remove legacy product names from Marcel's public argparse help tree."""
    seen: set[int] = set()
    ...
    def _brand(value):
        if not isinstance(value, str):
            return value
        return (
            value.replace("Nous Portal", "Marcel Router")
            .replace("Marcel Agent", "Marcel")
            .replace("Marcel", "Marcel")
        )
    ...
    def _walk(current) -> None:
        ...
        for attribute in ("description", "epilog"):
            setattr(current, attribute, _brand(getattr(current, attribute, None)))
        for action in getattr(current, "_actions", []):
            action.help = _brand(getattr(action, "help", None))
            action.metavar = _brand(getattr(action, "metavar", None))
            for choice_action in getattr(action, "_choices_actions", []):
                choice_action.help = _brand(getattr(choice_action, "help", None))
            for child in getattr(action, "choices", {}).values() if isinstance(
...
def _build_cli_parser():
    """Build the full ``marcel`` argparse tree -> ``(parser, subparsers)``.

    Registration ORDER is the ``marcel --help`` order; keep it stable. Groups
    live in ``marcel_cli/subcommands/<group>.py`` with handlers injected so
    those modules never import main.
    """
    from marcel_cli._parser import _active_entrypoint, build_top_level_parser

    parser, subparsers, chat_parser = build_top_level_parser()
    marcel_entrypoint = _active_entrypoint() == "marcel"
    chat_parser.set_defaults(func=cmd_chat)
    ...
    # OUTBOUND egress firewall; ``marcel proxy`` (gateway group) is the INBOUND one.
    build_egress_parser(subparsers)
    build_migrate_parser(subparsers)
    build_migrate_legacy_parser(subparsers)
    ...
    from marcel_cli.send_cmd import register_send_subparser
    ...
    from marcel_cli.subcommands.peer import build_peer_parser
    build_peer_parser(subparsers)

    if not marcel_entrypoint:
        from marcel_cli.portal_cli import add_parser as _add_portal_parser
        _add_portal_parser(subparsers)

    from marcel_cli.kanban import build_parser as _build_kanban_parser
    _build_kanban_parser(subparsers).set_defaults(func=cmd_kanban)

    from marcel_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # "desktop" is canonical (Marcel-Setup.exe tells users to run it, so it
    ...
    if marcel_entrypoint:
        _brand_parser_help_for_marcel(parser)
...
def _parse_cli_args(parser, subparsers, argv):
    ...
    routing; if that fails (``marcel -c model`` — 'model' is the session name)
...
def main():
    """Main entry point for marcel CLI."""
    _set_process_title()
    _advertise_agent_env()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from marcel_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``marcel.exe.old.*`` quarantine files from previous Windows
    # updates (see ``_quarantine_running_marcel_exe``). No-op elsewhere.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass
    ...
    # Self-heal a venv left half-built by an interrupted ``marcel update``, and
    ...
    # substring match is deliberately loose: over-matching (``marcel skills
    # install update``) only defers recovery one launch; under-matching
    # (``marcel -p work update``) would race. Never raises.
    # See #95294.
    if "update" not in sys.argv[1:]:
        try:
            ...
            pass
        try:
            from marcel_cli.update_cmd_fleet import _warn_pending_fleet_restart_on_startup
...
_PLUGIN_COMPAT_LAZY = {
    'line_input': ('marcel_cli.cli_output', 'line_input'),
}
