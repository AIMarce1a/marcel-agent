import type { RunExternalProcess } from '@marcel/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`marcel ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching marcel: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`marcel ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
