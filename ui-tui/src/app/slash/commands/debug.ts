...
import { terminalBackgroundHex } from '@marcel/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $MARCEL_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see MARCEL_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['MARCEL_TUI_BACKGROUND', process.env.MARCEL_TUI_BACKGROUND ?? '(unset)'],
            ['MARCEL_TUI_THEME', process.env.MARCEL_TUI_THEME ?? '(unset)'],
