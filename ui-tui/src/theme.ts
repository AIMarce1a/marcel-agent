import type { SkinBranding, SkinColors } from '@marcel/shared/skin'
...
// Mirrors @marcel/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @marcel/ink is bundled separately from packages/.
...
export const DARK_SEEDS: ThemeSeeds = {
  accent: '#FFBF00',
  // The classic Marcel navy surfaces are IDENTITY, not derivation drift —
...
// The classic light-mode Marcel look was never hand-authored: for years the
 ...
 *  answer when known (cached in MARCEL_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.MARCEL_TUI_BACKGROUND ?? '').trim()
...
// Terminal. Explicit MARCEL_TUI_THEME / COLORFGBG signals above still win,
...
// `MARCEL_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `MARCEL_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `MARCEL_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `MARCEL_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Marcel palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.MARCEL_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.MARCEL_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.MARCEL_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into MARCEL_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into MARCEL_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (Marcel navy
  ...
  // the derived value already carries the identity seeds (e.g. Marcel navy).
  const activeRow =
