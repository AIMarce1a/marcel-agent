import type { InputEvent, Key } from '@marcel/ink'
import * as Ink from '@marcel/ink'
 ...
 * but that accepts `key.meta` on macOS — and marcel-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.MARCEL_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which marcel-ink decodes as meta+'d'; without this branch it
