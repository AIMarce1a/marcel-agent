...
const ENV_KEYS = ['MARCEL_AUTO_HEAPDUMP', 'MARCEL_HEAPDUMP_DIR', 'MARCEL_HEAPDUMP_MAX_BYTES'] as const
...
describe('performHeapDump auto opt-in gate (#21767)', () => {
  ...
  beforeEach(() => {
    ...
    dir = mkdtempSync(join(tmpdir(), 'marcel-heapdump-test-'))
    process.env.MARCEL_HEAPDUMP_DIR = dir
  })
  ...
  it('writes diagnostics only for auto-high without MARCEL_AUTO_HEAPDUMP', async () => {
  ...
  it('writes diagnostics only for auto-critical without MARCEL_AUTO_HEAPDUMP', async () => {
  ...
  it('writes both diagnostics and snapshot for auto-high when MARCEL_AUTO_HEAPDUMP=1', async () => {
    process.env.MARCEL_AUTO_HEAPDUMP = '1'
  ...
  it('accepts truthy spellings (true|yes|on, case-insensitive) as opt-in', async () => {
    for (const value of ['true', 'YES', 'On']) {
      process.env.MARCEL_AUTO_HEAPDUMP = value
      const result = await performHeapDump('auto-high')
  ...
  it('writes both for manual triggers regardless of MARCEL_AUTO_HEAPDUMP', async () => {
...
describe('heapdump retention guard (#21767)', () => {
  ...
  beforeEach(() => {
    savedDir = process.env.MARCEL_HEAPDUMP_DIR
    savedMax = process.env.MARCEL_HEAPDUMP_MAX_BYTES
    delete process.env.MARCEL_AUTO_HEAPDUMP
    dir = mkdtempSync(join(tmpdir(), 'marcel-heapdump-prune-'))
    process.env.MARCEL_HEAPDUMP_DIR = dir
  })

  afterEach(() => {
    if (savedDir === undefined) {
      delete process.env.MARCEL_HEAPDUMP_DIR
    } else {
      process.env.MARCEL_HEAPDUMP_DIR = savedDir
    }

    if (savedMax === undefined) {
      delete process.env.MARCEL_HEAPDUMP_MAX_BYTES
    } else {
      process.env.MARCEL_HEAPDUMP_MAX_BYTES = savedMax
    }
  ...
  it('evicts oldest files when total bytes exceed the cap, retaining the newest', async () => {
    ...
    process.env.MARCEL_HEAPDUMP_MAX_BYTES = String(2 * 1024)
