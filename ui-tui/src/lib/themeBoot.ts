 ...
 * `marcel-boot-background` / `marcel-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports MARCEL_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.marcel matches get_marcel_home()'s default.
const bootFilePath = () => join(process.env.MARCEL_HOME ?? join(homedir(), '.marcel'), 'tui-theme-boot.json')

// Never touch the user's real ~/.marcel from test runs (the TS suite has no
// MARCEL_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into MARCEL_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.MARCEL_TUI_THEME || env.MARCEL_TUI_LIGHT) {
    return result
  }
  ...
  if (boot.mode) {
    env.MARCEL_TUI_THEME = boot.mode
  ...
  if (
    ...
    !env.MARCEL_TUI_BACKGROUND
  ) {
    env.MARCEL_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.MARCEL_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.MARCEL_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into MARCEL_TUI_THEME.
