...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.MARCEL_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Marcel on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via MARCEL_TUI_TRUECOLOR=1.
