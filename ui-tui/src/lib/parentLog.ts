...
const logDir = join(process.env.MARCEL_HOME?.trim() || join(homedir(), '.marcel'), 'logs')
...
// a real ~/.marcel (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('marcel-tui: parent lifecycle log unavailable\n')
    }
  }
}
