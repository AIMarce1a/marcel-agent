...
import { Box, Text } from '@marcel/ink'
...
/**
 * User widget apps — Marcel authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$MARCEL_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.marcel/plugins/`: files under MARCEL_HOME execute
...
const widgetsDir = () => join(process.env.MARCEL_HOME?.trim() || join(homedir(), '.marcel'), 'tui-widgets')
 ...
 *  every change, so a widget Marcel writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Marcel) ever writes must hot-load too — a 10s poll here read
