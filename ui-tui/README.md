...
React + Ink terminal UI for Marcel. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `MARCEL_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `marcel setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.marcel/.marcel_history` or under `MARCEL_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `marcel setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/marcel-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external marcel setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.marcel/tui-parent.log
    ...
    types/
      marcel-ink.d.ts            type declarations for @marcel/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent MarcelCLI subprocess for slash commands
```
