...
Marcel Agent (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``MARCEL_NIX_BUILD=1`` on the
Marcel package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("MARCEL_NIX_BUILD") == "1"

_BLOCK_MESSAGE = (
    "Building wheels or sdists for marcel-agent is not supported.\n"
    "Marcel is distributed via the shell installer, Docker image, or Nix.\n"
    ...
    "the Marcel Nix derivation sets MARCEL_NIX_BUILD=1. If it does, file a bug."
)
...
# Root single-file modules (``run_agent``, ``marcel_state``, ``toolsets``...)
