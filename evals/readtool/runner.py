"""Run the read-tool eval through the REAL Marcel AIAgent.

For each task: fresh temp MARCEL_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    marcel_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".marcel"
    ...
    try:
        ...
        shutil.rmtree(marcel_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.marcel/.env; set +a  — then relaunch."
        )
