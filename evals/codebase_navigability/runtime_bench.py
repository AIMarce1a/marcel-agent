...
Each probe runs in a FRESH subprocess with an isolated MARCEL_HOME so nothing is cached across reps.
...
ENV = {**os.environ, "MARCEL_HOME": HOME, "PYTHONPATH": TREE, "PYTHONDONTWRITEBYTECODE": "0", "OPENROUTER_API_KEY": "sk-bench-placeholder",
       "MARCEL_SKIP_UPDATE_CHECK": "1", "NO_COLOR": "1", "TERM": "dumb", "COLUMNS": "120"}
for k in list(ENV):
    if k.startswith(("MARCEL_SESSION", "MARCEL_PROFILE")): ENV.pop(k)
...
IMPORT_TARGETS = ["run_agent", "cli", "marcel_cli.main", "gateway.run", "tools.registry", "marcel_state", "tui_gateway.server", "marcel_cli.web_server", "model_tools", "agent.prompt_builder"]
probe = r'''
...
tree=os.getcwd(); foreign=[m for m,mod in list(sys.modules.items()) if getattr(mod,"__file__",None) and "marcel-agent" in mod.__file__ and not mod.__file__.startswith(tree) and "site-packages" not in mod.__file__]
...
# 2. CLI end-to-end startup: `marcel --version`, `marcel --help`, `marcel doctor --help`, `marcel config get model` (no network)
CLI = {"version": ["marcel_cli/main.py", "--version"], "help": ["marcel_cli/main.py", "--help"], "config_get": ["marcel_cli/main.py", "config", "get", "model"], "tools_list": ["marcel_cli/main.py", "tools", "--help"], "skills_help": ["marcel_cli/main.py", "skills", "--help"]}
cli = {}
...
try:
    ...
    T("resolve_toolset_marcel_default_ms", lambda: resolve_toolset("marcel-default"), 200)
...
try:
    import marcel_state, tempfile, uuid
    from pathlib import Path
    db=marcel_state.SessionDB(Path(tempfile.mkdtemp())/"s.db") if hasattr(marcel_state,"SessionDB") else None
    if db:
        sid=str(uuid.uuid4())
