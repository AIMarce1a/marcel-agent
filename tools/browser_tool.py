...
from marcel_cli.config import DEFAULT_CONFIG, cfg_get
...
# dependency could read every Marcel secret from process.env.
...
def _build_browser_env() -> dict:
    ...
    env = marcel_subprocess_env(inherit_credentials=False)
...
# than a concrete path (also compared in marcel_cli/tools_config.py and doctor.py).
...
_REAL_PROFILE_SESSION = "marcel-real-profile"
...
def _socket_safe_tmpdir() -> str:
    """Short temp dir for Unix sockets: macOS ``TMPDIR`` + ``agent-browser-marcel_…``
...
# Owner Marcel home per session: the janitor is one process-global thread, so each
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        if not cfg_get(read_raw_config(), "browser", "record_sessions", default=False):
            return
        recordings_dir = marcel_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    screenshots_dir = get_marcel_dir("cache/screenshots", "browser_screenshots")
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_marcel_home_override': ('marcel_constants', 'get_marcel_home_override'),
    'marcel_home_key': ('marcel_constants', 'marcel_home_key'),
    ...
    'node_tool_runnable': ('marcel_constants', 'node_tool_runnable'),
