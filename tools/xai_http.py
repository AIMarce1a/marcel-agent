...
def has_xai_credentials() -> bool:
    ...
    try:
        ...
        auth_path = get_marcel_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.marcel/.env`` first, then ``os.environ``.

    Wraps :func:`marcel_cli.config.get_env_value` so tests can patch ``tools.xai_http.get_env_value``.
    """
    try:
        from marcel_cli.config import get_env_value as _marcel_get_env_value
    except ImportError:
        return os.environ.get(name, default)
    value = _marcel_get_env_value(name)
    return value if value is not None else default


def marcel_xai_user_agent() -> str:
    """Return a stable Marcel-specific User-Agent for xAI HTTP calls."""
    try:
        from marcel_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Marcel-Agent/{__version__}"


def marcel_xai_default_headers() -> Dict[str, str]:
    """Default headers for OpenAI-SDK and raw HTTP clients talking to xAI (replaces the SDK User-Agent)."""
    return {"User-Agent": marcel_xai_user_agent()}
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Marcel home, then mark it seen."""
    notice = xai_storage_notice_text(section_name)
    if not notice:
        return None
    try:
        from marcel_constants import get_marcel_home
        marker_dir = get_marcel_home() / "state"
...
def _xai_base_url_override() -> str:
    """``MARCEL_XAI_BASE_URL`` then ``XAI_BASE_URL``, stripped; '' when unset."""
    return str(get_env_value("MARCEL_XAI_BASE_URL") or get_env_value("XAI_BASE_URL") or "").strip().rstrip("/")
...
def resolve_xai_http_credentials(
    *, force_refresh: bool = False, api_key_hint: Optional[str] = None, prefer_api_key: bool = False,
    ...
    Default order: Marcel-managed xAI OAuth, then ``XAI_API_KEY`` (via ``get_env_value`` so
    ``~/.marcel/.env`` keys count). ``prefer_api_key=True`` inverts that for API-metered
    endpoints where the subscription OAuth bearer authorizes but misbehaves (x_search answers
    without citations, TTS 403s). Both branches honor ``MARCEL_XAI_BASE_URL``/``XAI_BASE_URL``
    ...
    Prefers Marcel-managed xAI OAuth credentials when available, then falls back to ``XAI_API_KEY`` resolved
    via ``marcel_cli.config.get_env_value`` so keys stored in ``~/.marcel/.env`` (the standard Marcel
    ...
    scoping is identical to the fallback branch, and the base URL honors ``MARCEL_XAI_BASE_URL`` /
