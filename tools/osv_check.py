...
# ladders, parked-server self-probes and repeated `marcel mcp test` runs re-run the preflight
...
# The cache is also persisted under the Marcel home so separate processes and gateway
...
def _disk_cache_path() -> Optional[Path]:
    ...
    Uses ``marcel_constants.get_marcel_home()`` so the cache follows the
    active profile and is isolated across Marcel homes. The cache directory
    is created on demand. Returns ``None`` when Marcel home cannot be
...
def _query_osv(package: str, ecosystem: str, version: Optional[str] = None) -> list:
    ...
    req = urllib.request.Request(
        ...
        headers={"Content-Type": "application/json", "User-Agent": "marcel-agent-osv-check/1.0"},
        method="POST")
