...
# Sensitive write targets, matched via ~ / $HOME / $MARCEL_HOME spellings. The resolved absolute
# home is folded into these forms at detection time by _normalize_command_for_detection(), so no
# import-time path snapshot (stale once MARCEL_HOME is set after import) lives in the patterns.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_MARCEL_ENV_PATH = (
    r'(?:~\/\.marcel/|(?:\$home|\$\{home\})/\.marcel/|(?:\$marcel_home|\$\{marcel_home\})/)' r'\.env\b'
)
# ~/.marcel/config.yaml IS the security policy (approvals.mode, yolo, allowlist) and the config cache is mtime-keyed,
# so a write takes effect mid-session. Terminal-side coverage (sed -i, tee, >, cp) pairs the file_tools deny.
_MARCEL_CONFIG_PATH = (
    r'(?:~\/\.marcel/|(?:\$home|\$\{home\})/\.marcel/|(?:\$marcel_home|\$\{marcel_home\})/)' r'config\.yaml\b'
)
...
_SENSITIVE_WRITE_TARGET = (
    rf'(?:{_SYSTEM_CONFIG_PATH}|/dev/sd|{_SSH_SENSITIVE_PATH}|{_MARCEL_ENV_PATH}|{_MARCEL_CONFIG_PATH}|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]marcel[^\n]*\.env\b', "access to Marcel secrets (Windows path)"),
    # ── end of Windows tier
    ...
    # between `marcel` and `gateway` (`marcel -p ade gateway restart`) are allowed so a profile flag can't slip past.
    (r'\bmarcel\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart marcel gateway (kills running agents)"),
    (r'\bmarcel\s+update\b', "marcel update (restarts gateway, kills running agents)"),
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart marcel-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart marcel-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(marcel|gateway|cli\.py)\b', "kill marcel/gateway process (self-termination)"),
    ...
    # launchctl-driven gateway stop/restart on macOS (label `ai.marcel.gateway`). Two independent lookaheads, NOT a
    # sequential match: a for-loop building the label from a list defined EARLIER (`for item in 'ai.marcel...'; do
    # launchctl bootout "$label"`) never has "marcel" after the verb, and that slipped past and restarted 4 gateways
    # with zero approval. Erring broad is correct for an approval gate: an extra prompt is cheap.
    (r'(?=[\s\S]*\blaunchctl\s+(?:stop|kickstart|bootout|unload|kill|disable|remove)\b)(?=[\s\S]*\b(?:marcel|ai\.marcel)\b)', "stop/restart marcel launchd service (kills running agents)"),
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a credential/SSH/shell-rc/Marcel file (key implant, login-time
    ...
    # ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files, ~/.marcel/config.yaml/.env), but cp/mv/install was
    ...
    # sed -i on Marcel config/.env bypasses the redirection/tee rules; pairs the file_tools
    # write_file/patch deny so the terminal side is not an open door.
    # In-place edit of a Marcel-managed security file (~/.marcel/config.yaml or .env). sed -i bypasses the
    # redirection/tee patterns above because it mutates the file directly. See #14639.
    (rf'\bsed\s+-[^\s]*i.*(?:{_MARCEL_CONFIG_PATH}|{_MARCEL_ENV_PATH})', "in-place edit of Marcel config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_MARCEL_CONFIG_PATH}|{_MARCEL_ENV_PATH})', "in-place edit of Marcel config/env (long flag)"),
    ...
    # coverage from #14639.
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_MARCEL_CONFIG_PATH}|{_MARCEL_ENV_PATH})', "in-place edit of Marcel config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    command = re.sub(r'\\\r?\n', '', command)
    # Fold absolute user/Marcel home prefixes to ~/ and ~/.marcel/ so the static patterns catch /home/alice/.bashrc
    # and C:\Users\alice\.bashrc. Resolved at detection time (not import time) so it tracks HOME/MARCEL_HOME set
    # later. MUST run before the backslash strip (which would dissolve C:\Users\alice to C:Usersalice). Marcel home
    # first: on Windows it nests under the user home, and folding the user home first would eat the prefix it needs.
    command = _rewrite_resolved_marcel_home(command)
...
def _rewrite_resolved_marcel_home(command: str) -> str:
    """Resolved MARCEL_HOME (and its realpath) -> ``~/.marcel/`` so the _MARCEL_CONFIG_PATH /
    _MARCEL_ENV_PATH rules match Docker/gateway deployments that spell the absolute path."""
    try:
        ...
        return command
    return _fold_home_prefixes(command, paths, "~/.marcel")
...
_GATEWAY_LIFECYCLE_SPLICE_DESCRIPTION = "stop/restart marcel gateway via shell-spliced verb (kills running agents)"
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Marcel ad-hoc temp script."""
    try:
    ...
    return (
        ...
        and re.fullmatch(r"marcel-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
    )
...
def _is_shell_token_spliced_gateway_lifecycle(command: str) -> bool:
    ...
    verb is an ARGUMENT, so ``launchctl kick"start" -k gui/501/ai.marcel.gateway`` auto-approved.
    Delegates to ``cron.lifecycle_guard`` (shlex-tokenized, anchored on a marcel-gateway
