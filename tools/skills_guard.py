...
_MARCEL_CONFIG_FILES = r'\.marcel/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.marcel/\.env|\~/\.marcel/\.env',
     "marcel_env_access", "critical", "exfiltration", "directly references Marcel secrets file"),
    ...
    # quarantine only acts on "dangerous") but high for Marcel/other config (setup docs routinely say
    ...
    (_prose_modify_re(_MARCEL_CONFIG_FILES),
     "marcel_config_mod", "high", "persistence", "modification language aimed at Marcel configuration files (verify intent)"),
    (_shell_write_re(_MARCEL_CONFIG_FILES),
     "marcel_config_mod_shell", "critical", "persistence", "shell write (redirect/sed -i/tee/cp/mv) targeting Marcel configuration files"),
    (r'\.marcel/config\.yaml|\.marcel/SOUL\.md',
     "marcel_config_ref", "low", "persistence", "references Marcel configuration files (informational; only modification intent is scored)"),
...
# `.skillignore` is Marcel-native; `.clawhubignore` is honored for skills published through ClawHub.
