...
def _marcel_tags(fm: dict) -> Any:
    """``metadata.marcel.tags`` from parsed frontmatter, or ``[]`` (unvalidated type)."""
    metadata = fm.get("metadata", {})
    marcel_meta = metadata.get("marcel", {}) if isinstance(metadata, dict) else None
    return marcel_meta.get("tags", []) if isinstance(marcel_meta, dict) else []
