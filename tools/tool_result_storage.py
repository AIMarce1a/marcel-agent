...
is ALWAYS host-side ``$MARCEL_HOME/cache/spillover/{id}.txt`` (works for sessions that never
...
STORAGE_DIR = "/tmp/marcel-results"
...
def get_spillover_dir():
    """Return $MARCEL_HOME/cache/spillover as a Path (not created)."""
    from marcel_constants import get_marcel_home
    return get_marcel_home() / SPILLOVER_SUBDIR
...
def _write_to_spillover(content: str, filename: str):
    """Write host-side to $MARCEL_HOME/cache/spillover; returns path str or None."""
    try:
...
def _resolve_storage_dir(env) -> str:
    ...
    return f"{temp_dir.rstrip('/') or '/'}/marcel-results" if temp_dir else STORAGE_DIR
...
HEREDOC_MARKER = "MARCEL_PERSIST_EOF"
# ---- END PLUGIN-COMPAT ----
