#!/usr/bin/env python3
"""Image generation via FAL.ai (model picked in ``marcel tools``, persisted to ``image_gen.model``).
...
def _resolve_managed_fal_gateway():
    """Managed gateway config for the stored `marcel tools` selection, or ``None`` for direct FAL.
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                ...
                f"  • Pick a different model via `marcel tools` → Image Generation."
...
_CACHE_BASE_BY_BACKEND = {"docker": "/root/.marcel", "singularity": "/root/.marcel",
                          "modal": "/root/.marcel", "ssh": "~/.marcel"}
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.marcel"
...
def _prepare_fal_request(model_id, meta, prompt, aspect_ratio, seed, overrides, source_images):
    ...
    if source_images and not edit_endpoint:
        raise ValueError(
            ...
            f"via `marcel tools` → Image Generation.")
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed:
        lines.append("  2. Sign in to a Nous account that has the managed FAL gateway enabled "
                     "(`marcel setup`)")
    lines.append("  3. Configure a different image_gen provider via `marcel tools` → Image Generation "
                 "(run `marcel plugins list` to see installed backends)")
    return "\n".join(lines)
...
def check_image_generation_requirements() -> bool:
    ...
    try:
        ...
        if provider and provider.is_available():
            return True
        # The configured provider is a default, not a lock. Keep the tool available when another
        # connected backend can satisfy a per-call override.
        from agent.image_gen_registry import list_providers
        return any(candidate.is_available() for candidate in list_providers())
    except Exception:
        return False
...
def _dispatch_to_plugin_provider(
    ...
    explicit_provider = (
        ...
        and provider_name.strip().lower() not in {"", "auto"}
    )
    configured = (
        provider_name.strip().lower()
        if explicit_provider
        else _plugin_provider_name()
    )
    if configured is None:
        return None
    try:
    ...
    if provider is None:
        return _provider_error(
            ...
            f"Run `marcel plugins list` to see available image gen backends.", "provider_not_registered")
    ...
    try:
        _add_provider_kwargs(
            ...
            model=(model.strip() if isinstance(model, str) and model.strip()
                   else None if explicit_provider else _read_configured_image_model()))
        ...
        if is_type_error and ("image_url" in kwargs or "reference_image_urls" in kwargs):
            ...
            return _provider_error(
                ...
                f"text-to-image, or pick a backend that supports editing via `marcel tools` → "
...
def _handle_image_generate(args, **kw):
    ...
    # Per-call overrides are deliberately ephemeral: they never mutate the setup default.
    if isinstance(requested_provider, str) and requested_provider.strip().lower() not in {"", "auto"}:
        raw = _dispatch_to_plugin_provider(
            prompt, aspect_ratio, **sources, provider_name=requested_provider,
            model=requested_model)
    else:
        raw = _dispatch_to_plugin_provider(
            prompt, aspect_ratio, **sources, model=requested_model)
        if raw is None:
            raw = _maybe_route_managed_krea(prompt, aspect_ratio, **sources)
        if raw is None:
