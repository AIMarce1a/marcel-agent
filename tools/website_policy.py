...
Loads a user-managed website blocklist (``security.website_blocklist`` in ~/.marcel/config.yaml plus
...
def load_website_blocklist(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    default_path = get_marcel_home() / "config.yaml"
    ...
    for shared_file in shared_files:
        ...
        path = path if path.is_absolute() else (get_marcel_home() / path).resolve()
