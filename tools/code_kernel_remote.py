...
"""Auto-generated Marcel REMOTE session-kernel runner (file cell protocol)."""
import contextlib
import io
import json
import os
...
KDIR = os.environ["MARCEL_KERNEL_DIR"]
...
class RemoteKernel:
    ...
    # poll loop (same guard as tools.code_kernel, marcel-agent#101861).
    attached: int = 0
...
def _spawn_remote_kernel(env, env_type: str, owner: str, task_env_id: str,
    ...
    from tools.code_execution_tool import (
        MAX_STDOUT_BYTES, _ship_file_to_remote, _env_temp_dir, generate_marcel_tools_module,
    )
    kernel_dir = f"{_env_temp_dir(env)}/marcel_rkernel_{uuid.uuid4().hex[:12]}"
    ...
    try:
        ...
        _ship_file_to_remote(env, f"{kernel_dir}/marcel_tools.py",
                             generate_marcel_tools_module(list(sandbox_tools), transport="file"))
        env_prefix = (f"MARCEL_KERNEL_DIR={q_dir} MARCEL_RPC_DIR={shlex.quote(kernel_dir + '/rpc')} "
                      f"MARCEL_RPC_TOKEN={shlex.quote(rpc_token)} PYTHONDONTWRITEBYTECODE=1 PYTHONPATH={q_dir}")
