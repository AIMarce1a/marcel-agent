...
_FOREGROUND_GUIDANCE = (
    (
        ...
        "notify_on_complete=true) so Marcel tracks the process, then run readiness "
...
def gateway_lifecycle_block(
    *,
    ...
    ``systemctl``/``launchctl``/``marcel gateway restart|stop|uninstall``
    targeting marcel-gateway would SIGTERM the gateway — and this very
    ...
    SUPERVISED-gateway probe, not the raw ``_MARCEL_GATEWAY`` marker: that
    marker leaks into every process that merely imports gateway.run (marcel
    serve, CLI, web server), which must still be able to restart the gateway;
    an unsupervised foreground ``marcel gateway run`` has no KeepAlive to turn
    ...
    if lifecycle_scan_root_within_budget(command) and contains_launchctl_submit_command(command):
        return _blocked_json(
            ...
            "Use Marcel cron for one-shot delayed work, or install an "
    ...
    if contains_gateway_lifecycle_command_or_referenced_script(
        ...
        return _blocked_json(
            ...
            "to child processes). Run `marcel gateway restart` from a "
