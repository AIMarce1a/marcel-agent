...
refresh; this module supplies ``MarcelTokenStorage`` (on-disk persistence), the localhost callback
listener and ``build_oauth_auth()`` (legacy entry point). client_id is Marcel' Client ID Metadata
...
from marcel_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__marcel_user_skipped__"


def _get_token_dir(marcel_home: str | Path | None = None) -> Path:
    """``MARCEL_HOME/mcp-tokens/`` — per-profile token directory."""
    from marcel_constants import get_marcel_home

    return Path(marcel_home if marcel_home is not None else get_marcel_home()) / "mcp-tokens"
...
def _cached_client_info(storage: "MarcelTokenStorage | None") -> dict | None:
...
def _cached_redirect(storage: "MarcelTokenStorage | None") -> "tuple[str | None, int | None]":
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    ``lead`` is the boundary-specific first sentence; this helper appends the shared, actionable ``marcel
    mcp login`` next-step so the guidance wording lives in one place across every non-interactive OAuth
    boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} Run `marcel mcp login <server>` interactively to (re)authorize, then restart or reload the gateway."
        )
...
def _write_json(path: Path, data: dict) -> None:
    ...
    # secure_parent_dir refuses to chmod /, top-level dirs, or the marcel-agent install tree (#25821,
    # #93050).
    ...
    # marcel-agent install tree (#25821, #93050).
...
class MarcelTokenStorage:
    """Persist OAuth state as ``MARCEL_HOME/mcp-tokens/<server_name>`` + ``.json`` (tokens),
    ``.client.json`` (client info), ``.meta.json`` (server metadata), ``.cimd-off`` (CIMD refused)."""

    def __init__(self, server_name: str, *, marcel_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._marcel_home = Path(marcel_home) if marcel_home is not None else None

    def _path(self, suffix: str) -> Path:
        return _get_token_dir(self._marcel_home) / f"{self._server_name}{suffix}"
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._marcel_home)
...
def _make_callback_handler() -> tuple[type, dict]:
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = ("<h2>Authorization Successful</h2><p>You can close this tab and return to Marcel.</p>" if parsed["code"]
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `marcel mcp login <server>` later to authenticate, "
...
def _callback_outcome(result: dict, cimd_url: str | None):
    ...
    if result["auth_code"] is None:
        hint = (
            ...
            f"server rejected Marcel' Client ID Metadata Document ({cimd_url}); set ``cimd: false`` under that "
...
MarcelOAuthClientProvider: Any = None


def remove_oauth_tokens(server_name: str, *, marcel_home: str | Path | None = None) -> None:
    """Delete stored OAuth tokens and client info for a server."""
    MarcelTokenStorage(server_name, marcel_home=marcel_home).remove()
    logger.info("OAuth tokens removed for '%s'", server_name)


# CIMD (OAuth Client ID Metadata Documents): the client_id IS an HTTPS URL the server fetches for our
# name/logo/redirect URIs, replacing per-install DCR. The SDK does the protocol; Marcel only decides
...
def _server_declined_cimd(storage: "MarcelTokenStorage | None") -> bool:
    """True when cached metadata shows this server doesn't advertise CIMD. The SDK decides CIMD vs DCR
    in its 401 branch — after Marcel must fix the redirect URI — so cached metadata closes the gap;
...
def _maybe_use_cimd(cfg: dict, storage: "MarcelTokenStorage | None" = None) -> "tuple[str, int] | None":
...
def _configure_callback_port(cfg: dict, storage: "MarcelTokenStorage | None" = None) -> int:
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    metadata_kwargs: dict[str, Any] = {
        "client_name": cfg.get("client_name", "Marcel Agent"),
...
def _invalidate_tokens_on_client_change(
    storage: "MarcelTokenStorage", new_client_id: str, new_client_secret: str | None) -> None:
    ...
    if removed:
        logger.warning(
            ...
            "the previous client. Re-authorize with: marcel mcp login %s",
            storage._server_name, old_client_id, new_client_id, storage._server_name)


def _maybe_preregister_client(storage: "MarcelTokenStorage", cfg: dict, client_metadata: "OAuthClientMetadata") -> None:
...
def humanize_oauth_registration_error(
    ...
    when the user overrode it or an older Marcel is running."""
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(server_name: str, server_url: str, oauth_config: dict | None = None) -> "OAuthClientProvider | None":
    ...
    global MarcelOAuthClientProvider
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"requires browser authorization. Run `marcel mcp login {server_name}` interactively first to complete "
            "initial authorization, then cached tokens will be reused.")
    kwargs = build_provider_kwargs(cfg, storage, ssh_proxy_hint=True)
    if MarcelOAuthClientProvider is None:
        from tools.mcp_oauth_provider import MarcelProviderMixin

        MarcelOAuthClientProvider = type("MarcelOAuthClientProvider", (MarcelProviderMixin, _sdk_class("OAuthClientProvider")), {
            "__doc__": "SDK provider plus Marcel' token-endpoint fixes (see ``MarcelProviderMixin``).",
            "__module__": __name__, "_marcel_logger": logger})
    return MarcelOAuthClientProvider(server_url=server_url, **kwargs)
