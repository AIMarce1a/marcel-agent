"""Wake-word ("Hey Marcel") detection — hands-free session trigger.
...
# Minimum gap between two wake fires, so one "hey marcel" can't retrigger across
...
_DEFAULTS: Dict[str, Any] = {
    ...
    "provider": "openwakeword", "phrase": "hey marcel", "sensitivity": 0.6,
    "confirmation_frames": _DEFAULT_CONFIRMATION_FRAMES, "start_new_session": True,
}

# Bundled "hey marcel" model (tools/wakewords/) — the default; alias names resolve
# to it, not to an openWakeWord built-in.
_BUNDLED_MODEL_NAME = "hey_marcel"
_BUNDLED_MODEL_ALIASES = frozenset({"", "hey_marcel", "hey marcel", "marcel"})


def _bundled_wakeword_path(framework: str = "onnx") -> str:
    """Path to the shipped hey_marcel model (.onnx/.tflite) for ``framework``."""
...
def wake_phrase(cfg: Optional[Dict[str, Any]] = None) -> str:
    ...
    return str(_get(cfg, "phrase")) or "hey marcel"
...
def enrolled_profile_phrases() -> Dict[str, str]:
    ...
    with suppress(Exception):
        ...
        from marcel_cli.profiles import get_profile_dir, list_profiles
...
def silent_audio_hint(details: Dict[str, Any]) -> str:
    ...
    if sys.platform == "darwin":
        return ("Microphone delivers only silence. Grant the Marcel backend "
...
def check_wake_word_requirements(cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    ladder = (
        ...
        (bool(missing), lambda: (f"Wake word needs {missing} configured — run `marcel tools` "
...
# ── Process-wide singleton (mirrors marcel_cli.voice's continuous API) ──
...
def _lock_path() -> Path:
    ...
    return get_default_marcel_root() / "runtime" / "wake-word.lock"
