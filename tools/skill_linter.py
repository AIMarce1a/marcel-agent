...
def _check_frontmatter(frontmatter: Dict[str, Any], skill_dir: Optional[Path]) -> Iterator[LintFinding]:
    ...
    marcel_meta = meta.get("marcel") if isinstance(meta, dict) else None
    if not isinstance(marcel_meta, dict):
        yield _warn("missing-metadata", "frontmatter is missing metadata.marcel.{tags, related_skills}.")
    elif "tags" not in marcel_meta:
        yield _warn("missing-metadata", "metadata.marcel.tags is missing.")
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("marcel", "agent", "marcel agent") and (
        author != "Marcel Agent"):
        yield _warn("author-caps", f"author '{author}' should be 'Marcel Agent' (proper caps) "
