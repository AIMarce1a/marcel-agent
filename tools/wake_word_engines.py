...
class _OpenWakeWordEngine(_Engine):
    ...
    def _build(self, cfg, sub, ww) -> None:
        ...
        # Default (or explicit "hey_marcel") → the bundled model; built-in names / paths as-is.
...
# GigaSpeech), downloaded once under MARCEL_HOME. Keywords are tokenized at RUNTIME.
...
def _sherpa_model_root() -> Path:
    ...
    return get_marcel_home() / "cache" / "wakewords"
...
class _SherpaKwsEngine(_Engine):
    ...
    def _build(self, cfg, sub, ww) -> None:
        ...
        phrase = str(ww._get(cfg, "phrase") or "hey marcel").strip()
        ...
        kw = tempfile.NamedTemporaryFile(mode="w", suffix=".txt", prefix="marcel-kws-", delete=False,
                                         encoding="utf-8")
