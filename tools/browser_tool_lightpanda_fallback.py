...
def lightpanda_engine_status() -> Tuple[bool, str]:
    ...
    precedence with config-only gates (no network I/O) for ``/browser status`` / ``marcel doctor``.
    """
    _bt = _origin()
    ...
    return True, "Browser Use mode: Marcel spawns `lightpanda serve` per session"
