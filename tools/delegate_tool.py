...
from tools.delegate_worker_registry import WorkerConfigError, resolve_worker
...
def delegate_task(
    ...
    cfg = _load_config()
    # Named workers are declarative, credential-free profiles.  Their provider,
    # model and currently-supported execution limits override delegation-wide
    # defaults; resolving never puts a secret into tool output or prompts.
    worker_cfg = None
    if worker is not None:
        try:
            worker_cfg = resolve_worker(worker, cfg)
        except WorkerConfigError as exc:
            return tool_error(str(exc))
    effective_cfg = dict(cfg)
    if worker_cfg:
        effective_cfg.update(worker_cfg)
    default_max_iter = effective_cfg.get("max_iterations", DEFAULT_MAX_ITERATIONS)
    ...
    try:
        creds = _resolve_delegation_credentials(credentials_cfg if credentials_cfg else effective_cfg, parent_agent)
    except ValueError as exc:
...
DELEGATE_TASK_SCHEMA = {
    ...
    # Lazy (not at import) so cli.CLI_CONFIG isn't forced to load before the test conftest redirects MARCEL_HOME.
    "description": (
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "worker": _p(
                "string",
                "Optional named profile from delegation.workers. Its public defaults override delegation defaults; "
                "explicit call settings take precedence where supported.",
            ),
        },
        "required": [],
    },
...
registry.register(
    ...
    handler=lambda args, **kw: delegate_task(
        ...
        parent_agent=kw.get("parent_agent"), worker=args.get("worker"),
    ),
