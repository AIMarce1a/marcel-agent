...
NOT_REGULAR_SENTINEL = "__marcel_not_regular__"
...
MISSING_SENTINEL = "__marcel_missing__"

_READ_SENTINEL_PREFIX = "__MARCEL_RF_"
_WRITE_SENTINEL_PREFIX = "__MARCEL_WF_"
...
class ShellFileOperations(LintMixin, SearchMixin, FileOperations):
    ...
    def _escape_native_tool_arg(self, arg: str) -> str:
        ...
        understand the MSYS ``/c/...`` form and Marcel disables MSYS argument
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        tmpl = self._escape_shell_arg(".marcel-tmp.XXXXXX")
        script = (
            "set -e; "
            ...
            '|| mktemp "$d/.marcel-tmp.$$.XXXXXX" 2>/dev/null '
            '|| { tmp="$d/.marcel-tmp.$$"; : > "$tmp" && echo "$tmp"; })"; '
    ...
    def _try_read_utf16(self, path: str, offset: int, limit: int,
        ...
        snippet = (
            ...
            "    if size > MAX:\n"
            "        print('MARCEL_UTF16:NO'); sys.exit(0)\n"
            "    with open(p, 'rb') as f:\n"
            "        data = f.read()\n"
            "    sample = data[:SAMPLE]\n"
            "    enc = None\n"
    ...
    def _native_read_enabled(self) -> bool:
        ...
        file_operations holds Git-Bash-style paths there). ``MARCEL_NATIVE_FILE_READ=0``
        turns the fast path off."""
        flag = os.environ.get("MARCEL_NATIVE_FILE_READ", "1").strip().lower()
