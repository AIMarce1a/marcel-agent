...
_FORCE_SYNC_ENV = "MARCEL_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.marcel") -> list[tuple[str, str]]:
    """Enumerate all (host_path, remote_path) pairs to sync to a remote. Credential paths are
    remapped from the hardcoded /root/.marcel to *container_base* (remote home may differ)."""
    ...
    files = [
        (entry["host_path"], entry["container_path"].replace("/root/.marcel", container_base, 1))
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        per ``sync_interval`` unless *force* or ``MARCEL_FORCE_FILE_SYNC=1``. Transactional:
    ...
    def sync_back(self, marcel_home: Path | None = None) -> None:
        """Pull remote changes back to the host: download the remote ``.marcel/`` as a tar and
        ...
        with self._transaction_lock:
            self._sync_back_transaction(marcel_home=marcel_home)

    def _sync_back_transaction(self, marcel_home: Path | None = None) -> None:
        ...
        # to avoid retry storms against an uninitialized remote .marcel/.
        if not self._pushed_hashes and not self._synced_files:
            logger.debug("sync_back: no prior push state — skipping")
            return

        lock_path = (marcel_home or get_marcel_home()) / ".sync.lock"
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="marcel-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str, file_mapping: list[tuple[str, str]] | None = None, *,
        ...
        substitution (``/root/.marcel/skills/b.md`` -> ``~/.marcel/skills/b.md``)."""
