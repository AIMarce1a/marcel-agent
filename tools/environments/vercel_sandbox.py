...
under ``MARCEL_HOME`` and new sandboxes are restored from them on task reuse.
"""
...
def _ensure_vercel_sdk() -> None:
    ...
    # The SDK (>=0.7) ships default-on telemetry; Marcel policy is opt-in only, so disable it
...
def _snapshot_store() -> Path:
    return get_marcel_home() / "vercel_sandbox_snapshots.json"
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _remote_marcel_dir(self) -> str:
        return f"{self._remote_home.rstrip('/')}/.marcel"
    ...
    def _attach_fresh_sandbox(self, requested_cwd: str) -> None:
        ...
        container_base = self._remote_marcel_dir()
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        archive_member = self._remote_marcel_dir().lstrip("/")
        remote_tar = f"/tmp/.marcel_sync.{os.getpid()}.tar"
        sandbox = self._require_sandbox()
        try:
