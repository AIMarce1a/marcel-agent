"""Marcel-owned PYTHONPATH stripping for child processes. Launchers prepend the repo
root and the Marcel venv's site-packages so the backend can ``import tools``; leaked
into a child Python of a DIFFERENT version they load the backend's C extensions and
crash. Only entries proven Marcel-owned by *path provenance* are removed — never by a
cross-version heuristic. Module state (``_marcel_repo_root_aliases``, ``_in_venv``,
``_marcel_site_packages``) lives in ``tools.environments.local`` (via :func:`_state`)
...
def _build_marcel_repo_root_aliases(
    resolved_root: Path, lexical_root: Path, configured_home: Path,
) -> tuple[Path, ...]:
    """Exact repo-root spellings emitted by Marcel launchers. Mirrors
    ``gateway_windows._preserve_marcel_home_path`` (physical path under the resolved
    MARCEL_HOME -> configured spelling) so a junction-backed install matches without
    treating arbitrary MARCEL_HOME descendants as Marcel-owned. A repo-level junction
    ...
    # and the repo lives beside the profiles dir (as get_default_marcel_root() does).
...
def _validated_runtime_venv(env: dict) -> Path | None:
    ...
    if not env.get("VIRTUAL_ENV") or not any(
            _same_path(candidate, root / "venv") for root in _state()._marcel_repo_root_aliases):
        return None
    try:
...
def _get_marcel_site_packages(env: dict) -> list[Path]:
    """Exact site-packages dirs owned by the Marcel runtime (cached):
    ...
    local = _state()
    if local._marcel_site_packages is None:
        result: list[Path] = []
        if local._in_venv:
            try:
                import site
        ...
        local._marcel_site_packages = list(result)
    result = list(local._marcel_site_packages)
...
def _strip_marcel_owned_pythonpath_and_runtime_markers(env: dict) -> None:
    """Strip Marcel-owned PYTHONPATH entries, then the runtime marker vars. Order is
...
def _strip_marcel_owned_pythonpath(env: dict) -> None:
    """Remove Marcel-owned PYTHONPATH entries: only exact matches of the repo root
    ...
    if not pp:
        return
    owned_paths = [*_get_marcel_site_packages(env), *_state()._marcel_repo_root_aliases]
    ...
    if stripped:
        logger.debug("Stripped Marcel-owned entries from PYTHONPATH: %s", stripped)
