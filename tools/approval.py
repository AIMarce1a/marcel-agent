...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("MARCEL_YOLO_MODE", ""))
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode derives from Marcel YOLO. Lazy import so approval-only
...
def _unattended_contexts() -> list[_Unattended]:
    """Active unattended contexts in evaluation order: single-query first (``marcel chat -q``
    exports MARCEL_INTERACTIVE=1 but nobody answers); cron beats a platform marker because
...
def _presence(approval_callback=None) -> tuple:
    ...
    (-q) exports MARCEL_INTERACTIVE=1 but nobody answers prompts, and MARCEL_EXEC_ASK has no
    ...
    is_ask = env_var_enabled("MARCEL_EXEC_ASK")
    if _is_single_query_approval_context():
        ...
        for ctx in _unattended_contexts():
            ...
            if fail_closed_when_no_human:
                logger.warning("%s (pattern: %s): %s — no interactive user/gateway present; "
                               "BLOCKED (fail-closed). Set MARCEL_INTERACTIVE or "
                               "MARCEL_GATEWAY_SESSION to answer the prompt.", *log_args)
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'reset_marcel_interactive_context': ('tools.approval_context', 'reset_marcel_interactive_context'),
    ...
    'set_marcel_interactive_context': ('tools.approval_context', 'set_marcel_interactive_context'),
}
