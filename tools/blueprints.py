...
``metadata.marcel.blueprint`` (``schedule`` required; optional ``deliver`` [default "origin"],
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.marcel.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.marcel.blueprint`` is a mapping with a
    ...
    marcel = meta.get("marcel") if isinstance(meta, dict) else None
    blueprint = marcel.get("blueprint") if isinstance(marcel, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.marcel.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    ``metadata.marcel.blueprint`` block) ready for ``marcel skills publish``.
    ...
    frontmatter = {
        ...
        "metadata": {"marcel": {"tags": ["blueprint", "automation"], "blueprint": block}},
    }
