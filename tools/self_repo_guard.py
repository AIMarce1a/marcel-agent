...
def _block_message(operation: str, root: Path) -> str:
    ...
    scratch = (Path(marcel_home).expanduser() if marcel_home else Path.home() / ".marcel") / "scratch"
    return (
        f"Blocked: `{operation}` would rewrite Marcel's live source checkout "
        ...
        "is pushed. To change this checkout, stop Marcel, run the command externally, then restart "
        "Marcel.")
