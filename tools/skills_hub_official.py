"""Skills Hub official sources: repo-shipped optional skills and the centralized Marcel index."""
...
from tools.skills_hub_models import (
    SkillBundle, SkillMeta, SkillSource, _marcel_tags, _matches_query, _memo_json, _parse_frontmatter, hub,
)
...
class OptionalSkillSource(SkillSource):
    ...
    activated by default — absent from the system prompt and not copied to ~/.marcel/skills/ at
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        # (metadata.marcel.upstream); install pulls the live content from there.
        try:
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``main``; rather than demanding ``marcel update`` first, resolve against the live repo.
    ...
    def _upstream_pointer_from_content(self, content: Union[str, bytes]) -> Optional[Dict[str, str]]:
        """Parse ``metadata.marcel.upstream: {repo: owner/name, path: ...}`` out of SKILL.md content
        ...
        marcel_meta = meta_block.get("marcel") if isinstance(meta_block, dict) else None
        upstream = marcel_meta.get("upstream") if isinstance(marcel_meta, dict) else None
...
class MarcelIndexSource(SkillSource):
    """Skill source backed by the centralized Marcel Skills Index: a JSON catalog on the docs site,
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            from tools.skills_hub_search import _load_marcel_index
            self._index, self._loaded = _load_marcel_index(), True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        for github_id in filter(None, candidates):
            ...
            if bundle:
                bundle.source = entry.get("source", "marcel-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            ...
            source=entry.get("source", "marcel-index"), identifier=entry.get("identifier", ""),
