...
_marcel_config_resolved: str | None = None
_marcel_config_resolved_loaded = False
_real_marcel_home_cached: str | None = None
_real_marcel_home_loaded = False
...
def _marcel_home_real() -> str:
    from marcel_constants import get_marcel_home
    return os.path.realpath(str(get_marcel_home()))


def _get_marcel_config_resolved() -> str | None:
    """Return the resolved absolute path of the Marcel config file (cached)."""
    return _cached_lookup("_marcel_config_resolved", "_marcel_config_resolved_loaded", _config_path_resolved,
                          lambda: str(Path(_expand_tilde("~/.marcel/config.yaml")).resolve()))


def _get_real_marcel_home() -> str | None:
    """Return the realpath of the authoritative Marcel home (cached)."""
    return _cached_lookup("_real_marcel_home_cached", "_real_marcel_home_loaded", _marcel_home_real,
                          lambda: os.path.realpath(_expand_tilde("~/.marcel")))
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    marcel_config = _get_marcel_config_resolved()
    if marcel_config and marcel_config in candidates:
        return (
            f"Refusing to write to Marcel config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.marcel/config.yaml directly or use 'marcel config' instead.")
    return None
...
# vector (AGENTS.md / CLAUDE.md / SOUL.md / .cursorrules / project .marcel tree).
...
def _protected_instruction_config() -> tuple[bool, list[str]]:
    ...
    try:
        from marcel_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # ~/.marcel itself is governed by its own guards (config.yaml hard-block,
    # mirror guard, write_approval); this gate targets PROJECT-LOCAL files only.
    # Must run before the ``.marcel`` component rule, which would match the home.
    real_home = _get_real_marcel_home()
    ...
    for candidate in (normalized, resolved):
        ...
        if base_lower in _PROTECTED_INSTRUCTION_BASENAMES or any(
            ...
            return base
        # Project-local .marcel config dirs (<repo>/.marcel/config.yaml) steer
        # behavior too. Only the IMMEDIATE parent counts — matching any ancestor
        # would gate every write inside a checkout living under ~/.marcel.
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".marcel":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Marcel mirror prefix for persistent Docker file tools."""
    try:
        ...
        if env is not None:
            ...
            return "/root/.marcel" if persistent_docker else None
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    Marcel state (a write the host never reads). Not profile isolation — that guard was
