...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Marcel derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            marcel-agent = prev.marcel-agent.overrideAttrs (_old: {
              MARCEL_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $MARCEL_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$MARCEL_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        marcel-agent = prev.marcel-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "marcel-agent-env" {
    marcel-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "marcel-agent-editable-env" {
    marcel-agent = dependency-groups;
  };
}
