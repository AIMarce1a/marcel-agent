# nix/web.nix — Marcel Web Dashboard (Vite/React) frontend build
{ marcelNpmLib, ... }:
marcelNpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @marcel/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../marcel_cli/web_dist for the
