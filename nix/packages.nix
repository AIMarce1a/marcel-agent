# nix/packages.nix — Marcel Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./marcel-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.marcelTui;
        web = full.marcelWeb;
        desktop = full.marcelDesktop;

        update-npm-lockfile = full.marcelNpmLib.updateNpmLockfile;
      };
    };
}
