# nix/nixosModules.nix — the NixOS module for marcel-agent
#
...
#
# Container mode: marcel runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $MARCEL_HOME/.env
# and read by marcel at startup — no container recreation needed for env changes.
#
# Tool resolution: the marcel wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      ...
      marcelHome = "${cfg.stateDir}/.marcel";
      ...
      # MARCEL_HOME via addToSystemPackages, so they can save settings through the
      ...
      # marcel user + sudo on first boot (writable layer persists), then
      # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
      containerEntrypoint = pkgs.writeShellScript "marcel-container-entrypoint" ''
        set -eu

        MARCEL_UID="''${MARCEL_UID:?MARCEL_UID must be set}"
        MARCEL_GID="''${MARCEL_GID:?MARCEL_GID must be set}"

        # ── Group: ensure a group with GID=$MARCEL_GID exists ──
        # Check by GID (not name) to avoid collisions with pre-existing groups
        # (e.g. GID 100 = "users" on Ubuntu)
        EXISTING_GROUP=$(getent group "$MARCEL_GID" 2>/dev/null | cut -d: -f1 || true)
        if [ -n "$EXISTING_GROUP" ]; then
          GROUP_NAME="$EXISTING_GROUP"
        else
          GROUP_NAME="marcel"
          if command -v groupadd >/dev/null 2>&1; then
            groupadd -g "$MARCEL_GID" "$GROUP_NAME"
          elif command -v addgroup >/dev/null 2>&1; then
            addgroup -g "$MARCEL_GID" "$GROUP_NAME" 2>/dev/null || true
          fi
        fi

        # ── User: ensure a user with UID=$MARCEL_UID exists ──
        PASSWD_ENTRY=$(getent passwd "$MARCEL_UID" 2>/dev/null || true)
        if [ -n "$PASSWD_ENTRY" ]; then
          TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
          TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
        else
          TARGET_USER="marcel"
          TARGET_HOME="/home/marcel"
          if command -v useradd >/dev/null 2>&1; then
            useradd -u "$MARCEL_UID" -g "$MARCEL_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
          elif command -v adduser >/dev/null 2>&1; then
            adduser -u "$MARCEL_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
          fi
        fi
        mkdir -p "$TARGET_HOME"
        chown "$MARCEL_UID:$MARCEL_GID" "$TARGET_HOME"
        chmod 0750 "$TARGET_HOME"

        # Ensure MARCEL_HOME is owned by the target user.
        ...
        if [ -n "''${MARCEL_HOME:-}" ] && [ -d "$MARCEL_HOME" ]; then
          find "$MARCEL_HOME" \! -user "$MARCEL_UID" -exec chown "$MARCEL_UID:$MARCEL_GID" {} +
        fi
        ...
        if [ ! -f /var/lib/marcel-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
          ...
          touch /var/lib/marcel-tools-provisioned
        fi

        if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/marcel ]; then
          mkdir -p /etc/sudoers.d
          echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/marcel
          chmod 0440 /etc/sudoers.d/marcel
        fi
        ...
        if command -v setpriv >/dev/null 2>&1; then
          exec setpriv --reuid="$MARCEL_UID" --regid="$MARCEL_GID" --init-groups "$@"
      ...
      # so they can update without recreation. Env vars go through $MARCEL_HOME/.env.
      ...
      containerModeFile = pkgs.writeText "marcel-container-mode" ''
        # Written by the NixOS activation script. Do not edit manually.
        backend=${cfg.container.backend}
        container_name=${containerName}
        exec_user=${cfg.user}
        marcel_bin=${containerDataDir}/current-package/bin/marcel
      '';

      # Default: /var/lib/marcel/workspace → /data/workspace.
      ...
      commonServiceConfig = {
        ...
        # so interactive users in the marcel group can read/write them.
        UMask = "0007";
      ...
      // common.processEnvironment { inherit marcelHome; };
    ...
    in
    {
      options.services.marcel-agent =
        ...
        // (
          with lib;
          {
            ...
            stateDir = mkOption {
              type = types.str;
              default = "/var/lib/marcel";
              description = "State directory. Contains .marcel/ subdir (MARCEL_HOME).";
            };
            ...
            addToSystemPackages = mkOption {
              ...
              default = false;
              description = ''
                Add the marcel CLI to environment.systemPackages and export
                MARCEL_HOME system-wide (via environment.variables) so interactive
            ...
            container = {
              ...
              hostUsers = mkOption {
                ...
                default = [ ];
                description = ''
                  Interactive users who get a ~/.marcel symlink to the service
                  stateDir. These users are automatically added to the marcel group.
                '';
                example = [ "sidbin" ];
              };
            };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          # ── Host CLI ──────────────────────────────────────────────────────
          # Add the marcel CLI to system PATH and export MARCEL_HOME system-wide
          # so interactive shells share state (sessions, skills, cron) with the
          # gateway service instead of creating a separate ~/.marcel/.
          (lib.mkIf cfg.addToSystemPackages {
            environment.systemPackages = [ effectivePackage ];
            environment.variables.MARCEL_HOME = marcelHome;
          })
              ...
              ++ [
                {
                  ...
                  message = "services.marcel-agent: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }

          # ── Per-user profile for extraPackages ───────────────────────────
          # Wire extraPackages into the marcel user's per-user profile so the
          ...
          (lib.mkIf
            (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != [ ])
            {
              warnings = [
                ''
                  services.marcel-agent: container.enable is true and container.hostUsers
                  is set, but addToSystemPackages is false. Without a host-installed marcel
                  binary, container routing will not work for interactive users.
                  Set addToSystemPackages = true or ensure marcel is on PATH.
                ''
              ];
            }
          )
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${marcelHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${marcelHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."marcel-agent-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${marcelHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${marcelHome} ${cfg.workingDirectory}
                  ...
                  find ${marcelHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${marcelHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${marcelHome}/$_subdir"
                    chmod 2770 "${marcelHome}/$_subdir"
                    find "${marcelHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg marcelHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${marcelHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${marcelHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${marcelHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            let
                              ...
                              symlinkPath = "${userHome}/.marcel";
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${marcelHome}" ]; then
                                rm -f "${symlinkPath}"
                                echo "marcel-agent: removed symlink ${symlinkPath}"
                              fi
                            ''
                  ...
                  # Create ~/.marcel -> stateDir/.marcel for each hostUser so the
          ...
          (lib.mkIf (!cfg.container.enable) {
            systemd.services.marcel-agent = {
              ...
              # $MARCEL_HOME/.env by the activation script. load_marcel_dotenv()
          ...
          (lib.mkIf (!cfg.container.enable && cfg.backend.mode != "none") {
            systemd.services.marcel-backend = {
          ...
          (lib.mkIf cfg.container.enable {
            ...
            systemd.services.marcel-agent = {
              description = "Marcel Agent Gateway (container)";
              wantedBy = [ "multi-user.target" ];
              after = [
              ...
              preStart = ''
                ...
                if [ "$NEED_CREATE" = "true" ]; then
                  ...
                  MARCEL_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
                  MARCEL_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
                  ...
                  ${containerBin} create \
                    ...
                    --env MARCEL_UID="$MARCEL_UID" \
                    --env MARCEL_GID="$MARCEL_GID" \
                    --env MARCEL_HOME=${containerDataDir}/.marcel \
                    --env MARCEL_MANAGED=true \
                    --env HOME=${containerHomeDir} \
                    ${lib.concatStringsSep " " cfg.container.extraOptions} \
                    ${cfg.container.image} \
                    ${containerDataDir}/current-package/bin/marcel gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
