...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      marcel-agent = self'.packages.default;
      marcelVenv = marcel-agent.marcelVenv;
      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "marcel-check";
                homeDirectory = "/home/marcel-check";
                stateVersion = "24.11";
              };
            }
            { services.marcel-agent = settings; }
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.marcel-agent = programs;
              services.marcel-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.marcel-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.marcel-agent);
      ...
      configKeys = pkgs.runCommand "marcel-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${marcelVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "marcel-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "marcel-build-package" { } ''
          echo "PASS: package built at ${marcel-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "marcel-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.MARCEL_TEST = "1";
                environmentFiles = [ "/run/secrets/marcel-env" ];
                marcelHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "marcel" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "marcel" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.marcelAgentSetup.data;

            failures =
              lib.optional (names != [
                "marcel-agent"
                "marcel-backend"
              ]) "expected marcel-agent + marcel-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/marcel gateway" (argvOf "marcel-agent")
              ) "gateway process does not run `marcel gateway`: ${argvOf "marcel-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/marcel serve" (argvOf "marcel-backend")
              ) "backend process does not run `marcel serve`: ${argvOf "marcel-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "marcel-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/marcel-check/.marcel" (envOf n)) names
              ) "gateway and backend must share one MARCEL_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.MARCEL_HOME or null != "/home/marcel-check/.marcel"
              ) "programs.marcel-agent.enable must export MARCEL_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "marcel-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/marcel-check/.marcel/SOUL.md" activation
              ) "marcelHomeFiles must install into MARCEL_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads MARCEL_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "MARCEL_MANAGED=home-manager" (envOf n)) names
              ) "processes must report MARCEL_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "marcel-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "marcel-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/marcel-check";
            ...
            cases = [
              ...
              {
                name = "marcelHomeFiles needs no directory";
                ok = accepts { marcelHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "marcel-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one MARCEL_HOME ───────────────
        # `programs.enable` exports MARCEL_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.marcel
        # while the services use the MARCEL_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/marcel-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                marcelHome = "/home/marcel-check/.marcel-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "marcel-desktop") cfg.home.packages;
            ...
            # second Marcel runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "marcel-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.marcel-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.marcel-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one marcel-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "MARCEL_HOME" != "/home/marcel-check/.marcel-work"
              ) "the launcher must carry MARCEL_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "MARCEL_HOME")})"
              ++ lib.optional (
                setValue "MARCEL_MANAGED" != "home-manager"
              ) "the launcher must report MARCEL_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "MARCEL_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "MARCEL_DESKTOP_MARCEL" != "${lib.head agentPackages}/bin/marcel"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "MARCEL_DESKTOP_MARCEL")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "MARCEL_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "MARCEL_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "MARCEL_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "MARCEL_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "MARCEL_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "marcel-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares MARCEL_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set MARCEL_HOME" wrapper
              ) "the launcher must carry MARCEL_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "MARCEL_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "MARCEL_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? marcel-backend || cfg.launchd.agents ? marcel-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "marcel-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `marcel`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.marcel-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.marcel-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "marcel-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "marcel-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "marcel" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."marcel-agent-setup".text;

            failures =
              lib.optional (names != [
                "marcel-agent"
                "marcel-backend"
              ]) "expected marcel-agent + marcel-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/marcel gateway" (execOf "marcel-agent")
              ) "gateway unit does not run `marcel gateway`: ${execOf "marcel-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/marcel dashboard" (execOf "marcel-backend")
              ) "backend unit does not run `marcel dashboard`: ${execOf "marcel-backend"}"
              ++ lib.optional (
                units.marcel-agent.environment.MARCEL_HOME != units.marcel-backend.environment.MARCEL_HOME
              ) "gateway and backend must share one MARCEL_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/marcel/.marcel/SOUL.md" activation
              ) "marcelHomeFiles must install into MARCEL_HOME";
          ...
          in
          pkgs.runCommand "marcel-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs marcel, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.marcel-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/marcel serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec marcel directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "marcel-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "marcel-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec marcel, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/marcel dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "marcel-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        # $MARCEL_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                MARCEL_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "marcel-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'MARCEL_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--marcel-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "marcel-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "marcel-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${marcel-agent}/bin/marcel || (echo "FAIL: marcel binary missing"; exit 1)
          test -x ${marcel-agent}/bin/marcel-agent || (echo "FAIL: marcel-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${marcel-agent}/bin/marcel --version 2>&1 | grep -qi "marcel" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "marcel-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in marcel marcel-agent marcel-acp; do
            test -x ${marcel-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "marcel-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking marcel --help ==="
          ${marcel-agent}/bin/marcel --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${marcel-agent}/bin/marcel --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "marcel-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${marcel-agent}/share/marcel-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${marcel-agent}/share/marcel-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "MARCEL_BUNDLED_SKILLS" ${marcel-agent}/bin/marcel || \
            (echo "FAIL: MARCEL_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: MARCEL_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${marcel-agent}/share/marcel-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${marcel-agent}/share/marcel-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "MARCEL_OPTIONAL_SKILLS" ${marcel-agent}/bin/marcel || \
            (echo "FAIL: MARCEL_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, MARCEL_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "marcel-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${marcel-agent}/share/marcel-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${marcel-agent}/share/marcel-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "MARCEL_BUNDLED_PLUGINS" ${marcel-agent}/bin/marcel || \
            (echo "FAIL: MARCEL_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: MARCEL_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "marcel-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${marcel-agent}/share/marcel-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${marcel-agent}/share/marcel-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${marcel-agent}/share/marcel-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "MARCEL_BUNDLED_LOCALES" ${marcel-agent}/bin/marcel || \
            (echo "FAIL: MARCEL_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: MARCEL_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + MARCEL_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && MARCEL_BUNDLED_LOCALES=${marcel-agent}/share/marcel-agent/locales \
            ${marcelVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with MARCEL_BUNDLED_LOCALES set"; exit 1)
        ...
        # MARCEL_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "marcel-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${marcel-agent}/share/marcel-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${marcel-agent}/share/marcel-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "MARCEL_OPTIONAL_MCPS" ${marcel-agent}/bin/marcel || \
            (echo "FAIL: MARCEL_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: MARCEL_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${marcel-agent}/bin/marcel mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: marcel mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "marcel-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${marcel-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${marcel-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "MARCEL_TUI_DIR" ${marcel-agent}/bin/marcel || \
            (echo "FAIL: MARCEL_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: MARCEL_TUI_DIR set in wrapper"
        ...
        # Verify MARCEL_NODE is set in wrapper and points to Node 26+
        # (Marcel pins its toolchain to Node 26 everywhere)
        marcel-node = pkgs.runCommand "marcel-node-version" { } ''
          set -e
          echo "=== Checking MARCEL_NODE in wrapper ==="
          grep -q "MARCEL_NODE" ${marcel-agent}/bin/marcel || \
            (echo "FAIL: MARCEL_NODE not set in wrapper"; exit 1)
          echo "PASS: MARCEL_NODE present in wrapper"

          MARCEL_NODE=$(sed -n "s/^export MARCEL_NODE='\(.*\)'/\1/p" ${marcel-agent}/bin/marcel)
          test -x "$MARCEL_NODE" || (echo "FAIL: MARCEL_NODE=$MARCEL_NODE not executable"; exit 1)
          echo "PASS: MARCEL_NODE executable at $MARCEL_NODE"

          NODE_MAJOR=$("$MARCEL_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Marcel requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All MARCEL_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify MARCEL_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "marcel-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(MARCEL_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking MARCEL_MANAGED guards ==="
          check_blocked "config set" ${marcel-agent}/bin/marcel config set model foo
          check_blocked "config edit" ${marcel-agent}/bin/marcel config edit
        ...
        extra-python-packages = let
          ...
          marcelWithExtra = marcel-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "marcel-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${marcelWithExtra}/bin/marcel || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${marcelWithExtra}/bin/marcel || \
          ...
          if grep -q "PYTHONPATH" ${marcel-agent}/bin/marcel; then
        ...
        extra-dependency-groups = let
          marcelWithGroups = marcel-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "marcel-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${marcelWithGroups}"
          echo "venv: ${marcelWithGroups.marcelVenv}"
        ...
        messaging-variant = pkgs.runCommand "marcel-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.marcelVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "marcel-config-roundtrip" {
          ...
          merge_and_load() {
            local marcel_home="$1"
            export MARCEL_HOME="$marcel_home"
            ${configMergeScript} ${nixSettings} "$marcel_home/config.yaml"
            ${marcelVenv}/bin/python3 -c '
import json, sys
