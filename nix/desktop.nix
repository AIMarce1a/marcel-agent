# nix/desktop.nix — Marcel Desktop (Electron) app build + wrapper
#
# `marcelAgent` is the fully-built `.#default` package — it ships the
# `marcel` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `MARCEL_DESKTOP_MARCEL` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Marcel CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # MARCEL_HOME and MARCEL_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "marcel-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "marcel-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = marcelNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "marcel-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe marcelNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/marcel-desktop $out/bin
    cp -r ${renderer}/* $out/share/marcel-desktop/
    ...
    substituteInPlace $out/share/marcel-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/marcel-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # MARCEL_DESKTOP_MARCEL to the absolute path of the nix-built `marcel`
    # binary so the desktop's resolver step 4 ("existing Marcel CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/marcel-desktop \
      --add-flags "$out/share/marcel-desktop" \
      --set MARCEL_DESKTOP_MARCEL "${lib.getExe marcelAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/marcel.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../marcel_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/marcel-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/marcel.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/marcel.desktop
    runHook postInstall
  '';
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Marcel Agent";
    homepage = "https://github.com/NousResearch/hermes-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "marcel-desktop";
  };
}
