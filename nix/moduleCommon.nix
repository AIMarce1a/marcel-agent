...
#
# `services.marcel-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  marcelHome, programs.marcel-agent (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "marcel-config-attrs";
    description = "Marcel YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $MARCEL_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "Marcel Agent";
      ...
      package = mkOption {
        ...
        description = "The marcel-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The Marcel configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `marcel config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $MARCEL_HOME/.env. Marcel reads that file at each start, with
          load_marcel_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."marcel/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that Marcel refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          Marcel reads SOUL.md and memories/ from MARCEL_HOME, so put those
          files in `marcelHomeFiles`.
      ...
      marcelHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into MARCEL_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          Marcel reads SOUL.md and the memory files from MARCEL_HOME and not
          from the working directory. Declare those files here, or Marcel
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the marcel plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. Marcel discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          marcel_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as marcel (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in marcel-agent's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `marcel gateway`.";
      };
      ...
      # ── The backend: `marcel serve` or `marcel dashboard` ──────────────
      # `marcel serve` and `marcel dashboard` are the same entry point,
      # marcel_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that Marcel Desktop
      ...
      # controls an external gateway, with `marcel gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that Marcel
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            MARCEL_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. Marcel Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."marcel/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `marcel` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.marcel-agent.installPackage was removed. Marcel now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.marcel-agent.enable = ${lib.boolToString (value != false)};  # the marcel CLI, and MARCEL_HOME for your shells
        programs.marcel-agent.desktop.enable = true;  # the desktop application

      `services.marcel-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "marcel-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "marcel-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "marcel-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "marcel-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "marcel-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # MARCEL_MANAGED variable of the service. The value tells the shell
    ...
    in
    ''
      # Directories. The service units and Marcel make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${marcelHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. Marcel
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${marcelHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${marcelHome}/config.yaml
            ${run}chmod ${modes.config} ${marcelHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "marcel-managed" managedSystem} ${marcelHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${marcelHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${marcelHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${marcelHome}/auth.json"
        else
          ''
            if [ ! -e ${marcelHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${marcelHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree marcelHome cfg.marcelHomeFiles}
      ...
      ${run}find ${marcelHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "marcel-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${marcelHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/marcel"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps marcel as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "marcel-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "marcel-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "marcel-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          MARCEL_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export MARCEL_DASHBOARD_SESSION_TOKEN

          if [ -z "$MARCEL_DASHBOARD_SESSION_TOKEN" ]; then
            echo "marcel-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "marcel-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "marcel-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "marcel-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "marcel-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "marcel-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "marcel-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "Marcel Agent web dashboard and desktop backend"
    else
      "Marcel Agent backend for Marcel Desktop";

  # The environment that each Marcel process needs, from either module.
  #
  # managedSystem gives the value of MARCEL_MANAGED. The CLI reads that
    ...
    [
      {
        ...
        message = ''
          ...
          To give Marcel an identity and a memory, use
          ${optionPath}.marcelHomeFiles instead. Those files go to
          MARCEL_HOME. Marcel reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of MARCEL_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
