...
try:
    ...
    pass  # partial `marcel update` — only skips the Windows UTF-8 stdio setup
...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    """cwd to stamp on a new session row (``marcel -c`` / ``--resume``), or None.
...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        ...
        source = get_session_env("MARCEL_SESSION_SOURCE", "")
    except Exception:
        source = os.environ.get("MARCEL_SESSION_SOURCE", "")
...
_marcel_home = get_marcel_home()  # read by agent_init via _ra()._marcel_home
_loaded_env_paths = load_marcel_dotenv(marcel_home=_marcel_home, project_env=Path(__file__).parent / '.env')
...
class AIAgent(
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[marcel-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> Any:
        ...
        if (getattr(self, "lmstudio_load_mode", "explicit") or "explicit").strip().lower() == "jit":
            ...
            return None
        from marcel_cli.models_local import ensure_lmstudio_model_loaded
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        ``MARCEL_API_TIMEOUT`` > 1800s."""
        cfg = get_provider_request_timeout(self.provider, self.model)
        return cfg if cfg is not None else env_float("MARCEL_API_TIMEOUT", 1800.0)

    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        """Base non-stream stale timeout: per-model ``stale_timeout_seconds`` > provider-wide >
        ``MARCEL_API_CALL_STALE_TIMEOUT`` > reasoning floor > 90s.
        ...
        if cfg is not None:
            return cfg, False
        env_timeout = os.getenv("MARCEL_API_CALL_STALE_TIMEOUT")
    ...
    def _stale_timeout_is_explicit(self) -> bool:
        ...
        return (get_provider_stale_timeout(self.provider, self.model) is not None
                or os.getenv("MARCEL_API_CALL_STALE_TIMEOUT") is not None)
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        return (
            ...
            "See marcel-agent#21444 for symptom history."
        )
