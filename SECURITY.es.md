...
- Detalles del entorno (`marcel --version`, SHA del commit, SO, versión de Python).
...
- **Proceso del agente.** El intérprete Python que ejecuta Marcel Agent,
...
- **Envolvente de confianza.** El conjunto de recursos a los que un operador ha otorgado
  implícitamente acceso a Marcel Agent al ejecutarlo — típicamente, todo lo que
  la propia cuenta de usuario del operador puede alcanzar en el host.
- **Postura.** Una declaración explícita en la documentación o código de Marcel Agent
...
Marcel Agent admite dos posturas de aislamiento a nivel de SO. Abordan
...
Marcel Agent admite esto de dos maneras:

- **La propia imagen Docker de Marcel Agent y la configuración de Compose.** Más
...
Bajo una envoltura de proceso completo, las heurísticas en proceso de Marcel Agent
...
Marcel Agent filtra el entorno que pasa a sus componentes en proceso de
...
Un plugin malicioso o con errores no es una vulnerabilidad en Marcel Agent
en sí mismo. Los errores en la ruta de instalación o descubrimiento de plugins de Marcel Agent
...
4. **Dentro del conjunto autorizado, todos los llamadores tienen la misma confianza.**
   Marcel Agent no modela capacidades por llamador dentro de un único adaptador.
...
- Violaciones de la documentación del modelo de confianza: código que se comporta
  contrariamente a lo que esta política, la propia documentación de Marcel Agent o
  las expectativas razonables del operador predecirían — incluyendo casos donde
  Marcel Agent ha documentado una postura sobre cómo su salida debe ser
...
- **Configuraciones documentadas de emergencia.** Compensaciones seleccionadas por el operador
  ...
  la seguridad de marcel-home, y similares. Los informes contra esas
...
- **Habilidades y plugins contribuidos por la comunidad.** Las habilidades de terceros
  ...
  están en la superficie de revisión del operador, no en la superficie de confianza de Marcel Agent
  (§2.4, §2.5). Una habilidad o plugin que haga algo
  malicioso es el modo de falla esperado de uno que no fue
  revisado, no una vulnerabilidad en Marcel Agent. Los errores en la ruta de
  instalación de habilidades o plugins de Marcel Agent que impidan al
...
- Marcel Agent incluye guardias de cadena de suministro para lanzamientos de servidores
