"""
Marcel MCP Server — expose messaging conversations as MCP tools (`marcel mcp serve`).
...
Matches OpenClaw's 9-tool channel bridge surface plus the Marcel-specific
channels_list. Client config: {"mcpServers": {"marcel": {"command": "marcel", "args": ["mcp", "serve"]}}}
"""
...
logger = logging.getLogger("marcel.mcp_serve")
...
def _marcel_home() -> Path:
    try:
        ...
        return Path(os.environ.get("MARCEL_HOME", Path.home() / ".marcel"))


def _get_sessions_dir() -> Path:
    return _marcel_home() / "sessions"


def _read_state_db_mtime() -> float:
    try:
        return (_marcel_home() / "state.db").stat().st_mtime
...
def _load_channel_directory() -> dict:
    ...
    return _read_json(_marcel_home() / "channel_directory.json")
...
class EventBridge:
    ...
    event queue with waiter support (the Marcel analogue of OpenClaw's WebSocket
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Marcel MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(f"MCP server requires the 'mcp' package. Install with: {sys.executable} -m pip install 'mcp'")
    mcp = MCPServer("marcel", instructions=(
        "Marcel Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Marcel MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
