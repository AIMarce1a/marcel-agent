...
from marcel_constants import OPENROUTER_BASE_URL, get_marcel_home
from agent.retry_utils import jittered_backoff
from marcel_cli.env_loader import load_marcel_dotenv

# Load .env from MARCEL_HOME first, then project root as a dev fallback.
load_marcel_dotenv(marcel_home=get_marcel_home(), project_env=Path(__file__).parent / ".env")
    ...
    def _init_summarizer(self):
        ...
        if provider:
            ...
            if client is None:
                ...
                raise RuntimeError(
                    f"Provider '{provider}' is not configured. Check your API key or run: {runtime_command('setup')}"
                )
