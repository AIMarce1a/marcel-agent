# Marcel Agent Security Policy

This document describes Marcel Agent's trust model, names the one
...
- Environment details (`marcel --version`, commit SHA, OS, Python
  version).
...
- **Agent process.** The Python interpreter running Marcel Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Marcel Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Marcel Agent's documentation
...
Marcel Agent supports two OS-level isolation postures. They address
...
Marcel Agent supports this in two ways:

- **Marcel Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Marcel Agent's in-process heuristics
...
Marcel Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Marcel Agent
itself. Bugs in Marcel Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Marcel Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Marcel Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Marcel Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  marcel-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Marcel Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Marcel Agent. Bugs in Marcel
...
- Marcel Agent includes supply-chain guards for MCP server
