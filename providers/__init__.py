...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with marcel-agent)
2. User plugins: ``$MARCEL_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``marcel_agent.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$MARCEL_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$MARCEL_HOME/plugins/model-providers/`` if it exists."""
    try:
        from marcel_constants import get_marcel_home

        d = get_marcel_home() / "plugins" / "model-providers"
...
def _installed_plugins_dir() -> Path | None:
    """Return ``$MARCEL_HOME/plugins/`` if it exists.

    This is where ``marcel plugins install`` clones a plugin — flat, one
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple MARCEL_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_marcel_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``marcel_agent.plugins``
    ...
    * **Provider targets only.** The ``marcel_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$MARCEL_HOME``) keep their
    ...
    try:
        from marcel_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$MARCEL_HOME/plugins/model-providers/<name>/``
      2b. Plugins installed by ``marcel plugins install`` at
          ``$MARCEL_HOME/plugins/<name>/`` that declare ``kind: model-provider``
    ...
    # 0. Pip-installed plugins — entry points in the ``marcel_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$MARCEL_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with marcel-agent.
    ...
    # 2. User plugins — under $MARCEL_HOME/plugins/model-providers/<name>/.
    ...
    # 2b. Plugins installed by ``marcel plugins install`` / the plugin index.
    #     Those clone into $MARCEL_HOME/plugins/<name>/ — flat, NOT under
