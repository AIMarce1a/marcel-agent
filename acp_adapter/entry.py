"""CLI entry point for the marcel-agent ACP adapter.

Loads ``~/.marcel/.env``, routes logging to stderr (stdout is reserved for ACP
JSON-RPC), and starts the ACP agent server.

Usage::

    python -m acp_adapter.entry   # or: marcel acp / marcel-acp
"""

# IMPORTANT: marcel_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See marcel_bootstrap.py for full rationale.
try:
    import marcel_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Partial ``marcel update`` (git-reset landed, ``uv pip install -e .`` did not):
    # UTF-8 stdio setup is skipped on Windows; POSIX is unaffected.
    pass
else:
    # Stop a ``utils/``/``proxy/``/``ui/`` package in the launch cwd from shadowing Marcel modules.
...
def _load_env() -> None:
    """Load .env from MARCEL_HOME (default ``~/.marcel``)."""
    from marcel_cli.env_loader import load_marcel_dotenv

    marcel_home = get_marcel_home()
    loaded = load_marcel_dotenv(marcel_home=marcel_home)
    log = logging.getLogger(__name__)
    for env_file in loaded or ():
        log.info("Loaded env from %s", env_file)
    if not loaded:
        log.info("No .env found at %s, using system env", marcel_home / ".env")


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="marcel-acp", description="Run Marcel Agent as an ACP stdio server.")
    parser.add_argument("--version", action="store_true", help="Print Marcel version and exit")
    parser.add_argument("--check", action="store_true", help="Verify ACP dependencies and adapter imports, then exit")
    parser.add_argument("--setup", action="store_true",
                        help="Run interactive Marcel provider/model setup for ACP terminal auth")
    parser.add_argument("--setup-browser", action="store_true",
                        help="Install agent-browser + Playwright Chromium into ~/.marcel/node/ "
...
def _print_version() -> None:
    from marcel_cli import __version__ as marcel_version

    print(marcel_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import MarcelACPAgent  # noqa: F401

    print("Marcel ACP check OK")


def _run_setup() -> None:
    from marcel_cli.main import main as marcel_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "marcel", "model"]
        marcel_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting marcel-agent ACP adapter")
    ...
    import acp
    from .server import MarcelACPAgent
    ...
    if os.environ.get("MARCEL_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
