"""ACP auth helpers — detect and advertise Marcel authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "marcel-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Marcel runtime provider, or None if unavailable.
...
def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Marcel.

    The ACP registry requires at least one usable auth method in the initial
    handshake. A fresh Zed install may have no Marcel credentials yet, so the
    ...
    if provider:
        methods.append(AuthMethodAgent(
            ...
            description=f"Authenticate Marcel using the currently configured {provider} runtime credentials.",
        ))
    methods.append(TerminalAuthMethod(
        id=TERMINAL_SETUP_AUTH_METHOD_ID, name="Configure Marcel provider", type="terminal", args=["--setup"],
        description=("Open Marcel' interactive model/provider setup in a terminal. "
                     "Use this when Marcel has not been configured on this machine yet."),
    ))
    return methods
...
def has_provider() -> bool:
    """Return True if Marcel can resolve any runtime provider credentials."""
