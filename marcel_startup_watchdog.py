...
timeout. Config is env-only (``MARCEL_STARTUP_WATCHDOG=0``,
``MARCEL_STARTUP_WATCHDOG_TIMEOUT_S``) because config.yaml parsing is itself in scope.
...
ENV_STARTUP_WATCHDOG = "MARCEL_STARTUP_WATCHDOG"
ENV_STARTUP_WATCHDOG_TIMEOUT_S = "MARCEL_STARTUP_WATCHDOG_TIMEOUT_S"
...
# Module singleton: the arm sites (marcel_cli.main / marcel_cli.gateway /
...
def _process_marcel_home() -> Path:
    """MARCEL_HOME for diagnostic files — stdlib-only replica of the marcel_constants default."""
...
def get_startup_watchdog_dump_path(home: Optional[Path] = None) -> Path:
    """Return ``<MARCEL_HOME>/logs/gateway-startup-watchdog.log``."""
    base = home if home is not None else _process_marcel_home()
    return base.joinpath(*_DUMP_RELATIVE)


def startup_watchdog_disabled() -> bool:
    """True when ``MARCEL_STARTUP_WATCHDOG`` opts out explicitly."""
...
def arm_startup_watchdog(
    ...
    *,
    ...
    ``MARCEL_STARTUP_WATCHDOG=0`` or when the thread could not be started.
    """
    global _handle
    try:
