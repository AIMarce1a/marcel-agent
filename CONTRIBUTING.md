# Contributing to Marcel Agent

Thank you for contributing to Marcel Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Marcel to work everywhere.
...
Bundled skills (in `skills/`) ship with every Marcel install. They should be **broadly useful to most users**:
...
**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.marcel/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(marcel_home, config)` for setup-wizard integration
...
- Integrate with `marcel memory setup` via `post_setup()` — no need to touch core code
...
The installer creates the Marcel venv, wires the `marcel` command, stamps the
install method for `marcel update`, and clones the full git project into
`$MARCEL_HOME/marcel-agent` (usually `~/.marcel/marcel-agent`). That keeps your
...
Use this only if you intentionally do not want Marcel' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `marcel` entrypoint from this venv; running the
system `python3 -m marcel_cli.main` can pick up unrelated system Python
packages.
...
```bash
mkdir -p ~/.marcel/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.marcel/config.yaml
touch ~/.marcel/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.marcel/.env
```

### Run

```bash
# The standard installer already put `marcel` on PATH.
marcel doctor
marcel chat -q "Hello"
```

If you used the manual clone fallback, run `./marcel` from the checkout or
...
├── cli.py                    # MarcelCLI class — interactive CLI orchestrator (~4.6k LOC + marcel_cli/cli_*_mixin.py)
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (marcel-cli, marcel-telegram, etc.)
├── marcel_state.py           # SessionDB facade (~1.4k LOC); implementation in marcel_state_*.py (21 siblings) — FTS5 search, session titles
...
│
├── marcel_cli/               # CLI command implementations
│   ├── main.py                   # Entry point, argument parsing, command dispatch
│   ├── cli_*_mixin.py            # MarcelCLI mixins (slash commands, display, session, ...)
...
### User configuration (stored in `~/.marcel/`)

| Path | Purpose |
|------|---------|
| `~/.marcel/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.marcel/.env` | API keys and secrets |
| `~/.marcel/auth.json` | OAuth credentials (Nous Portal) |
| `~/.marcel/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.marcel/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.marcel/state.db` | SQLite session database |
| `~/.marcel/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.marcel/cron/` | Scheduled job data |
| `~/.marcel/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`marcel_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.marcel/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
(for example `_MARCEL_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.marcel`:
...
The user may skip setup and keep loading the skill. Marcel only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `marcel setup` or update `~/.marcel/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Marcel tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Marcel Agent" is the secondary collaborator. If the contributor's commit shows "Marcel Agent" as author because they used Marcel to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Marcel tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `marcel --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Marcel uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.marcel/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `marcel_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `marcel_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Marcel runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the marcel wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `marcel_cli/gateway.py::_scan_gateway_pids` for the pattern.
   ```
...
9. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `marcel_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
15. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `marcel_cli/gateway_windows.py::
...
Marcel has terminal access. Security matters.
...
2. **Test manually**: Run `marcel` and exercise the code path you changed
