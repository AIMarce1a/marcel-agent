"""Guard: Marcel-owned subprocesses must not resolve managed runtimes by bare PATH.

Marcel installs runtimes for itself — ``uv`` at ``$MARCEL_HOME/bin/uv``, Node at
``$MARCEL_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Marcel's own
...
* when a system copy also exists, the one Marcel does not own wins — which is
...
``iter_marcel_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Marcel provisions into MARCEL_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Marcel-owned subprocess code: plugins ship their own
...
_ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("marcel_cli/update_cmd_deps.py", "uv"): (
    ...
    ("marcel_cli/update_cmd_deps.py", "npm"): (
...
def _source_files() -> list[Path]:
    ...
    # (marcel_agent-<version>/) gets created and deleted concurrently, and
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Marcel-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$MARCEL_HOME/bin (uv) and $MARCEL_HOME/node are not on an "
        ...
        "  node/npm -> marcel_constants.find_node_executable()\n"
        "  PATH env -> marcel_constants.iter_marcel_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(marcel_constants, helper))


def test_managed_uv_helpers_exist():
    from marcel_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
