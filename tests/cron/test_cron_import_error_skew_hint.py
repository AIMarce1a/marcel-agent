...
Field incident: an interrupted `marcel update` (pull done, restart never ran)
...
had no way to know the fix was one `marcel gateway restart`. The failure
...
def test_import_error_without_skew_stays_a_plain_import_message(monkeypatch):
    ...
    assert "marcel gateway restart" not in msg
