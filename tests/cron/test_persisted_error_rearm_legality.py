...
def cron_store(tmp_path, monkeypatch):
    ...
    (marcel_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("MARCEL_HOME", str(marcel_home))
    monkeypatch.setattr(J, "MARCEL_DIR", marcel_home)
    monkeypatch.setattr(J, "CRON_DIR", marcel_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", marcel_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", marcel_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_marcel_now", lambda: now):
            due = J.get_due_jobs()
