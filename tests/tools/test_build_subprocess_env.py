...
def test_no_scrub_inherit_profile_home_bridges_context_override(tmp_path):
    ...
    assert env["MARCEL_HOME"] == str(tmp_path)
...
def test_e2e_child_sees_marcel_home_and_no_planted_secret(tmp_path, monkeypatch):
    """A real child spawned with a factory-built env must see MARCEL_HOME
    ...
    code = (
        "import os, json; "
        "print(json.dumps({'home': os.environ.get('MARCEL_HOME'), "
    ...
    assert result["home"] == str(marcel_home)
...
# ---------------------------------------------------------------------------
# E2E regression (#93082): cron/no_agent children keep bare `marcel` on PATH
# ---------------------------------------------------------------------------


def test_e2e_scrubbed_env_resolves_bare_marcel_under_minimal_parent_path(monkeypatch):
    """Regression for #92998/#93082: a gateway launched by systemd/cron with a
    minimal PATH (no marcel console-script dir) must still hand cron job
    children an env whose PATH resolves bare ``marcel``.
    ...
    bin_dir = local_mod._resolve_marcel_bin_dir()
    if not bin_dir or not os.path.isfile(
        os.path.join(bin_dir, "marcel.exe" if os.name == "nt" else "marcel")
    ):
        pytest.skip("no real marcel console-script install available")

    # Simulate the service-manager minimal PATH: marcel dir absent.
    minimal_path = os.pathsep.join(["/usr/bin", "/bin"])
    monkeypatch.setenv("PATH", minimal_path)
    assert shutil.which("marcel", path=minimal_path) is None

    env = build_subprocess_env(scrub_secrets=True)  # cron _run_job_script path

    resolved = shutil.which("marcel", path=env.get("PATH", ""))
    assert resolved is not None, (
        f"bare 'marcel' must resolve from the child PATH {env.get('PATH')!r}"
    )
