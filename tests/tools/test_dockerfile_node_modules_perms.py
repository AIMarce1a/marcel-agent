...
Older images made /opt/marcel/ui-tui and /opt/marcel/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV MARCEL_TUI_DIR=/opt/marcel/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R marcel:marcel /opt/marcel/ui-tui" not in text
    assert "chown -R marcel:marcel /opt/marcel/node_modules" not in text
