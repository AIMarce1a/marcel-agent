...
``marcel_constants.get_subprocess_home()`` — the same policy the terminal tool
...
class TestExpandTilde:
    ...
    def test_tilde_expands_to_profile_home(self):
        ...
        with patch("marcel_constants.get_subprocess_home", return_value="/opt/data/profiles/coder/home"):
...
class TestResolvePathUsesProfileHome:
    ...
    def test_relative_tilde_resolves_to_profile_home(self, tmp_path, monkeypatch):
        ...
        with patch("marcel_constants.get_subprocess_home", return_value=str(profile_home)):
