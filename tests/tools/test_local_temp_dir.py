...
Marcel exposes ``terminal.temp_dir`` (mirrored to ``TERMINAL_TEMP_DIR``) so
...
def test_default_is_marcel_cache_not_tmp(tmp_path, monkeypatch):
    """With no overrides at all, the default temp root is real storage under
    MARCEL_HOME (cache/terminal), NOT tmpfs /tmp."""
    import marcel_constants  # noqa: F401 — resolves MARCEL_HOME per call
    ...
    assert result == str(tmp_path / ".marcel" / "cache" / "terminal")
...
def test_cleanup_terminal_temp_cache(tmp_path, monkeypatch):
    ...
    assert not (root / "marcel_bg_dead1.log").exists()
