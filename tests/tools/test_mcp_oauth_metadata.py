...
Covers:
- :class:`MarcelTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.MarcelMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _MARCEL_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# MarcelTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = MarcelTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("MARCEL_HOME", str(tmp_path))
        storage = MarcelTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (MarcelMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: MarcelTokenStorage, **context_attrs):
    ...
    provider = _MARCEL_PROVIDER_CLS.__new__(_MARCEL_PROVIDER_CLS)
    provider._marcel_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MARCEL_HOME", str(tmp_path))
        storage = MarcelTokenStorage("mgr-srv")
        ...
        with patch.object(
            _MARCEL_PROVIDER_CLS.__bases__[-1], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MARCEL_HOME", str(tmp_path))
        storage = MarcelTokenStorage("flow-srv")
        ...
        with patch.object(
            _MARCEL_PROVIDER_CLS.__bases__[-1],
            "async_auth_flow",
