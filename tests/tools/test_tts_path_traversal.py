...
def test_output_path_rejects_marcel_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    marcel_home = tmp_path / "marcel-home"
    marcel_home.mkdir()
    monkeypatch.setattr(file_safety, "_marcel_home_path", lambda: marcel_home)
    monkeypatch.setattr(file_safety, "_marcel_root_path", lambda: marcel_home)

    target = marcel_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    marcel_home = tmp_path / "marcel-home"
    token_dir = marcel_home / "mcp-tokens"
