...
was gated on ``[ -f "$MARCEL_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'MARCEL_HOME="{home}"\n'
        # In tests we run unprivileged; as_marcel is a passthrough then.
        'as_marcel() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Marcel loads $MARCEL_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/marcel/.env.example -> $MARCEL_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $MARCEL_HOME/.env from it on first boot"
    )
