...
class TestWriteDenyExactPaths:
    ...
    def test_marcel_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.marcel/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "marcel_root"
        ...
        # Sanity check: MARCEL_HOME does point to the profile dir, not the root.
        from marcel_constants import get_marcel_home, get_default_marcel_root
...
class TestWriteAllowed:
    ...
    def test_marcel_control_files_requested_writable(self):
