...
class GetSpillConfigTests(unittest.TestCase):
    ...
    def test_load_config_exception_is_swallowed(self):
        with patch("marcel_cli.config.load_config", side_effect=RuntimeError("bad")):
            cfg = hos.get_spill_config()
        self.assertEqual(cfg["max_chars"], hos.DEFAULT_MAX_CHARS)
        self.assertTrue(cfg["enabled"])
...
class SpillIfOversizedTests(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="marcel-spill-test-")
    ...
    def test_default_directory_uses_marcel_home(self):
        """When no directory override, spill under MARCEL_HOME/hook_outputs."""
        test_home = tempfile.mkdtemp(prefix="marcel-home-")
        try:
            with patch.dict(os.environ, {"MARCEL_HOME": test_home}):
                # Also patch get_marcel_home to the env var to mirror production.
            ...
            # ~/.marcel/hook_outputs depending on get_marcel_home behaviour.
            candidates = [
                Path(test_home) / "hook_outputs" / "sess",
                Path(os.path.expanduser("~/.marcel/hook_outputs/sess")),
            ]
