...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_env._scrub_child_env — broad MARCEL_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: MARCEL_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_marcel_allowlist_and_secret_blocks():
    from tools.code_execution_env import _scrub_child_env

    env = {
        # operational allowlist → kept
        "MARCEL_HOME": "/h", "MARCEL_PROFILE": "p",
        "MARCEL_CONFIG": "/c.yaml", "MARCEL_ENV": "/e",
        "MARCEL_DELEGATED_CHILD_CONTEXT": "1",
        # other MARCEL_* → dropped (broad prefix removed)
        "MARCEL_BASE_URL": "https://x", "MARCEL_INTERACTIVE": "1",
        "MARCEL_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "MARCEL_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "MARCEL_BASE_URL", "MARCEL_INTERACTIVE", "MARCEL_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped MARCEL_* vars."""
    import logging
