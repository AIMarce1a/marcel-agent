...
def _reset_backend():
    ...
    with patch.dict(os.environ, {"MARCEL_COMPUTER_USE_BACKEND": "noop"}, clear=False):
        yield
    reset_backend_for_tests()
...
class TestRegistration:
    ...
    def test_cua_driver_cmd_env_override_is_resolved_dynamically(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MARCEL_CUA_DRIVER_CMD", str(driver))
...
class TestLazyMcpInstall:
    ...
    def test_start_reports_incompatible_existing_driver_before_mcp_setup(self):
        ...
        with patch.object(
            ...
            with pytest.raises(RuntimeError, match="marcel computer-use install"):
                cua_backend.CuaDriverBackend().start()
...
class TestContractAutoRepair:
    ...
    installed driver (a state Marcel' own version-floor bump created),
    ...
    def test_start_auto_repairs_incompatible_driver(self, monkeypatch):
        ...
        with patch.object(
             ...
             patch("marcel_cli.tools_config.install_cua_driver",
    ...
    def test_explicit_override_is_never_repaired(self, monkeypatch):
        ...
        with patch.object(
             ...
             patch("marcel_cli.tools_config.install_cua_driver") as installer, \
             patch("tools.lazy_deps.ensure"):
            with pytest.raises(RuntimeError, match="MARCEL_CUA_DRIVER_CMD"):
                cua_backend.CuaDriverBackend().start()
...
class TestCuaDriverSessionReconnect:
    ...
    def test_reconnect_restores_public_label_before_replaying_read(self):
        ...
        bridge = FakeBridge()
        session = self._make_session(bridge)
        session._declared_session_id = "marcel-label"
        ...
        assert bridge.calls == [
            ("call", "list_apps", {}),
            ("call", "start_session", {"session": "marcel-label"}),
            ("call", "list_apps", {}),
        ]
...
class TestCuaEnvironmentScrubbing:
    ...
    def test_cua_session_sanitizes_provider_env_vars(self):
        ...
        The cua-driver MCP subprocess should not inherit Marcel-managed
...
class TestMcpInvocationResolution:
    ...
    the MCP subcommand doesn't require a Marcel patch.
    ...
    def test_falls_back_when_manifest_missing_command(self):
        ...
        resolved driver path (so MARCEL_CUA_DRIVER_CMD still wins)."""
        from unittest.mock import patch
        from tools.computer_use.cua_backend_driver import _resolve_mcp_invocation
...
class TestSessionLifecycle:
    """Surface gap (audit June 2026): Marcel never declared a cua-driver
