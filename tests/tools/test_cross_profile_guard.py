...
@pytest.fixture
def fake_marcel(tmp_path, monkeypatch):
    """Build a two-profile Marcel layout and point MARCEL_HOME at
    the marcel-security profile (matching the original-incident shape).
    """
    root = tmp_path / "fake-marcel"
    (root / "skills" / "shared-skill").mkdir(parents=True)
    (root / "skills" / "shared-skill" / "SKILL.md").write_text(
        "---\nname: shared-skill\ndescription: default copy.\n---\n"
    )

    sec_home = root / "profiles" / "marcel-security"
    (sec_home / "skills").mkdir(parents=True)

    coder_home = root / "profiles" / "coder"
    (coder_home / "skills").mkdir(parents=True)

    monkeypatch.setenv("MARCEL_HOME", str(sec_home))

    import marcel_constants
    monkeypatch.setattr(marcel_constants, "get_default_marcel_root", lambda: root)

    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_marcel_home_path", lambda: sec_home)
    monkeypatch.setattr(fs, "_marcel_root_path", lambda: root)
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_marcel):
        from tools.file_tools import write_file_tool
        target = fake_marcel["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_allowed_guard_retired(self, fake_marcel):
        ...
        from tools.file_tools import write_file_tool
        target = fake_marcel["root"] / "skills" / "shared-skill" / "SKILL.md"
    ...
    def test_non_marcel_path_unaffected(self, fake_marcel, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_allowed_guard_retired(self, fake_marcel):
    ...
    def test_cross_profile_patch_bypass(self, fake_marcel):
        from tools.file_tools import patch_tool
        target = fake_marcel["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_writes_through_guard_retired(self, fake_marcel):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_marcel, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_marcel["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up MARCEL_HOME (set in
        ...
        assert "not found in active profile 'marcel-security'" in err
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.marcel/profiles/<name>/."""
        # Don't set MARCEL_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_marcel_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_marcel_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_marcel):
        """When active profile is 'marcel-security', the prompt warns
        ...
        assert "Active Marcel profile" in src
        assert "cross_profile=True" not in src  # guard retired
        assert "~/.marcel/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Marcel profile: default" in src
        assert "Active Marcel profile: {active_profile}" in src
