...
entirely on the process environment: ``MARCEL_HOME`` says *where* to write and
...
import marcel_state
import marcel_state_guard

REPO_ROOT = Path(__file__).resolve().parents[2]

# Probe run in the child: resolve the REAL platform state root (not a
# hardcoded ~/.marcel — that root is %LOCALAPPDATA%\marcel on Windows) and
...
root = marcel_state._real_platform_state_root()
if root is None:
    print("NO-ROOT")
else:
    try:
        marcel_state._ensure_test_isolation(root / "state.db")
    except RuntimeError:
        print("REFUSED")
    else:
        print("ALLOWED")
...
def _scrubbed_env(**overrides):
    ...
    Also strips ``MARCEL_TEST_ISOLATION`` — the conftest-exported marker
    ...
    env = {
        ...
        and k not in ("MARCEL_HOME", "MARCEL_TEST_ISOLATION")
    }
...
class TestScrubbedChildEnvironment:
    def test_child_without_pytest_env_still_refuses_production_db(self):
        """The #82770 escape: no PYTEST_* and no MARCEL_HOME, yet still a test.

        This is the exact shape of the leak — the child resolves the real
        ``state.db`` because ``MARCEL_HOME`` is gone, and the env-only guard
    ...
    def test_env_bypass_lets_a_deliberate_child_through(self):
        ...
        """
        env = _scrubbed_env(**{marcel_state._STATE_DB_GUARD_BYPASS_ENV: "1"})
...
class TestPytestProcessRecognition:
    ...
    def test_recognises_pytest_invocations(self, cmdline):
        assert marcel_state_guard._process_looks_like_pytest(self._FakeProc(cmdline))

    @pytest.mark.parametrize(
        "cmdline",
        [
            ["marcel", "gateway", "start"],
            ["/usr/bin/python", "-m", "marcel_cli.main", "sessions", "list"],
            # A path that merely *contains* "pytest" is not a pytest process:
            # tmp paths like /tmp/pytest-of-dev/... show up in real argv.
            ["marcel", "run", "--file", "/tmp/pytest-of-dev/test0/input.txt"],
        ],
    )
    def test_ignores_non_pytest_invocations(self, cmdline):
        assert not marcel_state_guard._process_looks_like_pytest(self._FakeProc(cmdline))
    ...
    def test_unreadable_process_is_not_pytest(self):
        ...
        assert not marcel_state_guard._process_looks_like_pytest(_Denied())
