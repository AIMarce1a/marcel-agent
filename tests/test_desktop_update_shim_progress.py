...
def progress(tmp_path):
    ...
    status = tmp_path / "marcel-update-status"
...
def test_every_state_carries_a_clock(progress):
    ...
    for state, message in [
        ...
        ("done", ""),
        ("manual", "Reopen Marcel to finish."),
        ("error", "Update failed."),
    ]:
...
# Stands in for `marcel update`, and reports the stage that was on screen
...
n="$(cat "$MARCEL_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$MARCEL_TEST_CALLS"
for f in "$TMPDIR"/marcel-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$MARCEL_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$MARCEL_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "marcel-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    marcel = install_root / "venv" / "bin" / "marcel"
    marcel.write_text(FAKE_MARCEL)
    marcel.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "MARCEL_TEST_CAPTURE": str(capture),
        "MARCEL_TEST_CALLS": str(calls),
        "MARCEL_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".marcel-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`marcel update` is the longest wait in the hand-off and the one the
