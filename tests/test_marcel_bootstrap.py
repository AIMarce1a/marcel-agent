"""Tests for marcel_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Marcel entry point
(marcel, marcel-agent, marcel-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Marcel entry point imports marcel_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported marcel_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("marcel_bootstrap", None)
    import marcel_bootstrap  # noqa: WPS433
    return marcel_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Marcel CLI
...
class TestEntryPointsImportBootstrap:
    """Every Marcel entry point must import marcel_bootstrap as its
    ...
    # Entry points that invoke Marcel as a process.  Each one must
    # import marcel_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "marcel_cli/main.py",   # marcel CLI (console_script)
        "run_agent.py",          # marcel-agent (console_script)
        "acp_adapter/entry.py",  # marcel-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import marcel_bootstrap' and that
        ...
        half-finished ``marcel update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``marcel_bootstrap`` as a top-level module) leaves marcel
        ...
        """
        # Resolve relative to the marcel-agent repo root.  Tests live
        # at tests/test_marcel_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # marcel start even when marcel_bootstrap hasn't been
        ...
        assert first_import_name == "marcel_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'marcel_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import marcel_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Marcel's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("MARCEL_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["MARCEL_PYTHON_SRC_ROOT"] = env
            elif "MARCEL_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["MARCEL_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/marcel")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("MARCEL_PYTHON_SRC_ROOT", None)
            else:
                os.environ["MARCEL_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/marcel", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Marcel root.  The guard must relocate Marcel to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/marcel"])
        assert result[0] == "/opt/marcel"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Marcel root.
        assert result.index("/opt/marcel") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["MARCEL_PYTHON_SRC_ROOT"] = "/env/marcel"
            hb.harden_import_path()
            assert sys.path[0] == "/env/marcel"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("MARCEL_PYTHON_SRC_ROOT", None)
