"""Shared fixtures for the marcel-agent test suite.
...
2. **Isolated MARCEL_HOME.** MARCEL_HOME points to a per-test tempdir so
   code reading ``~/.marcel/*`` via ``get_marcel_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".marcel"`` instead of the canonical
   ``get_marcel_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No MARCEL_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox MARCEL_HOME before ANY test module is imported ──────────────────
# `marcel_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_marcel_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.marcel/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes MARCEL_HOME, but fixtures run
...
# the REAL Marcel root — capture it BEFORE the sandbox rewires MARCEL_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.marcel (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("MARCEL_KANBAN_HOME", "").strip()
_PRE_SANDBOX_MARCEL_HOME = os.environ.get("MARCEL_HOME", "")


def _marcel_home_points_at_production(value: str) -> bool:
    """True when a pre-set MARCEL_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    MARCEL_HOME=~/.marcel``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``marcel_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.marcel`` — the escape vector that landed pytest
    ...
    MARCEL_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".marcel").resolve()
    except Exception:
        return True
...
if _marcel_home_points_at_production(os.environ.get("MARCEL_HOME", "")):
    _SESSION_MARCEL_HOME = tempfile.mkdtemp(prefix="marcel-test-home-")
    os.environ["MARCEL_HOME"] = _SESSION_MARCEL_HOME
    atexit.register(shutil.rmtree, _SESSION_MARCEL_HOME, True)
...
# like a real CLI") — which used to disarm marcel_state's live-DB guard in
# the child at the same moment the child lost the MARCEL_HOME redirect.
# MARCEL_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# marcel_state_guard._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export MARCEL_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["MARCEL_TEST_ISOLATION"] = os.environ.get("MARCEL_HOME", "") or "1"

#: MARCEL_HOME as it stood when conftest was imported - i.e. before any test
...
MARCEL_HOME_AT_CONFTEST_IMPORT = os.environ.get("MARCEL_HOME", "")
...
# MARCEL_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_MARCEL_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``marcel_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Marcel agent session inherits them and
    # marcel_constants home-resolution helpers prefer them over monkeypatched
    # HOME (test_subprocess_home_isolation red locally, green on CI).
    "MARCEL_REAL_HOME",
    "TERMINAL_HOME_MODE",
    "MARCEL_INTERACTIVE",
    "MARCEL_QUIET",
    "MARCEL_TOOL_PROGRESS",
    "MARCEL_TOOL_PROGRESS_MODE",
    "MARCEL_MAX_ITERATIONS",
    "MARCEL_SESSION_PLATFORM",
    "MARCEL_SESSION_CHAT_ID",
    "MARCEL_SESSION_CHAT_NAME",
    "MARCEL_SESSION_CHAT_TYPE",
    "MARCEL_SESSION_THREAD_ID",
    "MARCEL_SESSION_SOURCE",
    "MARCEL_SESSION_KEY",
    "MARCEL_GATEWAY_SESSION",
    "MARCEL_CRON_SESSION",
    "_MARCEL_GATEWAY",
    "MARCEL_PLATFORM",
    "MARCEL_MODEL",
    "MARCEL_INFERENCE_MODEL",
    "MARCEL_INFERENCE_PROVIDER",
    "MARCEL_TUI_PROVIDER",
    "MARCEL_MANAGED",
    "MARCEL_MANAGED_DIR",
    "MARCEL_DEV",
    "MARCEL_CONTAINER",
    "MARCEL_EPHEMERAL_SYSTEM_PROMPT",
    "MARCEL_TIMEZONE",
    "MARCEL_REDACT_SECRETS",
    "MARCEL_BACKGROUND_NOTIFICATIONS",
    "MARCEL_EXEC_ASK",
    "MARCEL_HOME_MODE",
    "MARCEL_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.marcel/kanban.db instead of the per-test MARCEL_HOME.
    "MARCEL_KANBAN_DB",
    "MARCEL_KANBAN_BOARD",
    "MARCEL_KANBAN_HOME",
    "MARCEL_KANBAN_WORKSPACES_ROOT",
    "MARCEL_KANBAN_LOGS_ROOT",
    "MARCEL_KANBAN_TASK",
    "MARCEL_KANBAN_WORKSPACE",
    "MARCEL_KANBAN_RUN_ID",
    "MARCEL_KANBAN_CLAIM_LOCK",
    "MARCEL_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    "MARCEL_DELEGATED_CHILD_CONTEXT",
    "MARCEL_TENANT",
    ...
    # otherwise-unrelated config tests away from the default "marcel" host.
    "MARCEL_HONCHO_HOST",
    ...
    "MARCEL_DASHBOARD_OAUTH_CLIENT_ID",
    "MARCEL_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and MARCEL_HOME to per-test tempdirs so code that
    reads ``~/.marcel/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral MARCEL_* vars that could change test semantics.
    for name in _MARCEL_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("MARCEL_HONCHO_HOST", "marcel")

    # 3. Redirect MARCEL_HOME to a per-test tempdir. Code that reads
    #    ``~/.marcel/*`` via ``get_marcel_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.marcel/*`` via
    #    ``Path.home() / ".marcel"`` instead of ``get_marcel_home()``
    #    is a bug to fix at the callsite.
    fake_marcel_home = tmp_path / "marcel_test"
    fake_marcel_home.mkdir()
    (fake_marcel_home / "sessions").mkdir()
    (fake_marcel_home / "cron").mkdir()
    (fake_marcel_home / "memories").mkdir()
    (fake_marcel_home / "skills").mkdir()
    monkeypatch.setenv("MARCEL_HOME", str(fake_marcel_home))
    ...
    # marcel_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("MARCEL_TEST_ISOLATION", str(fake_marcel_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("MARCEL_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. marcel_state computes ``DEFAULT_DB_PATH = get_marcel_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from marcel_state import ...``) that
    ...
    marcel_state_mod = sys.modules.get("marcel_state")
    if marcel_state_mod is not None and hasattr(marcel_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            marcel_state_mod, "DEFAULT_DB_PATH", fake_marcel_home / "state.db"
        )
    ...
    monkeypatch.setenv("MARCEL_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.marcel/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import marcel_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path MARCEL_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_marcel_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from marcel_cli import kanban_db_dispatch as _kbd_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.marcel. This autouse
...
# allow-list because test-level fixtures legitimately move MARCEL_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session MARCEL_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``MARCEL_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Marcel root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_MARCEL_HOME and not _marcel_home_points_at_production(
        _PRE_SANDBOX_MARCEL_HOME
    ):
        # MARCEL_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_marcel_root().resolve()
    # No pre-existing MARCEL_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".marcel").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.marcel`` captured at import time. Hermetic tests that legitimately
    move MARCEL_HOME to sibling tempdirs are unaffected.

    Only patches when ``marcel_cli.kanban_db_connect`` is *already imported*
    ...
    """
    _kdb = sys.modules.get("marcel_cli.kanban_db")
    _kdbc = sys.modules.get("marcel_cli.kanban_db_connect")
    if _kdb is None or _kdbc is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import marcel_cli.kanban_db`
    ...
    def _guarded_connect(db_path=None, *args, **kwargs):
        ...
        raise RuntimeError(
            ...
            f"to the real ~/.marcel. See #69283."
        )
...
# ``marcel_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Marcel root. This fixture wires the
# test-side knobs:
...
#     installs where MARCEL_HOME is not ~/.marcel) into the guard's
...
# so subprocess children that import marcel_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("marcel_state")
    ...
    if _PRE_SANDBOX_MARCEL_HOME and not _marcel_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_MARCEL_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Marcel home override redirects every later
    # ``get_marcel_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from marcel_constants import get_marcel_home_override, set_marcel_home_override

        if get_marcel_home_override() is not None:
            set_marcel_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal marcel config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``marcel-gateway`` process
...
# correlated with a ``tests/marcel_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> marcel-gateway``
...
def _wal_is_usable() -> bool:
    """True when Marcel will actually put a database into WAL mode here.

    Marcel refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Marcel can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Marcel managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``marcel_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_marcel_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_marcel_home`` fixture
    redirects ``MARCEL_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.marcel/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``marcel_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["MARCEL_VOICE_TTS"] = "1"``. The
...
#      ``marcel_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``marcel_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``marcel_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``marcel_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Marcel runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.marcel``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Marcel falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Marcel uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart marcel-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting marcel/python patterns are also blocked.
    """
    ...
    def _matches_marcel_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _MARCEL_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_marcel_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch marcel-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m marcel_cli.main".
                if (
                    "marcel" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live marcel-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting marcel/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `marcel update` (or the
        # equivalent `python -m marcel_cli.main update`).  These commands
        ...
        if "update" in low and (
            # marcel update / marcel update --gateway / setsid bash -c ... marcel update
            ("marcel" in low and "update" in low.split())
            or
            # python -m marcel_cli.main update --gateway
            ("marcel_cli" in low and "update" in low.split())
            or
            # venv/bin/marcel update  (absolute path variant used in tests)
            (".venv/bin/marcel" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`marcel update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_MARCEL_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import marcel_cli.voice as _voice
    except Exception:
