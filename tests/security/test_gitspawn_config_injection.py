...
``[diff "x"] command=/textconv=`` driver. Marcel gathers workspace context by
...
context-gathering git path Marcel runs neutralizes every sink. They use a real
...
def test_goals_fingerprint_is_safe(malicious_repo):
    from marcel_cli.goals import workspace_fingerprint
    repo, marker = malicious_repo
...
def test_web_git_diff_is_safe(malicious_repo):
    from marcel_cli import web_git
    repo, marker = malicious_repo
