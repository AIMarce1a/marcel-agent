...
def test_publish_marks_evidence_upload_failure_in_pr_comment(tmp_path, monkeypatch):
    comment = {
        "id": 123,
        "body": "before\n<!-- marcel-e2e-evidence:start -->\npending\n<!-- marcel-e2e-evidence:end -->\nafter",
    }
    updates = []
...
def test_find_review_comment_requires_the_evidence_marker():
    pending = "<!-- marcel-ci-review-bot -->\n<!-- marcel-e2e-evidence:start -->\npending\n<!-- marcel-e2e-evidence:end -->"

    assert _mod._find_review_comment([{"body": "<!-- marcel-ci-review-bot --> no evidence"}]) is None
