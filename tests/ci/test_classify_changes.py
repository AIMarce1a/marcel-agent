...
CASES = {
    ...
    "product python → nix": (["marcel_cli/config.py"], _lanes(python=True, scan=True)),
    ...
    # files change — not on every marcel_state.py PR.
    ...
    "python source alone → no desktop_updater lane": (["marcel_state.py"], _lanes(python=True, scan=True)),
