...
The ``marcel update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m marcel_cli.main gateway run`` — image name
    ``pythonw``, not ``marcel.exe`` — so the ``taskkill /IM marcel.exe`` guard
    ...
    # The marcel.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM marcel.exe' in text
