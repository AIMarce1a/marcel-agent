...
marcel_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # marcel_time checks an env override first; ensure it's unset so config wins.
    ...
    assert marcel_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (MARCEL_TIMEZONE, MARCEL_REDACT_SECRETS, MARCEL_MAX_ITERATIONS,
    ...
    # writing MARCEL_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
