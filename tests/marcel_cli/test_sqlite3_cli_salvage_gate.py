...
vulnerable sqlite3 CLI at a live Marcel database.
...
Marcel' own corruption banners used to instruct exactly that command
(`sqlite3 ~/.marcel/state.db ".recover"`). The fix routes operators to
`marcel sessions recover --source ...`, whose lane snapshots the damaged
...
from marcel_cli.session_lost_and_found import (
    _parse_sqlite3_cli_version,
    _wal_reset_vulnerable,
    find_sqlite3_cli,
    find_sqlite3_cli_refusal,
)
from marcel_cli.sqlite_runtime import is_sqlite_wal_reset_vulnerable


LIVE_DB_SALVAGE_COMMAND = 'sqlite3 ~/.marcel/state.db ".recover"'
...
class TestFindSqlite3CliRefusal:
    def test_missing_binary_refusal(self, monkeypatch):
        monkeypatch.setattr(
            "marcel_cli.session_lost_and_found.shutil.which", lambda _: None
        )
    ...
    def test_no_dbpage_refusal(self, monkeypatch):
        monkeypatch.setattr(
            "marcel_cli.session_lost_and_found.shutil.which",
            lambda _: "/usr/bin/sqlite3",
        )
        monkeypatch.setattr(
            "marcel_cli.session_lost_and_found._cli_supports_recover",
            lambda _: False,
        )
        assert find_sqlite3_cli() is None
    ...
    def test_wal_reset_vulnerable_refusal(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "marcel_cli.session_lost_and_found._parse_sqlite3_cli_version",
            lambda _: (3, 45, 1),
        )
        assert find_sqlite3_cli() is None
...
class TestParseSqlite3CliVersion:
    def test_parses_modern_output(self):
        ...
        with patch(
            "marcel_cli.session_lost_and_found.subprocess.run",
            return_value=Probe(),
        ):
...
class TestGuidanceNeverNamesLiveDb:
    ...
    def test_run_agent_corrupt_explanation(self):
        ...
        assert "marcel sessions recover --source" in explanation
    ...
    def test_repair_budget_error_names_safe_lane(self, tmp_path: Path):
        import marcel_state_repair as marcel_state  # helper lives in the split-out repair module

        message = marcel_state._persistent_repair_exhausted_error(
            tmp_path / "state.db"
        )
    ...
    def test_forensic_backup_refusals_name_safe_lane(self):
        ...
        body = inspect.getsource(marcel_state_repair._backup_db_file)
        assert ".recover\"`" not in body
        assert "sessions recover --source" in body
        assert "--inspect-only" in body

    def test_kanban_manual_recovery_warns_about_live_db(self):
        import marcel_cli.kanban_ops as kanban  # ``_cmd_repair`` lives here (split from marcel_cli.kanban)
...
# printed `marcel sessions recover --source <db>` — which cmd_sessions
...
class TestEmittedCommandsSatisfyCliContract:
    ...
    `marcel sessions recover --source <db>`, which cmd_sessions rejects
    ...
    the real `cmd_sessions` (the same function `marcel` main() hands the
    ...
    def _namespace(source: Path, **overrides) -> "argparse.Namespace":
        """The namespace marcel main() produces for `sessions recover`.

        Mirrors the registrations in marcel_cli/main.py (sessions_recover
    ...
    def test_old_v1_shape_is_still_rejected(self, tmp_path):
        ...
        import marcel_cli.sessions_cmd as sc
