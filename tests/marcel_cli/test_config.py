"""Tests for marcel_cli configuration management."""
    ...
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            if sys.platform == "win32":
                # Windows default is %LOCALAPPDATA%\marcel — see
                # marcel_constants._get_platform_default_marcel_home.
                ...
                assert home == base / "marcel"
            else:
                assert home == Path.home() / ".marcel"


class TestEnsureMarcelHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from marcel_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
    ...
    _PRE_REWRITE_DEFAULT_SOUL = (
        "You are Marcel Agent, an intelligent AI assistant created by Nous "
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``marcel logs`` surfaces it)
    ...
    def test_corrupt_config_is_backed_up(self, tmp_path, capsys):
        ...
        """
        from marcel_cli import config as cfg_mod
...
class TestSaveAndLoadRoundtrip:
    ...
    def test_atomic_config_write_refuses_unparseable_existing_config(self, tmp_path):
        ...
        from marcel_cli.config import atomic_config_write
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_quotes_values_containing_hash(self, tmp_path):
        ...
        with patch.dict(os.environ, {"MARCEL_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"MARCEL_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"MARCEL_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"MARCEL_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("marcel_cli.config.sanitize_env_file", return_value=2),
            patch(
                "marcel_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("marcel_cli.config.read_raw_config", return_value={}),
            patch("marcel_cli.config.get_missing_env_vars", return_value=[]),
            patch("marcel_cli.config.get_missing_config_fields", return_value=[]),
            patch("marcel_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from marcel_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """MARCEL_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `marcel setup`) lets a
        ...
        via config.yaml; MARCEL_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from marcel_cli.config import OPTIONAL_ENV_VARS
        assert "MARCEL_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `marcel setup` surface it (previously only Honcho was listed, leaving
...
class TestConfigSupportFloor:
    ...
    def test_v11_config_is_refused_and_untouched(self, tmp_path, capsys):
        ...
        assert "run `marcel setup` to regenerate" in captured.out
    ...
    def test_floor_message_uses_display_marcel_home(self):
        from marcel_cli.config_migrations import support_floor_message
        from marcel_constants import display_marcel_home

        msg = support_floor_message()
        assert f"{display_marcel_home()}/config.yaml" in msg
...
class TestCustomProviderCompatibility:
    ...
    def _run_ladder(current_ver: int):
        from marcel_cli.config_migrations import run_migrations
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or selected Marcel runtime/security controls.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Marcel
    process on next startup via the dotenv → ``os.environ`` chain in
    ``marcel_cli/env_loader.py``.
    ...
    def _marcel_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        "allowed_key",
        [
            "MARCEL_LANGFUSE_PUBLIC_KEY",
            "MARCEL_SPOTIFY_CLIENT_ID",
            "MARCEL_QWEN_BASE_URL",
            "MARCEL_MAX_ITERATIONS",
        ],
    )
    def test_marcel_integration_keys_still_writable(self, allowed_key):
        """``MARCEL_*`` overall is NOT blocked.
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "MARCEL_COPILOT_ACP_COMMAND",
            "MARCEL_COPILOT_ACP_ARGS",
            "MARCEL_YOLO_MODE",
            "MARCEL_ACCEPT_HOOKS",
            ...
            "MARCEL_CRON_SESSION",
            "MARCEL_SINGLE_QUERY_SESSION",
            "MARCEL_SESSION_KEY",
            "MARCEL_SESSION_PLATFORM",
        ],
    )
    def test_marcel_security_control_keys_are_not_writable(self, protected_key):
    ...
    def test_preexisting_optional_mcps_override_still_loads(self, tmp_path):
        ...
        from marcel_cli.config import invalidate_env_cache

        catalog = tmp_path / "custom-mcp-catalog"
        (tmp_path / ".env").write_text(
            f"MARCEL_OPTIONAL_MCPS={catalog}\n",
            encoding="utf-8",
        )
        invalidate_env_cache()

        assert load_env()["MARCEL_OPTIONAL_MCPS"] == str(catalog)

    @pytest.mark.parametrize(
        ("key", "expected"),
        [
            ("Path", "PATH"),
            ("Marcel_Yolo_Mode", "MARCEL_YOLO_MODE"),
            ("Marcel_Optional_Mcps", "MARCEL_OPTIONAL_MCPS"),
            ("Marcel_Copilot_Acp_Command", "MARCEL_COPILOT_ACP_COMMAND"),
            ("Marcel_Copilot_Acp_Args", "MARCEL_COPILOT_ACP_ARGS"),
        ],
    )
    def test_windows_policy_names_are_case_insensitive(self, key, expected):
        from marcel_cli.config import _env_var_policy_name
    ...
    def test_windows_env_assignment_matching_is_case_insensitive(self, prefix):
        from marcel_cli.config import _env_line_defines_key
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from marcel_cli.config import _persist_migration, read_raw_config
    ...
    def test_preserves_existing_codex_app_server_auto_value(self, tmp_path):
        with patch.dict(os.environ, {"MARCEL_HOME": str(tmp_path)}):
            self._write(
                ...
                "_config_version: 31\n"
                "compression:\n"
                "  codex_app_server_auto: marcel\n",
            )

            migrate_config(interactive=False, quiet=True)

            raw = yaml.safe_load((tmp_path / "config.yaml").read_text())
            assert raw["compression"]["codex_app_server_auto"] == "marcel"
...
def test_default_config_has_no_duplicate_top_level_keys():
    ...
    import ast
    import marcel_cli.config as cfg_mod
...
class TestConfigCommandFailClosedSurface:
    """`marcel config set/unset` must exit cleanly (no traceback) when the
