"""Tests for marcel_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $MARCEL_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Marcel-managed node dir.
"""
...
def _make_marcel_node(marcel_home: Path) -> Path:
    """Create a fake $MARCEL_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = marcel_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    marcel_home = fake_home / ".marcel"
    _make_marcel_node(marcel_home)
    ...
    removed = uninstall.remove_node_symlinks(marcel_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    marcel_home = fake_home / ".marcel"
    node_bin = _make_marcel_node(marcel_home)
