...
Canonical owners:
  ...
  * ``marcel_cli/config.py`` — ``load_config()`` / ``load_config_readonly()``
...
ALLOWLIST = {
    ...
    # gateway.run._marcel_home (delegates to read_raw_config otherwise).
    "gateway/run.py",
    # Reads the MANAGED-scope config.yaml (/etc/marcel/...), not the user's —
    # it IS the overlay source; the canonical loaders call into it.
    "marcel_cli/managed_scope.py",
