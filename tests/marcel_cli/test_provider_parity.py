...
the SAME provider universe as ``marcel model`` (the CLI/TUI picker).
...
def test_every_marcel_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `marcel model` universe."""
    ...
    assert not missing, (
        "providers shown in `marcel model` but not configurable in the desktop "
