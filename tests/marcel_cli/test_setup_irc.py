"""Tests for IRC gateway configuration via `marcel setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Marcel install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Marcel install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#marcel")
            monkeypatch.setenv("IRC_NICKNAME", "marcel-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `marcel setup gateway` experience with IRC present."""
