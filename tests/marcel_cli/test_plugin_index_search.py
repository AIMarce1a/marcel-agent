...
(remote → cache → seed), `marcel plugins search --json`, and install-time
...
from marcel_cli import plugin_index
from marcel_cli.plugin_index import (
    PluginIndexEntry,
    _parse_entries,
    load_index,
    resolve_name,
...
def marcel_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(plugin_index, "get_marcel_home", lambda: tmp_path)
    return tmp_path
...
class TestSearch:
    ...
    def test_fuzzy_typo_tolerance(self):
        results = search_index(self.entries, "marcel-media-studo")
...
class TestLoadIndex:
    def test_fresh_cache_wins_without_network(self, marcel_home, monkeypatch):
        _write_cache(marcel_home, SAMPLE)
    ...
    def test_expired_cache_triggers_remote(self, marcel_home, monkeypatch):
        _write_cache(marcel_home, SAMPLE, age_seconds=plugin_index.INDEX_CACHE_TTL + 60)
    ...
    def test_remote_failure_falls_back_to_stale_cache(self, marcel_home, monkeypatch):
        _write_cache(marcel_home, SAMPLE, age_seconds=plugin_index.INDEX_CACHE_TTL + 60)
    ...
    def test_no_cache_no_remote_falls_back_to_seed(self, marcel_home, monkeypatch):
        monkeypatch.setattr(plugin_index, "_fetch_remote", lambda: None)
        entries, source = load_index()
        assert source == "seed"
        assert len(entries) >= 3

    def test_refresh_bypasses_fresh_cache(self, marcel_home, monkeypatch):
        _write_cache(marcel_home, SAMPLE)
    ...
    def test_offline_skips_network(self, marcel_home, monkeypatch):
    ...
    def test_corrupt_cache_ignored(self, marcel_home, monkeypatch):
        cache = marcel_home / "cache" / "plugin_index.json"
        cache.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_remote_fetch_writes_cache(self, marcel_home, monkeypatch):
...
class TestResolveName:
    ...
    def test_case_insensitive(self):
        entry, _ = resolve_name(self.entries, "Marcel-Media-Studio")
    ...
    def test_ambiguous_partial(self):
        entry, candidates = resolve_name(self.entries, "marcel")
...
class TestInstallResolution:
    def test_bare_name_detection(self):
        from marcel_cli.plugins_cmd import _looks_like_bare_index_name
    ...
    def test_install_resolves_name_and_pins_ref(self, marcel_home, monkeypatch):
        from marcel_cli import plugins_cmd
    ...
    def test_install_explicit_ref_beats_index_pin(self, marcel_home, monkeypatch):
        from marcel_cli import plugins_cmd
    ...
    def test_install_ambiguous_name_lists_candidates_and_exits(
        self, marcel_home, monkeypatch, capsys
    ):
        from marcel_cli import plugins_cmd
        ...
        with pytest.raises(SystemExit) as exc:
            plugins_cmd.cmd_install("marcel", enable=False)
    ...
    def test_install_unknown_name_exits(self, marcel_home, monkeypatch, capsys):
        from marcel_cli import plugins_cmd
    ...
    def test_owner_repo_passthrough_skips_index(self, marcel_home, monkeypatch):
...
class TestCmdSearch:
    def test_json_output(self, marcel_home, monkeypatch, capsys):
        from marcel_cli import plugins_cmd
    ...
    def test_no_results_message(self, marcel_home, monkeypatch, capsys):
        from marcel_cli import plugins_cmd
    ...
    def test_dispatch_routes_search(self, marcel_home, monkeypatch):
        from marcel_cli import plugins_cmd
