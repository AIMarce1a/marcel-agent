...
`marcel update` migrated only the active profile's config.yaml; sibling
...
context-local MARCEL_HOME override.
...
import marcel_cli.update_cmd_config as update_cmd_config
...
def _setup(monkeypatch, tmp_path, active_home: Path):
    ...
    monkeypatch.setattr(
        marcel_constants, "get_process_marcel_home", lambda: active_home
    )
    monkeypatch.setattr(
...
def test_override_is_reset_after_run(monkeypatch, tmp_path):
    ...
    before = get_marcel_home_override()
    update_cmd._migrate_sibling_profile_configs()
    assert get_marcel_home_override() == before
