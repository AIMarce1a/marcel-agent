...
Regression guard for the bug where `marcel model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`marcel_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in MARCEL_OVERLAYS
        overlay = MARCEL_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `marcel_cli/main.py` (cmd_model): read
