"""Tests for `marcel memory setup [provider]` routing.
...
``marcel memory setup honcho``) without the interactive picker — which matters
because the per-provider ``marcel <provider>`` subcommand is only registered
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "marcel memory setup" in out
...
class TestInstallDependenciesRunner:
    ...
    def _run_with_missing_dep(self, tmp_path, which_side_effect, run_behavior=None):
        ...
        # The hermetic conftest sets MARCEL_DISABLE_LAZY_INSTALLS=1 so no test
        ...
        with _patch.dict(os.environ, {"MARCEL_DISABLE_LAZY_INSTALLS": "0"}), \
             patch("plugins.memory.find_provider_dir", return_value=tmp_path), \
             patch("marcel_cli.tools_config_cua.shutil.which", side_effect=which_side_effect), \
             patch("marcel_cli.tools_config_cua.subprocess.run", fake_run):
