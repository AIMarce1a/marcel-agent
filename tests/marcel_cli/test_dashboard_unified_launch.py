...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "marcel_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # MARCEL_HOME.  For a standard install (MARCEL_HOME unset) that root is
        # the platform-native default (~/.marcel), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from marcel_constants import get_default_marcel_root
        assert env.get("MARCEL_HOME") == str(get_default_marcel_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (MARCEL_DESKTOP=1) must NOT
