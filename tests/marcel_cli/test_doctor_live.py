"""Tests for ``marcel doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from marcel_cli import doctor_live
from marcel_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `marcel doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("marcel_cli.doctor.MARCEL_HOME", tmp_path / "home")
        monkeypatch.setattr("marcel_cli.doctor.PROJECT_ROOT", tmp_path / "root")
