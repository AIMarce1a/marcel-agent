...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from marcel_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiConfigRegistry:
    def test_optional_env_vars_include_gmi(self):
        ...
        # marcel model, not via upgrade prompts.
...
class TestGmiProvidersModule:
    def test_overlay_exists(self):
        ...
        assert "gmi" in MARCEL_OVERLAYS
        overlay = MARCEL_OVERLAYS["gmi"]
...
class TestGmiAuxiliary:
    def test_resolve_provider_client_uses_gmi_aux_default(self, monkeypatch):
        ...
        # GMI profile declares default_headers with a MarcelAgent User-Agent
        ...
        assert headers.get("User-Agent", "").startswith("MarcelAgent/")

    def test_gmi_profile_declares_marcel_user_agent(self):
        """The GMI plugin sets a MarcelAgent/<ver> User-Agent on its profile."""
        from providers import get_provider_profile

        profile = get_provider_profile("gmi")
        assert profile is not None
        ua = profile.default_headers.get("User-Agent", "")
        assert ua.startswith("MarcelAgent/"), (
            f"expected GMI profile User-Agent to start with 'MarcelAgent/', got {ua!r}"
        )
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("marcel_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
            "marcel_cli.main.cmd_chat",
            lambda args: recorded.setdefault("provider", args.provider),
        )
        monkeypatch.setattr(sys, "argv", ["marcel", "chat", "--provider", "gmi"])

        from marcel_cli.main import main
    ...
    def test_select_provider_and_model_routes_gmi_to_generic_flow(self, monkeypatch):
        ...
        monkeypatch.setattr("marcel_cli.auth.resolve_provider", lambda *args, **kwargs: None)
        ...
        monkeypatch.setattr("marcel_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
        monkeypatch.setattr("marcel_cli.main._model_flow_api_key_provider", fake_model_flow_api_key_provider)
    ...
    def test_model_flow_api_key_provider_persists_gmi_selection(self, monkeypatch):
        ...
        with patch(
            ...
            from marcel_cli.model_setup_flows import _model_flow_api_key_provider
        ...
        config = yaml.safe_load((get_marcel_home() / "config.yaml").read_text()) or {}
