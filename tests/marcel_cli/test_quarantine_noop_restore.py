...
``marcel*.exe`` shims aside (``marcel.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``marcel`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("marcel", "marcel-agent", "marcel-acp", "marcel-gateway"):
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "marcel.exe" in names, "marcel.exe must be restored after a no-op install"
    assert "marcel-acp.exe" in names
    assert "marcel-gateway.exe" in names
    assert (scripts / "marcel.exe").read_bytes() == b"MZ-old-marcel"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "marcel.exe").read_bytes() == b"MZ-new-marcel"
...
# ---------------------------------------------------------------------------
# marcel_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "marcel.exe" in names, "marcel.exe must be restored after a no-op recovery install"
    assert (scripts / "marcel.exe").read_bytes() == b"MZ-old-marcel"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "marcel.exe").write_bytes(b"MZ-new-marcel")
