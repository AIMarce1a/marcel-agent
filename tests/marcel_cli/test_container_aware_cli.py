...
def container_env(tmp_path, monkeypatch):
    """Set up a fake MARCEL_HOME with .container-mode file."""
    marcel_home = tmp_path / ".marcel"
    marcel_home.mkdir()
    monkeypatch.setenv("MARCEL_HOME", str(marcel_home))
    monkeypatch.delenv("MARCEL_DEV", raising=False)

    container_mode = marcel_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=marcel-agent\n"
        "exec_user=marcel\n"
        "marcel_bin=/data/current-package/bin/marcel\n"
    )
    return marcel_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("marcel_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "marcel-agent"
    assert info["exec_user"] == "marcel"
    assert info["marcel_bin"] == "/data/current-package/bin/marcel"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from marcel_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "marcel"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "marcel-agent" in cmd
    assert "/data/current-package/bin/marcel" in cmd
    assert "chat" in cmd
