"""Regression tests for #34107 — Docker UID/GID handling in ensure_marcel_home.

When Marcel runs in Docker with ``MARCEL_UID=1000`` / ``MARCEL_GID=911``,
the entrypoint chowns the top-level ``MARCEL_HOME`` once at startup. But
subdirectories created at runtime by ``ensure_marcel_home()`` — especially
...
The fix is a ``_chown_to_marcel_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_marcel_uid_gid
# ---------------------------------------------------------------------------


class TestResolveMarcelUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("MARCEL_UID", "1000")
        monkeypatch.setenv("MARCEL_GID", "911")
        from marcel_cli.config import _resolve_marcel_uid_gid
...
# ---------------------------------------------------------------------------
# _chown_to_marcel_uid
# ---------------------------------------------------------------------------


class TestChownToMarcelUid:
    def test_calls_os_chown_when_both_set(self, tmp_path, monkeypatch):
        monkeypatch.setenv("MARCEL_UID", "1000")
        monkeypatch.setenv("MARCEL_GID", "911")
        from marcel_cli import config as cfg

        d = tmp_path / "subdir"
        d.mkdir()

        with patch.object(cfg.os, "chown") as mock_chown:
            cfg._chown_to_marcel_uid(d)
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_marcel_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("MARCEL_UID", raising=False)
        monkeypatch.delenv("MARCEL_GID", raising=False)
        from marcel_cli import config as cfg
