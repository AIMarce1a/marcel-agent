...
The notice prints the most recent curator run summary on `marcel update`,
...
session start) so users normally never see the rename map. `marcel update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    from marcel_cli import update_cmd_maint
