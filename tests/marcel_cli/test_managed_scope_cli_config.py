...
marcel_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `marcel config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_marcel_home = get_marcel_home()`` at import time (module
    singleton), so monkeypatching MARCEL_HOME after import doesn't move it.
    ...
    this only matters for tests that swap MARCEL_HOME.)
    """
    import cli

    cli._marcel_home = home
