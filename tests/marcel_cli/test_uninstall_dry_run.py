...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_marcel_home", lambda: marcel_home)
    monkeypatch.setattr(uninstall, "_is_default_marcel_home", lambda home: False)
    ...
    assert str(marcel_home) in output
    assert project_root.exists()
    assert marcel_home.exists()
