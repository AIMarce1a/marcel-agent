...
Before the fix, ``marcel whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``marcel gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def _env_value(marcel_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from marcel_cli.main_platform_setup import cmd_whatsapp
    ...
    monkeypatch.setattr("marcel_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``marcel whatsapp`` with an existing paired session and
