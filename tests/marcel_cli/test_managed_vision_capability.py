...
def _stage(home_root, name):
    # Machine-scoped models dir (the shared root — tmp MARCEL_HOME IS the root here).
    from marcel_cli.local_runtime.bootstrap import models_dir
...
def test_not_ours_returns_none(marcel_home):
    from marcel_cli.local_runtime.capabilities import managed_model_supports_vision

    assert managed_model_supports_vision("gpt-4o") is None


def test_catalog_vision_model_with_projector_on_disk(marcel_home):
    """Staged catalog model with an mmproj present: True (server down —
    the catalog + on-disk projector answer)."""
    from marcel_cli.local_runtime.bootstrap import assets_dir
    from marcel_cli.local_runtime.capabilities import managed_model_supports_vision
    ...
    _stage(marcel_home, variant.model_id)
    adir = assets_dir()
...
def test_catalog_vision_model_missing_projector_is_blind(marcel_home):
...
def test_lookup_chain_consults_managed_runtime(marcel_home, monkeypatch):
    ...
    monkeypatch.setattr(
        "marcel_cli.local_runtime.capabilities.managed_model_supports_vision",
        lambda mid: True)
...
def test_webp_transcodes_to_png_for_managed_provider(marcel_home, monkeypatch, tmp_path):
...
def test_webp_passes_through_for_cloud_providers(marcel_home, monkeypatch, tmp_path):
...
def test_vision_analyze_normalization_narrows_for_managed(marcel_home, monkeypatch, tmp_path):
