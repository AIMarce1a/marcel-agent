"""Regression test: `marcel dashboard --tui` must not hard-crash.

Older Marcel desktop app shells (<= 0.15.x) spawn the backend as::

    marcel dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "Marcel couldn't
...
def _run_cli(args, timeout=60):
    """Invoke the real marcel_cli.main parser in a subprocess.
