...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (MARCEL_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via MARCEL_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in MARCEL_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from marcel_cli.providers import MARCEL_OVERLAYS
        assert "bedrock" in MARCEL_OVERLAYS
