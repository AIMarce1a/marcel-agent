...
``_reexec_dependency_sync_off_windows_shim`` with ``MARCEL_UPDATE_REEXEC=1``)
...
def _run_cmd_update(monkeypatch, impl, *, reexec: bool):
    ...
    monkeypatch.setattr("marcel_cli.config.detect_install_method", lambda root: "git")
    monkeypatch.setattr("marcel_cli.update_lock.UpdateLock", lambda: _FakeLock(events["order"]))
    ...
    monkeypatch.setattr("marcel_cli.update_receipt.finalize_pending_update_receipt", fake_receipt)
    monkeypatch.setattr("os._exit", fake_exit)
    if reexec:
        monkeypatch.setenv("MARCEL_UPDATE_REEXEC", "1")
    else:
        monkeypatch.delenv("MARCEL_UPDATE_REEXEC", raising=False)
