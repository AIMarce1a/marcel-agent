...
1. The standalone daemon path (``marcel kanban daemon --force`` /
   :func:`marcel_cli.kanban_db_dispatch.run_daemon`) never resolved
...
def test_run_daemon_resolves_and_passes_max_in_progress(
    ...
    ``marcel kanban dispatch`` paths both resolved the memory-derived
    default.
    """
    captured: dict = {}
...
def test_nonspawnable_review_does_not_tax_ready_budget(
    ...
    import marcel_cli.config as cfgmod
    import marcel_cli.profiles as profmod
