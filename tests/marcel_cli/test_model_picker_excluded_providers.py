...
interactive ``marcel model`` CLI picker.

The CLI picker (``marcel_cli.main.select_provider_and_model``) builds its
...
def test_cli_picker_hides_excluded_provider(config_home):
    ...
    from the ``marcel model`` provider menu."""
...
def test_cli_picker_hides_excluded_provider_by_alias(config_home):
    ...
    marcel_id / alias names."""
    # 'openai' is an alias-style marcel id; ensure excluding it hides the
    # canonical openai provider row if present. Use the canonical slug's
    # alias from _PROVIDER_ALIASES to stay robust to renames.
    from marcel_cli.models import _PROVIDER_ALIASES, CANONICAL_PROVIDERS
