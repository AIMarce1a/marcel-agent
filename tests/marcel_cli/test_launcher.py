"""Tests for the top-level `./marcel` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./marcel` should use `marcel_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "marcel"
    called = []

    fake_main_module = types.ModuleType("marcel_cli.main")

    def fake_main():
        called.append("marcel_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "marcel_cli.main", fake_main_module)
