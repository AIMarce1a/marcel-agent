...
def client(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MARCEL_DASHBOARD_SESSION_TOKEN", "soul-test-token")
...
def profile_dir(tmp_path, monkeypatch) -> Path:
    """Create a real profile directory under the test MARCEL_HOME."""
    monkeypatch.setenv("MARCEL_HOME", str(tmp_path))
...
class TestSoulWriteDurability:
    ...
    def test_existing_soul_survives_an_interrupted_save(
        ...
        # MARCEL_HOME patch the client/profile_dir fixtures installed.
...
class TestSoulIoIsOffTheEventLoop:
    ...
    ``tests/marcel_cli/test_web_profiles_off_loop.py``.
    """
