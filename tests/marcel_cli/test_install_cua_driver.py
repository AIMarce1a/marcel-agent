...
* Be supported-platform-only — no-op silently elsewhere so ``marcel update``
...
class TestCuaDriverRuntimeContract:
    def test_current_manifest_is_ready(self):
        from marcel_cli import tools_config_cua as tools_config
...
class TestInstallCuaDriverUpgrade:
    ...
    def test_install_target_writability_is_probed_for_real_on_macos(self):
        ...
        ``macos_only``: ``/Applications`` is the only install target Marcel
    ...
    def test_non_upgrade_repairs_incompatible_existing_driver(self):
        ...
        incompatible = {
            "ready": False,
            "version": "0.19.4",
            "reason": "Marcel computer use requires cua-driver 0.20.0 or newer",
        }
    ...
    def test_incompatible_explicit_override_is_not_replaced(self, monkeypatch):
        ...
        monkeypatch.setenv("MARCEL_CUA_DRIVER_CMD", "/opt/custom/cua-driver")
        incompatible = {
            "ready": False,
            "version": "0.19.4",
            "reason": "Marcel computer use requires cua-driver 0.20.0 or newer",
    ...
    def test_missing_explicit_override_does_not_install_standard_driver(
        ...
        monkeypatch.setenv("MARCEL_CUA_DRIVER_CMD", "/missing/custom/cua-driver")
        with patch.object(
                 tools_config,
                 "_resolved_cua_driver_cmd",
                 return_value=None,
...
class TestRequireConfirmedUpdate:
    """`marcel update` passes require_confirmed_update=True: the full
    ...
    Explicit `marcel computer-use install --upgrade` keeps the old
    ...
    def test_explicit_upgrade_still_falls_through_on_indeterminate(self):
        # `marcel computer-use install --upgrade` (default flag): the old
    ...
    def test_incompatible_driver_repairs_on_posix_despite_indeterminate_check(self):
        """Marcel' own version floor is the confirmation. When the installed
        driver fails the runtime contract, the `marcel update` refresh must
...
class TestUpdateCheckTimeoutDefaults:
    ...
    full reinstall fall-through during `marcel update`.
    """
...
class TestInstallerTimeoutDrainIsBounded:
    ...
    def test_drain_that_times_out_still_surfaces_the_run_timeout(self):
        ...
        so the existing manual re-run hint prints and ``marcel update``
...
class TestCuaVersionSummary:
    """`marcel computer-use status` prints one line, whatever the binary says.

    A binary chosen by MARCEL_CUA_DRIVER_CMD is under no obligation to answer
