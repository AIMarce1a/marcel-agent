...
import pytest
from marcel_cli import profile_cmd
...
def profiles():
    ...
    Sibling test files reload ``marcel_cli.profiles`` / ``marcel_cli.main``;
    a top-level ``from marcel_cli.profiles import ...`` here would bind the
    ...
    """
    return importlib.import_module("marcel_cli.profiles")
...
def test_custom_marcel_home_inside_a_checkout_uses_a_sibling_store(
    tmp_path, monkeypatch, profiles
):
    ...
    monkeypatch.setattr(profiles, "_get_default_marcel_home", lambda: checkout)
...
def test_checkout_detection_does_not_depend_on_cwd(tmp_path, monkeypatch, profiles):
    """MARCEL_HOME inside a checkout must be detected even when cwd is elsewhere.
    ...
    checkout.mkdir()
    (checkout / ".git").write_text("gitdir: ../git\n", encoding="utf-8")
    marcel_home = checkout / "data_home"
    marcel_home.mkdir()
    elsewhere = tmp_path / "elsewhere"
    elsewhere.mkdir()
    monkeypatch.chdir(elsewhere)  # cwd has no .git ancestor inside tmp_path
    monkeypatch.setattr(Path, "home", lambda: tmp_path / "home")
    monkeypatch.setattr(profiles, "_get_default_marcel_home", lambda: marcel_home)
...
def test_cli_export_rejects_bad_profile_name_without_traceback(
    ...
    main_mod = importlib.import_module("marcel_cli.main")
    default_home = tmp_path / ".marcel"
    default_home.mkdir()
    monkeypatch.setattr(profiles, "_get_default_marcel_home", lambda: default_home)
...
def test_cli_export_default_does_not_write_into_the_current_checkout(
    ...
    monkeypatch.setattr(profiles, "_get_default_marcel_home", lambda: default_home)
    monkeypatch.setattr(
        "marcel_constants.get_default_marcel_root", lambda: default_home
    )
...
def test_slash_export_uses_the_same_managed_destination(
    ...
    mixin_mod = importlib.import_module("marcel_cli.cli_commands_mixin")
    default_home = tmp_path / ".marcel"
    default_home.mkdir()
    monkeypatch.setattr(profiles, "_get_default_marcel_home", lambda: default_home)
...
async def test_profile_export_api_uses_the_shared_managed_destination(
    ...
    from marcel_cli.web_models import ProfileExport

    router_mod = importlib.import_module("marcel_cli.web_routers.profiles")
...
def test_cwd_in_unrelated_checkout_does_not_prove_safety(
    ...
    of a MARCEL_HOME inside checkout B."""
    ...
    monkeypatch.setattr(Path, "home", lambda: tmp_path / "home")
    monkeypatch.setattr(profiles, "_get_default_marcel_home", lambda: checkout_b)
