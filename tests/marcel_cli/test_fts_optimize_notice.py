...
def test_update_notice_offers_v1_trigram_tool_calls_rebuild(tmp_path, monkeypatch, capsys):
    ...
    class FakeSessionDB:
        ...
        _db_needs_fts_storage_upgrade = staticmethod(
            marcel_state.SessionDB._db_needs_fts_storage_upgrade
        )

    monkeypatch.setattr(marcel_constants, "get_marcel_home", lambda: tmp_path)
    monkeypatch.setattr(marcel_state, "SessionDB", FakeSessionDB)
    ...
    assert "marcel sessions optimize-storage" in capsys.readouterr().out
    conn.close()
