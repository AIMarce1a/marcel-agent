"""Tests for the `marcel memory reset` CLI command.
...
- Profile-scoped reset (uses MARCEL_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake MARCEL_HOME with memory files."""
    marcel_home = tmp_path / ".marcel"
    memories = marcel_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("MARCEL_HOME", str(marcel_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nMarcel repo is at ~/.marcel/marcel-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `marcel memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        marcel_home = tmp_path / ".marcel"
        (marcel_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("MARCEL_HOME", str(marcel_home))
