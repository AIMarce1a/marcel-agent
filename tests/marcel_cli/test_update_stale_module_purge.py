"""Tests for _purge_stale_marcel_modules — the class fix for stale
sys.modules breaking the gateway auto-restart after `marcel update`.

Field failure (2026-08-20, Teknium's Linux box): `marcel update` pulled a
checkout where marcel_cli/gateway.py newly imports `line_input` from
marcel_cli.cli_output, but the updater process had cli_output cached from
before that symbol existed. The function-level `from marcel_cli.gateway
...
the Marcel package prefixes so later imports rebuild a self-consistent
...
def _restore_sys_modules():
    ...
    The purge under test evicts real Marcel modules from the cache; later
...
def test_purge_evicts_marcel_prefixed_modules():
    victims = [
        "marcel_cli.cli_output",
        "marcel_cli.gateway",
        "gateway.status",
        "tools.ansi_strip",
        "tui_gateway.server",
    ...
    for name in victims:
        if name not in sys.modules:
            ...
            added.append(name)
    try:
        cli_main._purge_stale_marcel_modules()
        for name in victims:
...
def test_purge_protects_executing_modules():
    ...
    cli_main._purge_stale_marcel_modules()
    assert sys.modules.get("marcel_cli.update_cmd") is update_cmd
    assert sys.modules.get("marcel_cli.main") is cli_main
    assert "marcel_cli" in sys.modules
...
def test_purge_preserves_active_update_receipt(tmp_path, monkeypatch):
    ...
    import marcel_cli.update_receipt as receipt
    ...
    try:
        ...
        cli_main._purge_stale_marcel_modules()
        post_purge_receipt = importlib.import_module("marcel_cli.update_receipt")
...
def test_purge_never_raises_on_weird_sys_modules():
    ...
    sys.modules["marcel_cli._purge_test_none"] = None  # type: ignore[assignment]
    try:
        cli_main._purge_stale_marcel_modules()
    finally:
        sys.modules.pop("marcel_cli._purge_test_none", None)
...
def test_stale_symbol_scenario_end_to_end():
    ...
    try:
        ...
        try:
            from marcel_cli.cli_output import line_input  # noqa: F401
            raised = False
        except ImportError:
            raised = True
...
def test_purge_keeps_plan_record_class_identity():
    ...
    from marcel_cli.update_inventory import RuntimeRecord as before

    cli_main._purge_stale_marcel_modules()
    from marcel_cli.update_inventory import RuntimeRecord as after
