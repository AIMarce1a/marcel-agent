"""Tests for marcel_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("marcel_cli.skills_config.save_config")
...
class TestIsSkillDisabled:
    ...
    @patch("marcel_cli.config.load_config")
    ...
    @patch("marcel_cli.config.load_config")
    @patch.dict("os.environ", {"MARCEL_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """MARCEL_SESSION_PLATFORM should be used when MARCEL_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_marcel_platform_takes_precedence(self, tmp_path, monkeypatch):
        """MARCEL_PLATFORM should win over MARCEL_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from marcel_cli.skills_config import _get_categories
        skills = [
