"""Tests for Bug #12905 fix — stale OAuth token detection in marcel model flow.

Bug 3: `marcel model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.marcel/.env but no valid
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr("marcel_cli.secret_prompt.masked_secret_prompt", lambda _: "")

        from marcel_cli.model_setup_flows import _model_flow_anthropic
        cfg = {}
