...
from marcel_cli.backup import verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    MARCEL_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def marcel_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("marcel_cli.config") or mod == "marcel_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, marcel_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, marcel_home, capsys, monkeypatch):
