...
The desktop self-update chain (Desktop → marcel-setup --update →
``marcel update`` → ``marcel desktop --build-only`` → relaunch) rebuilds
Marcel.exe on the end user's machine. Before this gate, "build succeeded" was
...
# MACHINE_ATTRIBUTES.UserEnabled — mirrors marcel_cli.main.
...
def test_rollback_restores_backup_and_keeps_corrupt_copy(tmp_path):
    ...
    backup_exe = desktop_dir / "release" / "win-unpacked.bak" / "Marcel.exe"
    make_pe(backup_exe, PE_AMD64)  # valid old build

    with patch("marcel_cli.main_desktop._windows_native_machine", return_value="AMD64"):
    ...
    assert (desktop_dir / "release" / "win-unpacked.corrupt" / "Marcel.exe").exists()
...
def test_gate_fails_clearly_without_backup(tmp_path, capsys):
    ...
    with patch("marcel_cli.main_desktop._purge_electron_build_cache", return_value=[]), \
         patch("marcel_cli.main_desktop._desktop_stamp_path", return_value=tmp_path / "stamp.json"):
...
# ─── end-to-end: `marcel desktop --build-only` exits nonzero on corrupt exe ─
...
def test_build_only_fails_when_pack_produces_corrupt_exe(tmp_path, monkeypatch, capsys):
    """The updater chain's contract: a rebuild whose Marcel.exe cannot launch
    must exit nonzero (so marcel-setup's retry-once kicks in) and must leave
    ...
    live_exe = desktop_dir / "release" / "win-unpacked" / "Marcel.exe"
    ...
    def pack_into_staging(cmd, *args, **kwargs):
        ...
        make_pe(staging / "win-unpacked" / "Marcel.exe", PE_AMD64, truncate_to=0x300)
        return subprocess.CompletedProcess(list(cmd), 0)

    with patch("marcel_cli.main_desktop.shutil.which", return_value="/usr/bin/npm"), \
         patch("marcel_cli.main_install_repair._resolve_node_runtime_npm", return_value="npm.cmd"), \
         ...
         patch("marcel_cli.main_desktop._desktop_stamp_path", return_value=tmp_path / "stamp.json"), \
         patch("marcel_cli.main_desktop._write_desktop_build_stamp") as mock_stamp, \
         patch("marcel_cli.main_desktop._windows_native_machine", return_value="AMD64"), \
         patch("marcel_cli.main_desktop.subprocess.run", side_effect=pack_into_staging), \
