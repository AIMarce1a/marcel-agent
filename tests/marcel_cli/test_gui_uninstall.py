"""Tests for marcel_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import marcel_cli.gui_uninstall as gu


def _make_agent(marcel_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = marcel_home / "marcel-agent"
    (agent_root / "marcel_cli").mkdir(parents=True)
    (agent_root / "marcel_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(marcel_home: Path) -> None:
    """Create the source-built GUI artifacts a `marcel desktop` run produces."""
    desktop = marcel_home / "marcel-agent" / "apps" / "desktop"
    ...
    (marcel_home / "marcel-agent" / "node_modules").mkdir(parents=True)
    (marcel_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(marcel_home: Path) -> None:
    (marcel_home / "config.yaml").write_text("x: 1\n")
    (marcel_home / ".env").write_text("KEY=secret\n")
    (marcel_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(marcel_home)
    _make_gui_build(marcel_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(marcel_home)
    ...
    assert summary["marcel_home"] == str(marcel_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `marcel desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    marcel_home = tmp_path / ".marcel"
    _make_agent(marcel_home)
    icon = lde.icon_path(marcel_home / "marcel-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(marcel_home)
    ...
    assert lde.icon_path(marcel_home / "marcel-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (marcel_home / "marcel-agent" / "marcel_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".marcel")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import marcel_cli.uninstall as uninstall
