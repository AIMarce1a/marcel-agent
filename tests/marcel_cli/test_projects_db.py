"""Tests for the per-profile Projects store (marcel_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Marcel Agent", folders=["/tmp/marcel"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "marcel-agent"
    assert proj.name == "Marcel Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/marcel"
    assert [f.path for f in proj.folders] == ["/tmp/marcel"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "marcel-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' MARCEL_HOME.
