"""ensure_marcel_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".marcel"
    second = tmp_path / "b" / ".marcel"

    monkeypatch.setenv("MARCEL_HOME", str(first))
    cfg.ensure_marcel_home()

    # Profile switch: MARCEL_HOME moves → the new path is ensured too.
