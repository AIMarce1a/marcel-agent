...
When ``MARCEL_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.marcel/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.marcel            (has its own auth.json fixture)
    * Profile     -> tmp_path/.marcel/profiles/coder   (active, MARCEL_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Marcel is not
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (MARCEL_HOME != ~/.marcel, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    marcel_home = tmp_path / "classic"
    marcel_home.mkdir()
