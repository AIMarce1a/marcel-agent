...
Covers:
- Defense 1: gateway stop/restart refuse when _MARCEL_GATEWAY=1
...
from marcel_cli.cron import cron_command
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.marcel.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.marcel/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l marcel-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.marcel.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.marcel.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.marcel.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.marcel/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.marcel.gateway-apollo:/a.plist' "
            "'ai.marcel.gateway-cronus:/c.plist' 'ai.marcel.gateway-plutus:/p.plist' "
            "'ai.marcel.gateway:/Users/botuser/Library/LaunchAgents/ai.marcel.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated marcel note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.marcel.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.marcel.gateway',
        "launchctl kick'start' -k gui/501/ai.marcel.gateway",
        "launchctl kick\\start -k gui/501/ai.marcel.gateway",
        'launchctl "kickstart" -k gui/501/ai.marcel.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.marcel.gateway',
        "launchctl dis\\able gui/501/ai.marcel.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.marcel."gateway"',
        # Same class on the systemctl and marcel-CLI branches.
        'systemctl re"start" marcel-gateway',
        'marcel gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.marcel.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.marcel.update-checker',
        'systemctl re"start" marcel-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "marcel cron list",
        "marcel update",
        "marcel config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `marcel gateway start` is benign — starting a gateway from inside a
        ...
        "marcel gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway marcel
        # services must not be falsely blocked (the old `.*marcel` matched any
        # marcel token).
        "launchctl unload ai.marcel.update-checker.plist",
        "launchctl restart ai.marcel.daemon",
        ...
        "systemctl restart marcel-meta.service",
        "systemctl restart marcel-cron-helper",
        ...
        "echo after the marcel gateway restarted cleanly",
        "the marcel gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "marcel skill view gateway-notes && echo marcel gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `marcel` is a path component
        # there, not a command.
        "cat '/docs/marcel gateway restart-notes.md'",
        "less /home/user/notes/marcel gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "marcel gateway restart",
        "marcel gateway restart; echo done",
        "marcel gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;marcel gateway restart",
        "true && marcel gateway stop",
        "echo $(marcel gateway restart)",
        "echo `marcel gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.marcel.gateway"])',
        'subprocess.run(["marcel", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.marcel.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["marcel", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: marcel gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `marcel -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry MARCEL_HOME/MARCEL_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("MARCEL_PROFILE", "zeus")
        monkeypatch.delenv("MARCEL_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "marcel -p zeus gateway stop",
        "marcel -p zeus gateway restart",
        "marcel --profile zeus gateway restart",
        "marcel --profile zeus gateway stop",
        "marcel --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "marcel -v -p zeus gateway restart",
        "marcel -p zeus -v gateway restart",
        "marcel --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "marcel -p venus gateway stop",
        "marcel -p venus gateway restart",
        "marcel --profile venus gateway restart",
        "marcel --profile=venus gateway stop",
        "marcel -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "marcel -p zeus gateway start",
        "marcel -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("marcel gateway restart")
        assert _contains_gateway_lifecycle_command("marcel gateway stop")

    def test_marcel_home_derived_profile(self, monkeypatch):
        # Without MARCEL_PROFILE the guard falls back to the MARCEL_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("MARCEL_PROFILE", raising=False)
        monkeypatch.delenv("MARCEL_PROFILE_NAME", raising=False)
        import marcel_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("marcel -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("marcel -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("marcel -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("marcel gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_marcel_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade marcel then run marcel gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_marcel_gateway_once.sh). The script must live under
        # MARCEL_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("MARCEL_HOME", str(tmp_path / ".marcel"))
        scripts_dir = tmp_path / ".marcel" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_MARCEL_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _MARCEL_GATEWAY=1.

    Issue #37453: systemctl --user restart marcel-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart marcel-gateway",
        "systemctl --user restart marcel-gateway",
        "systemctl stop marcel-gateway.service",
        "marcel gateway restart",
        "marcel gateway uninstall",
        "launchctl kickstart gui/501/ai.marcel.gateway",
        "launchctl bootout gui/501/ai.marcel.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.marcel.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.marcel/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.marcel.gateway.restart-once.plist",
        "pkill -f marcel.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart marcel-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.marcel.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-marcel label: label-independent detection is the point
        # (#62891 second reproduction used `ai.marcel.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.marcel.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _MARCEL_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _MARCEL_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_MARCEL_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="marcel gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["marcel gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.marcel.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.marcel.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-marcel systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run marcel gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("marcel gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfemarcel gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under MARCEL_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("marcel gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/marcel\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 marcel gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_marcel_home()` falls back to Path.home(), which raises
        RuntimeError when neither MARCEL_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart marcel-gateway' .",
        "grep -rn 'marcel gateway restart' ./logs",
        "rg 'marcel gateway restart' ../archive",
        "grep -c 'systemctl stop marcel-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM marcel_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell marcel gateway restart"',
        'sqlite3 ./db ".system systemctl restart marcel-gateway"',
        'psql ./x -c "\\! systemctl restart marcel-gateway"',
        "grep -r 'marcel gateway restart' . | sh",
        "grep -r 'marcel gateway restart' ./logs | bash",
        "grep -r 'marcel gateway restart' . | sudo sh",
        "grep -r 'x' . ; marcel gateway restart",
        "grep -r 'x' . && systemctl restart marcel-gateway",
        'grep -r "$(marcel gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart marcel-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell marcel gateway restart"',
        'sqlite3 db ".system systemctl restart marcel-gateway"',
        'psql -c "\\! systemctl restart marcel-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    marcel_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run marcel gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MARCEL_HOME", str(tmp_path / ".marcel"))
        (tmp_path / ".marcel").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run marcel gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop marcel-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart marcel-gateway' /var/log/syslog",
        "rg 'marcel gateway restart' /home/user/.marcel/logs/",
        "journalctl -u marcel-gateway --grep 'systemctl restart marcel-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'marcel_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell marcel gateway restart"',
        'psql -c "\\! systemctl restart marcel-gateway"',
        "grep 'systemctl restart marcel-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart marcel-gateway",
        'grep "$(systemctl restart marcel-gateway)" f',
        "grep 'restart' log; systemctl restart marcel-gateway",
        'sqlite3 db "SELECT 1"; marcel gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "marcel gateway restart",
        "sudo systemctl stop marcel-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart marcel-gateway%'\""
        )
