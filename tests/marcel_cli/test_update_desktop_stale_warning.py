...
#88251: ``marcel update`` treated a failed desktop pack as non-fatal, printed
...
import marcel_cli.update_cmd_maint as update_cmd_maint
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "marcel_constants.with_marcel_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "marcel_constants.display_marcel_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_marcel_home", lambda: tmp_path)
