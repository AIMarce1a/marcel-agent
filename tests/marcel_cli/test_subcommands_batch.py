...
from marcel_cli.subcommands.dashboard import build_dashboard_parser
...
from marcel_cli.subcommands.whatsapp import build_whatsapp_parser
...
def test_config_get_unset_subcommands_parse():
    """`marcel config get/unset` parse key args (and --json for get)."""
...
# ── deprecated `marcel login` fails gracefully, not with argparse error ────
#
# `marcel login` is a removed command; its handler (`login_command` in
# `marcel_cli/auth.py`) prints a deprecation notice pointing at `marcel auth` /
# `marcel model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `marcel --help`.
