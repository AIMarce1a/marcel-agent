"""Tests for the unified provider catalog (marcel_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``marcel model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from marcel_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
