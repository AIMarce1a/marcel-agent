"""Tests for kanban board export / import (``marcel_cli.kanban_transfer``).
...
from marcel_cli import kanban_transfer as kt
from marcel_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("MARCEL_KANBAN_HOME", str(root))
        for var in ("MARCEL_KANBAN_DB", "MARCEL_KANBAN_WORKSPACES_ROOT",
                    "MARCEL_KANBAN_ATTACHMENTS_ROOT", "MARCEL_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this Marcel"):
