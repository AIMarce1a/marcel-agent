"""The conftest WAL gate must agree with marcel_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``marcel_state``. That is deliberate: importing
``marcel_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.marcel`` before the per-test ``MARCEL_HOME`` redirect, which makes tests
...
def test_conftest_gate_agrees_with_marcel_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and marcel_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_marcel_state_at_collection():
    """The gate must stay import-free of marcel_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.marcel, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``marcel_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "marcel_state" or name.startswith("marcel_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported marcel_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.marcel during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("marcel_state", None)
    ...
    try:
        ...
        if saved is not None:
            sys.modules["marcel_state"] = saved
    assert not blocked
