...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.marcel/marcel-agent/utils.py)
...
2. ``marcel update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``marcel_cli/main.py`` (the ``marcel update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``marcel_constants`` after the
