...
from marcel_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(marcel_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <marcel_home>/plugins/<name> and opt it in."""
    plugin_dir = marcel_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MARCEL_HOME", str(tmp_path / "marcel_empty"))
