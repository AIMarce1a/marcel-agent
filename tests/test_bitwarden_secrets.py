...
binary download are exercised manually by `marcel secrets bitwarden
...
def marcel_home(tmp_path, monkeypatch):
    """Point Marcel at an isolated home directory."""
    home = tmp_path / ".marcel"
    home.mkdir()
    monkeypatch.setenv("MARCEL_HOME", str(home))
    # Some modules cache get_marcel_home; clear if needed.
    import marcel_constants
    if hasattr(marcel_constants, "_MARCEL_HOME_CACHE"):
        marcel_constants._MARCEL_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(marcel_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from marcel_cli.env_loader import _apply_external_secret_sources
