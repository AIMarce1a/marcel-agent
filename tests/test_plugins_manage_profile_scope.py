...
``set_marcel_home_override`` / ``reset_marcel_home_override``. Because
``_plugins_dir()`` keys off ``get_marcel_home()``, the list action must then
...
def test_plugins_manage_profile_reads_that_profiles_dir(tmp_path, monkeypatch):
    ...
    import marcel_cli.profiles as profiles
