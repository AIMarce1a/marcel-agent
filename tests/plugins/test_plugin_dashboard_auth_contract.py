...
plugin SDK (``window.__MARCEL_PLUGIN_SDK__``): ``fetchJSON`` (JSON REST),
``authedFetch`` (uploads / blob downloads), and ``buildWsUrl`` /
``buildWsAuthParam`` (WebSockets). These handle BOTH dashboard auth modes —
loopback (``X-Marcel-Session-Token`` header) and gated OAuth
(``marcel_session_at`` cookie / single-use ``?ticket=``).

Plugins that hand-roll ``fetch`` / ``WebSocket`` and read
``window.__MARCEL_SESSION_TOKEN__`` directly send an empty token in gated mode
...
_FORBIDDEN = "__MARCEL_SESSION_TOKEN__"
