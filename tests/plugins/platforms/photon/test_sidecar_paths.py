...
``/opt/marcel``; ``resolve_sidecar_dir`` must run in place when the deps are
baked and current, and mirror the sidecar to the writable ``MARCEL_HOME``
...
def test_adapter_import_does_not_resolve_sidecar_dir(monkeypatch) -> None:
    ...
    files to MARCEL_HOME; the adapter and CLI resolve lazily on first use so
    a bare import (plugin discovery, `marcel --help`, test collection) has
