...
Run from the PR worktree:
    ...
    cd ~/.marcel/marcel-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.marcel/marcel-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.marcel/marcel-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a marcel-agent checkout"
)
...
# Isolated MARCEL_HOME for the config write.
