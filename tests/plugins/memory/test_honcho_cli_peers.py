"""Regression tests for #76414: `marcel honcho peers` showed "(not set)"
for every non-default profile.

_all_profile_host_configs() built the per-profile host key inline as
f"{HOST}.{profile}" ("marcel.work") while every other reader/writer —
profile_host_key(), resolve_active_host(), honcho status/enable/sync and
the runtime plugin — uses the underscore form ("marcel_work"). The lookup
...
a real honcho.json (temp MARCEL_HOME, no network).
"""
import io
import json
import sys
...
def honcho_home(tmp_path, monkeypatch):
    cfg = {
        "peerName": "alice",
        "hosts": {
            "marcel": {"peerName": "alice", "aiPeer": "marcel"},
            "marcel_work": {"peerName": "alice", "aiPeer": "marcel"},
            "marcel_my_profile": {"peerName": "bob", "aiPeer": "marcel"},
        },
    }
...
class TestAllProfileHostConfigs:
    def test_profile_host_keys_match_writer_form(self, honcho_home, monkeypatch):
        ...
        monkeypatch.setattr(
            "marcel_cli.profiles.list_profiles",
        ...
        assert host == "marcel_work"  # not "marcel.work"
    ...
    def test_sanitized_profile_names_resolve(self, honcho_home, monkeypatch):
        ...
        resolve — profile_host_key maps 'my.profile' -> 'marcel_my_profile';
    ...
    def test_legacy_dot_form_host_key_still_readable(self, honcho_home, monkeypatch):
        ...
        ("marcel.work") must keep working — the README promises those keys
        ...
        del cfg["hosts"]["marcel_work"]
        cfg["hosts"]["marcel.work"] = {"peerName": "carol", "aiPeer": "marcel"}
        path.write_text(json.dumps(cfg))
        monkeypatch.setattr(
            "marcel_cli.profiles.list_profiles",
            lambda: [SimpleNamespace(name="default"), SimpleNamespace(name="work")],
...
class TestCmdPeers:
    def test_peers_shows_populated_identity_not_host_key_leak(
        ...
        raw malformed key 'marcel.work' (or '(not set)')."""
        monkeypatch.setattr(
            "marcel_cli.profiles.list_profiles",
            lambda: [SimpleNamespace(name="default"), SimpleNamespace(name="work")],
        )
        out = _peers_output(SimpleNamespace())
        assert "marcel.work" not in out
        ...
        assert "alice" in work_line and "marcel" in work_line
    ...
    def test_peers_falls_back_cleanly_when_block_missing(
        ...
        out = _peers_output(SimpleNamespace())
        assert "marcel.new" not in out  # well-formed key, no dot-form leak
