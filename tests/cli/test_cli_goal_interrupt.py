...
minimal ``MarcelCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def marcel_home(tmp_path, monkeypatch):
    """Isolated MARCEL_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".marcel"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("MARCEL_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves MARCEL_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal MarcelCLI stub with an active goal wired in."""
    from cli import MarcelCLI
    from marcel_cli.goals import GoalManager

    cli = MarcelCLI.__new__(MarcelCLI)
...
class TestInterruptAutoPause:
    ...
    def test_interrupted_turn_is_resumable(self, marcel_home):
        ...
        with patch("marcel_cli.goals.judge_goal"):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, marcel_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, marcel_home):
        ...
        src = inspect.getsource(MarcelCLI.chat)
