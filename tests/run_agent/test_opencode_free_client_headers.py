...
@patch("agent.process_bootstrap.OpenAI")
def test_opencode_free_sends_marcel_attribution(mock_openai):
    """Keyless requests still identify as Marcel (attribution headers match
    ...
    headers = _zen_call_headers(mock_openai)
    assert headers.get("X-Title") == "Marcel Agent"
    assert str(headers.get("User-Agent", "")).startswith("MarcelAgent/")
