"""
Tests for timezone support (marcel_time module + integration points).
...
def _reset_marcel_time_cache():
    """Reset the marcel_time module cache."""
    marcel_time.reset_cache()


# =========================================================================
# marcel_time.now() — core helper
# =========================================================================

class TestMarcelTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_marcel_time_cache()

    def teardown_method(self):
        _reset_marcel_time_cache()
        os.environ.pop("MARCEL_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["MARCEL_TIMEZONE"] = "Asia/Kolkata"
        result = marcel_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["MARCEL_TIMEZONE"] = "UTC"
        result = marcel_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["MARCEL_TIMEZONE"] = "America/New_York"
        result = marcel_time.now()
        assert result.tzinfo is not None
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["MARCEL_TIMEZONE"] = "Europe/London"
        tz = marcel_time.get_timezone()
        assert isinstance(tz, ZoneInfo)
        assert str(tz) == "Europe/London"

    def test_cache_isolated_by_active_profile_config(self, tmp_path, monkeypatch):
        """Switching MARCEL_HOME must not reuse another profile's timezone."""
        ...
        monkeypatch.delenv("MARCEL_TIMEZONE", raising=False)

        monkeypatch.setenv("MARCEL_HOME", str(first_home))
        assert str(marcel_time.get_timezone()) == "Asia/Tokyo"

        # Multiplexed profile runtime scopes switch MARCEL_HOME in one process.
        monkeypatch.setenv("MARCEL_HOME", str(second_home))
        assert str(marcel_time.get_timezone()) == "America/New_York"
    ...
    def test_concurrent_profile_resolution_never_mixes_zones(
        ...
        def worker(key: str) -> None:
            barrier.wait()
            for _ in range(200):
                token = set_marcel_home_override(str(homes[key]))
                try:
                    tz = marcel_time.get_timezone()
                    if str(tz) != zones[key]:
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When MARCEL_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) MARCEL_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("MARCEL_TIMEZONE=" + os.environ.get("MARCEL_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "MARCEL_TIMEZONE=NOT_SET" in result["output"], (
            "MARCEL_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When MARCEL_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("MARCEL_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=marcel_tz) which shifted
        absolute time when system-local tz != Marcel tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Marcel tz
        ...
        # Use a Marcel timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _marcel_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["MARCEL_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_marcel_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Marcel timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["MARCEL_TIMEZONE"] = "US/Eastern"
        _reset_marcel_time_cache()
