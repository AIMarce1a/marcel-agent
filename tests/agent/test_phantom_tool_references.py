...
Covers:
  * MARCEL_AGENT_HELP_GUIDANCE degrades to the docs-only variant when the
...
class TestMarcelAgentHelpGuidance:
    def test_skill_variant_used_when_skill_view_present(self):
        from agent.prompt_builder import MARCEL_AGENT_HELP_GUIDANCE
        assert "skill_view(name='marcel-agent')" in MARCEL_AGENT_HELP_GUIDANCE

    def test_no_skills_variant_has_no_skill_view_reference(self):
        from agent.prompt_builder import MARCEL_AGENT_HELP_GUIDANCE_NO_SKILLS
        assert "skill_view" not in MARCEL_AGENT_HELP_GUIDANCE_NO_SKILLS
        assert "hermes-agent.nousresearch.com/docs" in MARCEL_AGENT_HELP_GUIDANCE_NO_SKILLS
...
class TestEssentialSkillsUndisableable:
    def test_agent_side_reader_strips_essential(self, monkeypatch, tmp_path):
        ...
        assert "marcel-agent" not in disabled
    ...
    def test_skill_manage_delete_refused(self):
        ...
        msg = _pinned_guard("marcel-agent")
...
class TestEssentialOnlySync:
    def test_opted_out_sync_seeds_only_essential(self, monkeypatch, tmp_path):
        """A profile with .no-bundled-skills still gets the marcel-agent skill."""
        ...
        for cat, name in [
            ("autonomous-ai-agents", "marcel-agent"),
            ("media", "gif-search"),
        ]:
            d = bundled / cat / name
        ...
        monkeypatch.setattr(ss, "_marcel_home", lambda: home)
        ...
        assert result["copied"] == ["marcel-agent"]
        assert (home / "skills" / "autonomous-ai-agents" / "marcel-agent" / "SKILL.md").exists()
