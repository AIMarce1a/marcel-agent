...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated MARCEL_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="marcel_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".marcel"))
    prev_home = os.environ.get("MARCEL_HOME")
    os.environ["MARCEL_HOME"] = os.path.join(test_home, ".marcel")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("marcel_"):
            del sys.modules[mod]
    ...
    try:
        ...
        # references into marcel_cli.*/tools.* and their monkeypatches would otherwise
        ...
        if prev_home is None:
            ...
            os.environ["MARCEL_HOME"] = prev_home
