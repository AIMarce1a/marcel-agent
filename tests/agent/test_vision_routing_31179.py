...
def isolated_home(monkeypatch):
    """Temp MARCEL_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="marcel_test_31179_")
    marcel_home = os.path.join(test_home, ".marcel")
...
def _fresh_modules():
    """Drop cached marcel modules so each test reloads against current env."""
    _drop_reload_targets()
