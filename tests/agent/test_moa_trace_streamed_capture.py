...
path via ``marcel chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp MARCEL_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point MARCEL_HOME at a temp dir and turn moa.save_traces on."""
    marcel_home = tmp_path / ".marcel"
    marcel_home.mkdir()
    monkeypatch.setenv("MARCEL_HOME", str(marcel_home))

    # save_moa_turn reads config via marcel_cli.config.load_config; stub it to
