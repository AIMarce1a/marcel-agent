...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"marcel-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "marcel-agent-dev", "declared": True},
        },
    )
    ...
    assert e["into"] == "marcel-agent-dev"
...
# `marcel curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `marcel curator pin <umbrella>` in the rename block so
