...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate MARCEL_HOME under tmp_path."""
    monkeypatch.setenv("MARCEL_HOME", str(tmp_path / ".marcel"))
    (tmp_path / ".marcel").mkdir()

    # Force the constants/image cache helpers to re-read MARCEL_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("marcel_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_marcel_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under MARCEL_HOME — gateway cleanup
