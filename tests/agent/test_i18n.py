...
from agent.branding import brand_user_facing_text, runtime_brand, runtime_command
...
def test_default_when_nothing_set(monkeypatch):
    ...
    monkeypatch.delenv("MARCEL_LANGUAGE", raising=False)
...
def test_t_brands_catalog_output_for_marcel_in_every_locale(monkeypatch):
    """A single presentation transform brands translated catalog values uniformly."""
    monkeypatch.setattr(i18n, "_load_catalog", lambda lang: {"greeting": "Marcel Agent: Marcel"})
    monkeypatch.setattr(i18n, "get_language", lambda: "en")
    monkeypatch.setattr("agent.branding.sys.argv", ["marcel"])

    for lang in i18n.SUPPORTED_LANGUAGES:
        assert i18n.t("greeting", lang=lang) == "Marcel: Marcel"


def test_t_keeps_legacy_catalog_output_and_interpolated_secrets_untouched(monkeypatch):
    """Branding is presentation-only and never reads, redacts, or exposes values."""
    secret = "sk-test-do-not-transform"
    monkeypatch.setattr(i18n, "_load_catalog", lambda lang: {"message": "Marcel received {token}"})
    monkeypatch.setattr("agent.branding.sys.argv", ["marcel"])

    assert i18n.t("message", lang="en", token=secret) == f"Marcel received {secret}"


def test_runtime_branding_keeps_legacy_invocations_and_identifiers_intact():
    """The compatibility command remains untouched outside Marcel invocation."""
    assert runtime_brand(["marcel"]) == "Marcel"
    assert runtime_command("gateway restart", ["marcel"]) == "marcel gateway restart"
    assert runtime_brand(["marcel"]) == "Marcel"
    assert runtime_command("gateway restart", ["marcel"]) == "marcel gateway restart"
    assert brand_user_facing_text(
        "Marcel Agent; MARCEL_HOME; ~/.marcel; marcel gateway", ["marcel"]
    ) == "Marcel; MARCEL_HOME; the Marcel private configuration store; marcel gateway"


def test_runtime_branding_hides_absolute_legacy_storage_paths():
    rendered = brand_user_facing_text(
        "Use /home/runner/.marcel/config.yaml, then run marcel model.", ["marcel"]
    )
...
def test_locales_dir_env_override_ignored_when_missing(tmp_path, monkeypatch):
    """A bogus MARCEL_BUNDLED_LOCALES falls through to source/wheel resolution
    instead of returning a path that doesn't exist."""
    monkeypatch.setenv("MARCEL_BUNDLED_LOCALES", str(tmp_path / "does-not-exist"))
