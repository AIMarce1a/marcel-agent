...
Marcel state stays profile-scoped through MARCEL_HOME. Host subprocesses should
...
class TestGetSubprocessHome:
    """Unit tests for marcel_constants.get_subprocess_home()."""

    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(marcel_constants, "is_container", lambda: False)
        monkeypatch.delenv("TERMINAL_HOME_MODE", raising=False)
        monkeypatch.delenv("MARCEL_REAL_HOME", raising=False)
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        marcel_home = real_home / ".marcel" / "profiles" / "coder"
        profile_home = marcel_home / "home"
        profile_home.mkdir(parents=True)
        monkeypatch.setenv("HOME", str(real_home))
        monkeypatch.setenv("MARCEL_HOME", str(marcel_home))
        from marcel_constants import get_subprocess_home
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MARCEL_REAL_HOME", str(real_home))

        from marcel_constants import get_subprocess_home, get_real_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MARCEL_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("MARCEL_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_host_auto_preserves_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["MARCEL_REAL_HOME"] == str(real_home)
    ...
    def test_profile_mode_injects_profile_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(marcel_home / "home")
        assert result["MARCEL_REAL_HOME"] == str(real_home)
    ...
    def test_no_injection_when_marcel_home_unset(self, monkeypatch):
