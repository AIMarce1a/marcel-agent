...
class TestToolsetConsistency:
    ...
    def test_marcel_platforms_share_core_tools(self):
        """All marcel-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on marcel-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["marcel-cli", "marcel-telegram", "marcel-discord", "marcel-whatsapp", "marcel-slack", "marcel-signal", "marcel-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_marcel_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("marcel-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("marcel-cli")
        second = resolve_toolset("marcel-cli")
        ...
        assert (
            "marcel-cli", True, registry_id, generation
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("marcel-cli", include_registry=False)
        second = resolve_toolset("marcel-cli", include_registry=False)
        assert first == second
