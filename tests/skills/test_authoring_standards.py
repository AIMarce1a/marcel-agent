...
def test_required_frontmatter_fields(p):
    ...
    marcel = (fm.get("metadata") or {}).get("marcel") or {}
    if not (marcel.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.marcel.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    marcel = (fm.get("metadata") or {}).get("marcel") or {}
    dangling = [
        rs for rs in (marcel.get("related_skills") or []) if rs not in _all_names()
    ]
