...
def test_frontmatter_required_fields():
    ...
    marcel = fm["metadata"]["marcel"]
    assert marcel["tags"]
    assert "related_skills" in marcel
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("Marcel Agent"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["marcel"]["related_skills"]:
        hits = (
