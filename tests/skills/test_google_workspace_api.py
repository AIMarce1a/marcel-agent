...
def test_gws_env_uses_secret_environment_token_without_a_credentials_file(
    ...
    with patch.object(
        api_module, "get_credentials", return_value=MagicMock(token="ya29.from-env")
    ):
        env = api_module._gws_env()
    assert env["GOOGLE_WORKSPACE_CLI_TOKEN"] == "ya29.from-env"
