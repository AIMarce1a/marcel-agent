...
Build the real image and verify at runtime:
  ...
  1. /opt/marcel is not writable by the marcel user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and MARCEL_DISABLE_LAZY_INSTALLS are set
  3. /opt/marcel/.install_method contains "docker" (code-scoped stamp)
  4. $MARCEL_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $MARCEL_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_marcel(
    built_image: str, container_name: str,
) -> None:
    """The marcel user must not be able to modify /opt/marcel.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/marcel as the marcel user
        "touch /opt/marcel/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"marcel user can write to /opt/marcel (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/marcel/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"marcel user can write to /opt/marcel/.venv: {r.stdout}"
    )


def test_marcel_disable_lazy_installs_and_dont_write_bytecode(
    built_image: str, container_name: str,
) -> None:
    """The container must set PYTHONDONTWRITEBYTECODE and
    MARCEL_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
    ...
    r = docker_exec_sh(
        ...
        'test "$MARCEL_DISABLE_LAZY_INSTALLS" = "1" && '
        'echo ENV_OK || echo ENV_MISSING',
        timeout=10,
    )
    assert "ENV_OK" in r.stdout, (
        f"expected PYTHONDONTWRITEBYTECODE=1 and "
        f"MARCEL_DISABLE_LAZY_INSTALLS=1, got: {r.stdout} stderr={r.stderr}"
    )
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $MARCEL_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $MARCEL_HOME was not healed on boot: "
        f"{r.stdout}"
    )
