...
Override the image with ``MARCEL_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("MARCEL_TEST_IMAGE", "marcel-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``MARCEL_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("MARCEL_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged marcel user
# ---------------------------------------------------------------------------
#
# Background: every Marcel runtime path inside the container drops to UID
# 10000 (the ``marcel`` user) via ``s6-setuidgid marcel``. ``docker exec``
...
# fails (PermissionError swallowed) for marcel, so a test that ran as root
...
# helpers below run every probe under ``-u marcel`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: marcel).
