"""Runtime smoke test for Docker $MARCEL_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the marcel user after container boot.
...
registered later runs its log service as the dropped marcel user and
...
def test_logs_gateways_seeded_and_marcel_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by marcel after boot."""
    ...
    assert "logs=marcel" in r.stdout, (
        f"logs/ not owned by marcel: {r.stdout}"
    )
    assert "gateways=marcel" in r.stdout, (
        f"logs/gateways/ not owned by marcel: {r.stdout}"
    )
