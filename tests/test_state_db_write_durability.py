...
The defect: marcel_state already knows macOS `fsync()` does not guarantee
...
import marcel_state
from marcel_state import repair_state_db_schema
from marcel_state_repair import _connect_repair_durable
...
def test_repair_path_has_no_bare_connects() -> None:
    ...
    """
    # The repair/probe helpers live in marcel_state_repair; marcel_state only
    # re-imports them.
    import marcel_state_repair

    source = Path(marcel_state_repair.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(marcel_state_repair.__file__))
