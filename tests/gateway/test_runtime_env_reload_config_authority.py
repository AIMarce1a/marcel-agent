...
MARCEL_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env MARCEL_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``marcel config get terminal.backend`` still says local.
    """
    marcel_home = tmp_path / ".marcel"
    marcel_home.mkdir()
    (marcel_home / "config.yaml").write_text(
    ...
    (marcel_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
