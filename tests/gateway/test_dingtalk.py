...
class TestSend:
    ...
    async def test_send_posts_to_webhook(self):
        ...
        assert payload["markdown"]["title"] == "Marcel"
...
class TestMentionPatterns:
    ...
    def test_pattern_matches_text(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^marcel"]}
        )
        assert adapter._message_matches_mention_patterns("marcel please help") is True
        assert adapter._message_matches_mention_patterns("please marcel help") is False
