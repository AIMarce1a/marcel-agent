...
class TestDMTopicFallbackReplyToMode:
    ...
    reply_to_mode 'off' was ignored when sending via Marcel-created DM topic
...
class TestDMTopicSyntheticSendRouting:
    ...
    response in a different chat lane than the Marcel topic the session runs
    in. The no-anchor branch must prefer the Marcel topic's
    ``message_thread_id`` whenever it resolves.
    """

    def test_no_anchor_prefers_marcel_topic_thread_id(self):
