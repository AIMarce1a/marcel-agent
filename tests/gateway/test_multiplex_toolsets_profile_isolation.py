...
E2E-style: real profile homes under a temp MARCEL root, real config.yaml
...
def marcel_root(tmp_path, monkeypatch):
    ...
    # get_default_marcel_root memoizes per (native_home, env_home) pair, so
...
class TestMultiplexOnToolsetIsolation:
    @pytest.mark.asyncio
    async def test_each_profile_sees_its_own_toolsets(self, marcel_root):
    ...
    @pytest.mark.asyncio
    async def test_owner_key_does_not_open_secondary_profile(self, marcel_root):
    ...
    @pytest.mark.asyncio
    async def test_unknown_profile_is_404(self, marcel_root):
        adapter = _make_adapter(multiplex=True)
        async with TestClient(TestServer(_make_app(adapter))) as cli:
            resp = await cli.get(
                "/p/ghost/v1/toolsets",
...
class TestMultiplexOffPrefixFailsClosed:
    ...
    def test_foreign_prefix_rejected(self, marcel_root):
    ...
    def test_self_referential_prefix_falls_through(self, marcel_root):
    ...
    def test_own_named_profile_prefix_falls_through(self, marcel_root, monkeypatch):
        """A gateway launched FOR profile lokaj accepts /p/lokaj/…"""
        monkeypatch.setenv(
            "MARCEL_HOME", str(marcel_root / "profiles" / "lokaj")
        )
        adapter = _make_adapter(multiplex=False)
    ...
    @pytest.mark.asyncio
    async def test_foreign_prefix_is_404_end_to_end(self, marcel_root):
        adapter = _make_adapter(multiplex=False)
        async with TestClient(TestServer(_make_app(adapter))) as cli:
            resp = await cli.get(
                "/p/lokaj/v1/toolsets",
