...
Proves, on windows-latest:
  1. `GatewayControlServer` binds ``\\\\.\\pipe\\marcel-gateway-<hash>`` via
...
os.environ["MARCEL_HOME"] = sys.argv[2]
...
def test_named_pipe_identify_status_and_fleet_consumer(live_server, monkeypatch):
    ...
    assert ident["kind"] == "marcel-gateway"
    ...
    monkeypatch.setattr("marcel_cli.profiles._get_default_marcel_home", lambda: home)
    monkeypatch.setattr(
        "marcel_cli.profiles._get_profiles_root", lambda: home / "no-profiles"
    )
...
def test_pipe_gone_after_kill_falls_back(live_server, monkeypatch):
    ...
    (home / "gateway_state.json").write_text(
        json.dumps(
            {"pid": os.getpid(), "code_sha": "OLD", "kind": "marcel-gateway"}
        ),
        encoding="utf-8",
    )
    monkeypatch.setattr(
