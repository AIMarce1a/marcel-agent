...
(/opt/marcel/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable MARCEL_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_marcel_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable MARCEL_HOME."""
    ...
    monkeypatch.setattr(
        "marcel_constants.get_marcel_home", lambda: marcel_home
    )
    ...
    expected = marcel_home / "scripts" / "whatsapp-bridge"
