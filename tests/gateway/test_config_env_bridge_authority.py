...
Guards against the 60-vs-500 bug where a stale `.env MARCEL_MAX_ITERATIONS=60`
...
def _run_gateway_import(marcel_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
    ...
    script = textwrap.dedent(
        ...
        for k in (
            ...
            "MARCEL_AGENT_TIMEOUT",
            "MARCEL_AGENT_TIMEOUT_WARNING",
            "MARCEL_TURN_LEASE_TIMEOUT",
            "MARCEL_SESSION_STALL_TIMEOUT",
            "MARCEL_GATEWAY_BUSY_INPUT_MODE",
            "MARCEL_GATEWAY_BUSY_TEXT_MODE",
    ...
    # otherwise intentionally clean so stale Marcel settings cannot leak in.
    for k in (
        "PATH",
        "PYTHONPATH",
        "VIRTUAL_ENV",
...
@pytest.fixture
def marcel_home(tmp_path: Path) -> Path:
    home = tmp_path / ".marcel"
    home.mkdir()
    return home


def test_config_gateway_timeout_wins_over_stale_env(marcel_home: Path) -> None:
    """Every agent.* bridge key must be config-authoritative, not .env-authoritative."""
    _write_config(marcel_home, agent_cfg={
        "gateway_timeout": 1800,
        "gateway_timeout_warning": 900,
        "session_stall_timeout": 300,
    })
    _write_env(marcel_home, {
        "MARCEL_AGENT_TIMEOUT": "60",
        "MARCEL_AGENT_TIMEOUT_WARNING": "30",
        "MARCEL_SESSION_STALL_TIMEOUT": "15",
    })

    env = _run_gateway_import(marcel_home, initial_env={})

    assert env.get("MARCEL_AGENT_TIMEOUT") == "1800"
    assert env.get("MARCEL_AGENT_TIMEOUT_WARNING") == "900"
    assert env.get("MARCEL_SESSION_STALL_TIMEOUT") == "300"


def test_config_turn_lease_timeout_wins_over_stale_env(marcel_home: Path) -> None:
    ...
    _write_env(
        marcel_home,
        {"MARCEL_TURN_LEASE_TIMEOUT": "60"},
    )

    env = _run_gateway_import(marcel_home, initial_env={})

    assert env.get("MARCEL_TURN_LEASE_TIMEOUT") == "600"
...
def test_default_turn_lease_timeout_overrides_stale_env_when_key_is_omitted(
    ...
    assert env.get("MARCEL_TURN_LEASE_TIMEOUT") == "5"
...
def test_config_platform_connect_timeout_supplies_env_when_unset(marcel_home: Path) -> None:
    ...
    _write_config(marcel_home, gateway_cfg={"platform_connect_timeout": 90})

    env = _run_gateway_import(marcel_home, initial_env={})

    assert env.get("MARCEL_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "90"


def test_env_platform_connect_timeout_wins_over_config(marcel_home: Path) -> None:
    """Unlike the agent.*/display.*/timezone bridges (config-authoritative),
    MARCEL_GATEWAY_PLATFORM_CONNECT_TIMEOUT is the manual-override escape hatch:
    ...
    env = _run_gateway_import(
        marcel_home,
        initial_env={"MARCEL_GATEWAY_PLATFORM_CONNECT_TIMEOUT": "120"},
    )

    assert env.get("MARCEL_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "120"
