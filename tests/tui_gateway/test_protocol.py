...
def server():
    ...
    # e.g. marcel_cli.active_sessions would bind the mocked get_marcel_home
    ...
    with patch.dict("sys.modules", {
        "marcel_constants": MagicMock(get_marcel_home=MagicMock(return_value="/tmp/marcel_test")),
        "marcel_cli.env_loader": MagicMock(),
        "marcel_cli.banner": MagicMock(),
        "marcel_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `MARCEL_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("MARCEL_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("MARCEL_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/marcel-agent-dev": {"name": "marcel-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "marcel-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_scopes_skill_lookup_to_session_profile(server, tmp_path):
    ...
    profile_home rather than the gateway process's ambient MARCEL_HOME
    ...
    # The gateway's own MARCEL_HOME (the test-isolation tempdir, no
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    """Real config + skin files: activating a skin (as `marcel config set` does)
    ...
    import marcel_cli.skin_engine as skin_engine

    (tmp_path / "skins").mkdir()
    (tmp_path / "skins" / "midnight.yaml").write_text(
        "name: midnight\ndescription: t\ncolors:\n  banner_title: '#00ffcc'\n  background: '#001010'\n"
    )
    monkeypatch.setattr(skin_engine, "get_marcel_home", lambda: tmp_path)
    ...
    # Activate midnight, as `marcel config set display.skin midnight` would.
