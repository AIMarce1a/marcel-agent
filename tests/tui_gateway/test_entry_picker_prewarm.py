...
guard is covered in ``tests/marcel_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``marcel_cli.model_switch_providers.prewarm_picker_cache_async``
...
import marcel_cli.model_switch_providers
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from marcel_cli.model_switch,
