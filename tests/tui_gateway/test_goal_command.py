...
def marcel_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves MARCEL_HOME.
...
@pytest.fixture()
def server(marcel_home, monkeypatch):
    ...
    # Pin config resolution to the isolated MARCEL_HOME. Sibling test
    ...
    # module-level ``_marcel_home = get_marcel_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_marcel_home", marcel_home)
...
def test_moa_bare_returns_usage(server, session, marcel_home):
    _write_moa_config(marcel_home, """
moa:
