...
def test_slash_worker_accepts_profile_home():
    ...
    # marcel_state evaluates get_marcel_home() / "state.db" at import time, so
    # the mock must return a Path (a bare str raises TypeError under per-file
    # subprocess isolation).
    with patch.dict("sys.modules", {
        "marcel_constants": MagicMock(
            get_marcel_home=MagicMock(return_value=Path("/tmp/marcel_test")),
        ),
    }):
        with patch("subprocess.Popen") as mock_popen:
            ...
            worker = _SlashWorker(
                ...
                model="test-model",
                profile_home="/home/luke/.marcel/profiles/work"
            )

            # Verify Popen was called
            assert mock_popen.called

            # Check that MARCEL_HOME was set in the environment
            call_kwargs = mock_popen.call_args[1]
            assert "env" in call_kwargs
            assert call_kwargs["env"]["MARCEL_HOME"] == "/home/luke/.marcel/profiles/work"
