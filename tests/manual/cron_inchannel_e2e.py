...
Uses a throwaway MARCEL_HOME so it never touches ~/.marcel. No real names.
"""
...
def _fresh_home():
    """Point MARCEL_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_marcel_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["MARCEL_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER MARCEL_HOME is set.
...
# a different get_marcel_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"MARCEL_HOME (throwaway): {HOME}")
