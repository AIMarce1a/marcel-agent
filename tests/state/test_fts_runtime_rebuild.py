...
import marcel_state
import marcel_state_holders
import marcel_state_schema
from marcel_state import SessionDB
from marcel_state_common import FTS_REBUILD_DEFERRAL_KEY, FTS_STALE_KEY, LEGACY_FTS_SQL, LEGACY_FTS_TRIGRAM_SQL, SCHEMA_SQL, _FTS_TRIGGERS
from marcel_state_dbfile import _concrete_state_db_holder_pids, _is_inactive_orphan_desktop_holder
...
class TestRuntimeFtsRebuild:
    def test_reap_candidates_exclude_uninspectable_holder_suspicions(
        ...
        assert _concrete_state_db_holder_pids(
            db_path,
            [
                (222, "uninspectable holder: python -m marcel_cli.main serve --port 0"),
    ...
    @pytest.mark.parametrize(
        "argv",
        (
            ("journalctl", "-u", "marcel-agent.service"),
            ("grep", "marcel-agent", "/var/log/syslog"),
            (
                "/usr/sbin/tailscaled",
                "be-child",
                "ssh",
                "--cmd=python -m marcel_cli.main gateway",
            ),
            ("tmux", "new-session", "/opt/marcel-agent/.venv/bin/marcel gateway"),
            ("python3", "/opt/marcel-agent/tools/check_state.py"),
            ("marcel-monitor", "gateway"),
            ("marcelctl", "serve"),
            ("python3", "worker.py", "marcel_cli.main"),
            ("python3", "-m", "other.module", "marcel_cli.main"),
            ("python3", "-c", "marcel_cli.main"),
            ("python3", "-Icprint('marcel_cli.main')", "marcel_cli/main.py"),
        ),
    )
    def test_uninspectable_non_marcel_process_is_not_a_holder(self, argv):
        assert not marcel_state_holders._looks_like_marcel(argv)

    @pytest.mark.parametrize(
        "argv",
        (
            ("/usr/local/bin/marcel", "gateway"),
            ("/usr/local/bin/marcel-agent", "serve"),
            ("/usr/local/bin/marcel-acp", "--stdio"),
            ("/usr/bin/python3", "-m", "marcel_cli.main", "gateway"),
            ("/usr/bin/python3", "-m", "acp_adapter"),
            ("/usr/bin/python3", "-Im", "marcel_cli.main", "gateway"),
            ("/usr/bin/python3", "-mmarcel_cli.main", "gateway"),
            ("/usr/bin/python3", "-W", "ignore", "-m", "marcel_cli.main"),
            ("/usr/bin/python3", "-Xdev", "-m", "marcel_cli.main"),
            (
                "/opt/marcel-agent/.venv/bin/python",
                "/opt/marcel-agent/marcel_cli/main.py",
                "gateway",
            ),
            ("python.exe", "--", "marcel_cli/main.py", "gateway"),
            ("python3", "/opt/marcel-agent/run_agent.py", "--query", "hello"),
        ),
    )
    def test_uninspectable_marcel_process_remains_a_holder(self, argv):
        assert marcel_state_holders._looks_like_marcel(argv)
    ...
    def test_foreign_holder_detection_proc_readlink_deleted_wal(
        ...
        monkeypatch.setattr(marcel_state_holders.os, "getpid", lambda: 111)
        real_listdir = os.listdir
        def _listdir(path):
            if isinstance(path, str):
                path = path.replace("/proc", str(proc_root))
            return real_listdir(path)
        monkeypatch.setattr(marcel_state_holders.os, "listdir", _listdir)
        real_readlink = os.readlink
        def _readlink(path):
            path = path.replace("/proc", str(proc_root))
            return real_readlink(path)
        monkeypatch.setattr(marcel_state_holders.os, "readlink", _readlink)
        real_stat = os.stat
        ...
        monkeypatch.setattr(marcel_state_holders.os, "stat", _stat)

        holders = marcel_state_holders.foreign_state_db_holders(db_path)
    ...
    def test_foreign_holder_ignores_same_path_with_different_file_identity(
        ...
        assert marcel_state_holders.foreign_state_db_holders(db_path) == []
    ...
    def test_foreign_holder_uninspectable_process_cmdline_fallback(
        ...
        flagged when /proc/<pid>/cmdline identifies it as a Marcel process."""
        db_path = tmp_path / "state.db"
        ...
        # PID 222's cmdline is world-readable and looks like Marcel
        cmdline_path = proc_root / "222" / "cmdline"
        cmdline_path.write_bytes(
            b"python3\x00-m\x00marcel_cli.main\x00chat\x00"
        )
        ...
        def _fake_argv(pid):
            ...
            try:
                ...
                return None
        monkeypatch.setattr(marcel_state_holders, "_read_proc_argv", _fake_argv)

        holders = marcel_state_holders.foreign_state_db_holders(db_path)
        # Should include PID 222 with the cmdline info
        assert len(holders) == 1
        assert holders[0][0] == 222
        assert "marcel_cli.main" in holders[0][1]
    ...
    def test_structural_corruption_propagates_without_live_fts_mutation(
        ...
        from marcel_state import StateDbCorruptError
    ...
    def test_repeated_deferrals_reap_inactive_orphan_then_rebuild(
        ...
        monkeypatch.setattr(marcel_state_schema.time, "time", lambda: 120.0)
...
class TestPhysicalCorruptionAcceptance:
    ...
    def test_canonical_btree_corruption_fails_closed(
        ...
        try:
            caplog.clear()
            with caplog.at_level("WARNING", logger="marcel_state"):
                with pytest.raises(sqlite3.DatabaseError) as caught:
