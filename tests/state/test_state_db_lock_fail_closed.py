...
work on a file several Marcel processes share (gateway service, the Desktop
app's `marcel serve` backend, CLI sessions, the TUI slash worker):

* `marcel_state_common.fts_rebuild_admission` — full structural FTS rebuilds
* `marcel_state_repair._cross_process_repair_lock`   — writable_schema surgery / VACUUM
