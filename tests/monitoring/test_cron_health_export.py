...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("marcel.gateway.up"), _M("marcel.gateway.active_agents"),
        _M("marcel.gateway.busy"), _M("marcel.gateway.drainable"),
        _M("marcel.gateway.restart_requested"),
        _M("marcel.platform.up"), _M("marcel.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("marcel.cron.scheduler.heartbeat_age_seconds"),
        _M("marcel.cron.scheduler.last_success_age_seconds"),
        _M("marcel.cron.scheduler.catch_up_occurrences"),
        _M("marcel.cron.jobs.enabled"), _M("marcel.cron.jobs.running"),
        _M("marcel.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Marcel Agent-owned Relay transport health" in text
