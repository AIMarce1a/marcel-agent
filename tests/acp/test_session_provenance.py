...
def test_root_session_no_compression(db):
    ...
    assert prov["currentMarcelSessionId"] == "root1"
    assert prov["rootMarcelSessionId"] == "root1"
    assert prov["parentMarcelSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_marcel_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentMarcelSessionId"] == "old"
    assert prov["rootMarcelSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousMarcelSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootMarcelSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"marcel"}
    assert "sessionProvenance" in meta["marcel"]
    assert meta["marcel"]["sessionProvenance"]["currentMarcelSessionId"] == "root1"
