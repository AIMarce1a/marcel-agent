...
def _open_continuable_cron_thread(job: dict, adapter, chat_id: str, loop) -> Optional[str]:
    ...
    if not callable(create_thread) or loop is None:
        return None
    thread_name = f"Marcel — {job.get('name') or job.get('id', 'cron')}"
    try:
...
def cron_delivery_targets() -> list[dict]:
    ...
    try:
        from marcel_cli.profiles import list_profile_names
...
def _deliver_to_bot_chat(job: dict, content: str, profile: str) -> Optional[str]:
    ...
    ``marcel [-p <profile>] chat --in ~ -c "Bot Chat" --create-if-missing -Q --query-file`` — the
    ...
    if marcel_bin:
        ...
        try:
            ...
            found = _ilu.find_spec("marcel_cli") is not None
        except Exception:
            found = False
        if not found:
            return "bot-chat delivery failed: marcel CLI not resolvable"
        argv = [sys.executable, "-m", "marcel_cli.main"]
    ...
    if profile:
        argv += ["-p", profile]
        # -p owns profile resolution; this scheduler's MARCEL_HOME must not shadow it.
        env.pop("MARCEL_HOME", None)
    ...
    try:
        with tempfile.NamedTemporaryFile(
            "w", encoding="utf-8", suffix=".txt", prefix="marcel-cron-botchat-", delete=False,
        ) as fh:
            fh.write(message)
            query_file = fh.name
...
def _deliver_result(
    ...
    # job's own attempt: a worker's script may dispatch another job in-process (`marcel cron run`),
    # and that nested delivery must not be keyed under the outer execution id.
    external_execution = os.environ.get("_MARCEL_CRON_EXTERNAL_WORKER", "")
    ...
    # persisted as ``last_delivery_unverified`` so `marcel cron list` shows it.
    ...
    # at boot; standalone runs (`marcel cron run`) did not, silently dropping files. Idempotent.
