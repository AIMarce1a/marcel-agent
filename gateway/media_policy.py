...
``validate_media_delivery_path`` reads ``MARCEL_MEDIA_DELIVERY_STRICT`` (gateway.strict),
``MARCEL_MEDIA_ALLOW_DIRS`` (gateway.media_delivery_allow_dirs) and
``MARCEL_MEDIA_TRUST_RECENT_FILES`` (gateway.trust_recent_files).  Every delivery
entrypoint (gateway startup, ``marcel cron run``, ``marcel send``) calls
...
_FLAG_ENVS = (("strict", "MARCEL_MEDIA_DELIVERY_STRICT"), ("trust_recent_files", "MARCEL_MEDIA_TRUST_RECENT_FILES"))
_ALLOW_DIRS_ENV = "MARCEL_MEDIA_ALLOW_DIRS"
