...
SCALE_TO_ZERO_ENV = "MARCEL_SCALE_TO_ZERO"  # stamped by NAS when the Labs toggle is on
...
def dashboard_client_heartbeat_path(marcel_home: Optional[os.PathLike | str] = None):
    """Path of the dashboard-client liveness marker under MARCEL_HOME."""
    if marcel_home is None:
        from marcel_constants import get_marcel_home
        marcel_home = get_marcel_home()
    return Path(marcel_home) / DASHBOARD_CLIENT_HEARTBEAT_REL
