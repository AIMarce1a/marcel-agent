"""Session-scoped context variables for the Marcel gateway.

Replaces the old ``os.environ``-based ``MARCEL_SESSION_*`` state with task-local ``ContextVar``s
...
_SESSION_VARS = (
    ...
    "MARCEL_SESSION_PLATFORM", "MARCEL_SESSION_SOURCE", "MARCEL_SESSION_CHAT_ID",
    "MARCEL_SESSION_CHAT_TYPE", "MARCEL_SESSION_CHAT_NAME", "MARCEL_SESSION_THREAD_ID",
    "MARCEL_SESSION_USER_ID", "MARCEL_SESSION_USER_ID_ALT", "MARCEL_SESSION_USER_NAME",
    "MARCEL_SESSION_SCOPE_ID", "MARCEL_SESSION_KEY", "MARCEL_SESSION_ID",
    "MARCEL_UI_SESSION_ID", "MARCEL_SESSION_MESSAGE_ID", "MARCEL_SESSION_PROFILE",
    "MARCEL_BROWSER_CONTROL_PRINCIPAL", "MARCEL_BROWSER_CONTROL_TRANSPORT_FAMILY",
    "MARCEL_CRON_SESSION",
))
...
_SESSION_ASYNC_DELIVERY = ContextVar("MARCEL_SESSION_ASYNC_DELIVERY", default=_UNSET)

# Cron auto-delivery vars, set per-job in run_job() so concurrent jobs don't clobber.
_CRON_AUTO_DELIVER_PLATFORM = ContextVar("MARCEL_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID = ContextVar("MARCEL_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID = ContextVar("MARCEL_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)
...
def set_current_session_id(session_id: str) -> None:
    """Synchronize ``MARCEL_SESSION_ID`` across ContextVar and ``os.environ`` (tools read it
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session var by legacy ``MARCEL_SESSION_*`` name; drop-in for os.getenv.  The
...
# Surfaces that are not a human chat channel (gateway binds MARCEL_SESSION_PLATFORM, CLI/TUI/
# desktop bind MARCEL_SESSION_SOURCE, so both are consulted).  Default-deny: an unrecognized
...
def session_is_messaging_surface() -> bool:
    ...
    ``MARCEL_PLATFORM``, then the session platform, then the session source)."""
    platform = os.getenv("MARCEL_PLATFORM") or get_session_env("MARCEL_SESSION_PLATFORM", "")
    idents = (platform, get_session_env("MARCEL_SESSION_SOURCE", ""))
...
def async_delivery_supported() -> bool:
    ...
    (``MARCEL_KANBAN_TASK``: one-shot subprocesses whose parent disappears after the turn)."""
