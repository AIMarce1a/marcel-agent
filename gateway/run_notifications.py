...
class GatewayNotificationsMixin:
    ...
    class _UpdatePaths:
        """Marker files ``marcel update --gateway`` and its watcher exchange under MARCEL_HOME."""
    ...
    def _update_paths(cls) -> "GatewayNotificationsMixin._UpdatePaths":
        ...
        return cls._UpdatePaths(
            pending=_marcel_home / ".update_pending.json",
            claimed=_marcel_home / ".update_pending.claimed.json", output=_marcel_home / ".update_output.txt",
            exit_code=_marcel_home / ".update_exit_code",
            prompt=_marcel_home / ".update_prompt.json", response=_marcel_home / ".update_response",
        )
    ...
    async def _watch_update_progress(
        ...
        """Watch ``marcel update --gateway``, streaming output + forwarding prompts.
        ...
        while loop.time() < deadline:
            if paths.exit_code.exists():
                ...
                with _log_suppressed(logging.WARNING, "Update final notification failed: %s"):
                    ...
                    await target.send(
                        f"✅ {runtime_brand()} update finished." if exit_code == 0
                        else f"❌ {runtime_brand()} update failed (exit code {exit_code})."
                    )
        ...
        if not paths.exit_code.exists():
            ...
            await _flush_buffer()
            with suppress(Exception):
                await target.send(f"❌ {runtime_brand()} update timed out after 30 minutes.")
            self._clear_update_markers(paths, session_key)
            ...
            if chat_id:
                ...
                if output:
                    if len(output) > 3500:
                        output = "…" + output[-3500:]
                    status = (
                        f"✅ {runtime_brand()} update finished."
                        if exit_code == 0 else f"❌ {runtime_brand()} update failed."
                    )
                    msg = f"{status}\n\n```\n{output}\n```"
                else:
                    msg = (
                        f"✅ {runtime_brand()} update finished successfully." if exit_code == 0 else
                        f"❌ {runtime_brand()} update failed. Check the gateway logs or run "
                        f"`{runtime_command('update')}` manually for details."
                    )
    ...
    async def _send_restart_notification(self) -> Optional[tuple[str, str, Optional[str]]]:
        ...
        from gateway.run import _marcel_home, _non_conversational_metadata
        notify_path = _marcel_home / ".restart_notify.json"
    ...
    async def _send_home_channel_startup_notifications(
        ...
        message = f"♻️ Gateway online — {runtime_brand()} is back and ready."
    ...
    async def _send_session_db_warning_notifications(self) -> None:
        ...
        if not error:
            return
        from marcel_state import _default_db_path, classify_persistence_error, format_session_db_unavailable
        if classify_persistence_error(error) == "corrupt":
            # Copy-pasteable, so name the real store (profiles / MARCEL_HOME do not live under ~/.marcel).
            db_path = _default_db_path()
            message = (
                "⚠️ Session database corruption detected. Messages may not be "
                "persisted. Recovery options:\n"
                f"1. Run `{runtime_command('doctor --fix')}`\n"
                "2. Stop the gateway, then recover with:\n"
                f"   {runtime_command('sessions recover')} --source {db_path} "
                "--inspect-only\n"
                ...
                "3. Restore from a backup in ~/.marcel/backups/\n"
                f"Run `{runtime_command('doctor')}` for sanitized diagnostics."
            )
        else:
            message = (
                f"⚠️ Session database unavailable — messages may not be persisted. "
                f"{format_session_db_unavailable()}\nRun `{runtime_command('doctor')}` for diagnostics."
            )
    ...
    async def _inject_watch_notification(self, synth_text: str, evt: dict) -> Optional[bool]:
        ...
        if not source:
            # API-server sessions bind the RAW X-Marcel-Session-Id key, not a structured ``agent:...`` key.
