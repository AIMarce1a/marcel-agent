...
There are two ways to add a platform to the Marcel gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.marcel/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Marcel code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `marcel gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `marcel_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Marcel core.
...
```python
"marcel-your-platform": {
    "description": "Your Platform bot toolset",
    "tools": _MARCEL_CORE_TOOLS,
    "includes": []
},
```

And add it to the `marcel-gateway` composite:

```python
"marcel-gateway": {
    "includes": [..., "marcel-your-platform"]
}
```
...
## 12. Status Display (`marcel_cli/status.py`)
...
## 13. Gateway Setup Wizard (`marcel_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ marcel_cli/ toolsets.py \
  --include="*.py" -l | sort -u
