...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = f"⚕ *{runtime_brand()}*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    ...
    /opt/marcel) is mirrored to MARCEL_HOME so npm install works."""
    import shutil
    ...
    marcel_home_bridge = get_marcel_home() / "scripts" / "whatsapp-bridge"
    try:
        ...
        pass
    if marcel_home_bridge.exists():
        return marcel_home_bridge
    try:
        marcel_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(install_bridge, marcel_home_bridge, dirs_exist_ok=False)
        return marcel_home_bridge
    except Exception:
        return install_bridge
