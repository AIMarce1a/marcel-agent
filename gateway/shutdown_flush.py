...
All hooks write atomic JSON payloads under ``<marcel_home>/pending_messages/``:
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active MARCEL_HOME."""
    from marcel_constants import get_marcel_home
    flush_dir = get_marcel_home() / "pending_messages"
...
def spool_dropped_transcript_message(session_id: str, message: Dict[str, Any]) -> Optional[Path]:
    ...
    message under ``<marcel_home>/pending_messages/``), so a runtime cap rotation no longer silently
