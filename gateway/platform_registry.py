...
class PlatformEntry:
    ...
    required_env: list = field(default_factory=list)  # ``marcel setup`` display
    ...
    plugin_name: str = ""  # owning manifest so ``marcel gateway setup`` can auto-enable it
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved MARCEL_HOME and overlay the
        ...
        # `marcel` invocation, so the import happens only when a lookup asks for it.
