...
class GatewayVoiceMixin:
    ...
    def _sync_voice_mode_state_to_adapter(self, adapter) -> None:
        ...
        if not chat_sets:
            return
        try:
            from marcel_cli.config import load_config  # lazy: no gateway -> marcel_cli module dep
            voice_config = load_config().get("voice") or {}
            auto_tts_default = bool(voice_config.get("auto_tts", False))
            audio_only = bool(voice_config.get("audio_only", False))
        except Exception:
            auto_tts_default = False
            audio_only = False
        if hasattr(adapter, "_auto_tts_default"):
            adapter._auto_tts_default = auto_tts_default
        adapter._auto_tts_audio_only = audio_only
