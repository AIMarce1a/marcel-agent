...
Defenses 1-2 (``_MARCEL_GATEWAY`` guard on ``marcel gateway stop|restart`` /
...
persisted to ``<MARCEL_HOME>/gateway/restart_loop.json`` and CHAIN while gaps stay
...
def _state_path():
    return get_marcel_home() / "gateway" / "restart_loop.json"
