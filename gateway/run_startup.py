...
class GatewayStartupMixin:
    ...
    async def _drain_startup_restore_queue(self) -> int:
        ...
        while queue:
            ...
            with suppress(Exception):
                setattr(event, "_marcel_startup_restore_replay", True)
            await adapter.handle_message(event)
            drained += 1
        return drained
    ...
    async def _recover_unclean_sessions(self) -> tuple[int, int]:
        ...
        with _log_suppressed(logging.WARNING, "Exact active-turn recovery on startup failed: %s"):
            agent_timeout = max(1.0, _float_env("MARCEL_AGENT_TIMEOUT", 1800))
    ...
    def _start_loop_heartbeat_task(self) -> None:
        ...
        with _log_suppressed(logging.DEBUG, "Failed to start gateway loop heartbeat", exc_info=True):
            ...
            task._marcel_supervised_watcher = True  # type: ignore[attr-defined]
    ...
    def _open_faulthandler_log(self):
        ...
        from gateway.run import get_marcel_home
        log_dir = getattr(self.config, "log_dir", None) or os.path.join(str(get_marcel_home()), "logs")
    ...
    def _start_log_startup_environment(self) -> None:
        ...
        if self._gateway_loop is not None:
            ...
            with _log_suppressed(logging.DEBUG, "Startup watchdog disarm failed", exc_info=True):
                from marcel_startup_watchdog import disarm_startup_watchdog
        ...
        with suppress(Exception):
            logger.info(
                ...
                "or MARCEL_MAX_ITERATIONS from .env, or default 500)",
                int(os.getenv("MARCEL_MAX_ITERATIONS", "500")),
            )
        ...
        with suppress(Exception):
            ...
            if _redact_raw.lower() in {"1", "true", "yes", "on"}:
                ...
                logger.warning(
                    "Secret redaction: DISABLED (MARCEL_REDACT_SECRETS=%s). API keys and tokens may appear "
        ...
        with _log_suppressed(logging.DEBUG, "security advisory check failed at gateway startup", exc_info=True):
            from marcel_cli.security_advisories import detect_compromised, gateway_log_message
            _adv_msg = gateway_log_message(detect_compromised())
            if _adv_msg:
                logger.warning("%s", _adv_msg)
                logger.warning(
                    "Run `%s` on the gateway host for full remediation steps.",
                    runtime_command("doctor"),
                )
        ...
        with _log_suppressed(logging.DEBUG, "check_systemd_timing_alignment failed: %s"):
            ...
            if _alignment is not None and _alignment.get("mismatch"):
                logger.warning(
                    ...
                    "mid-drain. Run `%s` to regenerate the unit, or shorten "
                    ...
                    _alignment["expected_min"], runtime_command("gateway install --force"),
                )
    ...
    def _register_config_hooks(fail_fmt: str, *fail_args, level: int = logging.DEBUG) -> None:
        ...
        Gateway has no TTY, so consent must come from --accept-hooks, MARCEL_ACCEPT_HOOKS, or
    ...
    async def _start_recover_previous_run(self) -> None:
        ...
        _clean_marker = _marcel_home / ".clean_shutdown"
    ...
    async def start(self) -> bool:
        ...
        logger.info("Starting %s Gateway...", runtime_brand())
    ...
    async def _handoff_resolve_destination(
        ...
        try:
            new_thread_id = await transport.adapter.create_handoff_thread(
                home_chat_id, f"{runtime_brand()} — {cli_title}",
            )
        except Exception as exc:
