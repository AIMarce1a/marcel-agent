#!/usr/bin/env python3
"""Google Workspace API CLI for Marcel Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Marcel-facing JSON contract and falls back to the Python client
...
# Ensure sibling modules (_marcel_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _marcel_home import get_marcel_home

MARCEL_HOME = get_marcel_home()
TOKEN_PATH = MARCEL_HOME / "google_token.json"
CLIENT_SECRET_PATH = MARCEL_HOME / "google_client_secret.json"
...
def _env_value(key: str) -> str:
    try:
        ...
        return get_env_value_prefer_dotenv(key) or ""
    except ImportError:
...
def _ensure_authenticated():
    env_ready = all(_env_value(key) for key in (
        "GOOGLE_WORKSPACE_CLIENT_ID",
        "GOOGLE_WORKSPACE_CLIENT_SECRET",
        "GOOGLE_WORKSPACE_REFRESH_TOKEN",
    ))
    if not env_ready and not TOKEN_PATH.exists():
...
def _stored_token_scopes() -> list[str]:
    env_scopes = _env_value("GOOGLE_WORKSPACE_SCOPES").split()
    if env_scopes:
        return env_scopes
    try:
...
def _gws_binary() -> str | None:
    override = os.getenv("MARCEL_GWS_BIN")
    if override:
        return override
...
def get_credentials():
    """Load and refresh credentials from secret environment, with legacy-file fallback."""
    _ensure_authenticated()

    if all(_env_value(key) for key in (
        "GOOGLE_WORKSPACE_CLIENT_ID",
        "GOOGLE_WORKSPACE_CLIENT_SECRET",
        "GOOGLE_WORKSPACE_REFRESH_TOKEN",
    )):
        from marcel_cli.auth import get_google_workspace_credentials
        return get_google_workspace_credentials()
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Marcel Agent")
