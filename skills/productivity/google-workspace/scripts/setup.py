#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Marcel Agent.
...
from _marcel_home import display_marcel_home, get_marcel_home
...
PENDING_AUTH_PATH = MARCEL_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Marcel profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Marcel Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Marcel Docker image's "
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Marcel home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_marcel_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Marcel")
