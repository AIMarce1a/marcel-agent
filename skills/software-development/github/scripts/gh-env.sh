#!/usr/bin/env bash
# GitHub environment detection helper for Marcel Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _marcel_env="${MARCEL_HOME:-$HOME/.marcel}/.env"; [ -f "$_marcel_env" ] && grep -q "^GITHUB_TOKEN=" "$_marcel_env" 2>/dev/null; then
