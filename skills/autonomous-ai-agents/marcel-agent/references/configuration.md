...
Edit with `marcel config edit` or `marcel config set section.key value`.
...
`marcel config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `marcel tools` (interactive) or `marcel tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_MARCEL_CORE_TOOLS` is the default bundle most platforms inherit).
