...
Author widget apps for the Marcel TUI (`marcel --tui`): glanceable ambient
...
- The TUI must be in use (`marcel --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.marcel/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.marcel/logs/tui_gateway_crash.log` if a widget never appears.
