...
Set via `marcel model` (picker) or `marcel setup`. 35+ provider profiles ship as
...
| nous | OAuth device code | `marcel auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `marcel auth add openai-codex` |
| qwen-oauth | OAuth | `marcel auth add qwen-oauth` |
| minimax-oauth | OAuth | `marcel auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`marcel auth`).
Fallback chain when the primary fails: `marcel fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`marcel_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   marcel config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
