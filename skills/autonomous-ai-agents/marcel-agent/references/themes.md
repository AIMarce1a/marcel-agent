# Themes / Skins — Author a Marcel Color Theme

Author a Marcel **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`marcel_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.marcel/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Marcel itself.
...
- Write access to the Marcel home dir — `~/.marcel` by default, or `$MARCEL_HOME`
  / the active profile's dir. Skins live in `<marcel-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `marcel config set`).
...
3. `write_file` it to `<marcel-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `marcel config set display.skin default` (or they can `/skin default`).
...
```
marcel skin set <key> <hex>      # e.g. marcel skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `marcel skin set background "#08201f"` changes
only the background; `marcel skin use <name>` / `marcel skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.marcel`** when a profile is active — resolve the real home
  from `$MARCEL_HOME` first, falling back to `~/.marcel`.
...
- **Never hand-edit `config.yaml` to activate.** Use `marcel config set
...
- **You apply it, not the user.** `marcel config set display.skin <name>` is
...
- `read_file` the written `<marcel-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `marcel config get display.skin` and confirm it reports `<name>`.
