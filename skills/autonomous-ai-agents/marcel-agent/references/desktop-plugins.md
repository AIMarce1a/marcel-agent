...
Write plugins for the Marcel desktop app: statusbar items, layout panes,
...
system (`~/.marcel/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$MARCEL_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$MARCEL_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The Marcel desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$MARCEL_HOME/desktop-plugins/` (usually
  `~/.marcel/desktop-plugins/`).

## How to Run

1. Create `$MARCEL_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.marcel/...` by default, or `~/.marcel/profiles/<profile>/...` under a
...
The ONLY import surface is `@marcel/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Marcel
  ...
  (`marcel://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.marcel/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@marcel/plugin-sdk`, `react`, and
