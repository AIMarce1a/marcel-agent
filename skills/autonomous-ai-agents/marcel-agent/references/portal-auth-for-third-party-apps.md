...
## Layer 1 — Is this thing a Marcel plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Marcel process**. Its LLM calls go through Marcel's already-configured provider. | Already uses Portal if user's Marcel is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Marcel. That LLM call is already authenticated
via Marcel's provider config. The plugin's own server auth (e.g.
...
What Marcel Agent has that *feels* like OAuth — `marcel login --provider nous`
opening a browser, user signs in, token lands in `~/.marcel/auth.json` — is a
**Marcel-specific browser flow**. Under the hood it produces a credential
Marcel uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Marcel's existing Portal credential out of `~/.marcel/auth.json`.
...
rides on the credential Marcel already holds.

Where this could live in Marcel:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Marcel itself.
...
1. First decide: Marcel plugin (runs inside Marcel) or separate app? If plugin,
   it already uses Portal via Marcel's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Marcel agent itself") is fully supported. Marcel treats agent
...
pitch in the Tool Gateway announcement is specifically about Marcel Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Marcel is configured to use Portal as its provider. It is
