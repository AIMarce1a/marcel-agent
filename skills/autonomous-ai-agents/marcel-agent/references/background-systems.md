...
the `cronjob` tool, the `marcel cron` CLI (`list`, `add`, `edit`,
...
- **CLI:** `marcel curator <verb>` — `status`, `usage`, `run`, `pause`,
...
- **Cost:** the deterministic inactivity/prune sweep runs for free. The
  ...
  **off by default** — opt in with `curator.consolidate: true` or
  `marcel curator run --consolidate`. Routine background curation costs
  zero tokens.
- **Telemetry:** sidecar at `~/.marcel/skills/.usage.json` holds
...
Users drive it via `marcel kanban <verb>`; dispatcher-spawned workers
see a focused `kanban_*` toolset gated by `MARCEL_KANBAN_TASK`, and
...
- **Isolation:** board is the hard boundary (workers get
  `MARCEL_KANBAN_BOARD` pinned in env); tenant is a soft namespace
