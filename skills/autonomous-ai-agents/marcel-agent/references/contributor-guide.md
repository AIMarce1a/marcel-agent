...
Config: `~/.marcel/config.yaml` (settings), `~/.marcel/.env` (API keys) — both under `$MARCEL_HOME` when it is set.
...
`_MARCEL_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_marcel_home()` for paths,
never hardcode `~/.marcel`. For custom/local-only tools, write a plugin in
`~/.marcel/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `marcel_cli/commands.py`
...
- Tests auto-redirect `MARCEL_HOME` to temp dirs — never touch real `~/.marcel/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/marcel_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_marcel_home()` from `marcel_constants` for all paths (profile-safe)
