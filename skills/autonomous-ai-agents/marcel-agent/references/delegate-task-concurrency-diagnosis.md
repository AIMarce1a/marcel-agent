...
**three** code paths in Marcel that cap a batch. If none of them fired, the
cap came from the **model itself** — not from Marcel — and the user's
narration of "the runtime caps at N" is the model rationalising its own
choice.

## The three real caps in Marcel
...
3. **Cost-warning** — same `_get_max_concurrent_children()`. When the
   ...
   log line** — it does not cap anything. Easy to mis-read as "Marcel is
   refusing my value."
...
marcel config get delegation.max_concurrent_children

# 2. Did Marcel' truncator or rejector actually fire?
grep -E "Truncated.*delegate_task|Too many tasks" ~/.marcel/logs/agent.log | tail
# If neither line appears, neither cap path executed.

# 3. Confirm the resolver returns what config says (in venv with marcel on path)
...
appears, **the cap is the model**, not Marcel.
