...
Ledger path resolution (first wins):
  --ledger PATH
  $MARCEL_CITATION_LEDGER
  $MARCEL_HOME/cache/citations/ledger.json
"""
...
from _marcel_home import get_marcel_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("MARCEL_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_marcel_home() / "cache" / "citations" / "ledger.json"
