...
an unlinked SQLite DB/WAL/SHM generation.  ``marcel_state`` supplies only the
...
_MARCEL_EXECUTABLES = frozenset({"marcel", "marcel-agent", "marcel-acp"})
_MARCEL_PYTHON_MODULES = frozenset({"acp_adapter", "marcel_cli.main"})
_MARCEL_PYTHON_SCRIPTS = frozenset({"marcel_cli/main.py", "run_agent.py"})
...
def _looks_like_marcel(argv: Sequence[str]) -> bool:
    """Return whether argv identifies a supported Marcel execution target."""
    if not argv:
        return False
    program = os.path.basename(argv[0]).lower().removesuffix(".exe")
    if program in _MARCEL_EXECUTABLES:
        return True
    ...
    if kind == "module":
        return normalized in _MARCEL_PYTHON_MODULES
    return any(
        normalized == script or normalized.endswith(f"/{script}")
        for script in _MARCEL_PYTHON_SCRIPTS
    )
        ...
        try:
            ...
            for pid_str in os.listdir("/proc"):
                ...
                try:
                    ...
                    if argv is not None and _looks_like_marcel(argv):
                        cmdline = " ".join(argv)
