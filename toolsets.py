...
_MARCEL_WEBHOOK_SAFE_TOOLS = ["web_search", "web_extract", "vision_analyze", "clarify"]
...
def _bundle(description, extras=()):
    """A `marcel-*` platform bundle: the shared core tools plus optional platform extras."""
    return _ts(description, _MARCEL_CORE_TOOLS + list(extras))


def _core_without(*excluded, kanban=True):
    """_MARCEL_CORE_TOOLS minus *excluded* (and, unless kanban=True, every kanban_* tool); order preserved."""
    return [t for t in _MARCEL_CORE_TOOLS if t not in excluded and (kanban or not t.startswith("kanban_"))]
...
TOOLSETS = {
    ...
    "x_search": _ts(
        ...
        "(SuperGrok OAuth or XAI_API_KEY). Off by default; enable in `marcel tools` → "
        "X (Twitter) Search.",
        ["x_search"],
    ),
    ...
    "video_gen": _ts(
        ...
        "separate tools. Configure via ``marcel tools`` → Video Generation.",
    ...
    "browser": _ts(
        ...
        "iframes, hold-click)",
        [t for t in _MARCEL_CORE_TOOLS if t.startswith("browser_")],
    ),
    "cronjob": _ts(
    ...
    "kanban": _ts(
        ...
        "the kanban dispatcher (MARCEL_KANBAN_TASK env set). The dispatcher runs "
        ...
        [t for t in _MARCEL_CORE_TOOLS if t.startswith("kanban_")],
    ),
    ...
    # config (see the non-configurable-toolset recovery loop in marcel_cli/tools_config.py).
    "coding": _ts(
    ...
    # Full Marcel toolsets (CLI + messaging platforms). All share the core tools;
    # there is deliberately no agent-callable send_message tool. marcel-acp is the
    ...
    "marcel-cli": _bundle("Full interactive CLI toolset - all default tools plus cronjob management"),

    # Mirrors marcel-cli; `marcel tools` platform config filters it down and
    # _get_platform_tools() drops _DEFAULT_OFF_TOOLSETS unless user-enabled.
    "marcel-cron": _bundle("Default cron toolset - same core tools as marcel-cli; gated by `marcel tools`"),
    "marcel-telegram": _bundle("Telegram bot toolset - full access for personal use (terminal has safety checks)"),
    "marcel-discord": _bundle(
        "Discord bot toolset - full access (terminal has safety checks via dangerous "
        "command approval)",
        ["discord", "discord_admin"],
    ),
    "marcel-whatsapp": _bundle("WhatsApp bot toolset - similar to Telegram (personal messaging, more trusted)"),
    "marcel-slack": _bundle("Slack bot toolset - full access for workspace use (terminal has safety checks)"),
    "marcel-signal": _bundle("Signal bot toolset - encrypted messaging platform (full access)"),
    "marcel-bluebubbles": _bundle("BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server"),
    "marcel-homeassistant": _bundle("Home Assistant bot toolset - smart home event monitoring and control"),
    "marcel-email": _bundle("Email bot toolset - interact with Marcel via email (IMAP/SMTP)"),
    "marcel-mattermost": _bundle("Mattermost bot toolset - self-hosted team messaging (full access)"),
    "marcel-matrix": _bundle("Matrix bot toolset - decentralized encrypted messaging (full access)"),
    "marcel-dingtalk": _bundle("DingTalk bot toolset - enterprise messaging platform (full access)"),
    "marcel-feishu": _bundle("Feishu/Lark bot toolset - enterprise messaging via Feishu/Lark (full access)", _FEISHU_TOOLS),
    "marcel-weixin": _bundle("Weixin bot toolset - personal WeChat messaging via iLink (full access)"),
    "marcel-qqbot": _bundle("QQBot toolset - QQ messaging via Official Bot API v2 (full access)"),
    "marcel-wecom": _bundle("WeCom bot toolset - enterprise WeChat messaging (full access)"),
    "marcel-wecom-callback": _bundle("WeCom callback toolset - enterprise self-built app messaging (full access)"),
    "marcel-yuanbao": {
        "description": "Yuanbao Bot 元宝消息平台工具集 - 群信息、成员查询、私聊、贴纸表情",
        "tools": _MARCEL_CORE_TOOLS + _YUANBAO_TOOLS,
        "module": "tools.yuanbao_tools",
        "includes": [],
    },
    "marcel-sms": _bundle("SMS bot toolset - interact with Marcel via SMS (Twilio)"),
    "marcel-webhook": _ts("Webhook toolset - receive and process external webhook events", _MARCEL_WEBHOOK_SAFE_TOOLS),
    "marcel-gateway": _ts(
        "Gateway toolset - union of all messaging platform tools",
        [],
        includes=[
            "marcel-telegram", "marcel-discord", "marcel-whatsapp", "marcel-slack",
            "marcel-signal", "marcel-bluebubbles", "marcel-homeassistant", "marcel-email",
            "marcel-sms", "marcel-mattermost", "marcel-matrix", "marcel-dingtalk",
            "marcel-feishu", "marcel-wecom", "marcel-wecom-callback", "marcel-weixin",
            "marcel-qqbot", "marcel-webhook", "marcel-yuanbao",
        ],
    ),
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """A bundle's tools minus _MARCEL_CORE_TOOLS (one level of includes).

    Disabling a `core + extras` bundle must not strip the core tools every other
    toolset shares. One `includes` pass suffices (only marcel-gateway nests
    bundles). Unknown names: full resolution minus core.
    """
    core = set(_MARCEL_CORE_TOOLS)
...
def _plugin_platform_bundle(name: str) -> List[str]:
    """Implicit `marcel-<platform>` bundle for a registered plugin platform: core
    tools plus whatever the plugin registered under the platform name. [] otherwise."""
    if not name.startswith("marcel-"):
        return []
    platform_name = name[len("marcel-"):]
    try:
