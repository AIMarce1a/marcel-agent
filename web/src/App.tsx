...
const SIDEBAR_COLLAPSED_KEY = "marcel-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="marcel-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="marcel-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateMarcel = status?.can_update_marcel === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'marcel update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "marcel update";
    return (
      t.status.updateMarcelConfirmMessage ??
      `This will run 'marcel update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateMarcelConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateMarcel) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateMarcel,
      runningLabel: t.status.updatingMarcel,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Marcel gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateMarcelConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateMarcelConfirmTitle ?? `${t.status.updateMarcel}?`}
    />
    </>
  );
}
