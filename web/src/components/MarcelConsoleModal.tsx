...
interface MarcelConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function MarcelConsoleModal({ open, onClose }: MarcelConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("marcel> ");
  const inputPromptRef = useRef("marcel> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "marcel> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Marcel Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="marcel-console-title"
    >
      <div
        className={cn(
          themedBody,
