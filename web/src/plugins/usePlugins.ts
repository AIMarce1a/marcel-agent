...
import { api, MARCEL_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "marcel:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${MARCEL_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${MARCEL_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?marcel_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-marcel-plugin", manifest.name);
