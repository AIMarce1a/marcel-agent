/**
 * ChatPage — embeds `marcel --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (marcel_cli/web_server.py)                          .
 *          ▼                                                              .
...
const PTY_ATTACH_TOKEN_KEY = "marcel.pty.token.chat";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__MARCEL_SESSION_TOKEN__ &&
    !window.__MARCEL_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `marcel dashboard`, not directly."
      : null,
  );
  ...
  const [chatPanelCollapsed, setChatPanelCollapsed] = useState(
    () => localStorage.getItem("marcel-chat-panel-collapsed") === "1",
  );
  const toggleChatPanel = useCallback(() => {
    setChatPanelCollapsed((prev) => {
      const next = !prev;
      localStorage.setItem("marcel-chat-panel-collapsed", next ? "1" : "0");
      return next;
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const gated = !!window.__MARCEL_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Marcel TUI has enabled xterm mouse-events
    ...
    //      payloads upload to MARCEL_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to MARCEL_HOME/images, then drive `/image`
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[marcel-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__MARCEL_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets MARCEL_HOME pointed at the
    ...
    // forwarding them into Marcel. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="marcel-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
