...
MARCEL_SIMPLEX_TEXT_BATCH_DELAY (quiet seconds, default 0.8, merging rapid-fire inbound text).
...
_CORR_PREFIX = "marcel-"  # marks requests we sent so our own echoes can be ignored
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(os.getenv("MARCEL_SIMPLEX_TEXT_BATCH_DELAY", "0.8"))
...
async def _standalone_send(
    ...
    this process (``marcel cron``). ``thread_id``/``force_document`` are signature parity only;
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``marcel setup gateway`` → SimpleX; writes ``~/.marcel/.env``."""
    print(
        "\nSimpleX Chat setup\n------------------\nRequirements:\n"
    ...
    try:
        ...
        print("marcel_cli.config not available; set SIMPLEX_* vars manually in ~/.marcel/.env")
        return
