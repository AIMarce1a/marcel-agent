...
``(id, projectSecret)``. Storage: runtime SDK creds -> ``~/.marcel/.env``; management
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a marcel dependency
...
# published CLI client until Marcel gets its own client_id.
...
DEFAULT_PROJECT_NAME = "Marcel Agent"
DEFAULT_POLL_INTERVAL = 5  # RFC 8628 polling defaults; Photon's `interval` / `expires_in` win
DEFAULT_POLL_TIMEOUT = 1800
E164_RE = re.compile(r"^\+[1-9]\d{6,14}$")


# -- auth.json helpers (shares the file with the rest of marcel-agent) ------------

def _auth_json_path() -> Path:
    """``~/.marcel/auth.json`` honouring the active Marcel profile."""
    try:
        from marcel_constants import get_marcel_home
        return Path(get_marcel_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.marcel")) / "auth.json"
...
def _store_pool_record(key: str, record: Dict[str, Any]) -> None:
    ...
    from marcel_cli.auth import _auth_store_lock
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.marcel/.env`` (secret never bound to a printable local
    in a caller — CodeQL clean flow)."""
    try:
        from marcel_cli.config import save_env_value
    except ImportError:
        logger.warning("photon: marcel_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    rows = [
        ...
        "──────────────────────",
        "  device token        : " + (
            "✓ stored" if load_photon_token() else "✗ missing (run `marcel photon setup`)"),
        "  project id          : " + (sid if sid else "✗ missing"),
        "  project secret      : " + ("✓ stored" if sec else "✗ missing"),
        "  my number           : " + (phone if phone else "✗ missing (run `marcel photon setup --phone ...`)"),
        "  assigned number     : " + (assigned if assigned else "✗ missing (run `marcel photon setup`)")]
    emit("\n".join(rows))
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_token() -> str:
        return (
            ...
            else "✗ missing (run `marcel photon setup`)"
        )
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `marcel photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `marcel photon setup`)"
