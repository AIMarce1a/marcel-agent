// Marcel Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Marcel): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Marcel -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Marcel-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `marcel photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Marcel can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "marcel-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Marcel'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Marcel allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Marcel, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-marcel-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Marcel supplied them so a known-good
