...
Small Node helper that bridges Marcel Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Marcel is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Marcel-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Marcel can receive inbound messages
...
The Marcel plugin's `marcel photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Marcel is Python, so replies go through this sidecar
