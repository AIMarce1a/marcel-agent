...
// `message.content.text` before Marcel can see it. We rewrite the two inbound
...
const MARKER = "Marcel patch: Preserve mixed text + attachment iMessage payloads";
...
export function patchSpectrumTs(root = scriptDir()) {
  ...
  for (const file of files) {
    ...
    // Marcel to patch; keep the legacy v8 path below for older pinned installs.
    if (
