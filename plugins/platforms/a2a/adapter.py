...
_DEFAULT_DESCRIPTION = "Marcel Agent — a general-purpose agent reachable over A2A."
...
def _default_agent_name() -> str:
    ...
    try:
        import socket
        return f"marcel-{socket.gethostname()}"
    except Exception:
        return "marcel-agent"
...
def _active_profile_name() -> str:
    try:
        ...
        return os.getenv("MARCEL_PROFILE", "default") or "default"
...
def _profile_home(profile: str) -> Optional[str]:
    ...
    if profile and profile != "default":
        return os.path.expanduser(f"~/.marcel/profiles/{profile}")
    with contextlib.suppress(Exception):
        from marcel_cli.config import get_marcel_home
        return str(get_marcel_home())
    return None
...
class A2AAdapter(BasePlatformAdapter):
    ...
    def _load_served_agents(self, extra: dict) -> dict[str, dict]:
        ...
        for key, val in items:
            ...
            agents[slug] = {
                ...
                "name": str(val.get("name") or f"Marcel {slug}"),
                "description": str(val.get("description") or f"Marcel profile '{profile or slug}' exposed over A2A."),
    ...
    def _forward_to_profile(self, agent: dict, peer: str, context_id: str, framed_text: str) -> tuple[str, str]:
        """Forward a routed task to another local profile via ``marcel chat``. First contact creates a
        ...
        with self._forward_lock(key):
            ...
            cmd = ["marcel", "chat", "-q", framed_text, "-Q", "--source", "a2a"] + (["--resume", session_id] if session_id else [])
            env = {**os.environ, "MARCEL_A2A_PEER": peer}
            if home := _profile_home(profile):
                env["MARCEL_HOME"] = home
            start = time.time()
            try:
