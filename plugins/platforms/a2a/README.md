# A2A — Agent-to-Agent protocol for Marcel
...
peer (another Marcel, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
marcel gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Marcel serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.marcel/a2a_audit.jsonl`.
- Conversations persist to `~/.marcel/a2a_conversations/` — they survive context
