...
(enabled/policy/allow_from) falling back independently. Config ~/.marcel/feishu_comment_rules.json (mtime-cached,
hot-reload); pairing store ~/.marcel/feishu_comment_pairing.json."""
...
# long after profile/MARCEL_HOME overrides have been applied, so freezing is safe.
RULES_FILE = get_marcel_home() / "feishu_comment_rules.json"
PAIRING_FILE = get_marcel_home() / "feishu_comment_pairing.json"
...
def _main() -> int:
    try:
        __import__("marcel_cli.env_loader", fromlist=["load_marcel_dotenv"]).load_marcel_dotenv()
    except Exception:
        pass
