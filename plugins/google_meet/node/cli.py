"""`marcel meet node ...` subcommand tree, wired under the ``marcel meet`` parser."""
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    run.add_argument("--display-name", default="marcel-meet-node")
...
def _cmd_run(args: argparse.Namespace, reg: NodeRegistry) -> int:
    ...
    print(f"[meet-node] display_name={server.display_name}\n"
          ...
          f"             marcel meet node approve <name> ws://<host>:{args.port} {token}")
    try:
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``marcel meet node ...``; returns a process exit code."""
