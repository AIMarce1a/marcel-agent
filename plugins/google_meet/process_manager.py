...
One active meeting at a time, recorded in ``$MARCEL_HOME/workspace/meetings/.active.json``
...
def _root() -> Path:
    return Path(get_marcel_home()) / "workspace" / "meetings"
...
def start(url: str, *, out_dir: Optional[Path] = None, headed: bool = False,
          auth_state: Optional[str] = None, guest_name: str = "Marcel Agent", duration: Optional[str] = None,
    ...
    for name in ("transcript.txt", "status.json"):
        with contextlib.suppress(OSError):
            (out / name).unlink()
    env = {**os.environ, "MARCEL_MEET_URL": url, "MARCEL_MEET_OUT_DIR": str(out),
           "MARCEL_MEET_GUEST_NAME": guest_name}
    for value, var in (
        (headed and "1", "MARCEL_MEET_HEADED"),
        (auth_state, "MARCEL_MEET_AUTH_STATE"),
        (duration, "MARCEL_MEET_DURATION"),
        (mode, "MARCEL_MEET_MODE"),  # bot defaults to transcribe when unset (v1 behavior)
        (realtime_model, "MARCEL_MEET_REALTIME_MODEL"),
        (realtime_voice, "MARCEL_MEET_REALTIME_VOICE"),
        (realtime_instructions, "MARCEL_MEET_REALTIME_INSTRUCTIONS")):
        if value:
            env[var] = value
    ...
    if not realtime_api_key:
        ...
        realtime_api_key = get_secret("MARCEL_MEET_REALTIME_KEY") or get_secret("OPENAI_API_KEY")
    if realtime_api_key:
        env["MARCEL_MEET_REALTIME_KEY"] = realtime_api_key
