...
Auto-tracks and cleans up ephemeral files created during Marcel Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$MARCEL_HOME` and `/tmp/marcel-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `MARCEL_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `MARCEL_HOME` or `/tmp/marcel-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$MARCEL_HOME/disk-cleanup/` is itself excluded
- `$MARCEL_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
