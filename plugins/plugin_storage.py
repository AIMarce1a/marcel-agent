"""Per-plugin persistent storage: ``<marcel home>/plugin-data/<name>/``.

Plugins must NOT park state in ``<marcel home>/plugins/<name>/`` (the install dir, deleted by
...
# Mirrors the plugin-name shape `marcel plugins install` accepts (no separators/traversal).
...
def plugin_data_dir(name: str) -> Path:
    """Return (and create) ``<marcel home>/plugin-data/<name>/``; resolves ``get_marcel_home()`` on
    ...
    root = get_marcel_home() / "plugin-data" / _validate_name(name)
