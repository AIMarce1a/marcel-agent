"""Interactive ``marcel memory setup`` wizard for the OpenViking provider.

Pure UI flow: prompts, menus, and persistence of the chosen connection (Marcel
``.env`` only, or mirrored to an ``ovcli.conf.<name>`` profile that Marcel then
...
def _save_marcel_only_config(*, config: dict, provider_config: dict, env_path: Path, values: dict) -> None:
    ov = _ov()
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Marcel session to activate.\n")
...
def _run_create_profile_setup(*, prompt, select, cancelled, config: dict, provider_config: dict, env_path: Path) -> bool | object:
    ...
    save_choice = select("  Save OpenViking config",
                         [("Keep in Marcel only", "write values only to Marcel .env"),
    ...
    _save_marcel_only_config(config=config, provider_config=provider_config, env_path=env_path, values=values)
    _print_openviking_ready("Connection saved to Marcel .env.")
    return True


def run_setup(marcel_home: str, config: dict) -> None:
    """Entry point for ``OpenVikingMemoryProvider.post_setup``."""
    from marcel_cli.config import save_config
    from marcel_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt
