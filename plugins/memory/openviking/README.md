...
- OpenViking server running and reachable from Marcel

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Marcel can identify older servers that expose the legacy status-only health
...
connection values into Marcel, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.marcel/.env`; for a named profile use
...
OpenViking's server config is separate from Marcel:
...
Marcel-side provider config is read from environment variables in the active
profile's `.env`:
...
When `OPENVIKING_API_KEY` is set, Marcel lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Marcel sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
Marcel also sends `User-Agent: openviking-memory-marcel/<version>` on
OpenViking requests. This standard harness identifier contains the Marcel
...
does not ask for a peer ID. Without a configured peer, Marcel sends neither
...
order remains environment, linked OpenViking config, then Marcel YAML. To use
...
on the old implicit `marcel` peer now use user memory for new writes. Without
...
Set `agent: marcel` to restore peer-scoped writes. Memories written at user
...
`viking_remember` creates a one-shot `marcel-remember-<random>` OpenViking
...
not advertised or used. The one-shot session is separate from the live Marcel
...
Use the same OpenViking profile and credentials as Marcel for manual recovery.
...
Marcel built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Marcel action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under user memory, or the configured peer memory directory |

Built-in `replace` and `remove` operations are not mirrored because Marcel
...
file URIs, such as
`viking://user/default/peers/marcel/memories/preferences/mem_abc123.md` (any
