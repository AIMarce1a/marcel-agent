...
core (the code can arrive via the loopback listener here or a ``marcel://``
...
# Loopback redirect registered for the Marcel OAuth client. IP-literal so the browser can't resolve the
...
_DEFAULT_CLIENT_ID = "marcel-agent"
...
_CALLBACK_HTML = _CALLBACK_PAGE.format(
    title="Honcho connected", body="Connected to Honcho. You can close this tab and return to Marcel."
).encode()
...
def start_loopback_flow_background(
    *, config_path: Path | None = None, host: str | None = None, source: str = "marcel-desktop",
    timeout: float = 300.0,
) -> dict[str, str]:
    ...
    global _flow_thread
    # Resolve under the caller's profile scope NOW — a context-local MARCEL_HOME override can't reach the worker.
