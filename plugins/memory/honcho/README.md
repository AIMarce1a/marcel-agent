...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/marcel>
...
```bash
marcel memory setup honcho   # configure Honcho directly (works on a fresh install)
marcel memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
marcel config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.marcel/.env
```

> `marcel honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `marcel memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `marcel model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$MARCEL_HOME/honcho.json` | Profile-local (isolated Marcel instances) |
| 2 | `~/.marcel/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Marcel profile: `marcel` (default) or `marcel_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `marcel honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Marcel session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`marcel-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`marcel`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-marcel-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening marcel in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Marcel session ID (timestamp + hex). Every `marcel` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Marcel profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`marcel`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Marcel profile: `marcel` (default) or `marcel_<profile>` (e.g. `marcel -p coder` -> host key `marcel_coder`). Older `marcel.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `MARCEL_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `marcel-agent`) |
...
|---------|-------------|
| `marcel memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `marcel honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `marcel memory setup`) |
| `marcel honcho status` | Show resolved config for active profile |
| `marcel honcho enable` / `disable` | Toggle Honcho for active profile |
| `marcel honcho mode <mode>` | Change recall or observation mode |
| `marcel honcho peer --user <name>` | Update user peer name |
| `marcel honcho peer --ai <name>` | Update AI peer name |
| `marcel honcho tokens --context <N>` | Set context token budget |
| `marcel honcho tokens --dialectic <N>` | Set dialectic max chars |
| `marcel honcho map <name>` | Map current directory to a session name |
| `marcel honcho sync` | Create host blocks for all Marcel profiles |
