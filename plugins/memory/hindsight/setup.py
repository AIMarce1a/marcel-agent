"""`marcel memory setup` wizard for the Hindsight provider (``post_setup``)."""
...
def _select(title: str, items: list, values: list, current) -> str | None:
    ...
    from marcel_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
...
def _prompt_embedded_llm(llm_provider: str, provider_config: dict, env_writes: dict, marcel_env: Path) -> None:
    ...
    env_writes["HINDSIGHT_LLM_API_KEY"] = llm_key or _load_simple_env(marcel_env).get("HINDSIGHT_LLM_API_KEY", "")


def run_setup(provider, marcel_home: str, config: dict) -> None:
    ...
    env_writes: dict = {}
    marcel_env = Path(marcel_home) / ".env"
    ...
    if mode == "cloud":
        ...
        _prompt_embedded_llm(llm_provider, provider_config, env_writes, marcel_env)

    provider_config.setdefault("bank_id", "marcel")
    ...
    save_config(config)
    provider.save_config(provider_config, marcel_home)
    if env_writes:
        _write_env(marcel_env, env_writes)
    ...
    if _hs_templates.supported_for_mode(mode):
        from marcel_cli.memory_setup import _CANCELLED, _curses_select
        ...
        _hs_templates.run_template_step(
            ...
            bank_id=provider_config.get("bank_id", "marcel"),
    ...
    if mode == "local_embedded":
        ...
        with contextlib.suppress(Exception):
            materialized_config = json.loads(
                (Path(marcel_home) / "hindsight" / "config.json").read_text(encoding="utf-8")
            )
        llm_api_key = (
            env_writes.get("HINDSIGHT_LLM_API_KEY", "")
            or _load_simple_env(marcel_env).get("HINDSIGHT_LLM_API_KEY", "")
