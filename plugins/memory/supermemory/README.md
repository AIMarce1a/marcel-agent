...
- Hosted: API key from [app.supermemory.ai/integrations?connect=marcel](http://app.supermemory.ai/integrations?connect=marcel)
...
Before running `marcel memory setup`, add the local endpoint to
...
Then run `marcel memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$MARCEL_HOME/supermemory.json`
...
| `container_tag` | `marcel` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `marcel-{identity}` → `marcel-coder`). |
...
`https://api.supermemory.ai`. Marcel resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: marcel`, and document writes stamp
`metadata.sm_source: marcel`. This is a **functional routing key, not telemetry**:
it groups Marcel-written memories into a dedicated "Marcel" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Marcel profile:

```json
{
  "container_tag": "marcel-{identity}"
}
```

For a profile named `coder`, this resolves to `marcel-coder`. The default profile resolves to `marcel-default`. Without `{identity}`, all profiles share the same container.
