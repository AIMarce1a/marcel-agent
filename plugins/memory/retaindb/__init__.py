...
class _Client:
    ...
    def _headers(self, path: str, json_body: bool = True) -> dict:
        ...
        return {"Authorization": f"Bearer {token}", "x-sdk-runtime": "marcel-plugin",
...
class RetainDBMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._user_id, self._session_id, self._agent_id = "default", "", "marcel"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Project: RETAINDB_PROJECT > config.yaml > marcel-<profile> > "default" (API auto-creates "default").
        project = os.environ.get("RETAINDB_PROJECT") or cfg.get("project")
        if not project:
            profile_name = os.path.basename(str(kwargs.get("marcel_home", "")))
            project = f"marcel-{profile_name}" if profile_name not in {"", ".marcel"} else "default"
        ...
        self._agent_id = kwargs.get("agent_id", "marcel") or "marcel"
