...
```bash
marcel memory setup    # select "mem0"
```
...
Behavioral settings live in `$MARCEL_HOME/mem0.json` (set them via `marcel memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.marcel/.env`.
...
| `user_id` | `marcel-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Marcel session and call `mem0_search` — it connects to your server.
...
```bash
marcel memory setup mem0 --mode oss \
...
```bash
marcel memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$MARCEL_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.marcel/mem0_qdrant"}}
  }
}
```
...
```bash
marcel memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$MARCEL_HOME/mem0.json`).
