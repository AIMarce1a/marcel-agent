...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Marcel finds ``~/.openclaw/``: migrate first, cleanup (which breaks OpenClaw) after."""
    return (
        "A legacy OpenClaw directory was detected at ~/.openclaw/.\n"
        "To port your config, memory, and skills over to Marcel, run `marcel claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, run `marcel claw cleanup` "
...
def pending_google_workspace_directive(config: Mapping[str, Any]) -> str:
    """Return a first-contact note when Google Workspace still needs authorization."""
    accounts = config.get("accounts") if isinstance(config, Mapping) else None
    profiles = accounts.get("profiles") if isinstance(accounts, Mapping) else None
    if not isinstance(profiles, list):
        return ""
    pending = any(
        isinstance(profile, Mapping)
        and str(profile.get("kind") or profile.get("type") or "").lower() == "google_workspace"
        and (
            profile.get("enabled") is not True
            or str(profile.get("authorization_status") or "").lower() != "verified"
        )
    ...
    if not pending:
        return ""
    return (
        "\n\n[System note: Google Workspace was selected during setup, but its authorization "
        "is still pending. In your first reply, clearly tell the user Google is not active yet "
        "and offer to help complete authorization. Do not access Gmail, Calendar, Drive, Docs, "
        "Sheets, or Contacts until verification succeeds. If the user accepts help, explain that "
        "a Google Cloud OAuth Client ID and Client Secret are required, then guide them through "
        "the Marcel authorization flow that opens Google's consent link. Never ask them to paste "
        "a client secret, token, or authorization code into chat.]"
    )
