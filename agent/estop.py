...
``marcel pause`` writes a sentinel at ``$MARCEL_HOME/ESTOP``; ``marcel resume``
...
(fail safe, e.g. ``touch ~/.marcel/ESTOP``). Ported from gastownhall/gastown estop.go (MIT).
"""
...
# Same profile-aware / fleet-root resolvers the file-safety guards use (fail-open to ~/.marcel).
from agent.file_safety import _marcel_home_path as _marcel_home, _marcel_root_path as _canonical_root
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel this process would write on `marcel pause`."""
    return _marcel_home() / SENTINEL_NAME


def _candidate_sentinel_paths() -> list:
    """Profile home first, then the fleet root if it is a different directory: a profile
    gateway (MARCEL_HOME=~/.marcel/profiles/<n>) must still honor an operator's ~/.marcel/ESTOP."""
...
def paused_reply() -> Optional[str]:
    ...
    return f"⏸️ Marcel is paused{tag}. New work is on hold; run `marcel resume` to pick things back up."
...
def check_paused(component: str, logger: logging.Logger) -> bool:
    ...
    if first:
        ...
        logger.info(
            "%s dispatch paused by global emergency stop%s — remove with `marcel resume` (%s)",
