...
Every Marcel request to the Nous Portal (main loop, auxiliary client, fallback
paths) must carry the same product-attribution tags, sent in OpenAI-compatible
``extra_body['tags']``: ``["product=marcel-agent", "client=marcel-client-v<__version__>"]``.
The version is read live from ``marcel_cli.__version__`` — do NOT pre-compute it
...
_affinity_scope: ContextVar[Optional[str]] = ContextVar("marcel_affinity_scope", default=None)
...
def marcel_client_tag() -> str:
    """``client=marcel-client-v<MAJOR>.<MINOR>.<PATCH>`` ("unknown" if marcel_cli is unimportable)."""
    try:
        from marcel_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"client=marcel-client-v{__version__}"
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    """
    tags = ["product=marcel-agent", marcel_client_tag()]
