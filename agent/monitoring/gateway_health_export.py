...
_OBSERVABLE_METRIC_NAMES = (
    "marcel.gateway.up", "marcel.gateway.state", "marcel.gateway.active_agents", "marcel.gateway.busy",
    "marcel.gateway.drainable", "marcel.gateway.restart_requested", "marcel.gateway.background_work",
    "marcel.gateway.background_delegations", "marcel.platform.up", "marcel.platform.degraded",
    "marcel.cron.scheduler.heartbeat_age_seconds", "marcel.cron.scheduler.last_success_age_seconds",
    "marcel.cron.scheduler.catch_up_occurrences", "marcel.cron.jobs.enabled", "marcel.cron.jobs.running",
    "marcel.cron.jobs.overdue",
)
...
class GatewayHealthExportRuntime:
    ...
    def shutdown(self) -> None:
        ...
        if closeables:
            worker = threading.Thread(target=_close, name="marcel-gateway-health-export-shutdown", daemon=True)
            worker.start()
...
def _read_runtime_snapshot(config: Dict[str, Any]):
    ...
    try:
        ...
        for name, read in (
            ("marcel.gateway.background_work", _read_background_work_count),
            ("marcel.gateway.background_delegations", _read_background_delegations_count),
        ):
...
def _start_metric_provider(config: Dict[str, Any], sdk: Dict[str, Any]) -> Any:
    ...
    meter = provider.get_meter("marcel.gateway.health")
...
class GatewayDiagnosticLogStreamer(EmitterStreamer):
    ...
    def __init__(self, config: Dict[str, Any], sdk: Dict[str, Any]):
        ...
        self._logger = self._provider.get_logger("marcel.gateway.diagnostics")
        self._sdk = sdk
        self.exported = 0
...
def start_gateway_health_export(config: Dict[str, Any]) -> GatewayHealthExportRuntime:
    ...
    if metrics_on or diagnostics_on:
        try:
            ...
            logger.warning("monitoring.gateway_health_export.enabled but OTLP SDK is unavailable; install 'marcel-agent[otlp]'", exc_info=True)
    ...
    if diagnostics_on:
        try:
            ...
            thread = threading.Thread(target=_run, name="marcel-gateway-health-export", daemon=True)
            thread.start()
