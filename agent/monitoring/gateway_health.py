...
def build_gateway_health_snapshot(
    ...
    base = {
        ...
        "marcel.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
    ...
    metrics: list[GatewayMetric] = [
        metric("marcel.gateway.up", int(bool(gateway_running))),
        metric("marcel.gateway.active_agents", active_agents),
        metric("marcel.gateway.busy", int(bool(busy))),
        metric("marcel.gateway.drainable", int(bool(drainable))),
        metric("marcel.gateway.restart_requested", int(bool(runtime.get("restart_requested")))),
        metric("marcel.gateway.state", 1, **{"marcel.gateway.state": gateway_state}),
    ]
    fatal_count = 0
    events: list[GatewayHealthEvent | GatewayDiagnosticEvent] = []
    ...
    for platform, pdata in platforms.items():
        ...
        pattrs = {"marcel.platform": str(platform), "marcel.platform.state": state}
        metrics.append(metric("marcel.platform.up", int(state in _RUNNING_PLATFORM_STATES), **pattrs))
        metrics.append(metric("marcel.platform.degraded", int(is_degraded), **pattrs, **{"marcel.error_code": error_code}))
        if is_degraded:
