...
_GLOBAL_ENV_EXACT = frozenset({
    # Marcel runtime / deployment
    "MARCEL_HOME", "MARCEL_PROFILE", "MARCEL_GATEWAY_LOCK_DIR",
    "MARCEL_MAX_ITERATIONS", "MARCEL_MAX_TOKENS", "MARCEL_API_TIMEOUT",
    "MARCEL_REDACT_SECRETS", "MARCEL_NOUS_TIMEOUT_SECONDS",
    "_MARCEL_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "MARCEL_KANBAN_DB", "MARCEL_KANBAN_WORKSPACES_ROOT", "MARCEL_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "MARCEL_KANBAN_",
    "MARCEL_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    from marcel_cli.config import _parse_env_value
...
def build_profile_secret_scope(marcel_home: Path) -> Dict[str, str]:
    ...
    secrets = load_env_file(Path(marcel_home) / ".env")
    try:
        from marcel_cli.env_loader import get_secret_source_values
        external_secrets = get_secret_source_values(Path(marcel_home))
    except Exception:
        external_secrets = {}
