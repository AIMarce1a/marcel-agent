...
from marcel_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_kanban() -> bool:
    ...
    if os.getenv("MARCEL_KANBAN_TASK") or os.getenv("MARCEL_KANBAN_BOARD"):
        try:
...
def _load_raw_config() -> Dict[str, Any]:
    """Read config.yaml with an mtime+size keyed cache (no marcel_cli.config import)."""
...
def _home_relative(p: Path) -> Path:
    """Anchor a relative config path at MARCEL_HOME; absolute paths pass through."""
    from marcel_constants import get_marcel_home
    return p if p.is_absolute() else get_marcel_home() / p


# Never disableable: `marcel-agent` is the agent's own operating manual and the
# system prompt points at it unconditionally.
ESSENTIAL_SKILLS: frozenset = frozenset({"marcel-agent"})


def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    """Disabled skill names from config.yaml: global list ∪ platform list
    (*platform* defaults to ``MARCEL_PLATFORM`` / ``MARCEL_SESSION_PLATFORM``)."""
    skills_cfg = _skills_cfg()
    if skills_cfg is None:
        return set()
    from gateway.session_context import get_session_env
    resolved_platform = platform or os.getenv("MARCEL_PLATFORM") or get_session_env("MARCEL_SESSION_PLATFORM")
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``marcel config set`` stores lists as quoted JSON/Python-literal strings;
...
def get_external_skills_dirs() -> List[Path]:
    ...
    are ``~``/``${VAR}`` expanded, relative to MARCEL_HOME; the local skills dir is skipped."""
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def get_skill_create_dir() -> Optional[Path]:
    ...
    relative to MARCEL_HOME; a value equal to the local skills dir counts as unset."""
...
def display_skill_create_dir() -> str:
    ...
    if create_dir is None:
        return f"{display_marcel_home()}/skills/"
...
def get_all_skills_dirs() -> List[Path]:
    """Skill dirs: local ``~/.marcel/skills/`` first, then create_dir, then external.
...
# Project-local skills (<root>/.marcel/skills, <root>/.agents/skills; root = nearest
...
PROJECT_SKILLS_SUBDIRS = (os.path.join(".marcel", "skills"), os.path.join(".agents", "skills"))
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    (MARCEL_HOME itself may live inside a git checkout)."""
    local_skills = get_skills_dir().resolve()
    dirs: List[Path] = []
...
# cached under MARCEL_HOME, never inside the repo); "dangerous" excludes the
# skill from index, list, view and slash commands ("caution" loads, as on the hub).

# ── Project skill quarantine (scan-time injection defense) ──────────────── Trust (`marcel skills trust`)
...
# high-confidence findings only. The scan cache lives under MARCEL_HOME, never inside the repo (we don't
...
def is_quarantined_project_skill(skill_md) -> bool:
    ...
    try:
        ...
        from marcel_constants import get_marcel_home
        cache_dir = get_marcel_home() / "cache" / "project_skill_scans"
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    # (which follows the live profile-scoped MARCEL_HOME), so normalization
    ...
    # ~/.marcel/skills/<name> may be a symlink to a checkout elsewhere, and
...
def _marcel_metadata(frontmatter: Dict[str, Any]) -> Dict[str, Any]:
    """``metadata.marcel`` mapping from frontmatter, or ``{}`` when malformed."""
    metadata = frontmatter.get("metadata")
    marcel = metadata.get("marcel") if isinstance(metadata, dict) else None
    return marcel if isinstance(marcel, dict) else {}
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    marcel = _marcel_metadata(frontmatter)
    return {key: marcel.get(key, []) for key in _CONDITION_KEYS}


def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Extract ``metadata.marcel.config`` declarations (key/description/default/prompt).
    Entries missing ``key`` or ``description`` are skipped; ``prompt`` defaults to the description."""
    raw = _marcel_metadata(frontmatter).get("config")
