...
from agent.prompt_builder import (
    ...
    MARCEL_AGENT_HELP_GUIDANCE, MARCEL_AGENT_HELP_GUIDANCE_NO_SKILLS, KANBAN_GUIDANCE, MARCEL_HELP_GUIDANCE, MEMORY_GUIDANCE,
    USER_PROFILE_GUIDANCE, PARALLEL_TOOL_CALL_GUIDANCE, PLATFORM_HINTS, SESSION_SEARCH_GUIDANCE,
    MARCEL_DEFAULT_AGENT_IDENTITY, SKILLS_GUIDANCE, STEER_CHANNEL_NOTE, TASK_COMPLETION_GUIDANCE, TELEGRAM_RICH_MESSAGES_HINT,
    TOOL_USE_ENFORCEMENT_GUIDANCE, TOOL_USE_ENFORCEMENT_MODELS, drain_truncation_warnings,
)
from agent import prompt_builder as _pb
from agent.branding import is_marcel_invocation
from agent.runtime_cwd import resolve_context_cwd
from marcel_constants import get_default_marcel_root, get_marcel_home
from utils import is_truthy_value

logger = logging.getLogger(__name__)
_PLUGIN_SECTION_FRAME_RE = re.compile(
    r"^## Plugin Context: (?P<id>[a-z0-9][a-z0-9._-]{0,127})\n<!-- marcel-plugin-section-chars:(?P<chars>[0-9]{1,4}) -->\n\n",
    re.MULTILINE,
)
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    """Append the desktop embedded-terminal clarifier when ``MARCEL_DESKTOP_TERMINAL``
    is set (only the desktop's TUI PTY, never the chat backend). Idempotent."""
    if not hint or _TUI_EMBEDDED_PANE_CLARIFIER in hint or not is_truthy_value(os.getenv("MARCEL_DESKTOP_TERMINAL")):
        return hint
...
def _active_profile_name(agent: Any, ambient) -> str:
    ...
    failure. Ambient resolution misreports on threads that lost the MARCEL_HOME
...
def _frozen_plugin_prompt_sections(agent: Any) -> tuple:
    ...
    if isinstance(stored_prompt, str) and stored_prompt:
        ...
        try:
            from marcel_cli.plugins import render_system_prompt_sections
...
def _plugin_section_blocks(sections: tuple, position: str) -> List[str]:
    from marcel_cli.plugins import format_system_prompt_sections
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    A bound MARCEL_HOME ContextVar override wins (the gateway multiplexes
...
def _profile_name_for_home(home: Path) -> str:
    ...
    Uses ``get_default_marcel_root()`` (NOT ``get_marcel_home()``): on a bound
    ...
    try:
        ...
        rel = home.resolve().relative_to((get_default_marcel_root() / "profiles").resolve())
...
def _active_profile_line(agent: Any) -> str:
    """Name the running profile so the agent doesn't conflate ``~/.marcel/skills``
    (default) with ``~/.marcel/profiles/<active>/skills``.  Resolved from the
    ...
    if active_profile == "default":
        ...
        # ROOT (get_marcel_home() on a bound profile session is the PROFILE dir).
        # Without one, keep the ambient (patchable) resolution byte-identical.
        _root_str = str(get_default_marcel_root() if _agent_home_path is not None else get_marcel_home())
        return (
            "Active Marcel profile: default. Other profiles (if any) live "
    ...
    profile_home = str(_agent_home_path) if _agent_home_path is not None else str(get_marcel_home())
    ...
    # default profile's data sits at the ROOT (get_default_marcel_root()), which in ambient profile mode is
    # NOT get_marcel_home().
    default_root = get_default_marcel_root()
    return (
        f"Active Marcel profile: {active_profile}. This session reads "
...
def _timestamp_line(agent: Any) -> str:
    ...
    from marcel_time import get_timezone as _marcel_tz, now as _marcel_now
    now = _marcel_now()
    _bits = _zone_bits(now, _marcel_tz())
...
def _context_files_part(agent: Any, ctx_len: Optional[int], soul_loaded: bool) -> List[str]:
    ...
    fallback they really are so the guard can reject Marcel's bundled AGENTS.md."""
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    _is_marcel = is_marcel_invocation()
    if _is_marcel and not _soul_loaded:
        stable_parts = [MARCEL_DEFAULT_AGENT_IDENTITY]
    # The skill_view() pointer dangles without skill tools OR without the
    # marcel-agent skill installed, so the variant is chosen after the skills
    # index is built; this slot holds its position.
    _help_guidance_slot = len(stable_parts)
    stable_parts.append(MARCEL_HELP_GUIDANCE if _is_marcel else MARCEL_AGENT_HELP_GUIDANCE_NO_SKILLS)
    stable_parts.extend(_guidance_parts(agent))
    skills_prompt = _skills_prompt(agent)
    # Skill-pointer variant requires BOTH skill_view AND the marcel-agent skill
    # in the rendered index (pure string check — inherits the index's stability).
    if not _is_marcel and "skill_view" in (agent.valid_tool_names or set()) and "- marcel-agent:" in skills_prompt:
        stable_parts[_help_guidance_slot] = MARCEL_AGENT_HELP_GUIDANCE
