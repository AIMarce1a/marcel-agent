"""OpenAI-shape bridge shared by Marcel' ACP clients.

ACP has no OpenAI-style ``tools``/``tool_calls`` channel, so Marcel' tool schemas travel INTO the
...
read/edit/execute tools forwards only Marcel' agent-level tools, since re-offering overlapping ones
makes Marcel redo finished work.
"""
...
class StreamChunks(list):
    """Chunk list that also carries response-level attributes (e.g. ``marcel_projected_messages``)
    Marcel reads off the ``create`` result; a plain list would drop them on the stream path."""
