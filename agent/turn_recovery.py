...
def _print_nous_401_diagnostics(agent: Any, api_error: Exception) -> None:
    ...
    _plines(
        agent,
        "   Troubleshooting:",
        f"     • Re-authenticate: {runtime_command('auth add nous')}",
        "     • Check credits / billing: https://portal.nousresearch.com",
        f"     • Verify stored credentials: {display_marcel_home()}/auth.json",
...
def _print_anthropic_401_diagnostics(agent: Any, key: Any) -> None:
    ...
    if is_token_provider(key):
        ...
        _plines(
            ...
            f"   Run `{runtime_command('doctor')}` for credential-chain diagnostics, or",
    ...
    _dhh = display_marcel_home()
    _plines(
        agent,
        "   Troubleshooting:",
        f"     • Check ANTHROPIC_TOKEN in {_dhh}/.env for {runtime_brand()}-managed OAuth/setup tokens",
        ...
        f"     • Legacy cleanup: {runtime_command('config set ANTHROPIC_TOKEN')} \"\"",
        f"     • Clear stale keys: {runtime_command('config set ANTHROPIC_API_KEY')} \"\"",
    )
...
def recover_after_classification(
    ...
    if (
        ...
        try:
            from marcel_cli.models_reasoning_caps import refresh_reasoning_caps_async
...
def _print_nonretryable_auth_guidance(
    ...
    if provider in {"openai-codex", "xai-oauth", "nous"} and status_code == 401:
        if provider == "openai-codex":
            _vlines(
                ...
                f"      2. Then run `{runtime_command('auth')}` to re-authenticate.",
            )
            ...
            _vlines(
                ...
                f"      1. Re-authenticate: {runtime_command('portal')}",
    ...
    _vlines(
        ...
        f"      • Is the key valid? Run: {runtime_command('setup')}",
...
def nonretryable_client_error_result(
    ...
    if classified.reason == FailoverReason.content_policy_blocked:
        _vlines(
            ...
            f"        {runtime_command('fallback add')}   (interactive picker — same as "
            f"`{runtime_command('model')}`)",
        )
    ...
    if classified.reason == FailoverReason.content_policy_blocked:
        _policy_response = (
            ...
            f"(not a {runtime_brand()}/gateway failure).\n\n"
...
def max_retries_exhausted_result(
    ...
    if _is_thinking_timeout:
        _vlines(
            ...
            f"in `~/.marcel/config.yaml` to extend the per-call timeout. ({runtime_brand()}'s built-in floor is 600s for "
...
def log_api_error_attempt(
    ...
    if getattr(api_error, "status_code", None) == 404:
        try:
            from marcel_cli.model_normalize import suggest_prefixed_model_id
        ...
        if _suggestion:
            _blines(
                ...
                f"      Did you mean '{_suggestion}'?  Re-pick it with `{runtime_command('model')}`.",
            )
