...
try:
    import marcel_cli as _marcel_cli

    _MARCEL_VERSION = str(_marcel_cli.__version__)
except Exception:
    _MARCEL_VERSION = "0.0.0"
_API_CLIENT = f"marcel-agent/{_MARCEL_VERSION}"  # client context per Gemini's partner-integration guidance
...
_FREE_TIER_GUIDANCE = (
    ...
    "Marcel typically makes 3-10 API calls per user turn, so the free tier is exhausted in a handful of "
...
_STANDARD_KEY_GUIDANCE = (
    ...
    "generativelanguage.googleapis.com). Then update GEMINI_API_KEY / GOOGLE_API_KEY in ~/.marcel/.env and "
...
_MISSING_KEY_ERROR = (
    ...
    "environment / ~/.marcel/.env (get one at https://aistudio.google.com/app/apikey), or run `marcel setup` to "
...
class GeminiAPIError(Exception):
    """Error shape compatible with Marcel retry/error classification."""
...
class GeminiNativeClient:
    ...
    # (No MARCEL_SKIP_ASYNC_WRAP — the async path has a real conversion, AsyncGeminiNativeClient.)
