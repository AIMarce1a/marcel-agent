"""Marcel-tools-as-MCP server for the codex_app_server runtime.

Codex owns the loop and tool list there, so a curated subset of Marcel tools is
exposed over stdio MCP; codex registers it via ``~/.codex/config.toml
[mcp_servers.marcel-tools]``. Run: ``python -m agent.transports.marcel_tools_mcp_server``.
"""
...
# Each name MUST match a registered Marcel tool ``model_tools.handle_function_call()`` can dispatch.
...
EXPOSED_TOOLS: tuple[str, ...] = (
    ...
    # Kanban handoff tools: stateless (read MARCEL_KANBAN_TASK, write kanban.db).
    ...
    # Orchestrator-only (the kanban tool gates them on MARCEL_KANBAN_TASK unset).
    "kanban_create", "kanban_unblock", "kanban_link",
)


def _build_server() -> Any:
    """Create the MCP server with Marcel tools attached (lazy imports: importable without ``mcp``)."""
    try:
        # mcp 2.0 renamed `mcp.server.fastmcp` to `mcp.server.MCPServer` (same surface).
        from mcp.server import MCPServer
    except ImportError as exc:  # pragma: no cover - install hint
        raise ImportError(f"marcel-tools MCP server requires the 'mcp' package: {exc}") from exc
    ...
    mcp = MCPServer(
        "marcel-tools",
        instructions=(
            "Marcel Agent's tool surface, exposed for use inside a Codex "
    ...
    # Authoritative Marcel schemas so MCP clients see the same parameter docs the model does.
    all_defs = {
    ...
    for name in EXPOSED_TOOLS:
        ...
        if spec is None:
            logger.debug("skipping %s — not registered in this Marcel process", name)
            continue
        description = spec.get("description") or f"Marcel {name} tool"
    ...
    logger.info("marcel-tools MCP server registered %d/%d tools", exposed_count, len(EXPOSED_TOOLS))
    return mcp


def main(argv: Optional[list[str]] = None) -> int:
    """Entry point for `python -m agent.transports.marcel_tools_mcp_server`."""
    ...
    # Keep Marcel' own banners off stdout (the MCP wire).
    os.environ.setdefault("MARCEL_QUIET", "1")
    os.environ.setdefault("MARCEL_REDACT_SECRETS", "true")

    try:
        server = _build_server()
    except ImportError as exc:
        sys.stderr.write(f"marcel-tools MCP server cannot start: {exc}\n")
        return 2
    try:
        ...
        logger.exception("marcel-tools MCP server crashed")
        sys.stderr.write(f"marcel-tools MCP server error: {exc}\n")
        return 1
    return 0
