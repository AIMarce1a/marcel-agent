...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # codex needs LLM provider creds but must not receive Tier-1 Marcel secrets (gateway/GitHub/infra tokens).
        ...
        # `os.environ.copy()` also handed it every Tier-1 Marcel secret — gateway bot tokens, GitHub auth,
        ...
        spawn_env = marcel_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("MARCEL_KANBAN_TASK"):
            kanban_db = spawn_env.get("MARCEL_KANBAN_DB")
            default_root = os.path.join(spawn_env.get("MARCEL_HOME", os.path.expanduser("~/.marcel")), "kanban")
            kanban_root = os.path.dirname(kanban_db) if kanban_db else spawn_env.get("MARCEL_KANBAN_ROOT", default_root)
            cmd += [
    ...
    def initialize(
        self, client_name: str = "marcel", client_title: str = "Marcel Agent",
