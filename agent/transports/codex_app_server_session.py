...
Owns one Codex thread per Marcel session: drives ``turn/start``, consumes
...
# Marcel' tools.terminal.security_mode -> Codex permissions profile id.
# Missing config -> workspace-write (Codex's own default).
_MARCEL_TO_CODEX_PERMISSION_PROFILE = {
...
class CodexAppServerSession:
    """One Codex thread per Marcel session, lifetime owned by AIAgent. Not thread-safe: one caller at a time."""
    ...
    def __init__(
        ...
        self._permission_profile = permission_profile or _MARCEL_TO_CODEX_PERMISSION_PROFILE.get(
            os.environ.get("MARCEL_TERMINAL_SECURITY_MODE", "auto"), "workspace-write"
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(client_name="marcel", client_title="Marcel Agent", client_version=_get_marcel_version())
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Answer a codex server request (approval / elicitation) via Marcel' approval flow.
    ...
    def _respond_elicitation(self, params: dict) -> dict:
        """MCP elicitation: auto-accept our own marcel-tools server (opted in by enabling the runtime;
        exposes nothing codex's shell can't do); decline others so the user opts in via codex's own flow."""
        action = "accept" if (params.get("serverName") or "") == "marcel-tools" else "decline"
...
# Marcel approval choice -> codex decision (app-server-protocol v2). "deny" and
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map a Marcel approval choice onto codex's approval decision wire value."""
...
def _get_marcel_version() -> str:
    """Best-effort Marcel version string for codex's userAgent line."""
    try:
