...
env-var-shaped values at process startup, AFTER ``~/.marcel/.env`` has loaded
and BEFORE the rest of Marcel reads ``os.environ``. The contract is deliberately
narrow: read-only; startup-time and synchronous (one ``fetch()`` per process per
MARCEL_HOME, under a registry-enforced wall-clock timeout, no background
...
_SOURCE_ENVIRONMENT = ContextVar("marcel_secret_source_environment", default=None)
...
_GENERIC_REMEDIATION = {
    ErrorKind.NOT_CONFIGURED: "Run `marcel secrets {name} setup` to finish configuration.",
    ErrorKind.BINARY_MISSING: "Run `marcel secrets {name} setup` to install the helper CLI.",
    ErrorKind.AUTH_FAILED: "Credentials rejected — run `marcel secrets {name} setup` to re-authenticate.",
    ErrorKind.AUTH_EXPIRED: "Credentials expired — run `marcel secrets {name} setup` to re-authenticate.",
