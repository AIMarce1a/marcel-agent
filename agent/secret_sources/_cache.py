...
a cache problem must never block Marcel startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """``home_path`` as resolved by ``load_marcel_dotenv()``, else ``$MARCEL_HOME``/``~/.marcel``."""
    if home_path is None:
        from marcel_constants import get_marcel_home

        home_path = get_marcel_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend at ``<marcel_home>/cache/<basename>``::
