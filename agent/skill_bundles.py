...
YAML files in ``<MARCEL_HOME>/skill-bundles/`` (``name``, ``description``,
...
def _bundles_dir() -> Path:
    """Bundles directory: ``MARCEL_BUNDLES_DIR`` override (tests) or ``<MARCEL_HOME>/skill-bundles``."""
    override = os.environ.get("MARCEL_BUNDLES_DIR")
    return Path(override).expanduser() if override else get_marcel_home() / "skill-bundles"
...
# File-level CRUD — used by `marcel bundles`.
