"""Lightweight i18n for Marcel' static user-facing strings (approval prompts, a few gateway replies).
...
Language resolution: explicit ``lang=`` > ``MARCEL_LANGUAGE`` > ``display.language`` > ``en``.
"""
...
def _locales_dir() -> Path:
    """Locale dir: ``MARCEL_BUNDLED_LOCALES`` (sealed packaging, e.g. Nix) if it exists, else ``<repo-root>/locales``.

    The source path is returned even when missing so ``_load_catalog`` can log
    the path it looked at rather than raise.
    """
    override = os.getenv("MARCEL_BUNDLED_LOCALES", "").strip()
    if override and Path(override).is_dir():
        return Path(override)
    if override:
        logger.warning(
            "MARCEL_BUNDLED_LOCALES points to a non-directory path (%s); "
...
def get_language() -> str:
    ...
    env_lang = os.environ.get("MARCEL_LANGUAGE")
...
def t(key: str, lang: str | None = None, **format_kwargs: Any) -> str:
    ...
    if not format_kwargs:
        return brand_user_facing_text(value)
    try:
        return brand_user_facing_text(value.format(**format_kwargs))
