...
Installs go to a Marcel-owned staging dir, ``<MARCEL_HOME>/lsp/bin/``, so the
...
missing binary skips the server and ``marcel lsp status`` reports it).
...
# ``bin`` in ``<MARCEL_HOME>/lsp/bin/`` first, then on PATH.  ``extra_pkgs``
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # the host so `marcel lsp status` reports its presence.
...
def marcel_lsp_bin_dir() -> Path:
    """Return the Marcel-owned bin staging dir for LSP servers."""
    from marcel_constants import get_marcel_home

    p = get_marcel_home() / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(marcel_lsp_bin_dir() / name):
...
def _link_into_bin(target: Path) -> str:
    ...
    link = marcel_lsp_bin_dir() / target.name
    if not link.exists():
        try:
...
def _install_npm(pkg: str, bin_name: str, extra_pkgs: Optional[list] = None) -> Optional[str]:
    ...
    # Managed npm first: $MARCEL_HOME/node isn't on an arbitrary process's
    # PATH, so a bare which() would miss the Node that Marcel installed.
    npm = find_node_executable("npm")
    if npm is None:
        logger.info("[install] cannot install %s: no usable npm found", pkg)
        return None
    staging = marcel_lsp_bin_dir().parent  # <MARCEL_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    ...
    pip_target = marcel_lsp_bin_dir().parent / "python-packages"
    pip_target.mkdir(parents=True, exist_ok=True)
    try:
        logger.info("[install] pip install --target %s %s", pip_target, pkg)
        from marcel_cli.tools_config import _pip_install
...
def detect_status(pkg: str) -> str:
    """Return ``installed``, ``missing``, or ``manual-only`` (for ``marcel lsp status``; spawns nothing)."""
    recipe = INSTALL_RECIPES.get(pkg)
...
__all__ = ["INSTALL_RECIPES", "try_install", "detect_status", "marcel_lsp_bin_dir"]
