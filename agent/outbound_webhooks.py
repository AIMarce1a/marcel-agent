...
(``X-Marcel-Signature-256: sha256=<hex>`` over the raw body) when a secret is configured;
``MARCEL_SAFE_MODE=1`` skips registration; registration is idempotent.
"""
...
def register_from_config(cfg: Optional[Dict[str, Any]]) -> List[WebhookTarget]:
    ...
    if env_var_enabled("MARCEL_SAFE_MODE"):
        logger.info("MARCEL_SAFE_MODE=1 — outbound webhook registration skipped")
        return []
...
def iter_configured_targets(cfg: Optional[Dict[str, Any]]) -> List[WebhookTarget]:
    """Parse ``hooks.outbound`` without registering anything (``marcel hooks list``)."""
    if not isinstance(cfg, dict):
        return []
...
def _serialize_payload(event: str, kwargs: Dict[str, Any], delivery_id: str) -> bytes:
    ...
    (also the ``X-Marcel-Delivery`` header) and ``timestamp`` live inside the HMAC-signed
...
def _build_delivery(event: str, target: WebhookTarget, body: bytes, delivery_id: str) -> Dict[str, Any]:
    headers = {
        "Content-Type": "application/json", "User-Agent": "Marcel-Agent-Outbound-Webhook",
        "X-Marcel-Event": event, "X-Marcel-Delivery": delivery_id,
    }
    if target.secret:
        digest = hmac.new(target.secret.encode("utf-8"), body, hashlib.sha256).hexdigest()
        headers["X-Marcel-Signature-256"] = f"sha256={digest}"
