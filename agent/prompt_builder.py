...
from marcel_constants import (
    get_marcel_home, get_skills_dir, is_wsl, reset_marcel_home_override, set_marcel_home_override,
)
...
def _find_marcel_md(cwd: Path) -> Optional[Path]:
    """Nearest ``.marcel.md`` / ``MARCEL.md`` from *cwd* up to the git root, else None."""
    ...
    for directory in [current, *current.parents] if stop_at else [current]:
        found = next((directory / n for n in (".marcel.md", "MARCEL.md") if (directory / n).is_file()), None)
...
DEFAULT_AGENT_IDENTITY = (
    ...
    "You are Marcel Agent, built by Nous Research. Be direct: match the length of your reply to the weight of the ask "
...
MARCEL_DEFAULT_AGENT_IDENTITY = (
    "You are Marcel. Marcel is the only product name to use in user-facing replies. Be direct: match the "
    ...
    "Do not mention internal compatibility layers, legacy product names, storage paths, or upstream implementation "
    "details unless the user explicitly requests technical detail. No filler, no restating the request, and no "
    "narrating tool calls the user can see."
)

MARCEL_AGENT_HELP_GUIDANCE = (
    # Injected only when skill_view exists AND the marcel-agent skill is installed (system_prompt.py slot
    # resolution). No "when the two differ" clause: docs-are-authoritative already carries the precedence.
    "You run on Marcel Agent (by Nous Research). When the user needs help with Marcel itself — configuring, "
    ...
    "authoritative reference and always holds the latest, most up-to-date information. The `marcel-agent` "
    "skill has the actual commands and proven workflows — load it with skill_view(name='marcel-agent') "
    "before configuring, modifying, or troubleshooting Marcel so you don't guess or invent workarounds."
)

# Variant for sessions without the skills toolset (e.g. Blank Slate): naming skill_view() there would dangle.
MARCEL_AGENT_HELP_GUIDANCE_NO_SKILLS = (
    "You run on Marcel Agent (by Nous Research). When the user needs help with Marcel itself — configuring, "
    "setting up, using, extending, or troubleshooting it — or when you need to understand your own features, "
...
MARCEL_HELP_GUIDANCE = (
    "You run as part of Marcel. Marcel is the only product name to use in user-facing replies. Do not "
    "mention internal compatibility layers, legacy product names, or upstream implementation details "
    "unless the user explicitly asks about them. When helping with your own setup or capabilities, explain the desired "
    "outcome in plain language and handle implementation yourself. Do not expose paths, configuration keys, "
    "internal architecture, or terminal commands unless a manual user action is unavoidable or technical "
    "detail was requested. If a command is necessary, use the `marcel` command."
)
...
KANBAN_GUIDANCE = (
    ...
    "You have been assigned ONE task from the shared board at `~/.marcel/kanban.db`. Your task id is in "
    "`$MARCEL_KANBAN_TASK`; your workspace is `$MARCEL_KANBAN_WORKSPACE`. The `kanban_*` tools in your schema are your "
    ...
    "2. **Work inside the workspace.** `cd $MARCEL_KANBAN_WORKSPACE` before any file operations. The workspace is "
    ...
    "- **Workspace.** `cd $MARCEL_KANBAN_WORKSPACE` first. For a `worktree` kind with no `.git`, `git worktree add "
    "<path> ${MARCEL_KANBAN_BRANCH:-wt/$MARCEL_KANBAN_TASK}` from the main repo, then cd there. For a project-linked "
    "task the workspace is a fresh `<repo>/.worktrees/<task-id>` and `$MARCEL_KANBAN_BRANCH` a deterministic "
    ...
    "sits in `ready` forever). Ground every assignee in a real profile (`marcel profile list`, or ask the user), and "
    "express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n\n"
    "- Do not shell out to `marcel kanban <verb>` for board operations. Use the `kanban_*` tools — they work across "
...
# cost that compounds over a long conversation. The marcel-agent runtime already executes a batch of tool
...
# to marcel-agent's Python prompt-assembly architecture.
...
STEER_CHANNEL_NOTE = (
    ...
    "Mid-turn, the user can steer you: Marcel appends their message to the end of a tool result, wrapped exactly as:\n"
    f"{STEER_MARKER_OPEN}\n<their message>\n{STEER_MARKER_CLOSE}\n"
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    gated = (
        (True,
         "[Note: this message came from HUD mode — a small floating Marcel "
         ...
         "rather than anything inside Marcel. read_window_below identifies that app."),
        (True,
...
PLATFORM_HINTS = {
    ...
    "telegram": (
        ...
        "When the user asks for a spoken reply or a voice sample, generate and send the audio immediately. Your final "
        "response for that turn must contain only the audio delivery directives—no transcript, preface, explanation, "
        "or duplicate text message."
    ),
    "discord": (
    ...
    "tui": (
        ...
        "You are in the Marcel terminal UI (TUI). Files: there is no attachment channel and MEDIA:/path tags "
    ...
    "desktop": (
        ...
        "You are chatting inside the Marcel desktop app, a graphical chat surface. Markdown renders with full GitHub "
        ...
        "or it measures full-bleed. Widgets talk back: data-marcel-send=\"prompt\" on any clickable element (or "
        "window.marcel.send(\"prompt\")) sends that prompt as a hidden user turn — answer it by updating the widget's "
...
def _remote_backend_hint(backend: str) -> str:
    ...
    if probe:
        return lead + (
            f"this {backend} environment — NOT on the machine where Marcel itself is running. The host OS, "
            f"home, and cwd of the Marcel process are irrelevant; only the following backend state matters:\n{probe}"
        )
    description = (
    ...
    return lead + (
        f"{description} — NOT on the machine where Marcel itself runs. The backend probe didn't respond at "
...
def _config_readonly(what: str) -> dict:
    ...
    try:
        from marcel_cli.config import load_config_readonly
...
def _embedder_environment_hint() -> str:
    """Embedder-supplied environment description: MARCEL_ENVIRONMENT_HINT (container ENV)
    wins over config.yaml ``agent.environment_hint``. Read once at prompt-build time."""
    return (os.getenv("MARCEL_ENVIRONMENT_HINT") or "").strip() or str(
...
def _skills_prompt_snapshot_path() -> Path:
    return get_marcel_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("MARCEL_PLATFORM") or os.environ.get("MARCEL_SESSION_PLATFORM")
    if platform:
        return platform
    get_session_env = getattr(sys.modules.get("gateway.session_context"), "get_session_env", None)
    try:
        return (get_session_env("MARCEL_SESSION_PLATFORM") if get_session_env else "") or ""
    except Exception:
        return ""
...
def build_skills_system_prompt(
    ...
    ``skills_dir_override`` makes home resolution EXPLICIT: a build thread that never bound the MARCEL_HOME
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_marcel_home_override(str(skills_dir.parent))
    else:
    ...
    try:
        ...
        if _home_token is not None:
            reset_marcel_home_override(_home_token)
...
def load_soul_md(context_length: Optional[int] = None, home_override: "Path | None" = None) -> Optional[str]:
    """SOUL.md from MARCEL_HOME (identity slot #1), or None.

    Callers must pass ``skip_soul=True`` to ``build_context_files_prompt`` so it isn't injected twice.
    ``home_override`` pins the profile home (a thread that lost the MARCEL_HOME ContextVar reads the wrong one).

    ``home_override`` scopes the read to an explicit profile home (the agent knows its own home from its
    session_db path). Without it, resolution is ambient — which on a thread that lost the MARCEL_HOME
    ...
    """
    try:
        from marcel_cli.config import ensure_marcel_home
        ensure_marcel_home()
    except Exception as e:
        logger.debug("Could not ensure MARCEL_HOME before loading SOUL.md: %s", e)
    soul_path = (Path(home_override) if home_override is not None else get_marcel_home()) / "SOUL.md"
...
def load_marcel_welcome_md(
    context_length: Optional[int] = None, home_override: "Path | None" = None,
) -> Optional[str]:
    """Generated Marcel capability/pending-state card, loaded independently of project context."""
    path = (Path(home_override) if home_override is not None else get_marcel_home()) / "MARCEL_WELCOME.md"
    content = _read_context_file(path)
    if not content:
        return None
    return _context_section(content, "Marcel startup status", "MARCEL_WELCOME.md", path, context_length)
...
def _load_marcel_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.marcel.md / MARCEL.md — nearest match walking up to the git root."""
    marcel_md_path = _find_marcel_md(cwd_path)
    content = _read_context_file(marcel_md_path) if marcel_md_path else ""
    if not content:
        return ""
    label = str(marcel_md_path.relative_to(cwd_path)) if marcel_md_path.is_relative_to(cwd_path) else marcel_md_path.name
    return _context_section(_strip_yaml_frontmatter(content), label, ".marcel.md", marcel_md_path, context_length)
...
def build_context_files_prompt(
    ...
    Only ONE project context type loads, first found wins: .marcel.md/MARCEL.md (walk to git root) →
    AGENTS.md chain (git root → cwd) → CLAUDE.md (cwd) → .cursorrules + .cursor/rules/*.mdc (cwd). SOUL.md
    from MARCEL_HOME is independent and always included unless *skip_soul* (already the identity slot).
    """
    cwd_path = Path(cwd if cwd is not None else os.getcwd()).resolve()
    # A FALLBACK-picked cwd inside the Marcel install tree must not gain system-prompt authority (the desktop
    # default would load this repo's contributor AGENTS.md). An explicit cwd is honored verbatim.
    # An explicitly configured cwd is honored verbatim — the Marcel tree is a legitimate workspace when the
    # user deliberately points a session at it — and CLI-style surfaces pass
    # allow_install_tree_fallback=True because their launch dir IS the user's shell cwd (developing Marcel
    # in-tree). See #64590.
    from agent.runtime_cwd import _is_install_tree
    if cwd is None and not allow_install_tree_fallback and _is_install_tree(cwd_path):
        logger.warning(
            "skipping project-context discovery: working-directory resolution fell back to the Marcel "
            "install tree (%s) — set terminal.cwd to your project directory", cwd_path,
        )
        sections = []
    else:
        sections = [_load_marcel_md(cwd_path, context_length) or _load_agents_md(cwd_path, context_length)
    ...
    sections.append(load_marcel_welcome_md(context_length, home_override=home_override))
