"""``$MARCEL_HOME/cache/<kind>/`` materialisation helpers for the image/video
...
def cache_dir(kind: str) -> Path:
    """Return ``$MARCEL_HOME/cache/<kind>/``, creating parents as needed."""
    from marcel_constants import get_marcel_home
    path = get_marcel_home() / "cache" / kind
