...
_AD_HOC_SCRIPT_NAME_PREFIXES = ("marcel-verify-", "marcel-ad-hoc-")
...
def _db_path() -> Path:
    return get_marcel_home() / "verification_evidence.db"
...
def record_verify_run(
    *, root: str | Path, session_id: str | None = None, ok: bool, command: str = "marcel verify",
    scope: str = "full", output: str = "",
) -> Optional[dict[str, Any]]:
    """Record a completed ``marcel verify`` run as verification evidence.
    ...
    return _insert_evidence(VerificationEvidence(
        command=command, canonical_command="marcel verify", kind="verify",
