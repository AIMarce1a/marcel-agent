"""Timezone-aware clock for Marcel.

``now()`` returns a tz-aware datetime in the user's configured IANA timezone. Resolution order:
``MARCEL_TIMEZONE`` env var, then ``timezone`` in ``~/.marcel/config.yaml``, else server-local
...
# MARCEL_HOME, so one unkeyed global would leak the first profile's timezone into later
...
def _timezone_cache_identity() -> Tuple[str, str]:
    tz_env = os.getenv("MARCEL_TIMEZONE", "").strip()
