...
land. Reuses ``marcel mcp login``'s probe under ``force_interactive_oauth`` plus
...
def _start_loopback_listener(flow) -> "http.server.HTTPServer":
    ...
    class _Handler(http.server.BaseHTTPRequestHandler):
        def do_GET(self):  # noqa: N802 — stdlib naming
            ...
            body = b"<h1>Authorization received</h1><p>You can close this tab and return to Marcel.</p>"
            status = 200
            try:
...
def _probe_with_rollback(
    server_name: str, cfg: dict, marcel_home: str, flow, reconnect_live: bool) -> None:
    """Run the OAuth probe; on ANY failure restore the prior token file + manager entry."""
    from marcel_cli.mcp_config import _oauth_tokens_present, _probe_single_server, _save_mcp_server
    from tools.mcp_oauth import MarcelTokenStorage
    from tools.mcp_oauth_manager import get_manager
    manager = get_manager()
    storage = MarcelTokenStorage(server_name)
    backup = storage.snapshot()
    previous_entry = None
    try:
        previous_entry = manager.remove(server_name, marcel_home=marcel_home)
        ...
        manager.restore_entry(server_name, previous_entry, marcel_home=marcel_home)
        raise


def _worker(
        session_id: str, marcel_home: str, server_name: str, cfg: dict, reconnect_live: bool) -> None:
    ...
    try:
        ...
        home_token = set_marcel_home_override(marcel_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(marcel_home)))
        try:
            with force_interactive_oauth(), dashboard_oauth_flow(flow):
                _probe_with_rollback(server_name, cfg, marcel_home, flow, reconnect_live)
        finally:
...
def start_flow(
    marcel_home: str, server_name: str, cfg: dict, *, reconnect_live: bool = False,
    ...
    with _sessions_lock:
        ...
        if any(r["server_name"] == server_name and r["marcel_home"] == marcel_home for r in active):
            raise RuntimeError(f"MCP OAuth for '{server_name}' is already in progress")

    session_id = secrets.token_urlsafe(24)
    flow = DashboardOAuthFlow(
        flow_id=session_id, server_name=server_name, profile=None, marcel_home=marcel_home,
    ...
    rec = {
        "session_id": session_id, "server_name": server_name, "marcel_home": marcel_home,
        "flow": flow, "httpd": httpd, "created_at": time.time()}
    with _sessions_lock:
        _sessions[session_id] = rec
    threading.Thread(
        target=_worker, args=(session_id, marcel_home, server_name, dict(cfg), reconnect_live),
