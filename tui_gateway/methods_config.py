...
from marcel_constants import display_marcel_home as _display_marcel_home
...
def _cfg_get_provider(params):
    from marcel_cli.models import list_available_providers, normalize_provider
...
_CONFIG_GETTERS = {
    ...
    "profile": lambda params: {"home": str(_marcel_home), "display": _display_marcel_home()},
...
def _readiness_check(rid, params, probe):
    ...
    optional ``profile`` param's MARCEL_HOME + ``.env`` secret scope (ContextVars: concurrent checks
    ...
    try:
        ...
        def probe(profile, scoped):
            ...
            if (not provider_configured and provider == "bedrock"
                ...
                return fail("No Marcel provider is configured.", source)
...
def _(rid, params: dict) -> dict:
    ...
    force-redaction pipeline as ``marcel debug share --nous``; redaction is NOT client-controllable
    ...
    try:
        from marcel_cli.debug import _redact_log_text, build_nous_bundle, collect_share_bundle
