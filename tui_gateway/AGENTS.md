...
activate with `marcel --tui` or `MARCEL_TUI=1`. `tui_gateway` is ALSO the backend the Desktop app
...
themselves come from `marcel_cli/commands.py` (`marcel_cli/AGENTS.md`).
...
npm run dev       # watch mode (rebuilds marcel-ink + tsx --watch)
npm start         # production
npm run build     # full build (marcel-ink + tsc)
