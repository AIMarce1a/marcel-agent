...
# Marcel WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: MARCEL_TINI_SHIM_TARGET overrides the `/init` path (and
# MARCEL_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${MARCEL_TINI_SHIM_TARGET:-/init}"
WRAPPER="${MARCEL_TINI_SHIM_WRAPPER:-/opt/marcel/docker/main-wrapper.sh}"
