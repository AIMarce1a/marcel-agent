#!/bin/sh
# shellcheck shell=sh
# /opt/marcel/docker/main-wrapper.sh — wraps the container's CMD with
...
# with-contenv before touching MARCEL_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `marcel` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `marcel <args>` (subcommand passthrough)
#
# Drop to marcel via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${MARCEL_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${MARCEL_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export MARCEL_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset MARCEL_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid marcel "$@"; exec "$@"; }
...
# an arbitrary non-root, non-marcel UID, the bootstrap was skipped and the
# baked image dirs (owned by the marcel build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u marcel)" ]; then
    cat >&2 <<EOF
[marcel] ERROR: container started with --user $cur_uid (an arbitrary, non-marcel UID) — not supported.
...
# to the marcel user's home before dropping privileges so libraries that
...
_marcel_orig_cwd="${MARCEL_ORIG_CWD:-$PWD}"
...
# the user's command so `marcel chat` starts in the Docker -w
...
# Marcel subcommand pass-through.
drop marcel "$@"
