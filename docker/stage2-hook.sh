...
#
# Wired into the image as /etc/cont-init.d/01-marcel-setup by the
...
MARCEL_HOME="${MARCEL_HOME:-/opt/data}"
INSTALL_DIR="/opt/marcel"

# Drop to marcel via s6-setuidgid, but skip it when already non-root.
as_marcel() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid marcel "$@"; }
...
# starts non-root. The baked install tree under /opt/marcel is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $MARCEL_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass MARCEL_UID/MARCEL_GID — or the PUID/PGID aliases — which the
...
# `id -u marcel` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# marcel UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u marcel)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-marcel UID).
...
and the baked /opt/marcel install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e MARCEL_UID=\$(id -u) -e MARCEL_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the marcel user to that UID/GID at boot and chowns the data
...
# --- Bootstrap MARCEL_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid marcel mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom MARCEL_HOME paths whose parents only
# root can create (e.g. `MARCEL_HOME=/home/marcel/.marcel` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for MARCEL_UID/MARCEL_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  MARCEL_UID/
# MARCEL_GID still win when both are set.  See #15290, salvages #25872.
MARCEL_UID="${MARCEL_UID:-${PUID:-}}"
MARCEL_GID="${MARCEL_GID:-${PGID:-}}"

if [ -n "${MARCEL_UID:-}" ] && validate_uid_gid "$MARCEL_UID" && [ "$MARCEL_UID" != "$(id -u marcel)" ]; then
    echo "[stage2] Changing marcel UID to $MARCEL_UID"
    usermod -u "$MARCEL_UID" marcel
fi
if [ -n "${MARCEL_GID:-}" ] && validate_uid_gid "$MARCEL_GID" && [ "$MARCEL_GID" != "$(id -g marcel)" ]; then
    echo "[stage2] Changing marcel GID to $MARCEL_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$MARCEL_GID" marcel 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised marcel user (UID 10000)
...
# marcel process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes marcel. Idempotent across container
...
#   - marcel is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G marcel 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] marcel already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" marcel 2>/dev/null; then
        echo "[stage2] Added marcel to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group marcel failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When MARCEL_UID is remapped or the top-level $MARCEL_HOME isn't owned by
# the runtime marcel UID, restore ownership to marcel — but ONLY for the
# directories marcel actually writes to. The full $MARCEL_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of marcel-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_marcel_uid=$(id -u marcel)
...
chown_marcel_tree() {
    ...
    chown -R marcel:marcel "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_marcel_owner() {
    target="$1"
    find "$target" \( ! -user marcel -o ! -group marcel \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$MARCEL_HOME" 2>/dev/null)" != "$actual_marcel_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $MARCEL_HOME (targeted) to marcel ($actual_marcel_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $MARCEL_HOME: chown the directory itself (not its contents)
    # so marcel can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$MARCEL_HOME"; then
        :
    else
        chown marcel:marcel "$MARCEL_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $MARCEL_HOME failed (rootless container?) — continuing"
    fi
    # Marcel-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by marcel (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$MARCEL_HOME/$sub" ] && tree_has_non_marcel_owner "$MARCEL_HOME/$sub"; then
            chown_marcel_tree "$MARCEL_HOME/$sub"
        fi
    done
fi
...
# marcel user. Hosted/container instances keep mutable state under
# $MARCEL_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# MARCEL_DISABLE_LAZY_INSTALLS=1. Keeping /opt/marcel root-owned and
...
# $MARCEL_HOME/lazy-packages dir on the data volume (Dockerfile sets
# MARCEL_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to marcel
...
# Always reset ownership of $MARCEL_HOME/profiles to marcel on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> marcel …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as marcel and walks
...
if [ -d "$MARCEL_HOME/profiles" ] && tree_has_non_marcel_owner "$MARCEL_HOME/profiles"; then
    chown_marcel_tree "$MARCEL_HOME/profiles"
fi

# Always reset ownership of $MARCEL_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged marcel runtime even
...
# gate as profiles/).
if [ -d "$MARCEL_HOME/cron" ] && tree_has_non_marcel_owner "$MARCEL_HOME/cron"; then
    chown_marcel_tree "$MARCEL_HOME/cron"
fi

# Always ensure logs/gateways is marcel-owned (#45258). Formerly healed by
...
# top-level $MARCEL_HOME is mis-owned, so a warm volume with marcel-owned
# MARCEL_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid marcel mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as marcel.
if [ -d "$MARCEL_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$MARCEL_HOME/logs/gateways"; then
        :
    else
        chown marcel:marcel "$MARCEL_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# marcel pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged marcel gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $MARCEL_HOME is
...
# and cron/.
if [ -d "$MARCEL_HOME/platforms/pairing" ] && tree_has_non_marcel_owner "$MARCEL_HOME/platforms/pairing"; then
    chown_marcel_tree "$MARCEL_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$MARCEL_HOME/pairing" ] && tree_has_non_marcel_owner "$MARCEL_HOME/pairing"; then
    chown_marcel_tree "$MARCEL_HOME/pairing"
fi

# Reset ownership of marcel-owned top-level state files on every boot.
# The targeted data-volume chown above only covers marcel-owned
# *subdirectories*; loose state files living directly under $MARCEL_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> marcel …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged marcel runtime then hits
...
# sweep so host-owned files in a bind-mounted $MARCEL_HOME are never
...
# entries of marcel_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$MARCEL_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$MARCEL_HOME/$f"; then
            :
        else
            chown marcel:marcel "$MARCEL_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the marcel runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$MARCEL_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$MARCEL_HOME/config.yaml"; then
        :
    else
        chown marcel:marcel "$MARCEL_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$MARCEL_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as marcel user ---
# Run as marcel via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $MARCEL_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_marcel mkdir -p \
    "$MARCEL_HOME/backups" \
    "$MARCEL_HOME/cron" \
    "$MARCEL_HOME/sessions" \
    "$MARCEL_HOME/logs" \
    "$MARCEL_HOME/logs/gateways" \
    "$MARCEL_HOME/hooks" \
    "$MARCEL_HOME/memories" \
    "$MARCEL_HOME/skills" \
    "$MARCEL_HOME/skins" \
    "$MARCEL_HOME/plans" \
    "$MARCEL_HOME/workspace" \
    "$MARCEL_HOME/home" \
    "$MARCEL_HOME/pairing" \
    "$MARCEL_HOME/platforms/pairing" \
    "$MARCEL_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/marcel/.install_method (see Dockerfile), NOT written here into
# $MARCEL_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $MARCEL_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.marcel:/opt/data) and sometimes shared with a
...
# 'marcel update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $MARCEL_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/marcel stamp, so deleting the data-dir copy is safe).
if [ -f "$MARCEL_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$MARCEL_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$MARCEL_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$MARCEL_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$MARCEL_HOME/$dest"; then
            :
        else
            as_marcel cp "$INSTALL_DIR/$src" "$MARCEL_HOME/$dest"
        fi
    fi
}
...
# the first-boot seed above depends on /opt/marcel/.env.example being present
...
# not generate one. Marcel loads $MARCEL_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$MARCEL_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$MARCEL_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $MARCEL_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$MARCEL_HOME/.env" ] && ! refuse_symlinked_path "clean" "$MARCEL_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$MARCEL_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$MARCEL_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$MARCEL_HOME/.env"; then
        :
    else
        if [ ! -f "$MARCEL_HOME/.env" ]; then
            ...
            (umask 077 && as_marcel touch "$MARCEL_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$MARCEL_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$MARCEL_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$MARCEL_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $MARCEL_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $MARCEL_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi
...
if [ -f "$MARCEL_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$MARCEL_HOME/.env"; then
        :
    else
        chown marcel:marcel "$MARCEL_HOME/.env" 2>/dev/null || true
        chmod 600 "$MARCEL_HOME/.env" 2>/dev/null || true
    fi
fi
...
# as the unprivileged `marcel` user — without this it gets EACCES on every
...
# marcel group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp marcel /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted marcel group access to the Fly Machines API socket"
    else
...
# $MARCEL_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `marcel update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set MARCEL_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$MARCEL_HOME/config.yaml" ]; then
    s6-setuidgid marcel "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$MARCEL_HOME/auth.json" ] && [ -n "${MARCEL_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$MARCEL_HOME/auth.json"; then
        :
    else
        printf '%s' "$MARCEL_AUTH_JSON_BOOTSTRAP" > "$MARCEL_HOME/auth.json"
        chown marcel:marcel "$MARCEL_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$MARCEL_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via MARCEL_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$MARCEL_HOME/auth.json" ] && [ -n "${MARCEL_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$MARCEL_HOME/auth.json"; then
        :
    else
        s6-setuidgid marcel "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$MARCEL_HOME/auth.json" \
...
# can set MARCEL_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# MARCEL_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$MARCEL_HOME/gateway_state.json" ] && \
        [ "${MARCEL_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$MARCEL_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$MARCEL_HOME/gateway_state.json"
        chown marcel:marcel "$MARCEL_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$MARCEL_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_marcel "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/marcel/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Marcel spawns for the
...
# main-wrapper.sh propagates it into the supervised ``marcel`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-marcel, dashboard, etc.).
