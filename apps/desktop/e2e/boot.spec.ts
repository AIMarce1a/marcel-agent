 ...
 * packaged binary) with a real `marcel serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → marcel serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Marcel title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Marcel')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { marcelDesktop?: Record<string, unknown> }).marcelDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → marcel serve is spawned → WS connects → config
