...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Marcel E2E'], { cwd: repo })
...
function configureRepoCwd(marcelHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(marcelHome, mockUrl)
  fs.appendFileSync(path.join(marcelHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(marcelHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.marcelHome, mock.url, repo)
