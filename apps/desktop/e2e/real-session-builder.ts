 ...
 * AIAgent loop, using the E2E mock provider configured in `marcelHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(marcelHome: string) {
  ...
  static async start(marcelHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(marcelHome)
