...
/**
 * E2E smoke tests for the packaged Marcel desktop app.
 *
 ...
 * (credential stripping, isolated MARCEL_HOME + userData, unique app name).
 *
...
test('window opens with the Marcel title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      marcelDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).marcelDesktop?.hud?.open({ sessionId: null })
  )
