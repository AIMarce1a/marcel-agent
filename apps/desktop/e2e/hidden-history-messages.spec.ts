...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  fs.appendFileSync(
    path.join(sandbox.marcelHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.marcelHome)
  const builder = await RealSessionBuilder.start(sandbox.marcelHome)
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.marcelHome, mock.url)
  fs.appendFileSync(path.join(sandbox.marcelHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
