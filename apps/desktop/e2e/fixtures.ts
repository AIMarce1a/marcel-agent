/**
 * Shared E2E fixtures for the Marcel desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → marcel serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `marcel desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `marcel-e2e-${prefix}-${Math.random()}`))
  const marcelHome = path.join(root, 'marcel-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(marcelHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(marcelHome, 'config.yaml')
 ...
 */
export function writeEnvFile(marcelHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(marcelHome, '.env')
 ...
 */
function writeEmptyConfig(marcelHome: string): void {
  const configPath = path.join(marcelHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - MARCEL_HOME → sandbox marcel-home (isolated config/sessions)
 *  - MARCEL_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - MARCEL_DESKTOP_IGNORE_EXISTING=1 → don't pick up `marcel` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - MARCEL_DESKTOP_MARCEL_ROOT → REPO_ROOT (dev checkout resolution)
 *  - MARCEL_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    MARCEL_HOME: sandbox.marcelHome,
    MARCEL_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    MARCEL_DESKTOP_IGNORE_EXISTING: '1',
    MARCEL_DESKTOP_MARCEL_ROOT: REPO_ROOT,
    MARCEL_DESKTOP_APP_NAME: `MarcelE2E-${Date.now()}`,
    ...
    MARCEL_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated MARCEL_HOME + userData
 * @param env      - the process environment (already has MARCEL_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeEnvFile(sandbox.marcelHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.marcelHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via MARCEL_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`marcel serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.marcelHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { MARCEL_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Marcel backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Marcel.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Marcel.app', 'Contents', 'MacOS', 'Marcel')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'marcel')
}
 ...
 * progress without spawning a real Marcel backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * MARCEL_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    MARCEL_DESKTOP_BOOT_FAKE: '1',
    MARCEL_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + marcel-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).MARCEL_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).MARCEL_DESKTOP_MARCEL
  delete (env as Record<string, string | undefined>).MARCEL_DESKTOP_MARCEL_ROOT
