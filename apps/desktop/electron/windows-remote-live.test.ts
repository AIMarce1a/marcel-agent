...
//   MARCEL_WIN_SSH_HOST   ssh alias/host of the Windows box
//   MARCEL_WIN_SSH_USER   remote user
//   MARCEL_WIN_SSH_MARCEL absolute path to the remote marcel.exe under test
const liveHost = process.env.MARCEL_WIN_SSH_HOST || ''
const liveUser = process.env.MARCEL_WIN_SSH_USER || ''
const configuredMarcel = process.env.MARCEL_WIN_SSH_MARCEL || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Marcel-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredMarcel)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredMarcel.replace('marcel.exe', 'python.exe')}' -m marcel_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredMarcel.replace('marcel.exe', 'python.exe')
        const terminate = `& '${python}' -m marcel_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.marcelPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m marcel_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m marcel_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
