export type ParentWatchdogEnv = {
  MARCEL_PARENT_PID: string
  MARCEL_PARENT_START_MARKER?: string
  MARCEL_PARENT_NONCE?: string
  /** Spawn tag consumed by marcel_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { MARCEL_PARENT_PID: String(pid) }
  env.MARCEL_SPAWN = spawnTag(pid, startMarker)
  ...
  env.MARCEL_PARENT_START_MARKER = startMarker
  env.MARCEL_PARENT_NONCE = nonce

  return env
}

/** Build the `MARCEL_SPAWN` tag mirrored by marcel_cli/process_identity.py.
