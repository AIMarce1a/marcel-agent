 ...
 *
 * 1. TORN BUNDLE after an update (#95575): `marcel update` replaces the app
...
export interface RendererLoadErrorDetails {
  ...
  /** Repair command hint, e.g. `marcel desktop --force-build`. */
  repairHint?: string
  /**
...
export function buildRendererLoadErrorPage(details: RendererLoadErrorDetails = {}): string {
  ...
  const title = 'Marcel couldn\u2019t start the desktop UI'
  ...
  const repair = details.repairHint ? `<p>Repair with: <code>marcel desktop --force-build</code></p>` : ''
...
<main>
  ...
  <code>marcel desktop --force-build</code>, then restart the app.</p>
