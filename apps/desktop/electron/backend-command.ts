// Backend subcommand routing for the desktop-managed Marcel process.
//
// The desktop app launches its own headless backend via `marcel serve` — it
...
// app hasn't updated yet, or an older `marcel` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Marcel profile to pin via `--profile`.
 */
 ...
 * `-m marcel_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `marcel_cli/subcommands/dashboard.py` source registers
