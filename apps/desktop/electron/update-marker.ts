...
export function markerPath(marcelHome) {
  return path.join(marcelHome, '.marcel-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(marcelHome)
  let raw
 ...
 *
 * The Tauri-based marcel-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(marcelHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(marcelHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * Marcel to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(marcelHome, opts)
