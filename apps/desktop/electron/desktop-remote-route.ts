...
export function resolveDesktopRemoteRoute({
  ...
  if (envUrl) {
    ...
    if (!envToken) {
      throw new Error(
        'MARCEL_DESKTOP_REMOTE_URL is set but MARCEL_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Marcel backend.'
      )
    }
  ...
  if (!modeIsRemoteLike(config.mode)) {
    ...
    // resolves local and spawns a loopback `marcel serve` the desktop never
