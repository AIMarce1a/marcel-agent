...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'marcel-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'marcel-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Marcel Test'], { cwd: dir })
