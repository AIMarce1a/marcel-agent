...
export interface MarcelReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForMarcelReady`).
   */
...
export const REMOTE_UNSIGNED_OAUTH_MESSAGE =
  'Remote Marcel gateway uses OAuth, but you are not signed in. ' +
 ...
 *  - readiness-probe exhaustion in waitForMarcelReady().
 *
...
/**
 * True when the backend URL points at a Nous-managed Marcel Cloud instance
 ...
 * `isReauthRequired` so startMarcel latches; `needsOauthLogin` alone only
...
export async function waitForMarcelReady(baseUrl: string, options: MarcelReadyOptions): Promise<void> {
  ...
  throw new Error(`Marcel backend did not become ready: ${detail}`)
}
