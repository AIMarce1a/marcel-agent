...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/marcel', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/marcel/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/marcel'), 'https://gw.example.com/marcel/auth/native/refresh')
})
