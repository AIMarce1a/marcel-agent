...
function legacyCandidates(): string[] {
  ...
  return [...new Set(LEGACY_DIRS.flatMap((name) => [
    appData && path.join(appData, name),
    config && path.join(config, name),
    mac && path.join(mac, name)
...
function copyTree(source: string, destination: string): number {
  ...
  for (const entry of fs.readdirSync(source, { withFileTypes: true })) {
    if (REBUILDABLE.test(entry.name)) continue
    const from = path.join(source, entry.name)
    const to = path.join(destination, entry.name)
    if (entry.isSymbolicLink()) throw new Error(`refusing durable symlink in legacy desktop data: ${entry.name}`)
...
function emptyScaffold(destination: string): boolean {
  if (!fs.statSync(destination, { throwIfNoEntry: false })?.isDirectory()) return false
...
function migrateOwnedConnectionSchema(stage: string): void {
  for (const name of ['connection.json', 'connections.json']) {
    ...
    if (upgraded !== raw) fs.writeFileSync(file, upgraded, { mode: fs.statSync(file).mode & 0o777 })
  }
}

export function migrateLegacyUserData(destination: string): void {
  if (fs.existsSync(path.join(destination, RECEIPT))) return
  ...
  if (fs.existsSync(stage)) throw new Error(`desktop migration staging path already exists: ${stage}`)
