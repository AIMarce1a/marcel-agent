...
import { buildDesktopBackendEnv, marcelManagedNodePathEntries, normalizeMarcelHomeRoot } from './backend-env'
import {
...
import { isMarcelOwnedVenvDaemon } from './venv-holder-select'
...
import {
  ...
  resolveVenvMarcelCommand
} from './windows-marcel-path'
import {
...
// Electron has not reached `ready`, so this is before any persisted desktop
// state is read. The importer is byte-preserving and idempotent.
const defaultUserData = path.resolve(USER_DATA_OVERRIDE || app.getPath('userData'))
migrateLegacyUserData(defaultUserData)
...
const DEV_SERVER = process.env.MARCEL_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.MARCEL_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with MARCEL_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[marcel] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[marcel] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. MARCEL_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[marcel] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[marcel] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via MARCEL_DESKTOP_PASSWORD_STORE, bridged by the
// `marcel desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[marcel] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[marcel] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `marcel desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Marcel sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[marcel] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[marcel] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[marcel] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[marcel] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[marcel] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('marcel:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Marcel burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[marcel] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[marcel] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[marcel] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[marcel] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// MARCEL_HOME — the user-facing root for everything Marcel-related. Mirrors
// scripts/install.ps1's $MarcelHome and scripts/install.sh's $MARCEL_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\marcel (matches install.ps1)
//   macOS / Linux: ~/.marcel (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.marcel directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\marcel yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// MARCEL_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// MARCEL_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.marcel / %LOCALAPPDATA%\marcel.
function resolveMarcelHome() {
  if (process.env.MARCEL_HOME) {
    return normalizeMarcelHomeRoot(process.env.MARCEL_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'marcel-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a MARCEL_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\marcel and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('MARCEL_HOME')

    if (fromRegistry) {
      return normalizeMarcelHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'marcel')
    const legacy = path.join(app.getPath('home'), '.marcel')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.marcel setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.marcel')
}

const MARCEL_HOME = resolveMarcelHome()

function pathWithMarcelManagedNode(...entries) {
  const managed = marcelManagedNodePathEntries(MARCEL_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_MARCEL_ROOT — the canonical mutable Marcel install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_MARCEL_ROOT = path.join(MARCEL_HOME, 'marcel-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_MARCEL_ROOT, 'venv')
...
// boots, going straight to `resolveMarcelBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_MARCEL_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_MARCEL_ROOT, '.marcel-bootstrap-complete')
...
// active-profile.json records which Marcel profile the desktop launches its
// local backend as. When set, startMarcel() passes `marcel --profile <name>
// dashboard …`, which deterministically pins MARCEL_HOME (see
// _apply_profile_override in marcel_cli/main.py) and bypasses the sticky
// ~/.marcel/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors marcel_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// marcelDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under MARCEL_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by marcel_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(MARCEL_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (marcel_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.MARCEL_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.MARCEL_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.MARCEL_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.MARCEL_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.MARCEL_DESKTOP_APP_NAME || 'Marcel'
...
// 'marcel:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.marcel) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.marcel')
}

// Seed the native About panel with the live Marcel version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `marcel update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveMarcelVersion(),
...
// startMarcel(); this pool only holds EXTRA profile
...
// The legacy MARCEL_DESKTOP_POOL_* env vars remain the initial-value fallback
...
function readPersistedPoolLimits() {
  try {
    ...
    const fromEnv = clampPoolLimits({
      maxBackends: Number(process.env.MARCEL_DESKTOP_POOL_MAX) || undefined,
      idleMs: Number(process.env.MARCEL_DESKTOP_POOL_IDLE_MS) || undefined
    })
...
//   * WSL2 IPC stall  = the renderer's `marcel:backend:touch` roundtrips
...
const POOL_KEEPALIVE_FRESH_MS = Math.max(
  120_000,
  Number(process.env.MARCEL_DESKTOP_POOL_KEEPALIVE_FRESH_MS) || 4 * 60_000
)
...
// onto the error so subsequent startMarcel() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning marcel serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Marcel backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  marcelLog.push(...lines)

  if (marcelLog.length > 300) {
    marcelLog.splice(0, marcelLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-marcel-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('marcel:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the marcel:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('marcel:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_MARCEL_ROOT
  })
}
...
// The Tauri updater writes MARCEL_HOME/.marcel-update-in-progress for the whole
...
// (`force_kill_other_marcel`, taskkill /IM marcel.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(MARCEL_HOME)),
...
const BUNDLE_SWAP_RELAUNCH_FLAG = '--marcel-bundle-swap-relaunched'
...
// The detached updater swaps the packaged bundle on disk AFTER `marcel update`
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Marcel will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(MARCEL_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'Marcel update did not finish',
        `${result.message}\n\nDetails: ${path.join(MARCEL_HOME, 'logs', 'desktop-update-handoff.log')}`
      )
    }
  } catch (err) {
  ...
  if (outcome === 'timeout') {
    ...
    await advanceBootProgress('backend.update-restart', 'Restarting Marcel to load the updated app…', 14)
...
function findOnPath(command) {
  ...
  // shell-script shim named `marcel` — must not shadow `marcel.cmd`/`marcel.exe`.
...
function unwrapWindowsVenvMarcelCommand(command, backendArgs) {
  return resolveVenvMarcelCommand(command, backendArgs, {
    isWindows: IS_WINDOWS,
    isCommandScript,
    fileExists,
    directoryExists,
...
// spawns `marcel serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `marcel` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'marcel_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `marcel --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, MARCEL_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveMarcelBackend step 4); execFileSync of a .cmd without
...
function isMarcelSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'marcel_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.MARCEL_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Marcel deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the marcel-import
...
// findGitBash — locate bash.exe on Windows. Resolves MARCEL_GIT_BASH_PATH
...
// normal MARCEL_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\marcel\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'marcel', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'marcel', 'git', 'bin', 'git.exe'))
  }
...
function recentMarcelLog() {
  return marcelLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's marcel root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_MARCEL_ROOT.
// MARCEL_DESKTOP_MARCEL_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.MARCEL_DESKTOP_MARCEL_ROOT && path.resolve(process.env.MARCEL_DESKTOP_MARCEL_ROOT),
    !IS_PACKAGED && isMarcelSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isMarcelSourceRoot(ACTIVE_MARCEL_ROOT) ? ACTIVE_MARCEL_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_MARCEL_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('marcel:updates:progress', merged)
  }
}
      ...
      const req = https.get(
        url,
        {
          headers: {
            ...
            'User-Agent': 'marcel-desktop-update-check'
          },
          timeout: 10_000
        },
        res => {
...
// Windows that binary owns ALL repo mutation — running `marcel update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(MARCEL_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `marcel update` can write
// fresh entry points. On Windows this is the file the running backend
// `marcel.exe` holds open; on POSIX it's never mandatory-locked.
function venvMarcelShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'marcel.exe')
    : path.join(updateRoot, 'venv', 'bin', 'marcel')
}
...
// Kill only Marcel-OWNED venv daemons (the memory plugin's hindsight daemon:
...
// venv files mapped. External holders (a user terminal running `marcel`,
...
function killMarcelOwnedVenvDaemons(updateRoot) {
  if (!IS_WINDOWS) {
    return
  }
  ...
  try {
    ...
    holders = (Array.isArray(parsed) ? parsed : [parsed]).filter(p =>
      isMarcelOwnedVenvDaemon(p?.ExecutablePath, p?.CommandLine, scriptsDir)
    )
  } catch {
  ...
  for (const holder of holders) {
    ...
    if (Number.isInteger(pid) && pid > 0) {
      rememberLog(`[updates] stopping Marcel-owned venv daemon (hindsight) PID ${pid} before hand-off`)
...
// only signals the direct child, so on Windows a backend `marcel.exe` that
// spawned its own grandchildren (a `marcel` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `Marcel backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live Marcel backend PID ${child.pid}; ` +
  ...
  try {
    ...
    child.marcelBackendIdentity = identity
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the Marcel backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}

function releaseBackendChild(child) {
  const identity = child?.marcelBackendIdentity
...
// `marcelProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `marcel.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a marcel
// REPL, a pty terminal, the gateway) keep `marcel.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const marcelProcess = backendConnectionState.getProcess()
  ...
  // unlock — the shim probe only covers venv\Scripts\marcel.exe, but the
  // backend is `python.exe -m marcel_cli.main serve`, which need not hold
  // the shim at all (#74805 first-attempt race).
  const initialPids = []

  if (marcelProcess && Number.isInteger(marcelProcess.pid)) {
    initialPids.push(marcelProcess.pid)
  }
  ...
  stopBackendTreesForUpdate(marcelProcess, {
  ...
  // `marcel gateway stop --all`: the CLI discovers every profile's gateway
  ...
  stopGatewayBeforeUpdate(venvMarcelShimPath(updateRoot), MARCEL_HOME)

  // Reap Marcel-OWNED venv daemons the tree-kill above cannot reach: the
  ...
  killMarcelOwnedVenvDaemons(updateRoot)

  const shim = venvMarcelShimPath(updateRoot)
  ...
  const gate = await waitForBackendRelease(
    initialPids,
    {
      ...
      collectStragglerPids: () => {
        ...
        const currentMarcelProcess = backendConnectionState.getProcess()

        if (currentMarcelProcess && Number.isInteger(currentMarcelProcess.pid)) {
          stragglers.push(currentMarcelProcess.pid)
        }
...
// `marcel update`). Instead we spawn the staged Marcel-Setup binary with
// --update and quit, so it can run `marcel update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `marcel update` → relaunch),
      ...
      // in: the MARCEL_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `marcel update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `marcel desktop`, never the Tauri installer that self-copies
      // marcel-setup.exe into MARCEL_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `marcel` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `marcel update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `marcel update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, marcelRoot: updateRoot }
      }

      rememberLog('[updates] no staged updater; using repo hand-off script for CLI install')
    }

    const handoffConflict = updateHandoffConflict(MARCEL_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Marcel — this window will close and the updater will open. Don’t reopen Marcel yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(MARCEL_HOME, rememberLog)

    // Stop our own backend(s) and wait for the venv shim to unlock BEFORE we
    // spawn the updater. Without this the updater races a still-locked
    // marcel.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running marcel, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Marcel install open ' +
        '(a second Marcel window or a terminal running marcel?). Close it and retry.'
      ...
      if (IS_WINDOWS) {
        ...
        startGatewaysAfterUpdateAbort(venvMarcelShimPath(updateRoot))
      }
    ...
    // Marcel processes running from this venv.  The updater normally refuses
    ...
    // `marcel update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `marcel update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          MARCEL_HOME,
          MARCEL_UPDATE_STARTED_AT: String(updateStartedAt),
          PATH: pathWithMarcelManagedNode(venvBin)
        },
        detached: true,
        stdio: 'ignore'
      })
      ...
      // The `marcel update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(MARCEL_HOME, child.pid, { startedAt: updateStartedAt })
      }
      ...
      // abort with "Another Marcel update is already running (PID <itself>)" —
      ...
      if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
        writeUpdateMarker(MARCEL_HOME, child.pid)
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. Marcel will keep running — try again, or run \`marcel update\` from a terminal.`
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvMarcel = path.join(venvBin, IS_WINDOWS ? 'marcel.exe' : 'marcel')
  const venvPython = path.join(venvBin, IS_WINDOWS ? 'python.exe' : 'python')

  // The updater invokes the venv's Marcel launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the marcel
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.marcel-bootstrap-complete')),
      hasVenvMarcel: fileExists(venvMarcel),
...
// Python-level pre-update snapshot inside `marcel update`.
function preflightStateDb(marcelHome, rememberLog) {
  const stateDbPath = path.join(marcelHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(marcelHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(marcelHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(marcelHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `marcel update`, swaps/relaunches the app bundle, and writes
// .marcel-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'marcel update', percent: null })

    return { ok: true, manual: true, command: 'marcel update', marcelRoot: updateRoot }
  }
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      MARCEL_HOME,
      MARCEL_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithMarcelManagedNode(path.join(updateRoot, 'venv', 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Marcel — this window will close. Don’t reopen Marcel yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_MARCEL_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isMarcelSourceRoot(ACTIVE_MARCEL_ROOT) &&
    fileExists(venvPython) &&
    canImportMarcelCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_MARCEL_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `marcel
...
function resolveWebDist() {
  const override = process.env.MARCEL_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set MARCEL_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: marcel desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: marcel desktop --force-build`
  )
...
function resolveMarcelCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Marcel.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.MARCEL_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveMarcelCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'marcel_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_MARCEL_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Marcel at ${ACTIVE_MARCEL_ROOT}`,
    command,
    args: ['-m', 'marcel_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      marcelHome: MARCEL_HOME,
      pythonPathEntries: [ACTIVE_MARCEL_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_MARCEL_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveMarcelBackend(backendArgs) {
  // 1. Explicit override -- MARCEL_DESKTOP_MARCEL_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.MARCEL_DESKTOP_MARCEL_ROOT && path.resolve(process.env.MARCEL_DESKTOP_MARCEL_ROOT)

  if (overrideRoot && isMarcelSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Marcel source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `marcel` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isMarcelSourceRoot.)
  if (!IS_PACKAGED && isMarcelSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Marcel source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_MARCEL_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\marcel\\marcel-agent (Windows) or ~/.marcel/marcel-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Marcel runtime at ${ACTIVE_MARCEL_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `marcel` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    MARCEL_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.MARCEL_DESKTOP_IGNORE_EXISTING !== '1') {
    let marcelCommand = null
    const marcelOverride = process.env.MARCEL_DESKTOP_MARCEL

    if (marcelOverride) {
      const resolvedOverride = findOnPath(marcelOverride)

      if (resolvedOverride) {
        marcelCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(marcelOverride, { isWsl: IS_WSL })) {
        marcelCommand = marcelOverride
      } else {
        rememberLog(`Ignoring Windows Marcel override under WSL: ${marcelOverride}`)
      }
    } else {
      marcelCommand = findOnPath('marcel')
    }

    if (marcelCommand) {
      if (looksLikeDesktopAppBinary(marcelCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Marcel CLI: ${marcelCommand}`)
        marcelCommand = null
      }
    }

    if (marcelCommand) {
      const unwrapped = unwrapWindowsVenvMarcelCommand(marcelCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `marcel` shim
      ...
      const shellForProbe = isCommandScript(marcelCommand)

      // MARCEL_DESKTOP_MARCEL is an explicit deployment override (used by
      ...
      if (shouldTrustMarcelOverride(marcelOverride) || verifyMarcelCli(marcelCommand, { shell: shellForProbe })) {
        ...
        // unwrapWindowsVenvMarcelCommand here: the second call repeats the
        // same un-memoized import probe, costing up to another full probe
        // timeout on the boot path for an answer we already have.
        return {
          label: `existing Marcel CLI at ${marcelCommand}`,
          command: marcelCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Marcel CLI at ${marcelCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed marcel_cli module via system Python.
  ...
  if (python) {
    ...
    // having marcel_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\marcel\marcel-agent\venv.
    if (canImportMarcelCli(python)) {
      return {
        kind: 'python',
        label: `installed marcel_cli module via ${python}`,
        command: python,
        args: ['-m', 'marcel_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: marcel_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveMarcelBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Marcel Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveMarcelBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Marcel install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Marcel recovery was handed off to Marcel Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      marcelHome: MARCEL_HOME,
      logRoot: path.join(MARCEL_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Marcel install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Marcel bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(MARCEL_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startMarcel() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // marcel:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveMarcelBackend(backend.args))
  }
  ...
  if (!isMarcelSourceRoot(ACTIVE_MARCEL_ROOT)) {
    throw new Error(
      `Marcel install at ${ACTIVE_MARCEL_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Marcel' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\marcel\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `marcel` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Marcel on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Marcel.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isMarcelSourceRoot()
    // plus an importable marcel_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `Marcel venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Marcel at ${ACTIVE_MARCEL_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Marcel runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----marcel-${crypto.randomBytes(12).toString('hex')}`
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          reject(new Error(`Unsupported Marcel backend URL protocol: ${parsed.protocol}`))
                ...
                reject(
                  new Error(
                    `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                      'The endpoint is likely missing on the Marcel backend.'
                  )
                )
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to Marcel backend after ${timeoutMs}ms`))
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      parsed,
      {
        agent,
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-Marcel-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Marcel-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('marcel:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveMarcelCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('marcel:preview-file-changed', payload)
}
...
// waitForMarcelReady how to read a 401 — rejected session vs gated route.
...
async function waitForMarcel(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForMarcelReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('marcel:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('marcel:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('marcel:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('marcel:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Marcel...".
function sendPowerResume() {
  ...
  webContents.send('marcel:power-resume')
}
...
// 'marcel:power-battery' push below.
ipcMain.handle('marcel:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('marcel:power-battery', next)
    }
  }
}
...
async function showPluginCompatNoticeOnce() {
  ...
  try {
    notice = pendingPluginCompatNotice(MARCEL_HOME, app.getPath('userData'))
  } catch (err) {
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('marcel:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('marcel:window-state-changed', state)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('marcel:context-menu-spellcheck', {
...
//
// Hosted Marcel gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``marcel_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / marcel #37247).
//     Both are set as HttpOnly cookies (``marcel_session_at`` ~15 min,
//     ``marcel_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
// "not signed in", which on the initial boot path (startMarcel → the renderer's
// single-shot boot() with no retry) surfaced as the "Marcel couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Marcel
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Marcel Cloud agent…' : 'Sign in to Marcel gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Marcel backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in marcel_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- Marcel Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:marcel-remote-oauth`:
...
// convention (marcel_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every Marcel surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.nousresearch.com'

function resolvePortalBaseUrl() {
  const raw = process.env.MARCEL_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the Marcel gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for marcel_session_at/rt that the portal
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing Marcel Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Marcel Cloud sign-in.'))
...
// Discover the hosted (Marcel Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Marcel Cloud. Open Settings → Gateway, choose Marcel Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your Marcel Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Marcel Cloud session has expired. Sign in to Marcel Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Marcel Cloud org tag on cloud-mode entries so Settings can
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every marcel:connections:* call.
 ...
 * editor doesn't carry — cloud `org`, ssh `remoteMarcelPath`/`remoteProfile` —
...
// a valid stored value (pins the root MARCEL_HOME explicitly); null means "no
...
// contains "marcel", avoiding false positives from stale gateway.pid files
// whose PID was recycled by the OS to an unrelated process.
function isMarcelProcess(pid) {
  try {
  ...
  try {
    ...
    return cmdline.includes('marcel')
  } catch {
...
//   1. Legacy ~/.marcel/active_profile (explicit CLI choice via marcel profile use)
//   2. Running gateway (gateway.pid with verified liveness + marcel identity)
...
function migrateActiveProfileIfMissing() {
  migrateActiveProfileIfMissingPure(DESKTOP_PROFILE_CONFIG_PATH, {
    legacyActivePath: path.join(MARCEL_HOME, 'active_profile'),
    marcelHome: MARCEL_HOME,
    profilesRoot: path.join(MARCEL_HOME, 'profiles'),
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.MARCEL_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.MARCEL_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted Marcel Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteMarcelPath: (ssh || savedSsh)?.remoteMarcelPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by MARCEL_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Marcel Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Marcel Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteMarcelPath: input.sshRemoteMarcelPath ?? existingBlock.remoteMarcelPath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (marcel #37247), and the gateway middleware transparently
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Marcel gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Marcel gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget(webContentsId?: number) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.MARCEL_DESKTOP_REMOTE_TOKEN,
      url: process.env.MARCEL_DESKTOP_REMOTE_URL
    },
    profile,
    registry: readDesktopConnectionsRegistry()
  })
...
async function rollbackSshBootstrapResult(ssh, result, profile, sshConfig, boundaryError) {
  ...
  try {
    const expected = {
      ...
      marcelPath: result.marcelPath,
      marcelHome: result.marcelHome,
    ...
    if (result.platform?.os === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        ssh,
        { marcelPath: result.marcelPath, marcelHome: result.marcelHome, python: result.pythonPath },
        expected
      )
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, metadata, fingerprint, lease) {
  ...
  try {
    ...
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteMarcelPath || '')
    const lifecycle = platform.os === 'Windows' ? connectWindowsRemote : remoteLifecycle.connect
    result = await lifecycle({
      ssh,
      profile: resolveRemoteSshDashboardProfile(sshConfig.remoteProfile, profile),
      remoteMarcelPath: sshConfig.remoteMarcelPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForMarcel: (baseUrl, token) => waitForMarcel(baseUrl, token, lease.signal, 'token'),
  ...
  await fenceManagedSshBootstrapPublication({
    ...
    publish: () => {
      ...
      sshConnections.set(scope, {
        ...
        host: sshConfig.host,
        hostLabel,
        marcelVersion: result.marcelVersion || '',
        ...
        // per-profile pools also use bare keys. Only startMarcel' explicit call
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.marcelVersion || 'marcel (version unknown)'} at ${result.marcelPath || '?'}`
  )
  ...
  return {
    ...connection,
    remoteMarcelVersion: result.marcelVersion || '',
    ssh: {
      effectiveConfigFingerprint: sshConfig.effectiveConfigFingerprint,
      host: sshConfig.host,
...
//   2. env override (MARCEL_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
function globalRemoteActive() {
  if (process.env.MARCEL_DESKTOP_REMOTE_URL) {
    return true
  }
...
// env → global) so a startMarcel() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Marcel
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteMarcelPath: input.sshRemoteMarcelPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteMarcelPath || '')
          let marcelPath
          let marcelVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            marcelPath = runtime.marcelPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.marcelPath])
            marcelVersion = inspection.version
            supported = inspection.supported
          } else {
            marcelPath = await remoteLifecycle.locateMarcel(ssh, sshConfig.remoteMarcelPath || '')
            marcelVersion = await remoteLifecycle.probeMarcelVersion(ssh, marcelPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, marcelPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Marcel on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteMarcelPath: marcelPath,
            remoteMarcelVersion: marcelVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startMarcel())
  ...
  // connect to Marcel gateway". Mirror the renderer's connect here so the test
...
function resetMarcelConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const marcelProcess = backendConnectionState.invalidate()
  stopBackendChild(marcelProcess)
...
// startMarcel() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetMarcelConnection() invalidates it.
  const marcelProcess = backendConnectionState.getProcess()
  const dying = marcelProcess && !marcelProcess.killed ? marcelProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('marcel:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'saved' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('marcel:connections:changed', payload)
    }
  }
}
...
async function ensureBackend(profile) {
  ...
  if (route.backend === 'primary') {
    const connection = await startMarcel()
  ...
  // spawning, so two children never share one profile's MARCEL_HOME.
  ...
  entry.connectionPromise = spawnPoolBackend(key, entry).catch(async error => {
    ...
    rememberLog(
      `Marcel backend for profile "${key}" failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
...
async function ensureRegistryBackend(connectionId, profile, managedUpdateCorrelation = '') {
  ...
  const resolveRegistrySshConfig = () => {
    ...
    if (!resolvedRegistrySshConfig) {
      resolvedRegistrySshConfig = normalizeSshConfig({
        ...
        remoteMarcelPath: source.remoteMarcelPath,
  ...
  if (source.kind === 'local') {
    ...
    localEntry.connectionPromise = spawnPoolBackend(profileKey, localEntry, {
      ...
      rememberLog(
        `Marcel backend for profile "${profileKey}" (forced-local) failed to start: ${error instanceof Error ? error.message : String(error)}`
      )
...
async function connectRegistryBackend(
  ...
  if (source.kind === 'ssh') {
    ...
    return {
      ...
      // is only the routing label. marcel:api uses it to translate explicit
      // self-profile query filters into the backend's namespace.
      remoteProfile: sshConfig.remoteProfile || '',
      logs: marcelLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  await waitForMarcel(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
function remoteUpdateTargetFromState(state): RemoteUpdateTarget {
  if (!state?.ssh || !state?.marcelPath || !state?.marcelHome) {
  ...
  return {
    ...
    marcelPath: state.marcelPath,
    marcelHome: state.marcelHome,
...
async function openManagedSshUpdateTransport(
  ...
  try {
    const platform: any = await detectRemotePlatform(ssh, config.remoteMarcelPath || '')

    if (platform.os === 'Windows') {
      const runtime = platform.marcelPath ? platform : await probeWindowsRemote(ssh, config.remoteMarcelPath || '')
      ...
      return {
        ...
        target: {
          ...
          platform: 'Windows',
          marcelPath: runtime.marcelPath,
          marcelHome: runtime.marcelHome,
          pythonPath: runtime.python
        }
      }
    }

    const marcelPath = await remoteLifecycle.locateMarcel(ssh, config.remoteMarcelPath || '')
    const marcelHome = await remoteLifecycle.probeRemoteMarcelHome(ssh)

    return {
      close: () => ssh.close(),
      target: { ssh, platform: platform.os, marcelPath, marcelHome }
    }
  } catch (error) {
    await ssh.close()
    throw error
...
async function drainManagedSshScope(scope) {
  ...
  try {
    ...
    if (state.remotePlatform === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        state.ssh,
        { marcelPath: state.marcelPath, marcelHome: state.marcelHome, python: state.pythonPath },
        expected
      )
    ...
    if (state && state.localPort && state.remotePort && scope.reuseToken) {
      try {
        ...
        if (!forwardClosed) {
          await waitForMarcel(`http://127.0.0.1:${state.localPort}`, scope.reuseToken, undefined, 'token')
        } else {
...
// local-spawn portion of startMarcel() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForMarcel(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // --profile wins over the inherited MARCEL_HOME env (see _apply_profile_override
  // step 3 in marcel_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveMarcelBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const marcelCwd = resolveMarcelCwd()
  ...
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(MARCEL_HOME, 'profiles', key))
  )
  rememberLog(`Starting Marcel backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: marcelCwd,
        MARCEL_DASHBOARD_SESSION_TOKEN: token,
        ...
        ...parentIdentityEnv,
        MARCEL_WEB_DIST: webDist,
        ...(readyFile ? { MARCEL_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Marcel backend for profile "${profile}" failed to start: ${error.message}`)
    void teardownFailedLocalBackend(poolKey, entry).catch(cleanupError => {
      rememberLog(
        `Marcel backend for profile "${profile}" cleanup failed: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`
      )
    })
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Marcel backend for profile "${profile}" exited (${signal || code})`)
    releaseLocalBackendSlot(entry)
    releaseBackendChild(child)
    ...
    if (!ready) {
      rejectStart?.(
        new Error(
          `Marcel backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  await Promise.race([waitForMarcel(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Marcel backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Marcel backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_marcel_home() recreates the deleted profile directory.
//
...
async function startMarcel() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Marcel Desktop is already running in another window.')
  }

  await reapOrphanedBackendsOnce()

  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startMarcel() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Marcel backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      if (!backendConnectionState.isCurrentAttempt(connectionAttempt)) {
        throw new Error('Marcel backend start was superseded by a newer connection attempt.')
      }

      await advanceBootProgress('backend.remote', `Connecting to remote Marcel backend at ${remote.baseUrl}`, 24)
      await waitForMarcel(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Marcel backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, marcelLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.marcel/active_profile file) and
    // resolves MARCEL_HOME the same way `marcel -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Marcel runtime', 28)

        return resolveMarcelBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting Marcel backend via ${backend.label}`, 84)
    rememberLog(`Starting Marcel backend via ${backend.label}`)
    ...
    const marcelProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin MARCEL_HOME for the child so Python's get_marcel_home()
          // resolves to the SAME location our resolveMarcelHome() picked. Without
          // this pin, Python falls back to ~/.marcel on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\marcel, which differs from C:\Users\<u>\.marcel.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets MARCEL_HOME via setx; the desktop
    ...
    primaryOutputTail.attach(marcelProcess)
    ...
    const portAnnouncement = waitForDashboardPortAnnouncement(marcelProcess, {
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, marcelProcess)

    if (!processOwner) {
      stopBackendChild(marcelProcess)
      await waitForBackendExit(marcelProcess)
      releaseBackendChild(marcelProcess)
      throw new Error('Marcel backend start was superseded by a newer connection attempt.')
    }

    marcelProcess.stdout.on('data', rememberLog)
    marcelProcess.stderr.on('data', rememberLog)
    ...
    marcelProcess.once('error', error => {
      releaseBackendChild(marcelProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Marcel backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Marcel backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Marcel backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Marcel backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    marcelProcess.once('exit', (code, signal) => {
      releaseBackendChild(marcelProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Marcel backend exit (${signal || code})`)
      ...
      rememberLog(`Marcel backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Marcel backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Marcel backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentMarcelLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Marcel backend to launch', 86)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Marcel backend to become ready', 90)
    await Promise.race([waitForMarcel(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => marcelProcess.exitCode === null && !marcelProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Marcel backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Marcel backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
    ...
    // The backend's plugin discovery just ran and refreshed MARCEL_HOME/.plugin-compat-report.json.
...
function wireCommonWindowHandlers(win, { zoom = true }: { zoom?: boolean } = {}) {
  ...
  // links arrive via `marcel:openExternal`, not here. See window-open-policy.ts.
...
function notifyBrowserPopoutClosed(tabId) {
  ...
  for (const other of BrowserWindow.getAllWindows()) {
    if (!other.isDestroyed()) {
      other.webContents.send('marcel:browser-popout:closed', tabId)
    }
  }
}
...
// here so it can leave the app's bounds and stay visible while Marcel is
...
// pushes pet state over IPC (marcel:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via marcel:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Marcel app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Marcel icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see marcel:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Marcel in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('marcel:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so Marcel can be driven
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('marcel:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function startHudGameOverlayFeed(win: BrowserWindow) {
  ...
  const push = (state: { active: boolean; app: string }) => {
    if (!win.isDestroyed()) {
      win.webContents.send('marcel:hud:game-overlay', state)
    }
  }
  ...
  const enumerate = async () => {
    ...
    if (!reported) {
      reported = true
      console.warn(`[marcel] HUD cannot enumerate windows: ${windows.reason}`)
    }
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('marcel:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `marcel:hud:set-bounds`, which flips resizable on for the call — the
  ...
  // grab, marcel:hud:workspace-transfer temporarily makes the X11 window
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('marcel:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('marcel:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('marcel:quick-entry:shown')
}
...
function createWindow() {
    ...
    callbacks: {
      ...
      onFailedLoadBudgetExhausted: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          ...
          repairHint: 'marcel desktop --force-build',
  ...
  startMarcel().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('marcel:connection', async (_event, profile) => {
...
ipcMain.handle('marcel:connection:for', async (_event, payload) => {
  const { connectionId, profile } = payload && typeof payload === 'object' ? (payload as any) : ({} as any)
  const registry = readDesktopConnectionsRegistry()
  const id = String(connectionId || '').trim() || registry.primary
  // Same single-owner claim as 'marcel:connection', keyed by the composite
...
ipcMain.on('marcel:connection:active-route', (event, route) => {
...
// "Starting Marcel…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('marcel:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: () => resetMarcelConnection({ soft: true }),
...
ipcMain.handle('marcel:backend:touch', async (_event, profile) => {
...
ipcMain.handle('marcel:pool-limits:get', async () => ({ ...poolLimits }))
ipcMain.handle('marcel:pool-limits:set', async (_event, raw) => {
...
ipcMain.handle('marcel:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('marcel:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('marcel:window:openInstance', async () => {
  createInstanceWindow()

  return { ok: true }
})
ipcMain.handle('marcel:window:openBrowser', async (_event, tabId) => {
...
ipcMain.handle('marcel:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = resolveMarcelBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'Marcel is not installed yet' }
    }
    ...
    const scriptPath = path.join(
      scriptDir,
      `marcel-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, MARCEL_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('marcel:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('marcel:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('marcel:zoom:get', event => {
...
ipcMain.on('marcel:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('marcel:backend:recycle', async (_event, profile) => {
...
ipcMain.handle('marcel:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startMarcel() call restarts the
...
ipcMain.handle('marcel:bootstrap:repair', async () => {
  // Forceful repair: force the next startMarcel() through the full installer
  ...
  // "bypass active runtime" path inside startMarcel() and fall through
...
ipcMain.handle('marcel:boot-progress:get', async () => bootProgressState)
ipcMain.handle('marcel:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('marcel:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('marcel:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('marcel:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('marcel:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('marcel:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('marcel:secret-storage:get', async () => ({ on: secretStoragePolicy().on }))
ipcMain.handle('marcel:secret-storage:set', async (_event: any, on: any) => applySecretStorageEncryption(on === true))
...
ipcMain.handle('marcel:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('marcel:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('marcel:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('marcel:connections:set-primary', async (_event, id) => {
...
ipcMain.handle('marcel:connections:set-launch-mode', async (_event, mode) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('marcel:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('marcel:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteMarcelPath: entry.remoteMarcelPath
    })
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteMarcelPath: connection.remoteMarcelPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteMarcelProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root MARCEL_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('marcel:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// marcel:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
...
ipcMain.handle('marcel:gateway:ws-url-for', async (_event, payload) => {
...
ipcMain.handle('marcel:connections:update-managed', async (_event, rawId) => requestManagedSshUpdate(rawId))

// Fan out `marcel update` to every eligible registered connection at once.
...
ipcMain.handle('marcel:connections:update-all', async (_event, payload) => {
  const registry = readDesktopConnectionsRegistry()
    ...
    registry.connections
      ...
      .map(async connection => {
        ...
        try {
          ...
          const body: any = await postJsonForBackend(descriptor, '/api/marcel/update', {}, { timeoutMs: 15_000 })
...
// analogue of the marcel:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('marcel:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('marcel:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startMarcel() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('marcel:connection-config:oauth-logout', async (_event, rawUrl) => {
  const baseUrl = normalizeRemoteBaseUrl(rawUrl)
...
// --- Marcel Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('marcel:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('marcel:connection-config:save', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('marcel:connection-config:apply', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('marcel:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
// Persistence-only sibling of marcel:profile:set: records the profile the
...
// the gateway (#79886).
ipcMain.handle('marcel:profile:remember', async (_event, name) => ({
  profile: writeActiveDesktopProfile(name)
}))
ipcMain.handle('marcel:profile:set', async (_event, name) => {
  assertCanMutateManagedPrimaryRouting()
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new MARCEL_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('marcel:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('marcel:requestMicrophoneAccess', async () => {
...
ipcMain.handle('marcel:window:readBelow', async event => {
...
async function handleMarcelApiRequest(request) {
  ...
  // backend calls ensure_marcel_home() which recreates the profile directory,
...
ipcMain.handle('marcel:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its MARCEL_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleMarcelApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleMarcelApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('marcel:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('marcel:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('marcel:focus-session', payload.sessionId)
    }

    // Plugin / session-less activation — serializable path (+ optional notifyId
    // for renderer callbacks). Same vocabulary as marcel://index-network/….
    if (payload?.activate || payload?.notifyId) {
      mainWindow.webContents.send('marcel:notification-activate', {
  ...
  notification.on('action', (_actionEvent, index) => {
    ...
    if (payload?.sessionId && !payload?.notifyId && !payload?.activate) {
      mainWindow.webContents.send('marcel:notification-action', { sessionId: payload.sessionId, actionId: action.id })
...
ipcMain.handle('marcel:data-url-read-max:get', () => ({
...
ipcMain.handle('marcel:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('marcel:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('marcel:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('marcel:readFileText', async (_event, filePath) => {
...
// `marcel:readFileText` is the *preview* read and silently truncates at
...
ipcMain.handle('marcel:readPluginSource', async (_event: unknown, filePath: unknown) => {
  const { resolvedPath, stat } = await resolveReadableFileForIpc(filePath, {
...
ipcMain.handle('marcel:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('marcel:writeClipboard', (_event, text) => {
...
ipcMain.handle('marcel:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('marcel:readClipboard', () => clipboard.readText())

ipcMain.handle('marcel:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('marcel:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))
...
ipcMain.handle('marcel:context-menu:edit', (event, command) => {
...
ipcMain.handle('marcel:context-menu:copy-image', event => {
...
ipcMain.handle('marcel:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('marcel:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('marcel:capturePreview', async (_event, payload) => {
  const guest = electronWebContents.fromId(Number(payload?.webContentsId))

  return capturePreviewContents(guest, payload?.rect, payload?.viewport)
})

ipcMain.handle('marcel:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('marcel:saveClipboardImage', async () => {
...
ipcMain.handle('marcel:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('marcel:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('marcel:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('marcel:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('marcel:active-work', (event, payload) => {
  const id = event.sender.id
...
ipcMain.on('marcel:titlebar-theme', (_event, payload) => {
...
ipcMain.on('marcel:native-theme', (_event, mode) => {
...
ipcMain.on('marcel:translucency:support', event => {
...
ipcMain.on('marcel:keep-awake', (_event, on) => {
...
ipcMain.handle('marcel:quick-entry:settings:get', async () => {
...
ipcMain.handle('marcel:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('marcel:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('marcel:quick-entry:submit', {
...
ipcMain.on('marcel:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('marcel:quick-entry:state', payload)
  }
})

ipcMain.on('marcel:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('marcel:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('marcel:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `marcel:window:openSession`).
...
ipcMain.handle('marcel:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('marcel:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('marcel:preview:reach', async (event, url) => reachablePreviewUrl(event.sender.id, String(url || '')))

ipcMain.handle('marcel:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveMarcelCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('marcel:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveMarcelCwd()
}))

ipcMain.handle('marcel:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('marcel:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('marcel:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('marcel:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('marcel:resolveFavicon', (_event, url) => resolveFaviconCached(url))

ipcMain.handle('marcel:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('marcel:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: marcelLog.slice(-200) }))
...
ipcMain.on('marcel:logs:renderer-error', (_event, report) => {
...
// Embedded terminal PTY host (marcel:terminal:*) — see terminal-ipc.ts.
...
ipcMain.handle('marcel:updates:check', async () =>
...
ipcMain.handle('marcel:updates:apply', async (_event, payload) =>
...
ipcMain.handle('marcel:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('marcel:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Marcel version (the one `release.py` bumps in
// marcel_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Marcel version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveMarcelVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'marcel_cli', '__init__.py')
...
// Renderer-bundle skew: `marcel update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live Marcel version and push it into the native About panel
// just before showing it, so an in-place `marcel update` is reflected without
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolveMarcelVersion()} — app build out of date, update the desktop app`
        : resolveMarcelVersion(),
      copyright: 'Copyright © 2026 Nous Research'
    })
    app.showAboutPanel()
  })
...
ipcMain.handle('marcel:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolveMarcelVersion(),
    electronVersion: process.versions.electron,
    nodeVersion: process.versions.node,
    platform: process.platform,
    marcelRoot: resolveUpdateRoot(),
...
// The About page's "Restart Marcel" button (shown when bundleSwapPending):
...
ipcMain.handle('marcel:app:relaunch', async () => {
...
// the actual removal via `marcel uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (marcel_cli/uninstall.py + marcel_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_MARCEL_ROOT
  ...
  const fallback = () => ({
    marcel_home: MARCEL_HOME,
    agent_installed: isMarcelSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'marcel_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, MARCEL_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Marcel agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import marcel_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_MARCEL_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `marcel-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `marcel-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('marcel:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('marcel:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('marcel:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('marcel:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// marcel:// deep links (e.g. marcel://blueprint/morning-brief?time=08:00,
// marcel://mcp/install?name=NAME&config=B64 — the vendor "Add to Marcel"
// button, or marcel://plugin/install?repo=owner/repo). Dev
// (`MARCEL_DESKTOP_DEV_SERVER`) registers marcel-dev:// instead — bare
// Electron or a stale OS handler often owns marcel:// on dev machines.
...
// ---------------------------------------------------------------------------
const MARCEL_PROTOCOL = DEV_SERVER ? 'marcel-dev' : 'marcel'
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['marcel-dev', 'marcel'] : ['marcel']
...
function handleDeepLink(url) {
  ...
  // marcel://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('marcel:deep-link', payload)
...
ipcMain.handle('marcel:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${MARCEL_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(MARCEL_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(MARCEL_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${MARCEL_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `marcel://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startMarcel), whose
...
app.whenReady().then(() => {
  ...
  // startMarcel() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
  ...
  // Win/Linux cold start: the launching marcel:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting Marcel never keeps another app's chord hostage.
