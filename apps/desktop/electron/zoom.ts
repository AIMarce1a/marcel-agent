...
export const ZOOM_STORAGE_KEY = 'marcel:desktop:zoomLevel'
 ...
 * but forgot to emit 'marcel:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('marcel:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `marcel:zoom:changed`, which never fired —
