 ...
 *   - the browser sees only a minimal "return to Marcel" page.
 */
...
const DONE_HTML =
  ...
  '<p>You can close this window and return to Marcel.</p>' +
...
export function registerMcpOauthCallbackIpc() {
  ...
  ipcMain.handle('marcel:mcp-oauth:listen', async () => {
  ...
  ipcMain.handle('marcel:mcp-oauth:wait', async (_event, id, timeoutMs) => {
  ...
  ipcMain.handle('marcel:mcp-oauth:cancel', (_event, id) => {
    dispose(String(id || ''))
