// Profile-delete routing logic for the `marcel:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_marcel_home() and
...
/** Parse a `marcel:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its MARCEL_HOME.
 *
...
/**
 * Route the next `marcel:api` request away from the primary/window backend
 ...
 * ensure_marcel_home() recreates the directory the delete just removed.
 */
