 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `marcel update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.marcelHome, 'Marcel home')
  const marcelPath = validateRemoteValue(target.marcelPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(marcelPath)

  const updateCommand =
    `env MARCEL_HOME=${homeWord} ` +
    `MARCEL_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'MARCEL_UPDATE_ORIGIN_PROFILE=default ' +
    `MARCEL_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `MARCEL_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:MARCEL_HOME=${psLiteral(home)}`,
    `$env:MARCEL_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:MARCEL_UPDATE_ORIGIN_PROFILE="default"',
    `$env:MARCEL_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:MARCEL_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:MARCEL_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:MARCEL_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:MARCEL_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(marcelPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.marcel-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote Marcel updated and every managed SSH profile is ready.'
        : restoreOk
