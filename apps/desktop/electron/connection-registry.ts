 ...
 * runtime, remote gateways, Marcel Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@marcel/shared`) — tsconfig project
...
export async function reuseMatchingPrimarySshBackend({
  ...
  if (
    ...
    String(source.remoteMarcelPath || '').trim() !== String(activeSsh.remoteMarcelPath || '').trim() ||
...
/** Turn `ls ~/.marcel/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. Marcel Cloud
 * instances are platform-managed — we never run `marcel update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteMarcelPath: input.remoteMarcelPath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/Marcel-managed names; an empty result stores nothing.
 ...
 * `remoteMarcelPath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Marcel Cloud' : 'Remote gateway'),
