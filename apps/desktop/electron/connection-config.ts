 ...
 *     `X-Marcel-Session-Token` header; WS uses `?token=`.
...
// marcel_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - marcel_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - marcel_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / marcel #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-marcel_session_at', '__Secure-marcel_session_at', 'marcel_session_at']
const RT_COOKIE_VARIANTS = ['__Host-marcel_session_rt', '__Secure-marcel_session_rt', 'marcel_session_rt']

// The Nous portal (NAS) does NOT use Marcel gateway session cookies — it is a
...
// Keep this aligned with marcel_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['marcel', 'test', 'tmp', 'root', 'sudo'])
...
/** Which Marcel profile the remote SSH dashboard should actually run as.
...
// Marcel Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteMarcelPath = String(entry.remoteMarcelPath || '').trim()

  if (remoteMarcelPath) {
    out.remoteMarcelPath = remoteMarcelPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Marcel installation. Preserve an explicit mapping
  // when it is a valid Marcel profile identifier; otherwise fall back to the
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_marcel_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `MARCEL_HOME` scopes it.
 *
...
function resolveProfileBackendRoute(profile, opts: ProfileRouteOptions = {}): ProfileBackendRoute {
  ...
  if (scopedProfile === primaryProfile) {
    ...
    // the wire: the dashboard's process MARCEL_HOME can belong to a different
 ...
 * `marcel:api` handler — both routes reach a backend whose namespace is the
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://marcel.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `marcel:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a marcel session ACCESS-token cookie
 ...
 * the Marcel gateway session cookies, so cloud sign-in / discovery liveness
