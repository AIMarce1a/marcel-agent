 ...
 *
 * The staged Tauri `marcel-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `marcel update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `marcel update`, swaps/relaunches
 * the app, and writes .marcel-update-result.json. With the app gone before
 * the update starts, the MARCEL_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `marcel update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into MARCEL_HOME on *every* platform
 * (`marcel-setup.exe` on Windows, `marcel-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_marcel_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `marcel update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(marcelHome, 'marcel-setup.exe')
 ...
 *
 * `copy_self_to_marcel_home` deliberately no-ops during `--update`
 ...
 * "Another Marcel update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `marcel update`, which
 ...
 * the installer is written to MARCEL_HOME at install/repair time, making mtime
