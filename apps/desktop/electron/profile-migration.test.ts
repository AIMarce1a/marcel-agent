 ...
 *      top of startMarcel() in main.ts, before primaryProfileKey() is read); the
...
function baseDeps(overrides: Record<string, unknown> = {}) {
  ...
  return {
    legacyActivePath: '/home/u/.marcel/active_profile',
    marcelHome: '/home/u/.marcel',
    profilesRoot: '/home/u/.marcel/profiles',
    existsSync: fs.existsSync,
    readFileSync: fs.readFileSync,
    statSync: fs.statSync,
    readdirSync: fs.readdirSync,
    isMarcelProcess: () => true,
    now: () => NOW,
    writeJson: () => {},
    isValidProfileName,
    ...overrides
...
test('findRunningGatewayProfiles returns [] when no pid files exist', () => {
  const fs = makeFs({ '/home/u/.marcel/profiles/coder': { dir: true } })
  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.marcel/profiles', ['coder'], {
      ...fs,
      isMarcelProcess: () => true
    }),
    []
...
test('findRunningGatewayProfiles drops stale (non-marcel) recycled PIDs', () => {
  // Two profiles have pid files, but only coder's pid is a live marcel process.
  // The recycled PID at 5678 belongs to an unrelated process (e.g. Chrome).
  const fs = makeFs({
    '/home/u/.marcel/profiles/coder/gateway.pid': { content: '{"pid":1234}' },
    '/home/u/.marcel/profiles/writer/gateway.pid': { content: '{"pid":5678}' }
  })

  const deps = {
    ...fs,
    isMarcelProcess: (pid: number) => pid === 1234
  }

  assert.deepEqual(findRunningGatewayProfiles('/home/u/.marcel/profiles', ['coder', 'writer'], deps), ['coder'])
})

test('findRunningGatewayProfiles tolerates malformed pid files', () => {
  const fs = makeFs({
    '/home/u/.marcel/profiles/coder/gateway.pid': { content: '{not json' }
  })
...
test('findRunningGatewayProfiles drops non-integer and non-positive PIDs', () => {
  const fs = makeFs({
    '/home/u/.marcel/profiles/coder/gateway.pid': { content: '{"pid":-1}' },
    '/home/u/.marcel/profiles/writer/gateway.pid': { content: '{"pid":1.5}' },
    '/home/u/.marcel/profiles/extra/gateway.pid': { content: '{"pid":0}' }
  })

  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.marcel/profiles', ['coder', 'writer', 'extra'], {
      ...fs,
      isMarcelProcess: () => true
    }),
    []
...
test('findRunningGatewayProfiles preserves order of allProfiles', () => {
  const fs = makeFs({
    '/home/u/.marcel/profiles/coder/gateway.pid': { content: '{"pid":1}' },
    '/home/u/.marcel/profiles/writer/gateway.pid': { content: '{"pid":2}' }
  })

  const deps = { ...fs, isMarcelProcess: () => true }
  assert.deepEqual(findRunningGatewayProfiles('/home/u/.marcel/profiles', ['coder', 'writer'], deps), [
    'coder',
    'writer'
  ])
})
...
test('listProfileDirs includes default and any name passing the regex', () => {
  const fs = makeFs({
    '/home/u/.marcel/profiles/default': { dir: true },
    '/home/u/.marcel/profiles/coder': { dir: true },
    '/home/u/.marcel/profiles/writer': { dir: true }
  })
...
test('listProfileDirs skips files (not directories) and invalid names', () => {
  const fs = makeFs({
    '/home/u/.marcel/profiles/default': { dir: true },
    '/home/u/.marcel/profiles/some-file.txt': { dir: false },
    '/home/u/.marcel/profiles/UPPERCASE': { dir: true } // regex rejects
  })
...
test('decideMigration suppresses write when best is default (single-profile fallback)', () => {
  ...
  // default-primary and we leave it alone. Default's DB is $MARCEL_HOME/state.db,
  // not profiles/default/state.db.
  const deps = baseDeps()

  const d = decideMigration(null, [], ['default', 'coder'], deps, p => (p.endsWith('/.marcel/state.db') ? 99 : 50))
...
test('migrateActiveProfileIfMissing is a no-op when a user-selected preference file exists', () => {
  ...
  const fs = makeFs({
    ...
    '/home/u/.marcel/profiles/coder': { dir: true },
    '/home/u/.marcel/profiles/writer': { dir: true },
    '/home/u/.marcel/profiles/writer/state.db': { size: 400 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing writes legacy choice with no _migrated flag', () => {
  ...
  const fs = makeFs({
    '/home/u/.marcel/active_profile': { content: 'coder' },
    '/home/u/.marcel/profiles/coder': { dir: true },
    '/home/u/.marcel/profiles/writer': { dir: true }
  })
...
test('migrateActiveProfileIfMissing writes heuristic choice with _migrated=true', () => {
  ...
  const fs = makeFs({
    '/home/u/.marcel/profiles/coder/state.db': { size: 50 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.marcel/profiles/writer/state.db': { size: 200 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing is a no-op for single-profile (default-only) installs', () => {
  ...
  // Production default DB is ~/.marcel/state.db, not profiles/default/state.db.
  let written: unknown = null

  const fs = makeFs({
    '/home/u/.marcel/state.db': { size: 10 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing prefers a single running gateway over heuristics', () => {
  ...
  const fs = makeFs({
    '/home/u/.marcel/profiles/coder/gateway.pid': { content: '{"pid":42}' },
    '/home/u/.marcel/profiles/writer/state.db': { size: 500 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })

  const deps = baseDeps({
    ...fs,
    isMarcelProcess: (pid: number) => pid === 42,
    writeJson: (_p: string, payload: unknown) => {
      written = payload
    }
  })
...
// ---------------------------------------------------------------------------
// Production layout: default is ~/.marcel, not ~/.marcel/profiles/default
// ---------------------------------------------------------------------------

test('profileStateDbPath puts default at marcelHome, named under profilesRoot', () => {
  assert.equal(profileStateDbPath('default', '/home/u/.marcel', '/home/u/.marcel/profiles'), '/home/u/.marcel/state.db')
  assert.equal(
    profileStateDbPath('conduit', '/home/u/.marcel', '/home/u/.marcel/profiles'),
    '/home/u/.marcel/profiles/conduit/state.db'
  )
})

test('profileGatewayPidPath puts default at marcelHome', () => {
  assert.equal(
    profileGatewayPidPath('default', '/home/u/.marcel', '/home/u/.marcel/profiles'),
    '/home/u/.marcel/gateway.pid'
  )
  assert.equal(
    profileGatewayPidPath('coder', '/home/u/.marcel', '/home/u/.marcel/profiles'),
    '/home/u/.marcel/profiles/coder/gateway.pid'
  )
})
...
test('findRunningGatewayProfiles sees default gateway.pid at marcelHome', () => {
  const fs = makeFs({
    '/home/u/.marcel/gateway.pid': { content: '{"pid":99}' },
    '/home/u/.marcel/profiles/coder/gateway.pid': { content: '{"pid":11}' }
  })

  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.marcel/profiles', ['default', 'coder'], {
      ...fs,
      marcelHome: '/home/u/.marcel',
      isMarcelProcess: pid => pid === 99
    }),
    ['default']
  )
})
...
test('migrateActiveProfileIfMissing does not pin a tiny named profile over a large default DB', () => {
  ...
  // ~/.marcel/profiles/<name>, never scored ~/.marcel/state.db, and wrote
  // { profile: named, _migrated: true }.
  let written: unknown = null

  const fs = makeFs({
    '/home/u/.marcel/profiles/conduit': { dir: true },
    '/home/u/.marcel/state.db': { size: 409 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.marcel/profiles/conduit/state.db': { size: 2 * 1024 * 1024, mtime: NOW - 60_000 }
  })
...
test('migrateActiveProfileIfMissing still pins a named profile that actually beats default', () => {
  ...
  const fs = makeFs({
    '/home/u/.marcel/profiles/work': { dir: true },
    '/home/u/.marcel/state.db': { size: 5 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.marcel/profiles/work/state.db': { size: 200 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing does not pin default when only default gateway is running', () => {
  ...
  const fs = makeFs({
    '/home/u/.marcel/gateway.pid': { content: '{"pid":7}' },
    '/home/u/.marcel/state.db': { size: 10 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })

  const deps = baseDeps({
    ...fs,
    isMarcelProcess: (pid: number) => pid === 7,
    writeJson: (_p: string, payload: unknown) => {
      written = payload
    }
  })
