...
test('environment URL without its token keeps the existing error', () => {
  assert.throws(
    ...
    /MARCEL_DESKTOP_REMOTE_TOKEN is not/
  )
})
...
test('profile SSH route fails closed when any dial field differs', () => {
  ...
  const variants = [
    { ...ssh, port: 2200 },
    { ...ssh, keyPath: '/keys/b' },
    { ...ssh, remoteMarcelPath: '/opt/marcel' },
...
test('global SSH treats an omitted port as 22 and checks the primary route', () => {
  const route = resolveDesktopRemoteRoute({
    config: { mode: 'ssh', remote: { mode: 'ssh', host: 'box.test', user: 'marcel' } },
    registry: registry('ssh-primary', [
      { id: 'ssh-primary', kind: 'ssh', label: 'SSH primary', host: 'box.test', user: 'marcel', port: 22 }
    ])
  })
...
test('profile remote wins over a registry-backed global SSH route', () => {
  const route = resolveDesktopRemoteRoute({
    config: {
      mode: 'ssh',
      remote: { mode: 'ssh', host: 'global-box.test', user: 'marcel' },
      profiles: {
        worker: { mode: 'remote', url: 'https://worker.test', authMode: 'token', token: tokenA }
      }
    },
    ...
    registry: registry('global-ssh', [
      { id: 'global-ssh', kind: 'ssh', label: 'Global SSH', host: 'global-box.test', user: 'marcel' },
...
test('profile SSH wins over a different registry primary SSH route', () => {
  const route = resolveDesktopRemoteRoute({
    config: {
      ...
      remote: { mode: 'ssh', host: 'global-box.test', user: 'marcel' },
      profiles: {
        worker: { mode: 'ssh', host: 'worker-box.test', user: 'marcel' }
      }
    },
    profile: 'worker',
    registry: registry('global-ssh', [
      { id: 'global-ssh', kind: 'ssh', label: 'Global SSH', host: 'global-box.test', user: 'marcel' },
      { id: 'worker-ssh', kind: 'ssh', label: 'Worker SSH', host: 'worker-box.test', user: 'marcel' }
    ])
  })
...
test('local route does not inherit an unrelated registry SSH connection', () => {
  const route = resolveDesktopRemoteRoute({
    config: { mode: 'local' },
    registry: registry('local', [
      { id: 'unused-ssh', kind: 'ssh', label: 'Unused SSH', host: 'box.test', user: 'marcel' }
    ])
  })
...
// remote transport, or startMarcel() spawns a loopback `marcel serve` the
