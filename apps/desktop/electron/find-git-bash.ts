 ...
 *   1. MARCEL_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\marcel\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect MARCEL_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.MARCEL_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'marcel', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'marcel', 'git', 'usr', 'bin', 'bash.exe'))
  }
