...
const READY_RE = /^MARCEL_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitMarcelPath = '') {
  const explicit = psLiteral(explicitMarcelPath)
  ...
  const script = [
    ...
    '$marcelHome=$env:MARCEL_HOME',
    'if(-not $marcelHome){$marcelHome=Join-Path $env:LOCALAPPDATA "marcel"}',
    'Assert-NoReparse $marcelHome $true',
    '$candidate=[IO.Path]::Combine($marcelHome, "marcel-agent\\venv\\Scripts\\marcel.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "marcel-agent\\.venv\\Scripts\\marcel.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $marcelHome "marcel-agent\\venv\\Scripts\\marcel.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "marcel-agent\\.venv\\Scripts\\marcel.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command marcel.exe -ErrorAction SilentlyContinue',
    ...
    '$marcel=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$marcel=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $marcel){throw "Marcel is not installed on the remote Windows host."}',
    'Assert-NoReparse $marcel $false',
    'if($explicit -and $marcel -ne $explicit){throw "The configured Marcel path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($marcel), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;marcelHome=$marcelHome;marcelPath=$marcel;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(marcelHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class MarcelMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$home=${psLiteral(marcelHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".marcel-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[MarcelMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, marcelHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(marcelHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
async function detectRemotePlatform(ssh, explicitMarcelPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitMarcelPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'marcel_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'marcel_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'marcel_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$home=${psLiteral(runtime.marcelHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".marcel-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};marcelPath=${psLiteral(reservation.marcelPath)};marcelHome=${psLiteral(reservation.marcelHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.marcelPath === 'string' &&
    typeof lock.marcelHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.marcelPath === runtime.marcelPath &&
    lock.marcelHome === runtime.marcelHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteMarcelPath)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.marcelHome)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.marcelPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Marcel on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.marcelPath = inspection.path
  const marcelVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located marcel at ${runtime.marcelPath}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, marcelPath: runtime.marcelPath }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForMarcel(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Marcel SSH"', 'powershell.exe -NoLogo')
