 ...
 * the three user-facing uninstall modes to the `marcel uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m marcel_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `marcel_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'marcel_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Marcel.app/Contents/MacOS/Marcel  → …/Marcel.app
 *   Windows: …\Marcel\Marcel.exe                 → …\Marcel  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Marcel.app/Contents/MacOS/Marcel → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Marcel.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Marcel.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Marcel$/i.test(dir) || /[\\/]marcel-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/marcel
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import marcel_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, marcelHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export MARCEL_HOME=${q(marcelHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import marcel_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Marcel install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "MARCEL_HOME=${String(marcelHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
