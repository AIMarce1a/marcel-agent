...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'marcel-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/marcel-runner-test',
    sourceRepoRoot: null,
    marcelHome: '/tmp/marcel-runner-test',
    logRoot: '/tmp/marcel-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'marcel-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'marcel-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/marcel-agent',
      marcelHome: '/tmp/marcel'
    }),
    ['--dir', '/tmp/marcel-agent', '--marcel-home', '/tmp/marcel', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/marcel-agent', '--marcel-home', '/tmp/marcel', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/marcel',
      marcelHome: '/tmp/home'
    }),
    ['--dir', '/tmp/marcel', '--marcel-home', '/tmp/home', '--branch', 'main']
  )
})
