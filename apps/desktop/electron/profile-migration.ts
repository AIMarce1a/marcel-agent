...
export interface MigrationDeps {
  ...
  /** Default profile home (`~/.marcel`). Default's state.db and gateway.pid live here. */
  marcelHome: string
  /** Named-profile root (`~/.marcel/profiles`). Does not contain `default`. */
  profilesRoot: string
  ...
  isMarcelProcess: (pid: number) => boolean
  now: () => number
...
/**
 * Production layout: default IS `marcelHome`; named profiles are children of
 * `profilesRoot`. There is no `profiles/default` directory on a normal install.
 */
export function profileStateDbPath(name: string, marcelHome: string, profilesRoot: string): string {
  return name === 'default' ? `${marcelHome}/state.db` : `${profilesRoot}/${name}/state.db`
}

export function profileGatewayPidPath(name: string, marcelHome: string, profilesRoot: string): string {
  return name === 'default' ? `${marcelHome}/gateway.pid` : `${profilesRoot}/${name}/gateway.pid`
}

function resolveMarcelHome(profilesRoot: string, marcelHome?: string): string {
  if (marcelHome) {
    return marcelHome
  }

  // Tests that predate marcelHome pass only profilesRoot.
...
/**
 * Return the profile names whose gateway.pid file points to a live marcel process.
 ...
 *
 * `marcelHome` is optional so existing call sites that only pass `profilesRoot`
 * still work: it is derived as the parent of `…/profiles`.
 */
export function findRunningGatewayProfiles(
  profilesRoot: string,
  allProfiles: string[],
  deps: Pick<MigrationDeps, 'existsSync' | 'readFileSync' | 'isMarcelProcess'> & { marcelHome?: string }
): string[] {
  const marcelHome = resolveMarcelHome(profilesRoot, deps.marcelHome)
  const running: string[] = []

  for (const name of allProfiles) {
    const pidFile = profileGatewayPidPath(name, marcelHome, profilesRoot)
...
export function decideMigration(
  ...
  for (const name of candidates) {
    const s = score(profileStateDbPath(name, deps.marcelHome, deps.profilesRoot))
...
/** Default is always a candidate; it is `$MARCEL_HOME`, not `$MARCEL_HOME/profiles/default`. */
...
export function migrateActiveProfileIfMissing(desktopProfileConfigPath: string, deps: MigrationDeps): boolean {
  ...
  // launches `marcel --profile default` and is worse than writing nothing
