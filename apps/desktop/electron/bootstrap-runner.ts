 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Marcel Agent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.marcel-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(marcelHome) {
  return path.join(marcelHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.marcel/marcel-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(marcelHome) {
  if (!marcelHome) {
    return null
  }

  const candidate = path.join(marcelHome, 'marcel-agent', 'scripts', installScriptName())
...
function cachedScriptPath(marcelHome, commit) {
  return path.join(bootstrapCacheDir(marcelHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(marcelHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(marcelHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, marcelHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass MARCEL_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          MARCEL_HOME: marcelHome || process.env.MARCEL_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, marcelHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, marcelHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--marcel-home', marcelHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, marcelHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, marcelHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, marcelHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(marcelHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, marcelHome, emit })
