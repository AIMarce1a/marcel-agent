...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(MARCEL_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Marcel-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.marcel/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'marcel-desktop-ssh')
  }

  return path.join(os.homedir(), '.marcel', 'desktop-ssh')
}
