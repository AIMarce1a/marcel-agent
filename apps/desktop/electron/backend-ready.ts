...
// `marcel serve` announces MARCEL_BACKEND_READY; the legacy `marcel dashboard`
// backend announces MARCEL_DASHBOARD_READY. Accept either so the desktop spawn
// works against both the headless backend and old/dashboard runtimes.
const _READY_RE = /^MARCEL_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m

// The announcement clock starts the instant the backend process is spawned —
// before uvicorn binds its socket. On a cold install the child must first
// compile and import the whole `marcel_cli.main` → `web_server` → FastAPI/
 ...
 * MARCEL_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS env override (for users on slow
...
function resolvePortAnnounceTimeoutMs(env = process.env) {
  const parsed = Number(env.MARCEL_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS)
...
/**
 * Watch a child process's stdout for the `MARCEL_(BACKEND|DASHBOARD)_READY
...
function waitForDashboardPort(
  ...
  return new Promise((resolve, reject) => {
    ...
    function onExit(code, signal) {
      cleanup()
      reject(new Error(`Marcel backend: exited before port announcement (${signal || code})${describeOutputTail()}`))
    }
    ...
    const timer = setTimeout(() => {
      cleanup()
      reject(new Error(`Timed out waiting for Marcel backend port announcement (${timeoutMs}ms)`))
    }, timeoutMs)
