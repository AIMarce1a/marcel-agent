...
function fakeSsh(rules: any[] = []) {
  ...
  return {
    ...
    async exec(cmd) {
      ...
      if (cmd.includes('.marcel-update-in-progress') && !cmd.includes('marker_clear()') && !/setsid|nohup/.test(cmd)) {
        return 'CLEAR'
      }
...
test('POSIX relaunch gate refuses live and uncertain install markers without executing Marcel', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = {
      async exec(command) {
        ...
        if (command.includes('MARCEL_HOME')) {
          return '/home/alice/.marcel\n'
        }

        if (command.includes('.marcel-update-in-progress')) {
          return observation
        }
...
test('POSIX relaunch gate permits absent/dead markers and normalizes named-profile homes install-wide', async () => {
  ...
  await assertRemoteInstallUpdateClear(ssh, '/home/alice/.marcel/profiles/research')
  assert.match(commands[0], /home\.parent\.name/)
  assert.match(commands[0], /profiles/)
  assert.match(commands[0], /\.marcel-update-in-progress/)
})
...
test('listRemoteMarcelProfiles inventories Mini-style profile dirs without spawning a dashboard', async () => {
  const ssh = fakeSsh([
    [/MARCEL_HOME/, '/Users/zillajr/.marcel\n'],
    [/ls -1/, 'bob\ndixie\ngoose\nrambo\nbob.rollback-old\n']
  ])

  assert.deepEqual(await listRemoteMarcelProfiles(ssh), ['default', 'bob', 'dixie', 'goose', 'rambo'])
  assert.equal(
...
test('listRemoteMarcelProfiles rejects a hostile MARCEL_HOME', async () => {
  const ssh = fakeSsh([[/MARCEL_HOME/, '/tmp/x; echo pwned\n']])

  await assert.rejects(
    () => listRemoteMarcelProfiles(ssh),
    (err: any) => {
...
test('locateMarcel prefers the explicit profile path when executable', async () => {
  const ssh = fakeSsh([[/\[ -x .*\/opt\/marcel/, 'OK']])
  assert.equal(await locateMarcel(ssh, '/opt/marcel'), '/opt/marcel')
})

test('locateMarcel throws (no silent fallback) when an EXPLICIT path is not executable', async () => {
  // command -v WOULD find a different install, but an explicit path must not
  // silently fall back to it — that is the "connected to the wrong marcel" bug.
  const ssh = fakeSsh([
    [/command -v marcel/, '/home/u/.local/bin/marcel\n'],
    [/\[ -x .*\.local\/bin\/marcel/, 'OK']
  ])

  await assert.rejects(
    () => locateMarcel(ssh, '/bad/path/marcel'),
    (err: any) => {
      assert.equal(err.kind, 'marcel-not-found')
      assert.match(err.message, /\/bad\/path\/marcel/)
...
test('locateMarcel falls back to the login-shell command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v marcel/, '/home/u/.local/bin/marcel\n'],
    [/\[ -x .*\.local\/bin\/marcel/, 'OK']
  ])

  assert.equal(await locateMarcel(ssh, ''), '/home/u/.local/bin/marcel')
})

test('locateMarcel preserves an installer wrapper instead of resolving its interpreter', async () => {
  // install.sh venv mode writes: exec "$MARCEL_BIN" "$MARCEL_ENTRYPOINT" "$@",
  // where $MARCEL_BIN is the venv python. The old canonicalization returned
  ...
  const ssh = fakeSsh([
    ...
    [/python3 -c/, '/home/u/.marcel/marcel-agent/venv/bin/python\n']
  ])

  assert.equal(await locateMarcel(ssh, ''), '/home/u/.local/bin/marcel')
  assert.ok(
    !ssh.calls.some(cmd => cmd.includes('python3 -c')),
    'locateMarcel must not shell out to a python3 parser to rewrite the launcher'
  )
})

test('locateMarcel returns an explicit remoteMarcelPath unchanged', async () => {
  // The override half of #74411: an explicit remoteMarcelPath pointing at a
  // wrapper was also canonicalized to its interpreter, so overriding to
  // ~/.local/bin/marcel changed nothing for affected users.
  const ssh = fakeSsh([
    [/\[ -x .*\.local\/bin\/marcel/, 'OK'],
    [/python3 -c/, '/home/u/.marcel/marcel-agent/venv/bin/python\n']
  ])

  assert.equal(await locateMarcel(ssh, '~/.local/bin/marcel'), '~/.local/bin/marcel')
  assert.ok(!ssh.calls.some(cmd => cmd.includes('python3 -c')), 'an explicit remoteMarcelPath must never be rewritten')
})

test('locateMarcel falls back to ~/.local/bin/marcel when the login-shell probe misses', async () => {
  ...
  assert.equal(await locateMarcel(ssh, ''), '~/.local/bin/marcel')
})

test('locateMarcel tries the conventional venv path last', async () => {
  const ssh = fakeSsh([[/\[ -x .*venv\/bin\/marcel/, 'OK']])
  assert.equal(await locateMarcel(ssh, ''), '~/.marcel/marcel-agent/venv/bin/marcel')
})

test('locateMarcel throws a marcel-not-found error with an install hint', async () => {
...
test('locateMarcel uses a login shell for the command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v marcel/, '/x/marcel'],
    [/\[ -x/, 'OK']
  ])

  await locateMarcel(ssh, '')
  assert.ok(
...
test('ownership paths are isolated by ownership ID and spawn nonce', () => {
  assert.equal(ownershipDirectory(OWNERSHIP_ID), `~/.marcel/desktop-ssh/${OWNERSHIP_ID}`)
  assert.equal(lockfilePath(OWNERSHIP_ID), `~/.marcel/desktop-ssh/${OWNERSHIP_ID}/backend.lock.json`)
  assert.equal(spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE), `~/.marcel/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.log`)
})
...
test('metadata and process proof transport failures remain indeterminate', async () => {
  ...
  await assert.rejects(
    () => pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, failure]]), 5, SPAWN_NONCE, '/x/marcel'),
    (error: any) => error.kind === 'transient-transport-error'
  )
})

test('pidIsOurDashboard requires the exact serve ownership nonce', async () => {
  const ours = `/x/marcel serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'OWNED\n']]), 5, SPAWN_NONCE, '/x/marcel'), true)
  assert.equal(
...
test('pidIsOurDashboard accepts the venv entrypoint an installer wrapper execs into', async () => {
  ...
  assert.equal(
    await pidIsOurDashboard(ssh, 5, SPAWN_NONCE, '~/.local/bin/marcel', '/Users/cd9c/.marcel', OWNERSHIP_ID, 'ops'),
    true
  )
  assert.match(ownershipProbe, /marcel-agent.*venv.*bin.*marcel/)
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    const temp = await mkdtemp(path.join(os.tmpdir(), 'marcel wrapper ownership '))
    ...
    const entrypoint = path.join(installDir, 'marcel')
    const launcher = path.join(temp, 'marcel launcher')
    ...
    try {
      ...
      assert.equal(
        await pidIsOurDashboard(ssh, child.pid, SPAWN_NONCE, launcher, '/unrelated/marcel-home', OWNERSHIP_ID, 'ops'),
        true
      )
      assert.equal(
        await pidIsOurDashboard(
...
test('buildSpawnCommand is headless serve, detached, token not in argv', () => {
  const cmd = buildSpawnCommand('/x/marcel', 'work', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  ...
  assert.ok(!cmd.includes('MARCEL_DASHBOARD_SESSION_TOKEN'), 'token env var must not appear')
})
...
test('buildSpawnCommand atomically reserves the ownership slot through spawn and lock publication', () => {
  const cmd = buildSpawnCommand('/x/marcel', 'work', {
    marcelHome: '~/.marcel',
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    ownershipId: OWNERSHIP_ID,
  ...
  assert.ok(cmd.includes('.marcel-update-in-progress.mutex'))
  assert.match(cmd, /fcntl\.flock\(fd,fcntl\.LOCK_EX\)/)
  assert.match(cmd, /os\.O_CLOEXEC/)
  assert.match(
    cmd,
    /subprocess\.run\(\["sh","-c",payload,"marcel-update-mutex",str\(fd\)\],pass_fds=\(fd,\),check=False\)/
  )
  assert.doesNotMatch(cmd, /os\.set_inheritable\(fd,True\)/)
  assert.match(cmd, /marcel-update-child "\$1"/)
...
test.skipIf(process.platform === 'win32')('detached backend does not inherit the update mutex descriptor', async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'marcel-update-mutex-'))
  const marcelPath = path.join(directory, 'marcel')
...
for fd in /proc/$$/fd/*; do
  ...
  case "$target" in
    *marcel-update-in-progress.mutex) printf '%s\\n' "$target" >> ${reportPath} ;;
  esac
done
`,
      { mode: 0o700 }
    ...
    const command = buildSpawnCommand(marcelPath, '', {
      marcelHome: path.join(directory, 'home'),
      logPath
    })
...
test('spawnRemoteDashboard always spawns serve (legacy dashboard path removed)', async () => {
  ...
  await spawnRemoteDashboard(ssh, { marcelPath: '/x/marcel', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID })
...
test('READY_RE accepts both serve and dashboard sentinels', () => {
  assert.equal(READY_RE.exec('MARCEL_BACKEND_READY port=4321')?.[1], '4321')
  assert.equal(READY_RE.exec('MARCEL_DASHBOARD_READY port=8765')?.[1], '8765')
})
...
test('spawnRemoteDashboard rejects when no pid is returned', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { marcelPath: '/x/marcel', profile: '', token: 't', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('scrapeReadyPort reads only the named spawn log', async () => {
  ...
  const ssh = fakeSsh([[/cat/, 'some noise\nMARCEL_DASHBOARD_READY port=51234\n']])
...
test('connect() spawns fresh when there is no lockfile, adopts the served token', async () => {
  const ssh = fakeSsh([
    ...
    [/setsid/, '777\n'],
    [/kill -0 777/, 'ALIVE'],
    [/cat .*\.log/, 'MARCEL_DASHBOARD_READY port=51999\n']
  ])
...
test('managed SSH maps a local scope to a different non-default remote profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '778\n'],
    [/kill -0 778/, 'ALIVE'],
    [/cat .*\.log/, 'MARCEL_BACKEND_READY port=52000\n']
  ])
  ...
  assert.match(spawn, /\.marcel\/desktop-ssh\/[0-9a-f]{32}\/[0-9a-f]{16}\.token/)
...
test('connect() respawns when the requested remote profile differs from the lockfile profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/kill 333/, ''],
    [/--version/, 'Marcel Agent v0.18.2\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/setsid/, '890\n'],
    [/kill -0 890/, 'ALIVE'],
    [/cat .*\.log/, 'MARCEL_DASHBOARD_READY port=52050\n']
  ])
...
test('connect() respawns when the lockfile marcelPath differs from the resolved path', async () => {
  const reuseToken = 'stored-token'
  const lock = ownedLock({ marcelPath: '/old/stale/marcel', tokenFingerprint: fingerprintToken(reuseToken) })
  ...
  const result = await connect(
    connectDeps(ssh, { reuseToken, remoteMarcelPath: '/new/marcel', adoptServedToken: async () => 'fresh' })
  )
...
test('connect() respawns when the lockfile protocolVersion is incompatible', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '901\n'],
    [/kill -0 901/, 'ALIVE'],
    [/cat .*\.log/, 'MARCEL_DASHBOARD_READY port=44100\n']
  ])
...
test('connect() fresh spawn writes marcelHome + protocolVersion into the lockfile', async () => {
  const writes: string[] = []
  ...
  const ssh = fakeSsh([
    ...
    [/\[ -x/, 'OK'],
    [/cat .*lock\.json/, ''], // no lockfile
    [/MARCEL_HOME/, '/home/alice/.marcel\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/printf '%s\\n'/, ''],
    [/setsid/, '700\n'],
    [/kill -0 700/, 'ALIVE'],
    [/cat .*\.log/, 'MARCEL_DASHBOARD_READY port=45500\n'],
    [
      /printf '%s' '/,
      c => {
        writes.push(c)
  ...
  assert.match(lockWrite, /"marcelHome":"\/home\/alice\/\.marcel"/)
})
...
test('connect() respawns when the lockfile pid is dead (killed dashboard)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '888\n'],
    [/kill -0 888/, 'ALIVE'],
    [/cat .*\.log/, 'MARCEL_DASHBOARD_READY port=42000\n']
  ])
...
test('connect() respawns when the dashboard is wedged (alive pid, probe fails)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '999\n'],
    [/kill -0 999/, 'ALIVE'],
    [/cat .*\.log/, 'MARCEL_DASHBOARD_READY port=43000\n']
  ])
...
test('validateRemotePath accepts absolute POSIX paths', () => {
  assert.doesNotThrow(() => validateRemotePath('/usr/bin/marcel'))
  assert.doesNotThrow(() => validateRemotePath('/home/user/.marcel/marcel-agent/venv/bin/marcel'))
})

test('validateRemotePath accepts ~/ prefix paths', () => {
  assert.doesNotThrow(() => validateRemotePath('~/bin/marcel'))
  assert.doesNotThrow(() => validateRemotePath('~/.marcel/logs/desktop-ssh.log'))
  assert.doesNotThrow(() => validateRemotePath('~'))
})

test('validateRemotePath accepts paths with spaces and quotes', () => {
  assert.doesNotThrow(() => validateRemotePath('/home/user/my project/marcel'))
...
test('validateRemotePath rejects relative paths', () => {
  assert.throws(() => validateRemotePath('marcel'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('./bin/marcel'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('../etc/passwd'), /absolute|relative/i)
})

test('validateRemotePath rejects NUL and newline', () => {
  assert.throws(() => validateRemotePath('/usr/bin/marcel\x00'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/marcel\n'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/marcel\r'), /unsafe/i)
})

test('validateRemotePath preserves shell metacharacters as path data', () => {
  for (const p of ['/usr/$(whoami)/marcel', '/usr/`id`/marcel', '/usr/a;b|c&d<e>f']) {
...
test('expandRemotePath expands ~/ to "$HOME"/', () => {
  const result = expandRemotePath('~/.marcel/logs/desktop-ssh.log')
...
test('expandRemotePath returns quoted absolute paths unchanged', () => {
  const result = expandRemotePath('/usr/local/bin/marcel')
  assert.ok(result.includes('/usr/local/bin/marcel'))
  assert.ok(!result.includes('eval'))
})

test('expandRemotePath preserves spaces as data', () => {
  const result = expandRemotePath('/home/user/my project/marcel')
...
test('buildSpawnCommand does not embed the token in the command string', () => {
  ...
  assert.ok(!cmd.includes('MARCEL_DASHBOARD_SESSION_TOKEN'), 'env var name must not appear')
})

test('buildSpawnCommand includes --ssh-session-token-file when tokenFilePath is provided', () => {
  const cmd = buildSpawnCommand('/x/marcel', 'work', {
    tokenFilePath: `~/.marcel/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.token`,
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    spawnNonce: SPAWN_NONCE
  })

  assert.match(cmd, /--ssh-session-token-file/)
  assert.match(cmd, /\.marcel\/desktop-ssh\//)
})

test('buildSpawnCommand always uses serve, never dashboard', () => {
  const cmd = buildSpawnCommand('/x/marcel', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /serve --isolated/)
  assert.doesNotMatch(cmd, /\bdashboard\b/)
  assert.doesNotMatch(cmd, /--skip-build/)
...
test('buildSpawnCommand raises the SSH child file limit before execing Marcel', () => {
  const cmd = buildSpawnCommand('/x/marcel', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /ulimit -n 65536 2>\/dev\/null \|\| true; exec env MARCEL_DESKTOP=1/)
...
test('spawnRemoteDashboard removes a token file when upload reporting fails', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { marcelPath: '/x/marcel', profile: '', token: 'tok', ownershipId: OWNERSHIP_ID }),
    /channel closed/
  )
...
test('spawnRemoteDashboard fails with update-required when remote lacks --ssh-session-token-file', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { marcelPath: '/x/marcel', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('readLockfile treats a log path outside the exact ownership and spawn path as skew', async () => {
  const lock = ownedLock({ logPath: '~/.marcel/desktop-ssh/other.log' })
...
test('cleanupStale never deletes a lock-supplied unexpected log path', async () => {
  ...
  await cleanupStale(ssh, OWNERSHIP_ID, ownedLock({ logPath: '~/.marcel/unrelated.log' }))
...
test('pidIsOurDashboard requires an exact nonce option value', async () => {
  const prefix = `/x/marcel serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}ff`
  const suffix = `/x/marcel serve --isolated --ssh-owner-nonce xx${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/marcel'), false)
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/marcel'), false)
})
...
test('remote SSH ownership capability requires both secure bootstrap flags', async () => {
  ...
  assert.equal(await remoteSupportsSshOwnership(supported, '/x/marcel'), true)
  ...
  assert.equal(await remoteSupportsSshOwnership(unsupported, '/x/marcel'), false)
})
