 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Marcel dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Marcel install on the remote (login-shell probe),
 ...
 *     `MARCEL_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForMarcel in.
 *
 * The minted MARCEL_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^MARCEL_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.marcel/desktop-ssh'
...
// Resolve the remote marcel executable. An EXPLICIT path is honored strictly
...
async function locateMarcel(ssh, remoteMarcelPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The marcel binary or wrapper script
  ...
  if (remoteMarcelPath) {
    if (await isExecutable(remoteMarcelPath)) {
      return resolveLauncher(remoteMarcelPath)
    }

    const err: any = new Error(
      `The Marcel path you set is not an executable on the remote host: "${remoteMarcelPath}". ` +
        'Check the path (it must be the full path to the `marcel` binary on the remote, e.g. ' +
        '~/marcel-agent/.venv/bin/marcel), or clear it to auto-detect.'
    )

    err.kind = 'marcel-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v marcel')}`)).trim()
  ...
  candidates.push('~/.local/bin/marcel')
  candidates.push('/usr/local/bin/marcel')
  candidates.push('~/.marcel/marcel-agent/venv/bin/marcel')
...
// Probe the resolved binary's version string (first line of `<marcel> --version`,
// e.g. "Marcel Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH marcel a
// connection uses, so a stale/unexpected install is visible.
async function probeMarcelVersion(ssh, marcelPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(marcelPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Marcel Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The MARCEL_HOME the remote dashboard will use (explicit env wins, else
// ~/.marcel). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteMarcelHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${MARCEL_HOME:-$HOME/.marcel}"')).trim().split('\n').pop()

    return out || '~/.marcel'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Marcel home.')
...
marker=home/'.marcel-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing Marcel checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, marcelHome) {
  const home = assertSafeRemoteHome(marcelHome)
  let observation = ''
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote Marcel install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote Marcel update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote Marcel update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteMarcelProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteMarcelHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Marcel profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Marcel home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'marcelPath', 'marcelHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !marcelPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(marcelPath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/marcel`, leaving neither
      // launcher nor MARCEL_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `marcel_home=os.path.expanduser(${shq(marcelHome)}) if ${shq(marcelHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if marcel_home:\n' +
      ' expected_entries.add(os.path.join(marcel_home,"marcel-agent","venv","bin","marcel"))\n' +
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.marcelPath)
  const expectedHome = lock.marcelHome ? expandRemotePath(lock.marcelHome) : "''"
  ...
  const executableMatch = lock.marcelHome
...
function lockMatchesManagedUpdateScope(lock, expected) {
  return Boolean(
    ...
    lock.marcelPath === expected.marcelPath &&
    lock.marcelHome === expected.marcelHome
  )
}
...
expected_path=os.path.expanduser(${py(lock.marcelPath)})
marcel_home=os.path.expanduser(${py(lock.marcelHome)})
expected_entries={expected_path,os.path.join(marcel_home,"marcel-agent","venv","bin","marcel")}
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"marcel-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(marcelPath, profile, opts: any = {}) {
  const marcel = expandRemotePath(marcelPath)
  ...
  const marker = expandRemotePath(`${remoteInstallRoot(opts.marcelHome || '~/.marcel')}/.marcel-update-in-progress`)

  const updateMutex = expandRemotePath(
    `${remoteInstallRoot(opts.marcelHome || '~/.marcel')}/.marcel-update-in-progress.mutex`
  )
  ...
  const dashCmd =
    ...
    `exec env MARCEL_DESKTOP=1 ${marcel} ${profileArgs}${subCmd}`

  const detachedShell = `eval "exec $1>&-"; ${dashCmd} </dev/null >> ${logPath} 2>&1 & echo $!`
  const detachedSpawn = `child=$("$(command -v setsid || echo nohup)" sh -c ${shq(detachedShell)} marcel-update-child "$1" & echo $!)`
...
async function remoteSupportsSshOwnership(ssh, marcelPath) {
  const marcel = expandRemotePath(marcelPath)

  const out = await ssh.exec(
    `help="$(${marcel} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(
  ssh,
  { marcelPath, profile, token, ownershipId, marcelHome = '~/.marcel', assertInstallClear = async () => {} }
) {
  if (!(await remoteSupportsSshOwnership(ssh, marcelPath))) {
    const err: any = new Error(
      'The remote Marcel install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Marcel on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(marcelPath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        marcelHome,
 ...
 * opened SshConnection, forward/pickLocalPort/waitForMarcel, a token-gated
...
async function connect(deps) {
  ...
  const marcelHome = await probeRemoteMarcelHome(ssh)
  await assertRemoteInstallUpdateClear(ssh, marcelHome)
  const marcelPath = await locateMarcel(ssh, remoteMarcelPath)
  log(`located marcel at ${marcelPath}`)
  const marcelVersion = await probeMarcelVersion(ssh, marcelPath)

  if (marcelVersion) {
    log(`remote marcel version: ${marcelVersion}`)
  }
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this Marcel Desktop build (${lock.reason}). ` +
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.marcelPath === marcelPath &&
      lock.marcelHome === marcelHome
  ...
  const spawned = await spawnRemoteDashboard(ssh, {
    ...
    ownershipId,
    marcelHome,
    assertInstallClear: () => assertRemoteInstallUpdateClear(ssh, marcelHome)
  })
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForMarcel(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
