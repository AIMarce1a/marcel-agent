...
test('MARCEL_GIT_BASH_PATH override takes precedence', () => {
  const result = findGitBash({
    isWindows: true,
    env: { MARCEL_GIT_BASH_PATH: 'D:\\CustomGit\\bin\\bash.exe' },
    fileExists: yes,
    findOnPath: () => null
  })
...
test('MARCEL_GIT_BASH_PATH invalid path falls through to candidates', () => {
  const env = {
    MARCEL_GIT_BASH_PATH: 'X:\\Missing\\bash.exe',
...
test('MARCEL_GIT_BASH_PATH empty string is ignored', () => {
  const result = findGitBash({
    isWindows: true,
    env: { MARCEL_GIT_BASH_PATH: '', LOCALAPPDATA: '' },
    fileExists: no,
