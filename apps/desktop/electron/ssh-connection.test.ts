...
test('redactSecrets scrubs the spawn-time session token env var', () => {
  const line = 'setsid env MARCEL_DASHBOARD_SESSION_TOKEN=abc123deadbeef MARCEL_DESKTOP=1 marcel dashboard'
  const out = redactSecrets(line)
  assert.ok(!out.includes('abc123deadbeef'))
  assert.match(out, /MARCEL_DASHBOARD_SESSION_TOKEN=<redacted>/)
  // non-secret env vars are preserved
  assert.match(out, /MARCEL_DESKTOP=1/)
})
...
test('redactSecrets scrubs Authorization and X-Marcel-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-Marcel-Session-Token: hdr_888'), /X-Marcel-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-Marcel-Session-Token: hdr_888').includes('hdr_888'))
})
...
test('controlSocketPath default base stays under sun_path even with the temp-listener suffix', () => {
  ...
  const p = controlSocketPath('marcel', 'remote-build-server', 22) // no baseDir → default
...
test('buildExecArgs ends with host then the remote command', () => {
  ...
  const args = buildExecArgs(conn, 'command -v marcel', 15000)
  assert.equal(args[args.length - 1], 'command -v marcel')
...
test('close() removes the control socket when -O exit fails', async () => {
  const dir = path.join(os.tmpdir(), `marcel-ssh-close-${process.pid}-${Date.now()}`)
...
test('open() creates the control-socket directory if it does not exist', async () => {
  const dir = path.join(os.tmpdir(), `marcel-ssh-test-${process.pid}-${Date.now()}`)
