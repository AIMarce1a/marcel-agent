...
test('Windows spawn holds the update mutex across marker check and helper spawn', () => {
  const command = atomicWindowsSpawnCommand({
    marcelHome: 'C:\\Users\\andre\\.marcel',
    python: 'C:\\Users\\andre\\.marcel\\python.exe'
  })

  const encoded = command.match(/-EncodedCommand\s+([^\s]+)$/)?.[1]
  const script = encoded ? Buffer.from(encoded, 'base64').toString('utf16le') : ''
  assert.match(script, /\.marcel-update-in-progress/)
...
test('Windows spawn publishes the initial ownership record before releasing the mutex', () => {
  const command = atomicWindowsSpawnCommand(
    ...
    },
    {
      ...
      profile: 'default',
      marcelPath: 'C:\\Marcel\\marcel.exe',
      marcelHome: 'C:\\Users\\andre\\.marcel',
...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command marcel.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          marcelHome: 'C:\\Users\\alice\\.marcel',
          marcelPath: 'C:\\Marcel\\marcel.exe',
          python: 'C:\\Marcel\\python.exe'
        })
      }

      if (script.includes('.marcel-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('marcel_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\\.marcel\\profiles\\research')
  assert.match(script, /\.marcel-update-in-progress/)
...
test('Windows probe validates Marcel and Python topology before selection', async () => {
  let script = ''
  ...
  const fallbackJoin = script.indexOf('Join-Path $marcelHome')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('Marcel is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /Marcel is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\Marcel's\\python.exe" }, 'inspect', [
    'C:\\x y\\marcel.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'marcel_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /Marcel''s/)
  assert.match(script, /C:\\x y\\marcel\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { marcelPath: lock.marcelPath, marcelHome: lock.marcelHome }
