...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('marcel:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('marcel:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('marcel:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('marcel:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('marcel:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('marcel:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('marcel:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('marcel:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('marcel:pet-overlay:control', payload)
  })
}
