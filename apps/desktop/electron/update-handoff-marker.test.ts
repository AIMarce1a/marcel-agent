...
function sandbox(tag: string) {
  const home = fs.mkdtempSync(path.join(os.tmpdir(), `marcel-handoff-marker-${tag}-`))
  const installRoot = path.join(home, 'marcel-agent')
...
function markerStartedAt(home: string): number {
  const [, startedAt] = fs.readFileSync(path.join(home, '.marcel-update-in-progress'), 'utf8').split('\n')
...
function runPosix(installRoot: string, startedAt?: string) {
  ...
  if (startedAt === undefined) {
    delete env.MARCEL_UPDATE_STARTED_AT
  } else {
    env.MARCEL_UPDATE_STARTED_AT = startedAt
  }
...
function assertScriptHandoff(run: (installRoot: string, startedAt?: string) => ReturnType<typeof spawnSync>) {
  ...
  fs.writeFileSync(path.join(refreshed.home, '.marcel-update-in-progress'), '999999\n1\n')
