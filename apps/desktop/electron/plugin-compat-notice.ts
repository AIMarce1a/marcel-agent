 ...
 *
 * The Python side (marcel_cli/plugin_compat.py) statically scans the user's enabled external plugins on
 * every CLI/gateway/TUI start and writes MARCEL_HOME/.plugin-compat-report.json when any plugin imports a
...
export function readReport(marcelHome: string): PluginCompatReport | null {
  try {
    const raw = fs.readFileSync(path.join(marcelHome, REPORT_FILE), 'utf8')
...
export function pendingNotice(marcelHome: string, userData: string): PendingNotice | null {
  const report = readReport(marcelHome)
