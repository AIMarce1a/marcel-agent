...
beforeEach(async () => {
  dir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'marcel-download-fs-'))
})
...
test('a pre-open EEXIST collision leaves the seeded temp file and the destination untouched', async () => {
  const dest = path.join(dir, 'report.bin')
  const pinnedTemp = path.join(dir, '.marcel-download-pinned.part')
  ...
  assert.deepEqual(await listing(), ['.marcel-download-pinned.part', 'report.bin'])
})
