...
export function registerHudIpc({
  ...
  ipcMain.on('marcel:hud:native-drag', event => {
    event.returnValue = hudWindowing().move === 'native-drag'
  })

  ipcMain.on('marcel:hud:windowing', event => {
  ...
  ipcMain.on('marcel:hud:workspace-transfer', (event, transferring) => {
  ...
  ipcMain.handle('marcel:hud:open', async (_event, request) => {
    openHudWindow(
  ...
  ipcMain.handle('marcel:hud:frost', (_event, showing) => {
  ...
  // reaches the bar.
  ipcMain.on('marcel:hud:ignore-mouse', (_event, ignore) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('marcel:hud:begin-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('marcel:hud:end-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('marcel:hud:move-by', (event, delta) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('marcel:hud:set-bounds', (event, bounds) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('marcel:hud:reset-layout', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('marcel:hud:session', (event, sessionId) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('marcel:hud:close', async () => {
    closeHudWindow()
