// Unit tests for the pure Windows `marcel` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvMarcelCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `marcel` shim shadows
//      the real marcel.cmd/marcel.exe.
...
//   3. resolveVenvMarcelCommand() — must probe the venv python via
//      canImportMarcelCli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvMarcel: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvMarcel: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvMarcel: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvMarcel: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvMarcel: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvMarcel: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvMarcelCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportMarcelCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    marcelHome: '/fake/marcel-home',
...
test('resolveVenvMarcelCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvMarcelCommand('/root/venv/Scripts/marcel.exe', [], deps), null)
})

test('resolveVenvMarcelCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvMarcelCommand('/root/venv/Scripts/marcel.cmd', [], deps), null)
})

test('resolveVenvMarcelCommand: returns null when the basename is not marcel/marcel.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvMarcelCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvMarcelCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvMarcelCommand('/root/venv/bin/marcel.exe', [], deps), null)
})

test('resolveVenvMarcelCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvMarcelCommand('/root/venv/Scripts/marcel.exe', [], deps), null)
})

test('resolveVenvMarcelCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportMarcelCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvMarcelCommand('/root/venv/Scripts/marcel.exe', ['serve'], deps)
...
test('resolveVenvMarcelCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvMarcelCommand('/root/venv/Scripts/marcel.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'marcel_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvMarcelCommand: is case-insensitive on marcel.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvMarcelCommand('/root/venv/Scripts/MARCEL.EXE', [], deps))
  assert.ok(resolveVenvMarcelCommand('/root/venv/SCRIPTS/marcel.exe', [], deps))
})
