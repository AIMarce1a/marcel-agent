 ...
 * console. It writes MARCEL_HOME/.marcel-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Marcel within 30 minutes.
...
export function handoffResultPath(marcelHome: string): string {
  return path.join(marcelHome, '.marcel-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(marcelHome)
  let raw: string
