...
// hardening, MARCEL_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('marcel:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('marcel:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

  // Reveal a path in the OS file manager (Finder / Explorer / Files).
  ipcMain.handle('marcel:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('marcel:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<MARCEL_HOME>/desktop-plugins`,
  // resolved from the main-process MARCEL_HOME (see resolveMarcelHome) — NOT from
  // the connected backend. A remote backend reports its own `marcel_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    // profiles/<name>/, matching the profile-scoped marcel_home the backend
    // reported before this resolver existed. 'default'/unset pins the global root.
    const profile = readActiveDesktopProfile()
    const base = profile && profile !== 'default' ? path.join(marcelHome, 'profiles', profile) : marcelHome
  ...
  ipcMain.handle('marcel:fs:desktopPluginsRoot', async () => localPluginsRoot('desktop-plugins'))

  // The LOCAL logs root (`<MARCEL_HOME>/logs`, profile-aware) — the error
  // card's "Open Logs" action reveals agent.log/gateway.log without the user
  // knowing where MARCEL_HOME lives. Same Electron-local resolution as the
  // plugin roots: valid in every connection mode, created on demand.
  ipcMain.handle('marcel:fs:logsRoot', async () => localPluginsRoot('logs'))

  // The LOCAL agent-plugin root (`<MARCEL_HOME>/plugins`), same Electron-local
  ...
  ipcMain.handle('marcel:fs:agentPluginsRoot', async () => localPluginsRoot('plugins'))

  ipcMain.handle('marcel:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('marcel:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('marcel:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('marcel:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('marcel:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
