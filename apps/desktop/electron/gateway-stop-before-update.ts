 ...
 *
 * So we delegate to `marcel gateway stop --all`: the CLI discovers every
 ...
 * force-kills survivors — the same logic `marcel update`'s
...
export function stopGatewayBeforeUpdate(
  marcelCliPath: string,
  marcelHome: string,
  deps: StopGatewayBeforeUpdateDeps = {}
): boolean {
  return runGatewayLifecycleCommand(marcelCliPath, ['gateway', 'stop', '--all'], deps)
}
 ...
 */
export function startGatewaysAfterUpdateAbort(marcelCliPath: string, deps: StopGatewayBeforeUpdateDeps = {}): boolean {
  return runGatewayLifecycleCommand(marcelCliPath, ['gateway', 'start', '--all'], deps)
}

function runGatewayLifecycleCommand(marcelCliPath: string, args: string[], deps: StopGatewayBeforeUpdateDeps): boolean {
  ...
  if (deps.spy) {
    deps.spy(marcelCliPath, args)
  }

  if (!existsSync(marcelCliPath)) {
    return false
  }

  try {
    exec(marcelCliPath, args, {
