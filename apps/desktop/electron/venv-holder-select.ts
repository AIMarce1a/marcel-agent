 ...
 *
 * The pre-update handoff kills Marcel-OWNED venv daemons (the memory plugin's
 * hindsight daemon) so the updater never races a mapped shim. External
 * holders (a user terminal running `marcel`, unrelated scripts) must NOT be
...
/**
 * True when a process is a Marcel-owned venv daemon: its exe lives under
 ...
 * spawns DETACHED — it outlives Marcel and holds venv shims mapped).
 */
export function isMarcelOwnedVenvDaemon(
