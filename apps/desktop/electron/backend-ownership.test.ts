...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'marcel serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/marcel serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m marcel_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/marcel --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Marcel Runtime\\marcel.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('marcel chat --query serve'), false)
