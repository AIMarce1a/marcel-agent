  ...
  return {
    ...
    async reapOrphans(): Promise<number[]> {
      ...
      for (let i = 0; i < entries.length; i += 1) {
        ...
        // if it reaches reapOrphans (see main.ts startMarcel + #87295).
...
export function backendCommandMatches(command: unknown): boolean {
  return /(?:^|[\s/\\"])(?:marcel(?:\.exe)?|marcel_cli\.main|marcel_cli[/\\]main\.py)"?(?:\s+(?:--profile|-p)\s+\S+)?\s+(?:serve|dashboard)(?:\s|$)/i.test(
    String(command ?? '')
  )
}
