 ...
 * `iter_marcel_node_dirs()` in marcel_constants.py, which the Electron main
 * process cannot import.
 */
function marcelManagedNodePathEntries(
  marcelHome,
  ...
  const root = pathModule.join(marcelHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const marcelNodeDirs = marcelManagedNodePathEntries(marcelHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([marcelNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeMarcelHomeRoot(marcelHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!marcelHome) {
    return marcelHome
  }

  const resolved = pathModule.resolve(String(marcelHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). marcel_bootstrap sets this inside the child too, but only
