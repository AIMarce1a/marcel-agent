...
// registry, and the marcel:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // MARCEL_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.MARCEL_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip Marcel' TUI into
    ...
    env.TERM_PROGRAM = 'Marcel'
    env.TERM_PROGRAM_VERSION = app.getVersion()

    // Let a marcel/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from MARCEL_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.MARCEL_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `marcel:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('marcel:terminal:start', async (event, payload = {}) => {
  ...
  ipcMain.handle('marcel:terminal:attach', (event, id) => {
  ...
  ipcMain.handle('marcel:terminal:write', (_event, id, data) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('marcel:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('marcel:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('marcel:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
