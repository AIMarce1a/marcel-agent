/**
 * windows-marcel-path.ts
 *
 * Pure, dependency-injected pieces of Windows `marcel` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvMarcelCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `marcel` shim shadowed the real
 *      marcel.cmd/marcel.exe; the shim then failed the --version probe and
 ...
 *      and Marcel launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvMarcelCommand() — unwrapWindowsVenvMarcelCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `marcel`) must
 * not shadow `marcel.cmd`/`marcel.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the Marcel launcher that drives `marcel update`.
...
export interface ResolveVenvMarcelCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportMarcelCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `marcel`/`marcel.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/marcel(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m marcel_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportMarcelCli(). A
 * venv whose update died mid-`pip install` still has python.exe + marcel.exe
 ...
 *
 * Returns null when `command` is not a venv marcel shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvMarcelCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvMarcelCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^marcel(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Marcel at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Marcel Python at ${python}`,
    command: python,
