// Launching the Marcel TUI in the user's OWN terminal emulator.
//
...
// `marcel --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Marcel runtime is often a venv Python invoked
//     as `python -m marcel_cli.main`, not a `marcel` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Marcel
