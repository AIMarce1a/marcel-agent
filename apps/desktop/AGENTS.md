...
How to build Marcel Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Marcel surface can also
...
- A **runtime home change** (switching the underlying `MARCEL_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Marcel — do not assume
