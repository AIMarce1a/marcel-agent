...
<p align="center">
  <a href="https://github.com/AdMind-ai/marcel-agent/releases"><img src="https://img.shields.io/badge/Download-macOS%20%C2%B7%20Windows%20%C2%B7%20Linux-FFD700?style=for-the-badge" alt="Download"></a>
  <a href="https://hermes-agent.nousresearch.com/docs/"><img src="https://img.shields.io/badge/Docs-marcel--agent.nousresearch.com-FFD700?style=for-the-badge" alt="Documentation"></a>
  <a href="https://discord.gg/NousResearch"><img src="https://img.shields.io/badge/Discord-5865F2?style=for-the-badge&logo=discord&logoColor=white" alt="Discord"></a>
  <a href="https://github.com/AdMind-ai/marcel-agent/blob/main/LICENSE"><img src="https://img.shields.io/badge/License-MIT-green?style=for-the-badge" alt="License: MIT"></a>
</p>

**The native desktop app for [Marcel Agent](../../README.md) — the self-improving AI agent from [Nous Research](https://nousresearch.com).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.

<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other Marcel surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to Marcel and hear it back.</td></tr>
...
### Install with Marcel (recommended)

Already have the Marcel CLI? Just run:

```bash
marcel desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Marcel gateway or install Marcel locally. Local onboarding then walks you through choosing a provider and model.

### Prebuilt installers

Prebuilt installers are built and distributed via [the Marcel Desktop website.](https://hermes-agent.nousresearch.com/).
...
```bash
# throwaway MARCEL_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Marcel Agent runtime into `MARCEL_HOME`
(`~/.marcel`, or `%LOCALAPPDATA%\marcel` on Windows), using the same layout as a
CLI install.
...
- **Marcel Agent** runs as a headless `marcel serve` process and exposes the
...
1. `MARCEL_DESKTOP_MARCEL_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `MARCEL_DESKTOP_MARCEL`, or `marcel` on `PATH`
5. a system Python that can import the Marcel runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Marcel
...
screen offers **Connect to existing Marcel** before starting the local installer.
...
terminal commands, and file operations run against the remote Marcel host, not
...
transport- or Marcel-managed header names such as `Authorization`, `Cookie`,
`Host`, `Origin`, `Referer`, and `X-Marcel-Session-Token`.
...
Boot logs land in `MARCEL_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\marcel\marcel-agent\.marcel-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\marcel\marcel-agent\venv"
```

> The default Marcel home on Windows is `%LOCALAPPDATA%\marcel`. Set the `MARCEL_HOME` env var if you've relocated it.
