 ...
 * product name (`Marcel`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Marcel'
 *
 * This is a hard failure with no obvious cause for the user — `marcel desktop`
 ...
 * (the mac-unpacked Marcel.app bundle) and Windows (win-unpacked), so we
 ...
 * Marcel.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `marcel desktop --build-only` (marcel_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Marcel.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Marcel'}.exe`
