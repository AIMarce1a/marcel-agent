...
//   · its own MARCEL_HOME      → its own backend + sessions, no shared state.
//   · its own --remote-debugging-port → a private CDP endpoint.
//   · MARCEL_DESKTOP_BOOT_FAKE=1 → deterministic boot overlay.
...
// Seed an isolated MARCEL_HOME with just enough config (NOT sessions) so the
// spawned instance reaches an empty chat view instead of the onboarding wizard.
// A separate MARCEL_HOME dir means a separate gateway lock — no collision with
...
export async function startIsolatedInstance({
  ...
  const home = marcelHome ?? mkTemp('marcel-perf-home-')
  const userData = userDataDir ?? mkTemp('marcel-perf-ud-')
  const devUrl = prod ? null : `http://127.0.0.1:${devPort}`

  if (seedConfig && !marcelHome) {
    seedConfigFrom(join(homedir(), '.marcel'), home)
  }
  ...
  try {
    ...
    // MARCEL_HOME (backend + sessions). No DEV_SERVER env in prod → dist load.
    const electronBin = require('electron')
    // NB: do NOT set MARCEL_DESKTOP_BOOT_FAKE here — it injects artificial
    ...
    const env = {
      ...
      MARCEL_DESKTOP_CDP_PORT: String(port),
      XCURSOR_SIZE: '24'
    }

    if (devUrl) {
      env.MARCEL_DESKTOP_DEV_SERVER = devUrl
    }
...
export async function coldStartSamples({ runs = 3, port = 9222, devPort = 5174, prod = false, warm = true } = {}) {
  ...
  if (warm) {
    ...
    const home = mkdtempSync(join(tmpdir(), 'marcel-perf-cold-home-'))
    const userDataDir = mkdtempSync(join(tmpdir(), 'marcel-perf-cold-ud-'))
    seedConfigFrom(join(homedir(), '.marcel'), home)
