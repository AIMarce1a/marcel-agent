...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `marcel-dev-mock-${Date.now()}`))
  ...
  return { root, marcelHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(marcelHome, mockUrl) {
  fs.writeFileSync(
    path.join(marcelHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(marcelHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.marcelHome, mock.url)
  console.log(`  MARCEL_HOME: ${sandbox.marcelHome}`)
  ...
  const env = {
    ...
    MARCEL_DESKTOP_APP_NAME: `MarcelDevMock-${Date.now()}`,
  }
