...
// no longer bundles the Marcel Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Marcel.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Marcel'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Marcel.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Marcel'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default MARCEL_HOME for non-sandboxed runs -- matches main.ts's
// resolveMarcelHome(). On Windows it's %LOCALAPPDATA%\marcel; elsewhere
// it's ~/.marcel. The fresh-install sandbox launchFresh() sets its own
// MARCEL_HOME and never touches this.
const DEFAULT_MARCEL_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'marcel')
  }
  return path.join(os.homedir(), '.marcel')
})()
const VENV_ROOT = path.join(DEFAULT_MARCEL_HOME, 'marcel-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'marcel-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.MARCEL_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Marcel-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Marcel-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Marcel-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Marcel-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.MARCEL_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.MARCEL_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const marcelHome = path.join(sandbox, 'marcel-home')
  ...
  env.MARCEL_DESKTOP_CWD = cwd
  env.MARCEL_DESKTOP_IGNORE_EXISTING = '1'
  env.MARCEL_DESKTOP_TEST_MODE = 'fresh-install'
  env.MARCEL_DESKTOP_USER_DATA_DIR = userDataDir
  env.MARCEL_HOME = marcelHome
  delete env.MARCEL_DESKTOP_MARCEL
  delete env.MARCEL_DESKTOP_MARCEL_ROOT
  ...
  console.log(`  MARCEL_HOME: ${marcelHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(marcelHome, 'marcel-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Marcel Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of marcel_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'marcel-agent', 'marcel_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Marcel
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + MARCEL_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  MARCEL_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
