...
const LEGACY_STORAGE_KEY = 'marcel.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'marcel.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.marcelDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `marcel-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const marcelHome = join(sandbox, 'marcel-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(marcelHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(marcelHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        MARCEL_DESKTOP_APP_NAME: `MarcelShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        MARCEL_DESKTOP_MARCEL_ROOT: target.targetRoot,
        MARCEL_DESKTOP_IGNORE_EXISTING: '1',
        MARCEL_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'marcel-short-session-ab-'))
  const prepared = []
