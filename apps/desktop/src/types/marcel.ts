...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Marcel doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in marcel_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `marcel model` uses — instead of
...
export type MarcelConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `marcel model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// non-desktop) Marcel simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (marcel.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors marcel_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Marcel, 'hub' = installed. */
  provenance?: 'agent' | 'bundled' | 'hub'
}

/** One entry of the built-in optional-skills catalog (optional-skills/ in the
 *  repo) — official skills that ship with Marcel but install on demand. */
...
export interface ToolProvider {
   ...
   *  CLI `marcel tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Marcel); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** marcel-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** marcel-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/marcel/update/check` — the backend's own update state.
