...
import { getMarcelConfigRecord, type MarcelConfigRecord, saveMarcelConfig } from '@/marcel'
...
export interface I18nConfigClient {
  getConfig: () => Promise<MarcelConfigRecord>
  saveConfig: (config: MarcelConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.marcelDesktop?.api) {
      return Promise.resolve({})
    }

    return getMarcelConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.marcelDesktop?.api) {
    ...
    return saveMarcelConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: MarcelConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: MarcelConfigRecord, locale: Locale): MarcelConfigRecord {
