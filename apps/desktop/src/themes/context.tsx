...
const SKIN_KEY = 'marcel-desktop-theme-v2'
const MODE_KEY = 'marcel-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'marcel-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'marcel-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'marcel-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.marcelTheme = skinName
  root.dataset.marcelMode = rendered
  ...
  window.marcelDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('marcel-boot-background', chromeBg)
    window.localStorage.setItem('marcel-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.marcelThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.marcelDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Marcel authoring/activating a skin from a
