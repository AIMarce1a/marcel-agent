/**
 * Live skin sync from the Marcel backend.
 *
 * The backend resolves the active skin (built-in or `$MARCEL_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Marcel
 * authoring/activating a skin from a prompt, or `/skin` elsewhere) repaints.
 */

import type { MarcelSkin } from '@marcel/shared/skin'
...
const BACKEND_THEMES_KEY = 'marcel-desktop-backend-themes-v1'
 ...
 */
export function ingestBackendSkin(skin: MarcelSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as MarcelSkin)
