...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'marcel:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.marcelDesktop?.saveImageFromUrl) {
        if (await window.marcelDesktop.saveImageFromUrl(src)) {
