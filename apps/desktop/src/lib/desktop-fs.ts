import { marcelApi } from '@/api/client'
import type {
  MarcelConnection,
  MarcelReadDirResult,
  MarcelReadFileTextResult,
  MarcelSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: MarcelConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: MarcelConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<MarcelReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<MarcelReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<MarcelReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<MarcelReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.marcelDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: MarcelSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
