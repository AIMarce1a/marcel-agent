...
//
// When someone is in a hands-free "Hey Marcel" voice chat, the natural way to
...
// phrase (optionally addressed to Marcel), so a real turn that merely contains
...
// Optional address prefixes so "marcel stop" / "ok stop" / "hey marcel, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey marcel', 'hey marcel,', 'marcel', 'marcel,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("marcel") is not a stop command on its own.
      continue
    }
 ...
 * to Marcel). Returns false for anything that merely contains "stop" as part of
