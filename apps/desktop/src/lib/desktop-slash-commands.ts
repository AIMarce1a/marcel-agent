...
/** Why a known Marcel command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Marcel profile', surface: action('profile') },
  {
    name: '/skin',
...
function isKnownMarcelSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Marcel' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownMarcelSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Marcel and were never asked for. It is
