...
// Marcel is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.marcelMode
 ...
 * the only carriers of Marcel' palette — are dropped so the paste target's
