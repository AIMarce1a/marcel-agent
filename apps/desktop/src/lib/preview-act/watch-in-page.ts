...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __marcelWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__marcelWatch || (win.__marcelWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('marcel-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('marcel-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __marcelWatchTag?: number }).__marcelWatchTag
