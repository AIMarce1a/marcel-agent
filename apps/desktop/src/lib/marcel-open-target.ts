 ...
 * `marcel://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `marcel://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `marcel://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type MarcelOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }

const MARCEL_PROTOCOL = 'marcel:'
...
export function normalizeMarcelOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('marcel://') || trimmed.startsWith(`${MARCEL_PROTOCOL}//`)) {
    try {
      const url = new URL(trimmed)
      const host = url.hostname || ''
      const rest = decodeURIComponent((url.pathname || '').replace(/^\//, ''))

      // marcel://open/<path>?… → /<path>?…
      if (host === 'open') {
        if (!rest) {
          return null
        }
      ...
      // marcel://index-network/intent/1 → /index-network/intent/1
...
export function resolveMarcelOpenPath(target: MarcelOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeMarcelOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeMarcelOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeMarcelOpenString(target.path)
 ...
 * (`marcel://<kind>/<name>?…` → kind/name/params).
 */
export function pathFromMarcelDeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  if (!kind || !name) {
    return null
  }

  if (kind === 'open') {
    return resolveMarcelOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveMarcelOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `marcel://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromMarcelDeepLink('open', name, params)
}
