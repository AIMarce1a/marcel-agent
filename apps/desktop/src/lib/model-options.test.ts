...
import { getGlobalModelOptions } from '@/marcel'
...
const globalOptions = { model: 'marcel-4', provider: 'nous', providers: [] }
...
describe('requestModelOptions', () => {
  ...
  it('recovers an empty gateway catalog through profile-scoped REST without replacing the session selection', async () => {
    const gatewayPayload = { model: 'marcel-local', provider: 'marcel-local' }

    const restPayload = {
      model: 'profile-default',
      provider: 'openai-codex',
      providers: [{ models: ['marcel-local'], name: 'Marcel Local vLLM', slug: 'marcel-local' }]
    }
  ...
  it('keeps the gateway result when both catalog paths have no selectable models', async () => {
    const gatewayPayload = { model: 'marcel-local', provider: 'marcel-local', providers: [] }
  ...
  it('keeps an empty owner-routed catalog instead of replacing it from ambient REST', async () => {
    const ownerPayload = { model: 'berry-local', provider: 'marcel-local', providers: [] }
...
describe('manualPickRemoved', () => {
  ...
  it('never clobbers when the provider has an empty model list (re-auth)', () => {
    expect(manualPickRemoved(providers, 'nous', 'marcel-4')).toBe(false)
  })
