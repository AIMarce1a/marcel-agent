/**
 * @marcel/plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__MARCEL_PLUGIN_SDK__` and maps the import to it, so a
...
import { deleteProfile, getLogs, getStatus, marcelApi, type MarcelGateway } from '@/marcel'
import {
  $gateway,
...
import type { PaginatedSessions, UsageStats } from '@/types/marcel'
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend Marcel profile served by that route. */
...
async function requestPluginProfile<T>(
  ...
  const getAgentRoster = window.marcelDesktop?.getAgentRoster
...
async function pluginRouteStillRegistered(route: PluginProfileRoute): Promise<boolean> {
  const getProfileRoutes = window.marcelDesktop?.getProfileRoutes
...
//
// ensureGatewayProfile awaits the store's dial, and MarcelGateway.connect() has
...
export const host = {
  ...
  deleteProfile: async (profile: string | PluginProfileRoute): Promise<void> => {
    ...
    // restores on relaunch and re-creates the deleted profile (marcel-agent#94235).
    dropTilesForProfile(
  ...
  connections: async () => {
    const bridge = window.marcelDesktop?.connections

    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update Marcel Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.marcelDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update Marcel Desktop.')
    }
  ...
  newChat: (profile?: null | string | PluginProfileRoute, options: PluginNewChatOptions = {}): void => {
    if (options.workspaceMode === 'bots') {
      ...
      if (!openTab) {
        notify({ kind: 'error', message: 'Update Marcel Desktop to open another Bot chat.' })
   ...
   *
   *  `isStaleTile` (marcel-agent#90102): the caller's reconciliation probe
  ...
  profileRoutes: async () => {
    const desktop = window.marcelDesktop
    const getProfileRoutes = desktop?.getProfileRoutes

    if (!getProfileRoutes) {
      throw new Error('Marcel Desktop connection routing unavailable')
    }
  ...
  listPersistedSessions: async (
    ...
    return marcelApi<PaginatedSessions>({
  ...
  setPersistedSessionHidden: async (
    ...
    return marcelApi<{ ok: boolean; hidden: boolean }>({
      ...(route ? { connectionId: route.connectionId } : {}),
  ...
  request: async <T>(method: string, params: Record<string, unknown> = {}): Promise<T> => {
    ...
    if (!gateway) {
      throw new Error('Marcel gateway unavailable')
    }
 ...
 *  "MARCEL AGENT" with it, a `chat.empty` contribution writes its own name. */
...
export type { MarcelGateway } from '@/marcel'
...
export type { MarcelOpenTarget } from '@/lib/marcel-open-target'
...
/** Marcel' reasoning levels + their compact labels, so a plugin surfacing a
...
export type { RpcEvent, StatusResponse } from '@/types/marcel'
