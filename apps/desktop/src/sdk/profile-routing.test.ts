...
vi.mock('@/marcel', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn(), marcelApi: vi.fn() }))
...
const { deleteProfile, marcelApi } = await import('@/marcel')
...
describe('connection-aware plugin host APIs', () => {
  it('retires a profile gateway before deleting it', async () => {
    ...
    // profile's backend, re-creating its MARCEL_HOME (#94235).
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { marcelDesktop: unknown }).marcelDesktop = { getProfileRoutes }
  ...
  it('reads and hides persisted sessions through the source primary without activating the profile', async () => {
    ...
    expect(marcelApi).toHaveBeenNthCalledWith(
      1,
    ...
    expect(marcelApi).toHaveBeenNthCalledWith(2, {
      connectionId: 'source-a',
      path: '/api/sessions/bot-chat',
      method: 'PATCH',
      body: { hidden: true, profile: 'backend-worker' }
    })
    expect(vi.mocked(marcelApi).mock.calls.every(([request]) => !('profile' in request))).toBe(true)
