...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `MarcelPlugin` and
...
`$MARCEL_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`marcel-desktop-plugins` skill.
