...
describe('routing', () => {
  ...
  it('resolves @marcel to the default member', async () => {
    const { rounds } = await loadRoom()
    ...
    const parsed = rounds.parseGroupChatMentions('@marcel take a look', members)
...
describe('turn prompt', () => {
  it('addresses the default profile as @marcel', async () => {
    const { rounds } = await loadRoom()
    ...
    expect(own).toMatch(/You are @marcel,/)
    ...
    expect(peer).toMatch(/group chat with @marcel/)
  })
