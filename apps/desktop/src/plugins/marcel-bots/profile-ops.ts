...
import { forgetSessionUnread, host, queryClient } from '@marcel/plugin-sdk'
...
export function mergeServerMeta(roster: RosterRow[], fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['marcel-bots']
...
/** Permanently delete a bot's Marcel profile, then remove plugin-local state
 ...
 * directory (marcel-agent#52279). That is the "can't delete a bot" error. */
