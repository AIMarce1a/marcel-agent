...
export function displayName(bot: Partial<RosterRow>, meta?: BotMeta | null): string {
  ...
  // "Marcel". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `marcel profile rename
  ...
  // never generic "Marcel" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as Marcel (the agent it is)
