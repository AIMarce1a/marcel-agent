...
import { atom, host } from '@marcel/plugin-sdk'
...
const GROUP_CHAT_SYNC_META_KEY = 'marcel-bots-groups'
...
/** The v3 envelope stored under the default profile's `marcel-bots-groups`
 ...
 *  clients (Marcel Go, headless backends) see rooms regardless of which
 ...
 *  renamed bot never shows up as its raw profile id or a stale "Marcel"
 *  (community report, Aug 21 2026: renamed default still read "Marcel is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as Marcel (matching displayName and the
 *  @marcel handle) so the main agent never loses its name in rooms. */
export function groupSpeakerLabel(name?: null | string) {
  ...
  // Core profile display_name (`marcel profile rename …` / dashboard) from
  ...
  return trimmed.toLowerCase() === 'default' ? 'Marcel' : trimmed
}
