...
import { Button, host, Input, useI18n } from '@marcel/plugin-sdk'
...
// and the row falls back to the "run marcel mcp / Settings" hint. profile is
...
export function McpSetupButton({ profile, entry, onDone, ensureProfile }: McpSetupButtonProps) {
  ...
  const beginOAuth = async () => {
    ...
    const mcpOauthBridge =
      typeof window !== 'undefined' && window.marcelDesktop && window.marcelDesktop.mcpOauth
        ? window.marcelDesktop.mcpOauth
        : null
    ...
    try {
      ...
      } else if (typeof window !== 'undefined' && window.marcelDesktop && window.marcelDesktop.openExternal) {
        window.marcelDesktop.openExternal(authUrl)
      } else {
