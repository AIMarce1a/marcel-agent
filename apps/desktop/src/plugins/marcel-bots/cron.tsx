/**
 * Routines: the Marcel cron jobs scoped to the bot you're chatting with — the
...
// marcel -p <bot> chat delegation wrapper so the run reaches that bot's
...
export function routinePrompt(
  ...
  return (
    ...
    `marcel -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// Marcel-native schedule string; Advanced exposes it raw.
...
/** Compose the Marcel schedule string from picker state. */
