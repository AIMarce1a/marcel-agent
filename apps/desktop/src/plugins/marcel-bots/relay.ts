...
import { host, LruCache } from '@marcel/plugin-sdk'
...
// to (local, remote URL, SSH, Marcel Cloud, docker) must be able to find
...
// marcel_cli/config_defaults.py and tui_gateway/methods_bot_relay.py and fails
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['marcel-bots']?.title || profile?.display_name || ''),
