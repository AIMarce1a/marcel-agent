...
const preamble = () => `  var w = window;
  ...
  w.__marcelActHolder = w.__marcelActHolder || {};
  w.__marcelAct = ${actEngineSource()};
  w.__marcelWatch_fn = (${watchInPage.toString()});
  w.__marcelWatchTag = ${WATCH_TAG};
  var holder = w.__marcelActHolder;
  var act = function (a) { return w.__marcelAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__marcelWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__marcelHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'MARCEL-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
