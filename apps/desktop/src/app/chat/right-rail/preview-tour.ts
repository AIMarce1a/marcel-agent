...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__marcelTourEngine) {
    ${driverIife}
    w.__marcelTourHolder = {};
    w.__marcelTourCollect = (${collectTourTargets.toString()});
    w.__marcelTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__marcel-tour-style')) {
    var style = document.createElement('style');
    style.id = '__marcel-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__marcelTourEngine(
    w.driver.js.driver,
    w.__marcelTourHolder,
    ${JSON.stringify(action)},
    w.__marcelTourCollect,
    document
  ));
})()`
}
