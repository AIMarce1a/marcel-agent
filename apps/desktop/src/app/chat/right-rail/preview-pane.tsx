        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.marcelDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const annotateGuest = useCallback((): null | PreviewAnnotateGuest => {
    ...
    return {
      capture: async rect => {
        ...
        const dataUrl = await window.marcelDesktop.capturePreview?.({ rect, viewport, webContentsId })
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Marcel to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.marcelDesktop?.watchPreviewFile ||
      !window.marcelDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.marcelDesktop.onPreviewFileChanged(payload => {
    ...
    void window.marcelDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.marcelDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.marcelDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:marcel-preview')
    ...
    const onGuestContextMenu = (event: Event) => {
      ...
      const zoom = window.marcelDesktop?.zoom?.factor?.() || 1
      ...
      openGuestContextMenu(
        ...
        },
        {
          addToDictionary: (word: string) => {
            ...
            if (typeof webContentsId === 'number') {
              void window.marcelDesktop?.contextMenuGuestAddWord?.({ webContentsId, word })
            }
          },
          copyImage: () => void window.marcelDesktop?.contextMenuCopyImage?.(),
          ...
          <PreviewBrowserBar
            ...
            onOpenExternal={
              !isBrowserWindow() && !canOpenBrowserWindow()
                ? () => void window.marcelDesktop?.openExternal(currentUrl)
                : undefined
            }
