...
function setWorkspaceTransfer(transferring: boolean): void {
  window.marcelDesktop?.hud?.setWorkspaceTransfer?.(transferring)
}

function moveHud(state: PressState): void {
  window.marcelDesktop?.hud?.moveBy?.({
    width: state.originW,
...
function armGrab(state: PressState, workspaceTransfer: boolean): void {
  ...
  window.marcelDesktop?.hud?.beginMove?.()
}
 ...
 * (see marcel:hud:move-by — a transparent frameless window drifts wider on
...
export function useHudComposerDrag(
  ...
  const reset = useCallback(() => {
    ...
    if (state) {
      if (state.armed) {
        window.marcelDesktop?.hud?.endMove?.()
      }
