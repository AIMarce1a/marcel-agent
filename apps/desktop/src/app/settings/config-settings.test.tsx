...
const getMarcelConfigSchema = vi.fn()
const saveMarcelConfig = vi.fn()
const getElevenLabsVoices = vi.fn()

vi.mock('@/marcel', () => ({
  getMarcelConfigRecord: () => getMarcelConfigRecord(),
  getMarcelConfigSchema: () => getMarcelConfigSchema(),
  saveMarcelConfig: (config: unknown, profile?: string) => saveMarcelConfig(config, profile),
...
beforeEach(() => {
  ...
  getMarcelConfigSchema.mockResolvedValue({ fields: {} })
...
describe('ConfigSettings autosave', () => {
  it('sends a later revert instead of diffing it away against the stale page-load baseline', async () => {
    getMarcelConfigRecord.mockResolvedValue({ checkpoints: { enabled: false }, other: 'untouched' })
    ...
    try {
      ...
      await waitFor(() => expect(saveMarcelConfig).toHaveBeenCalledTimes(1))
      expect(saveMarcelConfig.mock.calls[0][0]).toEqual({ checkpoints: { enabled: true } })

      // Revert: flip it back to its original value and let autosave fire again.
      toggle.click()
      await vi.advanceTimersByTimeAsync(700)

      await waitFor(() => expect(saveMarcelConfig).toHaveBeenCalledTimes(2))
      ...
      expect(saveMarcelConfig.mock.calls[1][0]).toEqual({ checkpoints: { enabled: false } })
    } finally {
      vi.useRealTimers()
    }
  })
