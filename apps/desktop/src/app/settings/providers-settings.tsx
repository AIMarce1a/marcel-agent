...
import { disconnectOAuthProvider, listOAuthProviders } from '@/marcel'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/marcel'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.marcelDesktop?.terminal)
...
//      in marcel_cli/provider_catalog.py) — the SAME provider identity
//      `marcel model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Marcel can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Marcel can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Marcel never deletes creds another tool owns behind a silent API call.
