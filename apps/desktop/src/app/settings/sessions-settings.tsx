...
import type { MarcelConfigRecord, SessionInfo } from '@/types/marcel'
...
function AutoArchiveSetting() {
  ...
  const persist = useCallback(
    async (autoArchive: boolean, archiveDays: number) => {
      ...
      try {
        await saveMarcelConfig(updated)
      } catch (err) {
...
// / Program Files), which buried any files Marcel wrote there.
function DefaultProjectDirSetting() {
  ...
  useEffect(() => {
    ...
    // / Storybook / non-Electron context `window.marcelDesktop` is
    ...
    const settings = window.marcelDesktop?.settings
