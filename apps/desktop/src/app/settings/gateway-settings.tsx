...
// Marcel Cloud discovery lifecycle for the cloud-mode panel.
...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const setKeychainEncryption = async (on: boolean) => {
    ...
    try {
      const res = await window.marcelDesktop.setSecretStorageEncryption(on)
  ...
  useEffect(() => {
    if (state.mode !== 'ssh' || !window.marcelDesktop?.sshConfigHosts) {
      return
    }
  ...
  const payload = (allowPlainTextToken?: boolean) => ({
    ...
    sshRemoteMarcelPath: state.sshRemoteMarcelPath.trim(),
  ...
  const performSave = async (apply: boolean, allowPlainTextToken: boolean) => {
    ...
    try {
      const next = apply
        ? await window.marcelDesktop.applyConnectionConfig(payload(allowPlainTextToken))
        : await window.marcelDesktop.saveConnectionConfig(payload(allowPlainTextToken))
      ...
      const errors = {
        ...
        'marcel-not-found': g.sshErrNotInstalled,
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.marcelDesktop.saveConnectionConfig({
        mode: state.mode,
      ...
      const result = await window.marcelDesktop.oauthLoginConnectionConfig(trimmedUrl)
      ...
      if (result.connected) {
        const refreshed = await window.marcelDesktop.getConnectionConfig(null)
  ...
  const signOut = async () => {
    ...
    try {
      await window.marcelDesktop.oauthLogoutConnectionConfig(trimmedUrl)
      const refreshed = await window.marcelDesktop.getConnectionConfig(null)
  ...
  const resolveSshHost = async (host: string) => {
    if (!host || !window.marcelDesktop?.sshResolveHost) {
      return
    }
    ...
    try {
      const resolved = await window.marcelDesktop.sshResolveHost(host)
  ...
  const testSsh = async () => {
    ...
    try {
      const result = await window.marcelDesktop.testConnectionConfig(payload())
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.marcelDesktop.testConnectionConfig({
        mode: 'remote',
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
        remoteUrl: trimmedUrl
  ...
  if (!window.marcelDesktop?.getConnectionConfig) {
  ...
  return (
    <SettingsContent bare={embedded}>
      ...
      {/* Marcel Cloud panel: one portal sign-in, then a discovered-agent picker
          ...
          <ListRow
            action={
              <Input
                ...
                placeholder="https://gateway.example.com/marcel"
        ...
        <div className="mt-5 grid gap-1">
          ...
          <ListRow
            action={
              <Input
                className={cn('h-8 font-mono', CONTROL_TEXT)}
                onChange={event => setState(current => ({ ...current, sshRemoteMarcelPath: event.target.value }))}
                placeholder={g.sshMarcelPathPlaceholder}
                value={state.sshRemoteMarcelPath}
              />
            }
            description={g.sshMarcelPathDesc}
            title={g.sshMarcelPathTitle}
          />
        </div>
      ) : null}
      ...
      {embedded ? null : (
        <div className="mt-6 grid gap-1">
          ...
          <ListRow
            action={
              <Button onClick={() => void window.marcelDesktop?.revealLogs()} size="sm" variant="textStrong">
                <FileText />
                {g.openLogs}
              </Button>
            }
