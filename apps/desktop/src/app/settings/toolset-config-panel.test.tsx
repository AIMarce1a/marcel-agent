...
import type { ToolsetConfig } from '@/types/marcel'
...
beforeEach(() => {
  ...
  getMarcelConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getMarcelConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveMarcelConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveMarcelConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
