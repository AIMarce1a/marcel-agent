...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: MarcelConfigRecord = { display: { theme: 'mono' } }
  ...
  it('rejects prototype-polluting config paths', () => {
    const config: MarcelConfigRecord = {}
  ...
  describe('providerGroup', () => {
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // MARCEL_QWEN_ shares the MARCEL_ stem with other integrations.
      expect(providerGroup('MARCEL_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
  ...
  describe('enumOptionsFor — backend selector dropdowns', () => {
    ...
    it('surfaces user-defined command-type TTS providers (canonical providers nesting + legacy)', () => {
      const withCustom: MarcelConfigRecord = {
        tts: {
          provider: 'neutts',
    ...
    it('never offers a built-in name as a command provider, even one absent from the dropdown list', () => {
      const shadowing: MarcelConfigRecord = {
        tts: {
          provider: 'edge',
          providers: {
  ...
  describe('sectionFieldEntries', () => {
    it('renders memory.provider from config even when the backend schema omits it', () => {
      ...
      const config: MarcelConfigRecord = { memory: { memory_enabled: true, provider: '' } }
    ...
    it('infers the field type from the config value when the schema omits the key', () => {
      const config: MarcelConfigRecord = { memory: { provider: '', memory_enabled: true, memory_char_limit: 2200 } }
    ...
    it('prefers the backend schema entry over inference when both exist', () => {
      ...
      const config: MarcelConfigRecord = { memory: { provider: 'honcho' } }
  ...
  describe('clearsEnabledToolsets', () => {
    it('flags a non-empty → empty transition', () => {
      const prev: MarcelConfigRecord = { toolsets: ['memory', 'terminal', 'web_search'] }
      const next: MarcelConfigRecord = { toolsets: [] }
    ...
    it('does not flag a non-empty → missing transition (deep-merge preserves the key)', () => {
      ...
      const prev: MarcelConfigRecord = { toolsets: ['memory'] }
      const next: MarcelConfigRecord = {}

      expect(clearsEnabledToolsets(prev, next)).toBe(false)
    })

    it('does not flag when at least one toolset remains', () => {
      const prev: MarcelConfigRecord = { toolsets: ['memory', 'terminal'] }
      const next: MarcelConfigRecord = { toolsets: ['memory'] }

      expect(clearsEnabledToolsets(prev, next)).toBe(false)
    })

    it('does not flag when the list was already empty', () => {
      const prev: MarcelConfigRecord = { toolsets: [] }
      const next: MarcelConfigRecord = { toolsets: [] }
    ...
    it('does not flag an unrelated edit that never touched toolsets', () => {
      const prev: MarcelConfigRecord = { model: 'a', toolsets: ['memory'] }
      const next: MarcelConfigRecord = { model: 'b', toolsets: ['memory'] }
  ...
  describe('diffConfig', () => {
    it('omits a top-level key the draft never touched', () => {
      ...
      // an agent set via `marcel config set` while the page sat open must not
      // come back in the patch just because it's still present in the draft.
      const baseline: MarcelConfigRecord = { fallback_providers: ['nara1'], timezone: 'UTC' }
      const draft: MarcelConfigRecord = { fallback_providers: ['nara1'], timezone: 'America/New_York' }
    ...
    it('includes a nested key only when it actually changed, leaving siblings out', () => {
      const baseline: MarcelConfigRecord = { display: { personality: 'default', show_reasoning: true } }
      const draft: MarcelConfigRecord = { display: { personality: 'default', show_reasoning: false } }
    ...
    it('sends a new key that was absent from the baseline', () => {
      const baseline: MarcelConfigRecord = {}
      const draft: MarcelConfigRecord = { timezone: 'UTC' }
    ...
    it('returns an empty object when the draft matches the baseline exactly', () => {
      const baseline: MarcelConfigRecord = { toolsets: ['memory'], display: { personality: 'default' } }
      const draft: MarcelConfigRecord = { toolsets: ['memory'], display: { personality: 'default' } }

      expect(diffConfig(baseline, draft)).toEqual({})
    })

    it('treats an array as a whole value, not diffed element by element', () => {
      const baseline: MarcelConfigRecord = { toolsets: ['memory', 'terminal'] }
      const draft: MarcelConfigRecord = { toolsets: ['memory'] }
