...
import { getProfiles } from '@/marcel'
...
// User-installed plugins first — mirrors `marcel plugins list --user`.
...
function reveal(file: string) {
  void window.marcelDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // marcel_home breaks against a remote backend (#66899).
    const dir = await window.marcelDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.marcelDesktop?.openDir?.(dir)
...
// Agent plugins live under the BACKEND's marcel home (profile-aware), so the
// path comes from the gateway — not from the renderer's local MARCEL_HOME.
...
async function revealAgentPluginsDir(request: GatewayRequest) {
  try {
    ...
    const opened = await window.marcelDesktop?.openDir?.(`${home}/plugins`)
            ...
            <SelectContent>
              {profiles.map(profile => (
                <SelectItem key={profile.name} value={profile.name}>
                  {profile.is_default ? 'Marcel (default)' : profile.name}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
