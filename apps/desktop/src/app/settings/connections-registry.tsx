 ...
 * agent sources (local runtime + any number of remote gateways / Marcel Cloud
...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.marcelDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.marcelDesktop.oauthLoginConnectionConfig(editorUrl)
  ...
  // Fan out `marcel update` to every eligible source; per-connection results
