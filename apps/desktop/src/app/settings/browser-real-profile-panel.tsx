 ...
 * the default browser's profile (cookies/logins) into a Marcel-owned copy;
...
export function BrowserRealProfilePanel({ profile }: BrowserRealProfilePanelProps) {
  ...
  const { data: config } = useMarcelConfigRecord(profile)
  ...
  const toggle = useCallback(
    async (on: boolean) => {
      ...
      try {
        await saveMarcelConfigRecord(next, profile)
        notify({
          kind: 'info',
