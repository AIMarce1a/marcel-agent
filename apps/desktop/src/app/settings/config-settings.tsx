...
import { getElevenLabsVoices, getMarcelConfigSchema, saveMarcelConfig } from '@/marcel'
...
import type { ConfigFieldSchema, MarcelConfigRecord } from '@/types/marcel'
...
function ConfigSettingsInner({
  ...
  const [config, setConfig] = useState<MarcelConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useMarcelConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => marcelConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['marcel-config-schema'] : ['marcel-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getMarcelConfigSchema(scopeProfile),
    staleTime: 5 * 60 * 1000
  })
  ...
  // never touched — possibly changed out-of-band by `marcel config set`
  // while this page sat open — is never resent with its stale value.
  const configBaselineRef = useRef<MarcelConfigRecord | null>(null)
    ...
    const t = window.setTimeout(() => {
      ...
      saveQueueRef.current = saveQueueRef.current.then(async () => {
        try {
          ...
          const result = await saveMarcelConfig(patch, scopeProfile)
  ...
  const applyConfig = (next: MarcelConfigRecord) => {
    saveVersionRef.current += 1
    setConfig(next)
    setSaveVersion(saveVersionRef.current)
  }

  const updateConfig = (next: MarcelConfigRecord) => {
