...
vi.mock('@/marcel', async importOriginal => {
  const actual = await importOriginal<typeof MarcelModule>()

  class FakeMarcelGateway {
  ...
  return { ...actual, MarcelGateway: FakeMarcelGateway }
})
...
const fakeGateway = { connectionState: 'open' } as unknown as MarcelGateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as MarcelGateway, 'default')
  $gateway.set(primary as unknown as MarcelGateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.marcelDesktop, 'getGatewayWsUrlFor')
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as MarcelGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { marcelDesktop: unknown }).marcelDesktop = { getConnection }
    $gateway.set(dropped)
