import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@marcel/shared'
...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as MarcelGateway | null
  const gatewayRef = useRef<MarcelGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.marcelDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<MarcelGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as MarcelGateway | null
      }),
    []
  )
