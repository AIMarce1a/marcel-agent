import { isGatewayReauthRequired, JsonRpcGatewayError, resolveGatewayWsUrl } from '@marcel/shared'
...
import { MarcelGateway } from '@/marcel'
import {
  $desktopBoot,
...
import type { RpcEvent } from '@/types/marcel'
...
export function primaryRuntimeConnectionId(connection: Pick<MarcelConnection, 'connectionId' | 'mode'>): null | string {
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.marcelDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: MarcelGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: MarcelConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Marcel...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Marcel…". The probe is a no-op for a healthy or local backend.
        ...
        const conn = await withTimeout(
          ...
          RECONNECT_ATTEMPT_TIMEOUT_MS,
          'Timed out reconnecting to Marcel backend'
        )
        ...
        // as an opaque "Could not connect to Marcel gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshMarcelConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshMarcelConfig(false, ownsSwitch).catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startMarcel re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new MarcelGateway()
    ...
    async function boot() {
      try {
        ...
        // hanging "Starting Marcel…" forever.
        const wsUrl = await withTimeout(
          resolveGatewayWsUrl(desktop, conn),
          RECONNECT_ATTEMPT_TIMEOUT_MS,
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshMarcelConfig(),
          ...
          // retry it), not brick boot behind the "Marcel couldn't start"
