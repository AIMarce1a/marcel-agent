...
import { getMarcelConfigRecord, saveMcpServers } from '@/marcel'
...
import { setMarcelConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `marcel://mcp/install` deep links. The payload is
 ...
 * confirming lets Marcel spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getMarcelConfigRecord())
      ...
      setMarcelConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
