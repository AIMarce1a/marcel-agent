...
import { pathFromMarcelDeepLink, resolveMarcelOpenPath } from '@/lib/marcel-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    // "About Marcel" — it is the OS-standard affordance for updating THIS app,
    ...
    const unsubscribe = window.marcelDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow('client'))
  ...
  useEffect(() => {
    window.marcelDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.marcelDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.marcelDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // marcel://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.marcelDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future marcelDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveMarcelOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.marcelDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (marcel://index-network/intent/1, marcel://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromMarcelDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.marcelDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.marcelDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.marcelDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.marcelDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
