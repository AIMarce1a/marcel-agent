...
import { getLatestSessionMessages } from '@/marcel'
...
import { useMarcelConfigRecord } from '../hooks/use-config-record'
...
import { useMarcelConfig } from '../session/hooks/use-marcel-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshMarcelConfig, sttEnabled, voiceMaxRecordingSeconds } = useMarcelConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('marcel:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('marcel:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshMarcelConfig(true)
    void refreshActiveProfile()
    resetProjectTreeState()
  }, [gatewayScope, refreshCurrentModel, refreshMarcelConfig])
  ...
  const configRecord = useMarcelConfigRecord()
