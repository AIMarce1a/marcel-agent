import { registryBackendScopeKey } from '@marcel/shared'
...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshMarcelConfig, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshMarcelConfig — two REST calls (config + defaults) per event, per
