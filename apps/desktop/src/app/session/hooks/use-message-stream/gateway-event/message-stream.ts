import type { BillingBlock } from '@marcel/shared'
 ...
 * billing-specific toast — never the generic "Marcel error" — with a smart CTA
  ...
  if (event.type === 'message.complete') {
    ...
    const failure =
      payload?.status === 'error'
        ? {
            error: coerceGatewayText(payload.error).trim() || finalText || 'Marcel reported an error',
