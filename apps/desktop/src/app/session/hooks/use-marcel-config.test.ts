...
import { getMarcelConfig } from '@/marcel'
...
import { useMarcelConfig } from './use-marcel-config'

vi.mock('@/marcel', () => ({
  getMarcelConfig: vi.fn(),
  getMarcelConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'marcel.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getMarcelConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getMarcelConfig>>)

describe('useMarcelConfig refreshMarcelConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to Marcel' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useMarcelConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshMarcelConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useMarcelConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getMarcelConfig>>>()
    vi.mocked(getMarcelConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useMarcelConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshMarcelConfig(true)
    })
    expect(getMarcelConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getMarcelConfig>>)
  ...
  it('does not publish config after its switch loses ownership', async () => {
    const staleConfig = deferred<Awaited<ReturnType<typeof getMarcelConfig>>>()
    vi.mocked(getMarcelConfig).mockReturnValueOnce(staleConfig.promise)
    const { result } = renderHook(() => useMarcelConfig({ activeSessionIdRef: { current: null } }))
    let ownsSwitch = true

    let refresh!: Promise<void>
    act(() => {
      refresh = result.current.refreshMarcelConfig(false, () => ownsSwitch)
    })
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getMarcelConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getMarcelConfig>>>()
    vi.mocked(getMarcelConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshMarcelConfig(true)
      refreshC = result.current.refreshMarcelConfig(true)
    })
