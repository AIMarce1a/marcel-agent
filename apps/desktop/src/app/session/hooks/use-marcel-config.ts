...
interface MarcelConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useMarcelConfig({ activeSessionIdRef }: MarcelConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshMarcelConfig = useCallback(
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        const [config, defaults] = await Promise.all([getMarcelConfig(), getMarcelConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Marcel' built-in medium over the user's config.
        if (!canPublish()) {
          return
        }
  ...
  return { refreshMarcelConfig, sttEnabled, voiceMaxRecordingSeconds }
}
