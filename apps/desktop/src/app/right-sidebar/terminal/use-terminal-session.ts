...
const MARCEL_PATHS_MIME = 'application/x-marcel-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(MARCEL_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(MARCEL_PATHS_MIME)
  ...
  const getPath = window.marcelDesktop?.getPathForFile
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.marcelDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (marcel --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.marcelDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[marcel-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.marcelDesktop?.terminal?.write(sessionId, `${command}\r`)
