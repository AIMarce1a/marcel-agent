...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { marcelDesktop: { readDir: typeof readDir } }).marcelDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): MarcelReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { marcelDesktop: unknown }).marcelDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: MarcelReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<MarcelReadDirResult>(resolve => {
  ...
  it('discards a stale live refresh after the active registered connection changes', async () => {
    let resolveRefreshFromA: ((result: MarcelReadDirResult) => void) | undefined
  ...
  it('discards a stale child read after the active registered connection changes', async () => {
    let resolveChildFromA: ((result: MarcelReadDirResult) => void) | undefined
  ...
  it('discards a stale root read after the active registered connection changes', async () => {
    let resolveFirst: ((result: MarcelReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<MarcelReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.marcelDesktop is missing', async () => {
    delete (window as unknown as { marcelDesktop?: unknown }).marcelDesktop
  ...
  it('keeps an unreadable root on screen while it re-probes', async () => {
    ...
    let releaseProbe: ((value: MarcelReadDirResult) => void) | undefined
  ...
  it('keeps loaded rows on screen while the root refreshes', async () => {
    ...
    let releaseRefresh: ((value: MarcelReadDirResult) => void) | undefined
  ...
  it('clears the rows when the same path is re-read from another backend', async () => {
    ...
    let releaseFromB: ((value: MarcelReadDirResult) => void) | undefined
