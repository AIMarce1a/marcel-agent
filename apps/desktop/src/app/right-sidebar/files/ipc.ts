...
import type { MarcelReadDirEntry, MarcelReadDirResult } from '@/global'
...
export type ProjectTreeEntry = MarcelReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: MarcelReadDirEntry) {
...
async function filterIgnored(entries: MarcelReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<MarcelReadDirResult> {
  if (!window.marcelDesktop) {
