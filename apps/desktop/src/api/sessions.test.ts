...
const marcelApi = vi.mocked(client.marcelApi)
...
describe('deleteSession profile scoping', () => {
  it('scopes the DELETE to the owning profile in the URL (object owner)', async () => {
    ...
    marcelApi.mockResolvedValue({ ok: true } as never)
    // Mirrors the real capabilityScoped for an object owner (remote-stamped row).
    vi.mocked(client.capabilityScoped).mockReturnValue({ profile: 'tommy', connectionId: 'marcel-pi' })

    await deleteSession('sess-1', { connectionId: 'marcel-pi', profile: 'tommy' })

    expect(marcelApi.mock.calls[0][0]).toMatchObject({
      method: 'DELETE',
      path: '/api/sessions/sess-1?profile=tommy',
      connectionId: 'marcel-pi',
      profile: 'tommy'
    })
  })
  ...
  it('omits the profile query when no owner is known', async () => {
    ...
    expect((marcelApi.mock.calls[0][0] as { path: string }).path).not.toContain('profile=')
  })
...
describe('setSessionArchived profile scoping', () => {
  ...
  it('omits the profile from the body when none is given', async () => {
    ...
    const req = marcelApi.mock.calls[0][0] as { body: Record<string, unknown> }
