import { JsonRpcGatewayClient } from '@marcel/shared'

import type { MarcelApiRequest } from '@/global'
...
// "Timed out connecting to Marcel backend" that hangs the UI (#48504).
//
...
export class MarcelGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Marcel gateway connection closed',
      connectErrorMessage: 'Could not connect to Marcel gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Marcel gateway is not connected',
...
// process to route them to that host (marcel:api's registry branch). Null
...
export function marcelApi<T>(request: MarcelApiRequest): Promise<T> {
  return window.marcelDesktop.api<T>({ ...connectionScoped(), ...request })
}
