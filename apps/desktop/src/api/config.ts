...
import { capabilityScoped, marcelApi, type ProfileScope, profileScoped, STARTUP_REQUEST_TIMEOUT_MS } from './client'

export function getStatus(): Promise<StatusResponse> {
  return marcelApi<StatusResponse>({
    ...profileScoped(),
    path: '/api/status'
  })
}
...
export function getLogs(params: {
  ...
  return marcelApi<LogsResponse>({
    ...profileScoped(),
    path: suffix ? `/api/logs?${suffix}` : '/api/logs'
  })
}

export function getMarcelConfig(profile?: string): Promise<MarcelConfig> {
  return marcelApi<MarcelConfig>({
...
export function getMarcelConfigRecord(profile?: ProfileScope): Promise<MarcelConfigRecord> {
  return window.marcelDesktop.api<MarcelConfigRecord>({
    ...capabilityScoped(profile),
    path: '/api/config'
  })
}

export function getMarcelConfigDefaults(): Promise<MarcelConfigRecord> {
  return marcelApi<MarcelConfigRecord>({
    ...profileScoped(),
...
export function getMarcelConfigSchema(profile?: null | string): Promise<ConfigSchemaResponse> {
  return marcelApi<ConfigSchemaResponse>({
    ...profileScoped(profile),
    path: '/api/config/schema'
  })
}

export function saveMarcelConfig(config: MarcelConfigRecord, profile?: null | string): Promise<{ ok: boolean }> {
  return marcelApi<{ ok: boolean }>({
    ...profileScoped(profile),
    path: '/api/config',
    method: 'PUT',
    body: { config }
...
/** Capability-scoped counterpart of saveMarcelConfig — writes the config of
 *  the profile/connection the Capabilities scope selector points at (possibly
 *  on another registered gateway), mirroring getMarcelConfigRecord. */
export function saveMarcelConfigRecord(config: MarcelConfigRecord, profile?: ProfileScope): Promise<{ ok: boolean }> {
  return window.marcelDesktop.api<{ ok: boolean }>({
    ...capabilityScoped(profile),
    path: '/api/config',
    method: 'PUT',
    body: { config }
...
export function getEnvVars(profile?: null | string): Promise<Record<string, EnvVarInfo>> {
  return marcelApi<Record<string, EnvVarInfo>>({
    ...profileScoped(profile),
    path: '/api/env'
  })
}
...
export function revealEnvVar(key: string, profile?: ProfileScope): Promise<{ key: string; value: string }> {
  return window.marcelDesktop.api<{ key: string; value: string }>({
    ...capabilityScoped(profile),
    path: '/api/env/reveal',
    method: 'POST',
    body: { key }
...
export function validateProviderCredential(
  ...
  return marcelApi<{ ok: boolean; reachable: boolean; message: string; models?: string[] }>({
    ...profileScoped(),
...
export function getCustomEndpoints(): Promise<CustomEndpointsResponse> {
  return marcelApi<CustomEndpointsResponse>({
    ...profileScoped(),
...
export function validateCustomEndpoint(endpoint: CustomEndpointUpdate): Promise<CustomEndpointValidationResponse> {
  return marcelApi<CustomEndpointValidationResponse>({
...
export function activateCustomEndpoint(id: string): Promise<{ ok: boolean; provider: string; model: string }> {
  return marcelApi<{ ok: boolean; provider: string; model: string }>({
    ...profileScoped(),
...
export function listOAuthProviders(): Promise<OAuthProvidersResponse> {
  return marcelApi<OAuthProvidersResponse>({
    ...profileScoped(),
...
export function disconnectOAuthProvider(providerId: string): Promise<{ ok: boolean; provider: string }> {
  return marcelApi<{ ok: boolean; provider: string }>({
    ...profileScoped(),
...
export function startOAuthLogin(providerId: string, profile?: ProfileScope): Promise<OAuthStartResponse> {
  return window.marcelDesktop.api<OAuthStartResponse>({
    ...capabilityScoped(profile),
...
export function submitOAuthCode(providerId: string, sessionId: string, code: string): Promise<OAuthSubmitResponse> {
  return marcelApi<OAuthSubmitResponse>({
    ...profileScoped(),
...
export function pollOAuthSession(
  ...
  return window.marcelDesktop.api<OAuthPollResponse>({
    ...capabilityScoped(profile),
