...
import { capabilityScoped, marcelApi, type ProfileScope, STARTUP_REQUEST_TIMEOUT_MS } from './client'

export function getProfiles(): Promise<ProfilesResponse> {
  return marcelApi<ProfilesResponse>({
    path: '/api/profiles',
...
export function createProfile(body: ProfileCreatePayload): Promise<{ name: string; ok: boolean; path: string }> {
  return marcelApi<{ name: string; ok: boolean; path: string }>({
    path: '/api/profiles',
    method: 'POST',
    body
  })
...
export function deleteProfile(name: string, scope?: ProfileScope): Promise<{ ok: boolean; path: string }> {
  ...
  return marcelApi<{ ok: boolean; path: string }>({
    ...profileOwnerScoped(scope),
...
export function getProfileSetupCommand(name: string): Promise<ProfileSetupCommand> {
  return marcelApi<ProfileSetupCommand>({
...
export function exportProfileArchive(
  ...
  return marcelApi<{ archive: string; ok: boolean }>({
...
export function importProfileArchive(
  ...
  return marcelApi<{ desktop: null | ProfileDesktopOverlay; name: string; ok: boolean; path: string }>({
