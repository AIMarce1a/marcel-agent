...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Marcel gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>, overrides: Record<string, unknown> = {}) {
  const original = window.marcelDesktop
  Object.defineProperty(window, 'marcelDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config, ...overrides }
  })

  return () => Object.defineProperty(window, 'marcelDesktop', { configurable: true, value: original })
}
