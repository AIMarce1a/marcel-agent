...
const TREE_KEY = 'marcel.desktop.layoutTree.v2'
const USER_PLACED_KEY = 'marcel.desktop.userPlacedPanes.v1'
const LEGACY_HEAL_KEY = 'marcel.desktop.paneDockHeals.v1'
  ...
  children: [
    {
      ...
      children: [
        ...
        { type: 'group', id: 'g-bots', panes: ['marcel-bots:pane'], active: 'marcel-bots:pane' }
      ]
    },
...
describe('enforced dock (stacked Bots pane → sessions-zone tab, every boot)', () => {
  ...
  it('re-homes a stacked bots pane into the sessions tab strip, keeping sessions active', async () => {
    ...
    const group = model.findGroupOfPane(tree.$layoutTree.get()!, 'marcel-bots:pane')!

    expect(group.panes).toEqual(['sessions', 'marcel-bots:pane'])
    ...
    expect(JSON.stringify(persisted)).toContain('"panes":["sessions","marcel-bots:pane"]')
  })

  it('re-homes even a USER-PLACED pane — the owner invariant beats the drag record', async () => {
    window.localStorage.setItem(USER_PLACED_KEY, JSON.stringify(['marcel-bots:pane']))
  ...
  it('re-homes even when the retired heal token was already burned, and clears the stale ledger', async () => {
    window.localStorage.setItem(LEGACY_HEAL_KEY, JSON.stringify(['marcel-bots:pane:sessions-tab-v1']))
  ...
  it('is idempotent within a boot and does not fight an intra-session drag', async () => {
    ...
    expect(model.findGroupOfPane(tree.$layoutTree.get()!, 'marcel-bots:pane')!.panes).toContain('sessions')
    ...
    expect(group.panes).toEqual(['marcel-bots:pane'])
  })
  ...
  it('re-homes again on the NEXT boot after a drag persisted the stacked shape', async () => {
    ...
    const group = second.model.findGroupOfPane(second.tree.$layoutTree.get()!, 'marcel-bots:pane')!
      ...
      children: [
        {
          type: 'group',
          id: 'g-left',
          panes: ['sessions', 'marcel-bots:pane'],
          active: 'marcel-bots:pane',
          headerHidden: true
        },
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' }
      ]
  ...
  it('re-homes an edge-enforced pane stranded in the sessions tab strip', async () => {
      ...
      children: [
        {
          type: 'group',
          id: 'g-sessions',
          panes: ['sessions', 'marcel-bots:pane', 'marcel-bots:routines'],
          active: 'marcel-bots:pane'
        },
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' }
      ]
    ...
    const botsGroup = model.findGroupOfPane(tree.$layoutTree.get()!, 'marcel-bots:pane')!
    const routinesGroup = model.findGroupOfPane(tree.$layoutTree.get()!, 'marcel-bots:routines')!

    expect(botsGroup.panes).toEqual(['sessions', 'marcel-bots:pane'])
    expect(botsGroup.active).toBe('marcel-bots:pane')
    expect(routinesGroup.panes).toEqual(['marcel-bots:routines'])
    ...
    const dockedRoutinesTree = {
      ...
      children: [
        ...
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' },
        {
          type: 'group',
          id: 'g-routines',
          panes: ['marcel-bots:routines'],
          active: 'marcel-bots:routines'
        }
      ]
    }
