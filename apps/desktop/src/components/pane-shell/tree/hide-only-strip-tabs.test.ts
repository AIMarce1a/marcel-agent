...
function sessionsBotsTree() {
  ...
  registerPane('marcel-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'marcel-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('marcel-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('marcel-bots:pane')
    expect($hiddenStripTabs.get()).toContain('marcel-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('marcel-bots:pane')

    expect(setStripTabHidden('marcel-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('marcel-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('marcel-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('marcel-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('marcel.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('marcel-bots:pane')
    ...
    revealTreePane('marcel-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('marcel-bots:pane')
    expect(window.localStorage.getItem('marcel.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'marcel-bots:pane', title: 'marcel-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'marcel-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
