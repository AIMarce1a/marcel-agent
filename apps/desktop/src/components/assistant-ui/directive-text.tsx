...
const MARCEL_REF_TYPES = WIRE_REFERENCE_KINDS
type MarcelRefType = (typeof MARCEL_REF_TYPES)[number]
...
const MARCEL_DIRECTIVE_RE = referenceRe()
...
export const marcelDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(MARCEL_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...marcelDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Marcel directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.marcelDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.marcelDesktop?.readFileDataUrl(id)
