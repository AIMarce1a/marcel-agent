 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.marcel.send(prompt)` (or declarative
 * `data-marcel-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'marcel-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'marcel-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `marcel.send(prompt)`.
 ...
 *  wires `data-marcel-send` so declarative HTML works with zero script:
 *  `<button data-marcel-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.marcel={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-marcel-send]"):null;' +
    'if(el)send(el.getAttribute("data-marcel-send")||"")},true)})()</script>'
  )
}
...
export function InlinePreviewDirective({
  ...
  // URL connection — including a same-machine `marcel serve` — renders live.)
