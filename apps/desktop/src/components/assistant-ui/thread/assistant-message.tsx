...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's MARCEL_HOME/logs. On a
  ...
  const openLogs = useCallback(async () => {
    try {
      const root = await window.marcelDesktop?.logsRoot?.()
      ...
      const result = await window.marcelDesktop?.openDir?.(root)
