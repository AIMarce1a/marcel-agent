...
describe('delivery command detection', () => {
  it('matches the canonical delivery command', () => {
    const cmd = 'marcel -p turqoise chat --in ~ -c "Bot Chat" -Q -q "Message from 🤖 Marcel (@marcel): hi there"'

    expect(deliveryTargetFromCommand(cmd)).toBe('turqoise')
  })

  it('matches with a cd prefix and timeout wrapper', () => {
    const cmd = 'cd ~ && timeout 240 marcel -p mr-tester chat --in "~" -Q -q "Message from 🤖 Marcel: hello"'
  ...
  it('ignores ordinary terminal commands', () => {
    ...
    expect(deliveryTargetFromCommand('marcel -p turqoise chat -q "plain question"')).toBeNull()
    expect(deliveryTargetFromCommand('marcel sessions list')).toBeNull()
  })
})
...
describe('reply extraction', () => {
  it('strips session_id bookkeeping and keeps the reply', () => {
    ...
    expect(replyTextFromResult({ output })).toBe('Hi Marcel! Good to hear from you.')
  })
