...
import type { MarcelReviewFile, MarcelReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Marcel' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'marcel.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'marcel.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'marcel.desktop.reviewTreeMode'
const SELECTED_KEY = 'marcel.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<MarcelReviewFile[]>([])
...
export const $reviewShipInfo = atom<MarcelReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['marcelDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: MarcelReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly MarcelReviewFile[], path: string): MarcelReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.marcelDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.marcelDesktop?.openExternal?.(url)
    }
