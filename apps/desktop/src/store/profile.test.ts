...
const { getProfiles } = await import('@/marcel')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/marcel/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<MarcelConnection> = {}): MarcelConnection =>
  ({ baseUrl: 'https://marcel-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as MarcelConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { marcelDesktop: { getConnection } })
