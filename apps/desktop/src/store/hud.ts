 ...
 * composer with the reply scrolling above it, so Marcel can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.marcelDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.marcelDesktop?.hud
...
export function resetHudLayout(): void {
  void window.marcelDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.marcelDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.marcelDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
