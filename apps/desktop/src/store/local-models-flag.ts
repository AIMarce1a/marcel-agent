 ...
 *
 * Local models ship on main behind `--local` (either `marcel desktop --local`
 * or the flag on Marcel.exe itself). The flag is strict: without it the GUI
...
export const $localModelsEnabled = atom<boolean>(
  typeof window !== 'undefined' && window.marcelDesktop?.localModelsEnabled === true
)
