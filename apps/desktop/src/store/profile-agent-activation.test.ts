...
const agentConn = (over: Partial<MarcelConnection> = {}): MarcelConnection =>
  ({ baseUrl: 'https://homelab.invalid', mode: 'remote', profile: 'research', ...over }) as MarcelConnection

const localConn = (over: Partial<MarcelConnection> = {}): MarcelConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as MarcelConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<MarcelConnection>>()

const getConnectionFor =
  vi.fn<(payload: { connectionId?: null | string; profile?: null | string }) => Promise<MarcelConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { marcelDesktop: { getConnection, getConnectionFor }, localStorage })
...
describe('ensureGatewayAgent commit hook (beforeActivate) — the Sessions switcher door (#93937)', () => {
  ...
  it('a descriptor lookup that never answers is bounded, fails open, and releases the mutex', async () => {
    ...
    try {
      getConnectionFor.mockImplementationOnce(() => new Promise<MarcelConnection>(() => undefined))
