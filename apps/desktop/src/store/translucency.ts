 ...
 * `@marcel/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-marcel-glass]`
 * block in styles.css).
 */
...
} from '@marcel/shared/translucency'
 ...
 * sets `marcelDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.marcelDesktop?.glassSupported === 'boolean'
    ? window.marcelDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.marcelDesktop?.translucencySupported === 'boolean'
    ? window.marcelDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-marcel-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-marcel-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-marcel-glass-on', glassLive)
  root.toggleAttribute('data-marcel-glass', glassOn)
  root.toggleAttribute('data-marcel-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-marcel-glass-scope', scope)
  } else {
    root.removeAttribute('data-marcel-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.marcelDesktop?.setTranslucency?.(state)
  })
