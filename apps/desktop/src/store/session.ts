import type { ConnectionState } from '@marcel/shared'
...
import type { SessionInfo, UsageStats } from '@/types/marcel'
...
const COMPOSER_MODEL_KEY = 'marcel.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'marcel.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'marcel.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'marcel.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'marcel.desktop.composer.fast'
...
function composerScopeForConnection(connection: MarcelConnection | null): string | null {
  if (!connection) {
    return null
  }
...
const LAST_SESSION_KEY = 'marcel.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'marcel.desktop.lastRoute'
...
function workspaceCwdKey(connection: MarcelConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(shouldPublish: () => boolean = () => true): Promise<string> {
  const settings = window.marcelDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(shouldPublish: () => boolean = () => true): Promise<void> {
  const sanitize = window.marcelDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<MarcelConnection | null>(null)
...
const SESSION_OWNER_HINTS_KEY = 'marcel.desktop.sessionOwnerHints.v1'
 ...
 * MarcelConnection lookup is unavailable. */
...
export const setConnection = (next: Updater<MarcelConnection | null>) => {
...
// until config loads, and re-seeded on every profile switch by useMarcelConfig.
