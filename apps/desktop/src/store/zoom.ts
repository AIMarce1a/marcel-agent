...
export function setZoomPercent(percent: number): void {
  window.marcelDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.marcelDesktop?.zoom) {
  void window.marcelDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.marcelDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
