import { type ConnectionState, type GatewayEvent, registryBackendScopeKey, resolveGatewayWsUrl } from '@marcel/shared'
import { atom } from 'nanostores'

import type { MarcelConnection } from '@/global'
import { MarcelGateway, setApiRequestConnection } from '@/marcel'
...
const isOpen = (gateway: MarcelGateway | null): boolean => gateway?.connectionState === 'open'
...
interface RegistryConfig {
  ...
  onActiveConnectionChanged?: (connection: MarcelConnection) => void
  /**
...
interface Secondary {
  ...
  connection: MarcelConnection | null
  gateway: MarcelGateway
...
interface GatewayRegistryState {
  ...
  primaryGateway: MarcelGateway | null
  ...
  $gateway: ReturnType<typeof atom<MarcelGateway | null>>
  $activeProfile: ReturnType<typeof atom<string>>
}

const STATE_KEY = Symbol.for('marcel.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<MarcelGateway | null>(null),
...
export function setPrimaryGateway(gateway: MarcelGateway | null, profile = 'default'): void {
...
export function setPrimaryGatewayConnection(connection: Pick<MarcelConnection, 'connectionId'> | null): void {
...
export function activeGateway(): MarcelGateway | null {
  if (g.activeKey === g.primaryProfile) {
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the marcel module (null
...
function publishActiveConnection(connection: MarcelConnection): void {
...
function createSecondary(profile: string, connectionId: null | string = null): Secondary {
  const gateway = new MarcelGateway()
...
async function gatewayForProfile(
  profile: string,
  leaseRequest = false
): Promise<{ gateway: MarcelGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
  const key = normKey(profile)
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`Marcel gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.marcelDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update Marcel Desktop.')
  }
...
export async function ensureActiveGatewayOpen(): Promise<MarcelGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "Marcel gateway is not connected" on the Sessions `+` action (#88880).
