...
import type { MarcelGitWorktree, MarcelRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: MarcelGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, MarcelRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, MarcelGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<MarcelRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<MarcelGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<MarcelRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<MarcelGitWorktree[]>>()
const $noRepoStatus = atom<MarcelRepoStatus | null>(null)
const $noWorktrees = atom<MarcelGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<MarcelRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<MarcelGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: MarcelRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: MarcelGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<MarcelRepoStatus | null>
  seq: number
  target: string
}
