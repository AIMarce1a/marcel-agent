...
/**
 * Feature store for backend (agent) plugins — the native Marcel plugins plus
 ...
 * primitives `marcel plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native Marcel. */
  portable?: boolean
}
 ...
 *  MARCEL_HOME. Always refetches (it's a cheap local disk scan on the
