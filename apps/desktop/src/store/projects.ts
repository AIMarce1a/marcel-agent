...
import type { MarcelGitBaseBranch, MarcelGitBranch } from '@/global'
import { getMarcelConfig, marcelApi, type MarcelGateway } from '@/marcel'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/marcel'
...
const PROJECT_SCOPE_KEY = 'marcel.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Marcel gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== projectProfile()) {
    throw new Error('Active Marcel profile changed while connecting')
  }
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await marcelApi<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<MarcelGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<MarcelGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  if (isDesktopFsRemoteMode()) {
    ...
    // with `scan: true` — added in #81723) so repos with zero Marcel
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getMarcelConfig(context.profile))
...
// repo, returning where Marcel should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<MarcelGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<MarcelGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.marcelDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.marcelDesktop?.writeClipboard?.(path)
  }
}
