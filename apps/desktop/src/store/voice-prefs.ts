...
import { getMarcelConfigRecord, saveMarcelConfig } from '@/marcel'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getMarcelConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveMarcelConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
