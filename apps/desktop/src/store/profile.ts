import { LOCAL_CONNECTION_ID } from '@marcel/shared'
...
import type { ProfileInfo } from '@/types/marcel'
...
// /api/profiles/active `current`). "default" is the root ~/.marcel. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'marcel.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'marcel.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await marcelApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new MARCEL_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.marcelDesktop.profile.set(name)
}
...
async function resolveConnectionForProfile(profile: string): Promise<MarcelConnection | null> {
  const getConnection = window.marcelDesktop?.getConnection
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<MarcelConnection | null> {
  const getConnectionFor = window.marcelDesktop?.getConnectionFor
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'marcel.desktop.showAllProfiles'
...
export function selectProfile(name: string): void {
  ...
  void Promise.all([activateOnCurrentSource(target), shouldRememberStartupProfile])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
        return window.marcelDesktop?.profile?.remember(target)
      }
...
async function isLocalDesktopProfile(target: string): Promise<boolean> {
  const getConnectionConfig = window.marcelDesktop?.getConnectionConfig
...
// Switch to the default (root ~/.marcel) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.marcelDesktop?.touchBackend?.(target).catch(() => undefined)
}
