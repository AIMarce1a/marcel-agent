...
import type { MarcelBranchPullRequest } from '@/global'
import { desktopGit } from '@/lib/desktop-git'
import { Codecs, persistentAtom } from '@/lib/persisted'
import { scanSessionPullRequests, type SessionInfo } from '@/marcel'
...
export const $pullRequestsByBranch = atom<Record<string, MarcelBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'marcel.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('marcel.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: MarcelBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
