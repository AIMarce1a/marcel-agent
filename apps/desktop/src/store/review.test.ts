...
// reaches the gateway); everything else routes through window.marcelDesktop.git,
...
function file(path: string, over: Partial<MarcelReviewFile> = {}): MarcelReviewFile {
  return { path, status: 'modified', staged: false, added: 1, removed: 0, ...over } as MarcelReviewFile
}

type ReviewStub = Record<string, ReturnType<typeof vi.fn>>

// Install a review bridge on window.marcelDesktop. Any op not supplied defaults
...
describe('ship flow', () => {
  ...
  it('createOrOpenPr opens the existing PR without creating a new one', async () => {
    const review = stubReview()
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'https://example.com/pr/9' } } as MarcelReviewShipInfo)

    await createOrOpenPr()

    expect(review.createPr).not.toHaveBeenCalled()
    expect(
      (window.marcelDesktop as unknown as { openExternal: ReturnType<typeof vi.fn> }).openExternal
...
describe('refreshShipInfo', () => {
  it('populates ship info from the bridge', async () => {
    const info: MarcelReviewShipInfo = {
      ghReady: true,
      pr: { url: 'https://example.com/pr/3' }
    } as MarcelReviewShipInfo
  ...
  it('resets ship info when there is no bridge', async () => {
    delete (window as unknown as { marcelDesktop?: unknown }).marcelDesktop
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'x' } } as MarcelReviewShipInfo)
