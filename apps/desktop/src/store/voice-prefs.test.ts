...
vi.mock('@/marcel', () => ({
  getMarcelConfigRecord: vi.fn(async () => ({})),
  saveMarcelConfig: vi.fn(async () => undefined)
}))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye marcel', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye marcel')
  })
