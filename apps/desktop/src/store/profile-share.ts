 ...
 *
 * The archive is the CLI's own `marcel profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/marcel'
...
import type { ProfileDesktopOverlay } from '@/types/marcel'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Marcel profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.marcelDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.marcelDesktop?.selectPaths?.({
