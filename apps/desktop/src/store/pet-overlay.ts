 ...
 * app's bounds and stays visible while Marcel is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'marcel.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'marcel.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.marcelDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.marcelDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.marcelDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.marcelDesktop?.petOverlay?.close()
}
