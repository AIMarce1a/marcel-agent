 ...
 *  Working in `~/www/marcel-agent`, the draft says "marcel-agent" constantly
 *  — a "Use skill: marcel-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`marcel-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/marcel-agent-dev`, `plugin:skill`). */
