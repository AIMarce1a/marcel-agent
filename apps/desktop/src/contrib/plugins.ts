 ...
 *    a `MarcelPlugin` registers automatically (vite glob — drop a folder in).
 *    `marcel-bots` (Bot Mode) ships in-tree and is ON by default; other
 ...
 *  - RUNTIME: the on-disk doors (`<marcel home>/desktop-plugins/<name>/plugin.js`
 *    and the unified-package half `<marcel home>/plugins/<name>/desktop/plugin.js`)
...
const modules = import.meta.glob<{ default: MarcelPlugin }>('../plugins/*/plugin.{js,ts,tsx}', { eager: true })
...
export function discoverBundledPlugins(): void {
  ...
  for (const [path, mod] of Object.entries(modules)) {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      console.warn(`[plugins] ${path} has no valid default MarcelPlugin export — skipped`)
