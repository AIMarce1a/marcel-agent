import type { GatewayWsUrlResult } from '@marcel/shared'
import type { TranslucencyState } from '@marcel/shared/translucency'
...
declare global {
  interface Window {
    marcelDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<MarcelConnection>
      ...
      // Resume this session in the user's own terminal emulator (`marcel --tui
      ...
      // Marcel Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new MARCEL_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: MarcelApiRequest) => Promise<T>
      notify: (payload: MarcelNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<MarcelReadFileTextResult>
      ...
      readPluginSource?: (filePath: string) => Promise<MarcelReadFileTextResult>
      selectPaths: (options?: MarcelSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<MarcelPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<MarcelPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<MarcelPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: MarcelActiveWork) => void
      setTitleBarTheme?: (payload: MarcelTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<MarcelReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<MARCEL_HOME>/desktop-plugins),
      ...
      /** LOCAL `<MARCEL_HOME>/logs` (profile-aware) — error card "Open Logs". */
      logsRoot?: () => Promise<string>
      // Local AGENT-plugin root (<MARCEL_HOME>/plugins), same Electron-local
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<MarcelGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<MarcelGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<MarcelGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<MarcelRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: MarcelReviewScope, baseRef?: null | string) => Promise<MarcelReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: MarcelReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<MarcelReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<MarcelRepoPullRequests>
          ...
          fetchPrComment: (repoPath: string, url: string) => Promise<MarcelPrComment | null>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: MarcelTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<MarcelTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: MarcelWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: MarcelPreviewFileChanged) => void) => () => void
...
export interface MarcelTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface MarcelTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `marcel update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  marcelRoot?: string
...
export interface MarcelConnection {
  baseUrl: string
  darwinMajor?: number
...
export interface MarcelTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface MarcelActiveWork {
  count: number
  titles: string[]
}

export interface MarcelWindowState {
  darwinMajor?: number
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Marcel Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Marcel Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Marcel Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the marcel gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Marcel Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface MarcelApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface MarcelNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface MarcelPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface MarcelReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface MarcelPreviewWatch {
  id: string
  path: string
}
...
export interface MarcelGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface MarcelGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface MarcelGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface MarcelRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface MarcelRepoStatus {
  branch: null | string
...
// turn began.
export type MarcelReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface MarcelReviewFile {
  path: string
  added: number
  removed: number
...
export interface MarcelReviewList {
  files: MarcelReviewFile[]
...
export interface MarcelReviewPr {
  url: string
  state: string
  number: number
}
...
export interface MarcelBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface MarcelRepoPullRequests {
  ghReady: boolean
  prs: MarcelBranchPullRequest[]
}
...
export interface MarcelPrComment {
  author: string
  body: string
  diffHunk: string
  kind: 'issue' | 'review'
...
export interface MarcelReviewShipInfo {
  ghReady: boolean
  pr: MarcelReviewPr | null
}

export interface MarcelReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface MarcelReadDirResult {
  entries: MarcelReadDirEntry[]
  error?: string
}

export interface MarcelPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface MarcelSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
