...
/// desktop app handing off via `Marcel-Setup.exe --update`).
///
...
pub fn run() {
    // Tracing → bootstrap-installer.log under MARCEL_HOME/logs/ so install
    ...
    // Marcel is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "Marcel installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Marcel is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "Marcel" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "Marcel" shortcuts that install.ps1 creates, and a
            // reliable detached relaunch there needs the DETACHED_PROCESS +
            // startup-grace handling used by launch_marcel_desktop — out of
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::marcel_home().join("marcel-agent");
                if bootstrap::marcel_is_installed(&install_root) {
                    match bootstrap::spawn_installed_desktop(&install_root) {
                        Ok(()) => {
                            // Brief grace so the spawned app is registered
                            // before we exit (mirrors launch_marcel_desktop).
                            std::thread::sleep(std::time::Duration::from_millis(200));
                            tracing::info!(
                                "Marcel already installed — relaunched desktop; exiting installer"
                            );
                            app.handle().exit(0);
                            return Ok(());
                        }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            update::start_update,
            // Hand-off
            bootstrap::launch_marcel_desktop,
            // Diagnostics
            paths::get_log_path,
            paths::get_marcel_home,
            paths::open_log_dir,
        ])
        .run(tauri::generate_context!())
        .expect("error while running Marcel Setup");
}
