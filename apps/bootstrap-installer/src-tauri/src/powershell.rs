...
/// read side does not see EOF until the last of them closes it. `marcel update`
...
/// stranded with it.
///
/// Same bound `Invoke-MarcelStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `marcel_home_override` propagates to the child as $MARCEL_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, marcel_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = marcel_home_override {
        cmd.env("MARCEL_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, marcel_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = marcel_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_marcel_home() {
    ...
    /// child. `marcel update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
