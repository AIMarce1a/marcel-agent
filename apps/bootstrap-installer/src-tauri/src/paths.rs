...
//!
//! Mirrors `marcel_constants.get_marcel_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\marcel
//!   macOS:   ~/.marcel
//!   Linux:   ~/.marcel  (override via $MARCEL_HOME)
//!
//! NOTE (macOS): Python's get_marcel_home(), scripts/install.sh, and the
//! Electron desktop's resolveMarcelHome() ALL use ~/.marcel on macOS — there
...
/// Returns the canonical Marcel home directory, respecting $MARCEL_HOME if set.
pub fn marcel_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("MARCEL_HOME") {
    ...
    {
        // %LOCALAPPDATA%\marcel — matches scripts/install.ps1's $MarcelHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("marcel");
        }
    }

    // macOS + Linux + fallback: ~/.marcel (matches Python get_marcel_home(),
    // install.sh, and the Electron desktop's resolveMarcelHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".marcel");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    marcel_home().join("bootstrap-cache")
}
...
/// MARCEL_HOME so it survives repo checkout deletion (unlike anything under
/// marcel-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\marcel\marcel-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_marcel`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under MARCEL_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves MARCEL_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    marcel_home().join(".marcel-update-in-progress")
}
...
pub fn copy_self_to_marcel_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to MARCEL_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_MARCEL_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".marcel-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under MARCEL_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[marcel-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("MARCEL_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_marcel_home() -> String {
    marcel_home().to_string_lossy().into_owned()
}
