...
export const $marcelHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.marcel/logs/bootstrap-installer.log')
    $marcelHome.set('~/.marcel')
  ...
  try {
    const [logPath, marcelHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_marcel_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $marcelHome.set(marcelHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Marcel" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Marcel-Setup.exe --update);
...
export async function launchMarcelDesktop(): Promise<void> {
  ...
  await invoke('launch_marcel_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Marcel repository' },
