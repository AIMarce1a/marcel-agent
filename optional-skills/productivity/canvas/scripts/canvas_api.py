#!/usr/bin/env python3
"""Canvas LMS API CLI for Marcel Agent.
...
def _check_config():
    ...
    if missing:
        marcel_env = os.path.join(
            os.environ.get("MARCEL_HOME", os.path.expanduser("~/.marcel")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {marcel_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Marcel Agent"
    )
