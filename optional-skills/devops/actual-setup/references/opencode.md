...
is typically only in `~/.marcel/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike Marcel,
...
`marcel-custom-providers` skill, pitfall 2). Marcel hits this because it forwards
