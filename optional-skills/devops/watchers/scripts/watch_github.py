...
Usage (via cron with --no-agent):
    ...
    marcel cron create marcel-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$MARCEL_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name marcel-issues --repo NousResearch/hermes-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Marcel .env file
(``${MARCEL_HOME:-~/.marcel}/.env``) to avoid the 60 req/hr
