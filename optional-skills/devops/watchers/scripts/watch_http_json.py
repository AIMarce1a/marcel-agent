...
Usage (via cron with --no-agent):
    ...
    marcel cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$MARCEL_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Marcel-Watcher/1.0"})
