...
All per-subject data lives under PDD_DATA_DIR (default: $MARCEL_HOME/unbroker),
which is the same trust boundary Marcel uses for .env and OAuth tokens.
"""
...
def marcel_home() -> Path:
    return Path(os.environ.get("MARCEL_HOME") or (Path.home() / ".marcel"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else marcel_home() / "unbroker"
