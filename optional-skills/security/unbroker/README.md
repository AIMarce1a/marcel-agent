...
The design is **Marcel-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
marcel skills install official/security/unbroker
```

Then start a new Marcel session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Marcel session:
...
- **PII handling.** Dossiers live under the Marcel home directory (`0600`, optionally
