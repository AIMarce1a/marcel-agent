...
## 5. GODMODE FAST — Marcel 4 405B (Zero Refusal)

**Model:** `nousresearch/marcel-4-405b`
**Strategy:** Marcel models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Marcel Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.marcel/config.yaml`:
