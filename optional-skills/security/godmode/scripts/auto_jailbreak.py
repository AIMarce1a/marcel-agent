...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("MARCEL_HOME", os.path.expanduser("~/.marcel")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("MARCEL_HOME", Path.home() / ".marcel")) / "skills" / "red-teaming" / "godmode"
...
MARCEL_HOME = Path(os.getenv("MARCEL_HOME", Path.home() / ".marcel"))
CONFIG_PATH = MARCEL_HOME / "config.yaml"
PREFILL_PATH = MARCEL_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Marcel models — already uncensored, just needs clean prompt
    "marcel": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "marcel" in model_lower or "nous" in model_lower:
        return "marcel"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Marcel config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.marcel/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Marcel for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Marcel for changes to take effect.")
