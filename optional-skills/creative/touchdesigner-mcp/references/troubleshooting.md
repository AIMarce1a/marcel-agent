...
## 5. Marcel Configuration

### Config location

`$MARCEL_HOME/config.yaml` (defaults to `~/.marcel/config.yaml` when `MARCEL_HOME` is unset)
...
Restart the Marcel session for changes to take effect. The MCP connection is
