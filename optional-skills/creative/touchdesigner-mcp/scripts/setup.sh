...
MARCEL_HOME_DIR="${MARCEL_HOME:-$HOME/.marcel}"
MARCEL_CFG="${MARCEL_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Marcel config has twozero_td MCP entry ──
if [[ ! -f "$MARCEL_CFG" ]]; then
    echo -e " ${FAIL} Marcel config not found at ${MARCEL_CFG}"
    manual_steps+=("Create ${MARCEL_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$MARCEL_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Marcel config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Marcel config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${MARCEL_CFG} manually"); }
    manual_steps+=("Restart Marcel session to pick up config change")
fi
