...
### 1. Start order: editor first, Marcel second

Marcel probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Marcel session.
...
restart the editor, restart the Marcel session.
...
server fails to bind (Output Log) or Marcel' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.marcel/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Marcel

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Marcel' connection
lives in `~/.marcel/config.yaml` via `marcel mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Marcel — just don't
mistake it for the Marcel setup step.
...
explicitly warns against overlapping calls. Marcel executes same-turn tool
...
### 9. Timeouts: Marcel gives up before Unreal does

Marcel' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Marcel runs elsewhere (SSH backend,
