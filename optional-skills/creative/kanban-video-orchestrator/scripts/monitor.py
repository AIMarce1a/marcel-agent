#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `marcel kanban list` and
...
def marcel_available() -> bool:
    return shutil.which("marcel") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed marcel CLI."""
    try:
        out = subprocess.run(
            ["marcel", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `marcel kanban list`
    out = subprocess.run(
        ["marcel", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["marcel", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not marcel_available():
        print("ERROR: 'marcel' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
