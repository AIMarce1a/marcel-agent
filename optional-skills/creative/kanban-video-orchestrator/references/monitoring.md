...
marcel kanban watch --tenant <project-slug>

# Snapshot of the board
marcel kanban list --tenant <project-slug>
marcel kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
marcel kanban stats --tenant <project-slug>

# Visual dashboard (browser)
marcel dashboard

# Inspect a specific task (includes comments + events)
marcel kanban show <task-id>

# Follow a single task's event stream
marcel kanban tail <task-id>
```

Verify available subcommands with `marcel kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `marcel kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `marcel kanban block <id>` then `marcel kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `marcel kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `marcel kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `marcel profile list` (profile exists?), `marcel status` (gateway/dispatcher up?), and verify tenant. |
...
marcel kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
marcel kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(marcel kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
marcel kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
marcel kanban block <task-id>
marcel kanban archive <task-id>

# Diagnose what happened
marcel kanban show <task-id>      # task body, comments, recent events
marcel kanban tail <task-id>      # follow the live event stream
marcel kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `marcel kanban create` for the director
...
while true; do
    clear
    marcel kanban list --tenant <slug>
    echo "---"
    marcel kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `marcel kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `marcel profile list` and start
