...
the actual `setup.sh` that creates the project workspace, configures Marcel
...
2. **Create profiles** — `marcel profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.marcel/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `marcel kanban create` assigned to the director
...
```bash
marcel profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.marcel/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.marcel/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Marcel
...
Each profile gets a `SOUL.md` at `~/.marcel/profiles/<name>/SOUL.md` that
...
```bash
marcel kanban create "Direct production of <video title>" \
    --assignee director \
...
the Marcel `.env` (`${MARCEL_HOME:-$HOME/.marcel}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _marcel_env="${MARCEL_HOME:-$HOME/.marcel}/.env"
    if grep -q "^${var}=" "$_marcel_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_marcel_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_marcel_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY marcel ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY marcel OPENROUTER_API_KEY || exit 1
# ...
```
