...
Maps each role archetype to the Marcel skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public marcel-agent
...
## Marcel skills relevant to video production

### Visual / rendering skills (`marcel-agent/skills/creative/`)
...
### Design / pre-production skills (`marcel-agent/skills/creative/`)
...
### Audio / media skills (`marcel-agent/skills/creative/` + `skills/media/`)
...
These are **not** Marcel skills but external CLIs / APIs that profiles invoke.
...
## Built-in Marcel tools for media review

These are native Marcel tools — not invoked via terminal but through their own
...
key is present in `${MARCEL_HOME:-~/.marcel}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Marcel profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${MARCEL_HOME:-~/.marcel}/.env` → environment variable.
...
When a new Marcel-public video skill ships:
