...
Usage:
  ...
  <server_name> matches the files in $MARCEL_HOME/mcp-tokens/<server>.json etc.
...
def _marcel_home():
    # Prefer Marcel' own resolver (profile-safe); fall back to env then ~/.marcel.
    try:
        from marcel_constants import get_marcel_home
        return str(get_marcel_home())
    except Exception:
        return os.environ.get("MARCEL_HOME") or os.path.expanduser("~/.marcel")


def _tokens_dir():
    return os.path.join(_marcel_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "marcel-diag", "version": "1.0"}},
        },
        headers={
