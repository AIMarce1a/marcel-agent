...
VERSION_FILE = REPO_ROOT / "marcel_cli" / "__init__.py"
...
LEGACY_AUTHOR_MAP = {
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level MARCEL_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (marcel journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.marcel/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows marcel-shim resolution + prefer --update on recovery; #52378)
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on marcel mcp add)
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (MARCEL_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (MARCEL_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: marcel-eval community link)
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Marcel runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level MARCEL_HOME state files)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Marcel
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/AdMind-ai/marcel-agent",
    ...
    lines.append(f"# Marcel Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Marcel Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Marcel Agent Release Tool")
    ...
    print(f"{'='*60}")
    print("  Marcel Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Marcel Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Marcel Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Marcel Agent v{new_version} ({calver_date})' "
