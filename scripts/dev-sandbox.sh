...
INSTALL_REF=""
UPSTREAM_URL="${MARCEL_DEV_SANDBOX_UPSTREAM:-https://github.com/AIMarce1a/marcel-agent.git}"
...
if [ "$INSTALL_SHORTCUT" = true ]; then
  ...
  set -- bash -c '
    set +e
    curl -fsSL https://raw.githubusercontent.com/AIMarce1a/marcel-agent/main/scripts/install.sh | bash -s -- "$@"
    install_status=$?
