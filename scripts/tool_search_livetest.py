#!/usr/bin/env python3
"""Live test harness for Marcel Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any marcel imports.
ORIGINAL_HOME = os.environ.get("MARCEL_HOME")
ORIGINAL_AUTH = Path.home() / ".marcel" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.marcel/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.marcel/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="marcel_ts_live_"))
    marcel_home = home_dir / ".marcel"
    marcel_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, marcel_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".marcel" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, marcel_home / ".env")
        ...
        load_marcel_dotenv(marcel_home=str(Path.home() / ".marcel"))
    ...
    (marcel_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return marcel_home
...
def reset_module_state():
    """Drop cached modules so the new MARCEL_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "marcel_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["MARCEL_HOME"] = str(home)
...
def main():
    ...
    # Restore original MARCEL_HOME
    if ORIGINAL_HOME is not None:
        os.environ["MARCEL_HOME"] = ORIGINAL_HOME
    else:
