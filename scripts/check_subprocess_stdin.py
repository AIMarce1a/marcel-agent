...
When Marcel runs in TUI mode, the gateway child process communicates with
...
# ``get_marcel_home() / "plugins"`` (user) and ``./.marcel/plugins/`` (project,
# gated behind ``MARCEL_ENABLE_PROJECT_PLUGINS``) — see
# ``marcel_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``marcel_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import marcel_constants (this script
    ...
    # plugins in get_marcel_home()/plugins/ and project plugins in
    # ./.marcel/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_marcel_home() / "plugins"]
    if os.environ.get("MARCEL_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".marcel" / "plugins")
