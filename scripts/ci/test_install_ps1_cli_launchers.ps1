# Behavioral test for install.ps1's marcel launcher staging (PR #92092,
...
#
# The test lifts the real Install-MarcelCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $MarcelHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-MarcelCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-MarcelCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'marcel-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'marcel-agent'
    ...
    $missingThrew = $false
    try {
        Install-MarcelCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing marcel.exe fails the launcher stage'
    ...
    $marcelV1 = [byte[]](77, 90, 1)
    $marcelV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'marcel.exe'), $marcelV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-MarcelCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $marcelV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'marcel.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'marcel-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'marcel.exe'), $marcelV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'marcel-acp.exe'), $acp)
    Install-MarcelCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $marcelV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'marcel.exe'))) `
        'installer refreshes an existing Marcel launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'marcel-acp.exe'))) `
    ...
    Install-MarcelCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'marcel.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'marcel.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'marcel.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'marcel.exe')) -and $cmdBody.Contains('%*')) `
