...
# tree generates llms.txt — the index every LLM (Marcel included, via the
# marcel-agent skill) reads to learn what Marcel can do — and
...
_MCP_CATALOG_FILES = {"marcel_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Marcel-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``marcel serve`` backend, the Docker image)
