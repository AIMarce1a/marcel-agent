"""Run a real Marcel CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_marcel_executable(marcel_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "marcel",
        Path(".venv") / "Scripts" / "marcel.exe",
    ):
        candidate = marcel_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("marcel")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Marcel executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Marcel source checkout containing .venv/bin/marcel",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated MARCEL_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["marcel.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['marcel.client.active']}"
        )
    [model] = by_name["marcel.model_route.count"]
    expected_model = {
        "name": "marcel.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['marcel.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["marcel.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['marcel.task_run.started']}"
        )
    [terminal] = by_name["marcel.task_run.finished"]
    ...
    [tool] = by_name["marcel.tool_call.count"]
    ...
    lifecycle = by_name["marcel.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["marcel.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Marcel development environment requires jsonschema"
        ) from exc
    ...
    if metrics["marcel.client.active"] != [
        {
            "name": "marcel.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['marcel.client.active']}"
        )
    [model] = metrics["marcel.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['marcel.model_route.count']}"
        )
    [terminal] = metrics["marcel.task_run.finished"]
    ...
    [tool] = metrics["marcel.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["marcel.skill.lifecycle.count"]
    ...
    loads = metrics["marcel.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    marcel_repo = args.marcel_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    marcel = _resolve_marcel_executable(marcel_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="marcel-relay-shared-metrics-"))
    home = root / "marcel-home"
    ...
    try:
        ...
        env = os.environ.copy()
        env["MARCEL_HOME"] = str(home)
        python_paths = [str(marcel_repo)]
    ...
    (root / "marcel.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "marcel.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Marcel exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Marcel model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Marcel did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Marcel did not print the mock model response")
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "marcel.shared_metrics.v2.schema.json",
    )

    print("Marcel -> NeMo Relay shared-metrics smoke test passed")
