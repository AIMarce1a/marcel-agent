...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[marcel] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[marcel`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[marcel`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        MarcelHome = $(if ($paths) { $paths.marcel_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\marcel\marcel-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-marcel'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-marcel') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
