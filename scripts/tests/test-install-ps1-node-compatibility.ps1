...
# changing PATH, installing software, or touching the user's Marcel home.

$repoRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path))
$installScript = Join-Path $repoRoot 'scripts\install.ps1'
$testRoot = Join-Path $env:TEMP ("marcel-node-compatibility-test-" + [Guid]::NewGuid().ToString('N'))
$MarcelHome = Join-Path $testRoot 'home'
$InstallDir = Join-Path $testRoot 'missing-checkout'
. $installScript -MarcelHome $MarcelHome -InstallDir $InstallDir
