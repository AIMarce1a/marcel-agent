...
measured in the production session DB) through `marcel chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    MARCEL_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Marcel home with credentials for the
...
""".strip(),
            ...
            env.update({
                ...
                "MARCEL_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "marcel_cli.main", "chat", "--query", q,
