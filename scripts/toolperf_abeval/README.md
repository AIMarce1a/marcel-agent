...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Marcel home, same model, same tasks, same reps.
...
1. Create a dedicated Marcel home with credentials for the models under test:
   ...
   integration at it; no Marcel observability plugin needs to be enabled.
